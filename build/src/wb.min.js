...
this.axhr.get(b.pdf).then(a=>{virtualclass.pdfRender[virtualclass.gObj.currWb].afterPdfPrefetch(b.id,a.data)}).catch(a=>{console.error("Request failed with error ",a)})},afterPdfPrefetch(a,b){virtualclass.gObj.next={};virtualclass.gObj.next[a]=b},async loadPdf(a,b,c){virtualclass.gObj.hasOwnProperty("getDocumentTimeout")&&clearTimeout(virtualclass.gObj.getDocumentTimeout);null==virtualclass.gObj.getDocumentTimer||!1===virtualclass.gObj.getDocumentTimer?(this.loadPdfActual(a,b,c),virtualclass.gObj.getDocumentTimer=
!0,virtualclass.gObj.getDocumentTimeout=setTimeout(()=>{virtualclass.gObj.getDocumentTimer=!1},1E3)):virtualclass.gObj.getDocumentTimeout=setTimeout(()=>{this.loadPdfActual(a,b,c);virtualclass.gObj.getDocumentTimer=!1},300)},loadPdfActual(a,b,c){virtualclass.gObj.next.hasOwnProperty(c)?this.afterPdfLoad(b,c,virtualclass.gObj.next[c]):this.axhr.get(a).then(a=>{virtualclass.pdfRender[virtualclass.gObj.currWb].afterPdfLoad(b,c,a.data)}).catch(d=>{console.error("Request failed with error ",d);setTimeout(()=>
{virtualclass.pdfRender[virtualclass.gObj.currWb].loadPdfActual(a,b,c)},1E3)})},async afterPdfLoad(a,b,c){this.canvasWrapper=document.querySelector(`#canvasWrapper${virtualclass.gObj.currWb}`);this.canvas=a;a={};a.data=c;null!=virtualclass.gObj.myworker&&(a.worker=virtualclass.gObj.myworker);const d=this;c=virtualclass.gObj.currWb;virtualclass.currApp==virtualclass.currApp?(d.wbId="undefined"===typeof b?virtualclass.gObj.currWb:b,console.log(`-----------START ${virtualclass.currApp}----------`),console.log("PDF render request to pdf.js 1"),
PDFJS.workerSrc=`${whiteboardPath}build/src/pdf.worker.min.js`,PDFJS.cMapUrl=`${whiteboardPath}build/cmaps/`,PDFJS.cMapPacked=!0,PDFJS.getDocument(a).then(a=>{null==virtualclass.gObj.myworker&&(virtualclass.gObj.myworker=a.loadingTask._worker);d.displayPage(a,1,()=>{},!0);d.shownPdf=a;roles.hasControls()||(d.topPosY=0,d.leftPosX=0);d.scrollEvent()})):"DocumentShare"==virtualclass.currApp&&virtualclass.wbCommon.deleteWhiteboard(c);"undefined"!==typeof virtualclass.gObj.currWb&&null!=virtualclass.gObj.currWb&&
(b=document.querySelector(`#note${b}`),null!=b&&null!=b.nextElementSibling&&virtualclass.pdfRender[virtualclass.gObj.currWb].prefechPdf(b.nextElementSibling.dataset.slide))},updateScrollPosition(a,b){"object"===typeof this.scroll[b]&&this.scroll[b].hasOwnProperty("b")?(this.scroll[b].b=a,this.scroll[b].c=this.scroll[b].b+this.scroll[b].studentVPm):console.log("Scroll b is undefined")},scrollEvent(){const a=this.canvasWrapper,b=a.scrollTop,c=a.scrollLeft;virtualclass.topPosY=b;virtualclass.leftPosX=
c;this.topPosY=b;this.leftPosX=c;const d=this;a.onscroll=function(){d.onScroll(a)}},onScroll(a,b){var c=a.scrollTop;var d=a.scrollLeft;(0<c||"undefined"!==typeof b)&&this._scroll(d,c,a,"Y");(0<d||"undefined"!==typeof b)&&this._scroll(d,c,a,"X");roles.hasControls()||virtualclass.pdfRender[virtualclass.gObj.currWb].setScrollPosition({scX:d,scY:c})},_scroll(a,b,c,d){if(roles.hasControls())return this.topPosY=b,this.leftPosX=a,this.scrollPosition(c,d);if("Y"==d)var f=b;else"X"==d&&(f=a);this.updateScrollPosition(f,
d);return null},scrollPosition(a,b){const {canvas:c}=this;if("Y"==b)var d=a.scrollTop,k=c.height;else"X"==b&&(d=a.scrollLeft,k=c.width);this[`scrollPos${b}`]=d/k*100;a=virtualclass.vutil.getElemM(`canvasWrapper${virtualclass.gObj.currWb}`,b);this[`actualVp${b}`]=a;this[`viewPort${b}`]=a/k*100;k={};k[`vp${b}`]=this[`viewPort${b}`];k[`sc${b}`]=this[`scrollPos${b}`];return k},scroll:{caclculatePosition(a,b,c){a=this.type=c;null==this[a]&&(this[a]={});this[a].a=0;this[a].d=b;b=`canvasWrapper${virtualclass.gObj.currWb}`;
c=document.querySelector(`#${b}`);null!=c&&("X"==this.type?this[a].b=c.scrollLeft:"Y"==this.type&&(this[a].b=c.scrollTop),this[a].studentVPm=virtualclass.vutil.getElemM(b,a),this[a].c=this[a].b+this[a].studentVPm)}},setScrollPosition(a){a.hasOwnProperty("scY")&&null!=a.scY&&this.scroll.caclculatePosition(a.scY,this.canvas.height,"Y");a.hasOwnProperty("scX")&&null!=a.scX&&this.scroll.caclculatePosition(a.scX,this.canvas.width,"X")},customMoustPointer(a,b,c){this.scroll[b].e=c;this.scroll[b].e>this.scroll[b].c?
(a=this.scroll[b].b+(this.scroll[b].d-this.scroll[b].c),a>this.scroll[b].e&&(a=this.scroll[b].e-(this.scroll[b].b+this.scroll[b].c)/2),console.log(`custom mouse down pointer ay=${this.scroll[b].a} by=${this.scroll[b].b} cy=${this.scroll[b].c} dy=${this.scroll[b].d} ey${this.scroll[b].e} scrollPos=${a}`),"Y"==b?this.canvasWrapper.scrollTop=a:(this.canvasWrapper.scrollLeft=a,console.log(`Scroll left ${this.canvasWrapper.scrollLeft}`)),this.scroll[b].b=a,this.scroll[b].c=this.scroll[b].b+this.scroll[b].studentVPm):
this.scroll[b].e<this.scroll[b].b&&(a=this.scroll[b].b-this.scroll[b].a,this.scroll[b].c-a<this.scroll[b].e&&(a=(this.scroll[b].b+this.scroll[b].c)/2-this.scroll[b].e),"Y"==b?this.canvasWrapper.scrollTop-=a:this.canvasWrapper.scrollLeft-=a,this.scroll[b].b=a,this.scroll[b].c=this.scroll[b].b+this.scroll[b].studentVPm)},setCustomMoustPointer(a,b){var c=`scrollDiv${b}${virtualclass.gObj.currWb}`,d=document.querySelector(`#${c}mousePointer`);null==d&&(d=document.createElement("div"),d.className="mousepointer",
d.id=`${c}mousePointer`,c=document.querySelector(`#scrollDiv${b}${virtualclass.gObj.currWb}`),null!=c&&c.appendChild(d));"Y"==b?d.style.top=`${a.y-this.scroll[b].a}px`:"X"==b&&(d.style.left=`${a.x-this.scroll[b].a}px`)},sendScroll(){virtualclass.vutil.setDefaultScroll()},sendCurrentScroll(a){let b={};if(null!=this.currentScroll){b=Object.assign(b,this.currentScroll);console.log(`Send scroll first time ${this.currentScroll}`);const c=this;c.currentScrolltoUser=a;setTimeout(()=>{c.currentScrolltoUser=
a;b.cf="scf";b.toUser=a;virtualclass.vutil.beforeSend({toUser:a,cf:"scf",scY:b.scY,vpY:b.vpY},a);console.log(`Send scroll ${b}to user ${a}`);console.log(`Send scroll ${b}`)},2E3)}},async renderPage(a,b){console.log("#### render page");if(null!=virtualclass.gObj.currWb){var f=this.pdfScale;null!=virtualclass.zoom.canvasScale&&""!=virtualclass.zoom.canvasScale&&(0<virtualclass.zoom.canvasScale?f=virtualclass.zoom.canvasScale:console.log("Why negative value"));var g="Whiteboard"==virtualclass.currApp&&
null!=this.wbId&&virtualclass.gObj.currWb!=this.wbId?-1<this.wbId.indexOf("_doc_")?this.wbId:`_doc_${this.wbId}_${this.wbId}`:virtualclass.gObj.currWb;const {canvas:k}=virtualclass.wb[g].vcan.main;virtualclass.gObj.hasOwnProperty("fitToScreen")?(k.width=d.innerWidth-virtualclass.zoom.getReduceValueForCanvas(),console.log("==== a canvas width fit to screen"),roles.hasControls()||(k.width+=50)):virtualclass.zoom.hasOwnProperty("performZoom")?(k.width=virtualclass.zoom.canvasDimension.width,delete virtualclass.zoom.performZoom):
