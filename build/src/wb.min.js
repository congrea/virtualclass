...
this.chunks.length=0);this.mousedown=!1}else{this.innerMouseUp(c,b,!0);if(!a.e.isTrusted)return;virtualclass.wbWrapper.gObj.previousData&&(c=virtualclass.wbWrapper.protocol.encode("sp",virtualclass.wbWrapper.gObj.previousData),virtualclass.wbWrapper.msg.send(c),c=virtualclass.wbWrapper.protocol.encode("sp",{event:"u",name:this.name,x:virtualclass.wbWrapper.gObj.previousData.x,y:virtualclass.wbWrapper.gObj.previousData.y}),virtualclass.wbWrapper.msg.send(c))}delete virtualclass.wbWrapper.gObj.previousData;
delete this.freeDrawPrevious;delete virtualclass.gObj.lastSendDataTime}innerMouseUp(c,b,a){this.mousedown=!1;"freeDrawing"!==this.name?this[this.name].setCoords():(a||(a={isPrimary:!0}),b.myPencil.onMouseUp(a),c=b.canvas.getObjects(),c[c.length-1].set({rotatingPointOffset:this.default.rotatingPointOffset*virtualclass.zoom.canvasScale,cornerSize:this.default.cornerSize*virtualclass.zoom.canvasScale}),b.activeAllObj.disableLastElement(virtualclass.gObj.currWb));delete b.myPencil}};class WhiteboardFreeDrawing extends WhiteboardCommonShape{constructor(c){super(c);this.name=c;this.chunks=[]}innerMouseMove(c,b,a){a||(a={e:{isPrimary:!0}});b.myPencil.onMouseMove(c,a)}mouseMove(c,b,a){this.innerMouseMove(c,b,a);a.e.isTrusted&&(this.collectingData(c),virtualclass.wbWrapper.msg.optimizeToSend(this.chunks,3E3,"sf"))}collectingData(c){c={x:c.x,y:c.y};if(!this.freeDrawPrevious||1<Math.abs(c.x-this.freeDrawPrevious.x)||1<Math.abs(c.y-this.freeDrawPrevious.y))this.freeDrawPrevious=c,this.chunks.push(`${c.x}_${c.y}`);
...
c.fontSize);c.fontColor&&a.set("fill",c.fontColor);b.canvas.renderAll()}renderText(c,b){var a=b.canvas.getObjects("i-text");0<a.length&&null!=c.index?(a=a[c.index])?this.updateText(c,b,a):this.createText(c,b):this.createText(c,b)}createText(c,b){""!==c.value&&(this.startLeft=c.x,this.startTop=c.y,this.coreObj.left=this.startLeft,this.coreObj.top=this.startTop,c=c.value?c.value:this.placeHolder,this.coreObj.rotatingPointOffset=this.default.rotatingPointOffset*virtualclass.zoom.canvasScale,this.coreObj.cornerSize=
this.default.cornerSize*virtualclass.zoom.canvasScale,this.coreObj.fontSize=this.default.fontSize,b.textFontSize&&(this.coreObj.fontSize=+b.textFontSize),this.coreObj.fill=this.default.fill,b.activeToolColor&&(this.coreObj.fill=b.activeToolColor),this[this.name]=new fabric.IText(c,this.coreObj),b.canvas.add(this[this.name]))}finalizeText(c){const b=virtualclass.wb[virtualclass.gObj.currWb];this.textEditing=!1;if(!this.isDefault(c.target)){virtualclass.wbWrapper.gObj.lastSentDataTime=(new Date).getTime();
var a={x:c.target.left,y:c.target.top,text:c.target.text};null!=this.editingIndex&&(a.index=this.editingIndex);+b.textFontSize&&c.target.fontSize!==+b.textFontSize&&(a.fontSize=b.textFontSize);b.activeToolColor&&c.target.fill!==b.activeToolColor&&(a.fontColor=b.activeToolColor);virtualclass.wbWrapper.msg.optimizeToSend(a,0,"tx");delete this.editingIndex;b.activeAllObj.enable(virtualclass.gObj.currWb,"i-text");(a.fontColor||a.fontSize)&&this.updateText(a,b,c.target)}}isDefault(c){return c.text.trim()===
this.placeHolder}afterSelected(c){console.log("selected 1");virtualclass.wbWrapper.gObj.textSelected=c}afterDeSelected(){console.log("deselected 1");virtualclass.wbWrapper.gObj.textSelected=null}mouseMove(){}mouseUp(){console.log("mouse up")}};class Whiteboard{constructor(){this.selectedTool=this.canvas=null;this.rectangleObj=new WhiteboardRectangle("rectangle");this.activeAllObj=new WhiteboardActiveAll;this.freeDrawingObj=new WhiteboardFreeDrawing("freeDrawing");this.lineObj=new WhiteboardLine("line");this.triangleObj=new WhiteboardTriangle("triangle");this.circleObj=new WhiteboardCircle("circle");this.textObj=new WhiteboardText("text");this.gObj={}}init(c){this.wbId=c;delete virtualclass.wbWrapper.gObj.textSelected;this.textObj.textEditing=
