...
a.sub<this.subRecordingIndex)&&this.replayFromStart();this._seek(a);console.log("seek is finished");this.triggerSynchPacket()},triggerSynchPacket(){this.triggerPlayProgress();this.binarySyncMsg&&(this.handleSyncPacket(),this.binarySyncMsg=null);this.handleSyncStringPacket()},_seek(a){this.controller._pause();let b;for(;;){b=null!=a?this.masterIndex!=a.master?this.masterRecordings[this.masterIndex].length:a.sub:this.masterRecordings[this.masterIndex].length;for(let a=this.subRecordingIndex;a<b;a++){try{if("B"!=
...
delete virtualclass.videoUl.player}};this.sync=function(){};this.recs=function(d){if(virtualclass.isPlayMode)11===d.message.ac?virtualclass.videoHost.onmessage("off"):virtualclass.videoHost.onmessage("on");else virtualclass.settings.onMessage(d.message)}};function Config(){}Config.prototype.setNewSession=function(d){localStorage.setItem("mySession",d);!virtualclass.isPlayMode&&roles.hasControls()&&(d=(new Date).getTime(),d=JSON.stringify({createdDate:d}),localStorage.setItem("myConfig",d))};
