...
(-1<this.backDetectValues.frameNavigator.indexOf("MSIE ")||-1<this.backDetectValues.frameNavigator.indexOf("Trident")?this.backDetectValues.frameDetect.src=`${this.backDetectValues.frameDataSrc}'?historyLoaded`:this.backDetectValues.frameDetect.src="about:blank?historyLoaded")}};class LiveStream{constructor(){this.mimeType="video/webm;codecs=vp8,opus";this.queue=[];this.MAX_TIME=999999999999;this.startedStream=!1;this.uploadEndPoint="https://stream.congrea.net";this.fileList=new OrderedList;this.streamQueue=[];this.sendFiles=[];this.bufferLength=2;this.listStream={};this.playByOgv=this.sharing=!1;this.resoluation={qga:{width:{ideal:320},height:{ideal:240}},vga:{width:{ideal:640},height:{ideal:480}},vga2:{width:{ideal:800},height:{ideal:600}},vga3:{width:{ideal:960},height:{ideal:720}},
vga4:{width:{ideal:1024},height:{ideal:768}},hd:{width:{ideal:1280},height:{ideal:720}},fullhd:{width:{ideal:1920},height:{ideal:1080}},fourk:{width:{ideal:4096},height:{ideal:2160}},eightk:{width:{ideal:7680},height:{ideal:4320}}};this.xhr=axios.create({responseType:"arraybuffer",withCredentials:!0});this.constraints={audio:{echoCancellation:{exact:!0}},video:this.resoluation.fullhd};this.constraints.video.frameRate={ideal:15}}init(){setTimeout(()=>{if(!document.querySelector("#networkStatusContainer.connecting-room")){const b=
document.getElementById("startLiveStream");null!=b&&b.classList.remove("disabled")}},1500);if(!this.alreadyInit){virtualclass.isPlayMode?(this.prefixUrl=`${this.uploadEndPoint}/${wbUser.lkey}/${wbUser.room}/${wbUser.session}`,console.log("Prefix url recording",this.prefixUrl)):null!=localStorage.mySession?(this.prefixUrl=`${this.uploadEndPoint}/${wbUser.lkey}/${wbUser.room}/${localStorage.mySession}`,console.log("Prefix url localstorage",this.prefixUrl)):virtualclass.gObj.currentSession&&(this.prefixUrl=
`${this.uploadEndPoint}/${wbUser.lkey}/${wbUser.room}/${virtualclass.gObj.currentSession}`);if(!this.prefixUrl){setTimeout(()=>{console.log("Prefix was not found",this.prefixUrl);this.init()},1500);return}roles.hasControls()&&(console.log("==> attach click event "),document.getElementById("startLiveStream").addEventListener("click",this.handlLiveStream.bind(this)));this.alreadyInit=!0;var c=document.getElementById("virtualclassCont");null!=c&&c.classList.add("ogvPlayer")}"Safari"===virtualclass.system.mybrowser.name&&
(this.playByOgv=!0);(this.playByOgv=!0,!this.isScriptAlreadyIncluded("../build/ogv/ogv.js"))&&this.loadFile("/virtualclass/build/ogv/ogv.js","js")}requestStream(c){c&&this.xhr.get(c).then(async b=>{this.afterReceivedStream(b)})}getFileName(c){const b=c.lastIndexOf("/");return c.substring(b+1).replace(".chvs","")}duringPlayFirstPacket(){var c=document.getElementById("virtualclassVideo");c&&(c.dataset.currapp="liveStream");if(c=document.getElementById("messageLayoutVideo"))c.style.display="none"}afterReceivedStream(c){console.log("play start data received");
const b=c.data.slice(4,c.data.length);c=this.getFileName(c.config.url);this.listStream[c]=b;console.log("request url, stream receive init data ",c);console.log("reponse receive at very start, Play start 0 ",c);this.playIfReady(c);console.log("====> response received for file ",c)}onBuffer(c){this.sourceBuffer&&this.sourceBuffer.appendBuffer(c)}readyParticipateVideo(){this.remoteVideo||(this.remoteVideo=document.getElementById("liveStream"),this.remoteVideo.srcObject=null,this.remoteVideo.currentTime=
0)}initPLayerForParticipaes(){console.log("current mode LIVE STREAM");this.mediaSource=new MediaSource;this.mediaSource.addEventListener("sourceopen",this.mediaSourceOpen.bind(this));this.readyParticipateVideo();this.remoteVideo.src=URL.createObjectURL(this.mediaSource)}mediaSourceOpen(){console.log("step 1");"open"===this.mediaSource.readyState&&(this.sourceBuffer=this.mediaSource.addSourceBuffer(this.mimeType),this.sourceBuffer.addEventListener("error",c=>{console.log(c)}),this.sourceBuffer.addEventListener("updateend",
()=>{console.log("====> appended start: remove 2");const c=this.fileList.getNextByID(virtualclass.liveStream.currentExecuted);console.log(" ===> actual PLAY START 2");c&&this.playIfReady(c.id)}))}handlLiveStream(c){"0"===c.currentTarget.dataset.sharinglivestream?this.startToShare():(this.stop(),c=document.querySelector("#dashboardnav button"),null!=c&&c.click())}disableLiveStreamButton(){const c=document.getElementById("startLiveStream");null!=c&&c.classList.add("disabled")}async startToShare(){this.stopTraditionalVideo();
document.getElementById("startLiveStream").dataset.sharinglivestream="1";let c=null;try{c=await navigator.mediaDevices.getUserMedia(this.constraints)}catch(b){this.disableLiveStreamButton(),virtualclass.media.handleUserMediaError(b)}c&&this.handleSuccess(c)}handleSuccess(c){var b=c.getVideoTracks()[0];const a=b.getSettings();console.dir("====> camera settings ",a);b=b.getSettings();console.log("Video constraints: media settings:",JSON.stringify(b));b=c.getAudioTracks()[0].getSettings();console.log("Audio constraints: media settings: ",
JSON.stringify(b));this.stream=c;virtualclass.dashboard.close();this.showLiveStreamHTML();document.getElementById("liveStream").srcObject=this.stream;this.startRecorder(a.width)}startRecorder(c){console.log("current mode LIVE STREAM");console.log("video bit per second ",25E4);MediaRecorder.isTypeSupported(this.mimeType)||console.error(`${this.mimeType} is not supported`);this.mediaRecorder=new MediaRecorder(this.stream,{mimeType:this.mimeType,videoBitsPerSecond:25E4});this.mediaRecorder.addEventListener("stop",
this.stopHandler.bind(this));this.mediaRecorder.addEventListener("dataavailable",this.handleLiveStreamData.bind(this));this.mediaRecorder.start(2E3)}stopHandler(){console.log("====STOP 1")}handleLiveStreamData(c){console.log("====> triggering data ");this.mediaRecorder&&"recording"===this.mediaRecorder.state&&this.saveToStreamServer(c.data)}async saveToStreamServer(c){c=await this.videoTypedArray(c);ioAdapter.sendStream(c)}async videoTypedArray(c){c=await c.arrayBuffer();c=new Uint8Array(c);let b=
new c.constructor(2);b[0]=72;b[1]=78;let a=new c.constructor(c.length+b.length);a.set(b,0);a.set(c,b.length);return a}stop(){document.getElementById("startLiveStream").dataset.sharinglivestream="0";this.mediaRecorder&&"inactive"!==this.mediaRecorder.state&&this.mediaRecorder.stop();this.hideLiveStreamHTML();this.disableLiveStreamButton();roles.hasControls()&&(ioAdapter.mustSend({cf:"liveStream",stop:!0}),this.clearEveryThing());setTimeout(()=>{const c=document.getElementById("startLiveStream");null!=
c&&c.classList.remove("disabled")},5E3)}stopTraditionalVideo(){Object.prototype.hasOwnProperty.call(virtualclass,"videoUl")&&Object.prototype.hasOwnProperty.call(virtualclass.videoUl,"player")&&virtualclass.videoUl.destroyPlayer();const c=document.getElementById("videoPlayerCont");c&&(c.style.display="none");roles.hasControls()&&ioAdapter.mustSend({cf:"liveStream",stopVideo:!0})}clearEveryThing(){console.log("Removed live stream");delete virtualclass.gObj.videoMode;console.log("delete normal video");
...
console.log("Source buffer remove")}delete this.sourceBuffer;delete this.mediaSource}this.startedStream=!1;c=document.getElementById("liveStream");null!=c&&(c.src=null);delete this.firstFile;console.log("====> Empty the list 2a");delete this.startingPoint;delete this.startedAppending;this.fileList.emptyList();delete this.lastFileRequested;delete virtualclass.liveStream.callFromSeek;delete this.startFromPageRefresh;delete this.currentExecuted;console.log("====> appended start: remove 1");this.playByOgv&&
this.destroyOGVPlayer();roles.hasControls()&&virtualclass.media.audio.isProcessError&&(virtualclass.precheck.afterComplete(!0),delete virtualclass.media.audio.isProcessError)}showLiveStreamHTML(){document.getElementById("virtualclassVideo").dataset.currapp="liveStream"}hideLiveStreamHTML(){document.getElementById("virtualclassVideo").dataset.currapp="normalVideo"}isLiveStreamMode(){return"liveStream"===document.getElementById("virtualclassVideo").dataset.currapp}onMessage(c){c.message.fileName?(console.log("Received file from server ",
c.message.fileName),setTimeout(()=>{"Video"===virtualclass.currApp&&this.mediaRecorder&&"recording"===this.mediaRecorder.state&&(console.log("====> send file ",c.message.fileName),ioAdapter.mustSend({cf:"liveStream",url:c.message.fileName}))},3E3)):c.message.stop?this.clearEveryThing():c.message.stopVideo?this.stopTraditionalVideo():c.message.url&&"Video"===virtualclass.currApp&&!Object.prototype.hasOwnProperty.call(virtualclass.gObj,"videoMode")&&roles.isStudent()&&(console.log("====> Empty the list 2"),
this.firstFile||(this.firstFile=c.message.url,console.log("This is first file ",this.firstFile)),this.lastFileRequested=c.message.url,console.log("Last file request ",this.lastFileRequested),this.fileList.insert(c.message.url,`${this.prefixUrl}/${c.message.url}.chvs`),console.log("====> insert list suman ",c.message.url),!this.startingPoint&&this.fileList.ol.order.length>=this.bufferLength&&this.readyStartingPoint(),this.insertTime&&(clearTimeout(this.insertTime),this.pageRefresh=!0),this.insertTime=
setTimeout(()=>{this.pageRefresh?5>=this.fileList.ol.order.length?(this.triggerStart(this.fileList.ol.order[0]),console.log("live stream, start from first ",this.fileList.ol.order.length)):this.startFromPageRefresh||(this.startFromPageRefresh=!0,virtualclass.liveStream.callFromSeek||this.requestInitializePacket(),console.log("live stream, start from latest",this.fileList.ol.order.length)):this.triggerStart(c.message.url);console.log("page refresh remove");this.insertTime=this.pageRefresh=!1;console.log("live video suman 1")},
100))}requestInitializePacket(c){this.stopTraditionalVideo();this.initPLayerForParticipaes();this.xhrInitPacket||(this.xhrInitPacket=axios.create({responseType:"arraybuffer",headers:{"Content-Type":"application/json","x-api-key":wbUser.lkey,"x-congrea-authuser":wbUser.auth_user,"x-congrea-authpass":wbUser.auth_pass,"x-congrea-room":wbUser.room,get:{Accept:"video/webm;codecs=vp8,opus"}}}));let b;b=virtualclass.isPlayMode?wbUser.session:null!=localStorage.mySession?localStorage.mySession:virtualclass.gObj.currentSession;
c=c?`https://api.congrea.net/data/stream?session=${b}&file=${c}`:`https://api.congrea.net/data/stream?session=${b}`;console.log("request url init packet ",c);this.requestInitializePacketFinal(c)}requestInitializePacketFinal(c){this.latesRequetInitUrl=c;console.log("request url live stream init data ",c);this.xhrInitPacket.get(c).then(async b=>{this.latesRequetInitUrl===b.config.url&&(this.currentFile=b.headers["x-congrea-seg"].split(".chvs")[0],console.log("request url, stream receive init data ",
this.currentFile,b.config.url.split("?")[1]),this.startedStream=!0,this.listStream[this.currentFile]=b.data,this.firstFile=b.headers["x-congrea-seg"].split(".chvs")[0],console.log("This is first file ",this.firstFile),delete this.startingPoint,console.log("calculate starting point ",this.firstFile),this.readyStartingPoint())})}triggerStart(c){this.startedStream?this.startedStream&&0<this.fileList.ol.order.length&&(c=this.fileList.getNextByID(this.currentFile),this.requestStream(c.data),this.currentFile=
c.id):(this.initPLayerForParticipaes(),this.requestStream(this.fileList.getCurrent(c)),this.currentFile=c,this.startedStream=!0)}isMyTurn(c){const b=this.fileList.getNextByID(this.currentExecuted);console.log("is my turn curernt, next ",this.currentExecuted,b.id,c);return b.id===c}inStreamList(c){return this.listStream[c]?this.listStream[c]:!1}readyStartingPoint(){let c=this.firstFile;if(-1>=this.fileList.getCurrentPosition(c))console.log("NOT FOUND ON LIST");else{var b;if(1<this.bufferLength){for(let a=
1;a<this.bufferLength;a++)if(b=this.fileList.getNextByID(c))c=b.id;c!=this.firstFile&&(this.startingPoint=c)}}}playIfReady(c){this.playByOgv?this.playIfReadyOGV(c):this.playIfReadyNormal(c)}playIfReadyNormal(c){var b=this.inStreamList(c);if(this.startingPoint&&c===this.startingPoint&&this.inStreamList(this.firstFile)){b=this.inStreamList(this.firstFile);try{this.onBuffer(b),console.log("Actual append buffer ",this.firstFile),this.currentExecuted=this.firstFile,delete this.listStream[this.firstFile],
this.startedAppending=!0,this.duringPlayFirstPacket(),this.startFromPageRefresh&&setTimeout(()=>{document.getElementById("liveStream").currentTime=this.MAX_TIME;console.log("REFRESH THE SCREEN")},virtualclass.isPlayMode?700:1600)}catch(a){this.requestInitializePacket(c),console.log("====> Error handlling request packet")}}else if(this.startedAppending&&this.isMyTurn(c)&&b)try{this.onBuffer(b),console.log("Actual append buffer ",c),delete this.listStream[c],this.currentExecuted=c}catch(a){this.requestInitializePacket(c),
console.log("====> Error handlling request packet")}}playIfReadyOGV(c){console.log("suman ogv total file Received to play ",c);!this.ogvPlayerReady&&this.inStreamList(this.firstFile)&&1<Object.keys(this.listStream).length&&1<virtualclass.liveStream.fileList.ol.order.length&&(this.readyOGVInstance(),this.ogvPlayerReady=!0,c=this.inStreamList(this.firstFile),console.log("Actual append buffer ",this.firstFile),console.log("LOADED META DATA SUMAN BOGATI init start playing"),this.ogvPlayer._loadCodec(c,
function(b){console.log("Laxmi ogv play ",virtualclass.liveStream.firstFile);virtualclass.liveStream.ogvPlayer._startProcessingVideo(b);virtualclass.liveStream.currentExecuted=virtualclass.liveStream.firstFile;delete virtualclass.liveStream.listStream[virtualclass.liveStream.firstFile];console.log("====> DELETE LIVE STREAM FILE ",virtualclass.liveStream.firstFile);virtualclass.liveStream.startedAppending=!0;virtualclass.liveStream.duringPlayFirstPacket();virtualclass.liveStream.ogvPlayerLoadedMedia=
setInterval(()=>{virtualclass.liveStream.ogvPlayer&&virtualclass.liveStream.ogvPlayer._codec&&virtualclass.liveStream.ogvPlayer._codec.loadedAllMetadata&&(clearInterval(virtualclass.liveStream.ogvPlayerLoadedMedia),virtualclass.liveStream.ogvPlayer.play())},500)}))}destroyOGVPlayer(){console.log("DESTROY OGV PLAYER");this.ogvPlayer&&this.ogvPlayer.stop();delete this.ogvPlayer;delete virtualclass.liveStream.listStream;virtualclass.liveStream.listStream={};delete this.ogvPlayerReady;const c=document.getElementById("ogvVideoContainer");
null!=c&&c.parentNode.removeChild(c)}readyOGVInstance(){this.ogvPlayer=new OGVPlayer({forceWebGL:!0,debug:!1});var c=document.createElement("div");c.id="ogvVideoContainer";c.appendChild(this.ogvPlayer);document.getElementById("liveStreamCont").appendChild(c)}getChunkForOgvPlayer(){const c=this.fileList.getNextByID(this.currentExecuted);if(c){virtualclass.liveStream.tempFile=c.id;const b=this.inStreamList(c.id);b&&console.log("Laxmi ogv play 2",c.id);return b}console.log("suman ogv total file execute play not");
return!1}loadFile(c,b){if("js"===b){var a=document.createElement("script");a.setAttribute("type","text/javascript");a.setAttribute("src",c)}"undefined"!=typeof a&&document.getElementsByTagName("head")[0].appendChild(a)}isScriptAlreadyIncluded(c){for(var b=document.getElementsByTagName("script"),a=0;a<b.length;a++)if(b[a].getAttribute("src")==c)return!0;return!1}};$.uiBackCompat=!1;
