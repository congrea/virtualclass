...
g?103:203,I[0],I[1],G[0],G[1],f.h,f.w]);f=new Uint8ClampedArray(d.length+g.length);f.set(g);f.set(d,g.length);d=f;null===q||void 0===q?q=d:(f=new Uint8ClampedArray(q.length+d.length),f.set(q),f.set(d,q.length),q=f)}}else console.log("====> screen share need full screen "),u[k]=z,m=1;q?(r=c(a.data.uid,8),p=new Uint8ClampedArray(["103","123",r[0],r[1],r[2],r[3]]),A=new Uint8ClampedArray(q.length+p.length),A.set(p),A.set(q,p.length),postMessage({sendMasterSlice:A,needFullScreen:m},[A.buffer]),A=null):
postMessage({masterSlice:null,needFullScreen:m});m=0;q=null}}}.toString(),")()"],{type:"application/javascript"})),sworker=new Worker(screenWorkerBlob);const dtConWorkerBlob=URL.createObjectURL(new Blob(["(",function(){const c=function(){function b(b,a){b=b.replace(/[^A-Za-z0-9\+\/]/g,"");const d=b.length;a=a?Math.ceil((3*d+1>>2)/a)*a:3*d+1>>2;const c=new Uint8Array(a);for(var f,n=0,q=0,t=0;t<d;t++){f=t&3;var z=b.charCodeAt(t);n|=(64<z&&91>z?z-65:96<z&&123>z?z-71:47<z&&58>z?z+4:43===z?62:47===z?63:0)<<6*(3-f);if(3===f||1===d-t){for(f=0;3>f&&q<a;f++,q++)c[q]=n>>>(16>>>f&24)&255;n=0}}return c}function a(b){return 26>b?b+65:52>b?b+71:62>b?b-4:62===
...
d={};let f=0;const g={};let k,m;const n=[];let q=0;class t extends AudioWorkletProcessor{constructor(){super();this.port.onmessage=c}process(b,c){q=n.length=0;for(k in a){b:{b=k;if(null!==a){if(288<=a[b].length){for(;96<=a[b].length;)a[b].shift();d[b]=!0;b=a[b].shift();break b}if(64<=a[b].length){d[b]=!0;b=a[b].shift();break b}if(0<a[b].length&&!0===d[b]){d[b]=!0;b=a[b].shift();break b}d[b]=!1;0===a[b].length&&delete a[b]}b=void 0}if(null!=b)if(q++,1===q)for(var g=0;128>g;g++)n[g]=b[g];else for(g=
0;128>g;g++)n[g]+=b[g]}if(1===q)b=n;else if(1<q){for(b=0;128>b;b++)n[b]/=q;b=n}else b=void 0;if(null!=b)c[0][0].set(b,0),f=b[127];else if(null!==f)for(b=0;128>b;b++)c[0][0][b]=f;return!0}}registerProcessor("worklet-audio-rec",t)}.toString(),")()"],{type:"application/javascript"}));/*
  2018 Suman Bogati  {@link http://vidyamantra.com}
 @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
c=f.log;k=f.detectBrowser(d);const m=a("./chrome/chrome_shim")||null,z=a("./edge/edge_shim")||null,v=a("./firefox/firefox_shim")||null,u=a("./safari/safari_shim")||null,p=a("./common_shim")||null,r={browserDetails:k,commonShim:p,extractVersion:f.extractVersion,disableLog:f.disableLog,disableWarnings:f.disableWarnings};switch(k.browser){case "chrome":if(!m||!m.shimPeerConnection||!g.shimChrome){c("Chrome shim is not included in this adapter release.");break}c("adapter.js shimming chrome.");r.browserShim=
m;p.shimCreateObjectURL(d);m.shimGetUserMedia(d);m.shimMediaStream(d);m.shimSourceObject(d);m.shimPeerConnection(d);m.shimOnTrack(d);m.shimAddTrackRemoveTrack(d);m.shimGetSendersWithDtmf(d);p.shimRTCIceCandidate(d);p.shimMaxMessageSize(d);p.shimSendThrowTypeError(d);break;case "firefox":if(!v||!v.shimPeerConnection||!g.shimFirefox){c("Firefox shim is not included in this adapter release.");break}c("adapter.js shimming firefox.");r.browserShim=v;p.shimCreateObjectURL(d);v.shimGetUserMedia(d);v.shimSourceObject(d);
v.shimPeerConnection(d);v.shimOnTrack(d);v.shimRemoveStream(d);p.shimRTCIceCandidate(d);p.shimMaxMessageSize(d);p.shimSendThrowTypeError(d);break;case "edge":if(!z||!z.shimPeerConnection||!g.shimEdge){c("MS edge shim is not included in this adapter release.");break}c("adapter.js shimming edge.");r.browserShim=z;p.shimCreateObjectURL(d);z.shimGetUserMedia(d);z.shimPeerConnection(d);z.shimReplaceTrack(d);p.shimMaxMessageSize(d);p.shimSendThrowTypeError(d);break;case "safari":if(!u||!g.shimSafari){c("Safari shim is not included in this adapter release.");
break}c("adapter.js shimming safari.");r.browserShim=u;p.shimCreateObjectURL(d);u.shimRTCIceServerUrls(d);u.shimCallbacksAPI(d);u.shimLocalStreamsAPI(d);u.shimRemoteStreamsAPI(d);u.shimTrackEventTransceiver(d);u.shimGetUserMedia(d);u.shimCreateOfferLegacy(d);p.shimRTCIceCandidate(d);p.shimMaxMessageSize(d);p.shimSendThrowTypeError(d);break;default:c("Unsupported browser!")}return r}},{"./chrome/chrome_shim":5,"./common_shim":7,"./edge/edge_shim":8,"./firefox/firefox_shim":10,"./safari/safari_shim":12,
...
typeof arguments[0]?f.apply(a,[function(f){f=d(a,f);c[0].apply(null,[f])},function(a){c[1]&&c[1].apply(null,a)},arguments[2]]):f.apply(a,arguments).then(c=>d(a,c))}});const p=a.RTCPeerConnection.prototype.setLocalDescription;a.RTCPeerConnection.prototype.setLocalDescription=function(){if(!arguments.length||!arguments[0].type)return p.apply(this,arguments);arguments[0]=c(this,arguments[0]);return p.apply(this,arguments)};const r=Object.getOwnPropertyDescriptor(a.RTCPeerConnection.prototype,"localDescription");
Object.defineProperty(a.RTCPeerConnection.prototype,"localDescription",{get(){const a=r.get.apply(this);return""===a.type?a:d(this,a)}});a.RTCPeerConnection.prototype.removeTrack=function(a){const d=this;if("closed"===d.signalingState)throw new DOMException("The RTCPeerConnection's signalingState is 'closed'.","InvalidStateError");if(!a._pc)throw new DOMException("Argument 1 of RTCPeerConnection.removeTrack does not implement interface RTCRtpSender.","TypeError");if(a._pc!==d)throw new DOMException("Sender was not created by this connection.",
...
this.gainNode.connect(this.audioCtx.destination)}},workerAudioSendOnmessage(){workerAudioSend.onmessage=a=>{Object.prototype.hasOwnProperty.call(a.data,"cmd")&&("muteAudio"===a.data.cmd?n.audio.notifiyMuteAudio():"unMuteAudio"===a.data.cmd&&n.audio.notifiyUnmuteAudio())}},audio:{audioStreamArr:[],tempAudioStreamArr:[],recordingLength:0,bufferSize:0,encMode:"alaw",recordAudio:!1,rec:"",otherSound:!1,audioNodes:[],sdElem:"silenceDetect",snode:[],workletAudioRec:!1,aChunksPlay:!1,audioContextReady:!1,
init(){const a=document.getElementById("speakerPressOnce").dataset.audioPlaying;virtualclass.gObj.audMouseDown="true"===a;this.graph={height:56,width:4,average:0,display(){const a=n.video;roles.hasControls()&&this.initDisplay(a.tempVidCont,this.height-this.height*this.average/100)},initDisplay(a,b){a.beginPath();a.moveTo(this.width,this.height);a.lineTo(this.width,b);a.lineWidth=this.width;a.strokeStyle="rgba(32, 37, 247, 0.8)";a.closePath();a.stroke()}};this.attachFunctionsToAudioWidget()},initAudiocontext(){Object.prototype.hasOwnProperty.call(this,
"Html5Audio")||virtualclass.gObj.meetingMode||(this.Html5Audio={audioContext:new (c.AudioContext||c.webkitAudioContext)},null==virtualclass.media.audio.Html5Audio.audioContext&&alert("audio context is null"),this.resampler=new Resampler(virtualclass.media.audio.Html5Audio.audioContext.sampleRate,8E3,1,4096),this.audioContextReady=virtualclass.gObj.isAudioContextReady=!0,virtualclass.system.mediaDevices.hasMicrophone&&!virtualclass.isPlayMode&&null!=n.video.tempStream&&(virtualclass.media.stream=n.video.tempStream,
...
b.setAttribute("data-silence-detect",a)},makeIconNotDraggable(a,b){const d=document.getElementById(a,b).getContext("2d"),f=new Image;f.onload=()=>{d.drawImage(f,0,0)};f.src=`${c.whiteboardPath}images/${b}`},attachFunctionsToAudioWidget(){const a=document.getElementById("audioWidget").getElementsByClassName("audioTool");for(let b=0;b<a.length;b++)"speakerPressOnce"!==a[b].id&&"speakerPressing"===a[b].id&&this.attachSpeakToStudent(a[b].id),"speakerPressing"!==a[b].id&&a[b].addEventListener("click",
...
new Int8Array(a))},str2ab(a){var b=new ArrayBuffer(a.length);b=new Int8Array(b);for(let d=0,c=a.length;d<c;d++)b[d]=a.charCodeAt(d);return b},recorderProcess(a){if(!f&&(this.recordAudio||(this.recordingLength+=this.bufferSize),virtualclass.gObj.audMouseDown&&io.webSocketConnected())){a=this.resampler.resampler(a);p=a.length;for(g=new Int16Array(p);p--;)g[p]=32767*Math.min(1,a[p]);a=g;const b=this.encodeAudio(a);this.silenceDetection(b,a)}},recorderProcessFallback(a){workerAudioSend.postMessage({cmd:"rawAudio",
msg:a.inputBuffer.getChannelData(0)})},encodeAudio(a){return G711.encode(a,{alaw:"alaw"===this.encMode})},audioForTesting(a){a=G711.encode(a,{alaw:"alaw"===this.encMode});virtualclass.gObj.audioForTest.push(a);virtualclass.gObj[virtualclass.gObj.uid]=!1},initPlayWithFallback(){Object.prototype.hasOwnProperty.call(this,"Html5Audio")||this.Html5Audio||(this.Html5Audio={audioContext:new (c.AudioContext||c.webkitAudioContext)});if(!u){const a=new MessageChannel;workerIO.postMessage({cmd:"workerAudioRec"},
[a.port1]);workerAudioRec.postMessage({cmd:"workerIO",sampleRate:this.Html5Audio.audioContext.sampleRate},[a.port2]);workerAudioRec.postMessage({cmd:"audioWorklet",msg:!1});workerAudioRec.onmessage=a=>{switch(a.data.cmd){case "noAudioWorklet":virtualclass.media.audio.queueWithFalback(a.data.msg.data,a.data.msg.uid),virtualclass.media.audio.playWithFallback(a.data.msg.uid)}};u=!0;virtualclass.gObj.audioRecWorkerReady=!0}},initPlay(){this.addingWorkletPending||(console.log("audio init worklet suman b"),
"undefined"!==typeof v&&(console.log("audio worklet disconnect"),v.disconnect()),Object.prototype.hasOwnProperty.call(this,"Html5Audio")||this.Html5Audio||(this.Html5Audio={audioContext:new (c.AudioContext||c.webkitAudioContext)}),this.addingWorkletPending=!0,n.audio.Html5Audio.audioContext.audioWorklet.addModule(workletAudioRecBlob).then(()=>{v=new AudioWorkletNode(n.audio.Html5Audio.audioContext,"worklet-audio-rec");n.audio.Html5Audio.MediaStreamDest=n.audio.Html5Audio.audioContext.createMediaStreamDestination();
v.connect(n.audio.Html5Audio.audioContext.destination);virtualclass.gObj.workletAudioRec=v;"Chrome"===virtualclass.system.mybrowser.name&&n.audio.bug_687574_callLocalPeers();var a=new MessageChannel;workerIO.postMessage({cmd:"workerAudioRec"},[a.port1]);workerAudioRec.postMessage({cmd:"workerIO",sampleRate:n.audio.Html5Audio.audioContext.sampleRate},[a.port2]);a=new MessageChannel;workerAudioRec.postMessage({cmd:"workletAudioRec"},[a.port1]);v.port.postMessage({cmd:"workerAudioRec"},[a.port2]);workerAudioRec.postMessage({cmd:"audioWorklet",
msg:!0});initchannel=!0;virtualclass.gObj.audioRecWorkerReady=!0;delete n.audio.addingWorkletPending}).catch(a=>{console.log("ERROR ",a)}))},playWithFallback(a){"suspended"===this.Html5Audio.audioContext.state?Object.prototype.hasOwnProperty.call(this,"audioSuspendTime")||(this.audioSuspendTime=setTimeout(()=>{"suspended"===this.Html5Audio.audioContext.state&&(this.snode.push(a),null===virtualclass.gObj.requestToScriptNode&&(this.Html5Audio.audioContext.resume(),virtualclass.gesture.initAudioResume(a),
virtualclass.gObj.requestToScriptNode=!0),delete this.audioSuspendTime)},2E3)):(this.innerPlayWithFallback(),this.audioSuspendTime&&delete this.audioSuspendTime)},innerPlayWithFallback(){const a=this;if(null===virtualclass.media.audioPlayerNode||"closed"===virtualclass.media.audioPlayerNode.context.state)null!==virtualclass.media.audioPlayerNode&&virtualclass.media.audioPlayerNode.disconnect(),virtualclass.media.audioPlayerNode=this.Html5Audio.audioContext.createScriptProcessor(4096,1,1),r=0,virtualclass.media.audioPlayerNode.onaudioprocess=
b=>{b=b.outputBuffer.getChannelData(0);var d=a.getMergedAudio();if(null!==d&&void 0!==d){for(let a=0;a<d.length;a++)b[a]=d[a];r=d[4095]}else for(d=0;d<b.length;d++)b[d]=r},virtualclass.media.audioPlayerNode.connect(this.Html5Audio.audioContext.destination)},queueWithFalback(a,c){null==b[c]&&(b[c]=[]);null==d[c]&&(d[c]=[]);for(let b=0;b<a.length;b++)d[c].push(a[b]);for(;4096<=d[c].length;)a=d[c].splice(0,4096),b[c].push(new Float32Array(a))},getMergedAudio(){q=[];t=0;for(var a in b){const b=this.getAudioChunks(a);
if(null!=b)if(t++,1===t)for(var d=0;4096>d;d++)q[d]=b[d];else for(d=0;4096>d;d++)q[d]+=b[d]}if(1===t)return q;if(1<t){for(a=0;4096>a;a++)q[a]/=t;return q}},getAudioChunks(d){if(null!==b){if(9<=b[d].length){for(;3<=b[d].length;)b[d].shift();a[d]=!0;return b[d].shift()}if(2<=b[d].length||0<b[d].length&&!0===a[d])return a[d]=!0,b[d].shift();a[d]=!1;0===b[d].length&&delete b[d]}},actualManiPulateStream(){this.triggermaniPulateStream=!0;n=virtualclass.media;setTimeout(()=>{n.detectAudioWorklet()?n.audio.maniPulateStream():
n.audio.maniPulateStreamWithFallback()},1E3)},maniPulateStream(){const {stream:a}=n;"undefined"!==typeof z&&z.disconnect();"undefined"!==typeof a&&null!=a&&n.audio.Html5Audio.audioContext.audioWorklet.addModule(workletAudioSendBlob).then(()=>{var b=n.audio.Html5Audio.audioContext.createMediaStreamSource(a);const d=n.audio.Html5Audio.audioContext.createBiquadFilter();d.type="lowpass";d.frequency.value=2E3;b.connect(d);z=new AudioWorkletNode(n.audio.Html5Audio.audioContext,"worklet-audio-send");z.onprocessorerror=
a=>{console.log("this is on process error");n.audio.notifiyMuteAudio();n.audio.isProcessError=!0};d.connect(z);z.connect(n.audio.Html5Audio.audioContext.destination);virtualclass.gObj.workletAudioSend=z;b=new MessageChannel;workerAudioSend.postMessage({cmd:"workerIO",sampleRate:this.Html5Audio.audioContext.sampleRate,uid:virtualclass.gObj.uid},[b.port1]);workerIO.postMessage({cmd:"workerAudioSend"},[b.port2]);b=new MessageChannel;workerAudioSend.postMessage({cmd:"audioWorkletSend",msg:{repMode:f}},
[b.port1]);z.port.postMessage({cmd:"workerAudioSend"},[b.port2]);n.workerAudioSendOnmessage()}).catch(a=>{n.audio.notifiyMuteAudio()})},maniPulateStreamWithFallback(){if("undefined"!==typeof n.stream&&null!=n.stream){var {stream:a}=n;a=n.audio.Html5Audio.audioContext.createMediaStreamSource(a);n.audio.bufferSize=4096;virtualclass.media.audioCreatorNode=n.audio.Html5Audio.audioContext.createScriptProcessor(n.audio.bufferSize,1,1);virtualclass.media.audioCreatorNode.onaudioprocess=n.audio.recorderProcessFallback.bind(n.audio);
const b=n.audio.Html5Audio.audioContext.createBiquadFilter();b.type="lowpass";b.frequency.value=2E3;a.connect(b);b.connect(virtualclass.media.audioCreatorNode);virtualclass.media.audioCreatorNode.connect(n.audio.Html5Audio.audioContext.destination);a=new MessageChannel;workerAudioSend.postMessage({cmd:"workerIO",uid:virtualclass.gObj.uid,sampleRate:n.audio.Html5Audio.audioContext.sampleRate},[a.port1]);workerIO.postMessage({cmd:"workerAudioSend"},[a.port2]);n.workerAudioSendOnmessage()}},updateInfo(){this.audioStreamArr=
[];f=this.recordAudio=!1},extractData(a){a=new Int8Array(a);return[virtualclass.vutil.numValidateFour(a[1],a[2],a[3],a[4]),a.subarray(5,a.length)]},removeAudioFromLocalStorage(){localStorage.removeItem("audEnable")},async bug_687574_callLocalPeers(){function a(a,b){if("connected"===b.currentTarget.connectionState)try{v.disconnect(),v.connect(n.audio.Html5Audio.MediaStreamDest)}catch(L){console.log("Audio error ",L)}else if("disconnected"===b.currentTarget.connectionState){g.close();k.close();k=g=
null;try{v.disconnect(),v.connect(n.audio.Html5Audio.audioContext.destination)}catch(L){console.log("Audio error ",L)}n.audio.bug_687574_callLocalPeers()}}async function b(a){try{await g.setLocalDescription(a),await k.setRemoteDescription(a)}catch(I){f()}try{const a=await k.createAnswer();await d(a)}catch(I){f()}}async function d(a){try{await k.setLocalDescription(a),await g.setRemoteDescription(a)}catch(I){f()}}async function c(a,b){if(b.candidate&&"host"===b.candidate.type)try{await (a===g?k:g).addIceCandidate(b.candidate)}catch(L){f()}}
function f(){try{v.connect(n.audio.Html5Audio.audioContext.destination),g.close(),k.close(),k=g=null}catch(K){k=g=null}}let g,k;g=new RTCPeerConnection;g.count=0;g.addEventListener("icecandidate",a=>c(g,a));g.addEventListener("connectionstatechange",b=>a(g,b));k=new RTCPeerConnection;k.count=0;k.addEventListener("icecandidate",a=>c(k,a));k.addEventListener("connectionstatechange",b=>a(k,b));k.addEventListener("track",function(a){const b=document.createElement("audio");b.srcObject=a.streams[0];b.autoplay=
!0});n.audio.Html5Audio.MediaStreamDest.stream.getTracks().forEach(a=>g.addTrack(a,n.audio.Html5Audio.MediaStreamDest.stream));try{const a=await g.createOffer();await b(a)}catch(K){f()}}},video:{width:75,height:56,tempVid:"",tempVidCont:"",myVideo:"",remoteVid:"",remoteVidCont:"",maxHeight:250,init(){this.videoCont=document.getElementById("allVideosCont");null!=this.videoCont&&(this.videoCont.style.maxHeight=`${this.maxHeight}px`)},calcDimension(){this.myVideo.width=this.width;this.myVideo.height=
this.height},removeUser(a){a=document.getElementById(`user${a}`);null!=a&&a.parentNode.removeChild(a)},createElement(a){const b=document.createElement("div");b.className="videoWrapper";const d=document.createElement("div");d.className="videoSubWrapper";d.id=`user${a.id}`;b.appendChild(d);k=a.id;const c=document.createElement("canvas");c.id=`video${k}`;c.className="userVideos";c.width=this.width;c.height=this.height;c.muted="muted";d.appendChild(c);virtualclass.media.util.imageReplaceWithVideo(a.id,
b)},updateHightInSideBar(a){const b=document.getElementById("widgetRightSide").offsetHeight;document.getElementById("chatContainer").style.height=`${b-a}px`},sendInBinary(a){io.webSocketConnected()&&ioAdapter.sendBinary(a)},send(){function a(){b.tempVidCont.clearRect(0,0,b.tempVid.width,b.tempVid.height);b.tempVidCont.drawImage(b.myVideo,0,0,b.width,b.height);roles.hasControls();virtualclass.system.webpSupport?(d=b.tempVid.toDataURL("image/webp",.6),c=1):(d=b.tempVid.toDataURL("image/jpeg",.3),c=
0);d=virtualclass.videoHost.convertDataURIToBinary(d);if(!virtualclass.videoHost.gObj.stdStopSmallVid&&(!roles.hasControls()||roles.hasControls()&&virtualclass.videoHost.gObj.videoSwitch)){var k=virtualclass.gObj.uid.toString();for(var n=k.length;8>n;n++)k=`0${k}`;k=k.match(/[\S]{1,2}/g)||[];k=new Uint8ClampedArray([11,k[0],k[1],k[2],k[3],c]);n=new Uint8ClampedArray(d.length+k.length);n.set(k);n.set(d,k.length);g.sendInBinary(n)}clearInterval(virtualclass.media.smallVid);if(Object.prototype.hasOwnProperty.call(virtualclass,
"connectedUsers"))if(k=m+2500*virtualclass.connectedUsers.length,f!==virtualclass.connectedUsers.length){f=virtualclass.connectedUsers.length;n=-1;for(var q=0;q<virtualclass.connectedUsers.length;q++)virtualclass.connectedUsers[0].userid===virtualclass.gObj.uid&&(n=q);q=k/f;0>n&&(n=0);virtualclass.media.smallVid=setInterval(a,k+n*q)}else virtualclass.media.smallVid=setInterval(a,k)}Object.prototype.hasOwnProperty.call(virtualclass.media,"smallVid")&&clearInterval(virtualclass.media.smallVid);const b=
this;let d,c;m=Math.floor(5001*Math.random()+3E3);let f=-1;const g=this;virtualclass.media.smallVid=setInterval(a,m)},startToStream(){n.video.calcDimension();n.video.send()},playWithoutSlice(a){this.remoteVid=document.getElementById(`video${a}`);null!=this.remoteVid&&(this.remoteVidCont=this.remoteVid.getContext("2d"),this.remoteVidCont.putImageData(imgData,0,0))},drawReceivedImage(a,b,d,c){this.remoteVid=chatContainerEvent.elementFromShadowDom(`#video${c}`);if(null!=this.remoteVid)if(this.remoteVidCont=
this.remoteVid.getContext("2d"),virtualclass.system.webpSupport||"jpeg"===b){const b=new Image,c=this;b.onload=()=>{c.remoteVidCont.drawImage(b,d.x,d.y)};b.src=a}else loadfile(a,this.remoteVid,this.videoPartCont)},justForDemo(){let a=0,b=document.getElementById("allVideosCont");b.style.maxHeight="250px";setInterval(()=>{if(20>=++a){b=document.getElementById("allVideosCont");const d=document.createElement("div");d.className="videoWrapper";const c=document.createElement("div");c.className="videoSubWrapper";
c.setAttribute("data-uname",+a);d.appendChild(c);const f=document.createElement("video");f.className="userVideo";f.muted="muted";f.src="http://html5demos.com/assets/dizzy.mp4";c.appendChild(f);b.appendChild(d);250<=b.offsetHeight&&null!=b.style.overflowY&&"scroll"!==b.style.overflowY&&(b.style.overflowY="scroll",document.getElementById(virtualclass.gObj.chat.mainChatBoxId).style.borderTop="3px solid #bbb")}},200)},createVideoElement(){const a=document.createElement("div");a.className="videoWrapper";
const b=document.createElement("div");b.className="videoSubWrapper";a.appendChild(b);const d=document.createElement("video");d.id=`video${virtualclass.gObj.uid}`;d.autoplay=!0;b.appendChild(d);return a},insertTempVideo(a){const b=document.createElement("canvas");b.id="tempVideo";a.parentNode.insertBefore(b,a)},tempVideoInit(){n.video.tempVid=chatContainerEvent.elementFromShadowDom("#tempVideo");n.video.tempVid.width=n.video.width;n.video.tempVid.height=n.video.height;n.video.tempVidCont=n.video.tempVid.getContext("2d")},
process(a){var b=new Uint8ClampedArray(a);a=virtualclass.vutil.numValidateFour(b[1],b[2],b[3],b[4]);var d={id:a};virtualclass.media.existVideoContainer(d)||virtualclass.media.video.createElement(d);d=b.subarray(6,b.length);1===b[5]?(b=`data:image/webp;base64,${btoa(virtualclass.videoHost.Uint8ToString(d))}`,d="webp"):(b=`data:image/jpeg;base64,${btoa(virtualclass.videoHost.Uint8ToString(d))}`,d="jpeg");virtualclass.media.video.drawReceivedImage(b,d,{x:0,y:0},a)}},sessionConstraints(){let a=!!virtualclass.system.mediaDevices.hasWebcam;
virtualclass.gObj.meetingMode&&a&&(a="mobTab"===virtualclass.system.device&&"iOS"===virtualclass.system.mybrowser.name||"Firefox"===virtualclass.system.mybrowser.name||"Safari"===virtualclass.system.mybrowser.name?!0:{width:{max:268},height:{max:142},frameRate:{max:6}});return[a,{video:a,audio:virtualclass.system.mediaDevices.hasMicrophone?{echoCancellation:!0,autoGainControl:!0,channelCount:1,noiseSuppression:!0}:!1}]},async init(){n=this;virtualclass.gesture.classJoin&&(virtualclass.gesture.attachHandler(),
delete virtualclass.gesture.classJoin);const [a,b]=this.sessionConstraints();this.video.init();virtualclass.user.control.audioDisable();virtualclass.user.control.videoDisable();if(!virtualclass.vutil.isPlayMode()){null==virtualclass.adpt&&(virtualclass.adpt=new virtualclass.adapter);if(null!=virtualclass.media.video.tempStream){var d=virtualclass.media.video.tempStream.getTracks();for(var c=0;c<d.length;c++)d[c].stop()}d=virtualclass.adpt.init(navigator);c=null;try{c=await d.mediaDevices.getUserMedia(b)}catch(C){this.handleUserMediaError(C),
this.audio.notifiyMuteAudio()}null!==c&&this.handleUserMedia(c)}!1===a&&(virtualclass.user.control.videoDisable(),virtualclass.vutil.addClass("virtualclassCont","nowebcam"))},handleUserMedia(a){localStorage.removeItem("dvid");var b=localStorage.getItem("audEnable");if(roles.isStudent()&&virtualclass.system.mediaDevices.hasMicrophone)virtualclass.settings.userAudioIcon();else if(virtualclass.system.mediaDevices.hasMicrophone)if(null!=b)if(b=JSON.parse(b),"false"===b.ac||!1===b.ac)virtualclass.gObj.audioEnable=
!1,virtualclass.user.control.audioDisable(!0);else{if("true"===b.ac||!0===b.ac)virtualclass.gObj.audioEnable=!0,virtualclass.user.control.audioWidgetEnable(!0)}else"undefined"!==typeof a&&virtualclass.user.control.audioWidgetEnable(!0);else virtualclass.user.control.audioDisable();b=a.getVideoTracks()[0];"undefined"!==typeof b?b.onended=()=>{if(roles.hasControls()){virtualclass.videoHost.clearTeacherVideoTime();virtualclass.system.mediaDevices.webcamErr.push("webcambusy");const a=document.getElementById("videoHostContainer");
null!==a&&a.classList.add("displayInterrupt");ioAdapter.mustSend({cf:"videoStop"})}}:virtualclass.system.mediaDevices.webcamErr.push("nopermission");n.video.tempStream=a;n.audio.init();n.audio.attachAudioStopHandler(a);b=chatContainerEvent.elementFromShadowDom(`#ml${virtualclass.gObj.uid}`);null!=b&&null!=b.getElementsByTagName("video")&&n.innerHandleUserMedia(virtualclass.gObj.uid);roles.hasAdmin()&&virtualclass.videoHost.isDomReady(()=>{virtualclass.videoHost.renderSelfVideo(a)});virtualclass.settings.userVideoIcon();
null==localStorage.getItem("prevApp")&&(roles.hasControls()?virtualclass.vutil.videoHandler("off"===virtualclass.vutil.selfVideoStatus()?"on":"off",!0):virtualclass.gObj.meetingMode&&virtualclass.vutil.videoHandler("off"));n.audio.audioContextReady&&!Object.prototype.hasOwnProperty.call(n.audio,"triggermaniPulateStream")&&(n.stream=n.video.tempStream,n.audio.actualManiPulateStream())},addUserRole(a,b){let d;a.dataset.role=b;"undefined"!==typeof b?(d="s"===b?"student":"teacher",a.classList.add(d)):
a.classList.add("student");"t"===virtualclass.gObj.uRole&&"student"===d&&document.getElementById("virtualclassCont").classList.remove("student")},innerHandleUserMedia(){if("undefined"!==typeof n){const a=n.video.tempStream;if("undefined"!==typeof a&&virtualclass.system.mediaDevices.hasWebcam){const b=n.video.createVideoElement();virtualclass.media.util.imageReplaceWithVideo(virtualclass.gObj.uid,b);n.video.insertTempVideo(b);n.video.tempVideoInit();n.video.myVideo=chatContainerEvent.elementFromShadowDom(`#video${virtualclass.gObj.uid}`);
virtualclass.adpt.attachMediaStream(n.video.myVideo,a);n.video.myVideo.muted=!0;n.stream=n.video.tempStream;n.video.myVideo.onloadedmetadata=()=>{n.video.startToStream()}}}},updateVideoContHeight(){const a=virtualclass.chat.boxHeight,b=document.getElementById("chat_div");b.scrollHeight>=a+1&&(b.style.overflowY="scroll");b.style.maxHeight=`${a}px`;virtualclass.isPlayMode&&(b.style.maxHeight=`${a+64}px`)},close(){Object.prototype.hasOwnProperty.call(virtualclass.media,"smallVid")&&clearInterval(virtualclass.media.smallVid)},
dispAllVideo(a){setTimeout(()=>{var b=document.getElementById(a);if(null!=b){b=b.getElementsByTagName("video");for(let a=0;a<b.length;a++)b[a].play()}},1040)},existVideoContainer(a){const b=chatContainerEvent.elementFromShadowDom(".userVideos","all");for(let d=0;d<b.length;d++)if(b[d].id===`video${a.id}`)return!0;return!1},handleUserMediaError(a){var b=virtualclass.lang.getString(a);b="object"===typeof a?virtualclass.lang.getString(a.name):b;null==b?virtualclass.view.createErrorMsg(a,"errorContainer",
"virtualclassAppFooterPanel"):"object"===typeof a?virtualclass.view.createErrorMsg(b,"errorContainer","virtualclassAppFooterPanel",{className:a.name}):virtualclass.view.createErrorMsg(b,"errorContainer","virtualclassAppFooterPanel");virtualclass.user.control.mediaWidgetDisable("vd");virtualclass.view.disappearBox("WebRtc");switch(a.name){case "NotFoundError":case "DevicesNotFoundError":a="nowebcam";break;case "SourceUnavailableError":a="webcambusy";break;case "PermissionDeniedError":case "SecurityError":a=
"nopermission";virtualclass.gObj.disableCamByUser=!0;break;case "NotAllowedError":a="nopermission";virtualclass.gObj.disableCamByUser=!0;break;default:a="rejected"}virtualclass.system.mediaDevices.webcamErr.push(a)},detectAudioWorklet:()=>{if("undefined"===typeof OfflineAudioContext)return!1;const a=new OfflineAudioContext(1,1,44100);return!(!a.audioWorklet||"function"!==typeof a.audioWorklet.addModule)}}}};(function(c){c.sharePt=function(){return{pptUrl:"",state:{},urlValue:"",localStoragFlag:!1,eventsObj:[],autoSlideTime:0,viewFlag:0,controlFlag:0,autoSlideFlag:0,autoSlideResumed:0,studentPpt:0,startFromFlag:0,startFrom:0,currId:"",order:[],init(b,a){virtualclass.orderList[virtualclass.currApp]||(virtualclass.orderList[virtualclass.currApp]=new OrderedList);this.pages={};this.order=[];this.autoSlideResumed=this.autoSlideTime=this.autoSlideFlag=0;"undefined"!==typeof virtualclass.previous&&"undefined"!==
...
6,b=>LZString._keyStrUriSafe.charAt(b))},decompressFromEncodedURIComponent(c){if(null==c)return"";if(""==c)return null;c=c.replace(/ /g,"+");return LZString._decompress(c.length,32,b=>LZString._getBaseValue(LZString._keyStrUriSafe,c.charAt(b)))},compress(c){return LZString._compress(c,16,b=>String.fromCharCode(b))},_compress(c,b,a){if(null==c)return"";let d,f;const g={},k={};let m,n,q="",t=2,z=3,v=2;const u=[];let p=0,r=0,A;for(A=0;A<c.length;A+=1)if(m=c[A],Object.prototype.hasOwnProperty.call(g,
m)||(g[m]=z++,k[m]=!0),n=q+m,Object.prototype.hasOwnProperty.call(g,n))q=n;else{if(Object.prototype.hasOwnProperty.call(k,q)){if(256>q.charCodeAt(0)){for(d=0;d<v;d++)p<<=1,r==b-1?(r=0,u.push(a(p)),p=0):r++;f=q.charCodeAt(0);for(d=0;8>d;d++)p=p<<1|f&1,r==b-1?(r=0,u.push(a(p)),p=0):r++,f>>=1}else{f=1;for(d=0;d<v;d++)p=p<<1|f,r==b-1?(r=0,u.push(a(p)),p=0):r++,f=0;f=q.charCodeAt(0);for(d=0;16>d;d++)p=p<<1|f&1,r==b-1?(r=0,u.push(a(p)),p=0):r++,f>>=1}t--;0==t&&(t=Math.pow(2,v),v++);delete k[q]}else for(f=
g[q],d=0;d<v;d++)p=p<<1|f&1,r==b-1?(r=0,u.push(a(p)),p=0):r++,f>>=1;t--;0==t&&(t=Math.pow(2,v),v++);g[n]=z++;q=String(m)}if(""!==q){if(Object.prototype.hasOwnProperty.call(k,q)){if(256>q.charCodeAt(0)){for(d=0;d<v;d++)p<<=1,r==b-1?(r=0,u.push(a(p)),p=0):r++;f=q.charCodeAt(0);for(d=0;8>d;d++)p=p<<1|f&1,r==b-1?(r=0,u.push(a(p)),p=0):r++,f>>=1}else{f=1;for(d=0;d<v;d++)p=p<<1|f,r==b-1?(r=0,u.push(a(p)),p=0):r++,f=0;f=q.charCodeAt(0);for(d=0;16>d;d++)p=p<<1|f&1,r==b-1?(r=0,u.push(a(p)),p=0):r++,f>>=1}t--;
0==t&&(t=Math.pow(2,v),v++);delete k[q]}else for(f=g[q],d=0;d<v;d++)p=p<<1|f&1,r==b-1?(r=0,u.push(a(p)),p=0):r++,f>>=1;t--;0==t&&v++}f=2;for(d=0;d<v;d++)p=p<<1|f&1,r==b-1?(r=0,u.push(a(p)),p=0):r++,f>>=1;for(;;)if(p<<=1,r==b-1){u.push(a(p));break}else r++;return u.join("")},decompress(c){return null==c?"":""==c?null:LZString._decompress(c.length,32768,b=>c.charCodeAt(b))},_decompress(c,b,a){const d=[];let f=4,g=4,k=3;const m=[];var n;let q,t,z,v;const u=LZString._f;var p=a(0),r=b,A=1;for(n=0;3>n;n+=
1)d[n]=n;q=0;z=Math.pow(2,2);for(v=1;v!=z;)t=p&r,r>>=1,0==r&&(r=b,p=a(A++)),q|=(0<t?1:0)*v,v<<=1;switch(q){case 0:q=0;z=Math.pow(2,8);for(v=1;v!=z;)t=p&r,r>>=1,0==r&&(r=b,p=a(A++)),q|=(0<t?1:0)*v,v<<=1;var D=u(q);break;case 1:q=0;z=Math.pow(2,16);for(v=1;v!=z;)t=p&r,r>>=1,0==r&&(r=b,p=a(A++)),q|=(0<t?1:0)*v,v<<=1;D=u(q);break;case 2:return""}n=d[3]=D;for(m.push(D);;){if(A>c)return"";q=0;z=Math.pow(2,k);for(v=1;v!=z;)t=p&r,r>>=1,0==r&&(r=b,p=a(A++)),q|=(0<t?1:0)*v,v<<=1;switch(D=q){case 0:q=0;z=Math.pow(2,
8);for(v=1;v!=z;)t=p&r,r>>=1,0==r&&(r=b,p=a(A++)),q|=(0<t?1:0)*v,v<<=1;d[g++]=u(q);D=g-1;f--;break;case 1:q=0;z=Math.pow(2,16);for(v=1;v!=z;)t=p&r,r>>=1,0==r&&(r=b,p=a(A++)),q|=(0<t?1:0)*v,v<<=1;d[g++]=u(q);D=g-1;f--;break;case 2:return m.join("")}0==f&&(f=Math.pow(2,k),k++);if(d[D])D=d[D];else if(D===g)D=n+n[0];else return null;m.push(D);d[g++]=n+D[0];f--;n=D;0==f&&(f=Math.pow(2,k),k++)}}};"undefined"!==typeof module&&null!=module&&(module.exports=LZString);window.addEventListener("message",c=>{if(c.origin===window.location.origin)if("gotScreen"===c.data.type){let b;if(""===c.data.sourceId){virtualclass.vutil.beforeSend({sd:!0,cf:"colorIndicator"});c=Error("NavigatorUserMediaError");c.name="PERMISSION_DENIED";if(Object.prototype.hasOwnProperty.call(virtualclass,"ss"))virtualclass.ss.onError(c);if("SharePresentation"===virtualclass.currApp||"DocumentShare"===virtualclass.currApp)c=document.querySelector("#dashboardnav button"),null!=c&&c.click();roles.hasControls()&&
...
(function(c){let b,a,d,f,g,k,m,n,q;const t=function(a){null==k&&(k=document.querySelector("#virtualclassScreenShareLocal"));null==m&&(m=document.createElement("canvas"),m.id="virtualclassScreenShareLocalVideoNew",k.appendChild(m),m=document.querySelector("#virtualclassScreenShareLocalVideoNew"));m.width=a.width;m.height=a.height;0>=virtualclass.studentScreen.base.width&&(virtualclass.studentScreen.base.width=m.width,virtualclass.studentScreen.base.height=m.height);null==n&&(n=document.querySelector("#virtualclassScreenShareLocalVideoNew").getContext("2d"));
...
0,virtualclass.studentScreen.setDimension(),t(q),virtualclass.studentScreen.triggerNormalView(a.data.stype),(a=virtualclass.vutil.getUserAllInfo(a.data.uid,virtualclass.connectedUsers))&&"t"!==a.role&&roles.isStudent()&&!virtualclass.gObj.studentSSstatus.shareToAll&&receiveFunctions.sview({message:"firstSs"})):t(q))});c.studentScreen=function(){return{scale:1,SCALE_FACTOR:1.04,szoom:!1,base:{width:0,height:0},ssProcess(a,b,d){a=new Uint8ClampedArray(a);let c,f;switch(a[0]){case 102:case 202:if(a[1]===
...
u,f):this.createAssignControl(a,b,u);else if("audio"===c[g]){var r=this.createControllerElement(b,"contrAud");var A=r[0];m=r[1];a.appendChild(A);A=z&&Object.prototype.hasOwnProperty.call(v,"aud")?!!v.aud:!0;virtualclass.user.control.changeAttribute(b,m,A,"audio","aud");p&&m.addEventListener("click",()=>{d.control.init.call(d,m)});roles.hasAdmin()&&"false"===m.dataset.audioDisable&&(A=localStorage.getItem("allAudioAction"),null!=A&&"disable"===A&&m.click())}else"chat"===c[g]?(r=this.createControllerElement(b,
"contrChat"),A=r[0],n=r[1],a.appendChild(A),p&&n.addEventListener("click",()=>{d.control.init.call(d,n)}),q=z&&Object.prototype.hasOwnProperty.call(v,"chat")?!!v.chat:!0,virtualclass.user.control.changeAttribute(b,n,q,"chat","chat")):"editorRich"===c[g]||"editorCode"===c[g]?roles.hasAdmin()&&(t=z&&Object.prototype.hasOwnProperty.call(v,c[g])?!!v[c[g]]:!1,r=this.createControllerElement(b,`contr${c[g]}`),A=r[0],r=r[1],A.className+=` controller${c[g]}`,a.appendChild(A),virtualclass.currApp!==virtualclass.vutil.capitalizeFirstLetter(c[g])&&
(A.style.display="none"),virtualclass.user.control.changeAttribute(b,r,t,c[g],c[g]),p&&r.addEventListener("click",d.closureEditor(d,r))):"RaiseHand"===c[g]&&(rhEnable=z&&Object.prototype.hasOwnProperty.call(v,"raiseHand")?!!v.raiseHand:!0,virtualclass.user.control.changeAttribute(b,n,q,"RaiseHand","RaiseHand"))},closureEditor(a,b){return function(){a.control.init.call(a,b)}},control:{toggleDisplayWriteModeMsgBox(a,d){let c=b.getElementById(`${a}writeModeBox`);var g=virtualclass.lang.getString("readonlymode"),
...
2+c)+(2===c?"":1===c?"=":"==")},base64DecToArrInt(a){a=base64DecToArr(a);const b=new Int8Array(a.length);for(let c=0;c<a.length;c++)b[c]=128<=a[c]?-1*(a[c]-127):a[c];return b},base64DecToArr(a,b){a=a.replace(/[^A-Za-z0-9\+\/]/g,"");const c=a.length;b=b?Math.ceil((3*c+1>>2)/b)*b:3*c+1>>2;const d=new Uint8Array(b);for(var k,m=0,n=0,q=0;q<c;q++){k=q&3;var t=a.charCodeAt(q);m|=(64<t&&91>t?t-65:96<t&&123>t?t-71:47<t&&58>t?t+4:43===t?62:47===t?63:0)<<6*(3-k);if(3===k||1===c-q){for(k=0;3>k&&n<b;k++,n++)d[n]=
...
c||[];if(a.length!==this.baseLength)throw Error("The operation's base length must be equal to the string's length.");const d=[];let f=0,g,m,n=0;const {ops:q}=this;for(let p=0,r=q.length;p<r;p++){const r=q[p];if(r.isRetain()){if(n+r.chars>a.length)throw Error("Operation can't retain more characters than are left in the string.");d[f++]=a.slice(n,n+r.chars);for(g=0;g<r.chars;g++){var A=b[n+g]||{};const a={};for(m in A)a[m]=A[m],k.assert(!1!==a[m]);for(m in r.attributes)!1===r.attributes[m]?delete a[m]:
a[m]=r.attributes[m],k.assert(!1!==a[m]);c.push(a)}n+=r.chars}else if(r.isInsert())for(d[f++]=r.text,g=0;g<r.text.length;g++){A={};for(m in r.attributes)A[m]=r.attributes[m],k.assert(!1!==A[m]);c.push(A)}else n+=r.chars}if(n!==a.length)throw Error("The operation didn't operate on the whole string.");a=d.join("");k.assert(a.length===c.length);return a};a.prototype.invert=function(b){let c=0;const d=new a,{ops:f}=this;for(let a=0,g=f.length;a<g;a++){const g=f[a];g.isRetain()?(d.retain(g.chars),c+=g.chars):
g.isInsert()?d.delete(g.text.length):(d.insert(b.slice(c,c+g.chars)),c+=g.chars)}return d};a.prototype.compose=function(b){function c(a,b,c){const d={};let f;for(f in a)d[f]=a[f];for(f in b)c&&!1===b[f]?delete d[f]:d[f]=b[f];return d}if(this.targetLength!==b.baseLength)throw Error("The base length of the second operation has to be the target length of the first operation");const d=new a,f=this.clone().ops;b=b.clone().ops;let g=0,k=0,m=f[g++],p=b[k++],r;for(;"undefined"!==typeof m||"undefined"!==typeof p;)if(m&&
m.isDelete())d.delete(m.chars),m=f[g++];else if(p&&p.isInsert())d.insert(p.text,p.attributes),p=b[k++];else{if("undefined"===typeof m)throw Error("Cannot compose operations: first operation is too short.");if("undefined"===typeof p)throw Error("Cannot compose operations: first operation is too long.");if(m.isRetain()&&p.isRetain())r=c(m.attributes,p.attributes),m.chars>p.chars?(d.retain(p.chars,r),m.chars-=p.chars,p=b[k++]):m.chars===p.chars?(d.retain(m.chars,r),m=f[g++],p=b[k++]):(d.retain(m.chars,
r),p.chars-=m.chars,m=f[g++]);else if(m.isInsert()&&p.isDelete())m.text.length>p.chars?(m.text=m.text.slice(p.chars),p=b[k++]):m.text.length===p.chars?(m=f[g++],p=b[k++]):(p.chars-=m.text.length,m=f[g++]);else if(m.isInsert()&&p.isRetain())r=c(m.attributes,p.attributes,!0),m.text.length>p.chars?(d.insert(m.text.slice(0,p.chars),r),m.text=m.text.slice(p.chars),p=b[k++]):m.text.length===p.chars?(d.insert(m.text,r),m=f[g++],p=b[k++]):(d.insert(m.text,r),p.chars-=m.text.length,m=f[g++]);else if(m.isRetain()&&
...
if("undefined"===typeof n)throw Error("Cannot transform operations: first operation is too long.");if(m.isRetain()&&n.isRetain()){const q=a.transformAttributes(m.attributes,n.attributes);if(m.chars>n.chars){var r=n.chars;m.chars-=n.chars;n=c[k++]}else m.chars===n.chars?(r=n.chars,m=b[g++],n=c[k++]):(r=m.chars,n.chars-=m.chars,m=b[g++]);d.retain(r,q[0]);f.retain(r,q[1])}else if(m.isDelete()&&n.isDelete())m.chars>n.chars?(m.chars-=n.chars,n=c[k++]):m.chars===n.chars?(m=b[g++],n=c[k++]):(n.chars-=m.chars,
m=b[g++]);else if(m.isDelete()&&n.isRetain())m.chars>n.chars?(r=n.chars,m.chars-=n.chars,n=c[k++]):m.chars===n.chars?(r=n.chars,m=b[g++],n=c[k++]):(r=m.chars,n.chars-=m.chars,m=b[g++]),d.delete(r);else if(m.isRetain()&&n.isDelete())m.chars>n.chars?(r=n.chars,m.chars-=n.chars,n=c[k++]):m.chars===n.chars?(r=m.chars,m=b[g++],n=c[k++]):(r=m.chars,n.chars-=m.chars,m=b[g++]),f.delete(r);else throw Error("The two operations aren't compatible");}return[d,f]};return a}();c.TextOperation=b})(window);(function(c){const b=function(a){function b(a,b){this.wrapped=a;this.meta=b}function c(a,b){for(const c in a)Object.prototype.hasOwnProperty.call(a,c)&&(b[c]=a[c])}function g(a,b){return a&&"object"===typeof a&&"function"===typeof a.transform?a.transform(b):a}b.prototype.apply=function(){return this.wrapped.apply.apply(this.wrapped,arguments)};b.prototype.invert=function(){const {meta:a}=this;return new b(this.wrapped.invert.apply(this.wrapped,arguments),a&&"object"===typeof a&&"function"===typeof a.invert?
...
for(b in a)if(Object.prototype.hasOwnProperty.call(a,b)){var c=f.getClientObject(b);a[b].name&&c.setName(a[b].name);({selection:c}=a[b]);c?f.clients[b].updateSelection(f.transformSelection(Selection.fromJSON(c))):f.clients[b].removeSelection()}},reconnect(){f.serverReconnect()}})}function n(a){return a[a.length-1]}const {Client:q}=c,{Cursor:t}=c,{UndoManager:z}=c,{TextOperation:v}=c,{WrappedOperation:u}=c;f.prototype.invert=function(){return new f(this.cursorAfter,this.cursorBefore)};f.prototype.compose=
...
(c,d)=>{a(null!==d);const f=new k(0,n);let g=f;b.pos>c&&(g.next=new k(b.pos-c,d.annotation),g=g.next);for(;b.end()>c+d.length;)c+=d.length,d=d.next;c=c+d.length-b.end();0<c&&(g.next=new k(c,d.annotation));return f.next})};g.prototype.updateSpan=function(b,c){0!==b.length&&this.wrapOperation_(b,(d,f)=>{a(null!==f);const g=new k(0,n);let m=g;var q=d,A=b.pos-q;a(A<f.length);0<A&&(m.next=new k(A,f.annotation),m=m.next,q+=m.length);for(;null!==f&&b.end()>=d+f.length;)q=d+f.length-q,m.next=new k(q,c(f.annotation,
q)),m=m.next,d+=f.length,f=f.next,q=d;A=b.end()-q;0<A&&(a(A<f.length),m.next=new k(A,c(f.annotation,A)),m=m.next,q+=m.length,m.next=new k(d+f.length-q,f.annotation));return g.next})};g.prototype.wrapOperation_=function(a,c){if(0>a.pos)throw Error("Span start cannot be negative.");const d=[],g=[];a=this.getAffectedNodes_(a);if(null!==a.start){var m=a.end.next;a.end.next=null}else m=a.succ;var n=c(a.startPos,a.start);let r=!1;c=!1;if(n){this.mergeNodesWithSameAnnotations_(n);let b;a.pred&&a.pred.annotation.equals(n.annotation)?
...
case "u":g(a,b.withTextFormatting(b.textFormatting.underline(!0)),c);break;case "i":case "em":g(a,b.withTextFormatting(b.textFormatting.italic(!0)),c);break;case "s":g(a,b.withTextFormatting(b.textFormatting.strike(!0)),c);break;case "font":d=a.getAttribute("face");var f=a.getAttribute("color"),r=parseInt(a.getAttribute("size"));d&&(b=b.withTextFormatting(b.textFormatting.font(d)));f&&(b=b.withTextFormatting(b.textFormatting.color(f)));r&&(b=b.withTextFormatting(b.textFormatting.fontSize(r)));g(a,
...
b+m.chars),f=0;f<d.length;f++){var r={};for(const a in m.attributes){var A=m.attributes[a];const b=d[f].attributes[a];!1===A?b&&(r[a]=b):A!==b&&(r[a]=b||!1)}n.retain(d[f].length,r);b+=d[f].length}else if(m.isInsert())n.delete(m.text.length);else if(m.isDelete()){r=c.getRange(c.posFromIndex(b),c.posFromIndex(b+m.chars));d=this.rtcm.getAttributeSpans(b,b+m.chars);for(f=A=0;f<d.length;f++)n.insert(r.substr(A,d[f].length),d[f].attributes),A+=d[f].length;b+=m.chars}}return new m(n,a.meta.invert())};return a}();
...
{};this.dirtyLines_=[]}function c(a,b){return a.line-b.line||a.ch-b.ch}function f(a){if(0===a.length)return 0;let b=0;for(let c=0;c<a.length;c++)b+=a[c].length;return b+a.length-1}function g(a){this.attributes=a||{}}function k(a,b){const c=a[b];a[b]=function(){c.apply(a,arguments)}}const {AnnotationList:m}=b,{Span:n}=b,{utils:q}=b,t=b.AttributeConstants,z={c:"color",bc:"background-color",fs:"font-size",li(a){return`padding-left: ${40*a}px`}},v={};q.makeEventEmitter(a,["change","attributesChange",
...
b,c,d,f){const k=[];let m=a;const r=this;this.annotationList_.updateSpan(new n(a,b-a),(a,b)=>{const n={};for(var A in a.attributes)n[A]=a.attributes[A];n[t.LINE_SENTINEL]&&!f||c(n);A={};const D={};r.computeChangedAttributes_(a.attributes,n,A,D);a:{for(const b in A){a=!1;break a}a=!0}a||k.push({start:m,end:m+b,attributes:A,attributesInverse:D,origin:d});m+=b;return new g(n)});0<k.length&&this.trigger("attributesChange",this,k)};a.prototype.computeChangedAttributes_=function(a,b,c,d){const f={};let g;
...
function(a){return q.elt("div",`${a}.`,{class:"vceditor-list-left"})};a.prototype.makeUnorderedListElement_=function(){return q.elt("div","\u2022",{class:"vceditor-list-left"})};a.prototype.toggleTodo=function(a){const b=t.LIST_TYPE,c=this.getCurrentLineAttributes_();let d;b in c&&("t"===c[b]||"tc"===c[b])?"t"===c[b]?d="tc":"tc"===c[b]&&(d=a?"t":!1):d="t";this.setLineAttribute(b,d)};a.prototype.makeTodoListElement_=function(a,b){const c={type:"checkbox",class:"vceditor-todo-left"};a&&(c.checked=!0);
a=q.elt("input",!1,c);const d=this;q.on(a,"click",q.stopEventAnd(a=>{d.codeMirror.setCursor({line:b(),ch:1});d.toggleTodo(!0)}));return a};a.prototype.lineIsListItemOrIndented_=function(a){a=this.getLineAttributes_(a);return!1!==(a[t.LIST_TYPE]||!1)||0!==(a[t.LINE_INDENT]||0)};a.prototype.onCursorActivity_=function(){const a=this;clearTimeout(a.cursorTimeout);a.cursorTimeout=setTimeout(()=>{a.updateCurrentAttributes_()},1)};a.prototype.getCurrentAttributes_=function(){this.currentAttributes_||this.updateCurrentAttributes_();
...
c.formatting.attributes);for(let a=0;a<c.textPieces.length;a++)d(c.textPieces[a]);d("\n")}const k=[];for(let a=0;a<c.length;a++)c[a]instanceof b.Line?g(c[a]):d(c[a]);return k};b=b||{};b.Vceditor=function(a){function d(a,b,g,k){if(!(this instanceof d))return new d(a,b,g,k);if(!r&&!A)throw Error("Couldn't find CodeMirror or ACE.  Did you forget to include codemirror.js or ace.js?");if(r&&b instanceof r)this.codeMirror_=this.editor_=b,a=this.codeMirror_.getValue();else if(A&&b&&b.session instanceof A.EditSession){if(this.ace_=
this.editor_=b,a=this.ace_.getValue(),""!==a)throw Error("Can't initialize Vceditor with an ACE instance that already contains text.");}else this.codeMirror_=this.editor_=new r(b);a=this.codeMirror_?this.codeMirror_.getWrapperElement():this.ace_.container;this.vcEditorWrapper_=p.elt("div",null,{class:"vceditor"});a.parentNode.replaceChild(this.vcEditorWrapper_,a);this.vcEditorWrapper_.appendChild(a);p.on(a,"dragstart",p.stopEvent);this.editor_.vceditor=this;this.options_=g||{};this.getOption("richTextShortcuts",
!1)&&(r.keyMap.richtext||this.initializeKeyMap_(),this.codeMirror_.setOption("keyMap","richtext"),this.vcEditorWrapper_.className+=" vceditor-richtext");this.imageInsertionUI=this.getOption("imageInsertionUI",!0);this.getOption("richTextToolbar",!1)&&(this.addToolbar_(),this.vcEditorWrapper_.className+=" vceditor-richtext vceditor-with-toolbar",virtualclass.editorRich.initAttachTool());this.addPoweredByLogo_();this.codeMirror_&&this.codeMirror_.refresh();this.getOption("userColor",f("JqrwCaZm29-CLg-Pfz2"));
...
b.appendChild(c);c=virtualclass.view.customCreateElement("div","gdRpt");c.className="tab-pane fade";k=this.UI.gradeReportLayout();c.appendChild(k);b.appendChild(c);a.appendChild(b)},gradeReport(a,b,c,k,m,n,q){const d=document.getElementById("qzReTbody");if(d){let f=document.getElementById(`user${q}`);null!==f&&(a=f.childNodes[0].innerHTML,f.parentNode.removeChild(f));f=document.createElement("tr");f.id=`user${q}`;d.appendChild(f);q=document.createElement("th");q.scope="row";q.innerHTML=a;f.appendChild(q);
a=document.createElement("td");a.innerHTML=b;f.appendChild(a);b=document.createElement("td");b.innerHTML=c;f.appendChild(b);c=document.createElement("td");c.innerHTML=`${k}%`;f.appendChild(c);k=document.createElement("td");k.innerHTML=m;f.appendChild(k);m=document.createElement("td");m.innerHTML=n;f.appendChild(m)}},quizTimer(a,b,c){function d(){g="asc"===c?a+((Date.now()-f)/1E3|0):a-((Date.now()-f)/1E3|0);q=g/3600|0;t=g%3600/60|0;z=g%60|0;q=10>q?`0${q}`:q;t=10>t?`0${t}`:t;z=10>z?`0${z}`:z;b.textContent=
`${q}:${t}:${z}`;if(0>=g&&("asc"!==c&&(b.textContent="00 : 00 : 00 "),f=0,null!=virtualclass.gObj.CDTimer&&clearInterval(virtualclass.gObj.CDTimer),ioAdapter.mustSend({quiz:{quizMsg:"quizTimeEnd",quizId:virtualclass.view.qzid},cf:"quiz"}),roles.hasControls())){let a=document.getElementById("closeQzBt");null!=a&&(a.disabled=!0)}}c="undefined"!==typeof c?c:"desc";let f=Date.now(),g,q,t,z;null!=virtualclass.gObj.CDTimer&&clearInterval(virtualclass.gObj.CDTimer);"asc"!==c&&d();virtualclass.gObj.CDTimer=
...
candidate:c.candidate,pt:b},a)}}function q(a,b){p[b]=a;virtualclass.multiVideo.removeUser(b);b=$(`<div class='videoCont remoteVideo'  id='vid${b}' data-userid='${b}' data-totaluser=''></div>`);const d=$("<video  class='videoBox' autoplay></video>");d.attr({src:c.URL.createObjectURL(a),autoplay:"autoplay"});b.append(d);$("#videosWrapper").append(b);virtualclass.multiVideo.updateVideoLength();b.click(function(){r.UI.displayMainVideo(this)});"undefined"!==typeof updateActiveTime&&clearTimeout(updateActiveTime);
updateActiveTime=setTimeout(()=>{var a=0;for(var b in p){var c=p[b].getVideoTracks()[0];const d=document.querySelector(`#vid${b}`);null!=c?(a++,null!=d&&(d.classList.add("vshow"),d.classList.remove("vhide"))):null!=d&&(d.classList.add("vhide"),d.classList.remove("vshow"))}c="one";4<a?c="morethanfour":2<a?c="four":1<a&&(c="two");a=document.querySelector("#virtualclassCont");b=document.querySelector("#videoConfrence");a.setAttribute("data-totalUser",c);b.setAttribute("data-totalUser",c)},3E3)}function t(a){return function(b){q(b.stream,
a)}}function z(a){u[a]=new RTCPeerConnection(pc_config);u[a].onicecandidate=n(a,"apc");u[a].onaddstream=t(a);"undefined"!==typeof _localStream&&u[a].addStream(_localStream);u[a].createOffer(b=>{u[a].setLocalDescription(b);ioAdapter.sendUser({cf:"mvid",sdp:b,offer:!0},a)},b)}let v,u;const p={},r={init(){this.UI.container();this.start();var a=document.querySelector("#videosWrapper .videoCont.selfVideo");a.id=`vid${virtualclass.gObj.uid}`;a.setAttribute("data-userid",virtualclass.gObj.uid);const b=this;
...
(d?IDBIndex:IDBObjectStore).prototype&&(f||r.includes(c)))return a=async function(a,...b){a=this.transaction(a,f?"readwrite":"readonly");let g=a.store;d&&(g=g.index(b.shift()));b=g[c](...b);f&&await a.done;return b},D.set(b,a),a}}const k=(a,b)=>b.some(b=>a instanceof b);let m,n;const q=new WeakMap,t=new WeakMap,z=new WeakMap,v=new WeakMap,u=new WeakMap;let p={get(a,b,c){if(a instanceof IDBTransaction){if("done"===b)return t.get(a);if("objectStoreNames"===b)return a.objectStoreNames||z.get(a);if("store"===
b)return c.objectStoreNames[1]?void 0:c.objectStore(c.objectStoreNames[0])}return f(a[b])},has(a,b){return a instanceof IDBTransaction&&("done"===b||"store"===b)?!0:b in a}};const r=["get","getKey","getAll","getAllKeys","count"],A=["put","add","delete","clear"],D=new Map;p=(a=>({get:(b,c,d)=>g(b,c)||a.get(b,c,d),has:(b,c)=>!!g(b,c)||a.has(b,c)}))(p);c.virtualclassOpenDB=function(a,b,{blocked:c,upgrade:d,blocking:g}={}){const k=indexedDB.open(a,b);a=f(k);d&&k.addEventListener("upgradeneeded",a=>{d(f(k.result),
...
(g=f.parentNode.dataset.event,k=f.parentNode.parentNode):this.event.values.includes(f.parentNode.parentNode.dataset.event)&&(g=f.parentNode.parentNode.dataset.event,k=f.parentNode.parentNode.parentNode);if(g){let m,n,q=null;c=k.dataset.component;let t=k.dataset.componentId;if(k.dataset.componentId&&"save"!==g)if(a=t,"reply"===g)a=null,q=t;else if("edit"===g||"markAnswer"===g||"delete"===g)q=k.dataset.parent?k.dataset.parent:null;if("save"===g){if("note"===c)m=f.value,n="create";else if("bookmark"===
