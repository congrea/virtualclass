...
a){if(!virtualclass.videoUl.player){virtualclass.videoUl.player=videojs("dispVideo",{html5:{hls:{overrideNative:!0}},controlBar:{pictureInPictureToggle:!1}});if(roles.hasControls()){$(".vjs-autoPlay-button").length||virtualclass.videoUl.UI.appendAutoPlayButton(virtualclass.videoUl.player);const a=document.getElementById("autoPlayListBtn");a&&(a.className=virtualclass.videoUl.autoPlayClass)}virtualclass.videoUl.UI.attachPlayerHandler(virtualclass.videoUl.player,a,b)}virtualclass.videoUl.lastSeek=0;
virtualclass.videoUl.UI.onEndedHandler(virtualclass.videoUl.player,a,b);virtualclass.videoUl.setPlayerUrlTime&&clearTimeout(virtualclass.videoUl.setPlayerUrlTime);virtualclass.videoUl.setPlayerUrlTime=setTimeout(()=>{"object"===typeof virtualclass.videoUl.player&&virtualclass.videoUl.UI.setPlayerUrl(virtualclass.videoUl.player,b)},100)},attachPlayerHandler(b){this.attachPlayer||(this.attachPlayer=!0,b.on("pause",a=>{roles.hasControls()&&ioAdapter.mustSend({videoUl:"pause",cf:"videoUl",currTime:b.currentTime()});
virtualclass.videoUl.isPaused=!0}),b.on("play",a=>{roles.hasControls()&&ioAdapter.mustSend({videoUl:{play:b.currentTime()},cf:"videoUl",videoTime:virtualclass.vutil.localToUTC()});virtualclass.videoUl.isPaused=!1}),b.on("fullscreenchange",a=>{setTimeout(()=>{virtualclass.vutil.showFullScreenButtonIfNeed()},0)}))},switchDisplay(b,a){var d=document.getElementById("dispVideo");d?d.style.display="block":virtualclass.videoUl.UI.createVideoElem(b);d=document.getElementById("messageLayoutVideo");"undefined"===
...
null==document.querySelector("#DocumentShareDashboard .qq-gallery")&&virtualclass.vutil.modalPopup("docs",["docsuploadContainer"]),virtualclass.dashboard.modalCloseHandler()))},createMainContent(a,b,c){},createSlides(a,b){let d=document.querySelector("#notesContainer");null==d&&(d=document.createElement("div"),d.className="notes",d.id="notesContainer");for(let a=0;a<b.length;a++)if(null==document.querySelector(`#note${b[a].id}`)){const d=document.createElement("div");d.id=`note${b[a].id}`;d.className=
"note";d.dataset.slide=b[a].id;d.dataset.status="true"===d.dataset.statuS||!0===d.dataset.statuS?1:"false"===d.dataset.status||!1===d.dataset.status?0:b[a].status}a.appendChild(d);roles.hasControls()&&(this.createNavigation(a,"prev"),this.createNavigation(a,"next"))},createNavigation(a,b){if(null==document.querySelector(`#docs${b}`)){const d=document.createElement("span");d.className=`${"nvgt "}${b}`;d.id=`docs${b}`;a.appendChild(d)}},createDocsNav(a,b){let d=document.getElementById("listDocs");null==
d&&(d=document.createElement("div"),d.id="listDocs",a.appendChild(d));a=this.createDocsNavLink(b);this.attachDocsNav(a,b);d.appendChild(a)},createDocsNavLink(a){const b=document.createElement("div");b.id=`link${a}`;b.className="linkdoc";b.innerHTML=a;b.dataset.screen=a;return b},attachDocsNav(a,b){a.onclick=virtualclass.dts.docs.goToDocs(b)}},createNoteNav(a){let b;virtualclass.orderList[this.appName].ol.order&&this.indexNav.init();for(let d=0;d<virtualclass.orderList[this.appName].ol.order.length;d++)"object"!==
typeof this.notes[virtualclass.orderList[this.appName].ol.order[d]]&&(b="true"===this.allNotes[virtualclass.orderList[this.appName].ol.order[d]].status||1===+this.allNotes[virtualclass.orderList[this.appName].ol.order[d]].status?1:0,this.notes[virtualclass.orderList[this.appName].ol.order[d]]=new virtualclass.page("screen-docs","notes","virtualclassDocumentShare","dts",b),this.notes[virtualclass.orderList[this.appName].ol.order[d]].init(virtualclass.orderList[this.appName].ol.order[d],`note_${this.allNotes[virtualclass.orderList[this.appName].ol.order[d]].lc_content_id}_${virtualclass.orderList[this.appName].ol.order[d]}`),
"undefined"===typeof a&&this.noteStatus(virtualclass.orderList[this.appName].ol.order[d],b)),0===d&&virtualclass.vutil.hideUploadMsg("docsuploadContainer");roles.hasControls()&&(this.indexNav.shownPage(this.indexNav.width),this.indexNav.addActiveNavigation(),document.querySelector("#dcPaging").addEventListener("change",function(){virtualclass.dts.docs.goToNavs(this.value)()}));document.querySelector(".congrea.teacher  #dashboardContainer .modal-header button.enable")||virtualclass.vutil.showFinishBtn();
this.indexNav.setTotalPages(virtualclass.orderList[virtualclass.dts.appName].ol.order.length)},addNoteHidClass(a,b,c){b>c?a.classList.add("hid","right"):a.classList.add("shw")},indexHandler(a){virtualclass.dts.docs.goToNavs(a)},createNoteNavAlt(a){let b;for(let d=0;d<virtualclass.orderList[this.appName].ol.order.length;d++)b="true"===this.allNotes[virtualclass.orderList[this.appName].ol.order[d]].status||1===+this.allNotes[virtualclass.orderList[this.appName].ol.order[d]].status?1:0,this.notes[virtualclass.orderList[this.appName].ol.order[d]]=
new virtualclass.page("screen-docs","notes","virtualclassDocumentShare","dts",b),this.notes[virtualclass.orderList[this.appName].ol.order[d]].init(virtualclass.orderList[this.appName].ol.order[d],`note_${this.allNotes[virtualclass.orderList[this.appName].ol.order[d]].lc_content_id}_${virtualclass.orderList[this.appName].ol.order[d]}`),"undefined"===typeof a&&this.noteStatus(virtualclass.orderList[this.appName].ol.order[d],b)},removeNoteNav(a){const b=document.querySelector(`#linknotes${a}`);null!=
