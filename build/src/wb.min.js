...
"virtualclassEditorCode","virtualclassEditorCodeBody");virtualclass.poll=d.poll();virtualclass.quiz=d.quiz();virtualclass.videoUl=d.videoUl();virtualclass.videoHost=d.videoHost;virtualclass.precheck=preCheck;virtualclass.page=page;console.log("==== session clear zoom object ready ");virtualclass.network=new Network;virtualclass.gesture=gesture;this.serverData=serverData;roles.hasControls()&&this.serverData.fetchAllData();null!=localStorage.uRole&&(virtualclass.gObj.uRole=localStorage.uRole,b.classList.add(virtualclass.vutil.getClassName(virtualclass.gObj.uRole)));
"undefined"===typeof c||null==c?this.makeAppReady(a,"byclick"):this.makeAppReady(a,"byclick",c);this.system.check();this.vutil.isSystemCompatible();this.system.mediaDevices.getMediaDeviceInfo();this.pageVisible=virtualclass.vutil.pageVisible();a==virtualclass.apps.ss&&this.system.setAppDimension();virtualclass.vutil.createReclaimButtonIfNeed();virtualclass.user.assignRole(virtualclass.gObj.uRole,a);this.media=new d.media;a=localStorage.getItem("precheck");null!=a&&(a=JSON.parse(a));virtualclass.makePreCheckAvailable?
virtualclass.precheck.init():(virtualclass.makeReadySocket(),virtualclass.isPlayMode||virtualclass.gesture.initClassJoin());b.classList.remove("loading");virtualclass.gObj.precheckScrn=!1;virtualclass.gObj.meetingMode||virtualclass.videoHost.init(320,240);virtualclass.vutil.videoController();a=document.getElementById("precheckSetting");a.addEventListener("click",()=>{virtualclass.gObj.precheckScrn=!0;virtualclass.precheck.init(virtualclass.precheck)});this.raiseHand=d.raiseHand;this.raiseHand.init();
this.appSetting=d.appSetting;this.appSetting.init();virtualclass.colorSelector=d.colorSelector;virtualclassSetting.theme.selectedColor&&this.colorSelector.makeThemeReady();a=document.getElementById("virtualclassCont");null!=a&&a.classList.add(virtualclass.system.mybrowser.name);a=document.getElementById("fullScreenButton");null!=a&&a.addEventListener("click",virtualclass.vutil.Fullscreen);a=document.getElementById("fullScreenExitButton");null!=a&&a.addEventListener("click",virtualclass.vutil.closeFullscreen);
document.onfullscreenchange=function(b){virtualclass.gObj.fullScreenMode?(document.querySelector("#fullScreenButton").style.display="block",document.querySelector("#fullScreenExitButton").style.display="none",virtualclass.gObj.fullScreenMode=!1):(document.querySelector("#fullScreenButton").style.display="none",document.querySelector("#fullScreenExitButton").style.display="block",virtualclass.gObj.fullScreenMode=!0)}},makeReadySocket(){if(!virtualclass.vutil.isPlayMode()){const a=this;var b=setInterval(()=>
{2<=a.gObj.editorInitDone&&(a.initSocketConn(),clearInterval(b))},100)}},initSocketConn(){this.system.webSocket&&(io.init(virtualclass.uInfo),d.userdata=virtualclass.uInfo)},html:{id:"virtualclassCont",optionsClass:"appOptions",init(b){this.virtualclass=b},leftAppBar(){if(!(0<document.getElementsByClassName("appOptions").length)){document.querySelector("#virtualclassApp #virtualclassAppLeftPanel");var b=virtualclass.getTemplate("appTools")();virtualclass.vutil.insertAppLayout(b);virtualclass.gObj.hasOwnProperty("errNotScreenShare")&&
virtualclass.view.disableSSUI();virtualclass.gObj.hasOwnProperty("errAppBar")&&virtualclass.view.disableLeftAppBar()}},createDiv(b,a,c,k,d){const f=document.createElement("a");f.href="#";const g=document.createElement("div");g.id=b;"undefined "!==typeof k&&(g.className=k);k=document.createElement("span");k.className=`icon-${a}`;f.appendChild(k);f.dataset.title=virtualclass.lang.getString(a);f.className="tooltip";"virtualclassWhiteboardTool"==b&&(f.dataset.doc=`${"_doc_0_"}${virtualclass.gObj.wbCount}`);
...
virtualclass.chat.chatWindow);this.saveWbOrder();"DocumentShare"!=virtualclass.currApp&&io.disconnect()}},initOnBeforeUnload(c){"iOS"==c?document.body.onunload=function(){virtualclass.vutil.beforeLoad()}:d.onbeforeunload=function(){const a=virtualclass.vutil.smallizeFirstLetter(virtualclass.currApp);virtualclass.vutil.beforeLoad();if(("editorRich"==a||"editorCode"==a)&&"Synchronized"!=virtualclass[a].cmClient.state.constructor.name)return virtualclass.lang.getString("editorinsync")}},isPlayMode(){return 1==
...
return b},createRoleWrapper(){const c=document.createElement("div");c.id="commandToolsWrapper_doc_0_0";c.className="commandToolsWrapper";return c},_reclaimRole(){virtualclass.vutil.beforeSend({reclaimRole:!0,cf:"reclaimRole"});const c=document.getElementById("t_reclaim");c.style.pointerEvents="none";c.style.opacity="0.5";const a=this;setTimeout(()=>{a.reclaimRole();virtualclass.user.control.changeAttrToAssign("enable")},2E3)},reclaimRole(){virtualclass.gObj.controlAssign=!1;this.vcResponseAssignRole(virtualclass.gObj.uid,
virtualclass.gObj.uid,!0)},vcResponseAssignRole(c,a,b){if(c!=a||"undefined"!==typeof b)if(virtualclass.vutil.assignRole(a),"object"===typeof virtualclass.wb&&virtualclass.wb[virtualclass.gObj.currWb].utility.uniqueArrOfObjsToTeacher(),roles.hasAdmin())c=document.getElementById("chat_div").getElementsByClassName("currTeacher")[0],null!=c&&virtualclass.user.control.removeCurrTeacherFromControl(c.id),virtualclass.user.control.changeAttrToAssign("enable");else{virtualclass.vutil.removeSessionTool();a=
chatContainerEvent.elementFromShadowDom(`#ml${c}`);b=["assign"];const f=document.getElementById(`${c}ControlContainer`);null!=f?virtualclass.user.createAssignControl(f,c,!0):(b=virtualclass.user.createControl(c,b),a.appendChild(b));localStorage.setItem("aId",c)}virtualclass.vutil.attachClickOutSideCanvas();"mobTab"==virtualclass.system.device&&(virtualclass.vutil.disableVirtualClass(),virtualclass.view.createErrorMsg(virtualclass.lang.getString("supportDesktop"),"errorContainer","chatWidget"))},vcResponseAReclaimRole(c,
...
((virtualclass.system.mybrowser.name=8<=virtualclass.system.mybrowser.version)&&/(iPad)/g.test(navigator.userAgent)&&virtualclass.vutil.enableVirtualClass(),virtualclass.view.removeErrorMsg("errorContainer",!0))}},createReclaimButtonIfNeed(){if(roles.isEducator()){const c=virtualclass.vutil.createRoleWrapper();virtualclass.vutil.createReclaimButton(c);return!0}return!1},whoIsTeacher(){if(virtualclass.hasOwnProperty("connectedUsers"))for(let c=0;c<virtualclass.connectedUsers.length;c++)if("t"==virtualclass.connectedUsers[c].role||
"e"==virtualclass.connectedUsers[c].role)return virtualclass.connectedUsers[c].userid;return 0},getMySelf(){if(virtualclass.hasOwnProperty("connectedUsers"))for(let c=0;c<virtualclass.connectedUsers.length;c++)if(virtualclass.connectedUsers[c].userid==virtualclass.gObj.uid)return virtualclass.connectedUsers[c]},enablePresentatorEditors(c){c={toUser:c,status:!0};virtualclass.user.control.received_editorRich(c);virtualclass.user.control.received_editorCode(c)},getClassName(c){let a;"t"==c?a="teacher":
"e"==c?a="educator":"s"==c?a="student":"p"==c&&(a="presenter");return a},isPresenterExist(){for(let c=0;c<virtualclass.connectedUsers.length;c++)if("p"==virtualclass.connectedUsers[c].role)return!0;return!1},setReadModeWhenTeacherIsDisConn(c){if(!roles.hasAdmin()){let a=localStorage.getItem("oTDisconn");null!=a&&((a=JSON.parse(a))?virtualclass.hasOwnProperty(c)?"object"===typeof virtualclass[c].cm?virtualclass[c].cm.setOption("readOnly","nocursor"):console.log(`Editor CM is notvutil defined for editor ${c}`):
console.log(`Editor type ${c} is not ready.`):console.log("Teacher is connected."))}},setReadModeWhenTeacherIsConn(c){localStorage.removeItem("oTDisconn");const a=document.getElementById(`${virtualclass.vutil.capitalizeFirstLetter(c)}writeModeBox`);null!=a?"true"==a.getAttribute("data-write-mode")?virtualclass[c].cm.setOption("readOnly",!1):virtualclass[c].cm.setOption("readOnly","nocursor"):console.log(`Editor:- writemode element is not found for ${c}`)},isTeacherAlreadyExist(c){if(virtualclass.hasOwnProperty("connectedUsers"))for(let a=
0;a<virtualclass.connectedUsers.length;a++)if(("t"==virtualclass.connectedUsers[a].role||"e"==virtualclass.connectedUsers[a].role)&&virtualclass.connectedUsers[a].userid!=c)return console.log(`joni Id ${c}`),!0;return!1},getPresenterId(){if(virtualclass.hasOwnProperty("connectedUsers"))for(let c=0;c<virtualclass.connectedUsers.length;c++)if("p"==virtualclass.connectedUsers[c].role)return virtualclass.connectedUsers[c].userid;return!1},isEducatorAlreadyExist(c){if(virtualclass.hasOwnProperty("connectedUsers"))for(let a=
0;a<virtualclass.connectedUsers.length;a++)if("e"==virtualclass.connectedUsers[a].role&&virtualclass.connectedUsers[a].userid!=c)return console.log(`joniId ${c}`),!0;return!1},isOrginalTeacherExist(c){if(virtualclass.hasOwnProperty("connectedUsers"))for(let a=0;a<virtualclass.connectedUsers.length;a++)if("t"==virtualclass.connectedUsers[a].role&&virtualclass.connectedUsers[a].userid!=c)return console.log(`joniId ${c}`),!0;return!1},createBecomeTeacherWidget(){if(null==document.getElementById("beTeacher")){const a=
document.createElement("div");a.id="beTeacher";var c=document.createElement("a");c.id=`${a.id}Anchor`;c.innerHTML=virtualclass.lang.getString("becomeTeacher");a.appendChild(c);c=document.getElementById("virtualclassCont");c.insertBefore(a,c.firstChild);a.addEventListener("click",virtualclass.vutil.initTeacherRole);a.style.pointerEvents="none";a.style.opacity="0.5";setTimeout(()=>{a.style.pointerEvents="visible";a.style.opacity="1"},6E4)}},removeBecomeTeacherWidget(){const c=document.getElementById("beTeacher");
null!=c&&c.parentNode.removeChild(c)},initTeacherRole(){ioAdapter.send({cf:"bt"});virtualclass.vutil.removeBecomeTeacherWidget();setTimeout(()=>{virtualclass.vutil.isOrginalTeacherExist(virtualclass.jId)?console.log("Already connected teacher"):(virtualclass.vutil.overrideRoles("t"),localStorage.setItem("beTeacher",!0),console.log("connected teacher"));d.location.reload()},virtualclass.gObj.mySetTime)},overrideRoles(c){virtualclass.uInfo.userobj.role=c;virtualclass.gObj.uRole=virtualclass.uInfo.userobj.role;
wbUser.role=virtualclass.uInfo.userobj.role;const a=document.getElementById("virtualclassCont");virtualclass.vutil.overrideRolesFromElem(a,c)},overrideRolesFromElem(c,a){"s"==a?(c.classList.remove("teacher"),c.classList.remove("orginalTeacher"),c.classList.remove("presenter"),c.classList.remove("educator"),virtualclassCont.classList.add("student")):"p"==a?(c.classList.remove("teacher"),c.classList.remove("orginalTeacher"),c.classList.remove("educator"),virtualclassCont.classList.add("student"),virtualclassCont.classList.add("presenter")):
"e"==a?(c.classList.add("teacher"),c.classList.remove("student"),c.classList.remove("presenter"),c.classList.add("educator"),c.classList.add("orginalTeacher")):"t"==a&&(c.classList.remove("student"),c.classList.remove("educator"),c.classList.remove("presenter"),c.classList.add("teacher"),c.classList.add("orginalTeacher"),console.log("add Teacher"))},sesionEndMsgBoxIsExisting(){const c=document.getElementById("sessionEndMsgCont");return c.hasAttribute("data-displaying")&&"true"==c.dataset.displaying},
removeVideoHostContainer(){const c=document.getElementById("videoHostContainer");null!=c&&c.parentNode.removeChild(c)},createDummyUser(c){c||(c=5E3);let a,b=0,f=31;var g=setInterval(()=>{b>c?clearInterval(g):(f+=b,a={joinUser:{key:f},message:[{lname:" ",name:`User${f}`,role:"s",userid:f}],newuser:null,type:"member_added",user:!0},b++,virtualclass.ioEventApi.readyto_member_add(a))},1)},getVisibleHeightElem(c){var a=$(c);c=$(d).scrollTop();const b=c+$(d).height(),f=a.offset().top;a=f+a.outerHeight();
return(a>b?b:a)-(f<c?c:f)},applyHeight(c){c=document.getElementById("virtualclassAppRightPanel");const a=this.getVisibleHeightElem("virtualclassCont");c.style.height=a!=nul1?a:d.innerHeight},insertIntoLeftBar(c){const a=document.querySelector("#virtualclassApp #virtualclassAppLeftPanel");null!=a?a.appendChild(c):alert("Error:- There is no Element to insert it.")},getWhiteboardId(c){c=c.closest(".vcanvas");if(null!=c)return c.dataset.wbId;alert("doc element is null")},getElementOffset(c){let a=0,b=
0;do a+=c.offsetTop||0,b+=c.offsetLeft||0,c=c.offsetParent;while(c);return{x:b,y:a}},updateCurrentDoc(c){virtualclass.gObj.currWb=`_doc_${c}_${c}`},getParentTag(c,a){return c.closest(a)},attachEventToUploadTab(c,a,b){c=document.getElementById("newDocBtn");null!=c&&(c.removeEventListener("click",virtualclass.vutil._attachEventToUploadTab),c.addEventListener("click",virtualclass.vutil._attachEventToUploadTab))},_attachEventToUploadTab(){var c=document.querySelector("#DocumentShareDashboard .qq-upload-button-selector.qq-upload-button input");
...
localStorage.setItem("wbOrder",JSON.stringify(virtualclass.wbCommon.order))},requestOrder(c,a){virtualclass.xhrn.sendData({noting:!0},virtualclass.api.GetRoomMetaData,b=>{"Error"==b?console.log("page order retrieve failed"):(b=JSON.parse(b).Item,null!=b&&"undefined"!==typeof b&&void 0!=b&&b.order.S&&virtualclass.vutil.IsJsonString(b.order.S)&&(b=JSON.parse(b.order.S),virtualclass.gObj.docOrder=b,a(b[c])))})},IsJsonString(c){try{JSON.parse(c)}catch(a){return!1}return!0},removeChildrens(c){console.log(`children selector ${c}`);
c=document.querySelectorAll(c);for(let a=0;a<c.length;a++)c[a].parentNode.removeChild(c[a])},initAcitveElement(){if("Video"==virtualclass.currApp)var c="#VideoDashboard .qq-uploader-selector.qq-uploader.qq-gallery",a="#listvideo";else"DocumentShare"==virtualclass.currApp&&(c="#DocumentShareDashboard .qq-uploader-selector.qq-uploader.qq-gallery",a="#listdocs");const b=document.querySelector(`#${virtualclass.currApp}Dashboard`);null==b||b.classList.contains("uploading")?virtualclass.vutil.makeElementDeactive(c):
(virtualclass.vutil.makeElementActive(c),virtualclass.vutil.makeElementDeactive(a))},isResponseAvailable(c){return void 0!=c&&"undefined"!=c&&null!=c},navWhiteboard(c,a,b){virtualclass.vutil.isTextWrapperExist()&&virtualclass.wb[virtualclass.gObj.currWb].obj.drawTextObj.finalizeTextIfAny();"undefined"!==typeof b?a.call(c,b):a.call(c);console.log(`whiteboard nav time${virtualclass.gObj.wbNavtime}`)},removeAllTextWrapper(){const c=document.querySelectorAll(".canvasWrapper .textBoxContainer");for(let a=
0;a<c.length;a++)c[a].parentNode.removeChild(c[a])},isDashboardOpened(c){c=document.querySelector("#dashboardnav button");return null!=c&&c.classList.contains("clicked")},stopConnection(){io.webSocketConnected()&&io.disconnect();virtualclass.gObj.invalidlogin=!0},initDefaultApp:()=>{const c=document.querySelector(`#virtualclass${virtualclass.gObj.defaultApp}Tool a`);null!=c&&c.click(c)},initssSharing:c=>{virtualclass.gObj.whoIsSharing=c;c=chatContainerEvent.elementFromShadowDom(`#ml${c} .icon-stdscreenImg`);
...
(c=document.querySelector("#listvideo .removeCtr"))&&c.classList.remove("removeCtr");roles.hasControls()||(virtualclass.gObj.hasOwnProperty("videoPauseTime")&&clearTimeout(virtualclass.gObj.videoPauseTime),"object"===typeof virtualclass.videoUl.player&&null!=virtualclass.videoUl.player.player_&&virtualclass.videoUl.player.paused()&&(console.log("==== Video is paused"),virtualclass.videoUl.player.pause()));"object"===typeof virtualclass.videoUl.player&&delete virtualclass.videoUl.player},removeSessionEndTool(){const c=
document.getElementById("virtualclassSessionEndTool");null!=c&&c.parentNode.removeChild(c)},overrideRoleTeacher(){console.log("Member add:- End session override teacher");virtualclass.storage.config.endSession();localStorage.setItem("uRole","t");delete virtualclass.gObj.doEndSession},clearEverthing(){localStorage.removeItem("editorRich");localStorage.removeItem("editorCode");virtualclass.notPLayed=!0;virtualclass.storage.config.endSession();virtualclass.chat.chatroombox=!1;var c=document.getElementById("chatrm");
null!==c&&c.parentNode.removeChild(c);c=document.getElementById("canvas");null!==c&&(c.style.pointerEvents="none")}}})(window);let globalImageData={},newCanvas;
...
b.length)for(let f=0;f<b.length;f++)if(0<b[f].id.indexOf("Aud")){var c=b[f].id.indexOf("Cont");0<c&&(c=b[f].id.substr(0,c),c=chatContainerEvent.elementFromShadowDom(`${c}Img`,null,!0),this.control.init.call(this,c,a,void 0,"actnotSend"))}},toggleAllVideo(a){ioAdapter.mustSend({action:a,cf:"toggleVideo"})},mediaSliderUI(a){var b="audio"==a?"Audio":"Video",g=c.querySelector(`.bulkUserActions #contr${b}AllImg`),d=virtualclass.user.defaultSettings(a,virtualclass.settings.info),m="enable"==d?"disable":
"enable";null!=d?(g.setAttribute("data-action",m),g.className=`slider round icon-all-${a}-${m} enable congtooltip cgIcon`,g.dataset.title=virtualclass.lang.getString(`${m}All${b}`),d=c.querySelector(`.bulkUserActions #contr${b}All input`),d.setAttribute("checked","true"),b=c.querySelector(`.congrea #contr${b}All`),b.classList.add(m)):(m={action:d?"enable":"disable",enable:"disable",disable:"enable"},g=c.querySelector(`.bulkUserActions #contr${b}AllImg`),g.setAttribute("data-action",m[m.action]),g.className=
`slider round icon-all-${a}-${m[m.action]} congtooltip cgIcon`,g.dataset.title=virtualclass.lang.getString(`${m[m.action]}All${b}`),d=c.querySelector(`.bulkUserActions #contr${b}All input`),d.removeAttribute("checked"),b=c.querySelector(`.congrea #contr${b}All`),b.classList.add(m[m.action]));if(virtualclass.isPlayMode)anchorTag.pointerEvents="none",anchorTag.style.cursor="default";else{const b=this;g.addEventListener("click",()=>{if("audio"==a){var c=b.toogleAudioIcon();virtualclass.settings.applySettings("enable"==
c,"studentaudio");"undefined"!==typeof c&&(localStorage.setItem("allAudAction",c),b.toggleAllAudio.call(virtualclass.user,c))}else c=b.toggleVideoIcon(),virtualclass.settings.applySettings("enable"==c,"studentvideo"),"undefined"!==typeof c&&(localStorage.setItem("allVideoAction",c),b.toggleAllVideo(c))})}},defaultSettings(a,c){let b,f;for(const g in c)if(f=c[g],"audio"===a&&"studentaudio"===g||"video"===a&&"studentvideo"===g)b=!0===f?"enable":"disable";return b},chatBoxesSwitch(){if(virtualclass.chat.chatroombox)toggleCommonChatBox();
...
const b=chatContainerEvent.elementFromShadowDom(`#ml${a} .contImg`,"all");let g=!1,d=localStorage.getItem(`virtualclass${a}`);null!=d&&(g=!0,d=JSON.parse(d),d.hasOwnProperty("currTeacher")&&(virtualclass.gObj[`${a}currTeacher`]={},1==d.currTeacher?(virtualclass.user.control.currTeacherAlready=!0,virtualclass.gObj[`${a}currTeacher`].ct=!0):virtualclass.gObj[`${a}currTeacher`].ct=!1));for(let f=0;f<b.length;f++)if(-1<b[f].className.indexOf("chat")){if(g&&d.hasOwnProperty("chat"))if(virtualclass.settings.user.hasOwnProperty(a)){var m=
virtualclass.settings.onLoadSettings(virtualclass.settings.user[a]);m=m.studentpc}else m=virtualclass.settings.info.studentpc;else m=virtualclass.settings.info.studentpc;virtualclass.user.control.changeAttribute(a,b[f],m,"chat","chat")}else-1<b[f].className.indexOf("aud")?(g&&d.hasOwnProperty("aud")?virtualclass.settings.user.hasOwnProperty(a)?(m=virtualclass.settings.onLoadSettings(virtualclass.settings.user[a]),m=m.studentaudio):m=virtualclass.settings.info.studentaudio:(m=c.querySelector("#contrAudioAll"),
m=null!=virtualclass.jId&&m.classList.contains("disable")?virtualclass.settings.info.studentaudio:virtualclass.gObj.stdaudioEnable),virtualclass.user.control.changeAttribute(a,b[f],m,"audio","aud"),roles.hasAdmin()&&"false"==b[f].dataset.audioDisable&&localStorage.getItem("allAudAction")):-1<b[f].className.indexOf("RaiseHand")?(m=g&&d.hasOwnProperty("raiseHand")?!!d.raiseHand:!1,virtualclass.user.control.changeAttribute(a,b[f],m,"RaiseHand","RaiseHand")):-1<b[f].className.indexOf("stdscreen")?virtualclass.gObj.studentSSstatus.hasOwnProperty("whoIsSharing")&&
virtualclass.vutil.initssSharing(virtualclass.gObj.whoIsSharing):-1<b[f].className.indexOf("editorRich")&&"EditorRich"==virtualclass.currApp?(c.querySelector("#alleditorRichContainerAnch"),m=g&&d.hasOwnProperty("editorRich")?!!d.editorRich:!1,virtualclass.user.control.changeAttribute(a,b[f],m,"editorRich","editorRich")):-1<b[f].className.indexOf("editorCode")&&(m=g&&d.hasOwnProperty("editorCode")?!!d.editorCode:!1,virtualclass.user.control.changeAttribute(a,b[f],m,"editorCode","editorCode"))}}}})(window,
...
b,c,f,g){const k=[];let m=a;const v=this;this.annotationList_.updateSpan(new n(a,b-a),(a,b)=>{const n={};for(var q in a.attributes)n[q]=a.attributes[q];n[r.LINE_SENTINEL]&&!g||c(n);q={};const C={};v.computeChangedAttributes_(a.attributes,n,q,C);a:{for(const b in q){a=!1;break a}a=!0}a||k.push({start:m,end:m+b,attributes:q,attributesInverse:C,origin:f});m+=b;return new d(n)});0<k.length&&this.trigger("attributesChange",this,k)};a.prototype.computeChangedAttributes_=function(a,b,c,f){const d={};let g;
