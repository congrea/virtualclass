...
return[virtualclass.vutil.numValidateFour(a[1],a[2],a[3],a[4]),a.subarray(5,a.length)]},removeAudioFromLocalStorage(){localStorage.removeItem("audEnable")},async bug_687574_callLocalPeers(){function a(a,b){if("connected"===b.currentTarget.connectionState)try{u.disconnect(g.audio.Html5Audio.audioContext.destination),u.connect(g.audio.Html5Audio.MediaStreamDest)}catch(O){}else if("disconnected"===b.currentTarget.connectionState){k.close();m.close();m=k=null;try{u.disconnect(g.audio.Html5Audio.MediaStreamDest),
u.connect(g.audio.Html5Audio.audioContext.destination)}catch(O){}g.audio.bug_687574_callLocalPeers()}}async function b(a){try{await k.setLocalDescription(a),await m.setRemoteDescription(a)}catch(N){f()}try{const a=await m.createAnswer();await d(a)}catch(N){f()}}async function d(a){try{await m.setLocalDescription(a),await k.setRemoteDescription(a)}catch(N){f()}}async function c(a,b){if(b.candidate&&"host"===b.candidate.type)try{await (a===k?m:k).addIceCandidate(b.candidate)}catch(O){f()}}function f(){try{u.connect(g.audio.Html5Audio.audioContext.destination),
