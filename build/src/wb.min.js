...
b);b.innerHTML=a;a=document.getElementById("vcanvas");a.parentNode.insertBefore(b,a)},removeElement(a){a=document.getElementById(a);null!=a&&a.parentNode.removeChild(a)},disableSSUI(){const a=document.getElementById("virtualclassScreenShareTool");null!=a&&(a.style.opacity="0.5",a.style.pointerEvents="none")},disableLeftAppBar(){const a=document.getElementById("virtualclassOptionsCont");null!=a&&(a.style.opacity="0.5",a.style.pointerEvents="none")}};a=a.init();a.window.resizeFinished=function(){let a=
...
d-this.trimontime,this.isTrimRecordingNow=!1);this.UIdownloadProgress(c,b);this.updateTotalTime();virtualclass.settings.info.trimRecordings&&0<this.masterRecordings.length?18E4>this.actualTotalPlayTime&&(this.actualTotalPlayTime=this.getTotalTimeInMilSeconds(this.masterRecordings.length-1,this.masterRecordings[this.masterRecordings.length-1].length-1)):this.actualTotalPlayTime=this.currentMin;(18E4<=this.actualTotalPlayTime||this.lastFile===c)&&0<this.masterRecordings.length&&(this.playStart?this.startToPlay():
this.alreadyAskForPlay||(this.alreadyAskForPlay=!0,this.askToPlay()))},handleStartToPlay(c){const a=document.querySelector(".rv-vanilla-modal-overlay.is-shown");null!=a&&a.removeEventListener("click",this.handleStartToPlay.bind(this));virtualclass.gesture.clickToContinue();console.log("===== Start to play");this.startToPlay();c.currentTarget.classList.remove("askToPlayCont")},startToPlay(){this.playStart?this.hasOwnProperty("isPausedByNotPresent")&&(delete this.isPausedByNotPresent,this.subRecordingIndex=
