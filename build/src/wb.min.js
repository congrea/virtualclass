...
c.classList.add("show"))}makeActiveTool(c,b){const a=document.getElementById(c);if(a){var d=b||virtualclass.gObj.currWb,f=document.querySelectorAll(`#commandToolsWrapper${d} .tool.active`);for(let b=0;b<f.length;b+=1)f[b].classList.remove("active");roles.hasControls()&&(f=document.getElementById(c).dataset.tool,document.querySelector(`#tool_wrapper${b}`).dataset.currtool=f);this.themeColorShapes(c,d);a.classList.add("active");localStorage.activeTool=a.id}}themeColorShapes(c,b){c=c.split(/_doc_*/)[0];
b=document.querySelector(`#tool_wrapper${b}.shapesToolbox`);if(!b)debugger;"t_line"===c||"t_circle"===c||"t_rectangle"===c||"t_triangle"===c?b.classList.add("active"):b.classList.remove("active")}handleActivateTool(c){let b=localStorage.getItem("activeTool");null!==b&&-1<b.indexOf(c)?(this.makeActiveTool(b,c),virtualclass.wb[c].selectedTool=b.split("_")[1],"activeall"!==virtualclass.wb[c].selectedTool&&virtualclass.wb[c].activeAllObj.disable(c)):virtualclass.wb[c].selectedTool&&(b=`t_${virtualclass.wb[c].selectedTool}${c}`,
this.makeActiveTool(b,c))}};class WhiteboardActiveAll{generateData(c,b,a){c=b.canvas.getPointer(c,!0);return virtualclass.wbWrapper.protocol.encode("ac",{event:a,x:c.x,y:c.y})}enable(c,b){c=b?virtualclass.wb[c].canvas.getObjects(b):virtualclass.wb[c].canvas.getObjects();for(b=0;b<c.length;b+=1)console.log("====> active all"),c[b].set("selectable",!0)}disable(c,b){c=b?virtualclass.wb[c].canvas.getObjects(b):virtualclass.wb[c].canvas.getObjects();for(b=0;b<c.length;b+=1)c[b].set("selectable",!1)}disableBut(c,b){c=virtualclass.wb[c].canvas.getObjects();
...
(await this.initWhiteboardData(b.wbId),roles.hasControls()&&(console.log("====> invoke handle active tool"),virtualclass.wbWrapper.util.handleActivateTool(b.wbId)));"object"===typeof this.shownPdf&&(io.globallock=!1,virtualclass.vutil.removeClass("virtualclassCont","resizeWindow"));this.firstTimeScroll&&roles.isStudent()&&"object"===typeof virtualclass.pdfRender[virtualclass.gObj.currWb]&&(virtualclass.pdfRender[virtualclass.gObj.currWb].setScrollPosition({scX:0,scY:0}),this.firstTimeScroll=!1)}},
