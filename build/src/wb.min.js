...
*/
const workerAudioRecBlob=URL.createObjectURL(new Blob(["(",function(){importScripts("https://cdn.congrea.net/build/src/audio-g711-and-resampler.min.js");const d={workletAudioRec:null,workerIO:null,samples:null,allAudioArr:{},audioToBePlay:{},gAudio:{},adSign:{},resamplerdecode:{},mergeAllAudioSend:[],allAudioSend:[],supportAudioWorklet:!1,sampleRate:null,extractData(c){c=new Int8Array(c);return[this.numValidateFour(c[1],c[2],c[3],c[4]),c.subarray(5,c.length)]},numValidateFour(c,a,b,f){c=this.preNumValidateTwo(c);
a=this.preNumValidateTwo(a);b=this.preNumValidateTwo(b);f=this.preNumValidateTwo(f);return parseInt(c+a+b+f)},numValidateTwo(c,a){c=this.preNumValidateTwo(c);a=this.preNumValidateTwo(a);return parseInt(c+a)},preNumValidateTwo(c){c=c.toString();if(1==c.length)return`0${c}`;if(2==c.length)return c},resample(c,a){this.audioToBePlay.hasOwnProperty(a)||(this.audioToBePlay[a]=[]);this.samples=G711.decode(c,{floating_point:!0,Eight:!0});"object"!==typeof this.resamplerdecode[a]&&(this.resamplerdecode[a]=
...
fromworkletAudioRec(){console.log("from audio worklet")},fromworkerIO(c){if(c.data.hasOwnProperty("msg")){({msg:c}=c.data);var a=this.extractData(c);c=a[0];this.adSign.hasOwnProperty(c)||(this.adSign[c]={},this.adSign[c].ad=!0,this.workerIO.postMessage({cmd:"firstAudio",msg:{userId:c},dest:"mainthread"}));a=this.resample(a[1],c);this.supportAudioWorklet?this.workletAudioRec.postMessage({cmd:"audio",msg:{data:a,uid:c}}):postMessage({cmd:"noAudioWorklet",msg:{data:a,uid:c}})}}};onmessage=function(c){d.onMessage(c)}}.toString(),
...
*/
const workerAudioSendBlob=URL.createObjectURL(new Blob(["(",function(){importScripts("https://cdn.congrea.net/build/src/audio-g711-and-resampler.min.js");const d={audioWorkletSend:null,workerIO:null,gObjUid:null,audMouseDown:!1,precheck:!1,repMode:!1,minthreshold:65535,audioWasSent:0,preAvg:0,curAvg:0,maxthreshold:0,convertFloat32ToInt16(c){let a=c.length;const b=new Int16Array(a);for(;a--;)b[a]=32767*Math.min(1,c[a]);return b},encodeAudio(c){return G711.encode(c,{alaw:!0})},breakintobytes(c,a){c=
c.toString();for(let b=c.length;b<a;b++)c=`0${c}`;return c.match(/[\S]{1,2}/g)||[]},sendBinary(c){null!=this.workerIO&&this.workerIO.postMessage({cmd:"sendBinary",msg:c})},audioSend(c,a){if(this.audMouseDown&&!this.precheck){a=this.breakintobytes(this.gObjUid,8);a=new Int8Array([101,a[0],a[1],a[2],a[3]]);const b=new Int8Array(c.length+a.length);b.set(a);b.set(c,a.length);this.sendBinary(b)}},silenceDetection(c,a){let b,f=sum=rate=0;for(i=0;i<a.length;i++)b=Math.abs(a[i]),f<b&&(f=b),sum+=b;this.curAvg=
...
noQuiz:"No quiz available. Congrea supports only mutliple choice questions",Novote:"No vote received for this poll",nowebcam:"Webcam is not available, or not connected properly.",offcollaboration:"Collaboration off",oncollaboration:"Collaboration on",opendsDbheading:"Document dashboard",openSharePresentationdbHeading:"Presentation dashboard",openvideoDashboard:"Video dashboard",operaBrowserIssue:"Your browser {virtualclass1} {virtualclass2} is partially supported. You will not be able to share your screen, Congrea fully support chrome and Firefox",
Options:"Options",optselectd:"OPTION SELECTED",Oval:"Oval",overallprogress:"Overall progress",Pause:"Pause",pclosetag:"<span>Are you sure to close this poll?  </span>",pclose:"Are you sure to close voting?",Pclosed:"Poll closed",pdfnotrender:"There is problem with loading window, please reload browser.",Pdsuccess:"Poll deleted successfully",PEdit:"Poll edit",PermissionDeniedError:"Webcam access has been denied.",PERMISSION_DENIED:"You denied access to your Webcam(camera/microphone).",Play:"Play",
playsessionmsg:"Click \u2018play\u2019 to start the session.",pleasewaitWhSynNewCont:"Please wait a while. Syncing new content.",plswaitwhile:"Please wait....",poll:"Poll",pollblank:"Question can not be left blank",pollCancel:"Close",pollDel:"Are you sure to delete this poll?",pollHead:"Vote this poll",pollmaybeshown:"Poll will be published",pollmybpublish:"Poll will be published shortly",pollresult:"Previous result",ppoll:"Publish poll",PQuestions:"Poll questions",PQuiz:"Publish quiz",Precheck:"Precheck",
precheckStart:"Start",prechkcmplt:"Precheck complete",Presult:"Poll result",preWllBshortly:"Presentation will be shared shortly",Prev:"Prev",proposedspeed:"Proposed speed",PSetting:"Poll setting",Publish:"Publish",QClosed:"Quiz closed",Question:"Question",Quiz:"Quiz","Quiz/page":"Quiz / page",Quizes:"Quizzes",quizmayshow:"Get ready for a quiz",quizreviewpublish:"Review & publish quiz",RaiseHandStdDisabled:"Undo raise hand",RaiseHandEnable:"Hand raised",raiseHandNotify:"Student(s) raised hand",RaiseHandStdEnabled:"Raise hand",
readonlymode:"Read only",receivedVotes:"Received votes",reclaim:"Reclaim",Rectangle:"Rectangle",rejected:"Another application on your computer might be using your webcam. Kindly close all other applications",reload:"Reload",reloadDoc:"Reload document",replay:"Re-play",Replay:"Replay from start.",replay_message:"Thanks for watching.",requestScreenShare:"Request screen share",Reset:"Reset",Rtime:"Remaining time",rusureCquiz:"<span>Are you sure to close this quiz? </span>",rvtu:"Votes received/ total users",
safariBrowserIssue:"Your browser Safari {virtualclass1} is not supported, We fully support Chrome  and Firefox",Save:"Save",savesession:"Do you want to save current session?",savesessionTechSupport:"Do you want to save current session? <br /> Once you download the session, Updates won't be available on re-downloading of same session.",screensharealready:"The screen is being shared.",ScreenShare:"Screen share",screensharemsg:"The above screen is being shared.",second:"Second",selfview:"Self view",
SetTimer:"Set timer",sesseionkeymissing:"Session key is missing from Front End.",sentPackets:"Sent <br/><span>Packets</span>",SessionEnd:"Session end",sessionexpired:"Session Expired",sessionendmsg:"Session has been closed. You may now close your browser.",setting:"Setting",Searchuser:"Search user",share:"Share",ShareVideo:"Share video",shareAnotherYouTubeVideo:"Share another YouTube video",SharePresentation:"Share presentation",SharePresentationdbHeading:"Presentation dashboard",sharetoall:"Share to all",
...
enableAllGroupChat:"Enable common chat",PermissionDeniedErrorExt:'<div class="errorMsg"> Camera access has been blocked.To provide webcam access, kindly follow below procedure <br />Go to camera icon on top right of the screen <br /> Click on "Always allow..." option and select the camera option <br />  Click on done and refresh the screen  </div> <div class="screenImages"> <figure class="chrome" > <img src="https://www.congrea.com/wp-content/uploads/2016/10/ff-video.png" > <figcaption>Chrome</figcaption> </figure></div>',
...
[a]=b.split(" ");a=Math.trunc(a/1E6);return a-c},insertPacketInto(c,a){a=Math.trunc(a/1E3);if(!isNaN(a)&&1<=a&&!this.isTrimRecordingNow){const b={playTime:1E3,recObjs:'{"0{"user":{"userid":"2"},"m":{"app":"nothing","cf":"sync"}} ',type:"J"};for(let f=0;f<a;f++)c.push(b)}return c},makeRecordingQueue(c,a){console.log(`File formatting ${c}`);var b=[],f=null;a=a.trim().split(/(?:\r\n|\r|\n)/g);let g;for(let c=0;c<a.length;c++)if(null!=a[c]&&""!=a[c]){this.totalElements++;var d=a[c].substring(0,21);var m=
a[c].substring(22,a[c].length);[d,g]=d.split(" ");this.tempTime=d;d=Math.trunc(d/1E6);null!=this.refrenceTime&&(f?(this.tempPlayTime=d-this.refrenceTime-f,f=null):this.tempPlayTime=d-this.refrenceTime);if(null!=m&&""!=m){!this.joinRoomRecevied&&-1<m.indexOf('"type":"joinroom"')&&(this.joinRoomRecevied=!0,m=JSON.parse(m),m.clientids=m.action,m=JSON.stringify(m));if(this.lastFileTime&&0===c){const a=Math.trunc(d-this.lastFileTime);b=this.insertPacketInto(b,a);this.tempPlayTime=1E3<a?a%1E3:a}c===a.length-
1&&(this.lastFileTime=d);virtualclass.settings.info.trimRecordings?this.isTrimRecordingNow?(b.push({playTime:0,recObjs:m,type:g}),-1<m.indexOf('{"ac":21,"cf":"recs"')&&(console.log("=== trim off"),this.trimofftime=d,this.isTrimRecordingNow=!1,this.totalTimeInMiliSeconds-=this.trimofftime-this.trimontime,this.trimontime=this.trimofftime=0)):-1<m.indexOf('{"ac":11,"cf":"recs"')?(console.log("=== trim on"),this.isTrimRecordingNow=!0,b.push({playTime:0,recObjs:m,type:g}),this.trimontime=d):b.push({playTime:this.tempPlayTime,
recObjs:m,type:g}):b.push({playTime:this.tempPlayTime,recObjs:m,type:g});"undefined"!==typeof a[c+1]&&(m=this.calculateNextTime(d,a[c+1]),b=this.insertPacketInto(b,m,!0),1E3<=m&&(f=1E3*Math.trunc(m/1E3)));this.refrenceTime=d}}for(a=0;a<b.length;a++)"B"===b[a].type&&(f=virtualclass.dtCon.base64DecToArr(b[a].recObjs),b[a].recObjs=f);this.masterRecordings.push(b);b=virtualclass.recorder.getTimeFromFile(c);this.finishRequestDataFromServer(b);this.isTrimRecordingNow&&this.allFileFound&&(this.totalTimeInMiliSeconds-=
d-this.trimontime,this.isTrimRecordingNow=!1);virtualclass.settings.info.trimRecordings&&0<this.masterRecordings.length?(this.actualTotalPlayTime=this.getTotalTimeInMilSeconds(this.masterRecordings.length-1,this.masterRecordings[this.masterRecordings.length-1].length-1),this.currentMin=this.actualTotalPlayTime/1E3/60):(b=(b-this.firstTimeInSeconds)/60,b>this.currentMin&&(this.currentMin=b));console.log("==== downloaded total min ",this.currentMin);this.updateTotalTime();this.UIdownloadProgress();
(18E4<=6E4*this.currentMin||this.lastFile===c)&&0<this.masterRecordings.length&&(this.playStart?this.startToPlay():this.alreadyAskForPlay||(this.alreadyAskForPlay=!0,this.askToPlay()))},handleStartToPlay(c){const a=document.querySelector(".rv-vanilla-modal-overlay.is-shown");null!=a&&a.removeEventListener("click",this.handleStartToPlay.bind(this));virtualclass.gesture.clickToContinue();console.log("===== Start to play");this.startToPlay();c.currentTarget.classList.remove("askToPlayCont")},startToPlay(){this.playStart?
this.hasOwnProperty("isPausedByNotPresent")&&(delete this.isPausedByNotPresent,this.subRecordingIndex=0,this.subRecordings=this.masterRecordings[this.masterIndex],this.controller._play(),virtualclass.popup.closeElem()):(this.playStart=!0,this.playInt(),virtualclass.popup.closeElem())},isDownloadedAllRecordings(c){return 9E5<this.sessionStartTime?c>=virtualclass.recorder.lastTimeInSeconds:!1},finishRequestDataFromServer(c){this.isDownloadedAllRecordings(c)&&(virtualclass.recorder.allFileFound=!0,virtualclass.recorder.alreadyAskForPlay&&
(c=document.getElementById("askplayMessage"),null!=c&&(c.innerHTML=virtualclass.lang.getString("playsessionmsg"))))},UIdownloadProgress(){const c=virtualclass.recorder.totalTimeInMiliSeconds/1E3/60;this.downloadInPercentage=100*this.currentMin/c;virtualclass.pbar.renderProgressBar(c,this.currentMin,"downloadProgressBar","downloadProgressValue");virtualclass.recorder.playStart&&!virtualclass.recorder.waitServer&&virtualclass.recorder.init()},playInt(){console.log("=====Play init recording=====");virtualclass.popup.closeElem();
...
const a=document.querySelector("#loadingWindowCont .askToPlay");null!=a&&a.addEventListener("click",this.handleStartToPlay.bind(this));document.getElementById("popupContainer").classList.add("playPopup");c.classList.add("askToPlayCont")},playProgressOutput(c){console.log("Offset ",c.offsetX)},getOffset(c){"touchend"==c.type?c=this.lastEvent:(c.offsetX=c.touches[0].pageX-c.touches[0].target.offsetLeft,c.offsetY=c.touches[0].pageY-c.touches[0].target.offsetTop);return this.lastEvent=c},seekHandler(c){void 0==
c.offsetX&&(c=this.getOffset(c));this.pauseBeforeSeek=this.controller.pause;if(this.startSeek)console.log("Earlier seek start is not end yet.");else{this.startSeek=!0;var a=document.querySelector("#virtualclassCont");null!=a&&a.classList.add("recordSeeking");a=c.offsetX;const b=this.getCustomWidth();this.seekValueInPercentage=a/b*100;console.log(`====Seek start ${this.seekValueInPercentage} ev current target=${c.currentTarget.id}`)}},getCustomWidth(){return document.getElementById("downloadProgress").clientWidth-
1},async seek(c){virtualclass.videoHost.UI.hideTeacherVideo();c=this.getSeekPoint(c);(c.master<this.masterIndex||c.master===this.masterIndex&&c.sub<this.subRecordingIndex)&&await this.replayFromStart();await this._seek(c);console.log("seek is finished");this.triggerSynchPacket()},triggerSynchPacket(){this.triggerPlayProgress();this.binarySyncMsg&&(this.handleSyncPacket(),this.binarySyncMsg=null);this.handleSyncStringPacket()},_seek(c){this.controller._pause();let a;for(;;){a=null!=c?this.masterIndex!=
c.master?this.masterRecordings[this.masterIndex].length:c.sub:this.masterRecordings[this.masterIndex].length;for(let b=this.subRecordingIndex;b<a;b++){try{if("B"!=this.subRecordings[this.subRecordingIndex].type)this.msg=io.cleanRecJson(this.subRecordings[this.subRecordingIndex].recObjs),-1<this.msg.indexOf('"m":{"unshareScreen"')?this.binarySyncMsg=null:-1<this.msg.indexOf('},"m":{"poll":{"pollMsg":"stdPublish",')?virtualclass.poll.recordStartTime={app:"Poll",data:{masterIndex:this.masterIndex,subIndex:this.subRecordingIndex}}:
-1<this.msg.indexOf('"m":{"videoUl":{"content_path"')?virtualclass.videoUl.videoStartTime={app:"Video",data:{masterIndex:this.masterIndex,subIndex:this.subRecordingIndex}}:-1<this.msg.indexOf('"m":{"quiz":{"quizMsg":"stdPublish"')&&(virtualclass.quiz.quizStartTime={app:"Quiz",data:{masterIndex:this.masterIndex,subIndex:this.subRecordingIndex}}),io.onRecMessage(this.convertInto({data:this.msg}));else{this.msg=this.subRecordings[this.subRecordingIndex].recObjs;if(104==this.msg[0]||204==this.msg[0]||
102==this.msg[0]||202==this.msg[0])if(0==this.msg[1]||1==this.msg[1])this.binarySyncMsg={data:{masterIndex:this.masterIndex,subIndex:this.subRecordingIndex}};"ScreenShare"!=virtualclass.currApp||104!=this.msg[0]&&204!=this.msg[0]&&103!=this.msg[0]&&203!=this.msg[0]&&102!=this.msg[0]&&202!=this.msg[0]||(io.onRecMessage(this.convertInto({data:this.msg})),this.binarySyncMsg=null)}this.collectElapsedPlayTime()}catch(f){}this.subRecordingIndex++}this.elapsedRecTime=this.elapsedTime=this.elapsedPlayTime;
if(void 0!=c&&this.masterIndex===c.master&&c.sub===this.subRecordingIndex)break;else this.subRecordingIndex=0,this.masterIndex++,this.subRecordings=this.masterRecordings[this.masterIndex]}console.log("==== recording final 1")},handleSyncStringPacket(){if("Poll"===virtualclass.currApp&&"object"===typeof virtualclass.poll.pollState.data&&virtualclass.poll.hasOwnProperty("recordStartTime")){var c=this.getTotalTimeInMilSeconds(virtualclass.poll.recordStartTime.data.masterIndex,virtualclass.poll.recordStartTime.data.subIndex);
virtualclass.poll.dataRec.setting.timer?this.pollUpdateTime(c,virtualclass.poll.pollState):(c=this.convertIntoReadable(this.elapsedPlayTime-c),virtualclass.poll.dataRec.newTime.min=c.m,virtualclass.poll.dataRec.newTime.sec=c.s,virtualclass.poll.elapsedTimer())}else if("Video"===virtualclass.currApp&&"object"===typeof virtualclass.videoUl&&virtualclass.videoUl.hasOwnProperty("videoStartTime"))c=this.getTotalTimeInMilSeconds(virtualclass.videoUl.videoStartTime.data.masterIndex,virtualclass.videoUl.videoStartTime.data.subIndex),
c=(this.elapsedPlayTime-c)/1E3,"object"===typeof virtualclass.videoUl.player?virtualclass.videoUl.playVideo(c):(console.log("====Video init to play start"),virtualclass.videoUl.isPaused=this.pauseBeforeSeek?!0:!1,virtualclass.videoUl.startTime=c);else if("Quiz"===virtualclass.currApp&&"object"===typeof virtualclass.quiz){c=document.querySelector("#qztime");var a=this.getTotalTimeInMilSeconds(virtualclass.quiz.quizStartTime.data.masterIndex,virtualclass.quiz.quizStartTime.data.subIndex);0<+virtualclass.quiz.plugin.config.quizTime?
(a=1E3*virtualclass.quiz.plugin.config.quizTime-(this.elapsedPlayTime-a),virtualclass.quiz.plugin.method.startTimer(a/1E3,c,"desc","vmQuiz")):(a=(this.elapsedPlayTime-a)/1E3,virtualclass.quiz.plugin.method.startTimer(a,c,"asc","vmQuiz"))}},pollUpdateTime(c,a){c=this.convertIntoReadable(6E4*a.data.stdPoll.newTime.min+1E3*a.data.stdPoll.newTime.sec-(this.elapsedPlayTime-c));virtualclass.poll.newTimer.sec=c.s;virtualclass.poll.newTimer.min=c.m;virtualclass.poll.showTimer(virtualclass.poll.newTimer)},
handleSyncPacket(){if(this.binarySyncMsg){console.log("Get full screen share");var c=this.binarySyncMsg.data.subIndex;let a=this.binarySyncMsg.data.masterIndex;for(;a<=this.masterIndex;){console.log(`Start from master index ${a} from Subindex ${c}`);const b=this.masterRecordings[a];let f;for(f=this.masterIndex!=a?this.masterRecordings[this.masterIndex].length:this.subRecordingIndex;c<=f;c++){this.msg=b[c];try{if(null!=this.msg&&"B"==this.msg.type){if(this.msg=this.msg.recObjs,104==this.msg[0]||204==
this.msg[0]||103==this.msg[0]||203==this.msg[0]||102==this.msg[0]||202==this.msg[0])io.onRecMessage(this.convertInto({data:this.msg}))}else console.log("Either this.msg is null or string")}catch(g){console.log(`PLAY ERROR ${g.errorCode}`)}}if(this.masterIndex==a&&c==this.subRecordingIndex)break;else a++,c=0}console.log(`===== Elapsed time 2 ==== ${this.elapsedPlayTime}`);this.playProgressBar(this.elapsedPlayTime)}},getSeekPoint(c){c=Math.trunc(this.totalTimeInMiliSeconds*c/100);console.log(`Seek index ${c}`);
...
(this.subRecordings=this.masterRecordings[this.masterIndex],io.cfg=JSON.parse(this.subRecordings[this.subRecordingIndex].recObjs),virtualclass.gObj.uRole="s")},executePacketToPlay(){this.calcPlayTime();0===this.playTime?(this.triggerPlayProgress(),this.executePacketToPlayActual(),this.calcPlayTimeNext(),this.play()):this.playTimeout=setTimeout(()=>{this.triggerPlayProgress();this.executePacketToPlayActual();this.calcPlayTimeNext();this.play()},this.playTime)},executePacketToPlayActual(){try{0>this.subRecordings[this.subRecordingIndex].recObjs.indexOf('"cf":"sync"')&&
...
{app:"Video",data:{masterIndex:this.masterIndex,subIndex:this.subRecordingIndex}},console.log("Capture video")):"Quiz"===virtualclass.currApp&&-1<this.subRecordings[this.subRecordingIndex].recObjs.indexOf('"m":{"quiz":{"quizMsg":"stdPublish",')&&(virtualclass.quiz.quizStartTime={app:"Quiz",data:{masterIndex:this.masterIndex,subIndex:this.subRecordingIndex}}))}catch(c){console.log(`PLAY ERROR ${c.errorCode}`)}},isPlayFinished(){return"undefined"===typeof this.masterRecordings[this.masterIndex]&&("undefined"===
...
virtualclass.videoUl.isPaused=!0)},triggerPlayVideo(){"Video"==virtualclass.currApp&&virtualclass.videoUl&&virtualclass.videoUl.player&&(console.log("VIDEO IS Played"),virtualclass.videoUl.player.play(),virtualclass.videoUl.isPaused=!1)},askAgainToPlay(){if(this.subRecordings&&0>this.subRecordings[this.subRecordingIndex].recObjs.indexOf("sEnd"))io.onRecMessage(this.convertInto({data:this.subRecordings[this.subRecordingIndex].recObjs}));virtualclass.recorder.allFileFound?virtualclass.recorder.allFileFound&&
this.initReplayWindow():(virtualclass.recorder.waitServer=!0,virtualclass.recorder.waitPopup=!1,this.displayWaitPopupIfNot())},initReplayWindow(){this.triggerPauseVideo();virtualclass.popup.replayWindow();virtualclass.popup.sendBackOtherElems();document.getElementById("replayClose").addEventListener("click",()=>{d.close()});document.getElementById("replayButton").addEventListener("click",()=>{virtualclass.recorder.controller.pause=!1;virtualclass.recorder.replayFromStart.call(virtualclass.recorder)})},
getCurrentPacket(){let c=0;if(0<this.masterIndex)for(let a=0;a<this.masterIndex;a++)c+=this.masterRecordings[a].length;return c+this.subRecordingIndex},collectElapsedPlayTime(){this.elapsedPlayTime+=this.subRecordings[this.subRecordingIndex].playTime},triggerPlayProgress(){this.collectElapsedPlayTime();0==this.masterIndex&&0==this.subRecordingIndex?this.playProgressBar(this.elapsedPlayTime):(this.playTimeNowSeconds=Math.round(this.elapsedPlayTime/1E3),this.playTimeNowSeconds>this.playTimePreviousSeconds&&
(this.playProgressBar(this.elapsedPlayTime),virtualclass.popup.sendBackOtherElems(),this.playTimePreviousSeconds=this.playTimeNowSeconds))},calcPlayTime(){"object"===typeof this.subRecordings[this.subRecordingIndex]&&(this.playTimePak=this.subRecordings[this.subRecordingIndex].playTime/this.controller.ff);this.timeNow=performance.now();this.diffTime=this.timeNow-this.startTime;this.startTime=this.timeNow;this.elapsedTime+=this.diffTime;this.playTime=Math.round(this.playTimePak-(this.elapsedTime-this.elapsedRecTime));
this.elapsedRecTime+=this.playTimePak;0>this.playTime&&(this.playTime=0)},calcPlayTimeNext(){this.subRecordingIndex+1==this.subRecordings.length?(this.masterIndex++,this.subRecordings=this.masterRecordings[this.masterIndex],this.subRecordingIndex=0):this.subRecordingIndex++},initController(){if(null!=document.getElementById("playControllerCont")){var c=this;const a=document.getElementsByClassName("ff");for(let b=0;b<a.length;b++)a[b].onclick=function(){var a=this.id.split("ff")[1];c.controller.fastForward(parseInt(a,
10));a=document.querySelector("#dispVideo");virtualclass.videoUl&&virtualclass.videoUl.player&&a.classList.contains("vjs-paused")&&virtualclass.videoUl.player.play();"replayFromStart"!=this.parentNode.id&&c.doControlActive(this)};c=this;const b=document.getElementById("recPause");b.addEventListener("click",()=>{b.parentNode.classList.contains("recordingPlay")?c.initRecPause():c.initRecPlay()})}},initRecPause(){const c=document.getElementById("recPause");this.controller._pause();this.doControlActive(c);
...
wbUser.auth_user);this.httpObj.setRequestHeader("x-congrea-authpass",wbUser.auth_pass);this.httpObj.setRequestHeader("x-congrea-room",wbUser.room);this.httpObj.setRequestHeader("Content-Type","application/json");this.httpObj.withCredentials=!1;this.httpObj.send(c)},sortingFiles(c){return c.sort(function(a,b){return a<b?-1:a>b?1:0})},setOrginalListOfFiles(c){const a={};for(let b=0;b<c.length;b++)a[c[b]]=null!=c[b+1]?{index:b,next:c[b+1]}:{index:b,next:"end"};return a},triggerDownloader(){console.log("Init trigger time to request 1");
this.tryNumberOfTimes=1;this.hasOwnProperty("triggerDownloaderTime")&&clearInterval(this.triggerDownloaderTime);this.hasOwnProperty("startTimeCounter")&&clearInterval(this.startTimeCounter);let c=0;this.startTimeCounter=setInterval(()=>{c++;console.log(`=====Timer ===${c}`)},1E3);this.triggerDownloaderTime=setInterval(()=>{console.log("Init trigger time to request 2");3<this.tryNumberOfTimes?(virtualclass.recorder.allFileFound=!0,clearInterval(this.triggerDownloaderTime.triggerDownloaderTime),this.isPlayFinished()&&
this.askAgainToPlay()):virtualclass.xhrn.vxhrn.post(virtualclass.api.recordingFiles,data).then(a=>{this.afterDownloadingList(a.data)});console.log("Time to request 180000")},18E4)},afterDownloadingList(c){if(null!=c&&c.hasOwnProperty("Item")){var a=+c.Item.time.N;this.sessionStartTime=(new Date).getTime()-a;a=[...new Set(c.Item.list.L.map(a=>a.S))];c=a.length;if(this.lastRecordings&&a.length==this.lastRecordings)this.tryNumberOfTimes=this.tryNumberOfTimes?++this.tryNumberOfTimes:1;else{delete this.alreadyCalcTotTime;
this.totalRecordingFiles=this.sortingFiles(a);this.lastFile=this.totalRecordingFiles[this.totalRecordingFiles.length-1];this.orginalListOfFiles=this.setOrginalListOfFiles(this.totalRecordingFiles);this.calculateTotalPlayTime();a=this.totalRecordingFiles.shift();this.firstxhr=axios.create({timeout:6E5,withCredentials:!0});this.requestDataFromServer(a,this.firstxhr);for(const a in this.totalRecordingFiles)virtualclass.createPrefetchLink(`https://recording.congrea.net/${wbUser.lkey}/${wbUser.room}/${virtualclass.recorder.session}/${this.totalRecordingFiles[a]}`);
9E5>this.sessionStartTime&&this.triggerDownloader()}this.lastRecordings=c}else this.tryNumberOfTimes=this.tryNumberOfTimes?++this.tryNumberOfTimes:1,this.triggerDownloader()},requestListOfFiles(){this.actualTotalPlayTime=0;this.session=wbUser.session;virtualclass.popup.loadingWindow();virtualclass.xhrn.vxhrn.post(virtualclass.api.recordingFiles,{session:virtualclass.recorder.session}).then(c=>{this.afterDownloadingList(c.data)})},calculateTotalPlayTime(){const c=Math.trunc(+virtualclass.recorder.totalRecordingFiles[0].split("-")[0]/
...
virtualclass.gObj.dstAll=JSON.parse(a.value.dstalldocs),virtualclass.gObj.dstAllNotes=JSON.parse(a.value.allNotes)),a=await a.continue();b||(console.log("document share store init"),virtualclass.gObj.dstAll="init")}},clearSingleTable(a,b){console.log("Clear single table ",a);this.db.transaction(a,"readwrite").store.clear();"undefined"!==typeof b&&b();"dstdata"==a&&(virtualclass.gObj.docs="init",console.log("==== Docs init "));"dstall"==a&&(virtualclass.gObj.dstall="init")},async clearStorageData(){ioAdapter.adapterMustData=
[];ioAdapter.serial=-1;ioAdapter.userSerial=[];ioAdapter.userAdapterMustData=[];ioMissingPackets.executedStore=[];ioMissingPackets.executedSerial={};ioMissingPackets.missRequest=[];ioMissingPackets.aheadPackets=[];ioMissingPackets.missRequestFlag=0;ioMissingPackets.executedUserStore=[];ioMissingPackets.executedUserSerial={};ioMissingPackets.missUserRequest=[];ioMissingPackets.aheadUserPackets=[];ioMissingPackets.missUserRequestFlag=0;await Promise.all([this.clearSingleTable("wbData"),this.clearSingleTable("dataAdapterAll"),
this.clearSingleTable("dataUserAdapterAll"),this.clearSingleTable("executedStoreAll"),this.clearSingleTable("executedUserStoreAll"),this.clearSingleTable("dstdata"),this.clearSingleTable("pollStorage"),this.clearSingleTable("quizData"),this.clearSingleTable("dstall")]);virtualclass.gObj.hasOwnProperty("sessionEndResolve")&&virtualclass.gObj.sessionEndResolve()},clearLastTable(){virtualclass.gObj.hasOwnProperty("sessionEndResolve")&&virtualclass.gObj.sessionEndResolve()},async getQuizData(a){const b=
[];let c=await this.db.transaction("quizData").store.openCursor();for(;c;)b[c.value.quizkey]=c.value.qzData,c=await c.continue();a(b)},clearTableData(a){this.db.transaction(a,"readwrite").store.clear()},async getWbData(a){const b=await (await this.db.transaction("wbData","readwrite")).objectStore("wbData").get(a);"undefined"!==typeof b?(console.log(`Whiteboard start store from local storage ${a}`),virtualclass.gObj.tempReplayObjs[a]=[],virtualclass.gObj.tempReplayObjs[a]=JSON.parse(b.repObjs)):virtualclass.gObj.tempReplayObjs[a]=
"nodata"}}})(window);(function(d,c){d.user=function(a){return{assignRole(a,c,g){roles.hasControls()&&(roles.isEducator()||virtualclass.html.leftAppBar(),virtualclass.attachFunction(),"Whiteboard"==c&&d.virtualclass.wb[virtualclass.gObj.currWb].attachToolFunction(virtualclass.gObj.commandToolsWrapperId[virtualclass.gObj.currWb],!0,virtualclass.gObj.currWb),virtualclass.hasOwnProperty("previrtualclass")?virtualclass.vutil.makeActiveApp(`virtualclass${c}`,virtualclass.previrtualclass):virtualclass.vutil.makeActiveApp(`virtualclass${c}`),
...
this.elem.uiChatboxLog;c.scrollTop(c.get(0).scrollHeight)}}},toggleContent(c){this.uiChatboxContent.toggle();this.uiChatboxContent.is(":visible")&&this.uiChatboxInputBox.focus()},widget(){return this.uiChatbox},_create(){const c=this,a=(c.uiChatbox=d("<div></div>")).appendTo(document.getElementById("congreaChatCont")).addClass("ui-widget ui-corner-top ui-chatroom disable").prop("id","chatrm");uiChatboxContent=(c.uiChatboxContent=d("<div></div>")).addClass("ui-widget-content ui-chatbox-content ").appendTo(a);
uiChatboxLog=(c.uiChatboxLog=c.element).addClass("ui-widget-content ui-chatbox-log").appendTo(uiChatboxContent);uiChatboxInputBox=(c.uiChatboxInputBox=d("<input></input>")).addClass("ui-widget-content ui-chatbox-input-box ").prop("id","ta_chrm2").prop("placeholder","Send message...").appendTo(document.querySelector(".congrea #congchatBarInput")).keydown(function(a){if(a.keyCode&&a.keyCode==d.ui.keyCode.ENTER){msg=d.trim(d(this).val());a=(new Date).getTime();var b={receiver:"chatroom",msg,cf:"msg",
time:a};0<msg.length&&(ioAdapter.mustSend(b),d(this).val(""),c.options.messageSent({name:io.cfg.userobj.name},{msg,time:a}),null!=localStorage.getItem("chatroom")?(b=JSON.parse(localStorage.getItem("chatroom")),a={userid:io.cfg.userid,name:io.cfg.userobj.name,msg,time:a},b.push(a),localStorage.setItem("chatroom",JSON.stringify(b))):(a={userid:io.cfg.userid,name:io.cfg.userobj.name,msg,time:a},localStorage.setItem("chatroom",JSON.stringify([a]))),virtualclass.chat.commonChat.push(a));return!1}}).focusin(()=>
{uiChatboxInputBox.addClass("ui-chatbox-input-focus")}).focusout(()=>{uiChatboxInputBox.removeClass("ui-chatbox-input-focus")});uiChatboxContent.children().click(()=>{c.uiChatboxInputBox.focus()});c._setWidth(c.options.width);c._position(c.options.offset);c.options.boxManager.init(c);roles.isStudent()&&virtualclass.settings.studentgc(virtualclass.settings.info.studentgc);c.options.hidden||(a.show(),virtualclass.vutil.isPlayMode()&&disCommonChatInput())},_setOption(c,a){if(null!=a)switch(c){case "hidden":a?
this.uiChatbox.hide():this.uiChatbox.show();break;case "offset":this._position(a);break;case "width":this._setWidth(a)}d.Widget.prototype._setOption.apply(this,arguments)},_setWidth(c){this.uiChatbox.width(`${c}px`)},_position(c){this.uiChatbox.css("left",c)}})})(jQuery);Array.indexOf||(Array.prototype.indexOf=function(d){for(let c=0;c<this.length;c++)if(this[c]==d)return c;return-1});
...
this.curr);virtualclass.precheck.initHandler(`${"#preCheckcontainer .precheck"} #${this.curr}Buttons .next`,this.curr)});const c=virtualclass.media.sessionConstraints();virtualclass.precheck.session=c[1];null==virtualclass.adpt&&(virtualclass.adpt=new virtualclass.adapter);virtualclass.precheck.cNavigator=virtualclass.adpt.init(navigator);virtualclass.precheck.cNavigator.mediaDevices.getUserMedia(virtualclass.precheck.session).then(a=>{console.log("GEtting stream");virtualclass.precheck.mediaStream=
a})},bandWidthInWords(d){console.log(`bandwidth speed ${d}`);600<d?(d="high",virtualclass.videoHost.gObj.MYSPEED=1):400<d?(d="medium",virtualclass.videoHost.gObj.MYSPEED=2):(virtualclass.videoHost.gObj.MYSPEED=3,d="low");ioAdapter.sendSpeed(virtualclass.videoHost.gObj.MYSPEED);return d},measureConnectionSpeed(d){const c=new Image;c.onload=()=>{this.endTime=(new Date).getTime();const a=this.calculateSpeed();d(virtualclass.lang.getString(`${this.bandWidthInWords(a)}BandWidthSpeed`))};c.onerror=function(a,
b){d(virtualclass.lang.getString("bandwitdhImageNotFound"))};this.startTime=(new Date).getTime();c.src=this.imageAddr+`?nnn=${this.startTime}`},calculateSpeed(){const d=((8*this.downloadSize/((this.endTime-this.startTime)/1E3)).toFixed(2)/1024).toFixed(2);return Math.round(d)}},speaker:{prev:"bandwidth",curr:"speaker",next:"mic",perform(){const d=document.createElement("div");d.innerHTML=virtualclass.lang.getString("speakerTest");document.querySelector(`${`#preCheckcontainer .precheck.${this.curr}`} .result`).appendChild(d);
...
{};for(const a in virtualclass.settings.info)d[a]=virtualclass.settings.info[a];d[a]=c;c=virtualclass.settings.settingsToHex(d)}ioAdapter.mustSendUser({cf:"settings",Hex:c,toUser:b},b);virtualclass.settings.user[b]=c;localStorage.setItem("userSettings",JSON.stringify(virtualclass.settings.user))},applyAttendeeSetting(c){for(const a in c)if(virtualclass.settings.info[a]!==c[a]&&(virtualclass.settings.info[a]=c[a],"trimRecordings"!==a))virtualclass.settings[a](c[a])},onMessage(c){if("string"===typeof c){if(roles.isStudent()){const a=
virtualclass.settings.parseSettings(c);this.applyAttendeeSetting(a);localStorage.setItem("settings",c)}}else this.recording.triggerSetting(c)},studentaudio(c){roles.isStudent()&&(!0===c?(virtualclass.user.control.audioWidgetEnable(!0),virtualclass.gObj.audioEnable=!0):!1===c&&(c=document.querySelector("#speakerPressOnce"),!0!==c.dataset.audioPlaying&&"true"!==c.dataset.audioPlaying||virtualclass.media.audio.clickOnceSpeaker("speakerPressOnce"),virtualclass.user.control.audioDisable(),virtualclass.gObj.audioEnable=
!1))},allowoverride(){console.log("TO DO")},studentpc(c){console.log("TO DO");!0===c?virtualclass.user.control.allChatEnable("pc"):!1===c&&virtualclass.user.control.disbaleAllChatBox()},studentgc(c){console.log("TO DO");!0===c?virtualclass.user.control.allChatEnable("gc"):virtualclass.user.control.disableCommonChat()},studentvideo(c){if(roles.isStudent()){const a=document.querySelector(".videoSwitchCont #videoSwitch");a.classList.contains("off")&&!1===c?console.log("do nothing"):a.classList.contains("on")&&
!0===c?console.log("do nothing"):a.classList.contains("on")&&!1===c&&virtualclass.vutil.videoHandler("off");virtualclass.videoHost.toggleVideoMsg(!1===c?"disable":"enable")}},raisehand(){console.log("TO DO")},userlist(){console.log("TO DO")},attendeerecording(){console.log("TO DO")},enableRecording(){console.log("TO DO")},recAllowpresentorAVcontrol(){console.log("TO DO")},recShowPresentorRecordingStatus(){console.log("TO DO")},attendeeAV(){console.log("TO DO")},recallowattendeeAVcontrol(){console.log("TO DO")},
...
virtualclass.joinUser=this.getJoinUser(virtualclass.connectedUsers,virtualclass.jId)}else console.log("User packet is not receving");virtualclass.jId===virtualclass.gObj.uid&&(virtualclass.videoHost.setDefaultValue("undefined"===typeof virtualclass.videoHost.gObj.MYSPEED?1:virtualclass.videoHost.gObj.MYSPEED),virtualclass.vutil.setDefaultScroll(),roles.hasControls()&&!localStorage.getItem("userSettings")&&(c=localStorage.getItem("settings"),null!==c&&ioAdapter.mustSend({cf:"settings",Hex:c}),virtualclass.settings.info.recAllowpresentorAVcontrol&&
(c=localStorage.getItem("recordingButton"),null!==c&&ioAdapter.mustSend({ac:JSON.parse(c),cf:"recs"}))));virtualclass.gObj.mySetTime=2E3;console.log("==== Add users");defaultOperation(d,void 0);roles.hasControls()&&virtualclass.poll.updateUsersOnPoll();if("undefined"!==typeof virtualclass.quiz&&0>virtualclass.quiz.uniqueUsers.indexOf(virtualclass.jId))for(a in virtualclass.quiz.uniqueUsers.push(virtualclass.jId),io.uniquesids)0>virtualclass.quiz.uniqueUsers.indexOf(a)&&virtualclass.quiz.uniqueUsers.push(a);
"s"==virtualclass.joinUser.role&&virtualclass.gObj.has_ts_capability&&ioAdapter.mustSend({uid:virtualclass.gObj.uid,ac:!0,cf:"tsr"});if(virtualclass.gObj.uid!=virtualclass.jId&&virtualclass.gObj.meetingMode)virtualclass.multiVideo.onUserJoin(virtualclass.jId);!roles.hasControls()&&("t"===d.message.role||virtualclass.gObj.uid==virtualclass.jId&&virtualclass.vutil.whoIsTeacher())&&(d=document.querySelector("#virtualclassCont.congrea"),d.classList.contains("tr_available")||d.classList.add("tr_available"))},
newmessage(d){const c=d.message;if(c.hasOwnProperty("cf"))if("function"===typeof receiveFunctions[c.cf])receiveFunctions[c.cf](d);else console.log(`CF ${c.cf} is not a function of receiveFunctions`)},readyto_user_logout(d){for(const c in d.action)d={type:"user_logout",fromUser:c,message:"offline"},virtualclass.ioEventApi.user_logout(d)},user_logout(d){console.log("user_logout");if(!roles.hasControls()&&(virtualclass.gObj.hasOwnProperty("whoIsTeacher")||(virtualclass.gObj.whoIsTeacher=virtualclass.vutil.whoIsTeacher()),
virtualclass.gObj.whoIsTeacher==d.fromUser)){var c=document.querySelector("#virtualclassCont.congrea");c&&c.classList.contains("tr_available")&&c.classList.remove("tr_available")}virtualclass.media.video.removeUser(d.fromUser);virtualclass.chat.isTechSupportExist(d.fromUser)&&virtualclass.chat.disableTechSupport(d.fromUser);c=d.fromUser;delete virtualclass.gObj.allUserObj[c];d=getPosition(virtualclass.connectedUsers,c);-1!==d&&virtualclass.connectedUsers.splice(d,1);d={removeUser:c};memberUpdateWithDelay(d,
"removed");if(virtualclass.gObj.meetingMode)virtualclass.multiVideo.onUserRemove(c);virtualclass.connectedUsers.length<virtualclass.gObj.userToBeDisplay&&(virtualclass.gObj.insertUser=!0)},error(d){virtualclass.gObj.displayError?(virtualclass.view.removeElement("serverErrorCont"),"undefined"==typeof d.message?console.log("Error ",d.message):("undefined"!==typeof d.message.stack?virtualclass.view.displayServerError("serverErrorCont",d.message.stack):console.log(`Error message ${d.message.stack} could not display`),
...
this.settings=function(d){if(d.message.hasOwnProperty("Hex"))virtualclass.settings.onMessage(d.message.Hex)};this.ena=function(d){};this.dia=function(d){};this.aEna=function(d){};this.aDia=function(d){};this.msg=function(d){messageUpdate(d)};this.sEnd=async function(d){document.querySelector("#joinClassModal").style.display="none";d=document.querySelector("#virtualclassCont #virtualclassApp");null!=d&&(d.style.display="block");await virtualclass.config.endSession();virtualclass.popup.sesseionEndWindow();
...
delete virtualclass.videoUl.player}};this.sync=function(){};this.recs=function(d){if(virtualclass.isPlayMode)if(virtualclass.videoHost.gObj.videoSwitch)virtualclass.videoHost.onmessage("on");else virtualclass.videoHost.onmessage("off");else if(virtualclass.settings.info.attendeeAV)virtualclass.settings.onMessage(d.message)}};function Config(){}Config.prototype.setNewSession=function(d){localStorage.setItem("mySession",d);!virtualclass.isPlayMode&&roles.hasControls()&&(d=(new Date).getTime(),d=JSON.stringify({createdDate:d}),localStorage.setItem("myConfig",d))};
Config.prototype.endSession=async function(d){console.log("==== End the session here");delete virtualclass.connectedUsers;virtualclass.gObj.hasOwnProperty("memberUpdateDelayTimer")&&(clearTimeout(virtualclass.gObj.memberUpdateDelayTimer),virtualclass.gObj.memberlistpending.length=0,delete virtualclass.gObj.memberUpdateDelayTimer);virtualclass.hasOwnProperty("poll")&&""!==virtualclass.poll&&(virtualclass.poll.pollState={},virtualclass.poll.dataRec={});var c=document.getElementById("congrealogo");null!=
...
!1)),null!=virtualclass.editorRich&&virtualclass.editorRich.removeEditorData(),virtualclass.pdfRender={});virtualclass.vutil.removeClass("audioWidget","fixed");await virtualclass.storage.clearStorageData();virtualclass.wbCommon.removeAllContainers();virtualclass.gObj.wbCount=0;virtualclass.gObj.currSlide=0;virtualclass.currApp=virtualclass.gObj.defaultApp;virtualclass.gObj.hasOwnProperty("disableCamByUser")?virtualclass.user.control.audioDisable():virtualclass.user.control.audioWidgetEnable(!0);virtualclass.user.control.allChatEnable();
...
[],b.gObj.veryFirstJoin=!0,await b.storage.init())};c.prototype.validDateSession=async function(){var a=localStorage.getItem("prvUser");const b=localStorage.getItem("thisismyplaymode");virtualclass.gObj.myConfig=localStorage.getItem("myConfig");virtualclass.isPlayMode||null!==b&&"thisismyplaymode"===b?await virtualclass.config.endSession():null!==a?(a=JSON.parse(a),a.id===wbUser.id&&a.room===wbUser.room&&wbUser.role===a.role&&a.settings===virtualclassSetting.settings||await virtualclass.config.endSession()):
null!==virtualclass.gObj.myConfig&&(a=JSON.parse(virtualclass.gObj.myConfig),a=a.createdDate,108E5<(new Date).getTime()-a&&await virtualclass.config.endSession())};c.prototype.loadData=function(){roles.hasControls()&&localStorage.setItem("uRole",virtualclass.gObj.uRole);null===localStorage.getItem("prvUser")&&virtualclass.setPrvUser();virtualclass.createMainContainer();var a=localStorage.getItem("wIds");null!=a&&(a=JSON.parse(a),virtualclass.gObj.wIds=a,virtualclass.gObj.wbCount=a.length-1);const b=
JSON.parse(localStorage.getItem("prevApp"));virtualclass.gObj.prevApp=b;if(null!=b){virtualclass.previousApp=b;var c=b.name;c=c.charAt(0).toUpperCase()+c.slice(1);if("Yts"==b.name||"DocumentShare"==b.name)if(null==b.metaData)var d=null;else d=b.metaData,d.fromReload=!0;else"Video"==b.name?null==b.metaData||null==b.metaData.init?d=null:(d=b.metaData,d.fromReload=!0):"Whiteboard"==b.name&&(null==a&&(virtualclass.gObj.wbCount=b.wbn),a=localStorage.getItem("currSlide"),null!=a&&(virtualclass.gObj.currSlide=
a,console.log("==== current slide ",virtualclass.gObj.currSlide)))}else c=virtualclass.gObj.defaultApp;"undefined"===typeof d&&(d=null);virtualclass.precheck=localStorage.getItem("precheck");a=localStorage.getItem("precheck");null!=a&&(virtualclass.isPrecheck=JSON.parse(a));wbUser.virtualclassPlay?(virtualclass.isPrecheck=!1,virtualclass.enablePreCheck=!1):(virtualclass.precheck=localStorage.getItem("precheck"),virtualclass.enablePreCheck=!0,a=localStorage.getItem("precheck"),null!=a&&(virtualclass.isPrecheck=
JSON.parse(a)));!virtualclass.enablePreCheck||null!=virtualclass.isPrecheck&&virtualclass.isPrecheck?(virtualclass.makePreCheckAvailable=!1,virtualclass.popup.waitMsg("refresh")):virtualclass.makePreCheckAvailable=!0;virtualclass.appData.appIs=c;virtualclass.appData.videoObj=d};c.prototype.appInit=async function(){virtualclass.settings.init();await virtualclass.init(wbUser.role,virtualclass.appData.appIs,virtualclass.appData.videoObj);if("Edge"===virtualclass.system.mybrowser.name){const a=document.getElementById("virtualclassCont");
null!=a&&a.classList.add("edge")}wbUser.virtualclassPlay||virtualclass.editorRich.veryInit();virtualclass.gObj.testChatDiv=document.querySelector("#chat_div");virtualclass.gObj.testChatDiv.attachShadow({mode:"open"})};c.prototype.setUpMedia=async function(){if(roles.isStudent()){const a=localStorage.getItem("audEnable");null!==a&&"false"===a.ac&&(virtualclass.user.control.mediaWidgetDisable(),virtualclass.gObj.audioEnable=!1)}virtualclass.gObj.hasOwnProperty("audIntDisable")||virtualclass.gObj.hasOwnProperty("vidIntDisable")||
await virtualclass.media.init()};c.prototype.readyToGo=async function(){null!=virtualclass.isPrecheck&&("undefined"===typeof virtualclass.videoHost.gObj.MYSPEED&&(virtualclass.videoHost.gObj.MYSPEED=1),await virtualclass.videoHost.afterSessionJoin())};d.Bootstrap=c})(window);$.uiBackCompat=!1;(function(d){d.onload=async function(){const c=new d.Bootstrap;await c.setBasicData(d);await c.validDateSession();await c.loadData();await c.appInit();await c.setUpMedia();await c.readyToGo()}})(window);
