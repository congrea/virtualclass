...
{this.afterReceivedStream(b)})}getFileName(c){const b=c.lastIndexOf("/");return c.substring(b+1).replace(".chvs","")}duringPlayFirstPacket(){var c=document.getElementById("virtualclassVideo");c&&(c.dataset.currapp="liveStream");if(c=document.getElementById("messageLayoutVideo"))c.style.display="none"}afterReceivedStream(c){const b=c.data.slice(4,c.data.length);c=this.getFileName(c.config.url);this.listStream[c]=b;this.playIfReady(c)}onBuffer(c){this.sourceBuffer&&this.sourceBuffer.appendBuffer(c)}readyParticipateVideo(){this.remoteVideo||
(this.remoteVideo=document.getElementById("liveStream"),this.remoteVideo.srcObject=null,this.remoteVideo.currentTime=0)}initPLayerForParticipaes(){"undefined"!=typeof MediaSource&&(this.mediaSource=new MediaSource,this.mediaSource.addEventListener("sourceopen",this.mediaSourceOpen.bind(this)),this.readyParticipateVideo(),this.remoteVideo.src=URL.createObjectURL(this.mediaSource))}mediaSourceOpen(){"open"===this.mediaSource.readyState&&(this.sourceBuffer=this.mediaSource.addSourceBuffer(this.mimeType),
this.sourceBuffer.addEventListener("error",c=>{console.log(c)}),this.sourceBuffer.addEventListener("updateend",()=>{const c=this.fileList.getNextByID(virtualclass.liveStream.currentExecuted);c&&this.playIfReady(c.id)}))}handlLiveStream(c){"0"===c.currentTarget.dataset.sharinglivestream?this.startToShare():(this.stop(),c=document.querySelector("#dashboardnav button"),null!=c&&c.click())}disableLiveStreamButton(){const c=document.getElementById("startLiveStream");null!=c&&c.classList.add("disabled")}async startToShare(){this.stopTraditionalVideo();
...
this.mediaRecorder.addEventListener("stop",this.stopHandler.bind(this));this.mediaRecorder.addEventListener("dataavailable",this.handleLiveStreamData.bind(this));this.mediaRecorder.start(2E3)}stopHandler(){console.log("====STOP 1")}handleLiveStreamData(c){console.log("====> triggering data ");this.mediaRecorder&&"recording"===this.mediaRecorder.state&&this.saveToStreamServer(c.data)}async saveToStreamServer(c){c=await this.videoTypedArray(c);ioAdapter.sendStream(c)}async videoTypedArray(c){c=await c.arrayBuffer();
c=new Uint8Array(c);let b=new c.constructor(2);b[0]=72;b[1]=78;let a=new c.constructor(c.length+b.length);a.set(b,0);a.set(c,b.length);return a}stop(){document.getElementById("startLiveStream").dataset.sharinglivestream="0";this.mediaRecorder&&"inactive"!==this.mediaRecorder.state&&this.mediaRecorder.stop();this.hideLiveStreamHTML();this.disableLiveStreamButton();roles.hasControls()&&(ioAdapter.mustSend({cf:"liveStream",stop:!0}),this.clearEveryThing());setTimeout(()=>{const c=document.getElementById("startLiveStream");
...
if(this.startingPoint&&c===this.startingPoint&&this.inStreamList(this.firstFile)){b=this.inStreamList(this.firstFile);try{this.onBuffer(b),console.log("Actual append buffer ",this.firstFile),this.currentExecuted=this.firstFile,delete this.listStream[this.firstFile],this.startedAppending=!0,this.duringPlayFirstPacket(),this.startFromPageRefresh&&setTimeout(()=>{document.getElementById("liveStream").currentTime=this.MAX_TIME;console.log("REFRESH THE SCREEN")},virtualclass.isPlayMode?700:1600)}catch(a){this.requestInitializePacket(c),
console.log("====> Error handlling request packet")}}else if(this.startedAppending&&this.isMyTurn(c)&&b)try{this.onBuffer(b),console.log("Actual append buffer ",c),delete this.listStream[c],this.currentExecuted=c}catch(a){this.requestInitializePacket(c),console.log("====> Error handlling request packet")}}playIfReadyOGV(){if(!this.ogvPlayerReady&&this.inStreamList(this.firstFile)&&1<Object.keys(this.listStream).length&&1<virtualclass.liveStream.fileList.ol.order.length){this.readyOGVInstance();this.ogvPlayerReady=
!0;const c=this.inStreamList(this.firstFile);this.ogvPlayer._loadCodec(c,function(b){virtualclass.liveStream.currentExecuted=virtualclass.liveStream.firstFile;virtualclass.liveStream.ogvPlayer._startProcessingVideo(b);console.log("Current executed file input ",virtualclass.liveStream.currentExecuted);delete virtualclass.liveStream.listStream[virtualclass.liveStream.firstFile];virtualclass.liveStream.startedAppending=!0;virtualclass.liveStream.duringPlayFirstPacket();virtualclass.liveStream.ogvPlayerLoadedMedia&&
clearInterval(virtualclass.liveStream.ogvPlayerLoadedMedia);virtualclass.liveStream.ogvPlayerLoadedMedia=setInterval(()=>{virtualclass.liveStream.ogvPlayer&&virtualclass.liveStream.ogvPlayer._codec&&virtualclass.liveStream.ogvPlayer._codec.loadedAllMetadata&&(clearInterval(virtualclass.liveStream.ogvPlayerLoadedMedia),virtualclass.system.mybrowser.iOS?virtualclass.popup.infoMsg(virtualclass.lang.getString("continueToLiveStream"),virtualclass.liveStream._playIfReadyOGVFinal):virtualclass.liveStream.ogvPlayer.play())},
500)})}}_playIfReadyOGVFinal(){virtualclass.liveStream.ogvPlayer.play()}destroyOGVPlayer(){this.ogvPlayer&&(this.ogvPlayer._stopPlayback(),this.ogvPlayer.stop());delete this.ogvPlayer;delete virtualclass.liveStream.listStream;virtualclass.liveStream.listStream={};delete this.ogvPlayerReady;const c=document.getElementById("ogvVideoContainer");null!=c&&c.parentNode.removeChild(c)}readyOGVInstance(){this.ogvPlayer=new OGVPlayer({forceWebGL:!0,debug:!1});const c=document.createElement("div");c.id="ogvVideoContainer";
c.appendChild(this.ogvPlayer);document.getElementById("liveStreamCont").appendChild(c)}getChunkForOgvPlayer(){const c=this.fileList.getNextByID(this.currentExecuted);return c?(virtualclass.liveStream.tempFile=c.id,this.inStreamList(c.id)):!1}isScriptAlreadyIncluded(c){for(var b=document.getElementsByTagName("script"),a=0;a<b.length;a++)if(b[a].getAttribute("src")==c)return!0;return!1}};$.uiBackCompat=!1;
