...
virtualclass.prevScreen.hasOwnProperty("currentStream")&&virtualclass.prevScreen.unShareScreen();virtualclass.previrtualclass=`virtualclass${virtualclass.gObj.defaultApp}`},async init(b,a,c){b=document.getElementById("virtualclassCont");b.classList.add("loading");const {wbUser:f}=d;virtualclass.uInfo={userid:f.id,sid:f.sid,rid:f.path,authuser:f.auth_user,authpass:f.auth_pass,userobj:{userid:f.id,name:f.name,lname:f.lname,img:f.imageurl,role:null!=localStorage.getItem("uRole")?localStorage.getItem("uRole"):
f.role},room:f.room};this.gObj.congCourse=typeof("undefiend"!=d.congCourse)?d.congCourse:0;this.wbConfig={id:`virtualclass${virtualclass.apps.wb}`,classes:"appOptions"};this.ssConfig={id:`virtualclass${virtualclass.apps.ss}`,classes:"appOptions"};this.edConfig={id:`virtualclass${virtualclass.apps.er}`,classes:"appOptions"};this.edCodeConfig={id:`virtualclass${virtualclass.apps.ec}`,classes:"appOptions"};this.ptConfig={id:`virtualclass${virtualclass.apps.sp}`,classes:"appOptions"};this.plConfig={id:`virtualclass${virtualclass.apps.poll}`,
...
const c=virtualclass.view.customCreateElement("div","contQzBody","modal-body");b.appendChild(c);this.UI.modalContentUI();$("#slickQuiz").slickQuiz(a)},saveInLocalStorage(){const a=virtualclass.quiz;null!=document.querySelector("#qztime")&&(a.quizSt.qtime=document.querySelector("#qztime").textContent,console.log(`quiz=${a.quizSt.qtime}`));null!=document.querySelector("#elsTime")&&(a.quizSt.qtime=document.querySelector("#elsTime").textContent,console.log(`quiz=${a.quizSt.qtime}`));console.log(`quiz=${a.quizSt.screen}`);
null!=document.getElementById("closeQzBt")&&1==document.getElementById("closeQzBt").disabled&&(a.quizSt.qClosed="true");a.quizSt&&localStorage.setItem("quizSt",JSON.stringify(a.quizSt));a.quizAttempted&&(console.log(`attempt data =${a.quizAttempted}`),virtualclass.storage.quizStorage("qAttempt",JSON.stringify(a.quizAttempted)))},onmessage(a,c){const b=virtualclass.quiz;"stdPublish"==a.quiz.quizMsg&&(b.dataRec=a.quiz.data,virtualclass.storage.quizStorage("qData",JSON.stringify(b.dataRec)),b.quizSt.screen=
"stdPublish",this.quizDisplay(a.quiz.data));if("stdShowResult"==a.quiz.quizMsg||"quizTimeEnd"==a.quiz.quizMsg){null!=document.querySelector("#timeText")&&(document.querySelector("#timeText").textContent="Quiz has been closed");var d=document.querySelector("#slickQuiz .quizResults");d&&"block"!=d.style.display&&(d=document.querySelectorAll("#slickQuiz .nextQuestion"),d[d.length-1].click())}"quizClosed"==a.quiz.quizMsg&&(localStorage.removeItem("quizSt"),this.quizSt={},localStorage.removeItem("qRep"),
virtualclass.storage.clearTableData("quizData"),this.UI.defaultLayoutForStudent());if("quizAttempt"==a.quiz.quizMsg&&roles.hasControls()){"undefined"===typeof this.attemptedUsers[a.quiz.questionId]&&(this.attemptedUsers[a.quiz.questionId]={});d=a.quiz.user;"undefined"===typeof this.attemptedUsers[a.quiz.questionId][d]&&(this.attemptedUsers[a.quiz.questionId][d]={});this.attemptedUsers[a.quiz.questionId][d]=a.quiz.ans;d=Object.keys(this.attemptedUsers[a.quiz.questionId]).length;let b=0;for(var f in this.attemptedUsers[a.quiz.questionId])1==
...
(a.quizAttempted[b].cA/a.quizAttempted[b].uA*100).toFixed(2),d=document.getElementById(`qPb_${b}`);d.innerHTML=`${c}% correct`;d.style.width=`${c}%`}},displayGradeReport(){const a=virtualclass.quiz;for(let b=0;b<a.qGrade.length;b++)a.gradeReport(b+1,a.qGrade[b].nm,a.qGrade[b].tt,a.qGrade[b].sc,a.qGrade[b].qAt,a.qGrade[b].ca)},attemptProgressReport(){var a=JSON.parse(virtualclass.quiz.quizJSON);const c=virtualclass.view.customCreateElement("div","attemptQzBody","modal-body");a=a.questions;let d=1;
for(i=0;i<a.length;i++){const b=virtualclass.view.customCreateElement("div","","q-area");c.appendChild(b);var k=virtualclass.view.customCreateElement("span","","q-no");k.innerHTML=d;b.appendChild(k);k=virtualclass.view.customCreateElement("div","","qscolor");k.innerHTML=a[i].q;b.appendChild(k);k=virtualclass.view.customCreateElement("div");k.innerHTML=`Users attempted : <span id='usA_${a[i].qid}'>0</span>`;b.appendChild(k);k=this.UI.createProgressBar(a[i].qid,0);b.appendChild(k);d++}return c},tabContent(){const a=
document.getElementById("resultQzLayout"),c=virtualclass.view.customCreateElement("div","","tab-content");var d=virtualclass.view.customCreateElement("div","qzOverv");d.className="tab-pane fade in active";var k=this.attemptProgressReport();d.appendChild(k);c.appendChild(d);d=virtualclass.view.customCreateElement("div","gdRpt");d.className="tab-pane fade";k=this.UI.gradeReportLayout();d.appendChild(k);c.appendChild(d);a.appendChild(c)},gradeReport(a,c,d,k,m,n){var b=document.getElementById("qzReTbody");
if(b){const f=document.createElement("tr");b.appendChild(f);b=document.createElement("th");b.scope="row";b.innerHTML=a;f.appendChild(b);a=document.createElement("td");a.innerHTML=c;f.appendChild(a);c=document.createElement("td");c.innerHTML=d;f.appendChild(c);d=document.createElement("td");d.innerHTML=`${k}%`;f.appendChild(d);k=document.createElement("td");k.innerHTML=m;f.appendChild(k);m=document.createElement("td");m.innerHTML=n;f.appendChild(m)}},quizTimer(a,c,d){function b(){g="asc"==d?a+((Date.now()-
f)/1E3|0):a-((Date.now()-f)/1E3|0);q=g/3600|0;r=g%3600/60|0;z=g%60|0;q=10>q?`0${q}`:q;r=10>r?`0${r}`:r;z=10>z?`0${z}`:z;c.textContent=`${q}:${r}:${z}`;0>=g&&("asc"!=d&&(c.textContent="00 : 00 : 00 "),f=0,"undefined"!==typeof CDTimer&&(clearInterval(CDTimer),console.log("Clear quiz interval")),ioAdapter.mustSend({quiz:{quizMsg:"quizTimeEnd",quizId:virtualclass.view.qzid},cf:"quiz"}),roles.hasControls()&&(document.getElementById("closeQzBt").disabled=!0))}d="undefined"!==typeof d?d:"desc";let f=Date.now(),
g,q,r,z;"asc"!==d&&b();CDTimer=setInterval(b,1E3)},quizModalClose(){const a=this,c=virtualclass.lang.getString("rusureCquiz");virtualclass.popup.confirmInput(c,b=>{b&&(localStorage.removeItem("quizSt"),a.quizSt={},virtualclass.storage.clearTableData("quizData"),localStorage.removeItem("qRep"),b=document.getElementById("editQuizModal"),b.parentNode.removeChild(b),a.attemptedUsers={},a.quizAttempted={},ioAdapter.mustSend({quiz:{quizMsg:"quizClosed",quizId:a.qzid},cf:"quiz"}))})},convertSecToTime(a){if(/^\d+$/.test(a)){const b=
new Date(null);b.setSeconds(a);return b.toISOString().substr(11,8)}return"00:00:00"},convertTimeToSec(a){return a?(a=a.split(":"),3600*+a[0]+60*+a[1]+ +a[2]):0},isJson(a){try{JSON.parse(a)}catch(f){return!1}return!0},UI:{id:"virtualclassQuiz",class:"virtualclass",container(){console.log("quiz layout check");var a=document.getElementById(this.id);null!=a&&a.parentNode.removeChild(a);a=virtualclass.view.customCreateElement("div",this.id,this.class);virtualclass.vutil.insertAppLayout(a);this.layout(a)},
layout(a){const b=virtualclass.view.customCreateElement("div","layoutQuiz","bootstrap container-fluid quizLayout");a.appendChild(b);a=document.createElement("nav");a.className="nav navbar ";b.appendChild(a);const c=virtualclass.view.customCreateElement("div","","vchead");c.innerHTML=virtualclass.lang.getString("Quiz");a.appendChild(c);a=virtualclass.view.customCreateElement("div","layoutQuizBody","quizMainCont");b.appendChild(a);this.createMszBox(a);this.createModalCont(a)},createModalCont(a){const b=
