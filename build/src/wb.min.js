'use strict';const screenDecodeBlob=URL.createObjectURL(new Blob(["(",function(){function d(a){myLoop=4*a.length;k=new Uint8ClampedArray(myLoop);for(let b=0;b<myLoop;b++)k[4*b]=36.5*(a[b]>>5),k[4*b+1]=36.5*((a[b]&28)>>2),k[4*b+2]=85*(a[b]&3),k[4*b+3]=255}function c(a,b,c,f,k){const d=g.width;let m=0;for(let n=b;n<f+b;n++)for(let b=a;b<c+a;b++){const a=4*(n*d+b);g.data[a]=k[m++];g.data[a+1]=k[m++];g.data[a+2]=k[m++];g.data[a+3]=k[m++]}}function a(a,c){a=b(a);c=b(c);return parseInt(a+c)}function b(a){a=
...
else if(3==a[1]&&0<this.recBinMsgQueue.length){this.recBinMsgQueue.push(a.subarray(2));for(c=b=0;c<this.recBinMsgQueue.length;c++)b+=this.recBinMsgQueue[c].length;b=101==a[0]?new Int8Array(b):new Uint8ClampedArray(b);for(a=c=0;c<this.recBinMsgQueue.length;c++)b.set(this.recBinMsgQueue[c],a),a+=this.recBinMsgQueue[c].length;postMessage({cmd:"endBinary",msg:b})}}},finallySend(c){this.sock.send(c)},sendBinary(c){if(this.sock.readyState&&c.length)if(6E5>=c.length){if(c.constructor===Int8Array)var a=new Int8Array(c.length+
2);else c.constructor===Uint8ClampedArray&&(a=new Uint8ClampedArray(c.length+2));a.set([c[0],0]);a.set(c,2);this.finallySend(a.buffer);postMessage({cmd:"stBinary",msg:a})}else{this.binMsgQueue=[];for(let b=0,f=0;b<c.length;f++)a=c.slice(b,b+55E4),c.constructor===Int8Array?this.binMsgQueue[f]=new Int8Array(a.length+2):c.constructor===Uint8ClampedArray&&(this.binMsgQueue[f]=new Uint8ClampedArray(a.length+2)),this.binMsgQueue[f].set([c[0],2]),this.binMsgQueue[f].set(a,2),a.length&&(b+=a.length);this.binMsgQueue[0][1]=
1;this.binMsgQueue[this.binMsgQueue.length-1][1]=3;for(c=0;c<this.binMsgQueue.length;c++)this.finallySend(this.binMsgQueue[c].buffer),postMessage({cmd:"stBinary",msg:this.binMsgQueue[c]});this.binMsgQueue=[]}},onMessage(c){switch(c.data.cmd){case "init":this.init(c.data.msg);break;case "wsconnect":this.wsconnect();break;case "disconnect":this.disconnect();break;case "sessionEndClose":this.sock&&this.sock.close();break;case "workerAudioRec":this.workerAudioRec=c.ports[0];this.workerAudioRec.onmessage=
function(a){a.data.hasOwnProperty("dest")&&"mainthread"==a.data.dest&&postMessage(a.data)};break;case "send":if(this.sock.readyState){if(0<d.packetQueue.length){for(let a=0;a<d.packetQueue.length;a++)this.finallySend(d.packetQueue[a]);d.packetQueue.length=0}this.finallySend(c.data.msg)}else d.packetQueue.push(c.data.msg);break;case "sendBinary":this.sendBinary(c.data.msg);break;case "workerAudioSend":this.workerAudioSend=c.ports[0];this.workerAudioSend.onmessage=this.fromWorkerAudioSend.bind(this);
break;case "onRecBinary":this.onRecBinary({data:c.data.msg});break;default:this.finallySend(c.data.msg)}},fromWorkerAudioSend(c){c.data.hasOwnProperty("cmd")&&this.sendBinary(c.data.msg)}};onmessage=function(c){d.onMessage(c)}}.toString(),")()"],{type:"application/javascript"})),workerIO=new Worker(workerIOBlob);/*
  2018 Suman Bogati  {@link http://vidyamantra.com}
 @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
d.poll();virtualclass.quiz=d.quiz();virtualclass.videoUl=d.videoUl();virtualclass.videoHost=d.videoHost;virtualclass.precheck=preCheck;virtualclass.page=page;console.log("==== session clear zoom object ready ");virtualclass.network=new Network;virtualclass.gesture=gesture;this.serverData=serverData;roles.hasControls()&&this.serverData.fetchAllData();null!=localStorage.uRole&&(virtualclass.gObj.uRole=localStorage.uRole,a.classList.add(virtualclass.vutil.getClassName(virtualclass.gObj.uRole)));"undefined"===
typeof g||null==g?this.makeAppReady(c,"byclick"):this.makeAppReady(c,"byclick",g);this.system.check();this.vutil.isSystemCompatible();this.system.mediaDevices.getMediaDeviceInfo();this.pageVisible=virtualclass.vutil.pageVisible();c==virtualclass.apps.ss&&this.system.setAppDimension();virtualclass.vutil.createReclaimButtonIfNeed();virtualclass.user.assignRole(virtualclass.gObj.uRole,c);this.media=new d.media;c=localStorage.getItem("precheck");null!=c&&(c=JSON.parse(c));virtualclass.makePreCheckAvailable?
virtualclass.precheck.init():(virtualclass.makeReadySocket(),virtualclass.isPlayMode||virtualclass.gesture.initClassJoin());a.classList.remove("loading");virtualclass.gObj.precheckScrn=!1;virtualclass.gObj.meetingMode||virtualclass.videoHost.init(320,240);virtualclass.vutil.videoController();c=document.getElementById("precheckSetting");c.addEventListener("click",()=>{virtualclass.gObj.precheckScrn=!0;virtualclass.precheck.init(virtualclass.precheck)});this.raiseHand=d.raiseHand;this.raiseHand.init();
this.appSetting=d.appSetting;this.appSetting.init();virtualclass.colorSelector=d.colorSelector;virtualclassSetting.theme.selectedColor&&this.colorSelector.makeThemeReady();c=document.getElementById("virtualclassCont");null!=c&&c.classList.add(virtualclass.system.mybrowser.name);c=document.getElementById("fullScreenButton");null!=c&&c.addEventListener("click",virtualclass.vutil.Fullscreen);c=document.getElementById("fullScreenExitButton");null!=c&&c.addEventListener("click",virtualclass.vutil.closeFullscreen);
...
{2<=b.gObj.editorInitDone&&(b.initSocketConn(),clearInterval(a))},100)}},initSocketConn(){this.system.webSocket&&(io.init(virtualclass.uInfo),d.userdata=virtualclass.uInfo)},html:{id:"virtualclassCont",optionsClass:"appOptions",init(a){this.virtualclass=a},leftAppBar(){if(!(0<document.getElementsByClassName("appOptions").length)){document.querySelector("#virtualclassApp #virtualclassAppLeftPanel");var a=virtualclass.getTemplate("appTools")();virtualclass.vutil.insertAppLayout(a);virtualclass.gObj.hasOwnProperty("errNotScreenShare")&&
...
a=document.createElement("div"),c&&c.appendChild(a))},xhrSendWithForm(c,a,b){const f=new FormData;for(const a in c)f.append(a,c[a]);c="undefined"===typeof a?d.webapi:`${d.webapi}&methodname=${a}&user=${virtualclass.gObj.uid}`;"undefined"!==typeof b?virtualclass.xhr.sendFormData(f,c,b):virtualclass.xhr.sendFormData(f,c)},createSaveButton(){const c=document.createElement("div");c.id="tech_support_save";const a=document.createElement("span");a.innerHTML="Save Session";c.appendChild(a);return c},setChatContHeight(c){},
setChatHeight(c){},alreadyConnected(c){return 0<virtualclass.connectedUsers.length?0<virtualclass.connectedUsers.filter(a=>a.userid==c).length:!1},UTCtoLocalTime(c){var a=new Date(c);c=a.getHours();a=`0${a.getMinutes()}`;return`${c}:${a.substr(-2)}`},UTCtoLocalTimeToSeconds(c){return c=(new Date(c)).getTime()},appIsForEducator(c){for(const a in virtualclass.apps)if(virtualclass.apps[a]==c)return!1;return!0},initDashboardNav(c){if(roles.hasControls()){var a=document.querySelector("#dashboardnav button");
null==a&&(a=virtualclass.getTemplate("dashboardNav")({app:virtualclass.currApp}),document.querySelector("#virtualclassAppOptionsCont").insertAdjacentHTML("beforeend",a),a=document.querySelector("#dashboardnav button"),null!=a&&(a.addEventListener("click",function(){const a=document.querySelector(".congrea #virtualclassApp");this.classList.contains("clicked")?(virtualclass.dashBoard.close(),a&&a.classList.contains("dashboard")&&a.classList.remove("dashboard")):(a&&(a.classList.contains("dashboard")||
...
virtualclass.vutil.showFinishBtn():virtualclass.vutil.removeFinishBtn()))}),c&&virtualclass.vutil.readyDashboard(c)));"DocumentShare"==virtualclass.currApp?virtualclass.dts.noteExist()?(virtualclass.dts.isUploaderExist()||virtualclass.vutil.modalPopup("docs",["docsuploadContainer"]),document.getElementById("dashboardnav").setAttribute("data-title",virtualclass.lang.getString("DocumentSharedbHeading"))):this.readyDashboard():"Video"==virtualclass.currApp?("undefined"===typeof c&&this.readyDashboard(),
(c=document.querySelector(".congrea #listvideo .linkvideo.playing"))||virtualclass.vutil.removeFinishBtn()):(this.readyDashboard(),(c=document.querySelector(".congrea #listppt .linkppt.playing"))||virtualclass.vutil.removeFinishBtn(),document.querySelector(".congrea .pptSharing")&&virtualclass.modal.hideModal())}},modalCloseHandler(){const c=document.querySelector(".congrea .dashboardContainer .modal-header .close");c&&c.addEventListener("click",()=>{const a=document.querySelector(".congrea #virtualclassApp");
a.classList.contains("dashboard")&&a.classList.remove("dashboard");c.setAttribute("data-dismiss","modal");virtualclass.modal.hideModal()})},readyDashboard(c){console.log("Ready Dashboard");var {currApp:a}=virtualclass;if(null==document.querySelector("#congdashboard")){var b=virtualclass.getTemplate("dashboard")({app:a});document.querySelector("#dashboardContainer").innerHTML=b}this.modalCloseHandler();if("DocumentShare"==a){var f=document.getElementById("dashboardnav");f.setAttribute("data-title",
virtualclass.lang.getString("DocumentSharedbHeading"));if(null==document.querySelector(`#${a}Dashboard`)){b=document.createElement("div");var g=document.querySelector("#congdashboard .modal-body");g.appendChild(b);b.id=`${a}Dashboard`}(f=document.querySelector("#docsDbCont"))&&f.parentNode.removeChild(f);document.querySelector(`#${a}Dashboard`).innerHTML=this.getDocsDashBoard(a);virtualclass.dts.init();virtualclass.vutil.attachEventToUploadTab();virtualclass.dts.isUploaderExist()||virtualclass.vutil.modalPopup("docs",
["docsuploadContainer"]);virtualclass.vutil.makeElementActive("#DocumentShareDashboard .qq-uploader-selector.qq-uploader.qq-gallery");virtualclass.vutil.makeElementActive("#listnotes")}else"Video"==a?(document.getElementById("dashboardnav"),null==document.querySelector(`#${a}Dashboard`)&&(b=document.createElement("div"),g=document.querySelector("#congdashboard .modal-body"),g.appendChild(b),b.id=`${a}Dashboard`),document.querySelector("#videoPopup")||(b=virtualclass.getTemplate("popup","videoupload")(),
document.querySelector("#VideoDashboard").insertAdjacentHTML("beforeend",b),(a=document.querySelector("#videoPopup  #uploadMsz div"))&&a.parentNode.removeChild(a),virtualclass.vutil.attachEventToUpload(),virtualclass.videoUl.UI.inputUrl()),virtualclass.videoUl.UI.popup(c),virtualclass.vutil.makeElementActive("#VideoDashboard .qq-uploader-selector.qq-uploader.qq-gallery"),virtualclass.vutil.makeElementActive("#listvideo"),c&&c.init.videoUrl&&(f=!0),a=document.querySelector("#dashboardnav button"),
null==a||f||virtualclass.vutil.isDashboardOpened()||a.click()):"SharePresentation"==a&&(f=document.getElementById("dashboardnav"),f.setAttribute("data-title",virtualclass.lang.getString("SharePresentationdbHeading")),null==document.querySelector(`#${a}Dashboard`)&&(b=document.createElement("div"),g=document.querySelector("#congdashboard .modal-body"),g.appendChild(b),b.id=`${a}Dashboard`))},initDashboard(c,a){document.querySelector("#mainContainer");"SharePresentation"!=c||document.querySelector("#pptDbCont")||
(null==document.querySelector(".docsDbCont")&&(document.querySelector("#SharePresentationDashboard").innerHTML=virtualclass.vutil.getPptDashBoard("SharePresentation")),virtualclass.sharePt.attachEvent("submitpurl","click",virtualclass.sharePt.initNewPpt),virtualclass.sharePt.ppts&&virtualclass.sharePt.ppts.length&&(virtualclass.sharePt.showPpts(virtualclass.sharePt.ppts),virtualclass.sharePt.retrieveOrder()));const b=document.querySelectorAll("#congdashboard .dbContainer");for(let a=0;a<b.length;a++)b[a].style.display=
b[a].dataset.app==virtualclass.currApp?"block":"none";console.log(`Dashboard is created for ${virtualclass.currApp}`);"DocumentShare"==c?("undefined"===typeof a&&(virtualclass.modal.showModal(),virtualclass.dashBoard.clickCloseButton()),virtualclass.dts.noteExist()&&virtualclass.vutil.hideUploadMsg("docsuploadContainer")):"Video"==c?"undefined"===typeof a&&virtualclass.modal.showModal():virtualclass.modal.showModal();virtualclass.dashBoard.actualCloseHandler();a=document.querySelector("#congdashboard .modal-header h4");
null!=a&&(a.innerHTML=virtualclass.lang.getString(`${c}dbHeading`))},getDocsDashBoard(c){const a=virtualclass.getTemplate("dashboard","documentSharing");c={app:c,hasControls:roles.hasControls()};return a(c)},getPptDashBoard(c){const a=virtualclass.getTemplate("dashboard","ppt");c={app:c,hasControls:roles.hasControls()};return a(c)},removeDashboardNav(){const c=document.querySelector("#dashboardnav");null!=c&&c.parentNode.removeChild(c)},makeElementDeactive(c){console.log(`drag drop Deactive element ${c}`);
c=document.querySelector(c);null!=c&&(c.style.pointerEvents="none")},makeElementActive(c){console.log(`drag drop Active element ${c}`);c=document.querySelector(c);null!=c&&(c.style.pointerEvents="visible")},initCommonSortingChat(){setTimeout(()=>{sortCommonChat()},800)},triggerDashboard(c,a){"DocumentShare"==c?null==document.querySelector("#screen-docs .note.current")&&("undefined"===typeof a?virtualclass.vutil.initDashboard(c):virtualclass.vutil.initDashboard(c,a)):"Video"==c?"undefined"===typeof a?
virtualclass.vutil.initDashboard(c):virtualclass.vutil.initDashboard(c,a):virtualclass.vutil.initDashboard(c)},trimExtension(c){return c.replace(/\.[^/.]+$/,"")},hideUploadMsg(c){c=document.querySelector(`#${c} .qq-uploader-selector.qq-uploader.qq-gallery`);null!=c&&c.setAttribute("qq-drop-area-text","")},showUploadMsg(c){c=document.querySelector(`#${c} .qq-uploader-selector.qq-uploader.qq-gallery`);null!=c&&c.setAttribute("qq-drop-area-text",virtualclass.lang.getString("dropfilehere"))},audioStatus(c,
a){const b=c.getElementsByClassName("congtooltip")[0];"true"==a?(c.setAttribute("data-audio-playing","true"),b.setAttribute("data-title",virtualclass.lang.getString("audioEnable")),c.className="audioTool active"):(c.setAttribute("data-audio-playing","false"),b&&b.setAttribute("data-title",virtualclass.lang.getString("audioDisable")),c.className="audioTool deactive")},selfVideoStatus(){const c=document.querySelector(".congrea .videoSwitchCont #videoSwitch");if(c.classList.contains("off"))return"off";
if(c.classList.contains("on"))return"on"},videoController(){const c=document.querySelector(".congrea .videoSwitchCont");c&&c.addEventListener("click",()=>{virtualclass.vutil.videoHandler("off"==virtualclass.vutil.selfVideoStatus()?"on":"off")})},videoHandler(c,a){let b;var f=document.querySelector(".congrea .videoSwitchCont #videoSwitch");if("on"==c){f.classList.remove("off");f.classList.add("on");b="on";f=document.querySelector(".videoSwitchCont");f.dataset.title="Video off";roles.hasControls()?
(virtualclass.videoHost.gObj.videoSwitch=1,console.log("videoswitch 1")):virtualclass.videoHost.gObj.stdStopSmallVid=!1;chatContainerEvent.elementFromShadowDom(`#ml${virtualclass.gObj.uid} .user-details a span`)||chatContainerEvent.elementFromShadowDom(`#ml${virtualclass.gObj.uid} .user-details a img`)?virtualclass.videoHost.removeUserIcon(virtualclass.gObj.uid):virtualclass.gObj.delayVid="hide";roles.hasControls()&&virtualclass.videoHost.UI.displayTeacherVideo();const a=document.querySelector(`#ml${virtualclass.gObj.uid} video`);
null!=a&&setTimeout(()=>{a.srcObject=virtualclass.media.stream},100)}else"off"==c&&(f.classList.remove("on"),f.classList.add("off"),b="off",f=document.querySelector(".videoSwitchCont"),f.dataset.title="Video on",virtualclass.videoHost&&(roles.hasControls()?virtualclass.videoHost.gObj.videoSwitch=0:virtualclass.videoHost.gObj.stdStopSmallVid=!0),chatContainerEvent.elementFromShadowDom(`#ml${virtualclass.gObj.uid} .user-details a .videoWrapper`)?virtualclass.videoHost.setUserIcon(virtualclass.gObj.uid):
virtualclass.gObj.delayVid="display",roles.hasControls()&&virtualclass.videoHost.UI.hideTeacherVideo());"undefined"===typeof a&&(virtualclass.gObj.meetingMode?virtualclass.multiVideo.setVideoStatus("off"!=c):virtualclass.gObj.uid==virtualclass.vutil.whoIsTeacher()&&ioAdapter.mustSend({congCtr:{videoSwitch:b},cf:"congController"}),roles.hasControls()||ioAdapter.mustSend({stdVideoCtr:{videoSwitch:virtualclass.videoHost.gObj.stdStopSmallVid},cf:"stdVideoCtrl"}))},isChromeExtension(){d.postMessage({type:"isInstalled",
id:1},"*");console.log("Chrome Extension:- Check");d.addEventListener("message",c=>{"yes"==c.data.type&&(virtualclass.gObj.chromeExt=!0)})},setWidth(c,a,b){a=document.querySelector(`#canvas${c}`);a.width=b},setHeight(c,a,b){a=document.querySelector(`#canvas${c}`);a.height=b},getWidth(c){return c.width},getHeight(c){return c.height},getValueWithoutPixel(c){return parseInt(c,10)},removeDecimal(c){return c.toFixed(2)},getElemM(c,a){if("Y"==a)var b=document.querySelector(`#${c}`).offsetHeight;else"X"==
a&&(b=document.querySelector(`#${c}`).offsetWidth);return this.getValueWithoutPixel(b)},getElemM2(c,a){if("Y"==a)var b=document.querySelector(`#${c}`).offsetHeight;else"X"==a&&(b=document.querySelector(`#${c}`).offsetWidth);return this.getValueWithoutPixel(b)},getElemHeight(c){c=document.querySelector(`#${c}`).offsetHeight;return this.getValueWithoutPixel(c)},getElemWidth(c){c=document.querySelector(`#${c}`).offsetWidth;return this.getValueWithoutPixel(c)},visibleElementHeighOldt(c,a){let b=0;for(c=
document.getElementById(c);c.offsetParent&&c.offsetParent.id!=a;)b+=c.offsetTop,c=c.offsetParent;return c.offsetHeight-b},elementIsVisible2(c){const a=c.getBoundingClientRect().top;c=c.getBoundingClientRect().bottom;return 0<=a&&c<=d.innerHeight},elementIsVisible(c){c=c.getBoundingClientRect();const a=Math.max(document.documentElement.clientHeight,d.innerHeight);return!(0>c.bottom||0<=c.top-a)},getVisibleHeight(c){var a=$(`#${c}`);c=$(d).scrollTop();const b=c+$(d).height(),f=a.offset().top;a=f+a.outerHeight();
return(a>b?b:a)-(f<c?c:f)},isNumeric(c){return!isNaN(parseFloat(c))&&isFinite(c)},setContainerDimension(c,a){},setDefaultScroll(){if(roles.hasControls()&&"Whiteboard"==virtualclass.currApp||"DocumentShare"==virtualclass.currApp){const c=virtualclass.gObj.currWb;null!=c&&null!=virtualclass.pdfRender[c].canvasWrapper&&(virtualclass.pdfRender[c].canvasWrapper.scrollTop=1)}},async createWhiteBoard(c){await virtualclass.appInitiator.Whiteboard.apply(virtualclass,Array.prototype.slice.call(["Whiteboard",
"byclick",c]));const a=virtualclass.system.measureResoultion({width:d.innerWidth,height:d.innerHeight});virtualclass.system.setCanvasWrapperDimension(a,c)},createHashString(c){let a=0;const b=c.length;for(let f=0;f<b;f++)a=31*a+c.charCodeAt(f),a&=a;return a},randomString(c){let a="";for(;0<c;--c)a+="0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"[Math.round(61*Math.random())];return a},isBulkDataFetched(){return 0<virtualclass.serverData.rawData.video.length||0<virtualclass.serverData.rawData.docs.length||
0<virtualclass.serverData.rawData.ppt.length},sendOrder(c,a,b){virtualclass.gObj.docOrder[c]=a;c={order:JSON.stringify(virtualclass.gObj.docOrder)};virtualclass.xhrn.sendData(c,virtualclass.api.UpdateRoomMetaData,b)},saveWbOrder(c){c&&(console.log(`nirmala${c}`),localStorage.setItem("wbOrder",JSON.stringify(virtualclass.wbCommon.order)))},requestOrder(c,a){virtualclass.xhrn.sendData({noting:!0},virtualclass.api.GetRoomMetaData,b=>{"Error"==b?console.log("page order retrieve failed"):(b=JSON.parse(b).Item,
null!=b&&"undefined"!==typeof b&&void 0!=b&&b.order.S&&virtualclass.vutil.IsJsonString(b.order.S)&&(b=JSON.parse(b.order.S),virtualclass.gObj.docOrder=b,a(b[c])))})},IsJsonString(c){try{JSON.parse(c)}catch(a){return!1}return!0},removeChildrens(c){console.log(`children selector ${c}`);c=document.querySelectorAll(c);for(let a=0;a<c.length;a++)c[a].parentNode.removeChild(c[a])},initAcitveElement(){if("Video"==virtualclass.currApp)var c="#VideoDashboard .qq-uploader-selector.qq-uploader.qq-gallery",a=
"#listvideo";else"DocumentShare"==virtualclass.currApp&&(c="#DocumentShareDashboard .qq-uploader-selector.qq-uploader.qq-gallery",a="#listdocs");const b=document.querySelector(`#${virtualclass.currApp}Dashboard`);null==b||b.classList.contains("uploading")?virtualclass.vutil.makeElementDeactive(c):(virtualclass.vutil.makeElementActive(c),virtualclass.vutil.makeElementDeactive(a))},isResponseAvailable(c){return void 0!=c&&"undefined"!=c&&null!=c},navWhiteboard(c,a,b){virtualclass.vutil.isTextWrapperExist()&&
virtualclass.wb[virtualclass.gObj.currWb].obj.drawTextObj.finalizeTextIfAny();"undefined"!==typeof b?a.call(c,b):a.call(c);console.log(`whiteboard nav time${virtualclass.gObj.wbNavtime}`)},removeAllTextWrapper(){const c=document.querySelectorAll(".canvasWrapper .textBoxContainer");for(let a=0;a<c.length;a++)c[a].parentNode.removeChild(c[a])},isDashboardOpened(c){c=document.querySelector("#dashboardnav button");return null!=c&&c.classList.contains("clicked")},stopConnection(){io.webSocketConnected()&&
io.disconnect();virtualclass.gObj.invalidlogin=!0},initDefaultApp:()=>{const c=document.querySelector(`#virtualclass${virtualclass.gObj.defaultApp}Tool a`);null!=c&&c.click(c)},initssSharing:c=>{virtualclass.gObj.whoIsSharing=c;c=chatContainerEvent.elementFromShadowDom(`#ml${c} .icon-stdscreenImg`);null!=c&&c.setAttribute("data-dcolor","green")},removeSSsharing:()=>{const c=document.querySelector("#chat_div .controleCont.ssSharing");null!=c&&c.classList.remove("ssSharing");delete virtualclass.gObj.whoIsSharing},
informIamSharing:()=>{const c=virtualclass.vutil.whoIsTeacher();roles.isStudent()&&virtualclass.gObj.studentSSstatus.mesharing&&ioAdapter.mustSendUser({cf:"sshare_user"},c)},resizeWindowIfBigger(){var {currWb:c}=virtualclass.gObj;c=document.querySelector(`#canvasWrapper${c}`);null!=c&&60<d.innerWidth-(c.offsetWidth+320)&&(console.log("Resize event is triggered"),system.initResize())},isTextWrapperExist(){return 0<document.querySelectorAll(".canvasWrapper .textBoxContainer").length},insertAppLayout:c=>
{document.querySelector("#virtualclassAppContainer");$("#virtualclassAppContainer").append(c)},prechkScrnShare(){if(localStorage.getItem("precheck")){var c=document.getElementById("preCheckcontainer");c.style.display="none";c=document.getElementById("virtualclassApp");c.style.display="block";virtualclass.videoHost._resetPrecheck()}else virtualclass.popup.waitMsg(),virtualclass.makeReadySocket(),c=document.getElementById("preCheckcontainer"),c.style.display="none",c=document.getElementById("virtualclassApp"),
c.style.display="block",localStorage.setItem("precheck",!0),virtualclass.videoHost.afterSessionJoin();virtualclass.gObj.precheckScrn=!1},showFinishBtn(){var c=document.querySelector(".congrea #dashboardContainer .modal-header button");c&&c.classList.add("enable");"DocumentShare"==virtualclass.currApp&&(c=document.querySelector("#virtualclassAppLeftPanel"),c.classList.add("showZoom"),c.classList.remove("hideZoom"))},removeFinishBtn(){var c=document.querySelector(".congrea.teacher  #dashboardContainer .modal-header button.enable");
c&&c.classList.remove("enable");"DocumentShare"==virtualclass.currApp&&(c=document.querySelector("#virtualclassAppLeftPanel"),c.classList.add("hideZoom"),c.classList.remove("showZoom"))},removeSharingClass(){const c=document.querySelector("#virtualclassCont");""!=c&&(c.classList.remove("studentScreenSharing"),document.querySelector("#chat_div").classList.remove("studentScreenSharing"))},addNoteClass(){if(roles.isStudent()){const c=document.querySelector("#docScreenContainer");null!=c&&(c.classList.add("noteDisplay"),
console.log("add note display"))}},calcBrightness(c){c=chroma(c).rgb();c=Math.round((299*parseInt(c[0])+587*parseInt(c[1])+114*parseInt(c[2]))/1E3);console.log(`brightne${c}`);return c},removeStudenScreenStatus(){const c=chatContainerEvent.elementFromShadowDom(".stdscreenImg","all");for(let a=0;a<c.length;a++)c[a].dataset.dcolor=""},insertAfter(c,a){a.parentNode.insertBefore(c,a.nextSibling)},triggerMouseEvent(c,a){if(null!=c){const b=document.createEvent("MouseEvents");b.initEvent(a,!0,!0);c.dispatchEvent(b)}},
getCookie(c){return(c=document.cookie.match(RegExp(`(?:^|;\\s*)${c.replace(/([.*+?\^${}()|\[\]\/\\])/g,"\\$1")}=([^;]*)`)))?c[1]:null},sendCurrAppOnUserJoin(){if("object"===typeof virtualclass.wb&&"Whiteboard"==virtualclass.currApp){const c=virtualclass.wb[virtualclass.gObj.currWb].vcan.main.replayObjs;0<c.length?ioAdapter.sendWithDelayAndDrop({repObj:c,cf:"repObj"},null,"mustSend","repObj",1500):console.log("Could not send the whiteboard data")}},setCurrApp(c,a){c.dataset.currapp=a;console.log(`Data set curr app ${a}`);
c=document.getElementById("chat_div");null!=c&&(c.dataset.currapp=a)},selfJoin(c){return c==virtualclass.gObj.uid?(ioAdapter.sendSpeed(virtualclass.videoHost.gObj.MYSPEED),!0):!1},getUrlVars(c){const a=[],b=c.slice(c.indexOf("?")+1).split("&");for(let f=0;f<b.length;f++)c=b[f].split("="),a.push(c[0]),a[c[0]]=c[1];return a},clearSyncTimeInterval(){virtualclass.videoUl.hasOwnProperty("syncTimeInterval")&&clearInterval(virtualclass.videoUl.syncTimeInterval)},pageVisible(){let c,a;const b={hidden:"visibilitychange",
webkitHidden:"webkitvisibilitychange",mozHidden:"mozvisibilitychange",msHidden:"msvisibilitychange"};for(c in b)if(c in document){a=b[c];break}return function(b){b&&document.addEventListener(a,b);return!document[c]}},showZoom(){const c=document.querySelector("#virtualclassAppLeftPanel.hideZoom");c&&(c.classList.remove("hideZoom"),c.classList.add("showZoom"))},removeBackgroundVideoApp(){virtualclass.videoUl.videoId="";var c=document.getElementById("dispVideo_Youtube_api");c&&c.contentWindow&&c.contentWindow.postMessage('{"event":"command","func":"pauseVideo","args":""}',
"*");if(c=document.querySelector(".congrea #dispVideo"))c.style.display="none",(c=document.querySelector(".congrea #dispVideo video"))&&c.setAttribute("src","");(c=document.querySelector("#listvideo .playing"))&&c.classList.remove("playing");(c=document.querySelector("#listvideo .removeCtr"))&&c.classList.remove("removeCtr");roles.hasControls()||(virtualclass.gObj.hasOwnProperty("videoPauseTime")&&clearTimeout(virtualclass.gObj.videoPauseTime),"object"===typeof virtualclass.videoUl.player&&null!=
virtualclass.videoUl.player.player_&&virtualclass.videoUl.player.paused()&&(console.log("==== Video is paused"),virtualclass.videoUl.player.pause()));"object"===typeof virtualclass.videoUl.player&&delete virtualclass.videoUl.player},removeSessionEndTool(){const c=document.getElementById("virtualclassSessionEndTool");null!=c&&c.parentNode.removeChild(c)},overrideRoleTeacher(){console.log("Member add:- End session override teacher");virtualclass.storage.config.endSession();localStorage.setItem("uRole",
"t");delete virtualclass.gObj.doEndSession},clearEverthing(){localStorage.removeItem("editorRich");localStorage.removeItem("editorCode");virtualclass.notPLayed=!0;virtualclass.storage.config.endSession();virtualclass.chat.chatroombox=!1;var c=document.getElementById("chatrm");null!==c&&c.parentNode.removeChild(c);c=document.getElementById("canvas");null!==c&&(c.style.pointerEvents="none")}}})(window);let globalImageData={},newCanvas;
...
!1;virtualclass.vutil.removeSharingClass();virtualclass.gObj.studentSSstatus.shareToAll=!1;virtualclass.gObj.studentSSstatus.sharing=!1;localStorage.setItem("studentSSstatus",JSON.stringify(virtualclass.gObj.studentSSstatus));const a=document.querySelector("#virtualclassScreenShare");null!=a&&a.parentNode.removeChild(a);virtualclass.vutil.removeSSsharing();delete virtualclass.ss;virtualclass.hasOwnProperty("studentScreen")&&delete virtualclass.studentScreen;virtualclass.zoom.removeZoomController();
virtualclass.ss=""},removeStream(){virtualclass.vutil.removeClass("audioWidget","fixed");this.localCont.clearRect(0,0,this.localCanvas.width,this.localCanvas.height);this.clearScreenShare()},initializeRecorder(a){c=1;this.prevStream&&(this.ssByClick=!1);"undefined"!==typeof virtualclass.prevScreen&&virtualclass.prevScreen.hasOwnProperty("currentStream")&&virtualclass.prevScreen.unShareScreen();this.video=document.getElementById(`${this.local}Video`);this.videoSmall=document.getElementById(`${this.local}Videosmall`);
...
!0),localStorage.removeItem("nEd")):console.log("Editor mode disable");console.log(`New role before clear ${virtualclass.gObj.uRole}`);localStorage.removeItem("settings");localStorage.removeItem("userSettings");virtualclass.settings.user={};c.config.createNewSession();virtualclass.videoHost&&roles.isStudent()&&!virtualclass.isPlayMode&&(virtualclass.settings.init(),virtualclass.settings.userAudioIcon(),virtualclass.settings.userVideoIcon(),a=document.querySelector("#virtualclassAppRightPanel"),a.classList.contains("vidHide")||
a.classList.add("vidHide"));console.log(`New role after clear ${virtualclass.gObj.uRole}`);virtualclass.enablePreCheck||virtualclass.popup.waitMsg();"object"===typeof virtualclass.dts&&null!=virtualclass.dts&&virtualclass.dts.destroyDts();if("object"===typeof virtualclass.raiseHand&&null!=virtualclass.raiseHand)if(roles.hasControls())virtualclass.raiseHand.rhCount=0,virtualclass.raiseHand.rhCountR=0,(a=document.querySelector(".congrea .vmchat_bar_button .hand_bt.highlight"))&&a.classList.remove("highlight"),
document.querySelector(".congrea .vmchat_bar_button .hand_bt #notifyText").innerHTML="";else if(a=document.querySelector(".congrea .handRaise.disable"))a.classList.remove("disable"),a.classList.add("enable"),a.setAttribute("data-title",virtualclass.lang.getString("RaiseHandStdEnabled")),document.querySelector(".congrea .handRaise #icHr").setAttribute("data-action","enable"),virtualclass.raiseHand.stdRhEnable="enabled";(a=chatContainerEvent.elementFromShadowDom(".vmchat_room_bt.ui-state-highlight"))&&
a.classList.remove("ui-state-highlight");(a=document.querySelector("#virtualclassCont.congrea.student"))&&a.classList.contains("videoff")&&a.classList.remove("videoff");a=document.querySelector("#virtualclassCont #memlist");b=document.querySelector("#virtualclassCont #chatrm");f=document.querySelector("#user_list");const g=document.querySelector("#chatroom_bt2");a&&!a.classList.contains("enable")&&(a.classList.add("enable"),a.classList.remove("disable"),b&&(b.classList.add("disable"),b.classList.remove("enable")));
null!=g&&(f.classList.contains("active")||f.classList.add("active"),g.classList.remove("active"));null!=virtualclass.serverData&&(virtualclass.serverData.rawData={video:[],ppt:[],docs:[]},roles.hasAdmin()&&virtualclass.serverData.fetchAllData());virtualclass.gObj.wIds=[0];virtualclass.wbCommon.order=[0];virtualclass.gObj.wbCount=0;virtualclass.wbCommon.clearNavigation();delete virtualclass.wb[virtualclass.gObj.currWb].activeToolColor}},dstdata:{async handleResult(b){for(;b;)b.value.hasOwnProperty("alldocs")&&
(console.log("document share store"),a=!0,virtualclass.gObj.docs=JSON.parse(b.value.alldocs)),b=await b.continue();a||(virtualclass.gObj.docs="init",console.log("==== Docs init "))}},dstAllStore(a){const b=this.db.transaction("dstall","readwrite");b.store.clear();const c=JSON.stringify(virtualclass.dts.allNotes);b.store.add({dstalldocs:JSON.stringify(a),allNotes:c,timeStamp:(new Date).getTime(),id:10});b.done.then(()=>{console.log("success")},()=>{console.log("failure")})},dstall:{async handleResult(a){for(;a;)a.value.hasOwnProperty("dstalldocs")&&
(console.log("document share store"),b=!0,virtualclass.gObj.dstAll=JSON.parse(a.value.dstalldocs),virtualclass.gObj.dstAllNotes=JSON.parse(a.value.allNotes)),a=await a.continue();b||(console.log("document share store init"),virtualclass.gObj.dstAll="init")}},clearSingleTable(a,b){this.db.transaction(a,"readwrite").store.clear();"undefined"!==typeof b&&b();"dstdata"==a&&(virtualclass.gObj.docs="init",console.log("==== Docs init "));"dstall"==a&&(virtualclass.gObj.dstall="init")},clearStorageData(){ioAdapter.adapterMustData=
...
null!=a&&a.parentNode.removeChild(a)},disable(a,c,g,d){g=virtualclass.gObj.testChatDiv.shadowRoot.querySelector(`.${c}Control${a} `+" .contImg");null!=g&&virtualclass.user.control._disable(g,c,a,d)},_disable(a,c,g,d){a.parentNode.setAttribute("data-title",virtualclass.lang.getString(`${c}Disable`));a.setAttribute(`data-${c}-disable`,"true");a.className=`icon-${c}Img block`+` ${c}Img`;"assign"==c?(a.parentNode.classList.remove("tooltip"),this.addCurrTeacherToControl(a.id),a=localStorage.getItem(`virtualclass${g}`),
a=JSON.parse(a),virtualclass.gObj.hasOwnProperty(`${g}currTeacher`)?(virtualclass.gObj[`${g}currTeacher`].ct||virtualclass.gObj.hasOwnProperty("controlAssign")&&virtualclass.gObj.controlAssign&&a.currTeacher)&&virtualclass.user.control.updateUser(g,"currTeacher",!0):virtualclass.gObj.hasOwnProperty("controlAssign")&&virtualclass.gObj.controlAssignId==g&&virtualclass.user.control.updateUser(g,"currTeacher",!0)):"audio"==c&&(a.className=`icon-${c}DisImg block`+` ${c}DisImg`);virtualclass.user.control.updateUser(g,
...
d.classList.contains("block")&&(d.parentNode.setAttribute("data-title",virtualclass.lang.getString("audioEnable")),d.setAttribute("data-audio-disable","false"))}f=c.querySelector("#virtualclassCont #memlist.enable");g=c.querySelector("#virtualclassCont #chatrm.enable");d=c.querySelector("#user_list");const m=c.querySelector("#chatroom_bt2");f&&!d.classList.contains("active")&&(d.classList.contains("active")||d.classList.add("active"),m.classList.remove("active"),virtualclass.chat.chatWindow="private");
g&&(m.classList.contains("active")||m.classList.add("active"),d.classList.remove("active"),virtualclass.chat.chatWindow="common");c.getElementsByClassName("ui-memblist-usr");for(f=0;f<a.length;f++)this.makeElemEnable(a[f])},resetmediaSetting(){if(roles.isStudent()){if(virtualclass.gObj.stdaudioEnable||virtualclass.user.control.audioDisable(),virtualclass.system.mediaDevices.hasWebcam)if(virtualclass.gObj.stdvideoEnable)virtualclass.vutil.videoHandler("off"),virtualclass.videoHost.toggleVideoMsg("enable");
else{virtualclass.videoHost.toggleVideoMsg("disable");var a=c.querySelector("#videoSwitch");null!=a&&a.classList.contains("on")&&(a.classList.remove("on"),a.classList.add("off"))}}else{(a=c.getElementById("contrAudioAllImg"))&&a.classList.contains("icon-all-audio-enable")&&(a.classList.remove("icon-all-audio-enable"),a.classList.add("icon-all-audio-disable"),a.setAttribute("data-title",virtualclass.lang.getString("muteAll")),"enable"==a.dataset.action&&(a.dataset.action="disable"));a=c.getElementById("videoSwitch");
if(virtualclass.system.mediaDevices.hasWebcam&&a.classList.contains("video")){const b=c.getElementById("rightCtlr");a.classList.contains("on")&&(virtualclass.vutil.videoHandler("off"),b.parentNode.setAttribute("data-title",virtualclass.lang.getString("videoon")))}this.mediaSliderSetting("audio");this.mediaSliderSetting("video")}roles.hasAdmin()&&(virtualclass.gObj.delayVid="display")},mediaSliderSetting(a){const b="audio"==a?"Audio":"Video",g={action:("audio"==a?virtualclass.gObj.stdaudioEnable:virtualclass.gObj.stdvideoEnable)?
"enable":"disable",enable:"disable",disable:"enable"};var d=c.querySelector(`.congrea #contr${b}All.${g.action}`);d&&(d.classList.remove(g.action),d.classList.add(g[g.action]),(d=c.querySelector(`.congrea #contr${b}All input`))&&d.removeAttribute("checked"));if(d=c.querySelector(`.congrea .slider.icon-all-${a}-${g.action}`))d.className=`slider round congtooltip icon-all-${a}-${g[g.action]}`,d.setAttribute("data-action",g[g.action]),d.setAttribute("data-title",virtualclass.lang.getString(`${g[g.action]}All${b}`))},
makeElemEnable(a){virtualclass.vutil.elemHasAnyClass(a.id)?(a.classList.remove("disable"),a.classList.add("enable")):a.className="enable";a=a.getElementsByClassName("ui-chatbox-input-box")[0];null!=a&&(a.disabled=!1)},toggleAllEditorController(a,c){a=virtualclass.vutil.smallizeFirstLetter(a);const b=chatContainerEvent.elementFromShadowDom(".controleCont","all");if(0<b.length)for(let d=0;d<b.length;d++)if(0<b[d].id.indexOf(a)){var f=b[d].id.indexOf("Cont");0<f&&(f=b[d].id.substr(0,f),f=chatContainerEvent.elementFromShadowDom(`${f}Img`,
null,!0),this.control.init.call(this,f,c,void 0,"actnotSend"))}"enable"==c?virtualclass.vutil.beforeSend({status:!0,control:"editorRich",cf:"control"}):virtualclass.vutil.beforeSend({status:!1,control:"editorRich",cf:"control"})},toggleDisplayEditorController(a,c){a=virtualclass.vutil.smallizeFirstLetter(a);if(virtualclass.gObj.hasOwnProperty("testChatDiv")){a=chatContainerEvent.elementFromShadowDom(`.controller${a}`,"all");for(let b=0;b<a.length;b++)a[b].style.display=c}},updateUser(a,c,d){const b=
localStorage.getItem(`virtualclass${a}`);let f={};null==b?f.id=a:f=JSON.parse(b);f[c]=d;localStorage[`virtualclass${f.id}`]=JSON.stringify(f);return f},audioSign2(a,f){if("create"==f){if(null==c.getElementById(`${a.id}AudEnableSign`)){f=c.createElement("div");f.id=`${a.id}AudEnableSign`;const b=c.createElement("img");imgName="audioenable";b.id=`${a.id+imgName}Img`;b.src=`${d.whiteboardPath}images/${imgName}.png`;const k=c.createElement("a");k.id=`${a.id+imgName}Anch`;k.className="audEnableSign tooltip controleCont";
k.setAttribute("data-title",virtualclass.lang.getString("studentAudEnable"));k.appendChild(b);f.appendChild(k);c.getElementById(`${a.id}ControlContainer`).appendChild(f)}}else a=c.getElementById(`${a.id}AudEnableSign`),a.parentNode.removeChild(a)},audioSign(a,c){"create"==c?this.iconAttrManupulate(a.id,"icon-audioEnaGreen"):a.aud?this.iconAttrManupulate(a.id,"icon-audioImg"):this.iconAttrManupulate(a.id,"icon-audioDisImg")},iconAttrManupulate(a,c){a=virtualclass.gObj.testChatDiv.shadowRoot.querySelector(`#ml${a} .audioImg`);
if(null!=a)for(let b=0;b<a.classList.length;b++)if("icon-"==a.classList[b].substring(0,5)){a.classList.remove(a.classList[b]);a.classList.add(c);break}},shouldApply(a){let b=localStorage.getItem(`virtualclass${a}`);null!=b&&(b=JSON.parse(b),b.ad&&virtualclass.user.control.audioSign({id:a},"create"))},changeAttrToAssign(a){const b=c.getElementById("chatWidget").getElementsByClassName("assignImg");for(let c=0;c<b.length;c++)"enable"==a?(b[c].classList.remove("block"),b[c].classList.add("enable"),b[c].parentNode.classList.add("tooltip"),
b[c].parentNode.setAttribute("data-title",virtualclass.lang.getString("transferControls")),b[c].setAttribute("data-assign-disable","false")):(b[c].classList.remove("enable"),b[c].classList.add("block"),b[c].parentNode.classList.remove("tooltip"),b[c].setAttribute("data-assign-disable","true"))}},displayStudentSpeaker(a){const b=c.getElementById("alwaysPress");null!=b&&(b.style.display=a?"block":"none")},toggleAllAudio(a){const b=chatContainerEvent.elementFromShadowDom(".controleCont","all");if(0<
b.length)for(let f=0;f<b.length;f++)if(0<b[f].id.indexOf("Aud")){var c=b[f].id.indexOf("Cont");0<c&&(c=b[f].id.substr(0,c),c=chatContainerEvent.elementFromShadowDom(`${c}Img`,null,!0),this.control.init.call(this,c,a,void 0,"actnotSend"))}},toggleAllVideo(a){ioAdapter.mustSend({action:a,cf:"toggleVideo"})},mediaSliderUI(a){var b="audio"==a?"Audio":"Video",d=c.querySelector(`.bulkUserActions #contr${b}AllImg`),k=virtualclass.user.defaultSettings(a,virtualclass.settings.info),m="enable"==k?"disable":
"enable";null!=k?(d.setAttribute("data-action",m),d.className=`slider round icon-all-${a}-${m} enable congtooltip cgIcon`,d.dataset.title=virtualclass.lang.getString(`${m}All${b}`),k=c.querySelector(`.bulkUserActions #contr${b}All input`),k.setAttribute("checked","true"),b=c.querySelector(`.congrea #contr${b}All`),b.classList.add(m)):(m={action:k?"enable":"disable",enable:"disable",disable:"enable"},d=c.querySelector(`.bulkUserActions #contr${b}AllImg`),d.setAttribute("data-action",m[m.action]),d.className=
`slider round icon-all-${a}-${m[m.action]} congtooltip cgIcon`,d.dataset.title=virtualclass.lang.getString(`${m[m.action]}All${b}`),k=c.querySelector(`.bulkUserActions #contr${b}All input`),k.removeAttribute("checked"),b=c.querySelector(`.congrea #contr${b}All`),b.classList.add(m[m.action]));if(virtualclass.isPlayMode)anchorTag.pointerEvents="none",anchorTag.style.cursor="default";else{const b=this;d.addEventListener("click",()=>{if("audio"==a){var c=b.toogleAudioIcon();virtualclass.settings.applySettings("enable"==
c,"studentaudio");"undefined"!==typeof c&&(localStorage.setItem("allAudAction",c),b.toggleAllAudio.call(virtualclass.user,c))}else c=b.toggleVideoIcon(),virtualclass.settings.applySettings("enable"==c,"studentvideo"),"undefined"!==typeof c&&(localStorage.setItem("allVideoAction",c),b.toggleAllVideo(c))})}},defaultSettings(a,c){let b,f;for(const d in c)if(f=c[d],"audio"===a&&"studentaudio"===d||"video"===a&&"studentvideo"===d)b=!0===f?"enable":"disable";return b},chatBoxesSwitch(){if(virtualclass.chat.chatroombox)toggleCommonChatBox();
else if(0==$("div#chat_room").length){var a=c.createElement("ul");a.id="chat_room";c.body.appendChild(a);virtualclass.chat.chatroombox=$("#chat_room").chatroom({id:"chat_room",user:{name:"test"},title:lang.chatroom_header,offset:"20px",messageSent(a,b){$("#chat_room").chatroom("option","boxManager").addMsg(a.name,b)}});virtualclass.gObj.hasOwnProperty("chatEnable")&&!virtualclass.gObj.chatEnable&&(a=c.getElementById("chatrm"),null!=a&&virtualclass.user.control.makeElemDisable(a));a=c.getElementById("cc_arrow_button");
...
a.className="slider round icon-all-audio-enable congtooltip",a.dataset.title=virtualclass.lang.getString("unmuteAll"),a=c.querySelector(".congrea .bulkUserActions #contrAudioAll"),a.classList.add("enable"),a.classList.remove("disable"));return b}},toggleVideoIcon(){var a=c.querySelector(".bulkUserActions #contrVideoAllImg");if(null!=a){let b=a.dataset.action;"enable"==a.dataset.action?(a.dataset.action="disable",a.className="slider round icon-all-video-disable congtooltip",a.dataset.title=virtualclass.lang.getString("disableAllVideo"),
a=c.querySelector(".congrea .bulkUserActions #contrVideoAll"),a.classList.add("disable"),a.classList.remove("enable")):(a.dataset.action="enable",a.className="slider round icon-all-video-enable congtooltip",a.dataset.title=virtualclass.lang.getString("enableAllVideo"),a=c.querySelector(".congrea .bulkUserActions #contrVideoAll"),a.classList.add("enable"),a.classList.remove("disable"));return b}},changeRoleOnFooter(a,c){chatContainerEvent.elementFromShadowDom(`#ml${a}`).dataset.role=c},initControlHandler(a){virtualclass.vutil.userIsOrginalTeacher(a);
const b=chatContainerEvent.elementFromShadowDom(`#ml${a} .contImg`,"all");let d=!1,k=localStorage.getItem(`virtualclass${a}`);null!=k&&(d=!0,k=JSON.parse(k),k.hasOwnProperty("currTeacher")&&(virtualclass.gObj[`${a}currTeacher`]={},1==k.currTeacher?(virtualclass.user.control.currTeacherAlready=!0,virtualclass.gObj[`${a}currTeacher`].ct=!0):virtualclass.gObj[`${a}currTeacher`].ct=!1));for(let f=0;f<b.length;f++)if(-1<b[f].className.indexOf("chat")){if(d&&k.hasOwnProperty("chat"))if(virtualclass.settings.user.hasOwnProperty(a)){var m=
virtualclass.settings.onLoadSettings(virtualclass.settings.user[a]);m=m.studentpc}else m=virtualclass.settings.info.studentpc;else m=virtualclass.settings.info.studentpc;virtualclass.user.control.changeAttribute(a,b[f],m,"chat","chat")}else-1<b[f].className.indexOf("aud")?(d&&k.hasOwnProperty("aud")?virtualclass.settings.user.hasOwnProperty(a)?(m=virtualclass.settings.onLoadSettings(virtualclass.settings.user[a]),m=m.studentaudio):m=virtualclass.settings.info.studentaudio:(m=c.querySelector("#contrAudioAll"),
m=null!=virtualclass.jId&&m.classList.contains("disable")?virtualclass.settings.info.studentaudio:virtualclass.gObj.stdaudioEnable),virtualclass.user.control.changeAttribute(a,b[f],m,"audio","aud"),roles.hasAdmin()&&"false"==b[f].dataset.audioDisable&&localStorage.getItem("allAudAction")):-1<b[f].className.indexOf("RaiseHand")?(m=d&&k.hasOwnProperty("raiseHand")?!!k.raiseHand:!1,virtualclass.user.control.changeAttribute(a,b[f],m,"RaiseHand","RaiseHand")):-1<b[f].className.indexOf("stdscreen")?virtualclass.gObj.studentSSstatus.hasOwnProperty("whoIsSharing")&&
...
b,c,f,g){const k=[];let m=a;const v=this;this.annotationList_.updateSpan(new n(a,b-a),(a,b)=>{const n={};for(var C in a.attributes)n[C]=a.attributes[C];n[r.LINE_SENTINEL]&&!g||c(n);C={};const B={};v.computeChangedAttributes_(a.attributes,n,C,B);a:{for(const b in C){a=!1;break a}a=!0}a||k.push({start:m,end:m+b,attributes:C,attributesInverse:B,origin:f});m+=b;return new d(n)});0<k.length&&this.trigger("attributesChange",this,k)};a.prototype.computeChangedAttributes_=function(a,b,c,f){const d={};let g;
...
b=localStorage.getItem("wbOrder");null!=b&&(b=JSON.parse(b),virtualclass.wbCommon.order=b);1<this.order.length&&(virtualclass.wbCommon.rearrangeNav(this.order),this.rearrange(this.order))}else virtualclass.wbCommon.indexNav.studentWBPagination(virtualclass.gObj.currIndex-1||virtualclass.gObj.currSlide);virtualclass.wbCommon.indexNav.setTotalPages(a.length)},displaySlide(a){this.hideElement();const b=document.querySelector(`#note${a}`);null!=b&&(b.classList.add("current"),virtualclass.gObj.currWb=
a);this.identifyFirstNote(a);this.identifyLastNote(a);roles.hasControls()||"undefined"!==typeof virtualclass.wbCommon.indexNav&&virtualclass.wbCommon.indexNav.studentWBPagination(virtualclass.gObj.currIndex-1||virtualclass.gObj.currSlide)},hideElement(){const a=document.querySelector("#virtualclassWhiteboard.whiteboard .current");null!=a&&(console.log("Whiteboard slide remove"),a.classList.remove("current"))},currentWhiteboard(a){null==document.querySelector(`#canvas${a}`)&&(this.hideElement(),virtualclass.vutil.createWhiteBoard(a),
this.displaySlide(a),virtualclass.gObj.currWb=a)},next:function(){this.hideElement();var a=this.whiteboardWrapperExist("next");if(null!==a){this.readyCurrentWhiteboard(a);this.setCurrSlideNumber(a);virtualclass.wbCommon.indexNav.addActiveNavigation(a);virtualclass.wbCommon.indexNav.UI.pageNavHandler("right");var b=a.slice(7);b=this.order.indexOf(b);virtualclass.vutil.beforeSend({cf:"cwb",diswb:!0,wid:a,currIndex:b});this.identifyLastNote(a);virtualclass.gObj.currWb=a}},newPage(){this.hideElement();
var a=virtualclass.gObj.currWb.slice(7);virtualclass.gObj.wbCount++;const b=this.whiteboardWrapperExist("next");virtualclass.gObj.hasOwnProperty("currSlide")&&-1==virtualclass.gObj.wIds.indexOf(Number(virtualclass.gObj.wbCount))&&(console.log("wids, From virtualclass "),virtualclass.gObj.wIds.push(virtualclass.gObj.wbCount));const c=`${"_doc_0_"}${virtualclass.gObj.wbCount}`,d=this.order.indexOf(a);this.whiteboardExist(c)||virtualclass.vutil.createWhiteBoard(c,d);this.setCurrSlideNumber(c);virtualclass.wbCommon.indexNav.UI.pageNavHandler("right");
this.displaySlide(c);null==b?(this.order.push(virtualclass.gObj.wbCount),virtualclass.wbCommon.indexNav.createWbNavigationNumber(virtualclass.gObj.wbCount,virtualclass.gObj.wbCount),a=this.order.length,virtualclass.wbCommon.identifyLastNote(virtualclass.gObj.currWb)):(a=this.order.indexOf(a),this.order.splice(a+1,0,virtualclass.gObj.wbCount),this.rearrange(this.order),this.indexNav.index=a+1,this.rearrangeNav(this.order));this.wbInd=a+1;virtualclass.vutil.beforeSend({cf:"cwb",wbCount:virtualclass.gObj.wbCount,
currIndex:a+1});virtualclass.gObj.currWb=c;this.indexNav.setTotalPages(virtualclass.gObj.wIds.length);virtualclass.wbCommon.indexNav.addActiveNavigation(virtualclass.gObj.currWb);this.identifyLastNote(c)},rearrange(a){const b=document.getElementsByClassName("whiteboardContainer")[0],c=document.createElement("div");c.className="whiteboardContainer";for(let b=0;b<a.length;b++){const d=document.getElementById(`note_doc_0_${a[b]}`);d&&c.appendChild(d)}b.parentNode.replaceChild(c,b)},rearrangeNav(a){document.querySelector("#dcPaging").innerHTML=
"";for(var b=0;b<=virtualclass.gObj.wbCount;b++)virtualclass.wbCommon.indexNav.createWbNavigationNumber(a[b],b);if("Whiteboard"==virtualclass.currApp)var c=virtualclass.gObj.currWb.split("doc_0_")[1];a=this.order.indexOf(c);(b=document.querySelector(".congrea #dcPaging .noteIndex.active"))&&b.classList.remove("active");if(b=document.querySelector(`#index${c}`))b.classList.add("active"),b.selected="selected";c=document.querySelector("#dcPaging .hid.right.active");const d=document.querySelector("#dcPaging .hid.left.active");
...
virtualclass.wbCommon.indexNav.studentWBPagination(virtualclass.gObj.currIndex-1||virtualclass.gObj.currSlide))},prev(){this.hideElement();var a=this.whiteboardWrapperExist("prev");this.readyCurrentWhiteboard(a);this.setCurrSlideNumber(a);virtualclass.wbCommon.indexNav.addActiveNavigation(a);virtualclass.wbCommon.indexNav.UI.pageNavHandler("left");a=document.querySelector(`#${virtualclass.wb[a].prvTool}`);null==a||a.classList.contains("active")||a.classList.add("active")},readyCurrentWhiteboard(a){null!=
...
this.shownPage(this.width);this.addActiveNavigation();this.setTotalPages(virtualclass.dts.order.length)};a.prototype.setTotalPages=function(a){var b=c.querySelector("#docShareNav #totalPages");b&&(roles.hasControls(),b.innerHTML=` of ${a}`,b=c.querySelector("#docShareNav"),a?(b.classList.add("show"),b.classList.remove("hide")):(b.classList.add("hide"),b.classList.remove("show")))};a.prototype.adjustPageNavigation=function(a,d){if("right"==d){d=c.querySelectorAll(".noteIndex.shw");if(d.length)var b=
d[d.length-1],f=parseInt(b.getAttribute("data-set"));for(;f<a;f++){"DocumentShare"==virtualclass.currApp&&virtualclass.dts.indexNav.UI.setClassPrevNext();var m=c.querySelector(".noteIndex.hid.right");m&&(m.classList.remove("hid","right"),m.classList.add("shw"),b=c.querySelector(".noteIndex.shw"))&&(b.classList.remove("shw"),b.classList.add("hid","left"))}"DocumentShare"==virtualclass.currApp&&(a=virtualclass.dts.docs.currNote,(a=c.querySelector(`.congrea #dcPaging #index${a}`))&&!a.classList.contains("active")&&
a.classList.add("active"))}else{for((d=c.querySelector(".noteIndex.shw"))&&(f=parseInt(d.getAttribute("data-set")));f>a;f--)"DocumentShare"==virtualclass.currApp&&virtualclass.dts.indexNav.UI.setClassPrevNext(),d=c.querySelectorAll(".noteIndex.hid.left"),d.length&&(m=d[d.length-1]),m&&(m.classList.remove("hid","left"),m.classList.add("shw"),d=c.querySelectorAll(".noteIndex.shw"),d.length&&(b=d[d.length-1]),b&&(b.classList.remove("shw"),b.classList.add("hid","right")));"DocumentShare"==virtualclass.currApp&&
(a=virtualclass.dts.docs.currNote,(a=c.querySelector(`.congrea #dcPaging #index${a}`))&&!a.classList.contains("active")&&(a.classList.add("active"),a.setAttribute("selected","selected")))}};a.prototype.removeNav=function(){const a=c.getElementById("docShareNav");for(;a.firstChild;)a.removeChild(a.firstChild)};a.prototype.addActiveNavigation=function(a){this.addActiveClass(a);a=c.querySelector("#rightNavPage.disable");this.UI.isNodeAvailable(".noteIndex.shw.active","next")?null!=a&&("Whiteboard"==
virtualclass.currApp?this.UI.setArrowStatus("rightNavPage","enable"):virtualclass.dts.indexNav.UI._setArrowStatusDocs(c.getElementById("rightNavPage"),"enable","disable")):"Whiteboard"==virtualclass.currApp?this.UI.setArrowStatus("rightNavPage","disable"):virtualclass.dts.indexNav.UI._setArrowStatusDocs(c.getElementById("rightNavPage"),"disable","enable");a=c.querySelector("#leftNavPage.disable");this.UI.isNodeAvailable(".noteIndex.shw.active","prev")?null!=a&&("Whiteboard"==virtualclass.currApp?
this.UI.setArrowStatus("leftNavPage","enable"):virtualclass.dts.indexNav.UI._setArrowStatusDocs(c.getElementById("leftNavPage"),"enable","disable")):"Whiteboard"==virtualclass.currApp?this.UI.setArrowStatus("leftNavPage","disable"):virtualclass.dts.indexNav.UI._setArrowStatusDocs(c.getElementById("leftNavPage"),"disable","enable")};a.prototype.addActiveClass=function(a){if("Whiteboard"==virtualclass.currApp){var b=a.split("doc_0_")[1];var d=virtualclass.gObj.wbCount+1}else d=virtualclass.dts.order.length;
(a=c.querySelector(".congrea #dcPaging .noteIndex.active"))&&a.classList.remove("active");b="DocumentShare"==virtualclass.currApp?virtualclass.dts.docs.currNote:b;if(a=c.querySelector(`#index${b}`)){a.classList.add("active");a.selected="selected";var k=c.querySelector("#totalPages");k&&k.parentNode.removeChild(k);const b=c.createElement("span");b.id="totalPages";k=c.querySelector("#pageAnc");k.appendChild(b);this.setTotalPages(d)}d=c.querySelector("#dcPaging .hid.right.active");k=c.querySelector("#dcPaging .hid.left.active");
if(d||k){if(d){var m=d.value;d="right"}else m=k.value,d="left";this.adjustPageNavigation(parseInt(m),d)}if("Whiteboard"==virtualclass.currApp){if(this.index=+b+1,m=c.querySelector(".noteIndex.active"))k=c.querySelector("#currIndex"),k.innerHTML=m.innerHTML,ioAdapter.mustSend({wbData:"wbIndex",wbIndex:m.innerHTML,cf:"wbData"})}else this.index=null!=m?m:null!=a&&"undefined"!==typeof a?a.getAttribute("data-set"):1,virtualclass.dts.order.length||(this.index=0),m=c.querySelector("#docShareNav"),virtualclass.dts.order.length?
...
d+1);this.index=d+1;b.onclick=virtualclass.dts.docs.goToNavs(a)};a.prototype.createWbNavigationNumber=function(a,d){var b="_doc_0_"+a,f=virtualclass.getTemplate("wbIndex","navigation")({app:virtualclass.currApp,id:a,order:d+1,wid:b});this.subCont.insertAdjacentHTML("beforeend",f);c.querySelector("#index"+a).className=a>this.shownPages?"noteIndex hid right":"noteIndex shw";virtualclass.gObj.currWb==b&&(virtualclass.wbCommon.indexNav.addActiveNavigation(b),c.querySelector("#currIndex").innerHTML=d+
1);f.onclick=function(){virtualclass.wbCommon.setCurrSlideNumber(b);virtualclass.wbCommon.indexNav.addActiveNavigation(b);virtualclass.wbCommon.readyCurrentWhiteboard(b);virtualclass.gObj.currWb=b}};a.prototype.newWbpage=function(a){virtualclass.vutil.navWhiteboard(virtualclass.wbCommon,virtualclass.wbCommon.newPage);virtualclass.wbCommon.hasOwnProperty("setNextWhiteboardTime")&&clearTimeout(virtualclass.wbCommon.setNextWhiteboardTime);"Whiteboard"==virtualclass.currApp&&(virtualclass.wbCommon.setNextWhiteboardTime=
...
c.getElementById("stdPageNo");if(b){b.innerHTML=parseInt(a)+1;const c=this;setTimeout(()=>{c.setTotalPages(virtualclass.gObj.wbCount+1)},100)}};a.prototype.UI={container(){for(var a=c.getElementById("docShareNav");a.firstChild;)a.removeChild(a.firstChild);var d=virtualclass.getTemplate("navMain","navigation")({app:virtualclass.currApp,control:roles.hasControls()});a.innerHTML=d;if(roles.hasControls()){a=c.querySelector(".congrea #docShareNav");c.querySelector("#currIndex").addEventListener("click",
function(){var a=c.querySelector("#dcPaging");a&&(a.classList.toggle("close"),a.classList.toggle("open"))});if("Whiteboard"==virtualclass.currApp){d=c.createElement("span");d.id="addNewPage";d.className="newPage";a.appendChild(d);var g=c.createElement("span");d.appendChild(g);g.className="icon-newPage congtooltip";g.setAttribute("data-title",virtualclass.lang.getString("newPage"));g.addEventListener("click",function(){virtualclass.wbCommon.indexNav.newWbpage(this.type)})}var k=c.querySelector("#dcPaging");
a.addEventListener("mouseover",function(){k.classList.add("open");k.classList.remove("close")});a.addEventListener("mouseout",function(){k.classList.add("close");k.classList.remove("open")});var m=this;c.querySelector("#leftNavPage").addEventListener("click",function(){"Whiteboard"==virtualclass.currApp?m.isNodeAvailable(".noteIndex.shw.active","prev")?c.querySelector("#virtualclassWhiteboard .prev").click():m.setArrowStatus("leftNavPage","disable"):(c.getElementById("docsprev").click(),m.setArrowStatus("leftNavPage",
"disable"))});c.querySelector("#rightNavPage").addEventListener("click",function(){"Whiteboard"==virtualclass.currApp?m.isNodeAvailable(".noteIndex.shw.active","next")?(c.querySelector("#virtualclassWhiteboard .next").click(),virtualclass.gObj.currWb.split("_doc_0")[1]>this.shownPages&&(c.querySelector(".noteIndex.shw").className="noteIndex hid left")):m.setArrowStatus("rightNavPage","disable"):(c.getElementById("docsnext").click(),m.setArrowStatus("rightNavPage","disable"))})}},setClassPrevNext(){const a=
c.querySelector(`#documentScreen #note${virtualclass.dts.docs.currNote}`);if(null!=a)var d=a.previousElementSibling,g=a.nextElementSibling;c.querySelector("#leftNavPage")&&(d?this.setArrowStatus("leftNavPage","enable"):this.setArrowStatus("leftNavPage","disable"));c.querySelector("#rightNavPage")&&(g?this.setArrowStatus("rightNavPage","enable"):this.setArrowStatus("rightNavPage","disable"))},pageNavHandler(a,d){if("right"==a){if(a=c.querySelector(".noteIndex.hid.right.active")){a.classList.remove("hid");
a.classList.remove("right");a.classList.add("shw");var b=c.querySelector(".noteIndex.shw");b&&(b.classList.remove("shw"),b.classList.add("hid","left"))}}else if(a=c.querySelector(".noteIndex.hid.left.active"))a.classList.remove("hid"),a.classList.remove("left"),a.classList.add("shw"),a=c.querySelectorAll(".noteIndex.shw"),a.length&&(b=a[a.length-1]),b&&(b.classList.remove("shw"),b.classList.add("hid"),b.classList.add("right"))},setArrowStatus(a,d){const b="disable"==d?"enable":"disable";a=c.getElementById(a);
...
CommonXHR.prototype.send=function(d,c,a){console.log(`Common URL ${d}`);this.cb=c;this.httpObj.open("GET",d,!0);this.httpObj.responseType=a;this.httpObj.setRequestHeader("x-api-key",wbUser.lkey);this.httpObj.setRequestHeader("x-congrea-authuser",wbUser.auth_user);this.httpObj.setRequestHeader("x-congrea-authpass",wbUser.auth_pass);this.httpObj.setRequestHeader("x-congrea-room",wbUser.room);this.httpObj.withCredentials=!0;this.httpObj.send()};(function(d){d.settings={info:{allowoverride:null,studentaudio:null,studentpc:null,studentgc:null,studentvideo:null,raisehand:null,userlist:null,x8:null,x16:null,enableRecording:null,recAllowpresentorAVcontrol:null,recShowPresentorRecordingStatus:null,attendeeAV:null,recallowattendeeAVcontrol:null,showAttendeeRecordingStatus:null,trimRecordings:null},user:{},temp:"",init(){var c=localStorage.getItem("settings");c||(c=virtualclassSetting.settings);virtualclass.settings.temp=c;c=virtualclass.settings.onLoadSettings(c);
for(const a in c)virtualclass.settings.info.hasOwnProperty(a)&&(virtualclass.settings.info[a]=c[a]);if(c=JSON.parse(localStorage.getItem("userSettings")))virtualclass.settings.user=c;this.recording.init()},settingsToHex(c){const a=[];a[0]=+c.allowoverride;a[1]=+c.studentaudio;a[2]=+c.studentvideo;a[3]=+c.studentpc;a[4]=+c.studentgc;a[5]=+c.raisehand;a[6]=+c.userlist;a[7]=+c.enableRecording;a[8]=+c.recAllowpresentorAVcontrol;a[9]=+c.recShowPresentorRecordingStatus;a[10]=+c.attendeeAV;a[11]=+c.recallowattendeeAVcontrol;
a[12]=+c.showAttendeeRecordingStatus;a[13]=+c.trimRecordings;a[14]=+c.x8;a[15]=+c.x16;return virtualclass.settings.binaryToHex(a.join(""))},parseSettings(c){const a={};(c=virtualclass.settings.hexToBinary(c))&&(c=c.split(""));a.allowoverride=!!+c[0];a.studentaudio=!!+c[1];a.studentvideo=!!+c[2];a.studentpc=!!+c[3];a.studentgc=!!+c[4];a.raisehand=!!+c[5];a.userlist=!!+c[6];a.enableRecording=!!+c[7];a.recAllowpresentorAVcontrol=!!+c[8];a.recShowPresentorRecordingStatus=!!+c[9];a.attendeeAV=!!+c[10];
a.recallowattendeeAVcontrol=!!+c[11];a.showAttendeeRecordingStatus=!!+c[12];a.trimRecordings=!!+c[13];a.x8=!!+c[14];a.x16=!!+c[15];return a},hexToBinary(c){let a,b="";const d={0:"0000",1:"0001",2:"0010",3:"0011",4:"0100",5:"0101",6:"0110",7:"0111",8:"1000",9:"1001",a:"1010",b:"1011",c:"1100",d:"1101",e:"1110",f:"1111",A:"1010",B:"1011",C:"1100",D:"1101",E:"1110",F:"1111"};for(a=0;a<c.length;a+=1)if(d.hasOwnProperty(c[a]))b+=d[c[a]];else return!1;return b},binaryToHex(c){let a,b,d,g,k="";for(a=c.length-
...
virtualclass.connectedUsers;for(var g=0;g<d.length;g++)"s"===d[g].role&&(virtualclass.user.control.changeAttribute(d[g].userid,virtualclass.gObj.testChatDiv.shadowRoot.getElementById(d[g].userid+"contrAudImg"),virtualclass.settings.info.studentaudio,"audio","aud"),virtualclass.user.control.changeAttribute(d[g].userid,virtualclass.gObj.testChatDiv.shadowRoot.getElementById(d[g].userid+"contrChatImg"),!0,"chat","chat"));localStorage.removeItem("userSettings");virtualclass.settings.user={};virtualclass.settings.info[b]=
c;b=virtualclass.settings.settingsToHex(virtualclass.settings.info);virtualclass.settings.send(b,a);localStorage.setItem("settings",b)}else{if(virtualclass.settings.user.hasOwnProperty(a))d=virtualclass.settings.onLoadSettings(virtualclass.settings.user[a]),d[b]=c,b=virtualclass.settings.settingsToHex(d);else{g={};const a=virtualclass.settings.info;for(d in a)g[d]=a[d];g[b]=c;b=virtualclass.settings.settingsToHex(g)}virtualclass.settings.send(b,a);virtualclass.settings.user[a]=b;localStorage.setItem("userSettings",
...
this._applySettings(c)):this.recording.triggerSetting(c)},onLoadSettings(c){return virtualclass.settings.parseSettings(c)},studentaudio(c){roles.isStudent()&&(!0===c?(virtualclass.user.control.audioWidgetEnable(!0),virtualclass.gObj.audioEnable=!0):!1===c&&(c=document.querySelector("#speakerPressOnce"),!0!==c.dataset.audioPlaying&&"true"!==c.dataset.audioPlaying||virtualclass.media.audio.clickOnceSpeaker("speakerPressOnce"),virtualclass.user.control.audioDisable(),virtualclass.gObj.audioEnable=!1))},
allowoverride(){console.log("TO DO")},studentpc(c){console.log("TO DO");!0===c?(virtualclass.user.control.allChatEnable(),virtualclass.gObj.chatEnable=!0,document.querySelector("#chatWidget").classList.remove("chat_disabled"),document.querySelector("#chat_div").classList.remove("chat_disabled")):!1===c&&(virtualclass.user.control.allChatDisable(),virtualclass.gObj.chatEnable=!1)},studentgc(){console.log("TO DO")},studentvideo(c){if(roles.isStudent()){const a=document.querySelector(".videoSwitchCont #videoSwitch");
a.classList.contains("off")&&!1===c?console.log("do nothing"):a.classList.contains("on")&&!0===c?console.log("do nothing"):a.classList.contains("on")&&!1===c&&virtualclass.vutil.videoHandler("off");virtualclass.videoHost.toggleVideoMsg(!1===c?"disable":"enable")}},raisehand(){console.log("TO DO")},userlist(){console.log("TO DO")},x8(){console.log("TO DO")},enableRecording(){console.log("TO DO")},recAllowpresentorAVcontrol(){console.log("TO DO")},recShowPresentorRecordingStatus(){console.log("TO DO")},
attendeeAV(){console.log("TO DO")},recallowattendeeAVcontrol(){console.log("TO DO")},showAttendeeRecordingStatus(){console.log("TO DO")},x16(){console.log("TO DO")},recording:{enableRecording:!1,allowpresentorAVcontrol:!1,attendeeAV:!1,allowattendeeAVcontrol:!1,audioVideo:!0,statusOnly:!1,trimRecordings:!1,allowattendeeAVcontrolByTeacher:!1,init(){this.enableRecording=virtualclass.settings.info.enableRecording;roles.hasControls()?(this.allowpresentorAVcontrol=virtualclass.settings.info.recAllowpresentorAVcontrol,
this.showPresentorRecordingStatus=virtualclass.settings.info.allowpresentorAVcontrol?!0:virtualclass.settings.info.recShowPresentorRecordingStatus):(this.attendeeAV=virtualclass.settings.info.attendeeAV,this.allowattendeeAVcontrol=virtualclass.settings.info.recallowattendeeAVcontrol,this.showAttendeeRecordingStatus=virtualclass.settings.info.allowattendeeAVcontrol?!0:virtualclass.settings.info.showAttendeeRecordingStatus);this.trimRecordings=virtualclass.settings.info.trimRecordings;this.showStatusButton=
this.showStatus();this.showButton();this.overrideSettingFromLocalStorage()},overrideSettingFromLocalStorage(){let c=localStorage.getItem("recsetting");if(null!==c&&void 0!==c)if(c=JSON.parse(c),roles.hasControls()||!0!==c.statusonly)this.updateSettingAV(!1);else virtualclass.settings.onMessage({ac:!1})},showButton(){const c=document.getElementById("recording");this.showStatusButton?(c.classList.remove("hide"),c.classList.add("show"),virtualclassCont.dataset.recording="on",this.statusOnly?c.classList.add("statusonly"):
this.attachHandler(c),this.trimRecordings&&c.classList.add("trimRecording")):(c.classList.remove("show"),c.classList.add("hide"))},attachHandler(c){c.addEventListener("click",this.recordingButtonAction.bind(this,c))},updateSettingAV(c){const a=document.querySelector("#recording"),b=document.querySelector("#recording .recordingText");(this.audioVideo=c)?(b.innerHTML="Recording",virtualclassCont.dataset.recording="on",a.setAttribute("data-title","Recording Started"),localStorage.removeItem("recsetting"),
"ScreenShare"===virtualclass.currApp&&null!=virtualclass.ss&&virtualclass.ss.initShareScreen("ss",500)):(roles.hasControls(),a.setAttribute("data-title","Recording Stopped"),virtualclassCont.dataset.recording="off",localStorage.setItem("recsetting",JSON.stringify({statusonly:this.statusOnly,rec:virtualclassCont.dataset.recording})));ioAdapter.setRecording()},recordingButtonAction(c){c.classList.contains("statusonly")?console.log(" Do nothing"):(c="off"===virtualclassCont.dataset.recording?!0:!1,this.updateSettingAV(c),
roles.hasControls()&&ioAdapter.mustSend({ac:c,cf:"recs"}))},showStatus(){return roles.hasControls()?this.enableRecording&&this.allowpresentorAVcontrol&&this.showPresentorRecordingStatus?!0:this.enableRecording&&!this.allowpresentorAVcontrol&&this.showPresentorRecordingStatus?this.statusOnly=!0:!1:this.enableRecording&&this.attendeeAV&&this.allowattendeeAVcontrol&&this.showAttendeeRecordingStatus?!0:this.enableRecording&&this.attendeeAV&&!this.allowpresentorAVcontrol&&this.showAttendeeRecordingStatus?
this.statusOnly=!0:!1},isRecordingOn(){return roles.hasControls()?this.enableRecording&&this.allowpresentorAVcontrol&&this.audioVideo:this.attendeeAV?this.allowattendeeAVcontrol?this.audioVideo:!0:!1},sendYesOrNo(){return this.isRecordingOn()?"yes":"no"},setStatusOnElement(){const c=document.querySelector("#recording");this.allowattendeeAVcontrolByTeacher?(c.classList.add("statusonly"),this.statusOnly=!0):(c.classList.remove("statusonly"),this.statusOnly=!1)},triggerSetting(c){this.allowattendeeAVcontrolByTeacher=
0==c.ac;this.setStatusOnElement();this.updateSettingAV(c.ac)}},userAudioIcon(){!1===virtualclass.settings.info.studentaudio?(virtualclass.gObj.audioEnable=!1,virtualclass.user.control.audioDisable(!0)):!0===virtualclass.settings.info.studentaudio?(virtualclass.gObj.audioEnable=!0,virtualclass.user.control.audioWidgetEnable(!0)):!0!==virtualclass.settings.info.studentaudio&&virtualclass.user.control.audioDisable()},userVideoIcon(){!1===virtualclass.settings.info.studentvideo&&roles.isStudent()?virtualclass.user.control.videoDisable():
(roles.isStudent()&&!0!==virtualclass.settings.info.studentvideo?(virtualclass.vutil.videoHandler("off"),virtualclass.videoHost.toggleVideoMsg("disable")):(virtualclass.user.control.videoEnable(),roles.isStudent()&&virtualclass.vutil.videoHandler("off")),!0===virtualclass.settings.info.studentvideo&&virtualclass.user.control.videoEnable())}}})(window);(d=>{function c(a){const b=new Promise((b,c)=>{const d=()=>{a.removeEventListener("success",g);a.removeEventListener("error",k)},g=()=>{b(f(a.result));d()},k=()=>{c(a.error);d()};a.addEventListener("success",g);a.addEventListener("error",k)});b.then(b=>{b instanceof IDBCursor&&q.set(b,a)});u.set(b,a);return b}function a(a){if(!r.has(a)){var b=new Promise((b,c)=>{const d=()=>{a.removeEventListener("complete",f);a.removeEventListener("error",g);a.removeEventListener("abort",g)},f=()=>{b();d()},g=()=>
...
function joinAsTeacher(d){virtualclass.gObj.veryFirstJoin=!1;virtualclass.vutil.isOrginalTeacherExist(d)||(overrideOperation("t"),console.log("Member add :- join as teacher"));virtualclass.gObj.hasOwnProperty("doEndSession")&&roles.isTeacher()&&virtualclass.vutil.overrideRoleTeacher()}var overrideOperation=function(d){"s"===d&&(virtualclass.view.disappearBox("drawArea"),removeAppsDom(),"object"===typeof virtualclass.wb&&virtualclass.wb[virtualclass.gObj.currWb].utility.makeCanvasDisable());virtualclass.vutil.overrideRoles(d)};
function initOverrideRoleTeacher(d){virtualclass.gObj.hasOwnProperty("doEndSession")&&selfJoin(d)&&"t"==virtualclass.joinUser.role&&virtualclass.vutil.overrideRoleTeacher()}function getPosition(d,c){return d.findIndex(a=>a.userid==c)}
...
virtualclass.joinUser=this.getJoinUser(virtualclass.connectedUsers,virtualclass.jId)}else console.log("User packet is not receving");virtualclass.jId==virtualclass.gObj.uid&&(virtualclass.videoHost.setDefaultValue("undefined"===typeof virtualclass.videoHost.gObj.MYSPEED?1:virtualclass.videoHost.gObj.MYSPEED),virtualclass.vutil.setDefaultScroll());virtualclass.gObj.mySetTime=2E3;console.log("==== Add users");defaultOperation(d,void 0);roles.hasControls()&&virtualclass.poll.updateUsersOnPoll();if("undefined"!==
typeof virtualclass.quiz&&0>virtualclass.quiz.uniqueUsers.indexOf(virtualclass.jId))for(c in virtualclass.quiz.uniqueUsers.push(virtualclass.jId),io.uniquesids)0>virtualclass.quiz.uniqueUsers.indexOf(c)&&virtualclass.quiz.uniqueUsers.push(c);"s"==virtualclass.joinUser.role&&virtualclass.gObj.has_ts_capability&&ioAdapter.mustSend({uid:virtualclass.gObj.uid,ac:!0,cf:"tsr"});if(virtualclass.gObj.uid!=virtualclass.jId&&virtualclass.gObj.meetingMode)virtualclass.multiVideo.onUserJoin(virtualclass.jId);
...
fromUser:c,message:"offline"},virtualclass.ioEventApi.user_logout(d)},user_logout(d){console.log("user_logout");if(!roles.hasControls()&&(virtualclass.gObj.hasOwnProperty("whoIsTeacher")||(virtualclass.gObj.whoIsTeacher=virtualclass.vutil.whoIsTeacher()),virtualclass.gObj.whoIsTeacher==d.fromUser)){var c=document.querySelector("#virtualclassCont.congrea");c&&c.classList.contains("tr_available")&&c.classList.remove("tr_available")}virtualclass.media.video.removeUser(d.fromUser);virtualclass.chat.isTechSupportExist(d.fromUser)&&
virtualclass.chat.disableTechSupport(d.fromUser);c=d.fromUser;delete virtualclass.gObj.allUserObj[c];d=getPosition(virtualclass.connectedUsers,c);-1!==d&&virtualclass.connectedUsers.splice(d,1);d={removeUser:c};memberUpdateWithDelay(d,"removed");if(virtualclass.gObj.meetingMode)virtualclass.multiVideo.onUserRemove(c);virtualclass.connectedUsers.length<virtualclass.gObj.userToBeDisplay&&(virtualclass.gObj.insertUser=!0)},error(d){virtualclass.gObj.displayError?(virtualclass.view.removeElement("serverErrorCont"),
...
virtualclass.recorder.startUpload)console.log("Popup box Could not close");else{virtualclass.popup.closePopup();virtualclass.vutil.setDefaultScroll();const c=document.getElementById("popupContainer");null!=c&&(c.style.display="none");console.log("Popup box Close All")}},d)},binrec(d){const c=new Uint8Array(d.message);switch(c[0]){case 102:case 103:case 104:case 202:case 203:case 204:if(roles.hasControls()){virtualclass.gObj.studentSSstatus.mesharing=!0;const a=document.querySelector("#virtualclassCont");
""!=a&&(a.classList.add("studentScreenSharing"),document.querySelector("#chat_div").classList.add("studentScreenSharing"))}virtualclass.hasOwnProperty("studentScreen")||(virtualclass.studentScreen=new studentScreen);virtualclass.gObj.precheckScrn&&virtualclass.vutil.prechkScrnShare();roles.hasControls()&&!virtualclass.gObj.studentSSstatus.mesharing||virtualclass.studentScreen.ssProcess(c,d.message,"ss","ScreenShare");virtualclass.gObj.studentSSstatus.sharing&&roles.isStudent()&&(d=document.getElementById("virtualclassScreenShareLocal"),
...
this.createArrow=function(d){"object"===typeof virtualclass.wb&&(roles.hasControls()||virtualclass.wb[virtualclass.gObj.currWb].response.createArrow(d.message))};this.repObj=function(d){"object"!==typeof virtualclass.wb&&"DocumentShare"!=virtualclass.currApp&&virtualclass.makeAppReady(virtualclass.apps.wb);"undefined"!==typeof virtualclass.gObj.currWb&&(virtualclass.wb[virtualclass.gObj.currWb].utility.removeWhiteboardMessage(),"t"==d.fromUser.role?virtualclass.wb[virtualclass.gObj.currWb].utility.replayObjsByFilter(d.message.repObj):
console.log("whiteboard -------------------------- We just lost a packet"))};this.replayAll=function(d){virtualclass.wb[virtualclass.gObj.currWb].response.replayAll()};this.dts=function(d){console.log(`Document share :- message received ${d.message.serial}`);console.dir(d.message.dts);if(d.message.dts.hasOwnProperty("init"))virtualclass.gObj.screenRh=100,virtualclass.makeAppReady("DocumentShare",void 0,d.message.dts),virtualclass.gObj.screenRh=60;else if("object"===typeof virtualclass.dts)virtualclass.dts.onmessage(d)};
this.poll=function(d){console.log(d.message.poll.pollMsg);"init"==d.message.poll.pollMsg?virtualclass.makeAppReady("Poll"):("object"!==typeof virtualclass.poll&&(virtualclass.makeAppReady("Poll"),virtualclass.poll.onmessage({pollMsg:d.message.poll.init,cf:"poll",user:"all"})),virtualclass.poll.onmessage(d.message,d.fromUser))};this.quiz=function(d){console.log(d.message.quiz.quizMsg);"init"==d.message.quiz.quizMsg?virtualclass.makeAppReady("Quiz"):("object"!==typeof virtualclass.quiz&&(virtualclass.makeAppReady("Quiz"),
virtualclass.quiz.onmessage({quizMsg:d.message.quiz.init,cf:"quiz",user:"all"})),virtualclass.quiz.onmessage(d.message,d.fromUser))};this.videoUl=function(d){if(d.fromUser.userid!=virtualclass.gObj.uid)virtualclass.videoUl.onmessage(d.message)};this.congController=function(d){if(d.fromUser.userid!=virtualclass.gObj.uid)virtualclass.videoHost.onmessage(d.message);"off"==d.message.congCtr.videoSwitch?(virtualclass.vutil.addClass("virtualclassCont","videoff"),virtualclass.vutil.addClass("virtualclassAppRightPanel",
"hide"),virtualclass.vutil.removeClass("virtualclassAppRightPanel","show"),virtualclass.videoHost.setUserIcon(d.fromUser.userid)):(document.querySelector("#virtualclassCont").classList.remove("videoff"),virtualclass.vutil.addClass("virtualclassAppRightPanel","show"),virtualclass.vutil.removeClass("virtualclassAppRightPanel","hide"))};this.tsr=function(d){};this.na=function(d){virtualclass.user.control.iconAttrManupulate(d.fromUser.userid,"icon-audioEnaOrange")};this.ya=function(d){virtualclass.user.control.iconAttrManupulate(d.fromUser.userid,
"icon-audioEnaGreen")};this.mem_add=function(d){virtualclass.ioEventApi.member_added({type:"member_added",newJoinUser:d.message.user,cmadd:!0})};this.mvid=function(d){console.log("multivideo, message received");virtualclass.multiVideo.onmessage(d.message,d.fromUser.userid)};this.sc=function(d){console.log("Recevied scroll");virtualclass.pdfRender[virtualclass.gObj.currWb].setScrollPosition(d.message)};this.scf=function(d){console.log("Recevied scroll first");console.dir(d.message);null!=virtualclass.gObj.currWb&&
virtualclass.pdfRender[virtualclass.gObj.currWb].setScrollPosition(d.message)};this.cwb=function(d){if(d.message.hasOwnProperty("diswb")){({wid:d}=d.message);virtualclass.gObj.currWb=d;virtualclass.wbCommon.whiteboardExist(virtualclass.gObj.currWb)||virtualclass.vutil.createWhiteBoard(virtualclass.gObj.currWb);const c=d.split("_");0<c.length&&(virtualclass.gObj.currSlide=c[c.length-1]);virtualclass.wbCommon.displaySlide(d);console.log(`whiteboard slide received=${d}`);virtualclass.vutil.resizeWindowIfBigger()}else d.message.hasOwnProperty("wbCount")&&
(virtualclass.gObj.wbCount=d.message.wbCount,-1==virtualclass.gObj.wIds.indexOf(Number(virtualclass.gObj.wbCount))&&virtualclass.gObj.wIds.push(virtualclass.gObj.wbCount))};this.reqscreen=function(d){"mobTab"==virtualclass.system.device||"Chrome"!=virtualclass.system.mybrowser.name&&"Firefox"!=virtualclass.system.mybrowser.name&&"Edge"!=virtualclass.system.mybrowser.name?virtualclass.vutil.beforeSend({ext:!0,cf:"colorIndicator",nosupport:!0}):(d=virtualclass.lang.getString("stdscreenshare"),virtualclass.gObj.precheckScrn&&
virtualclass.vutil.prechkScrnShare(),virtualclass.popup.confirmInput(d,c=>{c?(roles.isStudent()&&(virtualclass.gObj.studentSSstatus.mesharing=!0),virtualclass.makeAppReady("ScreenShare","byclick")):virtualclass.vutil.beforeSend({sd:!0,cf:"colorIndicator"})}))};this.sshare_user=d=>{virtualclass.vutil.removeSSsharing();virtualclass.gObj.whoIsSharing=d.fromUser.userid;virtualclass.vutil.initssSharing(d.fromUser.userid)};this.sview=function(d){const c=document.getElementById("virtualclassScreenShareLocal");
...
delete virtualclass.videoUl.player}};this.sync=function(){};this.wbData=function(d){console.log(d);d.message.wbIndex&&(virtualclass.gObj.currIndex=parseInt(d.message.wbIndex),virtualclass.wbCommon.indexNav.studentWBPagination(d.message.wbIndex-1))};this.recs=function(d){if(!virtualclass.isPlayMode)virtualclass.settings.onMessage(d.message)}};(function(d){function c(){}c.prototype.loadData=function(a){a.earlierWidth=a.innerWidth;a.earlierHeight=a.innerHeight;a.wbUser=wbUser;a.pageEnter=(new Date).getTime();const b=new a.virtualclass;b.config.data={};b.virtualclassIDBOpen=a.virtualclassOpenDB;a.virtualclass=b;b.gObj.displayError=1;b.lang={};b.lang.getString=a.getString;b.lang.message=a.message;b.createMainContainer();b.gObj.sessionClear=!1;b.settings=a.settings;b.settings.init();b.handleCurrentUserWithPrevious();b.ioEventApi=ioEventApi;
b.gObj.mobileVchOffset=vhCheck();localStorage.setItem("settings",b.settings.temp);a=localStorage.getItem("wIds");null!=a&&(a=JSON.parse(a),b.gObj.wids=a,b.gObj.wbCount=a.length-1);var c=localStorage.getItem("anyp");null==c?localStorage.setItem("anyp",wbUser.anyonepresenter):c!=wbUser.anyonepresenter&&(localStorage.removeItem("uRole"),localStorage.setItem("anyp",wbUser.anyonepresenter));wbUser.virtualclassPlay=parseInt(wbUser.virtualclassPlay,10);wbUser.virtualclassPlay&&(b.gObj.sessionClear=!0,b.gObj.role=
"s",localStorage.removeItem("teacherId"),wbUser.id=99955551230,b.gObj.uid=wbUser.id);const d=JSON.parse(localStorage.getItem("prevApp"));b.gObj.prevApp=d;if(null!=d)if(b.previousApp=d,c=d.name,c=c.charAt(0).toUpperCase()+c.slice(1),"Yts"==d.name||"DocumentShare"==d.name)if(null==d.metaData)var k=null;else k=d.metaData,k.fromReload=!0;else"Video"==d.name?null==d.metaData||null==d.metaData.init?k=null:(k=d.metaData,k.fromReload=!0):"Whiteboard"==d.name&&(null==a&&(b.gObj.wbCount=d.wbn),a=localStorage.getItem("currSlide"),
null!=a&&(b.gObj.currSlide=a));else c=b.gObj.defaultApp;"undefined"===typeof k&&(k=null);b.precheck=localStorage.getItem("precheck");a=localStorage.getItem("precheck");null!=a&&(b.isPrecheck=JSON.parse(a));wbUser.virtualclassPlay?(b.isPrecheck=!1,b.enablePreCheck=!1):(b.precheck=localStorage.getItem("precheck"),b.enablePreCheck=!0,a=localStorage.getItem("precheck"),null!=a&&(b.isPrecheck=JSON.parse(a)));b.popup=new PopUp({showOverlay:!0});!b.enablePreCheck||null!=b.isPrecheck&&b.isPrecheck?(b.makePreCheckAvailable=
!1,b.popup.waitMsg("refresh")):b.makePreCheckAvailable=!0;null!=localStorage.getItem("beTeacher")&&(b.gObj.doEndSession=!0,b.gObj.beTeacher=!0,wbUser.role="t",b.gObj.uRole="t",localStorage.setItem("uRole","t"),localStorage.removeItem("beTeacher"),console.log("From locastorage:- beteacher found"));b.gObj.fromPageRefresh=!0;b.config.data.appIs=c;b.config.data.videoObj=k};c.prototype.appInit=async function(){virtualclass.isPlayMode||"thisismyplaymode"!==localStorage.getItem("mySession")?await virtualclass.init(wbUser.role,
virtualclass.config.data.appIs,virtualclass.config.data.videoObj):(localStorage.clear(),virtualclass.init(wbUser.role,virtualclass.config.data.appIs,virtualclass.config.data.videoObj),await virtualclass.storage.config.endSession());if("Edge"===virtualclass.system.mybrowser.name){const a=document.getElementById("virtualclassCont");null!=a&&a.classList.add("edge")}wbUser.virtualclassPlay||(virtualclass.editorRich.veryInit(),virtualclass.editorCode.veryInit())};c.prototype.setUpData=function(a){virtualclass.vutil.isMiniFileIncluded("wb.min")&&
(virtualclass.gObj.displayError=0);2<a.virtualclass.error.length?a.virtualclass.error=[]:(virtualclass.vutil.initDefaultInfo(wbUser.role,virtualclass.config.data.appIs),roles.isStudent()&&(a=localStorage.getItem("audEnable"),null!==a&&"false"===a.ac&&(virtualclass.user.control.mediaWidgetDisable(),virtualclass.gObj.audioEnable=!1)),virtualclass.gObj.memberlistpending=[],virtualclass.gObj.veryFirstJoin=!0,virtualclass.gObj.testChatDiv=document.querySelector("#chat_div"),virtualclass.gObj.testChatDiv.attachShadow({mode:"open"}))};
c.prototype.readyToGo=function(){virtualclass.vutil.attachClickOutSideCanvas();virtualclass.vutil.isPlayMode()&&virtualclass.vutil.clearEverthing();null!=virtualclass.isPrecheck&&("undefined"===typeof virtualclass.videoHost.gObj.MYSPEED&&(virtualclass.videoHost.gObj.MYSPEED=1),virtualclass.videoHost.afterSessionJoin())};d.Bootstrap=c})(window);$.uiBackCompat=!1;(function(d){d.onload=async function(){const c=new d.Bootstrap;await c.loadData(d);await c.appInit();await c.setUpData(d);await c.readyToGo()}})(window);
