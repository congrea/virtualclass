...
recObjs:m,type:k}),"undefined"!==typeof b[a+1]&&(m=this.calculateNextTime(f,b[a+1]),c=this.insertPacketInto(c,m,!0),1E3<=m&&(g=1E3*Math.trunc(m/1E3))),this.refrenceTime=f)}console.log(`totalTime in seconds ${this.totalTimeInMiliSeconds/1E3}`);for(b=0;b<c.length;b++)"B"===c[b].type&&(g=virtualclass.dtCon.base64DecToArr(c[b].recObjs),c[b].recObjs=g);this.masterRecordings.push(c);this.UIdownloadProgress(a);this.updateTotalTime();(3<this.currentMin||this.lastFile===a)&&0<this.masterRecordings.length&&
(this.playStart?this.startToPlay():this.alreadyAskForPlay||(this.alreadyAskForPlay=!0,this.askToPlay()))},updateTrimTime(a){this.totalTrimTime+=a;console.log("==== total trim time",this.totalTrimTime)},handleStartToPlay(a){const b=document.querySelector(".rv-vanilla-modal-overlay.is-shown");null!=b&&b.removeEventListener("click",this.handleStartToPlay.bind(this));virtualclass.gesture.clickToContinue();console.log("===== Start to play");this.startToPlay();a.currentTarget.classList.remove("askToPlayCont")},
startToPlay(){this.playStart?this.hasOwnProperty("isPausedByNotPresent")&&(delete this.isPausedByNotPresent,this.subRecordingIndex=0,this.subRecordings=this.masterRecordings[this.masterIndex],this.controller._play(),virtualclass.popup.closeElem()):(this.playStart=!0,this.playInt(),virtualclass.popup.closeElem())},isDownloadedAllRecordings(a){return 9E5<this.sessionStartTime?a>=virtualclass.recorder.lastTimeInSeconds:!1},finishRequestDataFromServer(a){this.isDownloadedAllRecordings(a)&&(virtualclass.recorder.allFileFound=
!0,virtualclass.recorder.alreadyAskForPlay&&(a=document.getElementById("askplayMessage"),null!=a&&(a.innerHTML=virtualclass.lang.getString("playsessionmsg"))))},UIdownloadProgress(a){a=virtualclass.recorder.getTimeFromFile(a);var b=(a-this.firstTimeInSeconds)/60;b>this.currentMin&&(this.currentMin=b);console.log("=====  total trim time ",this.totalTrimTime);b=virtualclass.recorder.totalTimeInMiliSeconds/1E3/60;this.downloadInPercentage=100*this.currentMin/b;virtualclass.pbar.renderProgressBar(b,this.currentMin,
...
a.offsetX)},getOffset(a){"touchend"==a.type?a=this.lastEvent:(a.offsetX=a.touches[0].pageX-a.touches[0].target.offsetLeft,a.offsetY=a.touches[0].pageY-a.touches[0].target.offsetTop);return this.lastEvent=a},seekHandler(a){void 0==a.offsetX&&(a=this.getOffset(a));this.pauseBeforeSeek=this.controller.pause;if(this.startSeek)console.log("Earlier seek start is not end yet.");else{this.startSeek=!0;var b=document.querySelector("#virtualclassCont");null!=b&&b.classList.add("recordSeeking");b=a.offsetX;
const c=this.getCustomWidth();this.seekValueInPercentage=b/c*100;console.log(`====Seek start ${this.seekValueInPercentage} ev current target=${a.currentTarget.id}`)}},getCustomWidth(){return document.getElementById("downloadProgress").clientWidth-1},seek(a){virtualclass.videoHost.UI.hideTeacherVideo();a=this.getSeekPoint(a);(a.master<this.masterIndex||a.master==this.masterIndex&&a.sub<this.subRecordingIndex)&&this.replayFromStart();this._seek(a);console.log("seek is finished");this.triggerSynchPacket()},
selfSeek(){console.log("Start self seek");this.selfStartSeek=!0;this._seek();this.triggerSynchPacket();this.controller._play()},triggerSynchPacket(){this.triggerPlayProgress();this.binarySyncMsg&&(this.handleSyncPacket(),this.binarySyncMsg=null);this.handleSyncStringPacket()},seekFinished(a){if(void 0==a&&void 0!=this.masterRecordings[this.masterIndex][this.subRecordingIndex])return-1<this.masterRecordings[this.masterIndex][this.subRecordingIndex].recObjs.indexOf('{"ac":true,"cf":"recs"')},_seek(a){this.controller._pause();
let b;for(;;){b=null!=a?this.masterIndex!=a.master?this.masterRecordings[this.masterIndex].length:a.sub:this.masterRecordings[this.masterIndex].length;for(let a=this.subRecordingIndex;a<b;a++){try{if("B"!=this.subRecordings[this.subRecordingIndex].type)this.msg=io.cleanRecJson(this.subRecordings[this.subRecordingIndex].recObjs),-1<this.msg.indexOf('"m":{"unshareScreen"')?this.binarySyncMsg=null:-1<this.msg.indexOf('},"m":{"poll":{"pollMsg":"stdPublish",')?virtualclass.poll.recordStartTime={app:"Poll",
...
if(104==this.msg[0]||204==this.msg[0]||102==this.msg[0]||202==this.msg[0])if(0==this.msg[1]||1==this.msg[1])this.binarySyncMsg={data:{masterIndex:this.masterIndex,subIndex:this.subRecordingIndex}};"ScreenShare"!=virtualclass.currApp||104!=this.msg[0]&&204!=this.msg[0]&&103!=this.msg[0]&&203!=this.msg[0]&&102!=this.msg[0]&&202!=this.msg[0]||(io.onRecMessage(this.convertInto({data:this.msg})),this.binarySyncMsg=null)}this.collectElapsedPlayTime()}catch(g){}if(virtualclass.settings.info.trimRecordings&&
this.selfStartSeek&&this.seekFinished()){this.selfSeekFinished=!0;break}this.subRecordingIndex++}this.elapsedRecTime=this.elapsedTime=this.elapsedPlayTime;if(void 0!=a&&this.masterIndex===a.master&&a.sub===this.subRecordingIndex||this.selfSeekFinished)break;else this.subRecordingIndex=0,this.masterIndex++,this.subRecordings=this.masterRecordings[this.masterIndex]}},handleSyncStringPacket(){if("Poll"==virtualclass.currApp&&"object"===typeof virtualclass.poll.pollState.data&&virtualclass.poll.hasOwnProperty("recordStartTime")){var a=
...
virtualclass.quiz.quizStartTime.data.subIndex);0<+virtualclass.quiz.plugin.config.quizTime?(b=1E3*virtualclass.quiz.plugin.config.quizTime-(this.elapsedPlayTime-b),virtualclass.quiz.plugin.method.startTimer(b/1E3,a,"desc","vmQuiz")):(b=(this.elapsedPlayTime-b)/1E3,virtualclass.quiz.plugin.method.startTimer(b,a,"asc","vmQuiz"))}},pollUpdateTime(a,b){a=this.convertIntoReadable(6E4*b.data.stdPoll.newTime.min+1E3*b.data.stdPoll.newTime.sec-(this.elapsedPlayTime-a));virtualclass.poll.newTimer.sec=a.s;
virtualclass.poll.newTimer.min=a.m;virtualclass.poll.showTimer(virtualclass.poll.newTimer)},handleSyncPacket(){if(this.binarySyncMsg){console.log("Get full screen share");var a=this.binarySyncMsg.data.subIndex;let b=this.binarySyncMsg.data.masterIndex;for(;b<=this.masterIndex;){console.log(`Start from master index ${b} from Subindex ${a}`);const c=this.masterRecordings[b];let g;for(g=this.masterIndex!=b?this.masterRecordings[this.masterIndex].length:this.subRecordingIndex;a<=g;a++){this.msg=c[a];
try{if(null!=this.msg&&"B"==this.msg.type){if(this.msg=this.msg.recObjs,104==this.msg[0]||204==this.msg[0]||103==this.msg[0]||203==this.msg[0]||102==this.msg[0]||202==this.msg[0])io.onRecMessage(this.convertInto({data:this.msg}))}else console.log("Either this.msg is null or string")}catch(k){console.log(`PLAY ERROR ${k.errorCode}`)}}if(this.masterIndex==b&&a==this.subRecordingIndex)break;else b++,a=0}console.log(`===== Elapsed time 2 ==== ${this.elapsedPlayTime}`);this.playProgressBar(this.elapsedPlayTime)}},
getSeekPoint(a){a=Math.trunc(this.totalTimeInMiliSeconds*a/100);console.log(`Seek index ${a}`);let b=0;for(let c=0;c<this.masterRecordings.length;c++)for(let f=0;f<this.masterRecordings[c].length;f++){b+=this.masterRecordings[c][f].playTime;if(b==a)return{master:c,sub:f};if(b>=a)return 0<f?f--:(c--,f=0<=c?this.masterRecordings[c].length-1:0),console.log(`Seek index i = ${c} j=${f} totalTime=${b}`),{master:c,sub:f}}},play(){this.controller.pause||(this.config(),this.isPlayFinished()?this.allFileFound?
...
"undefined"!==typeof this.playTimeout&&""!=this.playTimeout&&clearTimeout(this.playTimeout);this.hasOwnProperty("playTime")||(this.subRecordings=this.masterRecordings[this.masterIndex],io.cfg=JSON.parse(this.subRecordings[this.subRecordingIndex].recObjs),virtualclass.gObj.uRole="s")},executePacketToPlay(){this.calcPlayTime();this.playTimeout=setTimeout(()=>{this.triggerPlayProgress();try{0>this.subRecordings[this.subRecordingIndex].recObjs.indexOf('"cf":"sync"')&&(io.onRecMessage(this.convertInto({data:this.subRecordings[this.subRecordingIndex].recObjs})),
"Poll"==virtualclass.currApp&&-1<this.subRecordings[this.subRecordingIndex].recObjs.indexOf('},"m":{"poll":{"pollMsg":"stdPublish",')?virtualclass.poll.recordStartTime={app:"Poll",data:{masterIndex:this.masterIndex,subIndex:this.subRecordingIndex}}:"Video"==virtualclass.currApp&&-1<this.subRecordings[this.subRecordingIndex].recObjs.indexOf('"m":{"videoUl":{"content_path"')?(virtualclass.videoUl.videoStartTime={app:"Video",data:{masterIndex:this.masterIndex,subIndex:this.subRecordingIndex}},console.log("Capture video")):
"Quiz"==virtualclass.currApp&&-1<this.subRecordings[this.subRecordingIndex].recObjs.indexOf('"m":{"quiz":{"quizMsg":"stdPublish",')?virtualclass.quiz.quizStartTime={app:"Quiz",data:{masterIndex:this.masterIndex,subIndex:this.subRecordingIndex}}:virtualclass.settings.info.trimRecordings&&-1<this.masterRecordings[this.masterIndex][this.subRecordingIndex].recObjs.indexOf('{"ac":false,"cf":"recs"')&&virtualclass.recorder.selfSeek())}catch(a){console.log(`PLAY ERROR ${a.errorCode}`)}this.calcPlayTimeNext();
...
!1;virtualclass.recorder.replayFromStart.call(virtualclass.recorder)})},getCurrentPacket(){let a=0;if(0<this.masterIndex)for(let b=0;b<this.masterIndex;b++)a+=this.masterRecordings[b].length;return a+this.subRecordingIndex},collectElapsedPlayTime(){this.elapsedPlayTime+=this.subRecordings[this.subRecordingIndex].playTime},triggerPlayProgress(){this.collectElapsedPlayTime();0==this.masterIndex&&0==this.subRecordingIndex?this.playProgressBar(this.elapsedPlayTime):(this.playTimeNowSeconds=Math.round(this.elapsedPlayTime/
...
for(let a=0;a<b.length;a++)b[a].classList.remove("controlActive");a.parentNode.classList.add("controlActive")},controller:{pause:!1,ff:1,_play(){virtualclass.recorder.startTime=performance.now();this.ff=1;this.pause=!1;virtualclass.recorder.play();console.log("====== Recording play")},_pause(){this.pause=!0;console.log("====== Recording pause")},fastForward(a){this.ff=a;this.pause=!1;virtualclass.recorder.play()}},sendData(a,b,c){this.cb=c;a=JSON.stringify(a);this.httpObj.open("POST",b);this.httpObj.setRequestHeader("x-api-key",
wbUser.lkey);this.httpObj.setRequestHeader("x-congrea-authuser",wbUser.auth_user);this.httpObj.setRequestHeader("x-congrea-authpass",wbUser.auth_pass);this.httpObj.setRequestHeader("x-congrea-room",wbUser.room);this.httpObj.setRequestHeader("Content-Type","application/json");this.httpObj.withCredentials=!1;this.httpObj.send(a)},sortingFiles(a){return a.sort(function(a,c){return a<c?-1:a>c?1:0})},setOrginalListOfFiles(a){const b={};for(let c=0;c<a.length;c++)b[a[c]]=null!=a[c+1]?{index:c,next:a[c+
1]}:{index:c,next:"end"};return b},triggerDownloader(){console.log("Init trigger time to request 1");this.tryNumberOfTimes=1;this.hasOwnProperty("triggerDownloaderTime")&&clearInterval(this.triggerDownloaderTime);this.hasOwnProperty("startTimeCounter")&&clearInterval(this.startTimeCounter);let a=0;this.startTimeCounter=setInterval(()=>{a++;console.log(`=====Timer ===${a}`)},1E3);this.triggerDownloaderTime=setInterval(()=>{console.log("Init trigger time to request 2");3<this.tryNumberOfTimes?(virtualclass.recorder.allFileFound=
!0,clearInterval(this.triggerDownloaderTime.triggerDownloaderTime),this.isPlayFinished()&&this.askAgainToPlay()):virtualclass.xhrn.sendData({session:this.session},virtualclass.api.recordingFiles,this.afterDownloadingList.bind(this));console.log("Time to request 180000")},18E4)},afterDownloadingList(a){a=JSON.parse(a);if(null!=a&&a.hasOwnProperty("Item")){var b=+a.Item.time.N;this.sessionStartTime=(new Date).getTime()-b;b=[...new Set(a.Item.list.L.map(a=>a.S))];a=b.length;this.lastRecordings&&b.length==
this.lastRecordings?this.tryNumberOfTimes=this.tryNumberOfTimes?++this.tryNumberOfTimes:1:(delete this.alreadyCalcTotTime,this.totalRecordingFiles=this.sortingFiles(b),this.lastFile=this.totalRecordingFiles[this.totalRecordingFiles.length-1],this.orginalListOfFiles=this.setOrginalListOfFiles(this.totalRecordingFiles),this.calculateTotalPlayTime(),b=this.totalRecordingFiles.shift(),this.requestDataFromServer(b),0<this.totalRecordingFiles.length&&(b=this.totalRecordingFiles.shift(),this.requestDataFromServer(b)),
9E5>this.sessionStartTime&&this.triggerDownloader());this.lastRecordings=a}else this.tryNumberOfTimes=this.tryNumberOfTimes?++this.tryNumberOfTimes:1,this.triggerDownloader()},requestListOfFiles(){this.session=wbUser.session;virtualclass.popup.loadingWindow();virtualclass.xhrn.sendData({session:virtualclass.recorder.session},virtualclass.api.recordingFiles,this.afterDownloadingList.bind(this))},calculateTotalPlayTime(){const a=Math.trunc(+virtualclass.recorder.totalRecordingFiles[0].split("-")[0]/
1E6),b=Math.trunc(+virtualclass.recorder.totalRecordingFiles[virtualclass.recorder.totalRecordingFiles.length-1].split("-")[1].split(".")[0]/1E6);this.totalTimeInMiliSeconds=b-a;this.lastTimeInSeconds=Math.trunc(b/1E3);this.firstTimeInSeconds=Math.trunc(a/1E3)},getTimeFromFile(a){return+a.split("-")[1].split(".")[0]/1E9},totalPlayTime(){let a=0;for(let b=0;b<virtualclass.recorder.masterRecordings.length;b++)for(let c=0;c<virtualclass.recorder.masterRecordings[b].length;c++)a+=virtualclass.recorder.masterRecordings[b][c].playTime;
return a/1E3},getTotalTimeInMilSeconds(a,b){let c=0,g=0;for(;c<=a;){for(let f=0;f<virtualclass.recorder.masterRecordings[c].length&&(g+=virtualclass.recorder.masterRecordings[c][f].playTime,a!==c||b!==f);f++);c+=1}return g},getTempTotalTimeInMilSeconds(a,b){let c=0,g=0;for(;c<=a;){for(let f=0;f<this.orginalTimes[c].length&&(g+=this.orginalTimes[c][f],a!=c||b!=f);f++);c++}return g},getTotalElementLength(){let a=0;for(let b=0;b<virtualclass.recorder.masterRecordings.length;b++)a+=virtualclass.recorder.masterRecordings[b].length;
return a},seekWithMouseMove(a){void 0==a.offsetX&&(a=this.getOffset(a));this.startSeek&&(this.controller._pause(),this.seekValueInPercentage=this.getSeekValueInPercentage(a),this.downloadInPercentage<this.seekValueInPercentage&&(this.seekValueInPercentage=Math.trunc(this.downloadInPercentage)),this.setPlayProgressTime(this.seekValueInPercentage),virtualclass.pbar.renderProgressBar(this.totalTimeInMiliSeconds,6E4*this.seekTimeWithMove.m+1E3*this.seekTimeWithMove.s,"playProgressBar",void 0),document.querySelector("#tillRepTime").innerHTML=
`${this.seekTimeWithMove.h}:${this.seekTimeWithMove.m}:${this.seekTimeWithMove.s}`,this.displayTimeInHover(a,this.seekValueInPercentage))},getSeekValueInPercentage(a){const b=this.getCustomWidth();return a.offsetX/b*100},handlerDisplayTime(a){const b=this.getSeekValueInPercentage(a);this.displayTimeInHover(a,b)},displayTimeInHover(a,b){this.setPlayProgressTime(b);a=20>a.offsetX?3:5>d.innerWidth-a.offsetX?a.offsetX-65:30>d.innerWidth-a.offsetX?a.offsetX-60:40>d.innerWidth-a.offsetX?a.offsetX-35:a.offsetX-
25;b=document.getElementById("timeInHover");b.style.display="block";b.style.marginLeft=`${a}px`;document.getElementById("timeInHover").innerHTML=`${this.seekTimeWithMove.h}:${this.seekTimeWithMove.m}:${this.seekTimeWithMove.s}`;document.querySelector("#virtualclassCont").classList.add("recordSeeking")},setPlayProgressTime(a){this.seekTimeWithMove=this.convertIntoReadable(a*this.totalTimeInMiliSeconds/100)},finalSeek(a){a.offsetX||(a=this.getOffset(a));this.startSeek&&this.hasOwnProperty("seekValueInPercentage")&&
