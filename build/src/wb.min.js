...
c.classList.add("active"));virtualclass.gObj.audMouseDown=!0;workerAudioSend.postMessage({cmd:"audioMouseDown",msg:{adMouseDown:virtualclass.gObj.audMouseDown}});virtualclass.vutil.beforeSend({sad:!0,cf:"sad"})}studentNotSpeak(c){Object.prototype.hasOwnProperty.call(virtualclass.gObj,"audMouseDown")&&virtualclass.gObj.audMouseDown&&("undefined"!==typeof c&&(document.getElementById(`${c.id}Button`),c.classList.remove("active"),c.classList.add("deactive")),c=document.getElementById("speakerPressOnce"),
c.setAttribute("data-audio-playing","false"),c.className="audioTool deactive",virtualclass.gObj.audMouseDown=!1,workerAudioSend.postMessage({cmd:"audioMouseDown",msg:{audMouseDown:virtualclass.gObj.audMouseDown}}),virtualclass.media.audio.setAudioStatus("stop"),virtualclass.vutil.beforeSend({sad:!1,cf:"sad"},null,!0))}ab2str(c){return String.fromCharCode.apply(null,new Int8Array(c))}str2ab(c){var b=new ArrayBuffer(c.length);b=new Int8Array(b);for(let a=0,d=c.length;a<d;a++)b[a]=c.charCodeAt(a);return b}recorderProcess(c){if(!this.repMode&&
(this.recordAudio||(this.recordingLength+=this.bufferSize),virtualclass.gObj.audMouseDown&&io.webSocketConnected())){c=this.resampler.resampler(c);c=convertFloat32ToInt16(c);const b=this.encodeAudio(c);this.silenceDetection(b,c)}}recorderProcessFallback(c){workerAudioSend.postMessage({cmd:"rawAudio",msg:c.inputBuffer.getChannelData(0)})}encodeAudio(c){return G711.encode(c,{alaw:"alaw"===this.encMode})}audioForTesting(c){c=G711.encode(c,{alaw:"alaw"===this.encMode});virtualclass.gObj.audioForTest.push(c);
virtualclass.gObj[virtualclass.gObj.uid]=!1}initPlayWithFallback(){Object.prototype.hasOwnProperty.call(this,"Html5Audio")||this.Html5Audio||(this.Html5Audio={audioContext:new (window.AudioContext||window.webkitAudioContext)});if(!this.workerAudioRecOnmessage){const c=new MessageChannel;workerIO.postMessage({cmd:"workerAudioRec"},[c.port1]);workerAudioRec.postMessage({cmd:"workerIO",sampleRate:this.Html5Audio.audioContext.sampleRate},[c.port2]);workerAudioRec.postMessage({cmd:"audioWorklet",msg:!1});
workerAudioRec.onmessage=b=>{switch(b.data.cmd){case "noAudioWorklet":virtualclass.media.audio.queueWithFalback(b.data.msg.data,b.data.msg.uid),virtualclass.media.audio.playWithFallback(b.data.msg.uid)}};this.workerAudioRecOnmessage=!0;virtualclass.gObj.audioRecWorkerReady=!0}}initPlay(){this.addingWorkletPending||(this.workletAudioRec&&(console.log("audio worklet disconnect"),this.workletAudioRec.disconnect()),Object.prototype.hasOwnProperty.call(this,"audioContextRec")||this.audioContextRec||(this.Html5Audio=
{audioContextRec:new (window.AudioContext||window.webkitAudioContext)}),this.addingWorkletPending=!0,virtualclass.media.audio.Html5Audio.audioContextRec.audioWorklet.addModule(workletAudioRecBlob).then(()=>{this.workletAudioRec=new AudioWorkletNode(this.Html5Audio.audioContextRec,"worklet-audio-rec");virtualclass.media.audio.Html5Audio.MediaStreamDest=this.Html5Audio.audioContextRec.createMediaStreamDestination();this.workletAudioRec.connect(this.Html5Audio.audioContextRec.destination);virtualclass.gObj.workletAudioRec=
this.workletAudioRec;"Chrome"===virtualclass.system.mybrowser.name&&virtualclass.media.audio.bug_687574_callLocalPeers();var c=new MessageChannel;workerIO.postMessage({cmd:"workerAudioRec"},[c.port1]);workerAudioRec.postMessage({cmd:"workerIO",sampleRate:this.Html5Audio.audioContextRec.sampleRate},[c.port2]);c=new MessageChannel;workerAudioRec.postMessage({cmd:"workletAudioRec"},[c.port1]);this.workletAudioRec.port.postMessage({cmd:"workerAudioRec"},[c.port2]);workerAudioRec.postMessage({cmd:"audioWorklet",
msg:!0});virtualclass.gObj.audioRecWorkerReady=!0;delete virtualclass.media.audio.addingWorkletPending}).catch(c=>{console.log("ERROR ",c)}))}playWithFallback(c){"suspended"===this.Html5Audio.audioContext.state?Object.prototype.hasOwnProperty.call(this,"audioSuspendTime")||(this.audioSuspendTime=setTimeout(()=>{"suspended"===this.Html5Audio.audioContext.state&&(this.snode.push(c),null===virtualclass.gObj.requestToScriptNode&&(this.Html5Audio.audioContext.resume(),virtualclass.gesture.initAudioResume(c),
virtualclass.gObj.requestToScriptNode=!0),delete this.audioSuspendTime)},2E3)):(this.innerPlayWithFallback(),this.audioSuspendTime&&delete this.audioSuspendTime)}innerPlayWithFallback(){const c=this;if(null===virtualclass.media.audioPlayerNode||"closed"===virtualclass.media.audioPlayerNode.context.state)null!==virtualclass.media.audioPlayerNode&&virtualclass.media.audioPlayerNode.disconnect(),virtualclass.media.audioPlayerNode=this.Html5Audio.audioContext.createScriptProcessor(4096,1,1),this.snNodePak=
0,virtualclass.media.audioPlayerNode.onaudioprocess=b=>{b=b.outputBuffer.getChannelData(0);var a=c.getMergedAudio();if(null!==a&&void 0!==a){for(let d=0;d<a.length;d++)b[d]=a[d];this.snNodePak=a[4095]}else for(a=0;a<b.length;a++)b[a]=this.snNodePak},virtualclass.media.audioPlayerNode.connect(this.Html5Audio.audioContext.destination)}queueWithFalback(c,b){null==this.audioToBePlay[b]&&(this.audioToBePlay[b]=[]);null==this.allAudioArr[b]&&(this.allAudioArr[b]=[]);for(let a=0;a<c.length;a++)this.allAudioArr[b].push(c[a]);
for(;4096<=this.allAudioArr[b].length;)c=this.allAudioArr[b].splice(0,4096),this.audioToBePlay[b].push(new Float32Array(c))}getMergedAudio(){this.allAudioSend=[];this.audioLen=0;for(var c in this.audioToBePlay){const a=this.getAudioChunks(c);if(null!=a)if(this.audioLen++,1===this.audioLen)for(var b=0;4096>b;b++)this.allAudioSend[b]=a[b];else for(b=0;4096>b;b++)this.allAudioSend[b]=allAudioSend[b]+a[b]}if(1===this.audioLen)return this.allAudioSend;if(1<this.audioLen){for(c=0;4096>c;c++)this.allAudioSend[c]/=
this.audioLen;return this.allAudioSend}}getAudioChunks(c){if(null!==this.audioToBePlay){if(9<=this.audioToBePlay[c].length){for(;3<=this.audioToBePlay[c].length;)this.audioToBePlay[c].shift();this.aChunksPlay[c]=!0;return this.audioToBePlay[c].shift()}if(2<=this.audioToBePlay[c].length||0<this.audioToBePlay[c].length&&!0===this.aChunksPlay[c])return this.aChunksPlay[c]=!0,this.audioToBePlay[c].shift();this.aChunksPlay[c]=!1;0===this.audioToBePlay[c].length&&delete this.audioToBePlay[c]}}actualManiPulateStream(){virtualclass.media.detectAudioWorklet()?
this.maniPulateStream():this.maniPulateStreamWithFallback()}maniPulateStream(){const {stream:c}=virtualclass.media;"undefined"!==typeof c&&null!=c&&virtualclass.media.audio.Html5Audio.audioContext.audioWorklet.addModule(workletAudioSendBlob).then(()=>{console.log("====> Add stream on media source ",c);this.audioInputForAudioWorklet&&(this.audioInputForAudioWorklet.disconnect(),this.filterAudioWorklet.disconnect());this.audioInputForAudioWorklet=virtualclass.media.audio.Html5Audio.audioContext.createMediaStreamSource(c);
this.filterAudioWorklet=virtualclass.media.audio.Html5Audio.audioContext.createBiquadFilter();this.filterAudioWorklet.type="lowpass";this.filterAudioWorklet.frequency.value=2E3;this.audioInputForAudioWorklet.connect(this.filterAudioWorklet);this.workletAudioSend=new AudioWorkletNode(virtualclass.media.audio.Html5Audio.audioContext,"worklet-audio-send");this.workletAudioSend.onprocessorerror=a=>{console.log("this is on process error");virtualclass.media.audio.notifiyMuteAudio();virtualclass.media.audio.isProcessError=
!0};this.filterAudioWorklet.connect(this.workletAudioSend);this.workletAudioSend.connect(virtualclass.media.audio.Html5Audio.audioContext.destination);virtualclass.gObj.workletAudioSend=this.workletAudioSend;var b=new MessageChannel;workerAudioSend.postMessage({cmd:"workerIO",sampleRate:this.Html5Audio.audioContext.sampleRate,uid:virtualclass.gObj.uid},[b.port1]);workerIO.postMessage({cmd:"workerAudioSend"},[b.port2]);b=new MessageChannel;workerAudioSend.postMessage({cmd:"audioWorkletSend",msg:{repMode:this.repMode}},
[b.port1]);this.workletAudioSend.port.postMessage({cmd:"workerAudioSend"},[b.port2]);virtualclass.media.audio.workerAudioSendOnmessage()}).catch(b=>{virtualclass.media.audio.notifiyMuteAudio()})}workerAudioSendOnmessage(){workerAudioSend.onmessage=c=>{Object.prototype.hasOwnProperty.call(c.data,"cmd")&&("muteAudio"===c.data.cmd?this.notifiyMuteAudio():"unMuteAudio"===c.data.cmd&&this.notifiyUnmuteAudio())}}maniPulateStreamWithFallback(){if("undefined"!==typeof virtualclass.media.stream&&null!=virtualclass.media.stream){var {stream:c}=
virtualclass.media;c=virtualclass.media.audio.Html5Audio.audioContext.createMediaStreamSource(c);virtualclass.media.audio.bufferSize=4096;virtualclass.media.audioCreatorNode=virtualclass.media.audio.Html5Audio.audioContext.createScriptProcessor(virtualclass.media.audio.bufferSize,1,1);virtualclass.media.audioCreatorNode.onaudioprocess=virtualclass.media.audio.recorderProcessFallback.bind(virtualclass.media.audio);const b=virtualclass.media.audio.Html5Audio.audioContext.createBiquadFilter();b.type=
"lowpass";b.frequency.value=2E3;c.connect(b);b.connect(virtualclass.media.audioCreatorNode);virtualclass.media.audioCreatorNode.connect(virtualclass.media.audio.Html5Audio.audioContext.destination);c=new MessageChannel;workerAudioSend.postMessage({cmd:"workerIO",uid:virtualclass.gObj.uid,sampleRate:virtualclass.media.audio.Html5Audio.audioContext.sampleRate},[c.port1]);workerIO.postMessage({cmd:"workerAudioSend"},[c.port2]);virtualclass.media.workerAudioSendOnmessage()}}updateInfo(){this.audioStreamArr=
[];this.repMode=this.recordAudio=!1}extractData(c){c=new Int8Array(c);return[virtualclass.vutil.numValidateFour(c[1],c[2],c[3],c[4]),c.subarray(5,c.length)]}removeAudioFromLocalStorage(){localStorage.removeItem("audEnable")}async bug_687574_callLocalPeers(){function c(a,b){if("connected"===b.currentTarget.connectionState)try{virtualclass.media.audio.workletAudioRec.disconnect(),virtualclass.media.audio.workletAudioRec.connect(virtualclass.media.audio.Html5Audio.MediaStreamDest)}catch(r){console.log("Audio error ",
...
;class MediaVideo{constructor(){this.width=75;this.height=56;this.remoteVidCont=this.remoteVid=this.myVideo=this.tempVidCont=this.tempVid="";this.enable=!1;this.randomTime=0;this.vidId=null}init(){this.videoCont=document.getElementById("allVideosCont");null!=this.videoCont&&(this.videoCont.style.maxHeight=`${this.maxHeight}px`)}calcDimension(){this.myVideo.width=this.width;this.myVideo.height=this.height}removeUser(c){c=document.getElementById(`user${c}`);null!=c&&c.parentNode.removeChild(c)}createElement(c){const b=
document.createElement("div");b.className="videoWrapper";const a=document.createElement("div");a.className="videoSubWrapper";a.id=`user${c.id}`;b.appendChild(a);this.vidId=c.id;const d=document.createElement("canvas");d.id=`video${this.vidId}`;d.className="userVideos";d.width=this.width;d.height=this.height;d.muted="muted";a.appendChild(d);virtualclass.media.util.imageReplaceWithVideo(c.id,b)}updateHightInSideBar(c){const b=document.getElementById("widgetRightSide").offsetHeight;document.getElementById("chatContainer").style.height=
`${b-c}px`}sendInBinary(c){io.webSocketConnected()&&ioAdapter.sendBinary(c)}send(){function c(){b.tempVidCont.clearRect(0,0,b.tempVid.width,b.tempVid.height);b.tempVidCont.drawImage(b.myVideo,0,0,b.width,b.height);roles.hasControls();virtualclass.system.webpSupport?(a=b.tempVid.toDataURL("image/webp",.6),d=1):(a=b.tempVid.toDataURL("image/jpeg",.3),d=0);a=virtualclass.videoHost.convertDataURIToBinary(a);if(!virtualclass.videoHost.gObj.stdStopSmallVid&&(!roles.hasControls()||roles.hasControls()&&virtualclass.videoHost.gObj.videoSwitch)){var k=
virtualclass.vutil.breakintobytes(virtualclass.gObj.uid,8);k=new Uint8ClampedArray([11,k[0],k[1],k[2],k[3],d]);var m=new Uint8ClampedArray(a.length+k.length);m.set(k);m.set(a,k.length);g.sendInBinary(m)}clearInterval(virtualclass.media.smallVid);if(Object.prototype.hasOwnProperty.call(virtualclass,"connectedUsers"))if(k=virtualclass.media.video.randomTime+2500*virtualclass.connectedUsers.length,f!==virtualclass.connectedUsers.length){f=virtualclass.connectedUsers.length;m=-1;for(var n=0;n<virtualclass.connectedUsers.length;n++)virtualclass.connectedUsers[0].userid===
virtualclass.gObj.uid&&(m=n);n=k/f;0>m&&(m=0);virtualclass.media.smallVid=setInterval(c,k+m*n)}else virtualclass.media.smallVid=setInterval(c,k)}Object.prototype.hasOwnProperty.call(virtualclass.media,"smallVid")&&clearInterval(virtualclass.media.smallVid);const b=this;let a,d;this.randomTime=Math.floor(5001*Math.random()+3E3);let f=-1;const g=this;virtualclass.media.smallVid=setInterval(c,this.randomTime)}startToStream(){virtualclass.media.video.calcDimension();virtualclass.media.video.send()}playWithoutSlice(c){this.remoteVid=
document.getElementById(`video${c}`);null!=this.remoteVid&&(this.remoteVidCont=this.remoteVid.getContext("2d"),this.remoteVidCont.putImageData(imgData,0,0))}drawReceivedImage(c,b,a,d){this.remoteVid=chatContainerEvent.elementFromShadowDom(`#video${d}`);if(null!=this.remoteVid)if(this.remoteVidCont=this.remoteVid.getContext("2d"),virtualclass.system.webpSupport||"jpeg"===b){const b=new Image,d=this;b.onload=()=>{d.remoteVidCont.drawImage(b,a.x,a.y)};b.src=c}else loadfile(c,this.remoteVid,this.videoPartCont)}createVideoElement(){const c=
document.createElement("div");c.className="videoWrapper";const b=document.createElement("div");b.className="videoSubWrapper";c.appendChild(b);const a=document.createElement("video");a.id=`video${virtualclass.gObj.uid}`;a.autoplay=!0;b.appendChild(a);return c}insertTempVideo(c){const b=document.createElement("canvas");b.id="tempVideo";c.parentNode.insertBefore(b,c)}tempVideoInit(){virtualclass.media.video.tempVid=chatContainerEvent.elementFromShadowDom("#tempVideo");virtualclass.media.video.tempVid.width=
virtualclass.media.video.width;virtualclass.media.video.tempVid.height=virtualclass.media.video.height;virtualclass.media.video.tempVidCont=virtualclass.media.video.tempVid.getContext("2d")}process(c){var b=new Uint8ClampedArray(c);c=virtualclass.vutil.numValidateFour(b[1],b[2],b[3],b[4]);var a={id:c};virtualclass.media.existVideoContainer(a)||virtualclass.media.video.createElement(a);a=b.subarray(6,b.length);1===b[5]?(b=`data:image/webp;base64,${btoa(virtualclass.videoHost.Uint8ToString(a))}`,a=
"webp"):(b=`data:image/jpeg;base64,${btoa(virtualclass.videoHost.Uint8ToString(a))}`,a="jpeg");virtualclass.media.video.drawReceivedImage(b,a,{x:0,y:0},c)}async handleVideoControl(){"off"===virtualclass.vutil.selfVideoStatus()?(this.enable=!0,await virtualclass.media.startMedia()):(this.enable=!1,virtualclass.media.stopMedia(),virtualclass.media.audio.enable&&virtualclass.media.startMedia())}};function MediaWrapper(c){virtualclass.adpt=new virtualclass.adapter;const b=virtualclass.adpt.init(navigator);return function(){return{isStarted:"",pc:[],cn:0,ba:!1,bb:!1,bc:!1,bNotRender:!1,remoteStream:"",turnReady:"",videoControlId:"videoContainer",videoContainerId:"videos",CONFIG:{width:{max:268},height:{max:142},frameRate:{max:6}},audioPlayerNode:null,audioCreatorNode:null,status:0,init(){this.audio=new MediaAudio;this.video=new MediaVideo},util:{imageReplaceWithVideo(a,b){var d=chatContainerEvent.elementFromShadowDom(`#ml${a}`);
...
toUser:a,cf:"reqscreen"},a);virtualclass.gObj.prvRequestScreenUser=a;"Video"===virtualclass.currApp&&null!=virtualclass.videoUl&&(ioAdapter.mustSend({videoUl:{init:"destroyPlayer"},cf:"destroyPlayer"}),virtualclass.videoUl.destroyPlayer())},audioWidgetEnable(a){if(null==localStorage.getItem("dvid")){const c=b.getElementById("speakerPressOnce");if(null!==c){"undefined"===typeof a&&(c.className="active");if(!0===virtualclass.settings.specificUser&&roles.isStudent()){a=b.getElementById("speakerPressOnce");
const c=a.getElementsByClassName("congtooltip")[0];virtualclass.media.audio.studentSpeak();a.setAttribute("data-audio-playing","true");c.setAttribute("data-title",virtualclass.lang.getString("audioEnable"));a.className="audioTool active"}c.style.opacity="1";c.style.pointerEvents="visible"}}},mediaWidgetDisable(a){virtualclass.system.mediaDevices.hasWebcam=!1;a=localStorage.getItem("audEnable");null!=a&&JSON.parse(a);a=b.getElementById("audioWidget");a.style.opacity="1";a.style.pointerEvents="visible";
