...
[a]=b.split(" ");a=Math.trunc(a/1E6);return a-c},insertPacketInto(c,a){a=Math.trunc(a/1E3);if(!isNaN(a)&&1<=a){const b={playTime:1E3,recObjs:'{"0{"user":{"userid":"2"},"m":{"app":"nothing","cf":"sync"}} ',type:"J"};for(let f=0;f<a;f++)c.push(b)}return c},makeRecordingQueue(c,a){console.log(`File formatting ${c}`);var b=[],f=null;a=a.trim().split(/(?:\r\n|\r|\n)/g);let g;for(let c=0;c<a.length;c++)if(null!=a[c]&&""!=a[c]){this.totalElements++;var d=a[c].substring(0,21);var m=a[c].substring(22,a[c].length);
[d,g]=d.split(" ");this.tempTime=d;d=Math.trunc(d/1E6);null!=this.refrenceTime&&(f?(this.tempPlayTime=d-this.refrenceTime-f,f=null):this.tempPlayTime=d-this.refrenceTime);if(null!=m&&""!=m){!this.joinRoomRecevied&&-1<m.indexOf('"type":"joinroom"')&&(this.joinRoomRecevied=!0,m=JSON.parse(m),m.clientids=m.action,m=JSON.stringify(m));if(this.lastFileTime&&0===c){const a=Math.trunc(d-this.lastFileTime);b=this.insertPacketInto(b,a);this.tempPlayTime=1E3<a?a%1E3:a}c===a.length-1&&(this.lastFileTime=d);
virtualclass.settings.info.trimRecordings?this.isTrimRecordingNow?(b.push({playTime:0,recObjs:m,type:g}),-1<m.indexOf('{"ac":21,"cf":"recs"')&&(this.trimofftime=d,this.isTrimRecordingNow=!1,this.totalTimeInMiliSeconds-=this.trimofftime-this.trimontime,this.trimontime=this.trimofftime=0)):-1<m.indexOf('{"ac":11,"cf":"recs"')?(this.isTrimRecordingNow=!0,b.push({playTime:0,recObjs:m,type:g}),this.trimontime=d):b.push({playTime:this.tempPlayTime,recObjs:m,type:g}):b.push({playTime:this.tempPlayTime,recObjs:m,
type:g});"undefined"!==typeof a[c+1]&&(m=this.calculateNextTime(d,a[c+1]),b=this.insertPacketInto(b,m,!0),1E3<=m&&(f=1E3*Math.trunc(m/1E3)));this.refrenceTime=d}}console.log(`totalTime in seconds ${this.totalTimeInMiliSeconds/1E3}`);for(a=0;a<b.length;a++)"B"===b[a].type&&(f=virtualclass.dtCon.base64DecToArr(b[a].recObjs),b[a].recObjs=f);this.masterRecordings.push(b);b=virtualclass.recorder.getTimeFromFile(c);this.finishRequestDataFromServer(b);this.isTrimRecordingNow&&this.allFileFound&&(this.totalTimeInMiliSeconds-=
this.trimofftime-d,this.isTrimRecordingNow=!1);this.UIdownloadProgress(c,b);this.updateTotalTime();(3<this.currentMin||this.lastFile===c)&&0<this.masterRecordings.length&&(this.playStart?this.startToPlay():this.alreadyAskForPlay||(this.alreadyAskForPlay=!0,this.askToPlay()))},handleStartToPlay(c){const a=document.querySelector(".rv-vanilla-modal-overlay.is-shown");null!=a&&a.removeEventListener("click",this.handleStartToPlay.bind(this));virtualclass.gesture.clickToContinue();console.log("===== Start to play");
...
(this.subRecordings=this.masterRecordings[this.masterIndex],io.cfg=JSON.parse(this.subRecordings[this.subRecordingIndex].recObjs),virtualclass.gObj.uRole="s")},executePacketToPlay(){this.calcPlayTime();this.playTimeout=setTimeout(()=>{this.triggerPlayProgress();try{0>this.subRecordings[this.subRecordingIndex].recObjs.indexOf('"cf":"sync"')&&(io.onRecMessage(this.convertInto({data:this.subRecordings[this.subRecordingIndex].recObjs})),"Poll"===virtualclass.currApp&&-1<this.subRecordings[this.subRecordingIndex].recObjs.indexOf('},"m":{"poll":{"pollMsg":"stdPublish",')?
virtualclass.poll.recordStartTime={app:"Poll",data:{masterIndex:this.masterIndex,subIndex:this.subRecordingIndex}}:"Video"===virtualclass.currApp&&-1<this.subRecordings[this.subRecordingIndex].recObjs.indexOf('"m":{"videoUl":{"content_path"')?(virtualclass.videoUl.videoStartTime={app:"Video",data:{masterIndex:this.masterIndex,subIndex:this.subRecordingIndex}},console.log("Capture video")):"Quiz"===virtualclass.currApp&&-1<this.subRecordings[this.subRecordingIndex].recObjs.indexOf('"m":{"quiz":{"quizMsg":"stdPublish",')&&
(virtualclass.quiz.quizStartTime={app:"Quiz",data:{masterIndex:this.masterIndex,subIndex:this.subRecordingIndex}}))}catch(c){console.log(`PLAY ERROR ${c.errorCode}`)}this.calcPlayTimeNext();this.play()},this.playTime)},isPlayFinished(){return"undefined"===typeof this.masterRecordings[this.masterIndex]&&("undefined"===typeof this.subRecordings||"undefined"===typeof this.subRecordings[this.subRecordingIndex])||"undefined"!==typeof this.subRecordings&&"undefined"!==typeof this.subRecordings[this.subRecordingIndex]&&
...
virtualclass.popup.sendBackOtherElems();document.getElementById("replayClose").addEventListener("click",()=>{d.close()});document.getElementById("replayButton").addEventListener("click",()=>{virtualclass.recorder.controller.pause=!1;virtualclass.recorder.replayFromStart.call(virtualclass.recorder)})},getCurrentPacket(){let c=0;if(0<this.masterIndex)for(let a=0;a<this.masterIndex;a++)c+=this.masterRecordings[a].length;return c+this.subRecordingIndex},collectElapsedPlayTime(){this.elapsedPlayTime+=
...
c.doControlActive(this)};c=this;const b=document.getElementById("recPause");b.addEventListener("click",()=>{b.parentNode.classList.contains("recordingPlay")?c.initRecPause():c.initRecPlay()})}},initRecPause(){const c=document.getElementById("recPause");this.controller._pause();this.doControlActive(c);virtualclass.recorder.triggerPauseVideo();c.parentNode.classList.remove("recordingPlay");c.dataset.title="Play"},initRecPlay(){const c=document.getElementById("recPause");this.controller._play();this.doControlActive(c);
virtualclass.videoUl&&virtualclass.videoUl.player&&virtualclass.videoUl.player.play();c.parentNode.classList.add("recordingPlay");c.dataset.title="Pause"},doControlActive(c){const a=document.getElementById("playControllerCont").getElementsByClassName("recButton");for(let b=0;b<a.length;b++)a[b].classList.remove("controlActive");c.parentNode.classList.add("controlActive")},controller:{pause:!1,ff:1,_play(){virtualclass.recorder.startTime=performance.now();this.ff=1;this.pause=!1;virtualclass.recorder.play();
...
"application/json");this.httpObj.withCredentials=!1;this.httpObj.send(c)},sortingFiles(c){return c.sort(function(a,b){return a<b?-1:a>b?1:0})},setOrginalListOfFiles(c){const a={};for(let b=0;b<c.length;b++)a[c[b]]=null!=c[b+1]?{index:b,next:c[b+1]}:{index:b,next:"end"};return a},triggerDownloader(){console.log("Init trigger time to request 1");this.tryNumberOfTimes=1;this.hasOwnProperty("triggerDownloaderTime")&&clearInterval(this.triggerDownloaderTime);this.hasOwnProperty("startTimeCounter")&&clearInterval(this.startTimeCounter);
let c=0;this.startTimeCounter=setInterval(()=>{c++;console.log(`=====Timer ===${c}`)},1E3);this.triggerDownloaderTime=setInterval(()=>{console.log("Init trigger time to request 2");3<this.tryNumberOfTimes?(virtualclass.recorder.allFileFound=!0,clearInterval(this.triggerDownloaderTime.triggerDownloaderTime),this.isPlayFinished()&&this.askAgainToPlay()):virtualclass.xhrn.vxhrn.post(virtualclass.api.recordingFiles,data).then(a=>{this.afterDownloadingList(a.data)});console.log("Time to request 180000")},
18E4)},afterDownloadingList(c){if(null!=c&&c.hasOwnProperty("Item")){var a=+c.Item.time.N;this.sessionStartTime=(new Date).getTime()-a;a=[...new Set(c.Item.list.L.map(a=>a.S))];c=a.length;if(this.lastRecordings&&a.length==this.lastRecordings)this.tryNumberOfTimes=this.tryNumberOfTimes?++this.tryNumberOfTimes:1;else{delete this.alreadyCalcTotTime;this.totalRecordingFiles=this.sortingFiles(a);this.lastFile=this.totalRecordingFiles[this.totalRecordingFiles.length-1];this.orginalListOfFiles=this.setOrginalListOfFiles(this.totalRecordingFiles);
this.calculateTotalPlayTime();a=this.totalRecordingFiles.shift();this.firstxhr=axios.create({timeout:6E5,withCredentials:!0});this.requestDataFromServer(a,this.firstxhr);for(const a in this.totalRecordingFiles)virtualclass.createPrefetchLink(`https://recording.congrea.net/${wbUser.lkey}/${wbUser.room}/${virtualclass.recorder.session}/${this.totalRecordingFiles[a]}`);9E5>this.sessionStartTime&&this.triggerDownloader()}this.lastRecordings=c}else this.tryNumberOfTimes=this.tryNumberOfTimes?++this.tryNumberOfTimes:
1,this.triggerDownloader()},requestListOfFiles(){this.session=wbUser.session;virtualclass.popup.loadingWindow();virtualclass.xhrn.vxhrn.post(virtualclass.api.recordingFiles,{session:virtualclass.recorder.session}).then(c=>{this.afterDownloadingList(c.data)})},calculateTotalPlayTime(){const c=Math.trunc(+virtualclass.recorder.totalRecordingFiles[0].split("-")[0]/1E6),a=Math.trunc(+virtualclass.recorder.totalRecordingFiles[virtualclass.recorder.totalRecordingFiles.length-1].split("-")[1].split(".")[0]/
1E6);this.totalTimeInMiliSeconds=a-c;this.lastTimeInSeconds=Math.trunc(a/1E3);this.firstTimeInSeconds=Math.trunc(c/1E3)},getTimeFromFile(c){return+c.split("-")[1].split(".")[0]/1E9},totalPlayTime(){let c=0;for(let a=0;a<virtualclass.recorder.masterRecordings.length;a++)for(let b=0;b<virtualclass.recorder.masterRecordings[a].length;b++)c+=virtualclass.recorder.masterRecordings[a][b].playTime;return c/1E3},getTotalTimeInMilSeconds(c,a){let b=0,f=0;for(;b<=c;){for(let g=0;g<virtualclass.recorder.masterRecordings[b].length&&
(f+=virtualclass.recorder.masterRecordings[b][g].playTime,c!==b||a!==g);g++);b+=1}return f},getTempTotalTimeInMilSeconds(c,a){let b=0,f=0;for(;b<=c;){for(let g=0;g<this.orginalTimes[b].length&&(f+=this.orginalTimes[b][g],c!=b||a!=g);g++);b++}return f},getTotalElementLength(){let c=0;for(let a=0;a<virtualclass.recorder.masterRecordings.length;a++)c+=virtualclass.recorder.masterRecordings[a].length;return c},seekWithMouseMove(c){void 0==c.offsetX&&(c=this.getOffset(c));this.startSeek&&(this.controller._pause(),
this.seekValueInPercentage=this.getSeekValueInPercentage(c),this.downloadInPercentage<this.seekValueInPercentage&&(this.seekValueInPercentage=Math.trunc(this.downloadInPercentage)),this.setPlayProgressTime(this.seekValueInPercentage),virtualclass.pbar.renderProgressBar(this.totalTimeInMiliSeconds,6E4*this.seekTimeWithMove.m+1E3*this.seekTimeWithMove.s,"playProgressBar",void 0),document.querySelector("#tillRepTime").innerHTML=`${this.seekTimeWithMove.h}:${this.seekTimeWithMove.m}:${this.seekTimeWithMove.s}`,
this.displayTimeInHover(c,this.seekValueInPercentage))},getSeekValueInPercentage(c){const a=this.getCustomWidth();return c.offsetX/a*100},handlerDisplayTime(c){const a=this.getSeekValueInPercentage(c);this.displayTimeInHover(c,a)},displayTimeInHover(c,a){this.setPlayProgressTime(a);c=20>c.offsetX?3:5>d.innerWidth-c.offsetX?c.offsetX-65:30>d.innerWidth-c.offsetX?c.offsetX-60:40>d.innerWidth-c.offsetX?c.offsetX-35:c.offsetX-25;a=document.getElementById("timeInHover");a.style.display="block";a.style.marginLeft=
`${c}px`;document.getElementById("timeInHover").innerHTML=`${this.seekTimeWithMove.h}:${this.seekTimeWithMove.m}:${this.seekTimeWithMove.s}`;document.querySelector("#virtualclassCont").classList.add("recordSeeking")},setPlayProgressTime(c){this.seekTimeWithMove=this.convertIntoReadable(c*this.totalTimeInMiliSeconds/100)},finalSeek(c){c.offsetX||(c=this.getOffset(c));this.startSeek&&this.hasOwnProperty("seekValueInPercentage")&&(console.log(`====Seek up ${this.seekValueInPercentage}`),this.downloadInPercentage<
this.seekValueInPercentage&&(this.seekValueInPercentage=Math.trunc(this.downloadInPercentage)),0<this.seekValueInPercentage&&this.seek(this.seekValueInPercentage),this.pauseBeforeSeek?(console.log("=== Video pause "),this.controller._pause(),this.triggerPauseVideo()):(console.log("=== Video play "),this.controller._play(),this.triggerPlayVideo()),document.getElementById("timeInHover").style.display="none");document.querySelector("#virtualclassCont").classList.remove("recordSeeking");console.log(c.offsetX);
delete this.seekValueInPercentage;this.startSeek=!1;c=document.getElementById("congrealogo");null!=c&&c.classList.remove("disbaleOnmousedown")}}})(window);(function(d){let c,a=!1,b=!1,f=!1;d.storage={version:8,async init(){c=this;this.tables="wbData dataAdapterAll dataUserAdapterAll executedStoreAll executedUserStoreAll dstdata pollStorage quizData dstall".split(" ");this.db=await virtualclass.virtualclassIDBOpen("vidya_apps",c.dbVersion,{upgrade(a,b,f,d){c.onupgradeneeded(a)}});"object"===typeof this.db.objectStoreNames&&null!=this.db.objectStoreNames?await this.onsuccess():this.init()},onupgradeneeded(a){a.objectStoreNames.contains("wbData")||a.createObjectStore("wbData",
