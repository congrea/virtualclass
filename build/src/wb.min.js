...
a&&a.addEventListener("click",virtualclass.vutil.closeFullscreen);a=document.getElementById("butInstall");null!=document.getElementById("installContainer")&&a.addEventListener("click",virtualclass.vutil.installPwaBtn);var f=document.getElementById("chat_div");a=document.getElementById("sidebarButton");null!=a&&a.addEventListener("click",function(){const b=document.getElementById("virtualclassApp");b.classList.contains("openRightbar")?(b.classList.remove("openRightbar"),b.classList.add("collapsedRightbar"),
...
()=>{virtualclass.onfullscreenchange()},!1);document.addEventListener("webkitfullscreenchange",()=>{virtualclass.onfullscreenchange()},!1);document.addEventListener("msfullscreenchange",()=>{virtualclass.onfullscreenchange()},!1);virtualclass.isPlayMode?virtualclass.settings.triggerSettings():virtualclass.vutil.checkUserRole()&&virtualclass.settings.triggerSettings()},onfullscreenchange(){console.log("====> on full screen change");"virtualclassAppRightPanel"==event.target.id?virtualclass.gObj.rightbarFullScreenMode?
(document.getElementById("virtualclassAppRightPanel").classList.contains("fullScreenMode")&&document.getElementById("virtualclassAppRightPanel").classList.remove("fullScreenMode"),console.log("=====> full screen show ask hide exit"),virtualclass.gObj.rightbarFullScreenMode=!1,virtualclass.gObj.ignoreFullScreen||document.querySelector("html").classList.contains("pwaRunning")||virtualclass.vutil.showFullScreenButton()):(virtualclass.gObj.rightbarFullScreenMode=!0,console.log("=====> full screen show ask show exit ")):
virtualclass.gObj.fullScreenMode?virtualclass.vutil.showFullScreenButton():virtualclass.vutil.hideFullScreenButton();delete virtualclass.gObj.ignoreFullScreen},makeReadySocket(){virtualclass.vutil.isPlayMode()||!virtualclass.config.makeWebSocketReady||virtualclass.gObj.initsocketws||(virtualclass.gObj.initsocketws=!0,this.initSocketConn())},initSocketConn(){this.system.webSocket&&(io.init(virtualclass.uInfo),c.userdata=virtualclass.uInfo)},html:{id:"virtualclassCont",optionsClass:"appOptions",init(b){this.virtualclass=
b},leftAppBar(){if(!(0<document.getElementsByClassName("appOptions").length)){document.querySelector("#virtualclassApp #virtualclassAppLeftPanel");var b=virtualclass.getTemplate("appTools")();virtualclass.vutil.insertAppLayout(b);Object.prototype.hasOwnProperty.call(virtualclass.gObj,"errNotScreenShare")&&virtualclass.view.disableSSUI();Object.prototype.hasOwnProperty.call(virtualclass.gObj,"errAppBar")&&virtualclass.view.disableLeftAppBar()}},createDiv(b,a,c,k,m){const d=document.createElement("a");
d.href="#";const f=document.createElement("div");f.id=b;"undefined"!==typeof k&&(f.className=k);k=document.createElement("span");k.className=`icon-${a}`;d.appendChild(k);d.dataset.title=virtualclass.lang.getString(a);d.className="tooltip";"virtualclassWhiteboardTool"===b&&(d.dataset.doc=`${"_doc_0_"}${virtualclass.gObj.wbCount}`);f.appendChild(d);"undefined"!==typeof m?(b=document.getElementById("virtualclassScreenShareTool"),c.replaceChild(f,b)):c.appendChild(f)},createElement(b,a,c){b=document.createElement(b);
"undefined"!==typeof a&&(b.id=a);if("undefined"!==typeof c&&1<c.length)for(a=0;a<c.length;a++)b.className+=`${c[a]} `;return b}},dispvirtualclassLayout(b){if("undefined"!==typeof this.previous)if(this.previous.toUpperCase()!==`virtualclass${this.currApp}`.toUpperCase()){var a=document.getElementById(virtualclass.previous);null!=a&&(a.style.display="none");"undefined"!==typeof b&&"EDITORRICH"===b.toUpperCase()&&(a=document.getElementById("virtualclassEditorCode"),null!=a&&(a.style.display="none"))}else{a=
...
m=document.getElementById("virtualclassCont");virtualclass.vutil.setCurrApp(m,this.currApp);const b=document.querySelector("#virtualclassApp");null!==b&&("DocumentShare"===this.currApp||"SharePresentation"===this.currApp||"Video"===this.currApp?virtualclass.vutil.setCurrApp(b,m.dataset.currapp):b.dataset.currapp="")}"undefined"!==typeof this.prevScreen&&Object.prototype.hasOwnProperty.call(this.prevScreen,"currentStream")&&this.prevScreen.unShareScreen();if("SharePresentation"===a)`virtualclass${a}`!==
virtualclass.previous&&this.appInitiator[a].call(virtualclass,b);else if("DocumentShare"===a)this.appInitiator[a].call(virtualclass,b),roles.hasControls()?(virtualclass.serverData.syncComplete?virtualclass.vutil.triggerDashboard(a,"hidepopup"):virtualclass.vutil.triggerDashboard(a),virtualclass.dts.noteExist()&&virtualclass.zoom.normalRender()):virtualclass.dts.noteExist()||virtualclass.vutil.hidePageNumber();else if("Whiteboard"===a)"object"!==typeof this.orderList[a]&&(this.orderList[a]=new OrderedList),
this.handleWhiteboardReady(a,c,d),"Whiteboard"===virtualclass.currApp&&(virtualclass.zoom.normalRender(),virtualclass.zoom.fitToElementTooltip("fitToScreen"));else{var n;c=Array.prototype.slice.call(arguments)[2];this.appInitiator[a].call(virtualclass,b);c&&c.init&&c.init.videoUrl&&c.fromReload&&(n=!0);roles.hasControls()&&"Video"===a&&(`virtualclass${a}`!==virtualclass.previous&&(c=document.querySelector("#dashboardnav button"),null!=c&&(c.setAttribute("data-currapp","Video"),c.classList.contains("clicked")&&
...
this.ss.init({type:"ss",app:b})},Yts(b,a,c){"undefined"!==typeof c&&null!=c?virtualclass.yts.init(c):null==document.querySelector("iframe#player")&&virtualclass.yts.init();this.previous=virtualclass.ytsConfig.id},EditorRich(b){this.appInitiator.editor.call(virtualclass,b)},EditorCode(b){this.appInitiator.editor.call(virtualclass,b)},SharePresentation(b){let a,d;b.app&&(a=b.app);b.cusEvent&&(d=b.cusEvent);"object"===typeof this.ss&&(this.ss.prevStream=!1);this.sharePt=new c.sharePt;this.sharePt.init(a,
d);this.previous=virtualclass.ptConfig.id;virtualclass.sharePt.attachMessageEvent("message",virtualclass.sharePt.pptMessageEventHandler)},Poll(){this.userListEnable();virtualclass.poll.init()},Quiz(){this.userListEnable();virtualclass.quiz.init()},Video(b,a,c){"undefined"!==typeof c&&null!=c?"undefined"===typeof c.type?virtualclass.videoUl.init(c):virtualclass.videoUl.init():virtualclass.videoUl.init()},editor(b){b=b.app;this.user.control.toggleDisplayEditorController(b.substring(b.indexOf("virtualclass"),
b.length),"block");var a=[];b===virtualclass.apps.er?(virtualclass.editorRich.init(0,a,"",""),this.previous=virtualclass.edConfig.id):(virtualclass.editorCode.init(0,a,"",""),this.previous=virtualclass.edCodeConfig.id);a=virtualclass.editorRich?virtualclass.editorRich.editorStatus:!1;if(roles.isStudent()){const d=virtualclass.vutil.smallizeFirstLetter(b);a?this[d].cm.setOption("readOnly",!1):this[d].cm.setOption("readOnly","nocursor");this.user.control.toggleDisplayWriteModeMsgBox(b,a);virtualclass.vutil.setReadModeWhenTeacherIsDisConn(d)}},
...
setPrvUser(){localStorage.setItem("prvUser",JSON.stringify({id:wbUser.id,room:wbUser.room,role:wbUser.role,settings:virtualclassSetting.settings}))},registerPartial(){const b={whiteboardPath},a="precheck rightBarHeader teacherVideo audioWidget appTools popupCont appToolsMeeting appSettingDetail joinclass".split(" "),c={hasControl:roles.hasControls()};let k;for(let d=0;d<a.length;d++)k=null,"precheck"===a[d]||"popupCont"===a[d]?k=b:"audioWidget"===a[d]?(k=virtualclassSetting,k.isControl=roles.hasControls(),
k.isMettingMode=virtualclass.gObj.meetingMode&&roles.isStudent()):"teacherVideo"===a[d]||"appTools"===a[d]||"appSettingDetail"===a[d]?k=c:"rightBarHeader"===a[d]&&(k={std:roles.isStudent(),isPlayMode:virtualclass.isPlayMode}),this.makeReadyTemplate(a[d],k);Handlebars.registerPartial({docNotesMain:this.getTemplate("notesMain","documentSharing"),whiteboardToolbar:this.getTemplate("toolbar","whiteboard"),rightBar:this.getTemplate("rightBar"),recordingControl:this.getTemplate("recordingControl"),leftBar:this.getTemplate("leftBar"),
footerBar:this.getTemplate("footerBar"),mobileLandscapeWarn:this.getTemplate("mobileLandscapeWarn"),main:this.getTemplate("main"),whiteboard:this.getTemplate("main","whiteboard"),dashboardCont:this.getTemplate("dashboardCont"),multiVideo:this.getTemplate("multiVideo")})},registerHelper(){Handlebars.registerHelper("getString",b=>virtualclass.lang.getString(b));Handlebars.registerHelper("debug",function(b){});Handlebars.registerHelper("getVideoType",b=>virtualclass.gObj.meetingMode?"multiVideo":"teacherVideo")},
createMainContainer(){const b=document.querySelector(`#${virtualclass.html.id}`);this.registerHelper();this.registerPartial();var a=this.getTemplate("main");const c={isPlay:virtualclass.isPlayMode,hasControls:roles.hasControls(),meetingMode:virtualclass.gObj.meetingMode};a=a(c);b.insertAdjacentHTML("afterbegin",a)},makeReadyTemplate(b,a){Handlebars.registerPartial(b,(0,JST[`${virtualclass.gObj.tempPrefix}/${b}.hbs`])(a))},getTemplate(b,a){return"undefined"===typeof a?JST[`${virtualclass.gObj.tempPrefix}/${b}.hbs`]:
JST[`${virtualclass.gObj.tempPrefix}/${a}/${b}.hbs`]},isMiniFileIncluded(b){b=new RegExp(`${b}.js?=\*([0-9]*)`);const a=document.getElementsByTagName("script");for(let d=0;d<a.length;d++)if(b.test(a[d].src))return!0;return!1},stickybarWidth(){const b=document.querySelector("#virtualclassApp #virtualclassAppLeftPanel").offsetWidth+"px";document.querySelector("#stickybar").style.width=b},removeSharingClass(){const b=document.querySelector("#virtualclassCont");b&&(b.classList.remove("studentScreenSharing"),
document.querySelector("#chat_div").classList.remove("studentScreenSharing"))},clearAllChat(){localStorage.removeItem(virtualclass.gObj.uid);localStorage.clear("chatroom");null!=virtualclass.chat&&(virtualclass.chat.idList.length=0);clearAllChatBox();const b=document.getElementById("chatWidget").getElementsByClassName("ui-chatbox-msg");if(0<b.length)for(;null!=b[0];)b[0].parentNode.removeChild(b[0])},userListEnable(){const b=document.querySelector("#user_list");b&&b.click()}}}})(window);class Whiteboard{constructor(){this.selectedTool=this.canvas=null;this.rectangleObj=new WhiteboardRectangle("rectangle");this.activeAllObj=new WhiteboardActiveAll;this.freeDrawingObj=new WhiteboardFreeDrawing("freeDrawing");this.lineObj=new WhiteboardLine("line");this.triangleObj=new WhiteboardTriangle("triangle");this.circleObj=new WhiteboardCircle("circle");this.textObj=new WhiteboardText("text");this.gObj={}}init(c){this.wbId=c;delete virtualclass.wbWrapper.gObj.textSelected;this.textObj.textEditing=
