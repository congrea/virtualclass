...
document.getElementById("startLiveStream");null!=c&&c.classList.remove("disabled")}this.alreadyInit||(virtualclass.isPlayMode?(this.prefixUrl=`${this.uploadEndPoint}/${wbUser.lkey}/${wbUser.room}/${wbUser.session}`,console.log("Prefix url recording",this.prefixUrl)):null!=localStorage.mySession?(this.prefixUrl=`${this.uploadEndPoint}/${wbUser.lkey}/${wbUser.room}/${localStorage.mySession}`,console.log("Prefix url localstorage",this.prefixUrl)):virtualclass.gObj.currentSession&&(this.prefixUrl=`${this.uploadEndPoint}/${wbUser.lkey}/${wbUser.room}/${virtualclass.gObj.currentSession}`,
console.log("Prefix url localstorage",this.prefixUrl)),this.prefixUrl?(roles.hasControls()&&(console.log("==> attach click event "),document.getElementById("startLiveStream").addEventListener("click",this.handlLiveStream.bind(this))),this.alreadyInit=!0):setTimeout(()=>{console.log("Prefix was not found",this.prefixUrl);this.init()},1500))}requestStream(c){c&&void 0!=c?(console.log("====> remain queue length ",this.fileList.ol.order.length),console.log("request url ",c),this.xhr.get(c).then(async b=>
{this.afterReceivedStream(b)}).catch(b=>{5>=this.fileList.ol.order.length?this.lastFileRequested=this.firstFile:setTimeout(()=>{console.log("trigger init packet with ",this.lastFileRequested);this.requestInitializePacket(this.lastFileRequested)},2E3)})):console.log("Invalid URL")}getFileName(c){const b=c.lastIndexOf("/");return c.substring(b+1).replace(".chvs","")}duringPlayFirstPacket(){var c=document.getElementById("virtualclassVideo");c&&(c.dataset.currapp="liveStream");if(c=document.getElementById("messageLayoutVideo"))c.style.display=
"none"}afterReceivedStream(c){console.log("play start data received");const b=c.data.slice(4,c.data.length);c=this.getFileName(c.config.url);this.listStream[c]=b;console.log("reponse receive at very start, Play start 0 ",c);this.playIfReady(c);console.log("====> response received for file ",c)}onBuffer(c){if(this.sourceBuffer){const b=new Uint8Array(c);console.log("play start first four ",b[0],b[1],b[2],b[3]);this.sourceBuffer.appendBuffer(c)}}readyParticipateVideo(){this.remoteVideo||(this.remoteVideo=
document.getElementById("liveStream"),this.remoteVideo.srcObject=null,this.remoteVideo.currentTime=0)}initPLayerForParticipaes(){console.log("current mode LIVE STREAM");this.mediaSource=new MediaSource;this.mediaSource.addEventListener("sourceopen",this.mediaSourceOpen.bind(this));this.readyParticipateVideo();this.remoteVideo.src=URL.createObjectURL(this.mediaSource)}mediaSourceOpen(){console.log("step 1");"open"===this.mediaSource.readyState?(this.sourceBuffer=this.mediaSource.addSourceBuffer(this.mimeType),
this.sourceBuffer.addEventListener("error",function(c){}),this.sourceBuffer.addEventListener("updateend",function(){console.log("====> appended start: remove 2");const c=virtualclass.liveStream.fileList.getNextByID(virtualclass.liveStream.currentExecuted);console.log(" ===> actual PLAY START 2");c&&virtualclass.liveStream.playIfReady(c.id)})):console.log("MEDIA SOURCE IS NOT OPEN")}handlLiveStream(c){"0"===c.currentTarget.dataset.sharinglivestream?this.startToShare():(this.stop(),c=document.querySelector("#dashboardnav button"),
null!=c&&c.click())}disableLiveStreamButton(){const c=document.getElementById("startLiveStream");null!=c&&c.classList.add("disabled")}async startToShare(){this.stopTraditionalVideo();var c=c=document.getElementById("startLiveStream");c.dataset.sharinglivestream="1";c=null;try{c=await navigator.mediaDevices.getUserMedia(this.constraints)}catch(b){this.disableLiveStreamButton(),virtualclass.media.handleUserMediaError(b)}c&&this.handleSuccess(c)}handleSuccess(c){const b=c.getVideoTracks()[0].getSettings();
console.log("====> camera settings ",b);console.log("getUserMedia() got stream:",c);this.stream=c;virtualclass.dashboard.close();this.showLiveStreamHTML();document.getElementById("liveStream").srcObject=this.stream;this.startRecorder(b.width)}startRecorder(c){let b=3E5;640<c&&1920>=c?b=5E5:1920<c&&(b=1E6);console.log("current mode LIVE STREAM");console.log("video bit per second ",b);MediaRecorder.isTypeSupported(this.mimeType)||console.error(`${this.mimeType} is not supported`);this.mediaRecorder=
new MediaRecorder(this.stream,{mimeType:this.mimeType,videoBitsPerSecond:b});this.mediaRecorder.addEventListener("stop",this.stopHandler.bind(this));this.mediaRecorder.addEventListener("dataavailable",this.handleLiveStreamData.bind(this));this.mediaRecorder.start(2E3)}stopHandler(){console.log("====STOP 1")}handleLiveStreamData(c){console.log("====> triggering data ");this.mediaRecorder&&"recording"===this.mediaRecorder.state&&this.saveToStreamServer(c.data)}async saveToStreamServer(c){c=await this.videoTypedArray(c);
...
delete virtualclass.gObj.videoMode;console.log("delete normal video");var c=document.getElementById("virtualclassVideo");null!=c&&(c.dataset.currapp="normalVideo",c=document.getElementById("messageLayoutVideo"))&&(c.style.display="block");this.stream&&this.stream.getTracks().forEach(function(b){b.stop()});this.mediaRecorder&&delete this.mediaRecorder;if(this.sourceBuffer){if(this.sourceBuffer.updating)this.sourceBuffer.abort(),console.log("Source buffer abort");else{try{this.sourceBuffer.remove(0,
300)}catch(b){this.sourceBuffer.abort()}console.log("====> appended start: remove 3");console.log("Source buffer remove")}delete this.sourceBuffer;delete this.mediaSource}this.startedStream=!1;c=document.getElementById("liveStream");null!=c&&(c.src=null);delete this.firstFile;console.log("====> Empty the list 2a");delete this.startingPoint;delete this.startedAppending;this.fileList.emptyList();delete this.lastFileRequested;delete virtualclass.liveStream.callFromSeek;delete this.startFromPageRefresh;
console.log("====> appended start: remove 1")}showLiveStreamHTML(){document.getElementById("virtualclassVideo").dataset.currapp="liveStream";document.querySelector("#startLiveStream .label").innerHTML=virtualclass.lang.getString("stopLiveSharing")}hideLiveStreamHTML(){document.getElementById("virtualclassVideo").dataset.currapp="normalVideo";document.querySelector("#startLiveStream .label").innerHTML=virtualclass.lang.getString("startLiveSharing")}isLiveStreamMode(){return"liveStream"===document.getElementById("virtualclassVideo").dataset.currapp}onMessage(c){c.message.fileName?
(console.log("Received file from server ",c.message.fileName),setTimeout(()=>{"Video"===virtualclass.currApp&&this.mediaRecorder&&"recording"===this.mediaRecorder.state&&(console.log("====> send file ",c.message.fileName),ioAdapter.mustSend({cf:"liveStream",url:c.message.fileName}))},3E3)):c.message.stop?this.clearEveryThing():c.message.stopVideo?this.stopTraditionalVideo():c.message.url&&"Video"===virtualclass.currApp&&!virtualclass.gObj.hasOwnProperty("videoMode")&&roles.isStudent()&&(console.log("====> Empty the list 2"),
this.firstFile||(this.firstFile=c.message.url),this.lastFileRequested=c.message.url,console.log("Last file request ",this.lastFileRequested),this.fileList.insert(c.message.url,`${this.prefixUrl}/${c.message.url}.chvs`),!this.startingPoint&&this.fileList.ol.order.length>=this.bufferLength&&this.readyStartingPoint(),this.insertTime&&(clearTimeout(this.insertTime),this.pageRefresh=!0),this.insertTime=setTimeout(()=>{this.pageRefresh?5>=this.fileList.ol.order.length?(this.triggerStart(this.fileList.ol.order[0]),
console.log("live stream, start from first ",this.fileList.ol.order.length)):this.startFromPageRefresh||(this.startFromPageRefresh=!0,virtualclass.liveStream.callFromSeek||this.requestInitializePacket(),console.log("live stream, start from latest",this.fileList.ol.order.length)):this.triggerStart(c.message.url);console.log("page refresh remove");this.insertTime=this.pageRefresh=!1;console.log("live video suman 1")},100),console.log("====> live stream receive file ",c.message.url," queue length ",
...
virtualclass.gObj.currentSession;c=c?`https://api.congrea.net/data/stream?session=${b}&file=${c}`:`https://api.congrea.net/data/stream?session=${b}`;console.log("request url init packet ",c);this.requestInitializePacketFinal(c)}requestInitializePacketFinal(c){this.latesRequetInitUrl=c;console.log("request url live stream init data ",c);this.xhrInitPacket.get(c).then(async b=>{this.latesRequetInitUrl===b.config.url&&(this.currentFile=b.headers["x-congrea-seg"].split(".chvs")[0],console.log("request url live stream receive init data ",
this.currentFile,b.config.url.split("?")[1]),this.startedStream=!0,this.listStream[this.currentFile]=b.data,this.firstFile=b.headers["x-congrea-seg"].split(".chvs")[0],delete this.startingPoint,console.log("calculate starting point"),this.readyStartingPoint())})}triggerStart(c){this.startedStream?this.startedStream&&0<this.fileList.ol.order.length&&(c=this.fileList.getNextByID(this.currentFile),this.requestStream(c.data),this.currentFile=c.id):(this.initPLayerForParticipaes(),this.requestStream(this.fileList.getCurrent(c)),
this.currentFile=c,this.startedStream=!0)}isMyTurn(c){const b=this.fileList.getNextByID(this.currentExecuted);console.log("is my turn curernt, next ",this.currentExecuted,b.id,c);return b.id===c}inStreamList(c){return this.listStream[c]?this.listStream[c]:!1}readyStartingPoint(){let c=this.firstFile;if(-1>=this.fileList.getCurrentPosition(c))console.log("NOT FOUND ON LIST");else{var b;if(1<this.bufferLength){for(let a=1;a<this.bufferLength;a++)if(b=this.fileList.getNextByID(c))c=b.id;c!=this.firstFile&&
(this.startingPoint=c)}}}playIfReady(c){var b=this.inStreamList(c);if(this.startingPoint&&c===this.startingPoint&&this.inStreamList(this.firstFile)){b=this.inStreamList(this.firstFile);try{this.onBuffer(b),console.log("Actual append buffer ",this.firstFile),this.currentExecuted=this.firstFile,delete this.listStream[this.firstFile],this.startedAppending=!0,this.duringPlayFirstPacket(),this.startFromPageRefresh&&setTimeout(()=>{document.getElementById("liveStream").currentTime=this.MAX_TIME;console.log("REFRESH THE SCREEN")},
virtualclass.isPlayMode?700:1600)}catch(a){this.requestInitializePacket(c),console.log("====> Error handlling request packet")}}else if(this.startedAppending&&this.isMyTurn(c)&&b)try{this.onBuffer(b),console.log("Actual append buffer ",c),delete this.listStream[c],this.currentExecuted=c}catch(a){this.requestInitializePacket(c),console.log("====> Error handlling request packet")}}};$.uiBackCompat=!1;
