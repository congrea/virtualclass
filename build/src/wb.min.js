...
n._q.push(()=>{m.addTest=f});n.addAsyncTest(()=>{function b(b,a,c){function g(a){const g=a&&"load"===a.type?1==d.width:!1;f(b,"webp"===b&&g?new Boolean(g):g);c&&c(a)}var d=new Image;d.onerror=g;d.onload=g;d.src=a}const a=[{uri:"data:image/webp;base64,UklGRiQAAABXRUJQVlA4IBgAAAAwAQCdASoBAAEAAwA0JaQAA3AA/vuUAAA=",name:"webp"},{uri:"data:image/webp;base64,UklGRkoAAABXRUJQVlA4WAoAAAAQAAAAAAAAAAAAQUxQSAwAAAABBxAR/Q9ERP8DAABWUDggGAAAADABAJ0BKgEAAQADADQlpAADcAD++/1QAA==",name:"webp.alpha"},{uri:"data:image/webp;base64,UklGRlIAAABXRUJQVlA4WAoAAAASAAAAAAAAAAAAQU5JTQYAAAD/////AABBTk1GJgAAAAAAAAAAAAAAAAAAAGQAAABWUDhMDQAAAC8AAAAQBxAREYiI/gcA",
...
a==virtualclass.apps.ss&&this.system.setAppDimension();virtualclass.vutil.createReclaimButtonIfNeed();virtualclass.user.assignRole(virtualclass.gObj.uRole,a);this.media=new d.media;a=localStorage.getItem("precheck");null!=a&&(a=JSON.parse(a));virtualclass.makePreCheckAvailable?virtualclass.precheck.init():(virtualclass.makeReadySocket(),virtualclass.isPlayMode||virtualclass.gesture.initClassJoin());b.classList.remove("loading");virtualclass.gObj.precheckScrn=!1;virtualclass.gObj.meetingMode||virtualclass.videoHost.init(320,
240);virtualclass.vutil.videoController();a=document.getElementById("precheckBtn");a.addEventListener("click",()=>{virtualclass.gObj.precheckScrn=!0;virtualclass.precheck.init(virtualclass.precheck)});this.raiseHand=d.raiseHand;this.raiseHand.init();this.appSetting=d.appSetting;this.appSetting.init();virtualclass.colorSelector=d.colorSelector;virtualclassSetting.theme.selectedColor&&this.colorSelector.makeThemeReady();a=document.getElementById("virtualclassCont");null!=a&&a.classList.add(virtualclass.system.mybrowser.name);
a=document.getElementById("fullScreenButton");null!=a&&a.addEventListener("click",virtualclass.vutil.Fullscreen);a=document.getElementById("fullScreenExitButton");null!=a&&a.addEventListener("click",virtualclass.vutil.closeFullscreen);document.onfullscreenchange=function(b){virtualclass.gObj.fullScreenMode?(document.querySelector("#fullScreenButton").style.display="block",document.querySelector("#fullScreenExitButton").style.display="none",virtualclass.gObj.fullScreenMode=!1):(document.querySelector("#fullScreenButton").style.display=
"none",document.querySelector("#fullScreenExitButton").style.display="block",virtualclass.gObj.fullScreenMode=!0)}},makeReadySocket(){virtualclass.vutil.isPlayMode()||this.initSocketConn()},initSocketConn(){this.system.webSocket&&(io.init(virtualclass.uInfo),d.userdata=virtualclass.uInfo)},html:{id:"virtualclassCont",optionsClass:"appOptions",init(b){this.virtualclass=b},leftAppBar(){if(!(0<document.getElementsByClassName("appOptions").length)){document.querySelector("#virtualclassApp #virtualclassAppLeftPanel");
...
c.querySelector(`.bulkUserActions #contr${b}AllImg`);var k=virtualclass.user.defaultSettings(a,virtualclass.settings.info);const m=c.querySelector(`.bulkUserActions #contr${b}All input`),n=c.querySelector(`.congrea #contr${b}All`),q="enable"===k?"disable":"enable";null!=k?(d.setAttribute("data-action",q),d.className=`slider round icon-all-${a}-${q} enable congtooltip cgIcon`,d.dataset.title=virtualclass.lang.getString(`${q}All${b}`),m.setAttribute("checked","true"),n.classList.add(q)):(k={action:k?
"enable":"disable",enable:"disable",disable:"enable"},d=c.querySelector(`.bulkUserActions #contr${b}AllImg`),d.setAttribute("data-action",k[k.action]),d.className=`slider round icon-all-${a}-${k[k.action]} congtooltip cgIcon`,d.dataset.title=virtualclass.lang.getString(`${k[k.action]}All${b}`),m.removeAttribute("checked"),n.classList.add(k[k.action]));if(virtualclass.isPlayMode)anchorTag.pointerEvents="none",anchorTag.style.cursor="default";else{const b=this;d.addEventListener("click",()=>{let c;
"audio"===a?(c=b.toogleAudioIcon(),virtualclass.settings.applySettings("enable"===c,"studentaudio"),"undefined"!==typeof c&&(localStorage.setItem("allAudAction",c),b.toggleAllAudio.call(virtualclass.user,c))):(c=b.toggleVideoIcon(),virtualclass.settings.applySettings("enable"===c,"studentvideo"),"undefined"!==typeof c&&(localStorage.setItem("allVideoAction",c),b.toggleAllVideo(c)))})}},defaultSettings(a,c){let b,f;for(const d in c)if(f=c[d],"audio"===a&&"studentaudio"===d||"video"===a&&"studentvideo"===
d)b=!0===f?"enable":"disable";return b},chatBoxesSwitch(){if(virtualclass.chat.chatroombox)toggleCommonChatBox();else if(0==$("div#chat_room").length){var a=c.createElement("ul");a.id="chat_room";c.body.appendChild(a);virtualclass.chat.chatroombox=$("#chat_room").chatroom({id:"chat_room",user:{name:"test"},title:lang.chatroom_header,offset:"20px",messageSent(a,b){$("#chat_room").chatroom("option","boxManager").addMsg(a.name,b)}});virtualclass.gObj.hasOwnProperty("chatEnable")&&!virtualclass.gObj.chatEnable&&
...
return b}},changeRoleOnFooter(a,c){chatContainerEvent.elementFromShadowDom(`#ml${a}`).dataset.role=c},initControlHandler(a){virtualclass.vutil.userIsOrginalTeacher(a);const b=chatContainerEvent.elementFromShadowDom(`#ml${a} .contImg`,"all");let d=!1,k=localStorage.getItem(`virtualclass${a}`);null!=k&&(d=!0,k=JSON.parse(k),k.hasOwnProperty("currTeacher")&&(virtualclass.gObj[`${a}currTeacher`]={},1==k.currTeacher?(virtualclass.user.control.currTeacherAlready=!0,virtualclass.gObj[`${a}currTeacher`].ct=
!0):virtualclass.gObj[`${a}currTeacher`].ct=!1));for(let f=0;f<b.length;f++)if(-1<b[f].className.indexOf("chat")){var m=d&&k.hasOwnProperty("chat")?virtualclass.settings.user.hasOwnProperty(a)?virtualclass.settings.parseSettings(virtualclass.settings.user[a]).studentpc:virtualclass.settings.info.studentpc:virtualclass.settings.info.studentpc;virtualclass.user.control.changeAttribute(a,b[f],m,"chat","chat")}else-1<b[f].className.indexOf("aud")?(d&&k.hasOwnProperty("aud")?m=virtualclass.settings.user.hasOwnProperty(a)?
virtualclass.settings.parseSettings(virtualclass.settings.user[a]).studentaudio:virtualclass.settings.info.studentaudio:(m=c.querySelector("#contrAudioAll"),null!=virtualclass.jId&&m.classList.contains("disable"),m=virtualclass.settings.info.studentaudio),virtualclass.user.control.changeAttribute(a,b[f],m,"audio","aud"),roles.hasAdmin()&&"false"==b[f].dataset.audioDisable&&localStorage.getItem("allAudAction")):-1<b[f].className.indexOf("RaiseHand")?(m=d&&k.hasOwnProperty("raiseHand")?!!k.raiseHand:
!1,virtualclass.user.control.changeAttribute(a,b[f],m,"RaiseHand","RaiseHand")):-1<b[f].className.indexOf("stdscreen")?virtualclass.gObj.studentSSstatus.hasOwnProperty("whoIsSharing")&&virtualclass.vutil.initssSharing(virtualclass.gObj.whoIsSharing):-1<b[f].className.indexOf("editorRich")&&"EditorRich"==virtualclass.currApp?(c.querySelector("#alleditorRichContainerAnch"),m=d&&k.hasOwnProperty("editorRich")?!!k.editorRich:!1,virtualclass.user.control.changeAttribute(a,b[f],m,"editorRich","editorRich")):
-1<b[f].className.indexOf("editorCode")&&(m=d&&k.hasOwnProperty("editorCode")?!!k.editorCode:!1,virtualclass.user.control.changeAttribute(a,b[f],m,"editorCode","editorCode"))}}}})(window,document);(function(d){d.xhr={init(){this.httpObj=d.XMLHttpRequest?new XMLHttpRequest:new ActiveXObject("Microsoft.XMLHTTP");this.httpObj.upload.addEventListener("progress",this.onProgress);this.onReadStateChange();this.httpObj.onerror=function(c){console.log(`Error ${c}`)};this.httpObj.withCredentials=!0;this.httpObj.onabort=function(c){console.log(`Error abort ${c}`)}},onProgress(c){virtualclass.pbar.currVal=c.loaded;""!=virtualclass.pbar.prvVal&&"undefined"!==typeof virtualclass.pbar.prvVal||virtualclass.isPlayMode||
...
this.congController=function(d){if(d.fromUser.userid!=virtualclass.gObj.uid)virtualclass.videoHost.onmessage(d.message);"off"==d.message.congCtr.videoSwitch?(virtualclass.vutil.addClass("virtualclassCont","videoff"),virtualclass.vutil.addClass("virtualclassAppRightPanel","hide"),virtualclass.vutil.removeClass("virtualclassAppRightPanel","show"),virtualclass.videoHost.setUserIcon(d.fromUser.userid)):(document.querySelector("#virtualclassCont").classList.remove("videoff"),virtualclass.vutil.addClass("virtualclassAppRightPanel",
...
delete virtualclass.videoUl.player}};this.sync=function(){};this.recs=function(d){if(!virtualclass.isPlayMode)virtualclass.settings.onMessage(d.message)}};function Config(){}Config.prototype.setNewSession=function(d){localStorage.setItem("mySession",d);!virtualclass.isPlayMode&&roles.hasControls()&&(d=(new Date).getTime(),d=JSON.stringify({createdDate:d}),localStorage.setItem("myConfig",d))};
