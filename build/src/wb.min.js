...
virtualclass.currApp&&0<virtualclass.serverData.rawData.ppt.length&&virtualclass.sharePt.awsPresentationList(virtualclass.serverData.rawData.ppt);virtualclass.vutil.requestOrder(()=>{})});b.notifyAboutCPU();virtualclass.isPlayMode&&virtualclass.settings.triggerSettings()}})(window);"null"!==window.location.search&&"mobTab"==window.system.device&&"Chrome"==window.system.mybrowser.name&&(document.querySelector("html").classList.add("pwaSupported"),window.addEventListener("beforeinstallprompt",c=>{console.log("\ud83d\udc4d","beforeinstallprompt",c);c.preventDefault();window.deferredPrompt=c}));
window.addEventListener("load",()=>{navigator.fullscreen?(console.log("Launched: Installed (iOS)"),document.querySelector("html").classList.add("pwaRunning")):matchMedia("(display-mode: fullscreen)").matches?(console.log("Launched: Installed"),document.querySelector("html").classList.add("pwaRunning")):console.log("Launched: Browser Tab")});
//# sourceMappingURL=https://live.congrea.net/mobile/virtualclass/build/src/wb-map
