...
102!=this.msg[0]&&202!=this.msg[0]||(io.onRecMessage(this.convertInto({data:this.msg})),this.binarySyncMsg=null)}this.collectElapsedPlayTime()}catch(f){}this.subRecordingIndex++}this.elapsedRecTime=this.elapsedTime=this.elapsedPlayTime;if(void 0!=c&&this.masterIndex===c.master&&c.sub===this.subRecordingIndex)break;else this.subRecordingIndex=0,this.masterIndex++,this.subRecordings=this.masterRecordings[this.masterIndex]}},handleSyncStringPacket(){if("Poll"===virtualclass.currApp&&"object"===typeof virtualclass.poll.pollState.data&&
virtualclass.poll.hasOwnProperty("recordStartTime")){var c=this.getTotalTimeInMilSeconds(virtualclass.poll.recordStartTime.data.masterIndex,virtualclass.poll.recordStartTime.data.subIndex);virtualclass.poll.dataRec.setting.timer?this.pollUpdateTime(c,virtualclass.poll.pollState):(c=this.convertIntoReadable(this.elapsedPlayTime-c),virtualclass.poll.dataRec.newTime.min=c.m,virtualclass.poll.dataRec.newTime.sec=c.s,virtualclass.poll.elapsedTimer())}else if("Video"===virtualclass.currApp&&"object"===
typeof virtualclass.videoUl&&virtualclass.videoUl.hasOwnProperty("videoStartTime"))c=this.getTotalTimeInMilSeconds(virtualclass.videoUl.videoStartTime.data.masterIndex,virtualclass.videoUl.videoStartTime.data.subIndex),c=(this.elapsedPlayTime-c)/1E3,"object"===typeof virtualclass.videoUl.player?virtualclass.videoUl.playVideo(c):(console.log("====Video init to play start"),virtualclass.videoUl.isPaused=this.pauseBeforeSeek?!0:!1,virtualclass.videoUl.startTime=c);else if("Quiz"===virtualclass.currApp&&
"object"===typeof virtualclass.quiz){c=document.querySelector("#qztime");var a=this.getTotalTimeInMilSeconds(virtualclass.quiz.quizStartTime.data.masterIndex,virtualclass.quiz.quizStartTime.data.subIndex);0<+virtualclass.quiz.plugin.config.quizTime?(a=1E3*virtualclass.quiz.plugin.config.quizTime-(this.elapsedPlayTime-a),virtualclass.quiz.plugin.method.startTimer(a/1E3,c,"desc","vmQuiz")):(a=(this.elapsedPlayTime-a)/1E3,virtualclass.quiz.plugin.method.startTimer(a,c,"asc","vmQuiz"))}},pollUpdateTime(c,
a){c=this.convertIntoReadable(6E4*a.data.stdPoll.newTime.min+1E3*a.data.stdPoll.newTime.sec-(this.elapsedPlayTime-c));virtualclass.poll.newTimer.sec=c.s;virtualclass.poll.newTimer.min=c.m;virtualclass.poll.showTimer(virtualclass.poll.newTimer)},handleSyncPacket(){if(this.binarySyncMsg){console.log("Get full screen share");var c=this.binarySyncMsg.data.subIndex;let a=this.binarySyncMsg.data.masterIndex;for(;a<=this.masterIndex;){console.log(`Start from master index ${a} from Subindex ${c}`);const b=
this.masterRecordings[a];let f;for(f=this.masterIndex!=a?this.masterRecordings[this.masterIndex].length:this.subRecordingIndex;c<=f;c++){this.msg=b[c];try{if(null!=this.msg&&"B"==this.msg.type){if(this.msg=this.msg.recObjs,104==this.msg[0]||204==this.msg[0]||103==this.msg[0]||203==this.msg[0]||102==this.msg[0]||202==this.msg[0])io.onRecMessage(this.convertInto({data:this.msg}))}else console.log("Either this.msg is null or string")}catch(g){console.log(`PLAY ERROR ${g.errorCode}`)}}if(this.masterIndex==
