...
0;class r extends AudioWorkletProcessor{constructor(){super();this.port.onmessage=d}process(c,g){console.log("Audio is playing");q=n.length=0;for(k in a){b:{c=k;if(null!==a){if(288<=a[c].length){for(;96<=a[c].length;)a[c].shift();b[c]=!0;c=a[c].shift();break b}if(64<=a[c].length){b[c]=!0;c=a[c].shift();break b}if(0<a[c].length&&!0===b[c]){b[c]=!0;c=a[c].shift();break b}b[c]=!1;0===a[c].length&&delete a[c]}c=void 0}if(null!=c)if(q++,1===q)for(var d=0;128>d;d++)n[d]=c[d];else for(d=0;128>d;d++)n[d]+=
c[d]}if(1===q)c=n;else if(1<q){for(c=0;128>c;c++)n[c]/=q;c=n}else c=void 0;if(null!=c)g[0][0].set(c,0),f=c[127];else if(null!==f)for(c=0;128>c;c++)g[0][0][c]=f;return!0}}registerProcessor("worklet-audio-rec",r)}.toString(),")()"],{type:"application/javascript"}));/*
  2018 Suman Bogati  {@link http://vidyamantra.com}
 @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
v=!0;virtualclass.gObj.audioRecWorkerReady=!0}},initPlay(){this.hasOwnProperty("Html5Audio")||this.Html5Audio||(this.Html5Audio={audioContext:new (d.AudioContext||d.webkitAudioContext)});"object"!==typeof u&&g.audio.Html5Audio.audioContext.audioWorklet.addModule(workletAudioRecBlob).then(()=>{if("undefined"===typeof p){u=new AudioWorkletNode(g.audio.Html5Audio.audioContext,"worklet-audio-rec");g.audio.Html5Audio.MediaStreamDest=g.audio.Html5Audio.audioContext.createMediaStreamDestination();u.connect(g.audio.Html5Audio.audioContext.destination);
"Chrome"===virtualclass.system.mybrowser.name&&(console.log("==== Chrome after change"),g.audio.bug_687574_callLocalPeers());var a=new MessageChannel;workerIO.postMessage({cmd:"workerAudioRec"},[a.port1]);workerAudioRec.postMessage({cmd:"workerIO",sampleRate:g.audio.Html5Audio.audioContext.sampleRate},[a.port2]);a=new MessageChannel;workerAudioRec.postMessage({cmd:"workletAudioRec"},[a.port1]);u.port.postMessage({cmd:"workerAudioRec"},[a.port2]);workerAudioRec.postMessage({cmd:"audioWorklet",msg:!0});
p=!0;virtualclass.gObj.audioRecWorkerReady=!0}})},playWithFallback(a){"suspended"===this.Html5Audio.audioContext.state?(this.hasOwnProperty("audioSuspendTime")&&clearTimeout(this.audioSuspendTime),this.audioSuspendTime=setTimeout(()=>{"suspended"===this.Html5Audio.audioContext.state&&(this.snode.push(a),null===virtualclass.gObj.requestToScriptNode&&(this.Html5Audio.audioContext.resume(),virtualclass.gesture.initAudioResume(a),virtualclass.gObj.requestToScriptNode=!0))},2E3)):this._playWithFallback()},
_playWithFallback(){const a=this;if(null===virtualclass.media.audioPlayerNode||"closed"===virtualclass.media.audioPlayerNode.context.state)console.log("script processor node is created"),null!==virtualclass.media.audioPlayerNode&&virtualclass.media.audioPlayerNode.disconnect(),virtualclass.media.audioPlayerNode=this.Html5Audio.audioContext.createScriptProcessor(4096,1,1),B=0,virtualclass.media.audioPlayerNode.onaudioprocess=function(b){b=b.outputBuffer.getChannelData(0);const c=a.getMergedAudio();
if(null!==c&&void 0!==c){for(var f=0;f<c.length;f++)b[f]=c[f];B=c[4095]}else for(f=0;f<b.length;f++)b[f]=B},virtualclass.media.audioPlayerNode.connect(this.Html5Audio.audioContext.destination)},queueWithFalback(a,b){null==k[b]&&(k[b]=[]);null==n[b]&&(n[b]=[]);for(let c=0;c<a.length;c++)n[b].push(a[c]);for(;4096<=n[b].length;)a=n[b].splice(0,4096),k[b].push(new Float32Array(a))},getMergedAudio(){r=[];z=0;for(q in k){var a=this.getAudioChunks(q);if(null!=a)if(z++,1===z)for(var b=0;4096>b;b++)r[b]=a[b];
else for(b=0;4096>b;b++)r[b]+=a[b]}if(1===z)return r;if(1<z){for(a=0;4096>a;a++)r[a]/=z;return r}},getAudioChunks(a){if(null!==k){if(9<=k[a].length){for(;3<=k[a].length;)k[a].shift();m[a]=!0;return k[a].shift()}if(2<=k[a].length||0<k[a].length&&!0===m[a])return m[a]=!0,k[a].shift();m[a]=!1;0===k[a].length&&delete k[a]}},_maniPulateStream(){console.log("Manipulate stream");this.triggermaniPulateStream=!0;const a=virtualclass.media;setTimeout(()=>{a.detectAudioWorklet()?a.audio.maniPulateStream():a.audio.maniPulateStreamWithFallback()},
1E3)},maniPulateStream(){const {stream:a}=g;"undefined"!==typeof t&&t.disconnect();"undefined"!==typeof a&&null!=a&&(console.log("Audio worklet init add module"),g.audio.Html5Audio.audioContext.audioWorklet.addModule(workletAudioSendBlob).then(()=>{console.log("== init audio worklet 3");var b=g.audio.Html5Audio.audioContext.createMediaStreamSource(a);const f=g.audio.Html5Audio.audioContext.createBiquadFilter();f.type="lowpass";f.frequency.value=2E3;b.connect(f);t=new AudioWorkletNode(g.audio.Html5Audio.audioContext,
"worklet-audio-send");t.onprocessorerror=function(a){g.audio.notifiyMuteAudio()};f.connect(t);t.connect(g.audio.Html5Audio.audioContext.destination);b=new MessageChannel;workerAudioSend.postMessage({cmd:"workerIO",sampleRate:this.Html5Audio.audioContext.sampleRate,uid:virtualclass.gObj.uid},[b.port1]);workerIO.postMessage({cmd:"workerAudioSend"},[b.port2]);b=new MessageChannel;workerAudioSend.postMessage({cmd:"audioWorkletSend",msg:{repMode:c}},[b.port1]);t.port.postMessage({cmd:"workerAudioSend"},
[b.port2]);g.workerAudioSendOnmessage();console.log("Audio worklet ready audio worklet module")}).catch(a=>{g.audio.notifiyMuteAudio()}))},maniPulateStreamWithFallback(){if("undefined"!==typeof g.stream&&null!=g.stream){var {stream:a}=g;a=g.audio.Html5Audio.audioContext.createMediaStreamSource(a);g.audio.bufferSize=4096;virtualclass.media.audioCreatorNode=g.audio.Html5Audio.audioContext.createScriptProcessor(g.audio.bufferSize,1,1);virtualclass.media.audioCreatorNode.onaudioprocess=g.audio.recorderProcessFallback.bind(g.audio);
const b=g.audio.Html5Audio.audioContext.createBiquadFilter();b.type="lowpass";b.frequency.value=2E3;a.connect(b);b.connect(virtualclass.media.audioCreatorNode);virtualclass.media.audioCreatorNode.connect(g.audio.Html5Audio.audioContext.destination);a=new MessageChannel;workerAudioSend.postMessage({cmd:"workerIO",uid:virtualclass.gObj.uid,sampleRate:g.audio.Html5Audio.audioContext.sampleRate},[a.port1]);workerIO.postMessage({cmd:"workerAudioSend"},[a.port2]);g.workerAudioSendOnmessage()}else console.log("No stream is found")},
updateInfo(){this.audioStreamArr=[];c=this.recordAudio=!1},extractData(a){a=new Int8Array(a);return[virtualclass.vutil.numValidateFour(a[1],a[2],a[3],a[4]),a.subarray(5,a.length)]},removeAudioFromLocalStorage(){console.log("Remove audio from local storage");localStorage.removeItem("audEnable")},async bug_687574_callLocalPeers(){function a(a,b){if("connected"===b.currentTarget.connectionState)try{console.log("PEER connected webrtc"),u.disconnect(g.audio.Html5Audio.audioContext.destination),u.connect(g.audio.Html5Audio.MediaStreamDest)}catch(N){}else if("disconnected"===
b.currentTarget.connectionState){console.log("PEER disconnected");k.close();m.close();m=k=null;try{u.disconnect(g.audio.Html5Audio.MediaStreamDest),u.connect(g.audio.Html5Audio.audioContext.destination),console.log("PEER connected normal audio api")}catch(N){}g.audio.bug_687574_callLocalPeers()}}async function b(a){try{await k.setLocalDescription(a),await m.setRemoteDescription(a)}catch(M){d()}try{const a=await m.createAnswer();await c(a)}catch(M){d()}}async function c(a){try{await m.setLocalDescription(a),
await k.setRemoteDescription(a)}catch(M){d()}}async function f(a,b){if(b.candidate&&"host"===b.candidate.type)try{await (a===k?m:k).addIceCandidate(b.candidate)}catch(N){d()}}function d(){console.log("PEER fallback");try{u.connect(g.audio.Html5Audio.audioContext.destination),k.close(),m.close(),m=k=null}catch(O){m=k=null}}let k,m;k=new RTCPeerConnection;k.count=0;k.addEventListener("icecandidate",a=>f(k,a));k.addEventListener("connectionstatechange",b=>a(k,b));m=new RTCPeerConnection;m.count=0;m.addEventListener("icecandidate",
...
null!=this.videoCont&&(this.videoCont.style.maxHeight=`${this.maxHeight}px`)},calcDimension(){this.myVideo.width=this.width;this.myVideo.height=this.height},removeUser(a){a=document.getElementById(`user${a}`);null!=a&&a.parentNode.removeChild(a)},createElement(a){const c=document.createElement("div");c.className="videoWrapper";const f=document.createElement("div");f.className="videoSubWrapper";f.id=`user${a.id}`;c.appendChild(f);b=a.id;const g=document.createElement("canvas");g.id=`video${b}`;g.className=
"userVideos";g.width=this.width;g.height=this.height;g.muted="muted";f.appendChild(g);virtualclass.media.util.imageReplaceWithVideo(a.id,c)},updateHightInSideBar(a){const b=document.getElementById("widgetRightSide").offsetHeight;document.getElementById("chatContainer").style.height=`${b-a}px`},sendInBinary(a){const b={name:virtualclass.gObj.uName,id:virtualclass.gObj.uid};io.webSocketConnected()&&(virtualclass.vutil.beforeSend({videoByImage:b,cf:"videoByImage"},null,!0),ioAdapter.sendBinary(a))},
send(){function a(){b.tempVidCont.clearRect(0,0,b.tempVid.width,b.tempVid.height);b.tempVidCont.drawImage(b.myVideo,0,0,b.width,b.height);roles.hasControls();if(virtualclass.system.webpSupport){var d=b.tempVid.toDataURL("image/webp",.6);var k=1}else d=b.tempVid.toDataURL("image/jpeg",.3),k=0;d=virtualclass.videoHost.convertDataURIToBinary(d);if(!virtualclass.videoHost.gObj.stdStopSmallVid&&!roles.hasControls()||roles.hasControls()&&virtualclass.videoHost.gObj.videoSwitch){var m=virtualclass.gObj.uid.toString();
for(let a=m.length;8>a;a++)m=`0${m}`;m=m.match(/[\S]{1,2}/g)||[];k=new Uint8ClampedArray([11,m[0],m[1],m[2],m[3],k]);m=new Uint8ClampedArray(d.length+k.length);m.set(k);m.set(d,k.length);g.sendInBinary(m)}clearInterval(virtualclass.media.smallVid);if(virtualclass.hasOwnProperty("connectedUsers"))if(d=f+2500*virtualclass.connectedUsers.length,c!=virtualclass.connectedUsers.length){c=virtualclass.connectedUsers.length;k=-1;for(m=0;m<virtualclass.connectedUsers.length;m++)virtualclass.connectedUsers[0].userid==
virtualclass.gObj.uid&&(k=m);m=d/c;0>k&&(k=0);virtualclass.media.smallVid=setInterval(a,d+k*m)}else virtualclass.media.smallVid=setInterval(a,d)}virtualclass.media.hasOwnProperty("smallVid")&&clearInterval(virtualclass.media.smallVid);const b=this;f=Math.floor(5001*Math.random()+3E3);let c=-1;const g=this;virtualclass.media.smallVid=setInterval(a,f)},startToStream(){g.video.calcDimension();g.video.send()},playWithoutSlice(a,b,c){this.remoteVid=document.getElementById(`video${a}`);null!=this.remoteVid&&
...
{};for(const a in m.attributes){var B=m.attributes[a];const b=f[d].attributes[a];!1===B?b&&(v[a]=b):B!==b&&(v[a]=b||!1)}n.retain(f[d].length,v);b+=f[d].length}else if(m.isInsert())n.delete(m.text.length);else if(m.isDelete()){v=c.getRange(c.posFromIndex(b),c.posFromIndex(b+m.chars));f=this.rtcm.getAttributeSpans(b,b+m.chars);for(d=B=0;d<f.length;d++)n.insert(v.substr(B,f[d].length),f[d].attributes),B+=f[d].length;b+=m.chars}}return new m(n,a.meta.invert())};return a}();d.Client=c})(window);const otAdapter=function(){function d(c,a){a=a.charAt(0).toLowerCase()+a.slice(1);const b=Vceditor.getvcEditor();c.operations&&c.revision>c.operations.length?(a=[],a[c.revision-c.operations.length-1]=null,this.operations=a.concat(c.operations)):this.operations=c.operations?c.operations:[];this.server=new b.Server(c.doc,this.operations);this.trigger=function(a){this.callbacks[a].apply(this,Array.prototype.slice.call(arguments,1))};this.teacherOT=function(a){a=this.doOT(a);"undefined"!==typeof a&&this.preSend(a,
...
b,c,f,g){const k=[];let m=a;const v=this;this.annotationList_.updateSpan(new n(a,b-a),(a,b)=>{const n={};for(var C in a.attributes)n[C]=a.attributes[C];n[r.LINE_SENTINEL]&&!g||c(n);C={};const q={};v.computeChangedAttributes_(a.attributes,n,C,q);a:{for(const b in C){a=!1;break a}a=!0}a||k.push({start:m,end:m+b,attributes:C,attributesInverse:q,origin:f});m+=b;return new d(n)});0<k.length&&this.trigger("attributesChange",this,k)};a.prototype.computeChangedAttributes_=function(a,b,c,f){const d={};let g;
