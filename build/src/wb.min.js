...
a.data.status&&c.sendOrder(c.order)});b=!1;const d=virtualclass.dts.allDocs[a];if(null!=d)for(let a=0;a<d.notesarr.length;a++)if(!d.notesarr[a].hasOwnProperty("deletedn")){b=!0;break}b||this._delete(a);roles.hasControls()&&this.indexNav.createIndex()},removePagesFromStructure(a){this.allDocs[a].deleted="0";let b;for(b in this.allNotes)-1<this.allNotes[b].id.indexOf(a)&&this._removePageFromStructure(this.allNotes[b].id)},updateInAllDocs(a){var b=a.split("_")[0];b=this.allDocs[b];if("undefined"!==typeof b.notes[a]){b.notes[a].deletedn=
a;for(let c=0;c<b.notesarr.length;c++)b.notesarr[c].id==a&&(b.notesarr[c].deletedn=a)}else console.log(`The note with the id ${a} is not available`)},_removePageFromStructure(a){this.removeWhiteboardFromStorage(`_doc_${a}_${a}`);this.allNotes[a].deletedn=a;this.storeInDocs(this.allNotes);this.updateInAllDocs(a)},_disable(a){this.docStatus(a)},_enable(a){this.docStatus(a)},docStatus(a,b){var c=document.querySelector(`#linkdocs${a} .controls.status`);null!=c?"undefined"===typeof b&&(b=1-+c.dataset.status):
console.log(`document share:- there is no element ${a}`);c=this.getDocs(a);for(let a=0;a<c.length;a++){const d=c[a].id;this.noteStatus(d,b);this.updatePageNavStatus(d,b)}this.storeInDocs(this.allNotes);roles.hasControls()&&ioAdapter.mustSend({dts:{docSt:b,doc:a},cf:"dts"})},updatePageNavStatus(a,b){const c=document.querySelector(`#linknotes${a}`);null!=c?(c.dataset.status=b,a=c.querySelector(".controls.status"),a.dataset.status=b,a.querySelector(".statusanch").innerHTML=`status${b}`):console.log(`${"Document share : there is no element #linknotes"}${a}`)},
_noteDisable(a){this.noteStatus(a);this.storeInDocs(this.allNotes);this.sendNoteStatus(a)},_noteEnable(a){this.noteStatus(a);this.storeInDocs(this.allNotes);this.sendNoteStatus(a)},sendNoteStatus(a){if(roles.hasControls()){const b=document.querySelector(`#note${a}`);null!=b?ioAdapter.mustSend({dts:{noteSt:b.dataset.status,note:a},cf:"dts"}):console.log("Element is null")}},noteStatus(a,b){const c=document.querySelector(`#note${a}`);null!=c?(b="undefined"===typeof b?1-+c.dataset.status:1==b||"true"==
b?1:0,c.dataset.status=b,b=this.allNotes[a],b.status=c.dataset.status,this.allNotes[a]=b):console.log(`there is no element #note${a}`)},getDocsOld(a){const b=[];for(const c in this.allNotes)a==this.allNotes[c].lc_content_id&&b.push(this.allNotes[c]);return b},getDocs(a){console.log("--------------");if(null!=this.allDocs&&"object"===typeof this.allDocs[a])return this.allDocs[a].notesarr;console.log(`There is no document with the id ${a}`)},getDocId(a){return a.split("_")[0]},getAllNotes(a){const b=
[];for(let c=0;c<a.length;c++)b.push(this.allNotes[a[c]]);return b},getNote(a){return this.allNotes[a]},updateLinkNotes(a){const b=document.querySelector("#listnotes .currentNav");null!=b&&b.classList.remove("currentNav");this.setCurrentNav(a)},setCurrentNav(a){a=document.querySelector(`#linknotes${a}`);null!=a&&a.classList.add("currentNav")},onAjaxResponse(a,b,c){if(c.hasOwnProperty("success")&&c.success){for(a=0;a<virtualclass.gObj.uploadingFiles.length;a++)this.afterUploadFile(virtualclass.gObj.uploadingFiles[a].uuid);
...
this.axhr.get(b.pdf).then(a=>{virtualclass.pdfRender[virtualclass.gObj.currWb].afterPdfPrefetch(b.id,a.data)}).catch(a=>{console.error("Request failed with error ",a)})},afterPdfPrefetch(a,b){virtualclass.gObj.next={};virtualclass.gObj.next[a]=b},async loadPdf(a,b,c){virtualclass.gObj.hasOwnProperty("getDocumentTimeout")&&clearTimeout(virtualclass.gObj.getDocumentTimeout);null==virtualclass.gObj.getDocumentTimer||!1===virtualclass.gObj.getDocumentTimer?(this._loadPdf(a,b,c),virtualclass.gObj.getDocumentTimer=
!0,virtualclass.gObj.getDocumentTimeout=setTimeout(()=>{virtualclass.gObj.getDocumentTimer=!1},1E3)):virtualclass.gObj.getDocumentTimeout=setTimeout(()=>{this._loadPdf(a,b,c);virtualclass.gObj.getDocumentTimer=!1},300)},_loadPdf(a,b,c){virtualclass.gObj.next.hasOwnProperty(c)?this.afterPdfLoad(b,c,virtualclass.gObj.next[c]):this.axhr.get(a).then(a=>{virtualclass.pdfRender[virtualclass.gObj.currWb].afterPdfLoad(b,c,a.data)}).catch(d=>{console.error("Request failed with error ",d);setTimeout(()=>{virtualclass.pdfRender[virtualclass.gObj.currWb]._loadPdf(a,
