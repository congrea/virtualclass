...
!0),localStorage.removeItem("nEd")):console.log("Editor mode disable");console.log(`New role before clear ${virtualclass.gObj.uRole}`);virtualclass.settings.user={};document.querySelector("#virtualclassWhiteboard").style.display="none";a=document.querySelector("#virtualclassCont");null!=a&&a.classList.remove("loading");if(virtualclass.isPlayMode){c.config.createNewSession();virtualclass.videoHost&&roles.isStudent()&&!virtualclass.isPlayMode&&(virtualclass.settings.init(),virtualclass.settings.userAudioIcon(),
virtualclass.settings.userVideoIcon(),a=document.querySelector("#virtualclassAppRightPanel"),a.classList.contains("vidHide")||a.classList.add("vidHide"));console.log(`New role after clear ${virtualclass.gObj.uRole}`);virtualclass.popup.waitMsg();"object"===typeof virtualclass.dts&&null!=virtualclass.dts&&virtualclass.dts.destroyDts();if("object"===typeof virtualclass.raiseHand&&null!=virtualclass.raiseHand)if(roles.hasControls())virtualclass.raiseHand.rhCount=0,virtualclass.raiseHand.rhCountR=0,(a=
document.querySelector(".congrea .vmchat_bar_button .hand_bt.highlight"))&&a.classList.remove("highlight"),document.querySelector(".congrea .vmchat_bar_button .hand_bt #notifyText").innerHTML="";else if(a=document.querySelector(".congrea .handRaise.disable"))a.classList.remove("disable"),a.classList.add("enable"),a.setAttribute("data-title",virtualclass.lang.getString("RaiseHandStdEnabled")),document.querySelector(".congrea .handRaise #icHr").setAttribute("data-action","enable"),virtualclass.raiseHand.stdRhEnable=
"enabled";(a=chatContainerEvent.elementFromShadowDom(".vmchat_room_bt.ui-state-highlight"))&&a.classList.remove("ui-state-highlight");(a=document.querySelector("#virtualclassCont.congrea.student"))&&a.classList.contains("videoff")&&a.classList.remove("videoff");a=document.querySelector("#virtualclassCont #memlist");b=document.querySelector("#virtualclassCont #chatrm");f=document.querySelector("#user_list");var g=document.querySelector("#chatroom_bt2");a&&!a.classList.contains("enable")&&(a.classList.add("enable"),
...
virtualclass.gObj.currIndex=1}else virtualclass.endSession=!0,virtualclass.popup.sesseionEndWindow()}},dstdata:{async handleResult(b){for(;b;)b.value.hasOwnProperty("alldocs")&&(console.log("document share store"),a=!0,virtualclass.gObj.docs=JSON.parse(b.value.alldocs)),b=await b.continue();a||(virtualclass.gObj.docs="init",console.log("==== Docs init "))}},dstAllStore(a){const b=this.db.transaction("dstall","readwrite");b.store.clear();const c=JSON.stringify(virtualclass.dts.allNotes);b.store.add({dstalldocs:JSON.stringify(a),
allNotes:c,timeStamp:(new Date).getTime(),id:10});b.done.then(()=>{console.log("success")},()=>{console.log("failure")})},dstall:{async handleResult(a){for(;a;)a.value.hasOwnProperty("dstalldocs")&&(console.log("document share store"),b=!0,virtualclass.gObj.dstAll=JSON.parse(a.value.dstalldocs),virtualclass.gObj.dstAllNotes=JSON.parse(a.value.allNotes)),a=await a.continue();b||(console.log("document share store init"),virtualclass.gObj.dstAll="init")}},clearSingleTable(a,b){this.db.transaction(a,
