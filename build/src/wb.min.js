...
subIndex:this.subRecordingIndex}};"ScreenShare"!=virtualclass.currApp||104!=this.msg[0]&&204!=this.msg[0]&&103!=this.msg[0]&&203!=this.msg[0]&&102!=this.msg[0]&&202!=this.msg[0]||(io.onRecMessage(this.convertInto({data:this.msg})),this.binarySyncMsg=null)}this.collectElapsedPlayTime()}catch(f){}this.subRecordingIndex++}this.elapsedRecTime=this.elapsedTime=this.elapsedPlayTime;if(void 0!=c&&this.masterIndex===c.master&&c.sub===this.subRecordingIndex)break;else this.subRecordingIndex=0,this.masterIndex++,
this.subRecordings=this.masterRecordings[this.masterIndex]}},handleSyncStringPacket(){if("Poll"===virtualclass.currApp&&"object"===typeof virtualclass.poll.pollState.data&&virtualclass.poll.hasOwnProperty("recordStartTime")){var c=this.getTotalTimeInMilSeconds(virtualclass.poll.recordStartTime.data.masterIndex,virtualclass.poll.recordStartTime.data.subIndex);virtualclass.poll.dataRec.setting.timer?this.pollUpdateTime(c,virtualclass.poll.pollState):(c=this.convertIntoReadable(this.elapsedPlayTime-
c),virtualclass.poll.dataRec.newTime.min=c.m,virtualclass.poll.dataRec.newTime.sec=c.s,virtualclass.poll.elapsedTimer())}else if("Video"===virtualclass.currApp&&"object"===typeof virtualclass.videoUl&&virtualclass.videoUl.hasOwnProperty("videoStartTime"))c=this.getTotalTimeInMilSeconds(virtualclass.videoUl.videoStartTime.data.masterIndex,virtualclass.videoUl.videoStartTime.data.subIndex),c=(this.elapsedPlayTime-c)/1E3,"object"===typeof virtualclass.videoUl.player?virtualclass.videoUl.playVideo(c):
(console.log("====Video init to play start"),virtualclass.videoUl.isPaused=this.pauseBeforeSeek?!0:!1,virtualclass.videoUl.startTime=c);else if("Quiz"===virtualclass.currApp&&"object"===typeof virtualclass.quiz){c=document.querySelector("#qztime");var a=this.getTotalTimeInMilSeconds(virtualclass.quiz.quizStartTime.data.masterIndex,virtualclass.quiz.quizStartTime.data.subIndex);0<+virtualclass.quiz.plugin.config.quizTime?(a=1E3*virtualclass.quiz.plugin.config.quizTime-(this.elapsedPlayTime-a),virtualclass.quiz.plugin.method.startTimer(a/
1E3,c,"desc","vmQuiz")):(a=(this.elapsedPlayTime-a)/1E3,virtualclass.quiz.plugin.method.startTimer(a,c,"asc","vmQuiz"))}},pollUpdateTime(c,a){c=this.convertIntoReadable(6E4*a.data.stdPoll.newTime.min+1E3*a.data.stdPoll.newTime.sec-(this.elapsedPlayTime-c));virtualclass.poll.newTimer.sec=c.s;virtualclass.poll.newTimer.min=c.m;virtualclass.poll.showTimer(virtualclass.poll.newTimer)},handleSyncPacket(){if(this.binarySyncMsg){console.log("Get full screen share");var c=this.binarySyncMsg.data.subIndex;
let a=this.binarySyncMsg.data.masterIndex;for(;a<=this.masterIndex;){console.log(`Start from master index ${a} from Subindex ${c}`);const b=this.masterRecordings[a];let f;for(f=this.masterIndex!=a?this.masterRecordings[this.masterIndex].length:this.subRecordingIndex;c<=f;c++){this.msg=b[c];try{if(null!=this.msg&&"B"==this.msg.type){if(this.msg=this.msg.recObjs,104==this.msg[0]||204==this.msg[0]||103==this.msg[0]||203==this.msg[0]||102==this.msg[0]||202==this.msg[0])io.onRecMessage(this.convertInto({data:this.msg}))}else console.log("Either this.msg is null or string")}catch(g){console.log(`PLAY ERROR ${g.errorCode}`)}}if(this.masterIndex==
...
!0,clearInterval(this.triggerDownloaderTime.triggerDownloaderTime),this.isPlayFinished()&&this.askAgainToPlay()):virtualclass.xhrn.sendData({session:this.session},virtualclass.api.recordingFiles,this.afterDownloadingList.bind(this));console.log("Time to request 180000")},18E4)},afterDownloadingList(c){c=JSON.parse(c);if(null!=c&&c.hasOwnProperty("Item")){var a=+c.Item.time.N;this.sessionStartTime=(new Date).getTime()-a;a=[...new Set(c.Item.list.L.map(a=>a.S))];c=a.length;if(this.lastRecordings&&a.length==
this.lastRecordings)this.tryNumberOfTimes=this.tryNumberOfTimes?++this.tryNumberOfTimes:1;else{delete this.alreadyCalcTotTime;this.totalRecordingFiles=this.sortingFiles(a);this.lastFile=this.totalRecordingFiles[this.totalRecordingFiles.length-1];this.orginalListOfFiles=this.setOrginalListOfFiles(this.totalRecordingFiles);this.calculateTotalPlayTime();a=this.totalRecordingFiles.shift();this.firstxhr=axios.create({timeout:6E5,withCredentials:!0});this.requestDataFromServer(a,this.firstxhr);a=document.getElementsByTagName("head")[0];
for(const b in this.totalRecordingFiles){const c=`https://recording.congrea.net/${wbUser.lkey}/${wbUser.room}/${virtualclass.recorder.session}/${this.totalRecordingFiles[b]}`,g=document.createElement("link");g.setAttribute("rel","prefetch");g.setAttribute("crossOrigin","use-credentials");g.setAttribute("href",c);a.appendChild(g)}9E5>this.sessionStartTime&&this.triggerDownloader()}this.lastRecordings=c}else this.tryNumberOfTimes=this.tryNumberOfTimes?++this.tryNumberOfTimes:1,this.triggerDownloader()},
requestListOfFiles(){this.session=wbUser.session;virtualclass.popup.loadingWindow();virtualclass.xhrn.sendData({session:virtualclass.recorder.session},virtualclass.api.recordingFiles,this.afterDownloadingList.bind(this))},calculateTotalPlayTime(){const c=Math.trunc(+virtualclass.recorder.totalRecordingFiles[0].split("-")[0]/1E6),a=Math.trunc(+virtualclass.recorder.totalRecordingFiles[virtualclass.recorder.totalRecordingFiles.length-1].split("-")[1].split(".")[0]/1E6);this.totalTimeInMiliSeconds=a-
c;this.lastTimeInSeconds=Math.trunc(a/1E3);this.firstTimeInSeconds=Math.trunc(c/1E3)},getTimeFromFile(c){return+c.split("-")[1].split(".")[0]/1E9},totalPlayTime(){let c=0;for(let a=0;a<virtualclass.recorder.masterRecordings.length;a++)for(let b=0;b<virtualclass.recorder.masterRecordings[a].length;b++)c+=virtualclass.recorder.masterRecordings[a][b].playTime;return c/1E3},getTotalTimeInMilSeconds(c,a){let b=0,f=0;for(;b<=c;){for(let g=0;g<virtualclass.recorder.masterRecordings[b].length&&(f+=virtualclass.recorder.masterRecordings[b][g].playTime,
c!==b||a!==g);g++);b+=1}return f},getTempTotalTimeInMilSeconds(c,a){let b=0,f=0;for(;b<=c;){for(let g=0;g<this.orginalTimes[b].length&&(f+=this.orginalTimes[b][g],c!=b||a!=g);g++);b++}return f},getTotalElementLength(){let c=0;for(let a=0;a<virtualclass.recorder.masterRecordings.length;a++)c+=virtualclass.recorder.masterRecordings[a].length;return c},seekWithMouseMove(c){void 0==c.offsetX&&(c=this.getOffset(c));this.startSeek&&(this.controller._pause(),this.seekValueInPercentage=this.getSeekValueInPercentage(c),
this.downloadInPercentage<this.seekValueInPercentage&&(this.seekValueInPercentage=Math.trunc(this.downloadInPercentage)),this.setPlayProgressTime(this.seekValueInPercentage),virtualclass.pbar.renderProgressBar(this.totalTimeInMiliSeconds,6E4*this.seekTimeWithMove.m+1E3*this.seekTimeWithMove.s,"playProgressBar",void 0),document.querySelector("#tillRepTime").innerHTML=`${this.seekTimeWithMove.h}:${this.seekTimeWithMove.m}:${this.seekTimeWithMove.s}`,this.displayTimeInHover(c,this.seekValueInPercentage))},
getSeekValueInPercentage(c){const a=this.getCustomWidth();return c.offsetX/a*100},handlerDisplayTime(c){const a=this.getSeekValueInPercentage(c);this.displayTimeInHover(c,a)},displayTimeInHover(c,a){this.setPlayProgressTime(a);c=20>c.offsetX?3:5>d.innerWidth-c.offsetX?c.offsetX-65:30>d.innerWidth-c.offsetX?c.offsetX-60:40>d.innerWidth-c.offsetX?c.offsetX-35:c.offsetX-25;a=document.getElementById("timeInHover");a.style.display="block";a.style.marginLeft=`${c}px`;document.getElementById("timeInHover").innerHTML=
`${this.seekTimeWithMove.h}:${this.seekTimeWithMove.m}:${this.seekTimeWithMove.s}`;document.querySelector("#virtualclassCont").classList.add("recordSeeking")},setPlayProgressTime(c){this.seekTimeWithMove=this.convertIntoReadable(c*this.totalTimeInMiliSeconds/100)},finalSeek(c){c.offsetX||(c=this.getOffset(c));this.startSeek&&this.hasOwnProperty("seekValueInPercentage")&&(console.log(`====Seek up ${this.seekValueInPercentage}`),this.downloadInPercentage<this.seekValueInPercentage&&(this.seekValueInPercentage=
Math.trunc(this.downloadInPercentage)),0<this.seekValueInPercentage&&this.seek(this.seekValueInPercentage),this.pauseBeforeSeek?(console.log("=== Video pause "),this.controller._pause(),this.triggerPauseVideo()):(console.log("=== Video play "),this.controller._play(),this.triggerPlayVideo()),document.getElementById("timeInHover").style.display="none");document.querySelector("#virtualclassCont").classList.remove("recordSeeking");console.log(c.offsetX);delete this.seekValueInPercentage;this.startSeek=
!1;c=document.getElementById("congrealogo");null!=c&&c.classList.remove("disbaleOnmousedown")}}})(window);(function(d){let c,a=!1,b=!1,f=!1;d.storage={version:8,async init(){c=this;this.tables="wbData dataAdapterAll dataUserAdapterAll executedStoreAll executedUserStoreAll dstdata pollStorage quizData dstall".split(" ");this.db=await virtualclass.virtualclassIDBOpen("vidya_apps",c.dbVersion,{upgrade(a,b,f,d){c.onupgradeneeded(a)}});"object"===typeof this.db.objectStoreNames&&null!=this.db.objectStoreNames?await this.onsuccess():this.init()},onupgradeneeded(a){a.objectStoreNames.contains("wbData")||a.createObjectStore("wbData",
...
else if(b.hasOwnProperty("allDocs"))this.allDocs=b.allDocs,this.afterUploadFile(b.doc);else if(b.hasOwnProperty("fallDocs")){var c=this;c.afterFirstRequestDocs(b.fallDocs);virtualclass.serverData.rawData.docs=b.fallDocs;a=document.getElementsByTagName("head")[0];for(var d in virtualclass.serverData.rawData.docs[0].notes){const b=document.createElement("link");b.setAttribute("rel","prefetch");b.setAttribute("crossOrigin","use-credentials");b.setAttribute("href",virtualclass.serverData.rawData.docs[0].notes[d].pdf);
a.appendChild(b)}}else b.hasOwnProperty("dres")?(this.docs.studentExecuteScreen(b),console.log(`${virtualclass.gObj.currWb} `+"document share :- Layout initialized")):b.hasOwnProperty("slideTo")?"object"!==typeof this.docs.note?(c=this,this.docs.executeScreen(b.docn,void 0,()=>{console.log("document share :- doc is not created");c.docs.note.currNote=b.slideTo;const a=document.querySelector(`#note${b.slideTo}`);c.docs.note.slideTo(a)},b.slideTo)):"object"===typeof this.docs.note&&b.docn!=this.docs.num?
(this.docs.currNote=b.slideTo,console.log(`Current note ${this.docs.currNote}`),this.docs.executeScreen(b.docn,void 0),this.docs.note.currentSlide(b.slideTo)):(d=document.querySelector(`#note${b.slideTo}`),null!=d?(this.docs.currNote=b.slideTo,console.log(`Current note ${this.docs.currNote}`),this.docs.note.getScreen(d),console.log(`${virtualclass.gObj.currWb} `+"document share :- Normal Case")):alert(`Note is not found ${b.slideTo}`)):b.hasOwnProperty("dispScreen")?(d=b.dispScreen.sc,virtualclass.vutil.updateCurrentDoc(d,
b.dispScreen.sn),virtualclass.dts.screenIsCreated(d)?(console.log(`${virtualclass.gObj.currWb} `+"document share :- Only display screen"),this.docs.displayScreen(d)):(console.log(`${virtualclass.gObj.currWb} `+"document share :- With screen Created"),this.docs.executeScreen(d,void 0,void 0,b.dispScreen.sn),"object"===typeof this.docs.note&&this.docs.note.getScreen(b.dispScreen.sn))):b.hasOwnProperty("rmnote")?this._delete(b.rmnote):b.hasOwnProperty("rmsnote")?(this._removePageUI(b.rmsnote),this._removePageFromStructure(b.rmsnote)):
b.hasOwnProperty("noteSt")?(this.noteStatus(b.note,b.noteSt),this.storeInDocs(this.allNotes)):b.hasOwnProperty("docSt")?this.docStatus(b.doc,b.docSt):b.hasOwnProperty("order_recived")?this.afterRequestOrder(b.order_recived):b.hasOwnProperty("norder")&&(this.order=b.norder,virtualclass.dts.indexNav.studentDocNavigation(this.docs.currNote))},sendCurrentSlide(){if(virtualclass.dts.docs.hasOwnProperty("currDoc")){const a=virtualclass.dts.docs.currDoc;void 0!=a?null!=document.querySelector("#listnotes .note")&&
(ioAdapter.mustSend({dts:{slideTo:virtualclass.dts.docs.note.currNote,docn:a},cf:"dts"}),console.log(`${virtualclass.gObj.currWb} `+"Document share send current slide")):console.log("Document sharing : doc number is undefined")}},sendCurrentDoc(){if(virtualclass.dts.docs.hasOwnProperty("currDoc"))if(void 0!=a){var a;ioAdapter.mustSend({dts:{doc:virtualclass.dts.docs.currDoc},cf:"dts"});console.log(`${virtualclass.gObj.currWb} `+"Document share send current doc only")}else console.log("Document sharing : doc number is undefined")},
consoleMessage(a){console.log(`${virtualclass.gObj.currWb} ${a}`)},_rearrange(a){alert(a.toString())},reArrangeNotes(a){this.order=a;this.reArrangeElements(a);roles.hasAdmin()&&(this.sendOrder(this.order),ioAdapter.mustSend({dts:{norder:this.order},cf:"dts"}))},sendOrder(a){virtualclass.vutil.sendOrder("docs",a)},reaArrangeThumbCount(){const a=document.querySelectorAll("#listnotes .thumbList");for(let b=0;b<a.length;b++)a[b].innerHTML=b+1},reArrangeElements(a){const b=document.getElementById("notesContainer"),
c=document.createElement("div");c.id="notesContainer";c.className="notes";for(let b=0;b<a.length;b++)c.appendChild(document.getElementById(`note${a[b]}`));b.parentNode.replaceChild(c,b);this.reaArrangeThumbCount();0<document.querySelectorAll("#dcPaging .noteIndex").length&&this.indexNav.rearrangePageNavigation(a);document.querySelector("#dcPaging").addEventListener("change",function(){virtualclass.dts.docs.goToNavs(this.value)()})},_delete(a){const b=document.querySelector(`#linkdocs${a}`);null!=
b&&b.parentNode.removeChild(b);const c=this;virtualclass.xhrn.sendData({uuid:a,action:"delete",page:0},virtualclass.api.UpdateDocumentStatus,a=>{"ok"==JSON.parse(a).status&&c.sendOrder(c.order)});delete this.pages[`docs${a}`];this.removePagesUI(a);this.removePagesFromStructure(a);this.upateInStorage()},_deleteNote(a,b){this._removePageUI(a,b);this._removePageFromStructure(a,b);roles.hasControls()&&ioAdapter.mustSend({dts:{rmsnote:a},cf:"dts"});b=a.split("_");a=b[0];b=parseInt(b[1]);const c=this;virtualclass.xhrn.sendData({uuid:a,
action:"delete",page:b},virtualclass.api.UpdateDocumentStatus,a=>{"ok"==JSON.parse(a).status&&c.sendOrder(c.order)});b=!1;const d=virtualclass.dts.allDocs[a];if(null!=d)for(let a=0;a<d.notesarr.length;a++)if(!d.notesarr[a].hasOwnProperty("deletedn")){b=!0;break}b||this._delete(a);roles.hasControls()&&this.indexNav.createIndex()},removePagesFromStructure(a){this.allDocs[a].deleted="0";let b;for(b in this.allNotes)-1<this.allNotes[b].id.indexOf(a)&&this._removePageFromStructure(this.allNotes[b].id)},
updateInAllDocs(a){var b=a.split("_")[0];b=this.allDocs[b];if("undefined"!==typeof b.notes[a]){b.notes[a].deletedn=a;for(let c=0;c<b.notesarr.length;c++)b.notesarr[c].id==a&&(b.notesarr[c].deletedn=a)}else console.log(`The note with the id ${a} is not available`)},_removePageFromStructure(a){this.removeWhiteboardFromStorage(`_doc_${a}_${a}`);this.allNotes[a].deletedn=a;this.storeInDocs(this.allNotes);this.updateInAllDocs(a)},_disable(a){this.docStatus(a)},_enable(a){this.docStatus(a)},docStatus(a,
b){var c=document.querySelector(`#linkdocs${a} .controls.status`);null!=c?"undefined"===typeof b&&(b=1-+c.dataset.status):console.log(`document share:- there is no element ${a}`);c=this.getDocs(a);for(let a=0;a<c.length;a++){const d=c[a].id;this.noteStatus(d,b);this.updatePageNavStatus(d,b)}this.storeInDocs(this.allNotes);roles.hasControls()&&ioAdapter.mustSend({dts:{docSt:b,doc:a},cf:"dts"})},updatePageNavStatus(a,b){const c=document.querySelector(`#linknotes${a}`);null!=c?(c.dataset.status=b,a=
c.querySelector(".controls.status"),a.dataset.status=b,a.querySelector(".statusanch").innerHTML=`status${b}`):console.log(`${"Document share : there is no element #linknotes"}${a}`)},_noteDisable(a){this.noteStatus(a);this.storeInDocs(this.allNotes);this.sendNoteStatus(a)},_noteEnable(a){this.noteStatus(a);this.storeInDocs(this.allNotes);this.sendNoteStatus(a)},sendNoteStatus(a){if(roles.hasControls()){const b=document.querySelector(`#note${a}`);null!=b?ioAdapter.mustSend({dts:{noteSt:b.dataset.status,
note:a},cf:"dts"}):console.log("Element is null")}},noteStatus(a,b){const c=document.querySelector(`#note${a}`);null!=c?(b="undefined"===typeof b?1-+c.dataset.status:1==b||"true"==b?1:0,c.dataset.status=b,b=this.allNotes[a],b.status=c.dataset.status,this.allNotes[a]=b):console.log(`there is no element #note${a}`)},getDocsOld(a){const b=[];for(const c in this.allNotes)a==this.allNotes[c].lc_content_id&&b.push(this.allNotes[c]);return b},getDocs(a){console.log("--------------");if(null!=this.allDocs&&
"object"===typeof this.allDocs[a])return this.allDocs[a].notesarr;console.log(`There is no document with the id ${a}`)},getDocId(a){return a.split("_")[0]},getAllNotes(a){const b=[];for(let c=0;c<a.length;c++)b.push(this.allNotes[a[c]]);return b},getNote(a){return this.allNotes[a]},updateLinkNotes(a){const b=document.querySelector("#listnotes .currentNav");null!=b&&b.classList.remove("currentNav");this.setCurrentNav(a)},setCurrentNav(a){a=document.querySelector(`#linknotes${a}`);null!=a&&a.classList.add("currentNav")},
onAjaxResponse(a,b,c){if(c.hasOwnProperty("success")&&c.success){for(a=0;a<virtualclass.gObj.uploadingFiles.length;a++)this.afterUploadFile(virtualclass.gObj.uploadingFiles[a].uuid);virtualclass.gObj.uploadingFiles=[];this.showUploadMsz(virtualclass.lang.getString("docUploadSuccess"),"alert-success")}else"duplicate"==c.message?this.showUploadMsz(virtualclass.lang.getString("duplicateUploadMsg"),"alert-error"):c.hasOwnProperty("error")?this.showUploadMsz(c.error,"alert-error"):this.showUploadMsz(virtualclass.lang.getString("someproblem"),
"alert-error");if(a=document.querySelector("#DocumentShareDashboard .qq-upload-list-selector.qq-upload-list"))a.style.display="none";null!=document.querySelector("#listnotes")?(virtualclass.vutil.makeElementDeactive("#DocumentShareDashboard .qq-uploader-selector.qq-uploader.qq-gallery"),virtualclass.vutil.makeElementActive("#listnotes")):console.log("List note is null")},showUploadMsz(a,b){const c=document.querySelector("#DocumentShareDashboard #docsUploadMsz");var d=document.querySelector("#DocumentShareDashboard #docsUploadMsz .alert");
...
this.axhr.get(b.pdf).then(function(a){virtualclass.pdfRender[virtualclass.gObj.currWb].afterPdfPrefetch(b.id,a.data)}).catch(function(){virtualclass.pdfRender[virtualclass.gObj.currWb].prefechPdf(a)})},afterPdfPrefetch(a,b){virtualclass.gObj.next={};virtualclass.gObj.next[a]=b},async loadPdf(a,b,c){virtualclass.gObj.hasOwnProperty("getDocumentTimeout")&&clearTimeout(virtualclass.gObj.getDocumentTimeout);null==virtualclass.gObj.getDocumentTimer||0==virtualclass.gObj.getDocumentTimer?(this._loadPdf(a,
b,c),virtualclass.gObj.getDocumentTimer=!0,virtualclass.gObj.getDocumentTimeout=setTimeout(()=>{virtualclass.gObj.getDocumentTimer=!1},1E3)):virtualclass.gObj.getDocumentTimeout=setTimeout(()=>{this._loadPdf(a,b,c);virtualclass.gObj.getDocumentTimer=!1},1E3)},_loadPdf(a,b,c){virtualclass.gObj.next.hasOwnProperty(c)?this.afterPdfLoad(b,c,virtualclass.gObj.next[c]):this.axhr.get(a).then(function(a){virtualclass.pdfRender[virtualclass.gObj.currWb].afterPdfLoad(b,c,a.data)}).catch(function(){virtualclass.pdfRender[virtualclass.gObj.currWb]._loadPdf(a,
b,c)})},async afterPdfLoad(a,b,c){this.canvasWrapper=document.querySelector(`#canvasWrapper${virtualclass.gObj.currWb}`);this.canvas=a;a={};a.data=c;null!=virtualclass.gObj.myworker&&(a.worker=virtualclass.gObj.myworker);const d=this;c=virtualclass.gObj.currWb;virtualclass.currApp==virtualclass.currApp?(d.wbId="undefined"===typeof b?virtualclass.gObj.currWb:b,console.log(`-----------START ${virtualclass.currApp}----------`),console.log("PDF render request to pdf.js 1"),PDFJS.workerSrc=`${whiteboardPath}build/src/pdf.worker.min.js`,
PDFJS.getDocument(a).then(a=>{null==virtualclass.gObj.myworker&&(virtualclass.gObj.myworker=a.loadingTask._worker);d.displayPage(a,1,()=>{},!0);d.shownPdf=a;roles.hasControls()||(d.topPosY=0,d.leftPosX=0);d.scrollEvent()})):"DocumentShare"==virtualclass.currApp&&virtualclass.wbCommon.deleteWhiteboard(c);"undefined"!==typeof virtualclass.gObj.currWb&&null!=virtualclass.gObj.currWb&&(b=document.querySelector(`#note${b}`),null!=b&&null!=b.nextElementSibling&&virtualclass.pdfRender[virtualclass.gObj.currWb].prefechPdf(b.nextElementSibling.dataset.slide))},
...
c,a,"Y");(0<d||"undefined"!==typeof b)&&this._scroll(d,c,a,"X");roles.hasControls()||virtualclass.pdfRender[virtualclass.gObj.currWb].setScrollPosition({scX:d,scY:c})},_scroll(a,b,c,d){if(roles.hasControls())return this.topPosY=b,this.leftPosX=a,this.scrollPosition(c,d);if("Y"==d)var f=b;else"X"==d&&(f=a);this.updateScrollPosition(f,d);return null},scrollPosition(a,b){const {canvas:c}=this;if("Y"==b)var d=a.scrollTop,k=c.height;else"X"==b&&(d=a.scrollLeft,k=c.width);this[`scrollPos${b}`]=d/k*100;
a=virtualclass.vutil.getElemM(`canvasWrapper${virtualclass.gObj.currWb}`,b);this[`actualVp${b}`]=a;this[`viewPort${b}`]=a/k*100;k={};k[`vp${b}`]=this[`viewPort${b}`];k[`sc${b}`]=this[`scrollPos${b}`];return k},scroll:{caclculatePosition(a,b,c){a=this.type=c;null==this[a]&&(this[a]={});this[a].a=0;this[a].d=b;b=`canvasWrapper${virtualclass.gObj.currWb}`;c=document.querySelector(`#${b}`);null!=c&&("X"==this.type?this[a].b=c.scrollLeft:"Y"==this.type&&(this[a].b=c.scrollTop),this[a].studentVPm=virtualclass.vutil.getElemM(b,
a),this[a].c=this[a].b+this[a].studentVPm)}},setScrollPosition(a){a.hasOwnProperty("scY")&&null!=a.scY&&this.scroll.caclculatePosition(a.scY,this.canvas.height,"Y");a.hasOwnProperty("scX")&&null!=a.scX&&this.scroll.caclculatePosition(a.scX,this.canvas.width,"X")},customMoustPointer(a,b,c){this.scroll[b].e=c;this.scroll[b].e>this.scroll[b].c?(a=this.scroll[b].b+(this.scroll[b].d-this.scroll[b].c),a>this.scroll[b].e&&(a=this.scroll[b].e-(this.scroll[b].b+this.scroll[b].c)/2),console.log(`custom mouse down pointer ay=${this.scroll[b].a} by=${this.scroll[b].b} cy=${this.scroll[b].c} dy=${this.scroll[b].d} ey${this.scroll[b].e} scrollPos=${a}`),
...
this.sdxhr.post(d,{}).then(function(c){virtualclass.serverData.afterResponse(c.data)}).catch(function(c){console.log(c);virtualclass.serverData.requestData(d)})},afterResponse(d){this.awsUrlArr(d);virtualclass.gObj.fetchedData=!0;"undefined"!==typeof virtualclass.serverData.cb&&virtualclass.serverData.cb()},awsUrlArr(d){this.rawData={video:[],ppt:[],docs:[]};const c=[];d=d.Items;for(let q=0;q<d.length;q++)if(d[q].hasOwnProperty("filetype"))switch(d[q].filetype.S){case "doc":var a=this.processObj(d[q]);
var b=d[q].processed_data.M.commonpath.S;var f=parseInt(d[q].processed_data.M.count.N);if(null!=b&&null!=f){b=`${"https://media.congrea.net"}/${d[q].processed_data.M.commonpath.S}`;a.notes={};a.notesarr=[];let r=[],z=[];a.hasOwnProperty("deletednes")&&(r=a.deletednes);a.hasOwnProperty("disablednes")&&(z=a.disablednes);const t=document.getElementsByTagName("head")[0];for(let c=1;c<=f;c++){var g=c;var k=3-`${g}`.length;var m=(0<k?Array(++k).join("0"):"")+g;g=`${b}/image/${m}.${d[q].processed_data.M.image.M.type.S}`;
k=`${b}/pdf/${m}.pdf`;m=`${b}/thumbnail/${m}.${d[q].processed_data.M.thumbnail.M.type.S}`;var n=document.createElement("link");n.setAttribute("rel","prefetch");n.setAttribute("crossOrigin","use-credentials");n.setAttribute("href",m);t.appendChild(n);n=document.createElement("link");n.setAttribute("rel","prefetch");n.setAttribute("crossOrigin","use-credentials");n.setAttribute("href",k);t.appendChild(n);n=99<c?`${a.fileuuid}_${c}`:9<c?`${a.fileuuid}_0${c}`:`${a.fileuuid}_00${c}`;const f={};0<r.length&&
-1<r.indexOf(c)&&(f.deletedn=n);f.id=n;f.pdf=k;f.image=g;f.thumbnail=m;0<z.length?-1<z.indexOf(c)?f.status=0:f.status=1:f.status=1;a.notes[n]=f;a.notesarr.push(f)}c.push(a);this.rawData.docs.push(a)}break;case "video":null!=d[q].processed_data&&"COMPLETED"==d[q].processed_data.S&&(a=this.processObj(d[q]),f=a.filepath.substr(0,a.filepath.lastIndexOf("/")),a.urls={},a.urls.thumbnail={},a.urls.videos={},a.urls.main_video=`${"https://media.congrea.net/"+f}/video/play_video.m3u8`,a.urls.videos["0400k"]=
`${"https://media.congrea.net/"+f}/video/0400k/video.m3u8`,a.urls.videos["0600k"]=`${"https://media.congrea.net/"+f}/video/0600k/video.m3u8`,a.urls.videos["1000k"]=`${"https://media.congrea.net/"+f}/video/1000k/video.m3u8`,a.urls.videos["1500k"]=`${"https://media.congrea.net/"+f}/video/1500k/video.m3u8`,a.urls.videos["2000k"]=`${"https://media.congrea.net/"+f}/video/2000k/video.m3u8`,a.urls.thumbnail["0400k"]=`${"https://media.congrea.net/"+f}/video/0400k/thumbs/00001.png`,a.urls.thumbnail["0600k"]=
