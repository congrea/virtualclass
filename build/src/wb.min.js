...
d-this.trimontime,this.isTrimRecordingNow=!1);this.UIdownloadProgress(c,b);this.updateTotalTime();(3<this.currentMin||this.lastFile===c)&&0<this.masterRecordings.length&&(this.playStart?this.startToPlay():this.alreadyAskForPlay||(this.alreadyAskForPlay=!0,this.askToPlay()))},handleStartToPlay(c){const a=document.querySelector(".rv-vanilla-modal-overlay.is-shown");null!=a&&a.removeEventListener("click",this.handleStartToPlay.bind(this));virtualclass.gesture.clickToContinue();console.log("===== Start to play");
this.startToPlay();c.currentTarget.classList.remove("askToPlayCont")},startToPlay(){this.playStart?this.hasOwnProperty("isPausedByNotPresent")&&(delete this.isPausedByNotPresent,this.subRecordingIndex=0,this.subRecordings=this.masterRecordings[this.masterIndex],this.controller._play(),virtualclass.popup.closeElem()):(this.playStart=!0,this.playInt(),virtualclass.popup.closeElem())},isDownloadedAllRecordings(c){return 9E5<this.sessionStartTime?c>=virtualclass.recorder.lastTimeInSeconds:!1},finishRequestDataFromServer(c){this.isDownloadedAllRecordings(c)&&
(virtualclass.recorder.allFileFound=!0,virtualclass.recorder.alreadyAskForPlay&&(c=document.getElementById("askplayMessage"),null!=c&&(c.innerHTML=virtualclass.lang.getString("playsessionmsg"))))},UIdownloadProgress(c,a){c=(a-this.firstTimeInSeconds)/60;c>this.currentMin&&(this.currentMin=c);c=virtualclass.recorder.totalTimeInMiliSeconds/1E3/60;this.downloadInPercentage=100*this.currentMin/c;virtualclass.pbar.renderProgressBar(c,this.currentMin,"downloadProgressBar","downloadProgressValue");virtualclass.recorder.playStart&&
...
const c=document.querySelector(".rv-vanilla-modal-overlay.is-shown");c.addEventListener("click",this.handleStartToPlay.bind(this));const a=document.querySelector("#loadingWindowCont .askToPlay");null!=a&&a.addEventListener("click",this.handleStartToPlay.bind(this));document.getElementById("popupContainer").classList.add("playPopup");c.classList.add("askToPlayCont")},playProgressOutput(c){console.log("Offset ",c.offsetX)},getOffset(c){"touchend"==c.type?c=this.lastEvent:(c.offsetX=c.touches[0].pageX-
c.touches[0].target.offsetLeft,c.offsetY=c.touches[0].pageY-c.touches[0].target.offsetTop);return this.lastEvent=c},seekHandler(c){void 0==c.offsetX&&(c=this.getOffset(c));this.pauseBeforeSeek=this.controller.pause;if(this.startSeek)console.log("Earlier seek start is not end yet.");else{this.startSeek=!0;var a=document.querySelector("#virtualclassCont");null!=a&&a.classList.add("recordSeeking");a=c.offsetX;const b=this.getCustomWidth();this.seekValueInPercentage=a/b*100;console.log(`====Seek start ${this.seekValueInPercentage} ev current target=${c.currentTarget.id}`)}},
getCustomWidth(){return document.getElementById("downloadProgress").clientWidth-1},async seek(c){virtualclass.videoHost.UI.hideTeacherVideo();c=this.getSeekPoint(c);(c.master<this.masterIndex||c.master==this.masterIndex&&c.sub<this.subRecordingIndex)&&await this.replayFromStart();this._seek(c);console.log("seek is finished");this.triggerSynchPacket()},triggerSynchPacket(){this.triggerPlayProgress();this.binarySyncMsg&&(this.handleSyncPacket(),this.binarySyncMsg=null);this.handleSyncStringPacket()},
_seek(c){this.controller._pause();let a;for(;;){a=null!=c?this.masterIndex!=c.master?this.masterRecordings[this.masterIndex].length:c.sub:this.masterRecordings[this.masterIndex].length;for(let b=this.subRecordingIndex;b<a;b++){try{if("B"!=this.subRecordings[this.subRecordingIndex].type)this.msg=io.cleanRecJson(this.subRecordings[this.subRecordingIndex].recObjs),-1<this.msg.indexOf('"m":{"unshareScreen"')?this.binarySyncMsg=null:-1<this.msg.indexOf('},"m":{"poll":{"pollMsg":"stdPublish",')?virtualclass.poll.recordStartTime=
{app:"Poll",data:{masterIndex:this.masterIndex,subIndex:this.subRecordingIndex}}:-1<this.msg.indexOf('"m":{"videoUl":{"content_path"')?virtualclass.videoUl.videoStartTime={app:"Video",data:{masterIndex:this.masterIndex,subIndex:this.subRecordingIndex}}:-1<this.msg.indexOf('"m":{"quiz":{"quizMsg":"stdPublish"')&&(virtualclass.quiz.quizStartTime={app:"Quiz",data:{masterIndex:this.masterIndex,subIndex:this.subRecordingIndex}}),io.onRecMessage(this.convertInto({data:this.msg}));else{this.msg=this.subRecordings[this.subRecordingIndex].recObjs;
...
this.elapsedTime=this.elapsedPlayTime;if(void 0!=c&&this.masterIndex===c.master&&c.sub===this.subRecordingIndex)break;else this.subRecordingIndex=0,this.masterIndex++,this.subRecordings=this.masterRecordings[this.masterIndex]}},handleSyncStringPacket(){if("Poll"===virtualclass.currApp&&"object"===typeof virtualclass.poll.pollState.data&&virtualclass.poll.hasOwnProperty("recordStartTime")){var c=this.getTotalTimeInMilSeconds(virtualclass.poll.recordStartTime.data.masterIndex,virtualclass.poll.recordStartTime.data.subIndex);
