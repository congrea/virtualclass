...
this.prevFile=a.file,a=this.orginalListOfFiles[a.file].next,"end"!==a&&this.formatRecording(a))},isFileVcp(a){return null!=a&&null!=a.match(/^.*\.(vcp)$/)},calculateNextTime(a,b){const c=b.substring(0,21);b.substring(22,b.length);[b]=c.split(" ");b=Math.trunc(b/1E6);return b-a},insertPacketInto(a,b){b=Math.trunc(b/1E3);var c=0;if(!isNaN(b)&&1<=b){this.isTrimRecordingNow?this.updateTrimTime(1E3*b):c=1E3;c={playTime:c,recObjs:'{"0{"user":{"userid":"2"},"m":{"app":"nothing","cf":"sync"}} ',type:"J"};
for(let f=0;f<b;f++)a.push(c)}return a},makeRecordingQueue(a,b){console.log(`File formatting ${a}`);var c=[],g=null;b=b.trim().split(/(?:\r\n|\r|\n)/g);for(let a=0;a<b.length;a++)if(null!=b[a]&&""!=b[a]){this.totalElements++;var d=b[a].substring(0,21);var m=b[a].substring(22,b[a].length);let [f,k]=d.split(" ");this.tempTime=f;f=Math.trunc(f/1E6);null!=this.refrenceTime&&(g?(this.tempPlayTime=f-this.refrenceTime-g,g=null):this.tempPlayTime=f-this.refrenceTime);null!=m&&""!=m&&(!this.joinRoomRecevied&&
-1<m.indexOf('"type":"joinroom"')&&(this.joinRoomRecevied=!0,m=JSON.parse(m),m.clientids=m.action,m=JSON.stringify(m)),this.lastFileTime&&0===a&&(d=Math.trunc(f-this.lastFileTime),c=this.insertPacketInto(c,d),this.tempPlayTime=1E3<d?d%1E3:d),a===b.length-1&&(this.lastFileTime=f),virtualclass.settings.info.trimRecordings&&-1<m.indexOf('{"ac":21,"cf":"recs"')&&(console.log("Trim off, ==== index ",c.length,"master ",this.masterRecordings.length),this.isTrimRecordingNow=!1,this.totalTimeInMiliSeconds-=
this.totalTrimTime),this.isTrimRecordingNow?(c.push({playTime:0,recObjs:m,type:k}),this.updateTrimTime(this.tempPlayTime),console.log("Trim on, ==== index ",a,"master ",this.masterRecordings.length)):virtualclass.settings.info.trimRecordings&&-1<m.indexOf('{"ac":11,"cf":"recs"')?(this.isTrimRecordingNow=!0,this.updateTrimTime(this.tempPlayTime),c.push({playTime:0,recObjs:m,type:k}),console.log("Trim on, ==== index ",c.length,"master ",this.masterRecordings.length)):c.push({playTime:this.tempPlayTime,
recObjs:m,type:k}),"undefined"!==typeof b[a+1]&&(m=this.calculateNextTime(f,b[a+1]),c=this.insertPacketInto(c,m,!0),1E3<=m&&(g=1E3*Math.trunc(m/1E3))),this.refrenceTime=f)}console.log(`totalTime in seconds ${this.totalTimeInMiliSeconds/1E3}`);for(b=0;b<c.length;b++)"B"===c[b].type&&(g=virtualclass.dtCon.base64DecToArr(c[b].recObjs),c[b].recObjs=g);this.masterRecordings.push(c);c=virtualclass.recorder.getTimeFromFile(a);this.finishRequestDataFromServer(c);this.isTrimRecordingNow&&this.allFileFound&&
(this.totalTimeInMiliSeconds-=this.totalTrimTime,this.isTrimRecordingNow=!1);this.UIdownloadProgress(a,c);this.updateTotalTime();(3<this.currentMin||this.lastFile===a)&&0<this.masterRecordings.length&&(this.playStart?this.startToPlay():this.alreadyAskForPlay||(this.alreadyAskForPlay=!0,this.askToPlay()))},updateTrimTime(a){this.totalTrimTime+=a;console.log("==== total trim time",this.totalTrimTime)},handleStartToPlay(a){const b=document.querySelector(".rv-vanilla-modal-overlay.is-shown");null!=b&&
b.removeEventListener("click",this.handleStartToPlay.bind(this));virtualclass.gesture.clickToContinue();console.log("===== Start to play");this.startToPlay();a.currentTarget.classList.remove("askToPlayCont")},startToPlay(){this.playStart?this.hasOwnProperty("isPausedByNotPresent")&&(delete this.isPausedByNotPresent,this.subRecordingIndex=0,this.subRecordings=this.masterRecordings[this.masterIndex],this.controller._play(),virtualclass.popup.closeElem()):(this.playStart=!0,this.playInt(),virtualclass.popup.closeElem())},
isDownloadedAllRecordings(a){return 9E5<this.sessionStartTime?a>=virtualclass.recorder.lastTimeInSeconds:!1},finishRequestDataFromServer(a){this.isDownloadedAllRecordings(a)&&(virtualclass.recorder.allFileFound=!0,virtualclass.recorder.alreadyAskForPlay&&(a=document.getElementById("askplayMessage"),null!=a&&(a.innerHTML=virtualclass.lang.getString("playsessionmsg"))))},UIdownloadProgress(a,b){a=(b-this.firstTimeInSeconds)/60;a>this.currentMin&&(this.currentMin=a);console.log("=====  total trim time ",
this.totalTrimTime);a=virtualclass.recorder.totalTimeInMiliSeconds/1E3/60;this.downloadInPercentage=100*this.currentMin/a;virtualclass.pbar.renderProgressBar(a,this.currentMin,"downloadProgressBar","downloadProgressValue");virtualclass.recorder.playStart&&!virtualclass.recorder.waitServer&&virtualclass.recorder.init()},playInt(){console.log("=====Play init recording=====");virtualclass.popup.closeElem();virtualclass.recorder.init(virtualclass.recorder.masterRecordings);virtualclass.config.setNewSession("thisismyplaymode");
virtualclass.recorder.initController();virtualclass.media.audio.initAudiocontext()},askToPlay(){document.querySelector("#loadingWindowCont .loading");document.querySelector("#loadingWindowCont .askToPlay");document.querySelector("#loadingWindowCont");const a=document.querySelector(".rv-vanilla-modal-overlay.is-shown");a.addEventListener("click",this.handleStartToPlay.bind(this));const b=document.querySelector("#loadingWindowCont .askToPlay");null!=b&&b.addEventListener("click",this.handleStartToPlay.bind(this));
document.getElementById("popupContainer").classList.add("playPopup");a.classList.add("askToPlayCont")},playProgressOutput(a){console.log("Offset ",a.offsetX)},getOffset(a){"touchend"==a.type?a=this.lastEvent:(a.offsetX=a.touches[0].pageX-a.touches[0].target.offsetLeft,a.offsetY=a.touches[0].pageY-a.touches[0].target.offsetTop);return this.lastEvent=a},seekHandler(a){void 0==a.offsetX&&(a=this.getOffset(a));this.pauseBeforeSeek=this.controller.pause;if(this.startSeek)console.log("Earlier seek start is not end yet.");
else{this.startSeek=!0;var b=document.querySelector("#virtualclassCont");null!=b&&b.classList.add("recordSeeking");b=a.offsetX;const c=this.getCustomWidth();this.seekValueInPercentage=b/c*100;console.log(`====Seek start ${this.seekValueInPercentage} ev current target=${a.currentTarget.id}`)}},getCustomWidth(){return document.getElementById("downloadProgress").clientWidth-1},seek(a){virtualclass.videoHost.UI.hideTeacherVideo();a=this.getSeekPoint(a);(a.master<this.masterIndex||a.master==this.masterIndex&&
a.sub<this.subRecordingIndex)&&this.replayFromStart();this._seek(a);console.log("seek is finished");this.triggerSynchPacket()},selfSeek(){console.log("Start self seek");this.selfStartSeek=!0;this._seek();this.triggerSynchPacket();this.controller._play()},triggerSynchPacket(){this.triggerPlayProgress();this.binarySyncMsg&&(this.handleSyncPacket(),this.binarySyncMsg=null);this.handleSyncStringPacket()},seekFinished(a){if(void 0==a&&void 0!=this.masterRecordings[this.masterIndex][this.subRecordingIndex])return-1<
...
getTimeFromFile(a){return+a.split("-")[1].split(".")[0]/1E9},totalPlayTime(){let a=0;for(let b=0;b<virtualclass.recorder.masterRecordings.length;b++)for(let c=0;c<virtualclass.recorder.masterRecordings[b].length;c++)a+=virtualclass.recorder.masterRecordings[b][c].playTime;return a/1E3},getTotalTimeInMilSeconds(a,b){let c=0,g=0;for(;c<=a;){for(let f=0;f<virtualclass.recorder.masterRecordings[c].length&&(g+=virtualclass.recorder.masterRecordings[c][f].playTime,a!==c||b!==f);f++);c+=1}return g},getTempTotalTimeInMilSeconds(a,
b){let c=0,g=0;for(;c<=a;){for(let f=0;f<this.orginalTimes[c].length&&(g+=this.orginalTimes[c][f],a!=c||b!=f);f++);c++}return g},getTotalElementLength(){let a=0;for(let b=0;b<virtualclass.recorder.masterRecordings.length;b++)a+=virtualclass.recorder.masterRecordings[b].length;return a},seekWithMouseMove(a){void 0==a.offsetX&&(a=this.getOffset(a));this.startSeek&&(this.controller._pause(),this.seekValueInPercentage=this.getSeekValueInPercentage(a),this.downloadInPercentage<this.seekValueInPercentage&&
...
return a.offsetX/b*100},handlerDisplayTime(a){const b=this.getSeekValueInPercentage(a);this.displayTimeInHover(a,b)},displayTimeInHover(a,b){this.setPlayProgressTime(b);a=20>a.offsetX?3:5>d.innerWidth-a.offsetX?a.offsetX-65:30>d.innerWidth-a.offsetX?a.offsetX-60:40>d.innerWidth-a.offsetX?a.offsetX-35:a.offsetX-25;b=document.getElementById("timeInHover");b.style.display="block";b.style.marginLeft=`${a}px`;document.getElementById("timeInHover").innerHTML=`${this.seekTimeWithMove.h}:${this.seekTimeWithMove.m}:${this.seekTimeWithMove.s}`;
document.querySelector("#virtualclassCont").classList.add("recordSeeking")},setPlayProgressTime(a){this.seekTimeWithMove=this.convertIntoReadable(a*this.totalTimeInMiliSeconds/100)},finalSeek(a){a.offsetX||(a=this.getOffset(a));this.startSeek&&this.hasOwnProperty("seekValueInPercentage")&&(console.log(`====Seek up ${this.seekValueInPercentage}`),this.downloadInPercentage<this.seekValueInPercentage&&(this.seekValueInPercentage=Math.trunc(this.downloadInPercentage)),0<this.seekValueInPercentage&&this.seek(this.seekValueInPercentage),
