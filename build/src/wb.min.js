...
value:d}),WhiteboardMessage.send(c))}makeActiveTool(c,b){const a=document.getElementById(c);if(a){var d=b||virtualclass.gObj.currWb,f=document.querySelectorAll(`#commandToolsWrapper${d} .tool.active`);for(let b=0;b<f.length;b+=1)f[b].classList.remove("active");roles.hasControls()&&(f=document.getElementById(c).dataset.tool,document.querySelector(`#tool_wrapper${b}`).dataset.currtool=f);this.constructor.themeColorShapes(c,d);a.classList.add("active");localStorage.activeTool=a.id}}handleActivateTool(c){var b=
localStorage.getItem("activeTool");null!==b&&-1<b.indexOf(c)?(this.makeActiveTool(b,c),b=b.split("_")[1],"activeall"===b&&(b="activeAll"),virtualclass.wb[c].selectedTool=b,"activeAll"!==virtualclass.wb[c].selectedTool&&virtualclass.wb[c].activeAllObj.disable(c)):virtualclass.wb[c].selectedTool&&(b=`t_${virtualclass.wb[c].selectedTool}${c}`,this.makeActiveTool(b,c));virtualclass.wb[c].selectedTool&&"text"===virtualclass.wb[c].selectedTool&&WhiteboardUtility.fontSizeSelector(c);virtualclass.wb[c].toolColor&&
(console.log("====> apply color background"),document.querySelector(`#t_color${c} .disActiveColor`).style.backgroundColor=virtualclass.wb[c].toolColor)}static updateToolStyle(c,b,a){a||(a=virtualclass.gObj.currWb);"color"===c?virtualclass.wb[a].toolColor=b:"strk"===c?virtualclass.wb[a].strokeSize=b:"font"===c&&(virtualclass.wb[a].fontSize=b)}static strokeSizeSelector(){var c=document.querySelector(`#t_font${virtualclass.gObj.currWb}`);null!=c&&(c.classList.remove("show"),c.classList.add("hide"));
c=document.querySelector(`#t_strk${virtualclass.gObj.currWb}`);null!=c&&(c.classList.remove("hide"),c.classList.add("show"))}static fontSizeSelector(c){c=c||virtualclass.gObj.currWb;const b=document.querySelector(`#t_strk${c}`);null!=b&&(b.classList.remove("show"),b.classList.add("hide"));c=document.querySelector(`#t_font${c}`);null!=c&&(c.classList.remove("hide"),c.classList.add("show"))}static themeColorShapes(c,b){c=c.split(/_doc_*/)[0];b=document.querySelector(`#tool_wrapper${b}.shapesToolbox`);
"t_line"===c||"t_circle"===c||"t_rectangle"===c||"t_triangle"===c?b.classList.add("active"):b.classList.remove("active")}static deleteActiveObject(c,b){b=virtualclass.wb[b];const a=b.canvas.getActiveObject();b.canvas.discardActiveObject();b.canvas.remove(a);c&&(c=virtualclass.wbWrapper.protocol.encode("da",virtualclass.gObj.currWb),WhiteboardMessage.send(c))}};class WhiteboardActiveAll{generateData(c,b,a){c=b.canvas.getPointer(c,!0);return virtualclass.wbWrapper.protocol.encode("ac",{event:a,x:c.x,y:c.y})}enable(c,b){c=b?virtualclass.wb[c].canvas.getObjects(b):virtualclass.wb[c].canvas.getObjects();for(b=0;b<c.length;b+=1)console.log("====> active all"),c[b].set("selectable",!0)}disable(c,b){c=b?virtualclass.wb[c].canvas.getObjects(b):virtualclass.wb[c].canvas.getObjects();for(b=0;b<c.length;b+=1)c[b].set("selectable",!1)}disableBut(c,b){c=virtualclass.wb[c].canvas.getObjects();
...
a.fontSize)&&this.updateText(a,b,c.target)}}isDefault(c){return c.text.trim()===this.placeHolder}afterSelected(c){console.log("selected 1");virtualclass.wbWrapper.gObj.textSelected=c}afterDeSelected(){console.log("deselected 1");virtualclass.wbWrapper.gObj.textSelected=null}mouseMove(){}mouseUp(){}};class Whiteboard{constructor(){this.selectedTool=this.canvas=null;this.rectangleObj=new WhiteboardRectangle("rectangle");this.activeAllObj=new WhiteboardActiveAll;this.freeDrawingObj=new WhiteboardFreeDrawing("freeDrawing");this.lineObj=new WhiteboardLine("line");this.triangleObj=new WhiteboardTriangle("triangle");this.circleObj=new WhiteboardCircle("circle");this.textObj=new WhiteboardText("text");this.gObj={}}init(c){this.wbId=c;delete virtualclass.wbWrapper.gObj.textSelected;this.textObj.textEditing=
...
virtualclass.gObj.wbData[a]=virtualclass.gObj.wbData[a].concat(b)},attachWhiteboardPopupHandler(b){c.addEventListener("mouseup",a=>{roles.hasControls()&&WhiteboardUtility.closeTray()})},setScreenShareDefualtColor(){var b=virtualclass.gObj.prvRequestScreenUser;virtualclass.gObj.prvRequestScreenUser&&(b=chatContainerEvent.elementFromShadowDom(`#ml${b} .icon-stdscreenImg`),null!==b&&(b.setAttribute("data-dcolor","black"),b.parentNode.setAttribute("data-title",virtualclass.lang.getString("requestScreenShare"))))},
