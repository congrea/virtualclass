...
b)},SharePresentation(b){let a,d;b.app&&(a=b.app);b.cusEvent&&(d=b.cusEvent);"object"===typeof this.ss&&(this.ss.prevStream=!1);this.sharePt=new c.sharePt;this.sharePt.init(a,d);this.previous=virtualclass.ptConfig.id;virtualclass.sharePt.attachMessageEvent("message",virtualclass.sharePt.pptMessageEventHandler)},Poll(){this.userListEnable();virtualclass.poll.init()},Quiz(){this.userListEnable();virtualclass.quiz.init()},Video(b,a,c){"undefined"!==typeof c&&null!=c?"undefined"===typeof c.type?virtualclass.videoUl.init(c):
virtualclass.videoUl.init():virtualclass.videoUl.init()},editor(b){b=b.app;this.user.control.toggleDisplayEditorController(b.substring(b.indexOf("virtualclass"),b.length),"block");var a=[];b===virtualclass.apps.er?(virtualclass.editorRich.init(0,a,"",""),this.previous=virtualclass.edConfig.id):(virtualclass.editorCode.init(0,a,"",""),this.previous=virtualclass.edCodeConfig.id);a=virtualclass.editorRich?virtualclass.editorRich.editorStatus:!1;if(roles.isStudent()){const d=virtualclass.vutil.smallizeFirstLetter(b);
a?this[d].cm.setOption("readOnly",!1):this[d].cm.setOption("readOnly","nocursor");this.user.control.toggleDisplayWriteModeMsgBox(b,a);virtualclass.vutil.setReadModeWhenTeacherIsDisConn(d)}},makeReadyDocumentShare(){if(Object.prototype.hasOwnProperty.call(virtualclass.dts.docs,"currDoc")){roles.hasControls()&&ioAdapter.mustSend({dts:{init:"studentlayout"},cf:"dts"});const b=virtualclass.dts.docs.currNote;"undefined"!==typeof b&&virtualclass.vutil.updateCurrentDoc(b)}virtualclass.previous=virtualclass.dtsConfig.id},
DocumentShare(b){let a;b.data&&(a=b.data);Object.prototype.hasOwnProperty.call(virtualclass,"dts")&&null!=virtualclass.dts?(virtualclass.dts.indexNav.init(),null!=a&&(a.slideTo?(b=document.getElementById(a.slideTo),virtualclass.dts.docs.note.getScreen(b)):!a.init||0===virtualclass.dts.docs.currNote||virtualclass.dts.pdfRender&&"object"===typeof virtualclass.dts.pdfRender[`_doc_${virtualclass.dts.docs.currNote}_${virtualclass.dts.docs.currNote}`]||(b=document.getElementById(`note${virtualclass.dts.docs.currNote}`),
...
c.classList.add("show"))}makeActiveTool(c,b){const a=document.getElementById(c);if(a){b=b||virtualclass.gObj.currWb;var d=document.querySelectorAll(`#commandToolsWrapper${b} .tool.active`);for(let b=0;b<d.length;b+=1)d[b].classList.remove("active");this.themeColorShapes(c,b);a.classList.add("active");localStorage.activeTool=a.id}}themeColorShapes(c,b){c=c.split(/_doc_*/)[0];b=document.querySelector(`#tool_wrapper${b}.shapesToolbox`);if(!b)debugger;"t_line"===c||"t_circle"===c||"t_rectangle"===c||
"t_triangle"===c?b.classList.add("active"):b.classList.remove("active")}handleActiviteTool(c){const b=localStorage.getItem("activeTool");null!==b&&-1<b.indexOf(c)&&(this.makeActiveTool(b,c),virtualclass.wb[c].selectedTool=b.split("_")[1],"activeall"!==virtualclass.wb[c].selectedTool&&virtualclass.wb[c].activeAllObj.disable(c))}};class WhiteboardActiveAll{generateData(c,b,a){c=b.canvas.getPointer(c,!0);return virtualclass.wbWrapper.protocol.encode("ac",{event:a,x:c.x,y:c.y})}enable(c,b){c=b?virtualclass.wb[c].canvas.getObjects(b):virtualclass.wb[c].canvas.getObjects();for(b=0;b<c.length;b+=1)console.log("====> active all"),c[b].set("selectable",!0)}disable(c,b){c=b?virtualclass.wb[c].canvas.getObjects(b):virtualclass.wb[c].canvas.getObjects();for(b=0;b<c.length;b+=1)c[b].set("selectable",!1)}disableBut(c,b){c=virtualclass.wb[c].canvas.getObjects();
...
(await this.initWhiteboardData(b.wbId),roles.hasControls()&&(console.log("====> invoke handle active tool"),virtualclass.wbWrapper.util.handleActiviteTool(b.wbId)));"object"===typeof this.shownPdf&&(io.globallock=!1,virtualclass.vutil.removeClass("virtualclassCont","resizeWindow"));this.firstTimeScroll&&roles.isStudent()&&"object"===typeof virtualclass.pdfRender[virtualclass.gObj.currWb]&&(virtualclass.pdfRender[virtualclass.gObj.currWb].setScrollPosition({scX:0,scY:0}),this.firstTimeScroll=!1)}},
async displayPage(b,a,c){a=await b.getPage(a);a.wbId=b.wbId;virtualclass.pdfRender[b.wbId].page=a;"undefined"!==typeof c?await this.renderPage(a,c):await this.renderPage(a,null)},async initWhiteboardData(b){"object"===typeof virtualclass.gObj.wbData[b]&&virtualclass.gObj.wbData[b]&&0<virtualclass.gObj.wbData[b].length&&await virtualclass.wbWrapper.util.replayFromLocalStroage(virtualclass.gObj.wbData[b],b)},async _zoom(b,a,c){virtualclass.vutil.setHeight(virtualclass.gObj.currWb,b,c);virtualclass.vutil.setWidth(virtualclass.gObj.currWb,
b,a);b=b.parentNode;c=virtualclass.vutil.getValueWithoutPixel(b.style.width);await virtualclass.pdfRender[virtualclass.gObj.currWb].displayPage(this.shownPdf,1);for(const a in virtualclass.pdfRender)this.zoomwhiteboardObjects(a);a>c&&b.classList.add("scrollX")},async innerFitToPage(b){if(" "!==this.shownPdf&&""!==this.shownPdf){await virtualclass.pdfRender[b].displayPage(this.shownPdf,1);for(const a in virtualclass.pdfRender)this.fitToScreenWhiteboardObjects(a)}},fitWhiteboardAtScale(b){if("object"===
typeof virtualclass.wb[b]){const {vcan:d}=virtualclass.wb[b];b=virtualclass.wb[b].canvas.getObjects();if(0<b.length)for(const d in b){var {scaleX:a}=b[d],{scaleY:c}=b[d];a=a/virtualclass.zoom.prvCanvasScale*virtualclass.zoom.canvasScale;c=c/virtualclass.zoom.prvCanvasScale*virtualclass.zoom.canvasScale;const f=b[d].x/b[d].scaleX*a,g=b[d].y/b[d].scaleY*c;b[d].scaleX=a;b[d].scaleY=c;b[d].x=f;b[d].y=g;b[d].setCoords()}d.renderAll()}},zoomwhiteboardObjects(b){virtualclass.wbWrapper.util.fitWhiteboardAtScale(b)},
zoomOutWhiteboardObjects(b){virtualclass.wbWrapper.util.fitWhiteboardAtScale(b)},fitToScreenWhiteboardObjects(b){virtualclass.wbWrapper.util.fitWhiteboardAtScale(b)},normalViewWhiteboardObjects(b){this.fitWhiteboardAtScale(b)},async _zoomOut(b,a,c){virtualclass.vutil.setHeight(virtualclass.gObj.currWb,b,c);virtualclass.vutil.setWidth(virtualclass.gObj.currWb,b,a);await virtualclass.pdfRender[virtualclass.gObj.currWb].displayPage(this.shownPdf,1);for(const a in virtualclass.pdfRender)this.zoomOutWhiteboardObjects(a)},
async _fitToScreen(b,a){virtualclass.vutil.setHeight(virtualclass.gObj.currWb,b,b.height);virtualclass.vutil.setWidth(virtualclass.gObj.currWb,b,a);b=b.parentNode;const c=b.offsetWidth;if(" "!==this.shownPdf){await virtualclass.pdfRender[virtualclass.gObj.currWb].displayPage(this.shownPdf,1);for(const a in virtualclass.pdfRender)this.fitToScreenWhiteboardObjects(a);a>c&&55<a-c?b.classList.add("scrollX"):b.classList.remove("scrollX")}roles.hasControls()&&(console.log("====> invoke handle active tool"),
virtualclass.wbWrapper.util.handleActiviteTool(this.shownPdf.wbId))},calculateScaleAtFirst(b,a){b=b.getViewport(a.width/b.getViewport(1).width);this.firstTime=!1;return b},isWhiteboardAlreadyExist(b){return null!=this.canvas},defaultScroll(){null!=virtualclass.gObj.currWb&&(this.canvasWrapper.scrollTop=1)}}}})(window);(function(c){c.wbCommon={order:[0],wbInd:0,initNavHandler(){document.querySelector("#virtualclassWhiteboard.whiteboard .next").addEventListener("click",virtualclass.vutil.navWhiteboard.bind(virtualclass.vutil,this,this.next));document.querySelector("#virtualclassWhiteboard.whiteboard .prev").addEventListener("click",virtualclass.vutil.navWhiteboard.bind(virtualclass.vutil,this,this.prev))},initNav(b){"undefined"===typeof this.indexNav&&(this.indexNav=new pageIndexNav("WB"));const a=virtualclass.gObj.currIndex||
