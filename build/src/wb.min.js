...
null!=g&&(f.classList.contains("active")||f.classList.add("active"),g.classList.remove("active"));null!=virtualclass.serverData&&(virtualclass.serverData.rawData={video:[],ppt:[],docs:[]},roles.hasAdmin()&&virtualclass.serverData.fetchAllData());virtualclass.gObj.wIds=[0];virtualclass.wbCommon.order=[0];virtualclass.gObj.wbCount=0;virtualclass.wbCommon.clearNavigation();delete virtualclass.wb[virtualclass.gObj.currWb].activeToolColor;virtualclass.gObj.currIndex=1}},dstdata:{async handleResult(b){for(;b;)b.value.hasOwnProperty("alldocs")&&
...
function(){virtualclass.vutil.navWhiteboard(a,a.prev);"object"===typeof virtualclass.wb[virtualclass.gObj.currWb]&&setTimeout(()=>{console.log("whiteboard zoom normal render");virtualclass.zoom.normalRender()},500)})},initNav(a){"undefined"===typeof this.indexNav&&(this.indexNav=new pageIndexNav("WB"));this.indexNav.init();if(roles.hasControls()){virtualclass.wbCommon.indexNav.createWbNavigationNumber(virtualclass.gObj.currIndex||a.length);virtualclass.wbCommon.indexNav.addActiveNavigation(virtualclass.gObj.currWb);
var b=localStorage.getItem("wbOrder");null!=b&&(b=JSON.parse(b),virtualclass.wbCommon.order=b);1<this.order.length&&(virtualclass.wbCommon.rearrangeNav(this.order),this.rearrange(this.order),b=localStorage.getItem("currIndex"),null!==b&&this.indexNav.createWbNavigationNumber(b))}else virtualclass.wbCommon.indexNav.studentWBPagination(virtualclass.gObj.currIndex||virtualclass.gObj.currSlide);virtualclass.wbCommon.indexNav.setTotalPages(a.length)},displaySlide(a){this.hideElement();const b=document.querySelector(`#note${a}`);
...
virtualclass.gObj.currIndex)}else d.message.hasOwnProperty("wbCount")&&(virtualclass.gObj.wbCount=d.message.wbCount,-1==virtualclass.gObj.wIds.indexOf(Number(virtualclass.gObj.wbCount))&&(virtualclass.gObj.wIds.push(virtualclass.gObj.wbCount),virtualclass.gObj.currIndex=d.message.currIndex,virtualclass.wbCommon.indexNav.studentWBPagination(virtualclass.gObj.currIndex)),localStorage.setItem("currIndex",virtualclass.gObj.currIndex))};this.reqscreen=function(d){"mobTab"==virtualclass.system.device||
"Chrome"!=virtualclass.system.mybrowser.name&&"Firefox"!=virtualclass.system.mybrowser.name&&"Edge"!=virtualclass.system.mybrowser.name?virtualclass.vutil.beforeSend({ext:!0,cf:"colorIndicator",nosupport:!0}):(d=virtualclass.lang.getString("stdscreenshare"),virtualclass.gObj.precheckScrn&&virtualclass.vutil.prechkScrnShare(),virtualclass.popup.confirmInput(d,c=>{c?(roles.isStudent()&&(virtualclass.gObj.studentSSstatus.mesharing=!0),virtualclass.makeAppReady("ScreenShare","byclick")):virtualclass.vutil.beforeSend({sd:!0,
cf:"colorIndicator"})}))};this.sshare_user=d=>{virtualclass.vutil.removeSSsharing();virtualclass.gObj.whoIsSharing=d.fromUser.userid;virtualclass.vutil.initssSharing(d.fromUser.userid)};this.sview=function(d){const c=document.getElementById("virtualclassScreenShareLocal");roles.isStudent()&&!virtualclass.gObj.studentSSstatus.mesharing&&null!=c&&(c.style.display="none");d.message.hasOwnProperty("firstSs")&&(virtualclass.gObj.studentSSstatus.sharing=!0);virtualclass.gObj.studentSSstatus.shareToAll=
!1;console.log("Share, self view")};this.sToAll=function(){const d=document.getElementById("virtualclassScreenShareLocal");null!=d&&(d.style.display="block");virtualclass.gObj.studentSSstatus.shareToAll=!0;virtualclass.gObj.studentSSstatus.sharing=!0;console.log("Share, to all")};this.rmStdScreen=function(d){virtualclass.vutil.initDefaultApp();virtualclass.vutil.beforeSend({unshareScreen:!0,st:"ss",cf:"unshareScreen"});"object"===typeof virtualclass.ss?virtualclass.ss.clearScreenShare():virtualclass.vutil.removeSSsharing()};
this.raiseHand=function(d){virtualclass.raiseHand.onMsgRec(d)};this.colorIndicator=function(d){var c=d.message;d=d.fromUser.userid;c.sd?(d=chatContainerEvent.elementFromShadowDom(`#ml${d} .icon-stdscreenImg`),null!=d&&d.setAttribute("data-dcolor","red")):c.ext&&(c=c.hasOwnProperty("nosupport")?"nosupport":"orange",d=chatContainerEvent.elementFromShadowDom(`#ml${d} .icon-stdscreenImg`),null!=d&&d.setAttribute("data-dcolor",c))};this.stdVideoCtrl=function(d){d.fromUser.userid!=virtualclass.gObj.uid&&
virtualclass.videoHost.stdVideoCtrlMsg(d)};this.toggleVideo=function(d){};this.destroyPlayer=function(d){if("Video"==virtualclass.currApp&&virtualclass.hasOwnProperty("videoUl")&&virtualclass.videoUl.videoUrl){virtualclass.videoUl.videoUrl="";virtualclass.videoUl.videoId="";(d=document.getElementById("dispVideo_Youtube_api"))&&d.contentWindow&&d.contentWindow.postMessage('{"event":"command","func":"pauseVideo","args":""}',"*");if(d=document.querySelector(".congrea #dispVideo"))d.style.display="none",
(d=document.querySelector(".congrea #dispVideo video"))&&d.setAttribute("src","");(d=document.querySelector("#listvideo .playing"))&&d.classList.remove("playing");(d=document.querySelector("#listvideo .removeCtr"))&&d.classList.remove("removeCtr");"object"===typeof virtualclass.videoUl.player&&delete virtualclass.videoUl.player}};this.sync=function(){};this.recs=function(d){if(!virtualclass.isPlayMode)virtualclass.settings.onMessage(d.message)}};(function(d){function c(){}c.prototype.loadData=function(a){a.earlierWidth=a.innerWidth;a.earlierHeight=a.innerHeight;a.wbUser=wbUser;a.pageEnter=(new Date).getTime();const b=new a.virtualclass;b.config.data={};b.virtualclassIDBOpen=a.virtualclassOpenDB;a.virtualclass=b;b.gObj.displayError=1;b.lang={};b.lang.getString=a.getString;b.lang.message=a.message;b.createMainContainer();b.gObj.sessionClear=!1;b.settings=a.settings;b.settings.init();b.handleCurrentUserWithPrevious();b.ioEventApi=ioEventApi;
