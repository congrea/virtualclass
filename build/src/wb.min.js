...
"enable":"disable",enable:"disable",disable:"enable"};var k=c.querySelector(`.congrea #contr${b}All.${d.action}`);k&&(k.classList.remove(d.action),k.classList.add(d[d.action]),(k=c.querySelector(`.congrea #contr${b}All input`))&&k.removeAttribute("checked"));if(k=c.querySelector(`.congrea .slider.icon-all-${a}-${d.action}`))k.className=`slider round congtooltip icon-all-${a}-${d[d.action]}`,k.setAttribute("data-action",d[d.action]),k.setAttribute("data-title",virtualclass.lang.getString(`${d[d.action]}All${b}`))},
...
delete virtualclass.videoUl.player}};this.sync=function(){};this.wbData=function(d){console.log(d);d.message.wbIndex&&(virtualclass.gObj.currIndex=parseInt(d.message.wbIndex),virtualclass.wbCommon.indexNav.studentWBPagination(d.message.wbIndex-1))};this.recs=function(d){if(!virtualclass.isPlayMode)virtualclass.settings.onMessage(d.message)}};$.uiBackCompat=!1;(function(d){d.onload=async function(){const c=new d.Bootstrap;await c.loadData(d);await c.appInit();await c.setUpData(d);await c.readyToGo()}})(window);
