...
"Can not add ICE candidate without a remote description"));else throw new TypeError("sdpMLineIndex or sdpMid required");else for(d=0;d<this.transceivers.length&&(this.transceivers[d].isDatachannel||(this.transceivers[d].iceTransport.addRemoteCandidate({}),a=p.splitSections(this.remoteDescription.sdp),a[d+1]+="a=end-of-candidates\r\n",this.remoteDescription.sdp=a.join(""),!this.usingBundle));d++);return Promise.resolve()};d.prototype.getStats=function(){const a=[];this.transceivers.forEach(d=>{["rtpSender",
"rtpReceiver","iceGatherer","iceTransport","dtlsTransport"].forEach(c=>{d[c]&&a.push(d[c].getStats())})});return new Promise(d=>{const c=new Map;Promise.all(a).then(a=>{a.forEach(a=>{Object.keys(a).forEach(d=>{var f=a[d];a[d].type={inboundrtp:"inbound-rtp",outboundrtp:"outbound-rtp",candidatepair:"candidate-pair",localcandidate:"local-candidate",remotecandidate:"remote-candidate"}[f.type]||f.type;c.set(d,a[d])})});d(c)})})};let f=["createOffer","createAnswer"];f.forEach(a=>{const c=d.prototype[a];
d.prototype[a]=function(){const a=arguments;return"function"===typeof a[0]||"function"===typeof a[1]?c.apply(this,[arguments[2]]).then(d=>{"function"===typeof a[0]&&a[0].apply(null,[d])},d=>{"function"===typeof a[1]&&a[1].apply(null,[d])}):c.apply(this,arguments)}});f=["setLocalDescription","setRemoteDescription","addIceCandidate"];f.forEach(a=>{const c=d.prototype[a];d.prototype[a]=function(){const a=arguments;return"function"===typeof a[1]||"function"===typeof a[2]?c.apply(this,arguments).then(()=>
{"function"===typeof a[1]&&a[1].apply(null)},d=>{"function"===typeof a[2]&&a[2].apply(null,[d])}):c.apply(this,arguments)}});["getStats"].forEach(a=>{const c=d.prototype[a];d.prototype[a]=function(){const a=arguments;return"function"===typeof a[1]?c.apply(this,arguments).then(()=>{"function"===typeof a[1]&&a[1].apply(null)}):c.apply(this,arguments)}});return d}},{sdp:2}],2:[function(a,d,c){const f={generateIdentifier:function(){return Math.random().toString(36).substr(2,10)}};f.localCName=f.generateIdentifier();
...
bandwidth:{prev:"browser",curr:"bandwidth",next:"speaker",perform(){virtualclass.precheck.display(`#preCheckcontainer .precheck.${this.curr}`);const c=`#preCheckcontainer .precheck.${this.curr} .result`;this.imageAddr="https://dl.congrea.com/bandwidth.jpg";this.downloadSize=1E6;this.measureConnectionSpeed(b=>{document.querySelector(c).innerHTML="";virtualclass.precheck.createMessage(c,b,"information");virtualclass.precheck.initHandler(`${"#preCheckcontainer .precheck"} #${this.curr}Buttons .prev`,
this.curr);virtualclass.precheck.initHandler(`${"#preCheckcontainer .precheck"} #${this.curr}Buttons .next`,this.curr)});virtualclass.precheck.session=virtualclass.media.sessionConstraints().session;null==virtualclass.adpt&&(virtualclass.adpt=new virtualclass.adapter);virtualclass.precheck.cNavigator=virtualclass.adpt.init(navigator);virtualclass.precheck.cNavigator.mediaDevices.getUserMedia(virtualclass.precheck.session).then(b=>{virtualclass.precheck.mediaStream=b;0<virtualclass.precheck.mediaStream.getVideoTracks().length&&
(virtualclass.precheck.cameraStreamStatus=1,"webcam"===virtualclass.precheck.currTest&&(document.getElementById("webcamTempVideo").classList.remove("novideo"),b=document.querySelector("#vcWebCamCheck .result .error"),null!=b&&(b.classList.remove("error"),b.classList.add("general"),b.innerHTML=virtualclass.lang.getString("webcamerainfo")),virtualclass.precheck.webcam.createVideo()))}).catch(function(b){virtualclass.media.handleUserMediaError(b);const [a,c]=virtualclass.precheck.webcam.currentStatus();
b=`#preCheckcontainer .precheck.${virtualclass.precheck.webcam.curr}`;const f=document.querySelector(`${b} .msg`);null!==f&&f.parentNode.removeChild(f);virtualclass.precheck.webcam.displayMessage(b,c,a);virtualclass.precheck.cameraStreamStatus=0})},bandWidthInWords(c){600<c?(c="high",virtualclass.videoHost.gObj.MYSPEED=1):400<c?(c="medium",virtualclass.videoHost.gObj.MYSPEED=2):(virtualclass.videoHost.gObj.MYSPEED=3,c="low");ioAdapter.sendSpeed(virtualclass.videoHost.gObj.MYSPEED);return c},measureConnectionSpeed(c){const b=
...
virtualclass.videoHost.UI.hideTeacherVideo());else if(c=JSON.parse(localStorage.getItem("stdVideoSwitch")))virtualclass.connectedUsers&&virtualclass.connectedUsers.length?virtualclass.videoHost.setUserIcon(virtualclass.gObj.uid):virtualclass.gObj.delayVid="display",virtualclass.videoHost.UI.hideTeacherVideo()},renderSelfVideo(c){"undefined"===typeof virtualclass.media.tempStream&&(console.log("Render self video"),this.getMediaStream(c))},isDomReady(c){const b=this;this.domReady?(null!=this.domreadyCheck&&
clearTimeout(this.domreadyCheck),c()):this.domreadyCheck=setTimeout(()=>{b.isDomReady(c)},1E3)},_init(){this.setCanvasAttr("vidHost","videoHost");this.setCanvasAttr("vidHostSlice","videoHostSlice");this.vidHostSlice.globalAlpha=.5;this.vidHostSlice.globalCompositeOperation="multiply"},onmessage(c){"on"===c?(virtualclass.videoHost.gObj.videoSwitch=1,this.UI.displayTeacherVideo(),localStorage.tvideo="show"):"off"===c&&(virtualclass.videoHost.gObj.videoSwitch=0,this.UI.hideTeacherVideo(),localStorage.tvideo=
"hide")},stdVideoCtrlMsg(c){const {userid:b}=c.fromUser;c.message.stdVideoCtr.videoSwitch?this.setUserIcon(b):this.removeUserIcon(b)},toggleVideoMsg(c){const b=document.querySelector("#congCtrBar");"enable"===c?(b.style.pointerEvents="visible",b.style.opacity="1",virtualclass.videoHost.gObj.allStdVideoOff=!1):(b.style.pointerEvents="none",b.style.opacity="0.5",virtualclass.videoHost.gObj.allStdVideoOff=!0)},toggleStdVideoIcon(c){const b=document.querySelector(".congrea .videoSwitchCont"),a=document.querySelector(".congrea #rightCtlr #videoSwitch");
...
b.video.myVideo=chatContainerEvent.elementFromShadowDom(`#video${virtualclass.gObj.uid}`),b.video.myVideo.muted=!0,virtualclass.adpt.attachMediaStream(b.video.myVideo,b.stream))},fromLocalStorage(){let c="";if(roles.hasControls())c=localStorage.getItem("videoSwitch"),localStorage.removeItem("videoSwitch");else{var b=localStorage.getItem("stdVideoSwitch");b=null!=b&&"undefined"!=b?JSON.parse(b):!1;localStorage.removeItem("stdVideoSwitch");var a=localStorage.getItem("allStdVideoOff");a=null!=a&&"undefined"!=
a?JSON.parse(a):!1;virtualclass.videoHost.gObj.allStdVideoOff=a;localStorage.removeItem("stdVideoSwitch")}if("undefined"!==typeof c&&c)if(virtualclass.videoHost.gObj.videoSwitch=+c,roles.hasControls()){if(a=document.getElementById("videoSwitch"))+c?a.classList.contains("off")&&(a.classList.add("on"),a.classList.remove("off")):a.classList.contains("on")&&(a.classList.add("off"),a.classList.remove("on"),virtualclass.gObj.meetingMode&&virtualclass.multiVideo.setVideoStatus(!1))}else virtualclass.gObj.meetingMode||
