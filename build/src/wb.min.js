...
0<virtualclass.recorder.totalRecordingFiles.length){const b=virtualclass.recorder.totalRecordingFiles.shift();virtualclass.recorder.requestDataFromServer(b,a)}}else console.log(`requested file ${c}`),a.get(`https://recording.congrea.net/${wbUser.lkey}/${wbUser.room}/${virtualclass.recorder.session}/${c}`).then(b=>{virtualclass.recorder.afterDownloading(c,b.data,a)}).catch(()=>{setTimeout(()=>{virtualclass.recorder.requestDataFromServer(c,a)},1E3)})},afterDownloading(c,a,b){if(0<virtualclass.recorder.totalRecordingFiles.length){const a=
...
"Audio":"Video",d={action:("audio"==a?virtualclass.gObj.stdaudioEnable:virtualclass.gObj.stdvideoEnable)?"enable":"disable",enable:"disable",disable:"enable"};var k=c.querySelector(`.congrea #contr${b}All.${d.action}`);k&&(k.classList.remove(d.action),k.classList.add(d[d.action]),(k=c.querySelector(`.congrea #contr${b}All input`))&&k.removeAttribute("checked"));if(k=c.querySelector(`.congrea .slider.icon-all-${a}-${d.action}`))k.className=`slider round congtooltip icon-all-${a}-${d[d.action]}`,k.setAttribute("data-action",
d[d.action]),k.setAttribute("data-title",virtualclass.lang.getString(`${d[d.action]}All${b}`))},makeElemEnable(a){virtualclass.vutil.elemHasAnyClass(a.id)?(a.classList.remove("disable"),a.classList.add("enable")):a.className="enable";a=a.getElementsByClassName("ui-chatbox-input-box")[0];null!=a&&(a.disabled=!1)},toggleAllEditorController(a,c){a=virtualclass.vutil.smallizeFirstLetter(a);const b=chatContainerEvent.elementFromShadowDom(".controleCont","all");if(0<b.length)for(let d=0;d<b.length;d++)if(0<
...
"0";let b;for(b in this.allNotes)-1<this.allNotes[b].id.indexOf(a)&&this._removePageFromStructure(this.allNotes[b].id)},updateInAllDocs(a){var b=a.split("_")[0];b=this.allDocs[b];if("undefined"!==typeof b.notes[a]){b.notes[a].deletedn=a;for(let c=0;c<b.notesarr.length;c++)b.notesarr[c].id==a&&(b.notesarr[c].deletedn=a)}else console.log(`The note with the id ${a} is not available`)},_removePageFromStructure(a){this.removeWhiteboardFromStorage(`_doc_${a}_${a}`);this.allNotes[a].deletedn=a;this.storeInDocs(this.allNotes);
...
this.axhr.get(b.pdf).then(a=>{virtualclass.pdfRender[virtualclass.gObj.currWb].afterPdfPrefetch(b.id,a.data)}).catch(()=>{setTimeout(()=>{virtualclass.pdfRender[virtualclass.gObj.currWb].prefechPdf(a)},1E3)})},afterPdfPrefetch(a,b){virtualclass.gObj.next={};virtualclass.gObj.next[a]=b},async loadPdf(a,b,c){virtualclass.gObj.hasOwnProperty("getDocumentTimeout")&&clearTimeout(virtualclass.gObj.getDocumentTimeout);null==virtualclass.gObj.getDocumentTimer||0==virtualclass.gObj.getDocumentTimer?(this._loadPdf(a,
b,c),virtualclass.gObj.getDocumentTimer=!0,virtualclass.gObj.getDocumentTimeout=setTimeout(()=>{virtualclass.gObj.getDocumentTimer=!1},1E3)):virtualclass.gObj.getDocumentTimeout=setTimeout(()=>{this._loadPdf(a,b,c);virtualclass.gObj.getDocumentTimer=!1},1E3)},_loadPdf(a,b,c){virtualclass.gObj.next.hasOwnProperty(c)?this.afterPdfLoad(b,c,virtualclass.gObj.next[c]):this.axhr.get(a).then(a=>{virtualclass.pdfRender[virtualclass.gObj.currWb].afterPdfLoad(b,c,a.data)}).catch(()=>{setTimeout(()=>{virtualclass.pdfRender[virtualclass.gObj.currWb]._loadPdf(a,
b,c)},1E3)})},async afterPdfLoad(a,b,c){this.canvasWrapper=document.querySelector(`#canvasWrapper${virtualclass.gObj.currWb}`);this.canvas=a;a={};a.data=c;null!=virtualclass.gObj.myworker&&(a.worker=virtualclass.gObj.myworker);const d=this;c=virtualclass.gObj.currWb;virtualclass.currApp==virtualclass.currApp?(d.wbId="undefined"===typeof b?virtualclass.gObj.currWb:b,console.log(`-----------START ${virtualclass.currApp}----------`),console.log("PDF render request to pdf.js 1"),PDFJS.workerSrc=`${whiteboardPath}build/src/pdf.worker.min.js`,
...
this.sdxhr.post(d,{}).then(c=>{virtualclass.serverData.afterResponse(c.data)}).catch(()=>{setTimeout(()=>{virtualclass.serverData.requestData(d)},1E3)})},afterResponse(d){this.awsUrlArr(d);virtualclass.gObj.fetchedData=!0;"undefined"!==typeof virtualclass.serverData.cb&&virtualclass.serverData.cb()},awsUrlArr(d){this.rawData={video:[],ppt:[],docs:[]};const c=[];d=d.Items;for(let q=0;q<d.length;q++)if(d[q].hasOwnProperty("filetype"))switch(d[q].filetype.S){case "doc":var a=this.processObj(d[q]);var b=
