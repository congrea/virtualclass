...
!0}},initPlay(){Object.prototype.hasOwnProperty.call(this,"Html5AudioRec")||this.Html5AudioRec||(this.Html5AudioRec={audioContext:new (c.AudioContext||c.webkitAudioContext)});"object"!==typeof u&&g.audio.Html5AudioRec.audioContext.audioWorklet.addModule(workletAudioRecBlob).then(()=>{if("undefined"===typeof p){u=new AudioWorkletNode(g.audio.Html5AudioRec.audioContext,"worklet-audio-rec");g.audio.Html5AudioRec.MediaStreamDest=g.audio.Html5AudioRec.audioContext.createMediaStreamDestination();u.connect(g.audio.Html5AudioRec.audioContext.destination);
virtualclass.gObj.workletAudioRec=u;"Chrome"===virtualclass.system.mybrowser.name&&g.audio.bug_687574_callLocalPeers();var a=new MessageChannel;workerIO.postMessage({cmd:"workerAudioRec"},[a.port1]);workerAudioRec.postMessage({cmd:"workerIO",sampleRate:g.audio.Html5Audio.audioContext.sampleRate},[a.port2]);a=new MessageChannel;workerAudioRec.postMessage({cmd:"workletAudioRec"},[a.port1]);u.port.postMessage({cmd:"workerAudioRec"},[a.port2]);workerAudioRec.postMessage({cmd:"audioWorklet",msg:!0});p=
!0;virtualclass.gObj.audioRecWorkerReady=!0}})},playWithFallback(a){"suspended"===this.Html5Audio.audioContext.state?Object.prototype.hasOwnProperty.call(this,"audioSuspendTime")||(this.audioSuspendTime=setTimeout(()=>{"suspended"===this.Html5Audio.audioContext.state&&(this.snode.push(a),null===virtualclass.gObj.requestToScriptNode&&(this.Html5Audio.audioContext.resume(),virtualclass.gesture.initAudioResume(a),virtualclass.gObj.requestToScriptNode=!0),delete this.audioSuspendTime)},2E3)):(this.innerPlayWithFallback(),
this.audioSuspendTime&&delete this.audioSuspendTime)},innerPlayWithFallback(){const a=this;if(null===virtualclass.media.audioPlayerNode||"closed"===virtualclass.media.audioPlayerNode.context.state)null!==virtualclass.media.audioPlayerNode&&virtualclass.media.audioPlayerNode.disconnect(),virtualclass.media.audioPlayerNode=this.Html5Audio.audioContext.createScriptProcessor(4096,1,1),A=0,virtualclass.media.audioPlayerNode.onaudioprocess=function(b){b=b.outputBuffer.getChannelData(0);var d=a.getMergedAudio();
if(null!==d&&void 0!==d){for(let a=0;a<d.length;a++)b[a]=d[a];A=d[4095]}else for(d=0;d<b.length;d++)b[d]=A},virtualclass.media.audioPlayerNode.connect(this.Html5Audio.audioContext.destination)},queueWithFalback(a,b){null==k[b]&&(k[b]=[]);null==n[b]&&(n[b]=[]);for(let d=0;d<a.length;d++)n[b].push(a[d]);for(;4096<=n[b].length;)a=n[b].splice(0,4096),k[b].push(new Float32Array(a))},getMergedAudio(){t=[];z=0;for(r in k){var a=this.getAudioChunks(r);if(null!=a)if(z++,1===z)for(var b=0;4096>b;b++)t[b]=a[b];
else for(b=0;4096>b;b++)t[b]+=a[b]}if(1===z)return t;if(1<z){for(a=0;4096>a;a++)t[a]/=z;return t}},getAudioChunks(a){if(null!==k){if(9<=k[a].length){for(;3<=k[a].length;)k[a].shift();m[a]=!0;return k[a].shift()}if(2<=k[a].length||0<k[a].length&&!0===m[a])return m[a]=!0,k[a].shift();m[a]=!1;0===k[a].length&&delete k[a]}},actualManiPulateStream(){this.triggermaniPulateStream=!0;const a=virtualclass.media;setTimeout(()=>{a.detectAudioWorklet()?a.audio.maniPulateStream():a.audio.maniPulateStreamWithFallback()},
1E3)},maniPulateStream(){const {stream:a}=g;"undefined"!==typeof v&&v.disconnect();"undefined"!==typeof a&&null!=a&&g.audio.Html5Audio.audioContext.audioWorklet.addModule(workletAudioSendBlob).then(()=>{var d=g.audio.Html5Audio.audioContext.createMediaStreamSource(a);const c=g.audio.Html5Audio.audioContext.createBiquadFilter();c.type="lowpass";c.frequency.value=2E3;d.connect(c);v=new AudioWorkletNode(g.audio.Html5Audio.audioContext,"worklet-audio-send");v.onprocessorerror=function(a){g.audio.notifiyMuteAudio()};
...
{g.audio.notifiyMuteAudio()})},maniPulateStreamWithFallback(){if("undefined"!==typeof g.stream&&null!=g.stream){var {stream:a}=g;a=g.audio.Html5Audio.audioContext.createMediaStreamSource(a);g.audio.bufferSize=4096;virtualclass.media.audioCreatorNode=g.audio.Html5Audio.audioContext.createScriptProcessor(g.audio.bufferSize,1,1);virtualclass.media.audioCreatorNode.onaudioprocess=g.audio.recorderProcessFallback.bind(g.audio);const b=g.audio.Html5Audio.audioContext.createBiquadFilter();b.type="lowpass";
b.frequency.value=2E3;a.connect(b);b.connect(virtualclass.media.audioCreatorNode);virtualclass.media.audioCreatorNode.connect(g.audio.Html5Audio.audioContext.destination);a=new MessageChannel;workerAudioSend.postMessage({cmd:"workerIO",uid:virtualclass.gObj.uid,sampleRate:g.audio.Html5Audio.audioContext.sampleRate},[a.port1]);workerIO.postMessage({cmd:"workerAudioSend"},[a.port2]);g.workerAudioSendOnmessage()}},updateInfo(){this.audioStreamArr=[];b=this.recordAudio=!1},extractData(a){a=new Int8Array(a);
return[virtualclass.vutil.numValidateFour(a[1],a[2],a[3],a[4]),a.subarray(5,a.length)]},removeAudioFromLocalStorage(){localStorage.removeItem("audEnable")},async bug_687574_callLocalPeers(){function a(a,b){if("connected"===b.currentTarget.connectionState)try{u.disconnect(g.audio.Html5AudioRec.audioContext.destination),u.connect(g.audio.Html5AudioRec.MediaStreamDest)}catch(O){}else if("disconnected"===b.currentTarget.connectionState){k.close();m.close();m=k=null;try{u.disconnect(g.audio.Html5AudioRec.MediaStreamDest),
u.connect(g.audio.Html5AudioRec.audioContext.destination)}catch(O){}g.audio.bug_687574_callLocalPeers()}}async function b(a){try{await k.setLocalDescription(a),await m.setRemoteDescription(a)}catch(N){f()}try{const a=await m.createAnswer();await d(a)}catch(N){f()}}async function d(a){try{await m.setLocalDescription(a),await k.setRemoteDescription(a)}catch(N){f()}}async function c(a,b){if(b.candidate&&"host"===b.candidate.type)try{await (a===k?m:k).addIceCandidate(b.candidate)}catch(O){f()}}function f(){try{u.connect(g.audio.Html5Audio.audioContext.destination),
k.close(),m.close(),m=k=null}catch(I){m=k=null}}let k,m;k=new RTCPeerConnection;k.count=0;k.addEventListener("icecandidate",a=>c(k,a));k.addEventListener("connectionstatechange",b=>a(k,b));m=new RTCPeerConnection;m.count=0;m.addEventListener("icecandidate",a=>c(m,a));m.addEventListener("connectionstatechange",b=>a(m,b));m.addEventListener("track",function(a){const b=document.createElement("audio");b.srcObject=a.streams[0];b.autoplay=!0});g.audio.Html5AudioRec.MediaStreamDest.stream.getTracks().forEach(a=>
k.addTrack(a,g.audio.Html5AudioRec.MediaStreamDest.stream));try{const a=await k.createOffer();await b(a)}catch(I){f()}}},video:{width:75,height:56,tempVid:"",tempVidCont:"",myVideo:"",remoteVid:"",remoteVidCont:"",maxHeight:250,init(){this.videoCont=document.getElementById("allVideosCont");null!=this.videoCont&&(this.videoCont.style.maxHeight=`${this.maxHeight}px`)},calcDimension(){this.myVideo.width=this.width;this.myVideo.height=this.height},removeUser(a){a=document.getElementById(`user${a}`);null!=
a&&a.parentNode.removeChild(a)},createElement(a){const b=document.createElement("div");b.className="videoWrapper";const c=document.createElement("div");c.className="videoSubWrapper";c.id=`user${a.id}`;b.appendChild(c);d=a.id;const f=document.createElement("canvas");f.id=`video${d}`;f.className="userVideos";f.width=this.width;f.height=this.height;f.muted="muted";c.appendChild(f);virtualclass.media.util.imageReplaceWithVideo(a.id,b)},updateHightInSideBar(a){const b=document.getElementById("widgetRightSide").offsetHeight;
document.getElementById("chatContainer").style.height=`${b-a}px`},sendInBinary(a){io.webSocketConnected()&&ioAdapter.sendBinary(a)},send(){function a(){b.tempVidCont.clearRect(0,0,b.tempVid.width,b.tempVid.height);b.tempVidCont.drawImage(b.myVideo,0,0,b.width,b.height);roles.hasControls();virtualclass.system.webpSupport?(d=b.tempVid.toDataURL("image/webp",.6),c=1):(d=b.tempVid.toDataURL("image/jpeg",.3),c=0);d=virtualclass.videoHost.convertDataURIToBinary(d);if(!virtualclass.videoHost.gObj.stdStopSmallVid&&
(!roles.hasControls()||roles.hasControls()&&virtualclass.videoHost.gObj.videoSwitch)){var m=virtualclass.gObj.uid.toString();for(var n=m.length;8>n;n++)m=`0${m}`;m=m.match(/[\S]{1,2}/g)||[];m=new Uint8ClampedArray([11,m[0],m[1],m[2],m[3],c]);n=new Uint8ClampedArray(d.length+m.length);n.set(m);n.set(d,m.length);k.sendInBinary(n)}clearInterval(virtualclass.media.smallVid);if(Object.prototype.hasOwnProperty.call(virtualclass,"connectedUsers"))if(m=f+2500*virtualclass.connectedUsers.length,g!==virtualclass.connectedUsers.length){g=
virtualclass.connectedUsers.length;n=-1;for(var r=0;r<virtualclass.connectedUsers.length;r++)virtualclass.connectedUsers[0].userid===virtualclass.gObj.uid&&(n=r);r=m/g;0>n&&(n=0);virtualclass.media.smallVid=setInterval(a,m+n*r)}else virtualclass.media.smallVid=setInterval(a,m)}Object.prototype.hasOwnProperty.call(virtualclass.media,"smallVid")&&clearInterval(virtualclass.media.smallVid);const b=this;let d,c;f=Math.floor(5001*Math.random()+3E3);let g=-1;const k=this;virtualclass.media.smallVid=setInterval(a,
f)},startToStream(){g.video.calcDimension();g.video.send()},playWithoutSlice(a,b,d){this.remoteVid=document.getElementById(`video${a}`);null!=this.remoteVid&&(this.remoteVidCont=this.remoteVid.getContext("2d"),this.remoteVidCont.putImageData(imgData,0,0))},drawReceivedImage(a,b,d,c){this.remoteVid=chatContainerEvent.elementFromShadowDom(`#video${c}`);if(null!=this.remoteVid)if(this.remoteVidCont=this.remoteVid.getContext("2d"),virtualclass.system.webpSupport||"jpeg"===b){const b=new Image,c=this;
b.onload=function(){c.remoteVidCont.drawImage(b,d.x,d.y)};b.src=a}else loadfile(a,this.remoteVid,this.videoPartCont)},justForDemo(){let a=0,b=document.getElementById("allVideosCont");b.style.maxHeight="250px";setInterval(()=>{if(20>=++a){b=document.getElementById("allVideosCont");const d=document.createElement("div");d.className="videoWrapper";const c=document.createElement("div");c.className="videoSubWrapper";c.setAttribute("data-uname",+a);d.appendChild(c);const f=document.createElement("video");
f.className="userVideo";f.muted="muted";f.src="http://html5demos.com/assets/dizzy.mp4";c.appendChild(f);b.appendChild(d);250<=b.offsetHeight&&null!=b.style.overflowY&&"scroll"!==b.style.overflowY&&(b.style.overflowY="scroll",document.getElementById(virtualclass.gObj.chat.mainChatBoxId).style.borderTop="3px solid #bbb")}},200)},createVideoElement(){const a=document.createElement("div");a.className="videoWrapper";const b=document.createElement("div");b.className="videoSubWrapper";a.appendChild(b);const d=
document.createElement("video");d.id=`video${virtualclass.gObj.uid}`;d.autoplay=!0;b.appendChild(d);return a},insertTempVideo(a){const b=document.createElement("canvas");b.id="tempVideo";a.parentNode.insertBefore(b,a)},tempVideoInit(){g.video.tempVid=chatContainerEvent.elementFromShadowDom("#tempVideo");g.video.tempVid.width=g.video.width;g.video.tempVid.height=g.video.height;g.video.tempVidCont=g.video.tempVid.getContext("2d")},process(a){var b=new Uint8ClampedArray(a);a=virtualclass.vutil.numValidateFour(b[1],
b[2],b[3],b[4]);var d={id:a};virtualclass.media.existVideoContainer(d)||virtualclass.media.video.createElement(d);d=b.subarray(6,b.length);1===b[5]?(b=`data:image/webp;base64,${btoa(virtualclass.videoHost.Uint8ToString(d))}`,d="webp"):(b=`data:image/jpeg;base64,${btoa(virtualclass.videoHost.Uint8ToString(d))}`,d="jpeg");virtualclass.media.video.drawReceivedImage(b,d,{x:0,y:0},a)}},sessionConstraints(){let a=!!virtualclass.system.mediaDevices.hasWebcam;virtualclass.gObj.meetingMode&&a&&(a="mobTab"===
...
let a,b;[a,b]=this.sessionConstraints();g.video.init();virtualclass.user.control.audioDisable();virtualclass.user.control.videoDisable();if(!virtualclass.vutil.isPlayMode()){null==virtualclass.adpt&&(virtualclass.adpt=new virtualclass.adapter);if(null!=virtualclass.media.video.tempStream){var d=virtualclass.media.video.tempStream.getTracks();for(var c=0;c<d.length;c++)d[c].stop()}d=virtualclass.adpt.init(navigator);c=null;try{c=await d.mediaDevices.getUserMedia(b)}catch(B){this.handleUserMediaError(B),
this.audio.notifiyMuteAudio()}null!==c&&this.handleUserMedia(c)}virtualclass.system.wbRtc.peerCon&&"undefined"===typeof localStorage.wbrtcMsg&&(virtualclass.view.multiMediaMsg("WebRtc"),localStorage.wbrtcMsg=!0);!1===a&&(virtualclass.user.control.videoDisable(),virtualclass.vutil.addClass("virtualclassCont","nowebcam"))},handleUserMedia(a){localStorage.removeItem("dvid");var b=localStorage.getItem("audEnable");if(roles.isStudent()&&virtualclass.system.mediaDevices.hasMicrophone)virtualclass.settings.userAudioIcon();
else if(virtualclass.system.mediaDevices.hasMicrophone)if(null!=b)if(b=JSON.parse(b),"false"===b.ac||!1===b.ac)virtualclass.gObj.audioEnable=!1,virtualclass.user.control.audioDisable(!0);else{if("true"===b.ac||!0===b.ac)virtualclass.gObj.audioEnable=!0,virtualclass.user.control.audioWidgetEnable(!0)}else"undefined"!==typeof a&&virtualclass.user.control.audioWidgetEnable(!0);else virtualclass.user.control.audioDisable();b=a.getVideoTracks()[0];"undefined"!==typeof b?b.onended=function(){if(roles.hasControls()){virtualclass.videoHost.clearTeacherVideoTime();
virtualclass.system.mediaDevices.webcamErr.push("webcambusy");const a=document.getElementById("videoHostContainer");null!==a&&a.classList.add("displayInterrupt");ioAdapter.mustSend({cf:"videoStop"})}}:virtualclass.system.mediaDevices.webcamErr.push("nopermission");g.video.tempStream=a;g.audio.init();g.audio.attachAudioStopHandler(a);b=chatContainerEvent.elementFromShadowDom(`#ml${virtualclass.gObj.uid}`);null!=b&&null!=b.getElementsByTagName("video")&&g.innerHandleUserMedia(virtualclass.gObj.uid);
roles.hasAdmin()&&virtualclass.videoHost.isDomReady(()=>{virtualclass.videoHost.renderSelfVideo(a)});virtualclass.settings.userVideoIcon();null==localStorage.getItem("prevApp")&&(roles.hasControls()?virtualclass.vutil.videoHandler("off"===virtualclass.vutil.selfVideoStatus()?"on":"off",!0):virtualclass.gObj.meetingMode&&virtualclass.vutil.videoHandler("off"));g.audio.audioContextReady&&!Object.prototype.hasOwnProperty.call(g.audio,"triggermaniPulateStream")&&(g.stream=g.video.tempStream,g.audio.actualManiPulateStream())},
addUserRole(a,b){let d;a.dataset.role=b;"undefined"!==typeof b?(d="s"===b?"student":"teacher",a.classList.add(d)):a.classList.add("student");"t"===virtualclass.gObj.uRole&&"student"===d&&document.getElementById("virtualclassCont").classList.remove("student")},innerHandleUserMedia(a){if("undefined"!==typeof g&&(a=g.video.tempStream,"undefined"!==typeof a&&virtualclass.system.mediaDevices.hasWebcam)){const b=g.video.createVideoElement();virtualclass.media.util.imageReplaceWithVideo(virtualclass.gObj.uid,
b);g.video.insertTempVideo(b);g.video.tempVideoInit();g.video.myVideo=chatContainerEvent.elementFromShadowDom(`#video${virtualclass.gObj.uid}`);virtualclass.adpt.attachMediaStream(g.video.myVideo,a);g.video.myVideo.muted=!0;g.stream=g.video.tempStream;g.video.myVideo.onloadedmetadata=function(){g.video.startToStream()}}},updateVideoContHeight(){var a=document.getElementById("virtualclassCont");virtualclass.vutil.getElementOffset(a);a=virtualclass.chat.boxHeight;const b=document.getElementById("chat_div");
b.scrollHeight>=a+1&&(b.style.overflowY="scroll");b.style.maxHeight=`${a}px`;virtualclass.isPlayMode&&(b.style.maxHeight=`${a+64}px`)},close(){Object.prototype.hasOwnProperty.call(virtualclass.media,"smallVid")&&clearInterval(virtualclass.media.smallVid)},dispAllVideo(a){setTimeout(()=>{var b=document.getElementById(a);if(null!=b){b=b.getElementsByTagName("video");for(let a=0;a<b.length;a++)b[a].play()}},1040)},existVideoContainer(a){const b=chatContainerEvent.elementFromShadowDom(".userVideos","all");
for(let d=0;d<b.length;d++)if(b[d].id===`video${a.id}`)return!0;return!1},handleUserMediaError(a){var b=virtualclass.lang.getString(a);b="object"===typeof a?virtualclass.lang.getString(a.name):b;null==b?virtualclass.view.createErrorMsg(a,"errorContainer","virtualclassAppFooterPanel"):"object"===typeof a?virtualclass.view.createErrorMsg(b,"errorContainer","virtualclassAppFooterPanel",{className:a.name}):virtualclass.view.createErrorMsg(b,"errorContainer","virtualclassAppFooterPanel");virtualclass.user.control.mediaWidgetDisable("vd");
virtualclass.view.disappearBox("WebRtc");switch(a.name){case "NotFoundError":case "DevicesNotFoundError":a="nowebcam";break;case "SourceUnavailableError":a="webcambusy";break;case "PermissionDeniedError":case "SecurityError":a="nopermission";virtualclass.gObj.disableCamByUser=!0;break;case "NotAllowedError":a="nopermission";virtualclass.gObj.disableCamByUser=!0;break;default:a="rejected"}virtualclass.system.mediaDevices.webcamErr.push(a)},detectAudioWorklet:()=>{if("undefined"===typeof OfflineAudioContext)return!1;
...
700:1600)}catch(a){this.requestInitializePacket(c),console.log("====> Error handlling request packet")}}else if(this.startedAppending&&this.isMyTurn(c)&&b)try{this.onBuffer(b),console.log("Actual append buffer ",c),delete this.listStream[c],this.currentExecuted=c}catch(a){this.requestInitializePacket(c),console.log("====> Error handlling request packet")}}};$.uiBackCompat=!1;
...
window.addEventListener("load",()=>{navigator.fullscreen?(console.log("Launched: pwa app from ios"),document.querySelector("html").classList.add("pwaRunning")):matchMedia("(display-mode: fullscreen)").matches?(console.log("Launched: pwa app from android"),document.querySelector("html").classList.add("pwaRunning")):console.log("Launched: Browser Tab");document.referrer.includes("net.live.congrea")&&(document.querySelector("html").classList.add("twaRunning"),console.log("Launched: Android app"))});
