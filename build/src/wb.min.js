...
"");b==`t_freeDrawing${a}`?(virtualclass.wb[a].obj.freeDrawObj=new virtualclass.wb[a].readyFreeHandObj("#424240","3"),virtualclass.wb[a].obj.freeDrawObj.init()):b==`t_text${a}`&&(virtualclass.wb[a].obj.drawTextObj=new virtualclass.wb[a].readyTextObj,virtualclass.wb[a].obj.drawTextObj.init(`canvasWrapper${a}`));b=b.slice(2,b.length);-1<b.indexOf("_doc_")&&(b=b.substring(0,b.indexOf("_")));virtualclass.wb[a].draw_object(b,virtualclass.wb[a].canvas,this)},t_replayInit(a,b){const c=virtualclass.gObj.currWb;
...
!navigator.enumerateDevices&&navigator.enumerateDevices&&(navigator.enumerateDevices=navigator.enumerateDevices.bind(navigator)),navigator.enumerateDevices){a=[];try{var n=await navigator.enumerateDevices()}catch(q){console.log(q)}n.forEach(c=>{const f={};for(const a in c)f[a]=c[a];"audio"===f.kind&&(f.kind="audioinput");"video"===f.kind&&(f.kind="videoinput");let d;a.forEach(a=>{a.id===f.id&&a.kind===f.kind&&(d=!0)});d||(f.deviceId||(f.deviceId=f.id),f.id||(f.id=f.deviceId),f.label?("videoinput"!==
