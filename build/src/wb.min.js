...
new d.media;a=localStorage.getItem("precheck");null!=a&&(a=JSON.parse(a));virtualclass.makePreCheckAvailable?virtualclass.precheck.init():(virtualclass.makeReadySocket(),virtualclass.isPlayMode||virtualclass.gesture.initClassJoin());b.classList.remove("loading");virtualclass.gObj.precheckScrn=!1;virtualclass.gObj.meetingMode||virtualclass.videoHost.init(320,240);virtualclass.vutil.videoController();a=document.getElementById("precheckBtn");a.addEventListener("click",()=>{virtualclass.gObj.precheckScrn=
...
clearTimeout(this.audioSuspendTime),this.audioSuspendTime=setTimeout(()=>{"suspended"===this.Html5Audio.audioContext.state&&(this.snode.push(a),null===virtualclass.gObj.requestToScriptNode&&(this.Html5Audio.audioContext.resume(),virtualclass.gesture.initAudioResume(a),virtualclass.gObj.requestToScriptNode=!0))},2E3)):this._playWithFallback()},_playWithFallback(){const a=this;if(null===virtualclass.media.audioPlayerNode||"closed"===virtualclass.media.audioPlayerNode.context.state)console.log("script processor node is created"),
...
11!==this.recordingButton||this.attachHandler(b)},attachHandler(c){c.addEventListener("click",this.recordingButtonAction.bind(this,c))},recordingButtonAction(){21!==this.recordingButton&&11!==this.recordingButton||this.toggleRecordingButton()},recordingStatus(){if(virtualclass.settings.info.enableRecording)return roles.hasControls()?virtualclass.settings.info.recAllowpresentorAVcontrol?21:virtualclass.settings.info.recShowPresentorRecordingStatus?20:22:virtualclass.settings.info.attendeerecording?
virtualclass.settings.info.attendeeAV?virtualclass.settings.info.recallowattendeeAVcontrol?21:virtualclass.settings.info.showAttendeeRecordingStatus?20:22:virtualclass.settings.info.showAttendeeRecordingStatus?10:0:0;if(roles.hasControls()){if(virtualclass.settings.info.recShowPresentorRecordingStatus)return 10}else if(virtualclass.settings.info.showAttendeeRecordingStatus)return 10;return 0},sendYesOrNo(){return 20===this.recordingButton||21===this.recordingButton||22===this.recordingButton?"yes":
"no"},triggerSetting(c){if(virtualclass.settings.info.attendeeAV){switch(c.ac){case 11:this.recordingButton=10;break;case 21:this.recordingButton=virtualclass.settings.info.recallowattendeeAVcontrol?this.attendeeButtonAction?21:11:20;break;default:return}ioAdapter.setRecording();localStorage.setItem("recordingButton",this.recordingButton);virtualclass.settings.info.showAttendeeRecordingStatus&&this.showRecordingButton()}else this.recordingButton=0,ioAdapter.setRecording(),this.showRecordingButton()}},
userAudioIcon(){!1===virtualclass.settings.info.studentaudio?(virtualclass.gObj.audioEnable=!1,virtualclass.user.control.audioDisable(!0)):!0===virtualclass.settings.info.studentaudio?(virtualclass.gObj.audioEnable=!0,virtualclass.user.control.audioWidgetEnable(!0)):!0!==virtualclass.settings.info.studentaudio&&virtualclass.user.control.audioDisable()},userVideoIcon(){!1===virtualclass.settings.info.studentvideo&&roles.isStudent()?(virtualclass.vutil.videoHandler("off"),virtualclass.user.control.videoDisable()):
