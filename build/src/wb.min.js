...
null;if(c=c?c:document.querySelector(`#shapes${virtualclass.gObj.currWb}`))c.classList.remove("close"),c.classList.add("open")}deleteActiveObject(c,b){b=virtualclass.wb[b];const a=b.canvas.getActiveObject();b.canvas.discardActiveObject();b.canvas.remove(a);c&&(c=virtualclass.wbWrapper.protocol.encode("da",virtualclass.gObj.currWb),virtualclass.wbWrapper.msg.send(c))}initActiveElement(c,b){c=document.querySelector(c);"undefined"===typeof virtualclass.gObj.wbTool[virtualclass.gObj.currWb]&&(virtualclass.gObj.wbTool[virtualclass.gObj.currWb]=
...
"d"),virtualclass.wbWrapper.gObj.previousData=b,virtualclass.wbWrapper.msg.send(b))}mouseMove(c,b,a){b.canvas.getPointer(a,!0);a.e.isTrusted&&this.activeDown&&this.down&&(c=this.generateData(a,b,"m"),virtualclass.wbWrapper.msg.optimizeToSend(c,2E3))}mouseUp(c,b,a){c=b.canvas.getPointer(a,!0);console.log("==== actives all mouse up",c.x,c.y);a.e.isTrusted&&(console.log("====> shoud not invoke"),this.activeDown&&this.down&&(this.down=!1,virtualclass.wbWrapper.msg.send(virtualclass.wbWrapper.gObj.previousData),
...
virtualclass.pdfRender[b].customMoustPointer({x:virtualclass.posX},"X",virtualclass.posX)}draw(c,b){const a=this.arrImg,d=virtualclass.wb[b].canvas.getContext("2d");d.clearRect(0,0,virtualclass.wb[b].canvas.width,virtualclass.wb[b].canvas.height);virtualclass.wb[b].canvas.renderAll();d.save();d.beginPath();d.translate(c.x,c.y);d.drawImage(a,-3,-3,18,24);d.closePath();d.restore()}};class WhiteboardMessage{send(c){roles.hasControls()&&(ioAdapter.mustSend(c),"sf"===c.wb[0].substring(0,2)?virtualclass.wbWrapper.util.storeAtMemory(c.wb,virtualclass.gObj.currWb,c.v):virtualclass.wbWrapper.util.storeAtMemory(c.wb,virtualclass.gObj.currWb))}onMessage(c){if("sf"===c.message.wb[0].substring(0,2)){var b=c.fromUser.role;c=virtualclass.wbWrapper.protocol.generateFreeDrawingData(c.message.v,c.message.s,!0);let a;for(let d=0;d<c.length;d+=1)a={message:{wb:c[d]},fromUser:{role:b}},this.onMessage(a)}else Array.isArray(c.message.wb)||
(c.message.wb=[c.message.wb]),b=c.message.wb,virtualclass.vutil.storeWhiteboardAtInlineMemory(c.message.wb),virtualclass.zoom.canvasScale&&virtualclass.gObj.currWb&&"object"===typeof virtualclass.wb[virtualclass.gObj.currWb]&&"t"===c.fromUser.role&&virtualclass.wbWrapper.util.applyCommand(b,virtualclass.gObj.currWb)}optimizeToSend(c,b,a){virtualclass.wbWrapper.gObj.previousData=c;virtualclass.wbWrapper.gObj.presentSendDataTime=(new Date).getTime();virtualclass.wbWrapper.gObj.presentSendDataTime-virtualclass.wbWrapper.gObj.lastSentDataTime>=
b&&(a&&(c=virtualclass.wbWrapper.protocol.encode(a,c),this.send(c)),virtualclass.wbWrapper.gObj.lastSentDataTime=(new Date).getTime(),"sf"===a&&(virtualclass.wb[virtualclass.gObj.currWb].freeDrawingObj.chunks.length=0))}};class WhiteboardText{constructor(c){this.default={rotatingPointOffset:40,cornerSize:13,fontSize:20,fill:"#00f"};this.name=c;this.coreObj={fontWeight:"normal",fontFamily:"arial",padding:7,editingBorderColor:"#08518f"}}isObjectInEditingMode(c){const b=c.canvas.getObjects("i-text");for(let a=0;a<b.length;a+=1)b[a].isEditing&&c.canvas.trigger("text:editing:exited",{target:b[a]})}mouseDown(c,b){virtualclass.wbWrapper.gObj.textSelected||this.textEditing?virtualclass.wbWrapper.gObj.textSelected&&!b.activeAllObj.activeDown&&
(virtualclass.wbWrapper.gObj.textSelected.enterEditing(),c=b.canvas.getObjects("i-text"),"Enter your text"!=virtualclass.wbWrapper.gObj.textSelected.text.trim()&&(this.editingIndex=c.indexOf(virtualclass.wbWrapper.gObj.textSelected)),b.activeAllObj.disable(virtualclass.gObj.currWb,"i-text"),this.textEditing=!0):this.renderText(c,b)}updateText(c,b,a){c.text?a.set("text",c.text):a.set("text",c.value);c.fontSize&&a.set("fontSize",c.fontSize);c.fontColor&&a.set("fill",c.fontColor);b.canvas.renderAll()}renderText(c,
b){console.log("====> render text suman");var a=b.canvas.getObjects("i-text");0<a.length&&null!=c.index?(a=a[c.index])?this.updateText(c,b,a):this.createText(c,b):this.createText(c,b)}createText(c,b){""===c.value||this.isEmptyText(b)||(this.startLeft=c.x,this.startTop=c.y,this.coreObj.left=this.startLeft,this.coreObj.top=this.startTop,c=c.value?c.value:"Enter your text",this.coreObj.rotatingPointOffset=this.default.rotatingPointOffset*virtualclass.zoom.canvasScale,this.coreObj.cornerSize=this.default.cornerSize*
virtualclass.zoom.canvasScale,this.coreObj.fontSize=this.default.fontSize,b.textFontSize&&(this.coreObj.fontSize=+b.textFontSize),this.coreObj.fill=this.default.fill,b.activeToolColor&&(this.coreObj.fill=b.activeToolColor),this[this.name]=new fabric.IText(c,this.coreObj),b.canvas.add(this[this.name]))}isEmptyText(c){c=c.canvas.getObjects("i-text");for(let b=0;b<c.length;b++)if("Enter your text"===c[b].text)return!0;return!1}finalizeText(c){const b=virtualclass.wb[virtualclass.gObj.currWb];this.textEditing=
!1;if(!this.isDefault(c.target)){virtualclass.wbWrapper.gObj.lastSentDataTime=(new Date).getTime();var a={x:c.target.left,y:c.target.top,text:c.target.text};null!=this.editingIndex&&(a.index=this.editingIndex);+b.textFontSize&&c.target.fontSize!==+b.textFontSize&&(a.fontSize=b.textFontSize);b.activeToolColor&&c.target.fill!==b.activeToolColor&&(a.fontColor=b.activeToolColor);virtualclass.wbWrapper.msg.optimizeToSend(a,0,"tx");delete this.editingIndex;b.activeAllObj.enable(virtualclass.gObj.currWb,
"i-text");(a.fontColor||a.fontSize)&&this.updateText(a,b,c.target)}}isDefault(c){return"Enter your text"==c.text.trim()}afterSelected(c){console.log("selected 1");virtualclass.wbWrapper.gObj.textSelected=c}afterDeSelected(){console.log("deselected 1");virtualclass.wbWrapper.gObj.textSelected=null}mouseMove(){}mouseUp(){console.log("mouse up")}};class Whiteboard{constructor(){this.selectedTool=this.canvas=null;this.rectangleObj=new WhiteboardRectangle("rectangle");this.activeAllObj=new WhiteboardActiveAll;this.freeDrawingObj=new WhiteboardFreeDrawing("freeDrawing");this.lineObj=new WhiteboardLine("line");this.triangleObj=new WhiteboardTriangle("triangle");this.circleObj=new WhiteboardCircle("circle");this.textObj=new WhiteboardText("text");this.gObj={}}init(c){this.wbId=c;delete virtualclass.wbWrapper.gObj.textSelected;this.textObj.textEditing=
...
c.currentTarget);this.innerToolbarHandler(c.dataset.tool,virtualclass.gObj.currWb);if(0<virtualclass.wb[virtualclass.gObj.currWb].canvas.getActiveObjects().length){virtualclass.wb[virtualclass.gObj.currWb].canvas.discardActiveObject();virtualclass.wb[virtualclass.gObj.currWb].canvas.renderAll();const b=virtualclass.wbWrapper.protocol.encode("ds",virtualclass.gObj.currWb);virtualclass.wbWrapper.msg.send(b)}(virtualclass.wbWrapper.shapes.includes(c.dataset.tool)||"activeAll"===c.dataset.tool)&&virtualclass.wbWrapper.util.makeActiveTool(c.id)}selectTool(c){if("rectangle"===
c||"line"===c||"circle"===c||"triangle"===c||"text"===c||"activeAll"===c||"freeDrawing"===c)this.selectedTool=c;"stroke"!==c&&"font"!==c&&("activeAll"!==c&&"text"!==c?virtualclass.wbWrapper.util.strokeSizeSelector():"text"===c&&virtualclass.wbWrapper.util.fontSizeSelector())}innerToolbarHandler(c,b){virtualclass.wbWrapper.util.closeTray();this.canvas.isDrawingMode=!1;this.selectTool(c);if("rectangle"!==c&&"line"!==c&&"circle"!==c&&"triangle"!==c&&"text"!==c)this[c](b);else"text"===c?(this.activeAllObj.enable(b,
"i-text"),this.activeAllObj.disableBut(b,"i-text"),delete virtualclass.wbWrapper.gObj.textSelected,this.textObj.textEditing=!1):this.activeAllObj.disable(b)}shapes(){const c=document.querySelector(`#shapes${this.wbId}`);virtualclass.wbWrapper.util.handleTrayDisplay(c)}stroke(){const c=document.querySelector(`#commandToolsWrapper${this.wbId} .strkSizeList`);virtualclass.wbWrapper.util.handleTrayDisplay(c);virtualclass.wbWrapper.util.initActiveElement(`#t_strk${this.wbId} ul`,{type:"strk",prop:"stroke"})}font(){const c=
document.querySelector(`#commandToolsWrapper${this.wbId} .fontSizeList`);virtualclass.wbWrapper.util.handleTrayDisplay(c);virtualclass.wbWrapper.util.initActiveElement(`#t_font${this.wbId} ul`,{type:"font",prop:"font"})}color(){const c=document.querySelector(`#commandToolsWrapper${this.wbId} .table`);virtualclass.wbWrapper.util.handleTrayDisplay(c);virtualclass.wbWrapper.util.initActiveElement(`#colorList${this.wbId}`,{type:"color",prop:"color"})}freeDrawing(c){this.canvas.isDrawingMode=!1;this.activeAllObj.disable(c)}activeAll(c){this.activeAllObj.enable(c);
console.log("====> I am being active here ")}clearAll(c){const b=virtualclass.lang.getString("clearAllWarnMessageW");virtualclass.popup.confirmInput(b,b=>{b&&(this.clear(),b=virtualclass.wbWrapper.protocol.encode("cr",c),virtualclass.wbWrapper.msg.send(b))})}clear(){this.canvas.clear();this.replayObjs=[];this.canvas._objects=[];delete virtualclass.wbWrapper.gObj.textSelected;this.textObj.textEditing=!1}};(function(c){c.message={ActiveAll:"Active all",addcontext:"Add bookmark",Addoption:"Add option",addnew:"Add new",adminusr:"Admin user",appPrerequites:"Validate system requirements",assign:"Assign",assignDisable:"Reclaim controls",askplayMessage:'<span id="askplaymsg"> Should we start playing the session?</span><br /><span id="remaingmsg">Remaining data will be downloaded in the background.</span>',askplaymsg:'<span id="askplaymsg"> "Downloading in progress, click Play to begin </span>"',audioInput:"Audio input",
...
virtualclass.popup.sendBackOtherElems();document.getElementById("replayClose").addEventListener("click",()=>{c.close()});document.getElementById("replayButton").addEventListener("click",()=>{virtualclass.recorder.controller.pause=!1;virtualclass.recorder.replayFromStart.call(virtualclass.recorder)})},getCurrentPacket(){let b=0;if(0<this.masterIndex)for(let a=0;a<this.masterIndex;a++)b+=this.masterRecordings[a].length;return b+this.subRecordingIndex},collectElapsedPlayTime(){this.elapsedPlayTime+=
