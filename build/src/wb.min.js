...
d.addEventListener("click",()=>{d.parentNode.classList.contains("recordingPlay")?b.initRecPause():b.initRecPlay()})}},initRecPause(){const b=document.getElementById("recPause");this.controller._pause();this.doControlActive(b);virtualclass.recorder.triggerPauseVideo();b.parentNode.classList.remove("recordingPlay");b.dataset.title=virtualclass.lang.getString("Play")},initRecPlay(){const b=document.getElementById("recPause");this.controller._play();this.doControlActive(b);virtualclass.videoUl&&virtualclass.videoUl.player&&
virtualclass.videoUl.player.play();b.parentNode.classList.add("recordingPlay");b.dataset.title=virtualclass.lang.getString("Pause")},doControlActive(b){const a=document.getElementById("playControllerCont").getElementsByClassName("recButton");for(let b=0;b<a.length;b++)a[b].classList.remove("controlActive");b.parentNode.classList.add("controlActive")},controller:{pause:!1,ff:1,_play(){virtualclass.recorder.startTime=performance.now();this.ff=1;this.pause=!1;virtualclass.recorder.play()},_pause(){this.pause=
!0},fastForward(b){this.ff=b;this.pause=!1;virtualclass.recorder.play()}},sendData(b,a,d){this.cb=d;b=JSON.stringify(b);this.httpObj.open("POST",a);this.httpObj.setRequestHeader("x-api-key",wbUser.lkey);this.httpObj.setRequestHeader("x-congrea-authuser",wbUser.auth_user);this.httpObj.setRequestHeader("x-congrea-authpass",wbUser.auth_pass);this.httpObj.setRequestHeader("x-congrea-room",wbUser.room);this.httpObj.setRequestHeader("Content-Type","application/json");this.httpObj.withCredentials=!1;this.httpObj.send(b)},
sortingFiles(b){return b.sort(function(a,b){return a<b?-1:a>b?1:0})},setOrginalListOfFiles(b){const a={};for(let d=0;d<b.length;d++)a[b[d]]=null!=b[d+1]?{index:d,next:b[d+1]}:{index:d,next:"end"};return a},triggerDownloader(){this.tryNumberOfTimes=1;Object.prototype.hasOwnProperty.call(this,"triggerDownloaderTime")&&clearInterval(this.triggerDownloaderTime);Object.prototype.hasOwnProperty.call(this,"startTimeCounter")&&clearInterval(this.startTimeCounter);let b=0;this.startTimeCounter=setInterval(()=>
{b++},1E3);this.triggerDownloaderTime=setInterval(()=>{3<this.tryNumberOfTimes?(virtualclass.recorder.allFileFound=!0,clearInterval(this.triggerDownloaderTime.triggerDownloaderTime),this.isPlayFinished()&&(-1<this.masterRecordings[this.masterIndex][this.subRecordingIndex].recObjs.indexOf('"cf":"sEnd"')&&this.playProgressBar(this.totalTimeInMiliSeconds),this.askAgainToPlay())):virtualclass.xhrn.vxhrn.post(virtualclass.api.recordingFiles).then(a=>{this.afterDownloadingList(a.data)})},18E4)},afterDownloadingList(b){let a;
if(null!=b&&Object.prototype.hasOwnProperty.call(b,"Item")){var d=+b.Item.time.N;this.sessionStartTime=(new Date).getTime()-d;d=[...new Set(b.Item.list.L.map(a=>a.S))];b=d.length;if(this.lastRecordings&&d.length===this.lastRecordings)this.tryNumberOfTimes=this.tryNumberOfTimes?++this.tryNumberOfTimes:1;else{delete this.alreadyCalcTotTime;this.totalRecordingFiles=this.sortingFiles(d);this.lastFile=this.totalRecordingFiles[this.totalRecordingFiles.length-1];this.orginalListOfFiles=this.setOrginalListOfFiles(this.totalRecordingFiles);
this.calculateTotalPlayTime();d=this.totalRecordingFiles.shift();this.firstxhr=axios.create({timeout:6E5,withCredentials:!0});this.requestDataFromServer(d,this.firstxhr);for(const b in this.totalRecordingFiles)d=virtualclass.recorder.session,a=this.totalRecordingFiles[b],virtualclass.createPrefetchLink(`https://recording.congrea.net/${wbUser.lkey}/${wbUser.room}/${d}/${a}`);9E5>this.sessionStartTime&&this.triggerDownloader()}this.lastRecordings=b}else this.tryNumberOfTimes=this.tryNumberOfTimes?++this.tryNumberOfTimes:
1,this.triggerDownloader()},requestListOfFiles(){this.fetchRecViewData();this.actualTotalPlayTime=0;this.session=wbUser.session;virtualclass.xhrn.vxhrn.post(virtualclass.api.recordingFiles,{session:virtualclass.recorder.session}).then(b=>{this.afterDownloadingList(b.data)})},calculateTotalPlayTime(){var b=virtualclass.recorder.totalRecordingFiles;const a=Math.trunc(+b[0].split("-")[0]/1E6);b=Math.trunc(+b[b.length-1].split("-")[1].split(".")[0]/1E6);this.totalTimeInMiliSeconds=b-a;this.lastTimeInSeconds=
Math.trunc(b/1E3);this.firstTimeInSeconds=Math.trunc(a/1E3)},getTimeFromFile(b){return+b.split("-")[1].split(".")[0]/1E9},totalPlayTime(){let b=0;for(let a=0;a<virtualclass.recorder.masterRecordings.length;a++)for(let d=0;d<virtualclass.recorder.masterRecordings[a].length;d++)b+=virtualclass.recorder.masterRecordings[a][d].playTime;return b/1E3},getTotalTimeInMilSeconds(b,a){let d=0,c=0;for(;d<=b;){for(let f=0;f<virtualclass.recorder.masterRecordings[d].length&&(c+=virtualclass.recorder.masterRecordings[d][f].playTime,
