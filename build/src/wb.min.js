...
c.type="lowpass";c.frequency.value=2E3;d.connect(c);v=new AudioWorkletNode(g.audio.Html5Audio.audioContext,"worklet-audio-send");v.onprocessorerror=function(a){g.audio.notifiyMuteAudio()};c.connect(v);v.connect(g.audio.Html5Audio.audioContext.destination);virtualclass.gObj.workletAudioSend=v;d=new MessageChannel;workerAudioSend.postMessage({cmd:"workerIO",sampleRate:this.Html5Audio.audioContext.sampleRate,uid:virtualclass.gObj.uid},[d.port1]);workerIO.postMessage({cmd:"workerAudioSend"},[d.port2]);
...
0);d=virtualclass.videoHost.convertDataURIToBinary(d);if(!virtualclass.videoHost.gObj.stdStopSmallVid&&(!roles.hasControls()||roles.hasControls()&&virtualclass.videoHost.gObj.videoSwitch)){var m=virtualclass.gObj.uid.toString();for(var n=m.length;8>n;n++)m=`0${m}`;m=m.match(/[\S]{1,2}/g)||[];m=new Uint8ClampedArray([11,m[0],m[1],m[2],m[3],c]);n=new Uint8ClampedArray(d.length+m.length);n.set(m);n.set(d,m.length);k.sendInBinary(n)}clearInterval(virtualclass.media.smallVid);if(Object.prototype.hasOwnProperty.call(virtualclass,
"connectedUsers"))if(m=f+2500*virtualclass.connectedUsers.length,g!==virtualclass.connectedUsers.length){g=virtualclass.connectedUsers.length;n=-1;for(var r=0;r<virtualclass.connectedUsers.length;r++)virtualclass.connectedUsers[0].userid===virtualclass.gObj.uid&&(n=r);r=m/g;0>n&&(n=0);virtualclass.media.smallVid=setInterval(a,m+n*r)}else virtualclass.media.smallVid=setInterval(a,m)}Object.prototype.hasOwnProperty.call(virtualclass.media,"smallVid")&&clearInterval(virtualclass.media.smallVid);const b=
this;let d,c;f=Math.floor(5001*Math.random()+3E3);let g=-1;const k=this;virtualclass.media.smallVid=setInterval(a,f)},startToStream(){g.video.calcDimension();g.video.send()},playWithoutSlice(a,b,d){this.remoteVid=document.getElementById(`video${a}`);null!=this.remoteVid&&(this.remoteVidCont=this.remoteVid.getContext("2d"),this.remoteVidCont.putImageData(imgData,0,0))},drawReceivedImage(a,b,d,c){this.remoteVid=chatContainerEvent.elementFromShadowDom(`#video${c}`);if(null!=this.remoteVid)if(this.remoteVidCont=
