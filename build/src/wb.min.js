...
new d.media;a=localStorage.getItem("precheck");null!=a&&(a=JSON.parse(a));if(virtualclass.makePreCheckAvailable){if(virtualclass.endSession)return;virtualclass.precheck.init()}else virtualclass.makeReadySocket(),virtualclass.isPlayMode||virtualclass.gesture.initClassJoin();b.classList.remove("loading");virtualclass.gObj.precheckScrn=!1;virtualclass.gObj.meetingMode||virtualclass.videoHost.init(320,240);virtualclass.vutil.videoController();a=document.getElementById("precheckSetting");a.addEventListener("click",
()=>{virtualclass.gObj.precheckScrn=!0;virtualclass.precheck.init(virtualclass.precheck)});this.raiseHand=d.raiseHand;this.raiseHand.init();this.appSetting=d.appSetting;this.appSetting.init();virtualclass.colorSelector=d.colorSelector;virtualclassSetting.theme.selectedColor&&this.colorSelector.makeThemeReady();a=document.getElementById("virtualclassCont");null!=a&&a.classList.add(virtualclass.system.mybrowser.name);a=document.getElementById("fullScreenButton");null!=a&&a.addEventListener("click",
virtualclass.vutil.Fullscreen);a=document.getElementById("fullScreenExitButton");null!=a&&a.addEventListener("click",virtualclass.vutil.closeFullscreen);document.onfullscreenchange=function(b){virtualclass.gObj.fullScreenMode?(document.querySelector("#fullScreenButton").style.display="block",document.querySelector("#fullScreenExitButton").style.display="none",virtualclass.gObj.fullScreenMode=!1):(document.querySelector("#fullScreenButton").style.display="none",document.querySelector("#fullScreenExitButton").style.display=
"block",virtualclass.gObj.fullScreenMode=!0)}},makeReadySocket(){if(!virtualclass.vutil.isPlayMode()){const a=this;var b=setInterval(()=>{2<=a.gObj.editorInitDone&&(a.initSocketConn(),clearInterval(b))},100)}},initSocketConn(){this.system.webSocket&&(io.init(virtualclass.uInfo),d.userdata=virtualclass.uInfo)},html:{id:"virtualclassCont",optionsClass:"appOptions",init(b){this.virtualclass=b},leftAppBar(){if(!(0<document.getElementsByClassName("appOptions").length)){document.querySelector("#virtualclassApp #virtualclassAppLeftPanel");
var b=virtualclass.getTemplate("appTools")();virtualclass.vutil.insertAppLayout(b);virtualclass.gObj.hasOwnProperty("errNotScreenShare")&&virtualclass.view.disableSSUI();virtualclass.gObj.hasOwnProperty("errAppBar")&&virtualclass.view.disableLeftAppBar()}},createDiv(b,a,c,k,d){const f=document.createElement("a");f.href="#";const g=document.createElement("div");g.id=b;"undefined "!==typeof k&&(g.className=k);k=document.createElement("span");k.className=`icon-${a}`;f.appendChild(k);f.dataset.title=
...
c.putImageData(f,0,0);virtualclass.gObj.isReadyForVideo=!0})},isSessionEnded(){if(virtualclass.endSession)return localStorage.clear(),!0}}})(window);let globalImageData={},newCanvas;
...
b&&0<b.length)for(var c=0;c<b.length;c++)null!=b[c]&&b[c].parentNode.removeChild(b[c]);virtualclass.gObj.precheckScrn&&virtualclass.vutil.prechkScrnShare();virtualclass.gObj.audioEnable=roles.hasControls()?!0:virtualclass.gObj.stdaudioEnable;virtualclass.storage.config.sessionEndFlag=!0;b=localStorage.getItem("nEd");c=localStorage.getItem("precheck");localStorage.clear();null!=virtualclass.chat&&(delete virtualclass.chat.vmstorage,virtualclass.chat.vmstorage={},virtualclass.chat.removeChatHighLight("chatrm"));
