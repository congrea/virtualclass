...
!0);0==b&&(virtualclass.user.control.videoDisable(),virtualclass.vutil.addClass("virtualclassCont","nowebcam"))},handleUserMedia(a){localStorage.removeItem("dvid");document.getElementById("audioWidget");var b=localStorage.getItem("audEnable");if(roles.isStudent()&&virtualclass.system.mediaDevices.hasMicrophone)!1===virtualclass.settings.info.studentaudio?(virtualclass.gObj.audioEnable=!1,virtualclass.user.control.audioDisable(!0)):!0===virtualclass.settings.info.studentaudio?(virtualclass.gObj.audioEnable=
...
{virtualclass.videoHost.renderSelfVideo(a)});!1===virtualclass.settings.info.studentvideo&&roles.isStudent()?virtualclass.user.control.videoDisable():(roles.isStudent()&&!0!==virtualclass.settings.info.studentvideo?(virtualclass.vutil.videoHandler("off"),virtualclass.videoHost.toggleVideoMsg("disable")):(virtualclass.user.control.videoEnable(),roles.isStudent()&&virtualclass.vutil.videoHandler("off")),!0===virtualclass.settings.info.studentvideo&&virtualclass.user.control.videoEnable());
...
"none",anchorTag.style.cursor="default";else{const b=this;d.addEventListener("click",()=>{if("audio"==a){var c=b.toogleAudioIcon();virtualclass.settings.applySettings("enable"==c,"studentaudio");"undefined"!==typeof c&&(localStorage.setItem("allAudAction",c),b.toggleAllAudio.call(virtualclass.user,c))}else c=b.toggleVideoIcon(),virtualclass.settings.applySettings("enable"==c,"studentvideo"),"undefined"!==typeof c&&(localStorage.setItem("allVideoAction",c),b.toggleAllVideo(c))})}},
defaultSettings(a,c){let b,f;for(const d in c)if(f=c[d],"audio"===a&&"studentaudio"===d||"video"===a&&"studentvideo"===d)b=!0===f?"enable":"disable";return b},chatBoxesSwitch(){if(virtualclass.chat.chatroombox)toggleCommonChatBox();else if(0==$("div#chat_room").length){var a=c.createElement("ul");a.id="chat_room";c.body.appendChild(a);virtualclass.chat.chatroombox=$("#chat_room").chatroom({id:"chat_room",user:{name:"test"},title:lang.chatroom_header,offset:"20px",messageSent(a,b){$("#chat_room").chatroom("option",
...
CommonXHR.prototype.send=function(d,c,a){console.log(`Common URL ${d}`);this.cb=c;this.httpObj.open("GET",d,!0);this.httpObj.responseType=a;this.httpObj.setRequestHeader("x-api-key",wbUser.lkey);this.httpObj.setRequestHeader("x-congrea-authuser",wbUser.auth_user);this.httpObj.setRequestHeader("x-congrea-authpass",wbUser.auth_pass);this.httpObj.setRequestHeader("x-congrea-room",wbUser.room);this.httpObj.withCredentials=!0;this.httpObj.send()};(function(d){d.settings={info:{allowoverride:null,studentaudio:null,studentpc:null,studentgc:null,studentvideo:null,raisehand:null,userlist:null,x8:null,x16:null,enableRecording:null,recAllowpresentorAVcontrol:null,recShowPresentorRecordingStatus:null,attendeeAV:null,recallowattendeeAVcontrol:null,showAttendeeRecordingStatus:null,trimRecordings:null},user:{},init(){let c=localStorage.getItem("settings");c||(c=virtualclassSetting.settings);c=virtualclass.settings.onLoadSettings(c);
for(const a in c)virtualclass.settings.info.hasOwnProperty(a)&&(virtualclass.settings.info[a]=c[a]);this.recording.init()},settingsToHex(c){const a=[];a[0]=+c.allowoverride;a[1]=+c.studentaudio;a[2]=+c.studentvideo;a[3]=+c.studentpc;a[4]=+c.studentgc;a[5]=+c.raisehand;a[6]=+c.userlist;a[7]=+c.enableRecording;a[8]=+c.recAllowpresentorAVcontrol;a[9]=+c.recShowPresentorRecordingStatus;a[10]=+c.attendeeAV;a[11]=+c.recallowattendeeAVcontrol;a[12]=+c.showAttendeeRecordingStatus;
a[13]=+c.trimRecordings;a[14]=+c.x8;a[15]=+c.x16;return virtualclass.settings.binaryToHex(a.join(""))},parseSettings(c){const a={};(c=virtualclass.settings.hexToBinary(c))&&(c=c.split(""));a.allowoverride=!!+c[0];a.studentaudio=!!+c[1];a.studentvideo=!!+c[2];a.studentpc=!!+c[3];a.studentgc=!!+c[4];a.raisehand=!!+c[5];a.userlist=!!+c[6];a.enableRecording=!!+c[7];a.recAllowpresentorAVcontrol=!!+c[8];a.recShowPresentorRecordingStatus=!!+c[9];a.attendeeAV=
...
studentaudio(c){roles.isStudent()&&(!0===c?(virtualclass.user.control.audioWidgetEnable(!0),virtualclass.gObj.audioEnable=!0):!1===c&&(c=document.querySelector("#speakerPressOnce"),!0!==c.dataset.audioPlaying&&"true"!==c.dataset.audioPlaying||virtualclass.media.audio.clickOnceSpeaker("speakerPressOnce"),virtualclass.user.control.audioDisable(),virtualclass.gObj.audioEnable=!1))},allowoverride(){console.log("TO DO")},studentpc(){console.log("TO DO")},studentgc(){console.log("TO DO")},
studentvideo(c){if(roles.isStudent()){const a=document.querySelector(".videoSwitchCont #videoSwitch");a.classList.contains("off")&&!1===c?console.log("do nothing"):a.classList.contains("on")&&!0===c?console.log("do nothing"):a.classList.contains("on")&&!1===c&&virtualclass.vutil.videoHandler("off");virtualclass.videoHost.toggleVideoMsg(!1===c?"disable":"enable")}},raisehand(){console.log("TO DO")},userlist(){console.log("TO DO")},x8(){console.log("TO DO")},enableRecording(){console.log("TO DO")},
