...
(console.log("wids, From virtualclass "),n.push(virtualclass.gObj.currSlide)),virtualclass.gObj.wbRearrang=!0,virtualclass.gObj.wIds=n)));virtualclass.wbCommon.initNav(virtualclass.gObj.wIds);virtualclass.gObj.wbRearrang&&(roles.hasControls()?(virtualclass.wbCommon.rearrange(virtualclass.wbCommon.order),virtualclass.wbCommon.indexNav.addActiveNavigation(virtualclass.gObj.currWb)):virtualclass.wbCommon.rearrange(virtualclass.gObj.wIds));virtualclass.wbCommon.identifyFirstNote(virtualclass.gObj.currWb);
virtualclass.wbCommon.identifyLastNote(virtualclass.gObj.currWb)}else f=Array.prototype.slice.call(arguments)[2],this.appInitiator[b].apply(virtualclass,Array.prototype.slice.call(arguments)),f&&f.init&&f.init.videoUrl&&f.fromReload&&(n=!0),roles.hasControls()&&"Video"==b&&(`virtualclass${b}`!=virtualclass.previous&&(f=document.querySelector("#dashboardnav button"),null!=f&&(f.setAttribute("data-currapp","Video"),f.classList.contains("clicked")&&f.classList.remove("clicked"))),virtualclass.vutil.triggerDashboard(b,
n));this.previrtualclass=this.previous;b!=virtualclass.apps.wb&&b!=virtualclass.apps.ss&&virtualclass.system.setAppDimension();if("Video"!=b&&virtualclass.hasOwnProperty("videoUl")){virtualclass.videoUl.videoId="";(n=document.getElementById("dispVideo_Youtube_api"))&&n.contentWindow&&n.contentWindow.postMessage('{"event":"command","func":"pauseVideo","args":""}',"*");if(n=document.querySelector(".congrea #dispVideo"))n.style.display="none",(n=document.querySelector(".congrea #dispVideo video"))&&
...
d:a}));virtualclass.gObj.currWb=c;"object"!==typeof this.pdfRender[c]?this.pdfRender[c]=d.pdfRender():("Whiteboard"==virtualclass.currApp||"DocumentShare"==virtualclass.currApp)&&virtualclass.zoom.adjustScreenOnDifferentPdfWidth();roles.isStudent()&&"Whiteboard"==virtualclass.currApp&&virtualclass.wbCommon.setCurrSlideNumber(c);if("undefined"!==typeof c){if("object"!==typeof this.wb[c])if("object"!==typeof this.wb&&(this.wb={}),virtualclass.gObj.commandToolsWrapperId[c]=`commandToolsWrapper${c}`,
this.wb[c]={},virtualclass.gObj.tempReplayObjs[c]=[],this.wb[c]=new d.whiteboard(this.wbConfig,c),null!=this.gObj.tempQueue[c]&&(this.wb[c].gObj.queue=this.gObj.tempQueue[c].slice(),this.gObj.tempQueue[c]=null),"Whiteboard"==virtualclass.currApp?(a=document.querySelector("#virtualclassWhiteboard .whiteboardContainer"),virtualclass.gObj.wbInitHandle||(virtualclass.wbCommon.initNavHandler(),virtualclass.gObj.wbInitHandle=!0)):a=document.getElementById(`cont${c}`),null!=a){if(null==document.querySelector(`vcanvas${c}`)){k=
virtualclass.getTemplate("main","whiteboard");if("Whiteboard"==virtualclass.currApp){virtualclass.wbCommon.hideElement();const b=`note${c}`;var f=document.querySelector(`#${b}`);null!=f?(f.classList.add("canvasContainer","current"),k=k({cn:c,hasControl:roles.hasControls()}),f.innerHTML=k):(k=`<div id='${b}' data-wb-id='${c}' class='canvasContainer current'>${k({cn:c,hasControl:roles.hasControls()})}</div>`,"_doc_0_0"!=c?a.insertAdjacentHTML("beforeend",k):(a.innerHTML=k,f=document.querySelector("#note_doc_0_0"),
null!=f&&f.classList.add("current")))}else k=k({cn:c,hasControl:roles.hasControls()}),a.innerHTML=k;f=document.querySelector(`#canvas${c}`)}this.wb[c].utility=new d.utility;this.wb[c].alreadyReplay=!1;this.wb[c].packContainer=new d.packContainer;this.wb[c].draw_object=d.draw_object;this.wb[c].makeobj=d.makeobj;this.wb[c].readyFreeHandObj=d.readyFreeHandObj;this.wb[c]._replay=_replay;this.wb[c].readyTextObj=d.readyTextObj;this.wb[c].bridge=d.bridge;this.wb[c].response=d.response;virtualclass.wb[c].utility.displayCanvas(c);
({vcan:a}=virtualclass.wb[c]);roles.hasControls()&&(virtualclass.wb[c].utility.setOrginalTeacherContent(b),virtualclass.wb[c].attachToolFunction(virtualclass.gObj.commandToolsWrapperId[c],!0,c),b=a.utility.canvasCalcOffset(a.main.canid),console.log(`whietboard offset x=${b.x} `+` y=${b.y}`));"DocumentShare"==virtualclass.currApp?({currNote:b}=virtualclass.dts.docs.note,virtualclass.pdfRender[c].init(f,b)):virtualclass.pdfRender[c].init(f);await virtualclass.storage.getWbData(c)}else alert("whiteboard container is null");
else b=`commandToolsWrapper${c}`,null==document.getElementById(`commandToolsWrapper${c}`)&&roles.hasControls()&&virtualclass.wb[c].attachToolFunction(b,!0,c);({vcan:a}=virtualclass.wb[c]);null!=document.getElementById(`canvas${c}`)&&(a.utility.canvasCalcOffset(a.main.canid),0!==this.gObj.tempReplayObjs[c].length&&"nodata"!==this.gObj.tempReplayObjs[c]||virtualclass.wb[c].utility.makeCanvasEnable());console.log(`==== previous set ${this.wbConfig.id}`);this.previous=this.wbConfig.id}else alert("id is undefined");
console.log("==== SESSION CLEAR");virtualclass.zoom.init();b=localStorage.getItem("activeTool");null!=b&&(f=document.querySelector(`#${b}`),null!=f&&(f.classList.add("active"),virtualclass.wb[c].prvTool=b));roles.hasControls()&&d.addEventListener("mouseup",b=>{var a=document.querySelector("#virtualclassCont").dataset.currapp;if(null!=a&&("Whiteboard"==a||"DocumentShare"==a))if(b.target.dataset.hasOwnProperty("stroke")||b.target.dataset.hasOwnProperty("font"))b=b.target.dataset.hasOwnProperty("stroke")?
