'use strict';class VirtualclassKeyboard{constructor(){}init(c){c=document.querySelector(`#canvasWrapper${c}`);c.tabIndex=1E3;c.addEventListener("keydown",this.handlerKeyDown,!1)}handlerKeyDown(c){8!==c.keyCode&&46!==c.keyCode||virtualclass.wbWrapper.util.deleteActiveObject(event,virtualclass.gObj.currWb)}};const screenDecodeBlob=URL.createObjectURL(new Blob(["(",function(){function c(b){myLoop=4*b.length;k=new Uint8ClampedArray(myLoop);for(let a=0;a<myLoop;a++)k[4*a]=36.5*(b[a]>>5),k[4*a+1]=36.5*((b[a]&28)>>2),k[4*a+2]=85*(b[a]&3),k[4*a+3]=255}function b(b,a,d,c,f){const k=g.width;let m=0;for(let n=a;n<c+a;n++)for(let a=b;a<d+b;a++){const b=4*(n*k+a);g.data[b]=f[m++];g.data[b+1]=f[m++];g.data[b+2]=f[m++];g.data[b+3]=f[m++]}}function a(b,a){b=d(b);a=d(a);return parseInt(b+a)}function d(b){b=b.toString();
...
G=function(b,a,d){const c=a.length;let f;if(a.length!==b.length)return!1;for(f=0;f<c;f+=1){if(a[f]!==b[f])return!1;f+=d}for(f=0;f<c;--f)if(f+=d,a[f]!==b[f])return!1;for(f=0;f<c;f+=7)if(a[f]!==b[f])return!1;return!0};onmessage=function(a){if(Object.prototype.hasOwnProperty.call(a.data,"initPrevImg"))u=[],v=null;else if(Object.prototype.hasOwnProperty.call(a.data,"resize"))for(k=0;k<a.data.resA*a.data.resB;k++)t=b(a),u[k]=D(t.x,t.y,t.w,t.h,a.data.offsetWidth,a.data.offsetHeight,a.data.img);else{{var d=
a.data.img,f=d.length/4;let b;var g;if(null===v||v.length!==f)v=new Uint8ClampedArray(f);for(g=0;g<f;g++){var F=d[4*g];var I=d[4*g+1];b=d[4*g+2];F=(Math.round(F/36.5)<<5)+(Math.round(I/36.5)<<2)+Math.round(b/85);v[g]=F}}for(k=0;k<a.data.resA*a.data.resB;k++)if(t=b(a),z=D(t.x,t.y,t.w,t.h,a.data.offsetWidth,a.data.offsetHeight,v),"undefined"!==typeof u[k]){if(n=G(z,u[k],t.w),!n){if(u[k].length!==z.length)u[k]=z;else for(d=0;d<z.length;d++)u[k][d]=z[d];d=z;f=t;g=a.data.type;I=c(f.x,4);F=c(f.y,4);g=new Uint8ClampedArray(["ss"===
...
postMessage({cmd:"endBinary",msg:d})}}},finallySend(b){this.sock.readyState&&this.sock.send(b)},sendBinary(b){if(this.sock.readyState&&b.length)if(6E5>=b.length){if(b.constructor===Int8Array)var a=new Int8Array(b.length+2);else b.constructor===Uint8ClampedArray&&(a=new Uint8ClampedArray(b.length+2));a.set([b[0],0]);a.set(b,2);this.finallySend(a.buffer);postMessage({cmd:"stBinary",msg:a})}else{this.binMsgQueue=[];for(let d=0,c=0;d<b.length;c++)a=b.slice(d,d+55E4),b.constructor===Int8Array?this.binMsgQueue[c]=
...
serverData;this.view=c.view;null!=localStorage.uRole&&(virtualclass.gObj.uRole=localStorage.uRole,b.classList.add(virtualclass.vutil.getClassName(virtualclass.gObj.uRole)));this.makeAppReady({app:a,cusEvent:"byclick"});this.system.check();this.vutil.isSystemCompatible();this.pageVisible=virtualclass.vutil.pageVisible();virtualclass.user.readyLeftBar(virtualclass.gObj.uRole,a);this.media=new c.media;virtualclass.makePreCheckAvailable?virtualclass.precheck.init():(virtualclass.makeReadySocket(),virtualclass.isPlayMode||
virtualclass.gesture.initClassJoin());b.classList.remove("loading");virtualclass.gObj.precheckScrn=!1;virtualclass.gObj.meetingMode||virtualclass.videoHost.init(320,240);virtualclass.vutil.videoController();document.getElementById("precheckBtn").addEventListener("click",()=>{virtualclass.gObj.precheckScrn=!0;virtualclass.precheck.init(virtualclass.precheck)});this.appSetting=c.appSetting;this.appSetting.init();virtualclass.colorSelector=c.colorSelector;virtualclassSetting.theme.selectedColor&&this.colorSelector.makeThemeReady();
a=document.getElementById("virtualclassCont");null!=a&&a.classList.add(virtualclass.system.mybrowser.name);a=document.getElementById("fullScreenButton");null!=a&&a.addEventListener("click",virtualclass.vutil.Fullscreen);a=document.getElementById("fullScreenExitButton");null!=a&&a.addEventListener("click",virtualclass.vutil.closeFullscreen);a=document.getElementById("askFullscreen");null!=a&&a.addEventListener("click",virtualclass.vutil.Fullscreen);a=document.getElementById("askExitFullscreen");null!=
a&&a.addEventListener("click",virtualclass.vutil.closeFullscreen);const f=document.getElementById("chat_div");a=document.getElementById("sidebarButton");null!=a&&a.addEventListener("click",function(){const b=document.getElementById("virtualclassApp");b.classList.contains("openRightbar")?(b.classList.remove("openRightbar"),b.classList.add("collapsedRightbar"),f.classList.add("collapsedRightbar"),localStorage.setItem("hideRightbar",!0),virtualclass.gObj.hideRightbar=localStorage.getItem("hideRightbar"),
roles.isStudent()&&ioAdapter.sendSpeed(3)):(localStorage.removeItem("hideRightbar"),localStorage.setItem("hideRightbar",!1),virtualclass.gObj.hideRightbar=localStorage.getItem("hideRightbar"),b.classList.remove("collapsedRightbar"),b.classList.add("openRightbar"),f.classList.remove("collapsedRightbar"),roles.isStudent()&&("desktop"===virtualclass.system.device?ioAdapter.sendSpeed(1):null!=document.querySelector("#techVideo.active")&&virtualclass.vutil.sendSpeedByMobile(1),"Poll"===virtualclass.currApp&&
virtualclass.poll.makeGraphResponsive()));if("ScreenShare"===virtualclass.currApp){if(roles.isStudent()&&!virtualclass.gObj.studentSSstatus.mesharing||roles.isTeacher()&&virtualclass.gObj.studentSSstatus.mesharing)virtualclass.studentScreen.doOpposite=!0,virtualclass.studentScreen.triggerFitControl(),virtualclass.ss.triggerFitToScreen()}else virtualclass.zoom.doOpposite=!0,virtualclass.zoom.triggerFitToScreen()});a=document.getElementById("virtualclassApp");virtualclass.gObj.hideRightbar?(a.classList.remove("openRightbar"),
a.classList.add("collapsedRightbar"),f.classList.add("collapsedRightbar"),ioAdapter.sendSpeed(3)):"mobTab"===virtualclass.system.device&&null==document.querySelector("#techVideo.active")&&virtualclass.vutil.sendSpeedByMobile(3);document.addEventListener("fullscreenchange",()=>{virtualclass.onfullscreenchange()},!1);document.addEventListener("mozfullscreenchange",()=>{virtualclass.onfullscreenchange()},!1);document.addEventListener("webkitfullscreenchange",()=>{virtualclass.onfullscreenchange()},!1);
document.addEventListener("msfullscreenchange",()=>{virtualclass.onfullscreenchange()},!1);virtualclass.isPlayMode?virtualclass.settings.triggerSettings():virtualclass.vutil.checkUserRole()&&virtualclass.settings.triggerSettings()},onfullscreenchange(){console.log("====> on full screen change");"virtualclassAppRightPanel"==event.target.id?virtualclass.gObj.rightbarFullScreenMode?(document.getElementById("virtualclassAppRightPanel").classList.contains("fullScreenMode")&&document.getElementById("virtualclassAppRightPanel").classList.remove("fullScreenMode"),
console.log("=====> full screen show ask hide exit"),virtualclass.gObj.rightbarFullScreenMode=!1,virtualclass.gObj.ignoreFullScreen||virtualclass.vutil.showFullScreenButton()):(virtualclass.gObj.rightbarFullScreenMode=!0,console.log("=====> full screen show ask show exit ")):virtualclass.gObj.fullScreenMode?virtualclass.vutil.showFullScreenButton():virtualclass.vutil.hideFullScreenButton();delete virtualclass.gObj.ignoreFullScreen},makeReadySocket(){virtualclass.vutil.isPlayMode()||!virtualclass.config.makeWebSocketReady||
virtualclass.gObj.initsocketws||(virtualclass.gObj.initsocketws=!0,this.initSocketConn())},initSocketConn(){this.system.webSocket&&(io.init(virtualclass.uInfo),c.userdata=virtualclass.uInfo)},html:{id:"virtualclassCont",optionsClass:"appOptions",init(b){this.virtualclass=b},leftAppBar(){if(!(0<document.getElementsByClassName("appOptions").length)){document.querySelector("#virtualclassApp #virtualclassAppLeftPanel");var b=virtualclass.getTemplate("appTools")();virtualclass.vutil.insertAppLayout(b);
Object.prototype.hasOwnProperty.call(virtualclass.gObj,"errNotScreenShare")&&virtualclass.view.disableSSUI();Object.prototype.hasOwnProperty.call(virtualclass.gObj,"errAppBar")&&virtualclass.view.disableLeftAppBar()}},createDiv(b,a,c,k,m){const d=document.createElement("a");d.href="#";const f=document.createElement("div");f.id=b;"undefined"!==typeof k&&(f.className=k);k=document.createElement("span");k.className=`icon-${a}`;d.appendChild(k);d.dataset.title=virtualclass.lang.getString(a);d.className=
"tooltip";"virtualclassWhiteboardTool"===b&&(d.dataset.doc=`${"_doc_0_"}${virtualclass.gObj.wbCount}`);f.appendChild(d);"undefined"!==typeof m?(b=document.getElementById("virtualclassScreenShareTool"),c.replaceChild(f,b)):c.appendChild(f)},createElement(b,a,c){b=document.createElement(b);"undefined"!==typeof a&&(b.id=a);if("undefined"!==typeof c&&1<c.length)for(a=0;a<c.length;a++)b.className+=`${c[a]} `;return b}},dispvirtualclassLayout(b){if("undefined"!==typeof this.previous)if(this.previous.toUpperCase()!==
...
document.createElement("div");a.className="whiteboard";a.dataset.wid=b;a.id=`cont${a.dataset.wid}`;b=b.split("_");b=document.querySelector(`.note[data-slide='${`${b[2]}_${b[3]}`}']`);null!=b&&b.insertBefore(a,b.firstChild)},appInitiator:{Whiteboard(b){let a,d,c,m;({app:a,cusEvent:d,position:m,...b}=b);b.data&&(c=b.data);virtualclass.gObj.currWb=c;console.log("====> whiteboard apply ",virtualclass.gObj.currWb);"object"===typeof this.orderList[virtualclass.currApp]&&-1>=this.orderList[virtualclass.currApp].ol.order.indexOf(virtualclass.gObj.currWb)&&
...
registerPartial(){const b={whiteboardPath},a="precheck rightBarHeader teacherVideo audioWidget appTools popupCont appToolsMeeting appSettingDetail joinclass".split(" "),c={hasControl:roles.hasControls()};let k;for(let d=0;d<a.length;d++)k=null,"precheck"===a[d]||"popupCont"===a[d]?k=b:"audioWidget"===a[d]?(k=virtualclassSetting,k.isControl=roles.hasControls(),k.isMettingMode=virtualclass.gObj.meetingMode&&roles.isStudent()):"teacherVideo"===a[d]||"appTools"===a[d]||"appSettingDetail"===a[d]?k=c:"rightBarHeader"===
a[d]&&(k={std:roles.isStudent(),isPlayMode:virtualclass.isPlayMode}),this.makeReadyTemplate(a[d],k);Handlebars.registerPartial({docNotesMain:this.getTemplate("notesMain","documentSharing"),whiteboardToolbar:this.getTemplate("toolbar","whiteboard"),rightBar:this.getTemplate("rightBar"),recordingControl:this.getTemplate("recordingControl"),leftBar:this.getTemplate("leftBar"),footerBar:this.getTemplate("footerBar"),mobileLandscapeWarn:this.getTemplate("mobileLandscapeWarn"),main:this.getTemplate("main"),
whiteboard:this.getTemplate("main","whiteboard"),dashboardCont:this.getTemplate("dashboardCont"),multiVideo:this.getTemplate("multiVideo")})},registerHelper(){Handlebars.registerHelper("getString",b=>virtualclass.lang.getString(b));Handlebars.registerHelper("debug",function(b){});Handlebars.registerHelper("getVideoType",b=>virtualclass.gObj.meetingMode?"multiVideo":"teacherVideo")},createMainContainer(){const b=document.querySelector(`#${virtualclass.html.id}`);this.registerHelper();this.registerPartial();
var a=this.getTemplate("main");const c={isPlay:virtualclass.isPlayMode,hasControls:roles.hasControls(),meetingMode:virtualclass.gObj.meetingMode};a=a(c);b.insertAdjacentHTML("afterbegin",a)},makeReadyTemplate(b,a){Handlebars.registerPartial(b,(0,JST[`${virtualclass.gObj.tempPrefix}/${b}.hbs`])(a))},getTemplate(b,a){return"undefined"===typeof a?JST[`${virtualclass.gObj.tempPrefix}/${b}.hbs`]:JST[`${virtualclass.gObj.tempPrefix}/${a}/${b}.hbs`]},isMiniFileIncluded(b){b=new RegExp(`${b}.js?=\*([0-9]*)`);
const a=document.getElementsByTagName("script");for(let d=0;d<a.length;d++)if(b.test(a[d].src))return!0;return!1},stickybarWidth(){const b=document.querySelector("#virtualclassApp #virtualclassAppLeftPanel").offsetWidth+"px";document.querySelector("#stickybar").style.width=b},removeSharingClass(){const b=document.querySelector("#virtualclassCont");b&&(b.classList.remove("studentScreenSharing"),document.querySelector("#chat_div").classList.remove("studentScreenSharing"))},clearAllChat(){localStorage.removeItem(virtualclass.gObj.uid);
localStorage.clear("chatroom");null!=virtualclass.chat&&(virtualclass.chat.idList.length=0);clearAllChatBox();const b=document.getElementById("chatWidget").getElementsByClassName("ui-chatbox-msg");if(0<b.length)for(;null!=b[0];)b[0].parentNode.removeChild(b[0])},userListEnable(){const b=document.querySelector("#user_list");b&&b.click()}}}})(window);class WhiteboardUtility{applyCommand(c,b){for(let a=0;a<c.length;a+=1)this.executeData(c[a],b)}executeData(c,b){this.storeAtMemory(c,b);virtualclass.wbWrapper.replay.replayData([c],b)}storeAtMemory(c,b,a){virtualclass.wb[b].replayObjs||(virtualclass.wb[b].replayObjs=[]);let d=c;a&&(d=virtualclass.wbWrapper.protocol.generateFreeDrawingData(a));if(Array.isArray(c))for(c=0;c<d.length;c+=1)virtualclass.wb[b].replayObjs.push(d[c]);else virtualclass.wb[b].replayObjs.push(c)}createFabricNewInstance(c,b){virtualclass.wb[c].canvas&&
virtualclass.wb[c].canvas.upperCanvasEl&&virtualclass.wb[c].canvas.dispose();delete virtualclass.wb[c].canvas;const a={selection:!1};b&&(a.width=b.width,a.height=b.height);virtualclass.wb[c].canvas=new fabric.Canvas(`canvas${c}`,a);roles.hasControls()&&(virtualclass.wb[c].attachMouseMovementHandlers(),console.log("=====> attach handlers with whiteboard id ",c));this.createCanvasPdfInstance(c,virtualclass.wb[c].canvas.upperCanvasEl)}createCanvasPdfInstance(c,b){if(null==document.querySelector(`#canvas${c}_pdf`)){console.log("suman create pdf canvas ",
c);const a=document.createElement("canvas");a.id=`canvas${c}_pdf`;a.className="pdfs";a.width=b.width;a.height=b.height;b.parentNode.parentNode.insertBefore(a,b.parentNode)}}replayFromLocalStroage(c,b){console.log("====> whiteboard pdf suman draw whiteboard");"undefined"!==typeof Storage&&(virtualclass.wb[b].clear(b),virtualclass.wb[b].replayObjs=[],delete virtualclass.wb[b].currStrkSize,delete virtualclass.wb[b].activeToolColor,virtualclass.wb[b].gObj.tempRepObjs=c,0<c.length&&this.applyCommand(c,
b))}fitWhiteboardAtScale(c){console.log("====> canvas set zoom scale ",virtualclass.zoom.canvasScale);virtualclass.wb[c].canvas.setZoom(virtualclass.zoom.canvasScale);"object"===typeof virtualclass.wb[c]&&(delete virtualclass.wb[c].myPencil,virtualclass.wb[c].replayObjs&&0<virtualclass.wb[c].replayObjs.length&&this.replayFromLocalStroage(virtualclass.wb[c].replayObjs,c))}readyMouseEvent(c,b){return new MouseEvent(c,{clientX:b.x,clientY:b.y,buttons:1,bubbles:!0,which:1,composed:!0})}closeShapeContainer(c){this.selectedTool=
null;if(c=c?c:document.querySelector(`#shapes${virtualclass.gObj.currWb}`))c.classList.remove("open"),c.classList.add("close")}closeTray(){const c=document.querySelector(`#commandToolsWrapper${virtualclass.gObj.currWb} .openTray`);c&&c.classList.remove("openTray")}openTray(c){c&&c.classList.add("openTray")}handleTrayDisplay(c){c.classList.contains("openTray")?(this.selectedTool=null,virtualclass.wbWrapper.util.closeTray()):virtualclass.wbWrapper.util.openTray(c)}openShapeContainer(c){this.selectedTool=
null;if(c=c?c:document.querySelector(`#shapes${virtualclass.gObj.currWb}`))c.classList.remove("close"),c.classList.add("open")}deleteActiveObject(c,b){b=virtualclass.wb[b];b.canvas.remove(b.canvas.getActiveObject());c&&(c=virtualclass.wbWrapper.protocol.encode("da",virtualclass.gObj.currWb),virtualclass.wbWrapper.msg.send(c))}initActiveElement(c,b){c=document.querySelector(c);"undefined"===typeof virtualclass.gObj.wbTool[virtualclass.gObj.currWb]&&(virtualclass.gObj.wbTool[virtualclass.gObj.currWb]=
{});"undefined"===typeof virtualclass.gObj.wbTool[virtualclass.gObj.currWb][b.type]&&(c.addEventListener("click",this.activeElementHandler.bind(this,b)),virtualclass.gObj.wbTool[virtualclass.gObj.currWb][b.type]=!0)}activeElementHandler(c,b){this.activeElement(b,c)}activeElement(c,b){var a=document.querySelector(`#t_${b.type}${virtualclass.gObj.currWb} .selected`);null!=a&&a.classList.remove("selected");a=c.target.dataset[b.prop];null!=a&&(c.target.classList.add("selected"),this.changeToolProperty(b.type,
a,virtualclass.gObj.currWb),"color"===b.type&&(document.querySelector(`#t_color${virtualclass.gObj.currWb} .disActiveColor`).style.backgroundColor=virtualclass.wb[virtualclass.gObj.currWb].activeToolColor),c=virtualclass.wbWrapper.protocol.encode("ot",{type:b.type,value:a}),virtualclass.wbWrapper.msg.send(c))}changeToolProperty(c,b,a){a||(a=virtualclass.gObj.currWb);"color"===c?virtualclass.wb[a].activeToolColor=b:"strk"===c?virtualclass.wb[a].currStrkSize=b:"font"===c&&(virtualclass.wb[a].textFontSize=
b)}strokeSizeSelector(){var c=document.querySelector(`#t_font${virtualclass.gObj.currWb}`);null!=c&&(c.classList.remove("show"),c.classList.add("hide"));c=document.querySelector(`#t_strk${virtualclass.gObj.currWb}`);null!=c&&(c.classList.remove("hide"),c.classList.add("show"))}fontSizeSelector(){var c=document.querySelector(`#t_strk${virtualclass.gObj.currWb}`);null!=c&&(c.classList.remove("show"),c.classList.add("hide"));c=document.querySelector(`#t_font${virtualclass.gObj.currWb}`);null!=c&&(c.classList.remove("hide"),
c.classList.add("show"))}makeActiveTool(c){const b=document.getElementById(c);if(b){var a=document.querySelectorAll(`#commandToolsWrapper${virtualclass.gObj.currWb} .tool.active`);for(let b=0;b<a.length;b+=1)a[b].classList.remove("active");this.themeColorShapes(c);b.classList.add("active");localStorage.activeTool=b.id}}themeColorShapes(c){c=c.split(/_doc_*/)[0];const b=document.querySelector(`#tool_wrapper${virtualclass.gObj.currWb}.shapesToolbox`);"t_line"===c||"t_circle"===c||"t_rectangle"===c||
"t_triangle"===c?b.classList.add("active"):b.classList.remove("active")}};class WhiteboardActiveAll{generateData(c,b,a){c=b.canvas.getPointer(c,!0);return virtualclass.wbWrapper.protocol.encode("ac",{event:a,x:c.x,y:c.y})}enable(c,b){c=b?virtualclass.wb[c].canvas.getObjects(b):virtualclass.wb[c].canvas.getObjects();for(b=0;b<c.length;b+=1)console.log("====> active all"),c[b].set("selectable",!0)}disable(c,b){c=b?virtualclass.wb[c].canvas.getObjects(b):virtualclass.wb[c].canvas.getObjects();for(b=0;b<c.length;b+=1)c[b].set("selectable",!1)}disableBut(c,b){c=virtualclass.wb[c].canvas.getObjects();
for(let a=0;a<c.length;a+=1)c[a].type!==b&&c[a].set("selectable",!1)}disableLastElement(c){c=virtualclass.wb[c].canvas.getObjects();c[c.length-1].set("selectable",!1)}mouseDown(c,b,a){c=b.canvas.getPointer(a,!0);console.log("====> whiteboard pdf ========================== active mouse trigger",c.x,c.y);console.log("==== convert actives all mouse down",c.x,c.y);a.e.isTrusted&&(this.down=!0,this.activeDown&&(virtualclass.wbWrapper.gObj.lastSentDataTime=(new Date).getTime()),b=this.generateData(a,b,
"d"),virtualclass.wbWrapper.gObj.previousData=b,virtualclass.wbWrapper.msg.send(b))}mouseMove(c,b,a){c=b.canvas.getPointer(a,!0);console.log("==== actives all mouse move",c.x,c.y," orginal x, y",a.e.clientX,a.e.clientY);a.e.isTrusted&&this.activeDown&&this.down&&(b=this.generateData(a,b,"m"),virtualclass.wbWrapper.msg.optimizeToSend(b,2E3))}mouseUp(c,b,a){c=b.canvas.getPointer(a,!0);console.log("==== actives all mouse up",c.x,c.y);a.e.isTrusted&&(console.log("====> shoud not invoke"),this.activeDown&&
this.down&&(this.down=!1,virtualclass.wbWrapper.msg.send(virtualclass.wbWrapper.gObj.previousData),b=this.generateData(a,b,"u"),virtualclass.wbWrapper.msg.send(b)))}};class WhiteboardReplay{init(c){this.objs=virtualclass.wb[c].vcanMainReplayObjs;this.objNo=0}renderObj(c){var b=virtualclass.wbWrapper.protocol.decode(this.objs[this.objNo],c);let a,d;switch(b.action){case "sp":virtualclass.wb[c].selectedTool=b.tool;a=b.event;d=`${b.shape}Obj`;virtualclass.wb[c][d][a](b.actual,virtualclass.wb[c]);break;case "ac":virtualclass.wb[c].selectedTool=b.tool;b.event&&"mousedown"===b.event&&virtualclass.wb[c].innerToolbarHandler(virtualclass.wbWrapper.keyMap[b.action],c);b=
virtualclass.wbWrapper.util.readyMouseEvent(b.event,b.actual);b.wId=c;virtualclass.wb[c].canvas.upperCanvasEl.dispatchEvent(b);break;case "cr":virtualclass.wb[b.actual].clear();break;case "ds":virtualclass.wb[b.actual].canvas.discardActiveObject();virtualclass.wb[b.actual].canvas.renderAll();break;case "da":virtualclass.wbWrapper.util.deleteActiveObject(!1,c);break;case "ot":virtualclass.wbWrapper.util.changeToolProperty(b.tool,b.actual.value,c);break;case "tx":virtualclass.wb[c].selectedTool=b.tool,
virtualclass.wb[c].textObj.renderText(b.actual,virtualclass.wb[c]);default:console.log("====> do nothing")}}replayInit(c){this.init(c);this.renderObj(c)}replayData(c,b){virtualclass.wb[b].vcanMainReplayObjs=[];0<c.length&&(virtualclass.wb[b].vcanMainReplayObjs=c,this.replayInit(b))}};class WhiteboardProtocol{encode(c,b){return this[c](b,"encode")}decode(c,b){c=c.split("_");return this[c[0]](c,"decode",b)}sp(c,b){let a={};if("encode"===b){b=virtualclass.wbWrapper.keyMap[`${c.name}Short`];if(!b){alert("Capture the problem with ",JSON.stringify(c));debugger}a={wb:[`sp_${b}_${c.event}_${c.x}_${c.y}`],cf:"wb"}}else"decode"===b&&(a={},a.action=c[0],a.tool=virtualclass.wbWrapper.keyMap[c[1]],a.shape=virtualclass.wbWrapper.keyMap[c[1]],a.event=virtualclass.wbWrapper.keyMap[c[2]],a.actual=
{x:+c[3],y:+c[4]});return a}changeWithScale(c,b){const a={};"divide"===c?(a.x=b.x/virtualclass.zoom.canvasScale,a.y=b.y/virtualclass.zoom.canvasScale):(a.x=+b.x*virtualclass.zoom.canvasScale,a.y=+b.y*virtualclass.zoom.canvasScale);return a}ac(c,b,a){const d={};"encode"===b?(a=this.changeWithScale("divide",c),d.wb=[`ac_${c.event}_${a.x}_${a.y}`],d.cf="wb"):"decode"===b&&(d.action=c[0],d.tool=virtualclass.wbWrapper.keyMap[c[0]],3<c.length&&(d.event=virtualclass.wbWrapper.keyMap[`ac${c[1]}`],c=this.changeWithScale("multiply",
{x:c[2],y:c[3]}),d.actual={x:c.x,y:c.y},roles.hasControls()&&(c=document.getElementById(`commandToolsWrapper${a}`),d.actual.y+=c?c.offsetHeight:44,c=document.getElementById("virtualclassAppOptionsCont"),d.actual.x+=c?c.offsetWidth:55),a=document.querySelector(`#canvasWrapper${a}`),d.actual.x-=a.scrollLeft,d.actual.y-=a.scrollTop));return d}generateWhiteboardId(c,b){"encode"===b?(b=c.split("_"),c=10<c.length?`${b[b.length-2]}::${b[b.length-1]}`:b[b.length-1]):10<c[c.length-1].length?(c=c[c.length-
1].split("::"),c=`${c[c.length-2]}_${c[c.length-1]}`,c=`_doc_${c}_${c}`):c=`_doc_0_${c[c.length-1]}`;return c}cr(c,b){const a=this.generateWhiteboardId(c,b);return"encode"===b?{wb:[`cr_${a}`],cf:"wb"}:{action:c[0],actual:a}}sf(c,b){let a;if("encode"===b)for(a={wb:["sf"],cf:"wb",v:[]},b=0;b<c.length;b++)a.v.push(c[b]);return a}generateFreeDrawingData(c){const b=[];let a,d,f;for(let g=0;g<c.length;g+=1)a=c[g].split("_"),d=+a[0],f=+a[1],2<a.length?b.push(`sp_f_${a[2]}_${d}_${f}`):b.push(`sp_f_m_${d}_${f}`);
return b}tx(c,b){"encode"===b?(b=`tx_${c.x}_${c.y}_${c.text}`,b=null!=c.index?b+`_${c.index}`:b+"_-1",b=c.fontSize?b+`_${c.fontSize}`:b+"_0",b=c.fontColor?b+`_${c.fontColor}`:b+"_0",b={wb:[b],cf:"wb"}):(b={action:"tx",tool:"text",shape:"text",event:"mousedown",actual:{x:+c[1],y:+c[2],value:c[3]}},-1!==+c[4]&&(b.actual.index=c[4]),0!==+c[5]&&(b.actual.fontSize=+c[5]),0!==+c[6]&&(b.actual.fontColor=c[6]));return b}ds(c,b){const a=this.generateWhiteboardId(c,b);return"encode"===b?{wb:[`ds_${a}`],cf:"wb"}:
{action:c[0],actual:a}}da(c,b){const a=this.generateWhiteboardId(c,b);return"encode"===b?{wb:[`da_${a}`],cf:"wb"}:{action:c[0],actual:a}}ot(c,b){return"encode"===b?{wb:[`ot_${virtualclass.wbWrapper.keyMap[c.type]}_${c.value}`],cf:"wb"}:{action:c[0],tool:virtualclass.wbWrapper.keyMap[c[1]],actual:{value:c[2]}}}};class WhiteboardWrapper{constructor(){this.gObj={};this.keyMap={triangle:"Triangle",oval:"Circle",rectangle:"Rect",line:"Line",text:"Text",circle:"Circle",r:"rectangle",f:"freeDrawing",l:"line",c:"circle",tr:"triangle",d:"innerMouseDown",m:"innerMouseMove",u:"innerMouseUp",mousedown:"d",mousemove:"m",mouseup:"u",acd:"mousedown",acm:"mousemove",acu:"mouseup",ovalShort:"o",rectangleShort:"r",freeDrawingShort:"f",lineShort:"l",textShort:"t",triangleShort:"tr",circleShort:"c",sp:"shape",ac:"activeAll",
cl:"color",cw:"clear",sk:"strk",ft:"font",font:"ft",strk:"sk",color:"cl"};this.shapes="rectangle triangle text freeDrawing circle line".split(" ")}init(c,b){this.createWhiteboardContainerHtml(c,b);this.replay=new WhiteboardReplay;this.util=new WhiteboardUtility;this.protocol=new WhiteboardProtocol;this.cursor=new WhiteboardCursor;this.cursor.init();this.msg=new WhiteboardMessage}createWhiteboardContainerHtml(c,b){let a;var d=virtualclass.getTemplate("main","whiteboard");a="Whiteboard"===b?document.querySelector("#virtualclassWhiteboard .whiteboardContainer"):
document.getElementById(`cont${c}`);if("Whiteboard"===b){virtualclass.wbCommon.hideElement();const f=`note${c}`;b=document.querySelector(`#${f}`);null!==b?(b.classList.add("canvasContainer","current"),d=d({cn:c,hasControl:roles.hasControls()}),b.innerHTML=d):(d=`<div id='${f}' data-wb-id='${c}' class='canvasContainer current'>${d({cn:c,hasControl:roles.hasControls()})}</div>`,"_doc_0_0"!==c?a.insertAdjacentHTML("beforeend",d):(a.innerHTML=d,c=document.querySelector("#note_doc_0_0"),null!==c&&c.classList.add("current")))}else d=
d({cn:c,hasControl:roles.hasControls()}),a.innerHTML=d}};class WhiteboardCommonShape{constructor(c){this.name=c;this.default={rotatingPointOffset:40,cornerSize:13,strokeWidth:1,stroke:"#00f"};this.coreObj={angle:0,selectable:!1,fill:"none",transparentCorners:!1,width:0,height:0,backgroundColor:"transparent"}}sendMouseDownData(c){virtualclass.wbWrapper.gObj.lastSentDataTime=(new Date).getTime();c={event:"d",name:this.name,x:c.x,y:c.y};const b=virtualclass.wbWrapper.protocol.encode("sp",c);virtualclass.wbWrapper.msg.send(b);virtualclass.wbWrapper.gObj.previousData=
...
b,a){this.mousedown=!0;"freeDrawing"===this.name?(b.myPencil||(b.myPencil=new fabric.PencilBrush(b.canvas),b.myPencil.width=this.default.strokeWidth,b.currStrkSize&&(b.myPencil.width=+b.currStrkSize),b.myPencil.color=this.default.stroke,b.activeToolColor&&(b.myPencil.color=b.activeToolColor,console.log("====> stroke color ",b.myPencil.color))),a||(a={e:{isPrimary:!0}}),b.myPencil.onMouseDown(c,a)):(this.startLeft=c.x,this.startTop=c.y,this.coreObj.left=this.startLeft,this.coreObj.top=this.startTop,
this.coreObj.width=1,this.coreObj.height=1,this.coreObj.rotatingPointOffset=this.default.rotatingPointOffset*virtualclass.zoom.canvasScale,this.coreObj.cornerSize=this.default.cornerSize*virtualclass.zoom.canvasScale,this.coreObj.strokeWidth=this.default.strokeWidth,this.coreObj.stroke=this.default.stroke,b.currStrkSize&&(this.coreObj.strokeWidth=+b.currStrkSize),b.activeToolColor&&(this.coreObj.stroke=b.activeToolColor),a=virtualclass.wbWrapper.keyMap[this.name],"line"===this.name?(this.coreObj.points=
[c.x,c.y,c.x,c.y],this[this.name]=new fabric[a](this.coreObj.points,this.coreObj)):this[this.name]=new fabric[a](this.coreObj),b.canvas.add(this[this.name]))}mouseMove(){}mouseUp(c,b,a){if("freeDrawing"===this.name){this.innerMouseUp(c,b,a);if(!a.e.isTrusted)return;virtualclass.wbWrapper.gObj.previousData&&(virtualclass.wbWrapper.gObj.lastSentDataTime=(new Date).getTime(),this.chunks.push(`${c.x}_${c.y}_u`),c=virtualclass.wbWrapper.protocol.encode("sf",this.chunks),virtualclass.wbWrapper.msg.send(c),
this.chunks.length=0);this.mousedown=!1;b.activeAllObj.disableLastElement(virtualclass.gObj.currWb)}else{this.innerMouseUp(c,b,!0);if(!a.e.isTrusted)return;virtualclass.wbWrapper.gObj.previousData&&(b=virtualclass.wbWrapper.protocol.encode("sp",virtualclass.wbWrapper.gObj.previousData),virtualclass.wbWrapper.msg.send(b),b=virtualclass.wbWrapper.protocol.encode("sp",{event:"u",name:this.name,x:virtualclass.wbWrapper.gObj.previousData.x,y:virtualclass.wbWrapper.gObj.previousData.y}),virtualclass.wbWrapper.msg.send(b))}delete virtualclass.wbWrapper.gObj.previousData;
delete this.freeDrawPrevious;delete virtualclass.gObj.lastSendDataTime}innerMouseUp(c,b,a){this.mousedown=!1;"freeDrawing"!==this.name?this[this.name].setCoords():(a||(a={isPrimary:!0}),b.myPencil.onMouseUp(a),c=b.canvas.getObjects(),c[c.length-1].set({rotatingPointOffset:this.default.rotatingPointOffset*virtualclass.zoom.canvasScale,cornerSize:this.default.cornerSize*virtualclass.zoom.canvasScale}));delete b.myPencil}};class WhiteboardFreeDrawing extends WhiteboardCommonShape{constructor(c){super(c);this.name=c;this.chunks=[]}innerMouseMove(c,b,a){a||(a={e:{isPrimary:!0}});b.myPencil.onMouseMove(c,a)}mouseMove(c,b,a){this.innerMouseMove(c,b,a);a.e.isTrusted&&(this.collectingData(c),virtualclass.wbWrapper.msg.optimizeToSend(this.chunks,3E3,"sf"))}collectingData(c){c={x:c.x,y:c.y};if(!this.freeDrawPrevious||1<Math.abs(c.x-this.freeDrawPrevious.x)||1<Math.abs(c.y-this.freeDrawPrevious.y))this.freeDrawPrevious=c,this.chunks.push(`${c.x}_${c.y}`),
console.log("====> actual x, y sendin =============FREE DRAWING==== before scale ",c.x,c.y);virtualclass.wbWrapper.gObj.previousData=c}};class WhiteboardRectangle extends WhiteboardCommonShape{constructor(c){super(c);this.name=c}mouseMove(c,b){this.sendMouseMoveData(c);this.innerMouseMove(c,b)}innerMouseMove(c,b){if(this.mousedown){var a=c.x;c=c.y;var d=a-this.startLeft,f=c-this.startTop;0<d?this.rectangle.set("width",d):(this.rectangle.set("left",a),this.rectangle.set("width",-1*d));0<f?this.rectangle.set("height",f):(this.rectangle.set("top",c),this.rectangle.set("height",-1*f));b.canvas.renderAll()}}};class WhiteboardLine extends WhiteboardCommonShape{constructor(c){super(c);this.name=c}mouseMove(c,b){this.sendMouseMoveData(c);this.innerMouseMove(c,b)}innerMouseMove(c,b){this.mousedown&&(this.line.set({x2:c.x,y2:c.y}),b.canvas.renderAll())}};class WhiteboardTriangle extends WhiteboardCommonShape{constructor(c){super(c);this.name=c}mouseMove(c,b){this.sendMouseMoveData(c);this.innerMouseMove(c,b)}innerMouseMove(c,b){if(this.mousedown){var a=c.x;c=c.y;var d=a-this.startLeft,f=c-this.startTop;0<d?this.triangle.set("width",d):this.triangle.set("width",-1*d);0<f?(this.triangle.set("angle",0),this.triangle.set("height",f),0<d?this.triangle.set("left",a-d):this.triangle.set("left",a)):(this.triangle.set("angle",180),this.triangle.set("height",
-1*f),this.triangle.set("top",c+-1*f),0<d?this.triangle.set("left",a):this.triangle.set("left",a+-1*d));b.canvas.renderAll()}}};class WhiteboardCircle extends WhiteboardCommonShape{constructor(c){super(c);this.name=c}mouseDown(c,b,a){this.innerMouseDown(c,b,a);a.e.isTrusted&&this.sendMouseDownData(c)}innerMouseDown(c,b){this.mousedown=!0;this.startLeft=c.x;this.startTop=c.y;this.coreObj.left=this.startLeft;this.coreObj.top=this.startTop;this.coreObj.rx=c.x-this.startLet;this.coreObj.ry=c.y-this.startTop;this.coreObj.angle=0;this.coreObj.stroke=this.default.stroke;this.coreObj.strokeWidth=this.default.strokeWidth;this.coreObj.rotatingPointOffset=
this.default.rotatingPointOffset*virtualclass.zoom.canvasScale;this.coreObj.cornerSize=this.default.cornerSize*virtualclass.zoom.canvasScale;this.coreObj.strokeWidth=this.default.strokeWidth;b.activeToolColor&&(this.coreObj.stroke=b.activeToolColor);b.currStrkSize&&(this.coreObj.strokeWidth=+b.currStrkSize);this[this.name]=new fabric.Ellipse(this.coreObj);b.canvas.add(this[this.name])}mouseMove(c,b){this.sendMouseMoveData(c);this.innerMouseMove(c,b)}innerMouseMove(c,b){let a=Math.abs(this.startLeft-
c.x)/2,d=Math.abs(this.startTop-c.y)/2;a>this.circle.strokeWidth&&(a-=this.circle.strokeWidth/2);d>this.circle.strokeWidth&&(d-=this.circle.strokeWidth/2);this.circle.set({rx:a,ry:d});this.startLeft>c.x?this.circle.set({originX:"right"}):this.circle.set({originX:"left"});this.startTop>c.y?this.circle.set({originY:"bottom"}):this.circle.set({originY:"top"});b.canvas.renderAll()}};class WhiteboardCursor{init(){this.arrImg||(this.arrImg=new Image,this.arrImg.src="https://cdn.congrea.net/resources/images/arrow.png",this.arrImgDraw=!1,this.arrImg.onload=()=>{this.arrImgDraw=!0})}send(c){this.lastarrowtime||(this.lastarrowtime=(new Date).getTime());this.presentarrowtime=(new Date).getTime();100<=this.presentarrowtime-this.lastarrowtime&&!virtualclass.wb[virtualclass.gObj.currWb].mousedown&&(ioAdapter.send(c),this.lastarrowtime=(new Date).getTime())}handle(c){c.isTrusted&&(c=virtualclass.wb[virtualclass.gObj.currWb].canvas.getPointer(c,
!0),c=virtualclass.wbWrapper.protocol.changeWithScale("divide",c),this.send({msg:`${c.x}_${c.y}`,cf:"ca"}))}onMessage(c){const b={};c=c.msg.split("_");b.x=c[0];b.y=c[1];this.create(b)}create(c){const b=virtualclass.gObj.currWb;c={x:c.x*virtualclass.zoom.canvasScale,y:c.y*virtualclass.zoom.canvasScale};virtualclass.posY=c.y;virtualclass.posX=c.x;this.draw(c,b);null!=virtualclass.pdfRender[b].scroll.Y&&virtualclass.pdfRender[b].customMoustPointer({y:virtualclass.posY},"Y",virtualclass.posY);null!=virtualclass.pdfRender[b].scroll.X&&
virtualclass.pdfRender[b].customMoustPointer({x:virtualclass.posX},"X",virtualclass.posX)}draw(c,b){const a=this.arrImg,d=virtualclass.wb[b].canvas.getContext("2d");d.clearRect(0,0,virtualclass.wb[b].canvas.width,virtualclass.wb[b].canvas.height);virtualclass.wb[b].canvas.renderAll();d.save();d.beginPath();d.translate(c.x,c.y);d.drawImage(a,-3,-3,18,24);d.closePath();d.restore()}};class WhiteboardMessage{send(c){roles.hasControls()&&(console.log("sending the data here guys ",JSON.stringify(c)),console.log("====> free drawing ",JSON.stringify(c)),ioAdapter.mustSend(c),"sf"===c.wb[0].substring(0,2)?virtualclass.wbWrapper.util.storeAtMemory(c.wb,virtualclass.gObj.currWb,c.v):virtualclass.wbWrapper.util.storeAtMemory(c.wb,virtualclass.gObj.currWb))}onMessage(c){if("sf"===c.message.wb[0].substring(0,2)){var b=c.fromUser.role;c=virtualclass.wbWrapper.protocol.generateFreeDrawingData(c.message.v,
c.message.s,!0);let a;for(let d=0;d<c.length;d+=1)a={message:{wb:c[d]},fromUser:{role:b}},this.onMessage(a)}else Array.isArray(c.message.wb)||(c.message.wb=[c.message.wb]),b=c.message.wb,virtualclass.vutil.storeWhiteboardAtInlineMemory(c.message.wb),virtualclass.zoom.canvasScale&&virtualclass.gObj.currWb&&"object"===typeof virtualclass.wb[virtualclass.gObj.currWb]&&"t"===c.fromUser.role&&virtualclass.wbWrapper.util.applyCommand(b,virtualclass.gObj.currWb)}optimizeToSend(c,b,a){virtualclass.wbWrapper.gObj.previousData=
c;virtualclass.wbWrapper.gObj.presentSendDataTime=(new Date).getTime();virtualclass.wbWrapper.gObj.presentSendDataTime-virtualclass.wbWrapper.gObj.lastSentDataTime>=b&&(a&&(c=virtualclass.wbWrapper.protocol.encode(a,c),this.send(c)),virtualclass.wbWrapper.gObj.lastSentDataTime=(new Date).getTime(),"sf"===a&&(virtualclass.wb[virtualclass.gObj.currWb].freeDrawingObj.chunks.length=0))}};class WhiteboardText{constructor(c){this.default={rotatingPointOffset:40,cornerSize:13,fontSize:20,fill:"#00f"};this.name=c;this.coreObj={fontWeight:"normal",fontFamily:"arial",padding:7}}isObjectInEditingMode(c){const b=c.canvas.getObjects("i-text");for(let a=0;a<b.length;a+=1)b[a].isEditing&&c.canvas.trigger("text:editing:exited",{target:b[a]})}mouseDown(c,b){virtualclass.wbWrapper.gObj.textSelected||this.textEditing?virtualclass.wbWrapper.gObj.textSelected&&!b.activeAllObj.activeDown&&(virtualclass.wbWrapper.gObj.textSelected.enterEditing(),
c=b.canvas.getObjects("i-text"),"Enter your text"!=virtualclass.wbWrapper.gObj.textSelected.text.trim()&&(this.editingIndex=c.indexOf(virtualclass.wbWrapper.gObj.textSelected)),b.activeAllObj.disable(virtualclass.gObj.currWb,"i-text"),this.textEditing=!0):this.renderText(c,b)}updateText(c,b,a){c.text?a.set("text",c.text):a.set("text",c.value);c.fontSize&&a.set("fontSize",c.fontSize);c.fontColor&&a.set("fill",c.fontColor);b.canvas.renderAll()}renderText(c,b){var a=b.canvas.getObjects("i-text");0<a.length&&
null!=c.index?(a=a[c.index])?this.updateText(c,b,a):this.createText(c,b):this.createText(c,b)}createText(c,b){""===c.value||this.isEmptyText(b)||(this.startLeft=c.x,this.startTop=c.y,this.coreObj.left=this.startLeft,this.coreObj.top=this.startTop,c=c.value?c.value:"Enter your text",this.coreObj.rotatingPointOffset=this.default.rotatingPointOffset*virtualclass.zoom.canvasScale,this.coreObj.cornerSize=this.default.cornerSize*virtualclass.zoom.canvasScale,this.coreObj.fontSize=this.default.fontSize,
b.textFontSize&&(this.coreObj.fontSize=+b.textFontSize),this.coreObj.fill=this.default.fill,b.activeToolColor&&(this.coreObj.fill=b.activeToolColor),this[this.name]=new fabric.IText(c,this.coreObj),b.canvas.add(this[this.name]))}isEmptyText(c){c=c.canvas.getObjects("i-text");for(let b=0;b<c.length;b++)if("Enter your text"===c[b].text)return!0;return!1}finalizeText(c){const b=virtualclass.wb[virtualclass.gObj.currWb];this.textEditing=!1;if(!this.isDefault(c.target)){virtualclass.wbWrapper.gObj.lastSentDataTime=
(new Date).getTime();var a={x:c.target.left,y:c.target.top,text:c.target.text};null!=this.editingIndex&&(a.index=this.editingIndex);+b.textFontSize&&c.target.fontSize!==+b.textFontSize&&(a.fontSize=b.textFontSize);b.activeToolColor&&c.target.fill!==b.activeToolColor&&(a.fontColor=b.activeToolColor);virtualclass.wbWrapper.msg.optimizeToSend(a,0,"tx");delete this.editingIndex;b.activeAllObj.enable(virtualclass.gObj.currWb,"i-text");(a.fontColor||a.fontSize)&&this.updateText(a,b,c.target)}}isDefault(c){return"Enter your text"==
c.text.trim()}afterSelected(c){console.log("selected 1");virtualclass.wbWrapper.gObj.textSelected=c}afterDeSelected(){console.log("deselected 1");virtualclass.wbWrapper.gObj.textSelected=null}mouseMove(){console.log("mouse move")}mouseUp(){console.log("mouse up")}};class Whiteboard{constructor(){this.selectedTool=this.canvas=null;this.rectangleObj=new WhiteboardRectangle("rectangle");this.activeAllObj=new WhiteboardActiveAll;this.freeDrawingObj=new WhiteboardFreeDrawing("freeDrawing");this.lineObj=new WhiteboardLine("line");this.triangleObj=new WhiteboardTriangle("triangle");this.circleObj=new WhiteboardCircle("circle");this.textObj=new WhiteboardText("text");this.gObj={}}init(c){this.wbId=c;delete virtualclass.wbWrapper.gObj.textSelected;this.textObj.textEditing=
...
this.canvas.on("mouse:move",this.handlerMouseMove.bind(this));this.canvas.on("mouse:up",this.handlerMouseUp.bind(this));this.canvas.on("text:editing:exited",c=>{setTimeout(()=>{this.textObj.finalizeText(c)},0)})}handlerSelection(c){c.selected&&"activeAll"===this.selectedTool?this.activeAllObj.activeDown=!0:c.selected&&"i-text"===c.selected[0].type?(virtualclass.wbWrapper.gObj.textSelected=c.selected[0],console.log("====> selected text ",virtualclass.wbWrapper.gObj.textSelected)):c.deselected&&(this.activeAllObj.activeDown=
!1,virtualclass.wbWrapper.gObj.textSelected=!1)}triggerGetPointer(c){return virtualclass.wb[c.wId?c.wId:virtualclass.gObj.currWb].canvas.getPointer(c)}handlerMouseDown(c){if(c.e.isTrusted&&this.selectedTool&&this[`${this.selectedTool}Obj`]){this.mousedown=!0;const b=this.triggerGetPointer(c.e);this[`${this.selectedTool}Obj`].mouseDown(b,this,c)}}handlerMouseMove(c){if(c.e.isTrusted){var b=this.triggerGetPointer(c.e);this.mousedown&&this.selectedTool&&this[`${this.selectedTool}Obj`].mouseMove(b,this,
c);virtualclass.wbWrapper.cursor.handle(c.e)}}handlerMouseUp(c){if(c.e.isTrusted){var b=this.triggerGetPointer(c.e);this.mousedown&&this.selectedTool&&this[`${this.selectedTool}Obj`].mouseUp(b,this,c);this.mousedown=!1}}toolbarHandler(c){({parentNode:c}=c.currentTarget);this.innerToolbarHandler(c.dataset.tool,virtualclass.gObj.currWb);if(0<virtualclass.wb[virtualclass.gObj.currWb].canvas.getActiveObjects().length){virtualclass.wb[virtualclass.gObj.currWb].canvas.discardActiveObject();virtualclass.wb[virtualclass.gObj.currWb].canvas.renderAll();
const b=virtualclass.wbWrapper.protocol.encode("ds",virtualclass.gObj.currWb);virtualclass.wbWrapper.msg.send(b)}(virtualclass.wbWrapper.shapes.includes(c.dataset.tool)||"activeAll"===c.dataset.tool)&&virtualclass.wbWrapper.util.makeActiveTool(c.id)}selectTool(c){if("rectangle"===c||"line"===c||"circle"===c||"triangle"===c||"text"===c||"activeAll"===c||"freeDrawing"===c)this.selectedTool=c;"stroke"!==c&&"font"!==c&&("activeAll"!==c&&"text"!==c?virtualclass.wbWrapper.util.strokeSizeSelector():"text"===
c&&virtualclass.wbWrapper.util.fontSizeSelector())}innerToolbarHandler(c,b){virtualclass.wbWrapper.util.closeTray();this.canvas.isDrawingMode=!1;this.selectTool(c);if("rectangle"!==c&&"line"!==c&&"circle"!==c&&"triangle"!==c&&"text"!==c)this[c](b);else"text"===c?(this.activeAllObj.enable(b,"i-text"),this.activeAllObj.disableBut(b,"i-text"),delete virtualclass.wbWrapper.gObj.textSelected,this.textObj.textEditing=!1):this.activeAllObj.disable(b)}shapes(){const c=document.querySelector(`#shapes${this.wbId}`);
virtualclass.wbWrapper.util.handleTrayDisplay(c)}stroke(){const c=document.querySelector(`#commandToolsWrapper${this.wbId} .strkSizeList`);virtualclass.wbWrapper.util.handleTrayDisplay(c);virtualclass.wbWrapper.util.initActiveElement(`#t_strk${this.wbId} ul`,{type:"strk",prop:"stroke"})}font(){const c=document.querySelector(`#commandToolsWrapper${this.wbId} .fontSizeList`);virtualclass.wbWrapper.util.handleTrayDisplay(c);virtualclass.wbWrapper.util.initActiveElement(`#t_font${this.wbId} ul`,{type:"font",
prop:"font"})}color(){const c=document.querySelector(`#commandToolsWrapper${this.wbId} .table`);virtualclass.wbWrapper.util.handleTrayDisplay(c);virtualclass.wbWrapper.util.initActiveElement(`#colorList${this.wbId}`,{type:"color",prop:"color"})}freeDrawing(c){this.canvas.isDrawingMode=!1;this.activeAllObj.disable(c)}activeAll(c){this.activeAllObj.enable(c);console.log("====> I am being active here ")}clearAll(c){const b=virtualclass.lang.getString("clearAllWarnMessageW");virtualclass.popup.confirmInput(b,
b=>{b&&(this.clear(),b=virtualclass.wbWrapper.protocol.encode("cr",c),virtualclass.wbWrapper.msg.send(b))})}clear(){this.canvas.clear();this.replayObjs=[];this.canvas._objects=[];delete virtualclass.wbWrapper.gObj.textSelected;this.textObj.textEditing=!1}};(function(c){c.message={ActiveAll:"Active all",addcontext:"Add bookmark",Addoption:"Add option",addnew:"Add new",adminusr:"Admin user",appPrerequites:"Validate system requirements",assign:"Assign",assignDisable:"Reclaim controls",askplayMessage:'<span id="askplaymsg"> Should we start playing the session?</span><br /><span id="remaingmsg">Remaining data will be downloaded in the background.</span>',askplaymsg:'<span id="askplaymsg"> "Downloading in progress, click Play to begin </span>"',audioInput:"Audio input",
...
Delete:"Delete",DevicesNotFoundError:"Webcam is not available, or not connected properly.",deletepopup:"Are you sure to delete ?",disable:"Disable",disabled:"disabled",disableAllVideo:"Disable all video",dltDisabled:"Can be deleted  by creator of the poll",documentShare:"Share document",DocumentSharing:"Document sharing",DocumentSharedbHeading:"Document dashboard",docUploadSuccess:"Document uploaded successfully",download:"Download",downloadFile:"Download file",downloadsession:"Please wait while the recording is downloaded",
drawArea:"Draw area",dropfilehere:"Drop files here",duplicateUploadMsg:"This file already exists, please upload new file.",edit:"Edit",editorCode:"Code editor",edittitle:"Edit title",editorRichDisable:"Write mode",editorRichEnable:"Read only",educator:"Educator",EHTMLPresentUrl:"Enter HTML5 presentation URL",enable:"Enable",enablehistory:'"Browser history" should be enabled during the session.',enableAllVideo:"Enable all video",enteryouryoutubeurl:"Enter YouTube/online video URL",etDisabledA:"Can\u2019t edit, poll attempted",
etDisabledCr:"Can be edited by creator of the poll",ETime:"Elapsed time",FF2:"Fast forward 2",FF8:"Fast forward 8",filenotsave:"Your file could not be saved.",filetsaveTS:"The session is ready to save.",Finish:"Finish",fitToPage:"Fit to page",fitToScreen:"Fit to screen",Freehand:"Free hand",general:"General",Greport:"Grade report",highBandWidthSpeed:"Your bandwidth is good enough.",httpsmissing:"Only secure origins(https) are allowed for screen sharing.",ieBrowserIssue:"Internet Explorer is not supported, Congrea fully support Chrome  and Firefox.",
...
b);null!=b[0]&&b[0].parentNode.removeChild(b[0])},multiMediaMsg(b){},canvasDrawMsg(b,a){let d;document.getElementById(`vcanvas${a}`).classList.add("canvasMsgBoxParent");"Firefox"===virtualclass.system.mybrowser.name?(d=virtualclass.lang.getString("canvasDrawMsg"),this.displayMessage(d,"canvasDrawMsgContFirefox",this.msgBoxClass+b,"containerWb")):"Chrome"===virtualclass.system.mybrowser.name&&(d=virtualclass.lang.getString("canvasDrawMsg"),this.displayMessage(d,"canvasDrawMsgContChrome",this.msgBoxClass+
b,"containerWb",null,a))},drawLabel(b,a){const d=virtualclass.lang.getString("drawArea");this.displayMessage(d,"canvasDrawArea",this.msgBoxClass+b,"containerWb",!1,a)},displayMsgBox(b,a){const d=this.customCreateElement("div",b),f=this.customCreateElement("p",`${b}Para`);f.innerHTML=virtualclass.lang.getString(a);d.appendChild(f);a=this.customCreateElement("a",`${b}Anchor`);a.href="#";a.innerHTML=virtualclass.lang.getString("reload");a.onclick=function(){c.location.reload()};d.appendChild(a);b=document.getElementById("divForReloadMsg"===
b?"virtualclassCont":"virtualclassAppLeftPanel");b.insertBefore(d,b.firstChild)},displayServerError(b,a){b=this.customCreateElement("div",b);b.innerHTML=a;a=document.getElementById("vcanvas");a.parentNode.insertBefore(b,a)},removeElement(b){b=document.getElementById(b);null!=b&&b.parentNode.removeChild(b)},disableSSUI(){const b=document.getElementById("virtualclassScreenShareTool");null!=b&&(b.style.opacity="0.5",b.style.pointerEvents="none")},disableLeftAppBar(){const b=document.getElementById("virtualclassOptionsCont");
...
virtualclass.error.push(virtualclass.lang.getString("operaBrowserIssue",[c,d]))):this.mybrowser.detectIE()&&(virtualclass.gObj.errIE=!0,virtualclass.error.push(virtualclass.lang.getString("ieBrowserIssue")),virtualclass.vutil.initDisableVirtualClass())},mediaDevices:{webcamErr:[],async getMediaDeviceInfo(){navigator.mediaDevices&&navigator.mediaDevices.enumerateDevices&&(navigator.enumerateDevices=async function(){return await navigator.mediaDevices.enumerateDevices()});let b=[];const d="https:"===
...
a}),this.transceivers.forEach((a,c)=>{d._gather(a.mid,c)});else if("answer"===a.type){c=p.splitSections(d.remoteDescription.sdp);f=c.shift();const a=0<p.matchPrefix(f,"a=ice-lite").length;c.forEach((c,g)=>{const k=d.transceivers[g],{iceGatherer:n}=k,{iceTransport:D}=k,{dtlsTransport:r}=k,{localCapabilities:q}=k,{remoteCapabilities:G}=k;if(!(p.isRejected(c)&&0===p.matchPrefix(c,"a=bundle-only").length||k.isDatachannel)){const u=p.getIceParameters(c,f);c=p.getDtlsParameters(c,f);a&&(c.role="server");
d.usingBundle&&0!==g||(d._gather(k.mid,g),"new"===D.state&&D.start(n,u,a?"controlling":"controlled"),"new"===r.state&&r.start(c));g=m(q,G);d._transceive(k,0<g.codecs.length,!1)}})}this.localDescription={type:a.type,sdp:a.sdp};switch(a.type){case "offer":this._updateSignalingState("have-local-offer");break;case "answer":this._updateSignalingState("stable");break;default:throw new TypeError(`unsupported type "${a.type}"`);}return Promise.resolve()};c.prototype.setRemoteDescription=function(c){const f=
this;if(!n("setRemoteDescription",c.type,this.signalingState)||this._isClosed)return Promise.reject(u("InvalidStateError",`Can not set remote ${c.type} in state ${f.signalingState}`));const g={};this.remoteStreams.forEach(a=>{g[a.id]=a});const k=[],m=p.splitSections(c.sdp),D=m.shift(),q=0<p.matchPrefix(D,"a=ice-lite").length,G=0<p.matchPrefix(D,"a=group:BUNDLE ").length;this.usingBundle=G;const A=p.matchPrefix(D,"a=ice-options:")[0];this.canTrickleIceCandidates=A?0<=A.substr(14).split(" ").indexOf("trickle"):
!1;m.forEach((m,n)=>{var u=p.splitLines(m),v=p.getKind(m),A=p.isRejected(m)&&0===p.matchPrefix(m,"a=bundle-only").length,F=u[0].substr(2).split(" ")[2];u=p.getDirection(m,D);const C=p.parseMsid(m);var B=p.getMid(m)||p.generateIdentifier();if("application"===v&&"DTLS/SCTP"===F)f.transceivers[n]={mid:B,isDatachannel:!0};else{var H=p.parseRtpParameters(m);if(!A){var M=p.getIceParameters(m,D);var I=p.getDtlsParameters(m,D);I.role="client"}F=p.parseRtpEncodingParameters(m);var N=p.parseRtcpParameters(m),
...
d){for(let a=0;a<d.length;a++)b[a]=d[a];A=d[4095]}else for(d=0;d<b.length;d++)b[d]=A},virtualclass.media.audioPlayerNode.connect(this.Html5Audio.audioContext.destination)},queueWithFalback(a,b){null==k[b]&&(k[b]=[]);null==n[b]&&(n[b]=[]);for(let d=0;d<a.length;d++)n[b].push(a[d]);for(;4096<=n[b].length;)a=n[b].splice(0,4096),k[b].push(new Float32Array(a))},getMergedAudio(){t=[];z=0;for(r in k){var a=this.getAudioChunks(r);if(null!=a)if(z++,1===z)for(var b=0;4096>b;b++)t[b]=a[b];else for(b=0;4096>
b;b++)t[b]+=a[b]}if(1===z)return t;if(1<z){for(a=0;4096>a;a++)t[a]/=z;return t}},getAudioChunks(a){if(null!==k){if(9<=k[a].length){for(;3<=k[a].length;)k[a].shift();m[a]=!0;return k[a].shift()}if(2<=k[a].length||0<k[a].length&&!0===m[a])return m[a]=!0,k[a].shift();m[a]=!1;0===k[a].length&&delete k[a]}},actualManiPulateStream(){this.triggermaniPulateStream=!0;const a=virtualclass.media;setTimeout(()=>{a.detectAudioWorklet()?a.audio.maniPulateStream():a.audio.maniPulateStreamWithFallback()},1E3)},maniPulateStream(){const {stream:a}=
g;"undefined"!==typeof v&&v.disconnect();"undefined"!==typeof a&&null!=a&&g.audio.Html5Audio.audioContext.audioWorklet.addModule(workletAudioSendBlob).then(()=>{var d=g.audio.Html5Audio.audioContext.createMediaStreamSource(a);const c=g.audio.Html5Audio.audioContext.createBiquadFilter();c.type="lowpass";c.frequency.value=2E3;d.connect(c);v=new AudioWorkletNode(g.audio.Html5Audio.audioContext,"worklet-audio-send");v.onprocessorerror=function(a){g.audio.notifiyMuteAudio()};c.connect(v);v.connect(g.audio.Html5Audio.audioContext.destination);
...
virtualclass.gObj.wbData[a]=virtualclass.gObj.wbData[a].concat(b)},attachWhiteboardPopupHandler(b){c.addEventListener("mouseup",a=>{roles.hasControls()&&virtualclass.wbWrapper.util.closeTray()})},setScreenShareDefualtColor(){var b=virtualclass.gObj.prvRequestScreenUser;virtualclass.gObj.prvRequestScreenUser&&(b=chatContainerEvent.elementFromShadowDom(`#ml${b} .icon-stdscreenImg`),null!==b&&(b.setAttribute("data-dcolor","black"),b.parentNode.setAttribute("data-title",virtualclass.lang.getString("requestScreenShare"))))},
hidePageNumber(){const b=document.querySelector("#docShareNav");null!==b&&(b.classList.add("hide"),b.classList.remove("show"))},sendSpeedByMobile(b){"mobTab"===virtualclass.system.desktop&&ioAdapter.sendSpeed(b)},isRefreshMode(){return virtualclass.config.makeWebSocketReady},triggerFinalizeTextIfAny(b){},inputFocusHandler(b){console.log("====> focus input");b&&"string"!==typeof b&&virtualclass.isPlayMode&&virtualclass.userInteractivity.triggerPause();c.innerWidth>c.innerHeight&&(virtualclass.gObj.initHeight=
c.innerHeight,b=document.querySelector("#tabs .ui-state-focus"),"mobTab"===virtualclass.system.device&&null==b&&"Safari"!=virtualclass.system.mybrowser.name&&document.getElementById("virtualclassCont").classList.add("focusInput"))},inputFocusOutHandler(){console.log("====> focus output");"mobTab"===virtualclass.system.device&&document.getElementById("virtualclassCont").classList.remove("focusInput")},checkUserRole(){return virtualclass.isPlayMode?"t"===wbUser.orginalUserRole:"t"===virtualclass.gObj.uRole||
"p"===virtualclass.gObj.uRole},closeElement(b){b.classList.contains("open")&&(b.classList.remove("open"),b.classList.add("close"))},canvasDimensionFromPixel(b){const a=parseFloat(c.getComputedStyle(b).width);b=parseFloat(c.getComputedStyle(b).height);return{width:a,height:b}}}})(window);let globalImageData={};
...
b,io.onRecJson(b),console.log("====> my serieal ",b.m.serial)}},async getDataFromCacheIn(){for(var b=await this.db.transaction("cacheIn").store.openCursor();b;)b.value&&(Array.isArray(this.mycacheInQueue[b.key[0]])||(this.mycacheInQueue[b.key[0]]=[]),this.mycacheInQueue[b.key[0]][b.key[1]]=b.value),b=await b.continue();if(0<this.mycacheInQueue.length)for(const a in this.mycacheInQueue){ioMissingPackets.validateAllUserVariables(a);for(const d in this.mycacheInQueue[a]){b=JSON.parse(this.mycacheInQueue[a][d]);
ioMissingPackets.executedUserSerial[a]=b.m.userSerial;ioMissingPackets.executedUserStore[a][b.m.userSerial]=b;try{if("eddata"!==b.m.cf&&"colorIndicator"!==b.m.cf&&"reqscreen"!==b.m.cf&&"msg"!==b.m.cf)io.onRecJson(b)}catch(f){}}}},async getDataFromCacheOut(){for(var b=await this.db.transaction("cacheOut").store.openCursor();b;)b.value&&(Array.isArray(this.mycacheOutQueue[b.key[0]])||(this.mycacheOutQueue[b.key[0]]=[]),this.mycacheOutQueue[b.key[0]][b.key[1]]=b.value),b=await b.continue();if(0<this.mycacheOutQueue.length)for(const a in this.mycacheOutQueue)for(const d in this.mycacheOutQueue[a]){b=
...
a.prototype.replayWindowAction=function(a){document.getElementById("replayContainer").style.display=a};a.prototype.progressBarAction=function(a){document.getElementById("recordingContainer").style.display=a};a.prototype.confirmInput=function(a,b,d,c,f,t){var g=document.getElementById("about-modal");virtualclass.popup.open(g);this.hideAllPopups();g=document.getElementById("confirm");g.style.display="block";var k=g.getElementsByClassName("confirmChild");if(0<k.length)for(;1<=k.length;)k[0].parentNode.removeChild(k[0]);
k=document.createElement("div");k.id="confirmMessage";k.className="confirmChild";k.innerHTML=a;g.appendChild(k);const m=this;a=function(){m.confirmInit(this.id,b,d,c,f,t)};k=document.createElement("div");k.id="confirmOk";k.className="confirmButton confirmChild";k.addEventListener("click",a);const n=document.createElement("button");n.id="confirmOkButton";n.className="icon-check";n.innerHTML=virtualclass.lang.getString("confirmOk");k.appendChild(n);g.appendChild(k);k=document.createElement("div");k.id=
...
f);document.getElementById("saveNpublish").addEventListener("click",f)};a.prototype.action=function(a,b,d,c,f,t){b(a)};a.prototype.confirmInit=function(a,b,d,c,f,t){virtualclass.popup.closeElem();a="confirmOk"===a?virtualclass.popup.confirmOk():virtualclass.popup.confirmCancel();b(a,d,c,f,t)};a.prototype.confirmCancel=function(){return!1};a.prototype.confirmOk=function(){return!0};a.prototype.waitMsg=function(a){let b=0;"undefined"!==typeof a&&(b=1300);setTimeout(()=>{virtualclass.network.netWorkElementIsReady()},
...
2+d)+(2===d?"":1===d?"=":"==")},base64DecToArrInt(a){a=base64DecToArr(a);const b=new Int8Array(a.length);for(let d=0;d<a.length;d++)b[d]=128<=a[d]?-1*(a[d]-127):a[d];return b},base64DecToArr(a,b){a=a.replace(/[^A-Za-z0-9\+\/]/g,"");const d=a.length;b=b?Math.ceil((3*d+1>>2)/b)*b:3*d+1>>2;const c=new Uint8Array(b);for(var k,m=0,n=0,r=0;r<d;r++){k=r&3;var t=a.charCodeAt(r);m|=(64<t&&91>t?t-65:96<t&&123>t?t-71:47<t&&58>t?t+4:43===t?62:47===t?63:0)<<6*(3-k);if(3===k||1===d-r){for(k=0;3>k&&n<b;k++,n++)c[n]=
...
c.setAttribute("style",`background-color:${a[d]}`);b.push({content:c,value:a[d]})}return this.makeDropdown_("Color","color",b)};b.prototype.makeDropdown_=function(a,b,d,c){function g(){const a=p.parentNode.parentNode;q?(a.classList.remove("open"),a.classList.add("close")):(a.classList.remove("close"),a.classList.add("open"))}function k(){q&&(g(),f.off(document,"click",k,!0),q=!1);A=!0;setTimeout(()=>{A=!1},0)}function m(a,d){"object"!==typeof a&&(a=document.createTextNode(String(a)));a=f.elt("a",
[a]);f.on(a,"click",f.stopEventAnd(()=>{k();n.trigger(b,d+c)}));p.appendChild(a)}c=c||"";const n=this;var u=virtualclass.vutil.smallizeFirstLetter(a);a="Font"===a?f.elt("a",`${a} \u25be`,{class:"vceditor-btn vceditor-dropdown"}):f.elt("a",null,{class:`vceditor-btn vceditor-dropdown ${u}`});const p=f.elt("ul",[],{class:`vceditor-dropdown-menu ${u}-menu`});a.appendChild(p);let q=!1,A=!1;for(u=0;u<d.length;u++){const {content:a}=d[u],{value:b}=d[u];m(a,b)}f.on(a,"click",f.stopEventAnd(()=>{A||q||(g(),
...
d||[];if(a.length!==this.baseLength)throw Error("The operation's base length must be equal to the string's length.");const c=[];let f=0,g,m,n=0;const {ops:r}=this;for(let p=0,q=r.length;p<q;p++){const q=r[p];if(q.isRetain()){if(n+q.chars>a.length)throw Error("Operation can't retain more characters than are left in the string.");c[f++]=a.slice(n,n+q.chars);for(g=0;g<q.chars;g++){var A=b[n+g]||{};const a={};for(m in A)a[m]=A[m],k.assert(!1!==a[m]);for(m in q.attributes)!1===q.attributes[m]?delete a[m]:
a[m]=q.attributes[m],k.assert(!1!==a[m]);d.push(a)}n+=q.chars}else if(q.isInsert())for(c[f++]=q.text,g=0;g<q.text.length;g++){A={};for(m in q.attributes)A[m]=q.attributes[m],k.assert(!1!==A[m]);d.push(A)}else n+=q.chars}if(n!==a.length)throw Error("The operation didn't operate on the whole string.");a=c.join("");k.assert(a.length===d.length);return a};a.prototype.invert=function(b){let d=0;const c=new a,{ops:f}=this;for(let a=0,g=f.length;a<g;a++){const g=f[a];g.isRetain()?(c.retain(g.chars),d+=g.chars):
g.isInsert()?c.delete(g.text.length):(c.insert(b.slice(d,d+g.chars)),d+=g.chars)}return c};a.prototype.compose=function(b){function d(a,b,d){const c={};let f;for(f in a)c[f]=a[f];for(f in b)d&&!1===b[f]?delete c[f]:c[f]=b[f];return c}if(this.targetLength!==b.baseLength)throw Error("The base length of the second operation has to be the target length of the first operation");const c=new a,f=this.clone().ops;b=b.clone().ops;let g=0,k=0,m=f[g++],p=b[k++],q;for(;"undefined"!==typeof m||"undefined"!==typeof p;)if(m&&
m.isDelete())c.delete(m.chars),m=f[g++];else if(p&&p.isInsert())c.insert(p.text,p.attributes),p=b[k++];else{if("undefined"===typeof m)throw Error("Cannot compose operations: first operation is too short.");if("undefined"===typeof p)throw Error("Cannot compose operations: first operation is too long.");if(m.isRetain()&&p.isRetain())q=d(m.attributes,p.attributes),m.chars>p.chars?(c.retain(p.chars,q),m.chars-=p.chars,p=b[k++]):m.chars===p.chars?(c.retain(m.chars,q),m=f[g++],p=b[k++]):(c.retain(m.chars,
q),p.chars-=m.chars,m=f[g++]);else if(m.isInsert()&&p.isDelete())m.text.length>p.chars?(m.text=m.text.slice(p.chars),p=b[k++]):m.text.length===p.chars?(m=f[g++],p=b[k++]):(p.chars-=m.text.length,m=f[g++]);else if(m.isInsert()&&p.isRetain())q=d(m.attributes,p.attributes,!0),m.text.length>p.chars?(c.insert(m.text.slice(0,p.chars),q),m.text=m.text.slice(p.chars),p=b[k++]):m.text.length===p.chars?(c.insert(m.text,q),m=f[g++],p=b[k++]):(c.insert(m.text,q),p.chars-=m.text.length,m=f[g++]);else if(m.isRetain()&&
...
for(b in a)if(Object.prototype.hasOwnProperty.call(a,b)){var d=f.getClientObject(b);a[b].name&&d.setName(a[b].name);({selection:d}=a[b]);d?f.clients[b].updateSelection(f.transformSelection(Selection.fromJSON(d))):f.clients[b].removeSelection()}},reconnect(){f.serverReconnect()}})}function n(a){return a[a.length-1]}const {Client:r}=c,{Cursor:t}=c,{UndoManager:z}=c,{TextOperation:v}=c,{WrappedOperation:u}=c;f.prototype.invert=function(){return new f(this.cursorAfter,this.cursorBefore)};f.prototype.compose=
...
function(a){return this.cloneWithNewAttribute_(b.LINE_ALIGN,a)};a.prototype.listItem=function(a){vceditor.utils.assert(!1===a||"u"===a||"o"===a||"t"===a||"tc"===a);return this.cloneWithNewAttribute_(b.LIST_TYPE,a)};a.prototype.getIndent=function(){return this.attributes[b.LINE_INDENT]||0};a.prototype.getAlign=function(){return this.attributes[b.LINE_ALIGN]||0};a.prototype.getListItem=function(){return this.attributes[b.LIST_TYPE]||!1};return a}();c.LineFormatting=b})(window);(function(c){const b=function(){function a(a){return a===k.ORDERED?"</ol>":"</ul>"}function b(a,b){return a===b||a===k.TODO&&b===k.TODOCHECKED||a===k.TODOCHECKED&&b===k.TODO}const {utils:f}=c,g=c.AttributeConstants,{LIST_TYPE:k}=c.LineFormatting;return function(d,c){let m="",n=!0;const z=[];var v=!1;let u=!0;var p=!0;let q=0,A=d.ops[q],D=!1;for(;A;){f.assert(A.isInsert());const r=A.attributes;if(n){n=!1;var G=0,F=null,C="left";g.LINE_SENTINEL in r&&(G=r[g.LINE_INDENT]||0,F=r[g.LIST_TYPE]||null,C=
r[g.LINE_ALIGN]||"left");F&&(G=G||1);v?(m+="</li>",v=!1):u||(p&&(m+="<br/>"),m+="</div>");u=!1;for(f.assert(0<=G,"Indent must not be negative.");z.length>G||G===z.length&&null!==F&&!b(F,z[z.length-1]);)m+=a(z.pop());for(;z.length<G;)p=F||k.UNORDERED,D=F==k.TODO||F==k.TODOCHECKED||D,m+=p===k.ORDERED?"<ol>":p===k.UNORDERED?"<ul>":'<ul class="vceditor-todo">',z.push(p);p="left"!==C?` style="text-align:${C}"`:"";if(F){v="";switch(F){case k.TODOCHECKED:v=' class="vceditor-checked"';break;case k.TODO:v=
' class="vceditor-unchecked"'}m+=`<li${v}${p}>`;v=!0}else m+=`<div${p}>`;p=!0}if(g.LINE_SENTINEL in r){A=d.ops[++q];continue}if(g.ENTITY_SENTINEL in r){for(F=0;F<A.text.length;F++)G=vceditor.Entity.fromAttributes(r),G=c.exportToElement(G),m+=G.outerHTML;A=d.ops[++q];continue}G=F="";for(const a in r){C=r[a];var B;if(a===g.BOLD||a===g.ITALIC||a===g.UNDERLINE||a===g.STRIKE){f.assert(!0===C);var H=B=a}else a===g.FONT_SIZE?(H=`span style="font-size: ${C}`,H+="string"!==typeof C||-1===C.indexOf("px",C.length-
2)?'px"':'"',B="span"):a===g.FONT?(H=`span style="font-family: ${C}"`,B="span"):a===g.COLOR?(H=`span style="color: ${C}"`,B="span"):a===g.BACKGROUND_COLOR?(H=`span style="background-color: ${C}"`,B="span"):f.log(!1,`Encountered unknown attribute while rendering html: ${a}`);H&&(F+=`<${H}>`);B&&(G=`</${B}>${G}`)}({text:C}=A);const t=C.indexOf("\n");0<=t?(n=!0,A=t<C.length-1?new vceditor.TextOp("insert",C.substr(t+1),r):d.ops[++q],C=C.substr(0,t)):A=d.ops[++q];C=C.replace(/  +/g,a=>Array(a.length+1).join("\u00a0")).replace(/^ /,
"\u00a0").replace(/ $/,"\u00a0");0<C.length&&(p=!1);m+=F+C.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\u00a0/g,"&nbsp;")+G}v?m+="</li>":u||(p&&(m+="&nbsp;"),m+="</div>");for(;0<z.length;)m+=a(z.pop());D&&(m='<style>ul.vceditor-todo { list-style: none; margin-left: 0; padding-left: 0; } ul.vceditor-todo > li { padding-left: 1em; text-indent: -1em; } ul.vceditor-todo > li:before { content: "\\2610"; padding-right: 5px; } ul.vceditor-todo > li.vceditor-checked:before { content: "\\2611"; padding-right: 5px; }</style>\n'+
...
case "u":g(a,b.withTextFormatting(b.textFormatting.underline(!0)),d);break;case "i":case "em":g(a,b.withTextFormatting(b.textFormatting.italic(!0)),d);break;case "s":g(a,b.withTextFormatting(b.textFormatting.strike(!0)),d);break;case "font":c=a.getAttribute("face");var f=a.getAttribute("color"),q=parseInt(a.getAttribute("size"));c&&(b=b.withTextFormatting(b.textFormatting.font(c)));f&&(b=b.withTextFormatting(b.textFormatting.color(f)));q&&(b=b.withTextFormatting(b.textFormatting.fontSize(q)));g(a,
...
b+m.chars),f=0;f<c.length;f++){var q={};for(const a in m.attributes){var A=m.attributes[a];const b=c[f].attributes[a];!1===A?b&&(q[a]=b):A!==b&&(q[a]=b||!1)}n.retain(c[f].length,q);b+=c[f].length}else if(m.isInsert())n.delete(m.text.length);else if(m.isDelete()){q=d.getRange(d.posFromIndex(b),d.posFromIndex(b+m.chars));c=this.rtcm.getAttributeSpans(b,b+m.chars);for(f=A=0;f<c.length;f++)n.insert(q.substr(A,c[f].length),c[f].attributes),A+=c[f].length;b+=m.chars}}return new m(n,a.meta.invert())};return a}();
...
{};this.dirtyLines_=[]}function d(a,b){return a.line-b.line||a.ch-b.ch}function c(a){if(0===a.length)return 0;let b=0;for(let d=0;d<a.length;d++)b+=a[d].length;return b+a.length-1}function g(a){this.attributes=a||{}}function k(a,b){const d=a[b];a[b]=function(){d.apply(a,arguments)}}const {AnnotationList:m}=b,{Span:n}=b,{utils:r}=b,t=b.AttributeConstants,z={c:"color",bc:"background-color",fs:"font-size",li(a){return`padding-left: ${40*a}px`}},v={};r.makeEventEmitter(a,["change","attributesChange",
...
b,d,c,f){const k=[];let m=a;const q=this;this.annotationList_.updateSpan(new n(a,b-a),(a,b)=>{const n={};for(var A in a.attributes)n[A]=a.attributes[A];n[t.LINE_SENTINEL]&&!f||d(n);A={};const D={};q.computeChangedAttributes_(a.attributes,n,A,D);a:{for(const b in A){a=!1;break a}a=!0}a||k.push({start:m,end:m+b,attributes:A,attributesInverse:D,origin:c});m+=b;return new g(n)});0<k.length&&this.trigger("attributesChange",this,k)};a.prototype.computeChangedAttributes_=function(a,b,d,c){const f={};let g;
...
d=d.line;let f=c.line;b=b.getLine(f);c=this.areLineSentinelCharacters_(b.substr(0,c.ch));f>d&&c&&f--;this.updateLineAttributes(d,f,a)};a.prototype.updateLineAttributes=function(a,b,d){for(;a<=b;a++){var c=this.codeMirror.getLine(a);const b=this.codeMirror.indexFromPos({line:a,ch:0});c[0]!==u?(c={},c[t.LINE_SENTINEL]=!0,d(c),this.insertText(b,u,c)):this.updateTextAttributes(b,b+1,d,null,!0)}};a.prototype.replaceText=function(a,b,d,c,f){this.changeId_++;const g="cmrt-"+this.changeId_;this.outstandingChanges_[g]=
...
b);for(var f=0;f<a.length;f++)({attributes:d}=a[f].annotation),t.LINE_SENTINEL in d&&(c[this.codeMirror.posFromIndex(a[f].pos).line]=!0),(d=a[f].getAttachedObject())&&d.clear();for(f=0;f<b.length;f++){({annotation:d}=b[f]);var g=t.LINE_SENTINEL in d.attributes;const k=t.ENTITY_SENTINEL in d.attributes;a=this.codeMirror.posFromIndex(b[f].pos);g?c[a.line]=!0:k?this.markEntity_(b[f]):(d=this.getClassNameForAttributes_(d.attributes),""!==d&&(g=this.codeMirror.posFromIndex(b[f].pos+b[f].length),d=this.codeMirror.markText(a,
...
d=b.getLineHandle(a);return{clear(){b.removeLineClass(d,"text",".*")}}};a.prototype.emptySelection_=function(){const a=this.codeMirror.getCursor("start"),b=this.codeMirror.getCursor("end");return a.line===b.line&&a.ch===b.ch};a.prototype.onCodeMirrorBeforeChange_=function(a,b){if("+input"===b.origin||"paste"===b.origin){a=[];for(let d=0;d<b.text.length;d++){let c=b.text[d];c=c.replace(new RegExp(`[${u}${p}]`,"g"),"");a.push(c)}b.update(b.from,b.to,a)}};a.prototype.onCodeMirrorChange_=function(a,b){if("object"===
...
const k=[];for(let d=a.length-1;0<=d;d--){const c=a[d];g=b(g,c);const f=g(c.from),m=c.removed.join("\n"),n=c.text.join("\n");k.unshift({start:f,end:f+m.length,removed:m,text:n,origin:c.origin})}return k};a.prototype.markLineSentinelCharactersForChanges_=function(a){let b=Number.MAX_VALUE,d=-1;for(let c=0;c<a.length;c++){const f=a[c],{line:g}=f.from;if(1<f.removed.length||0<=f.removed[0].indexOf(u))b=Math.min(b,g),d=Math.max(d,g);1<f.text.length?(b=Math.min(b,g),d=Math.max(d,g+f.text.length-1)):0<=
f.text[0].indexOf(u)&&(b=Math.min(b,g),d=Math.max(d,g))}d=Math.min(d,this.codeMirror.lineCount()-1);this.markLineSentinelCharactersForChangedLines_(b,d)};a.prototype.markLineSentinelCharactersForChangedLines_=function(a,b){if(a<Number.MAX_VALUE)for(;0<a&&this.lineIsListItemOrIndented_(a-1);)a--;if(-1<b)for(var d=this.codeMirror.lineCount();b+1<d&&this.lineIsListItemOrIndented_(b+1);)b++;d=[];const c=this.codeMirror;for(;a<=b;a++){const b=c.getLine(a);var f=c.getLineHandle(a);c.removeLineClass(f,"text",
".*");if(0<b.length)for(f=b.indexOf(u);0<=f;){const g=f;for(;f<b.length&&b[f]===u;){const b=c.findMarksAt({line:a,ch:f});for(let a=0;a<b.length;a++)b[a].isForLineSentinel&&b[a].clear();f++}this.markLineSentinelCharacters_(a,g,f,d);f=b.indexOf(u,f)}else d=[]}return b};a.prototype.markLineSentinelCharacters_=function(a,b,d,c){const f=this.codeMirror;let g=null;var k=null,m=function(){const a=k.find();return a?a.from.line:null};if(0===b){const b=this.getLineAttributes_(a),d=b[t.LIST_TYPE];let f=b[t.LINE_INDENT]||
...
return this.currentAttributes_};a.prototype.updateCurrentAttributes_=function(){var a=this.codeMirror,d=a.indexFromPos(a.getCursor("anchor"));const c=a.indexFromPos(a.getCursor("head"));a=c;if(d>c){for(;a<this.end();){d=this.getRange(a,a+1);if("\n"!==d&&d!==u)break;a++}a<this.end()&&a++}else for(;0<a;){d=this.getRange(a-1,a);if("\n"!==d&&d!==u)break;a--}d=this.annotationList_.getAnnotatedSpansForPos(a);this.currentAttributes_={};a={};0<d.length&&!(t.LINE_SENTINEL in d[0].annotation.attributes)?a=
d[0].annotation.attributes:1<d.length&&(b.utils.assert(!(t.LINE_SENTINEL in d[1].annotation.attributes),"Cursor can't be between two line sentinel characters."),a=d[1].annotation.attributes);for(const b in a)"l"!==b&&"lt"!==b&&"li"!==b&&0!==b.indexOf(t.ENTITY_SENTINEL)&&(this.currentAttributes_[b]=a[b])};a.prototype.getCurrentLineAttributes_=function(){var a=this.codeMirror;const b=a.getCursor("anchor");a=a.getCursor("head");let {line:d}=a;0===a.ch&&b.line<a.line&&d--;return this.getLineAttributes_(d)};
a.prototype.getLineAttributes_=function(a){const d={},c=this.codeMirror.getLine(a);if(0<c.length&&c[0]===u){a=this.codeMirror.indexFromPos({line:a,ch:0});a=this.annotationList_.getAnnotatedSpansForSpan(new n(a,1));b.utils.assert(1===a.length);for(const b in a[0].annotation.attributes)d[b]=a[0].annotation.attributes[b]}return d};a.prototype.clearAnnotations_=function(){this.annotationList_.updateSpan(new n(0,this.end()),(a,b)=>new g({}))};a.prototype.newline=function(){const a=this.codeMirror,b=this;
if(this.emptySelection_()){const d=a.getCursor("head").line,c=this.getLineAttributes_(d),f=c[t.LIST_TYPE];f&&1===a.getLine(d).length?this.updateLineAttributes(d,d,a=>{delete a[t.LIST_TYPE];delete a[t.LINE_INDENT]}):(a.replaceSelection("\n","end","+input"),this.updateLineAttributes(d+1,d+1,a=>{for(const b in c)a[b]=c[b];"tc"===f&&(a[t.LIST_TYPE]="t");b.trigger("newLine",{line:d+1,attr:a})}))}else a.replaceSelection("\n","end","+input")};a.prototype.deleteLeft=function(){const a=this.codeMirror,b=a.getCursor("head");
var d=this.getLineAttributes_(b.line);const c=d[t.LIST_TYPE];d=d[t.LINE_INDENT];const f=this.emptySelection_()&&1===b.ch;f&&c?this.updateLineAttributes(b.line,b.line,a=>{delete a[t.LIST_TYPE];delete a[t.LINE_INDENT]}):f&&d&&0<d?this.updateLineAttributes(b.line,b.line,a=>{a[t.LINE_INDENT]--}):a.deleteH(-1,"char")};a.prototype.deleteRight=function(){const a=this.codeMirror,b=a.getCursor("head");var d=a.getLine(b.line);d=this.areLineSentinelCharacters_(d);const c=b.line+1<a.lineCount()?a.getLine(b.line+
1):"";this.emptySelection_()&&d&&c[0]===u?a.replaceRange("",{line:b.line,ch:0},{line:b.line+1,ch:0},"+input"):a.deleteH(1,"char")};a.prototype.indent=function(){this.updateLineAttributesForSelection(a=>{const b=a[t.LIST_TYPE];a[t.LINE_INDENT]?a[t.LINE_INDENT]++:a[t.LINE_INDENT]=b?2:1})};a.prototype.unindent=function(){this.updateLineAttributesForSelection(a=>{const b=a[t.LINE_INDENT];b&&1<b?a[t.LINE_INDENT]=b-1:(delete a[t.LIST_TYPE],delete a[t.LINE_INDENT])})};a.prototype.getText=function(){return this.codeMirror.getValue().replace(new RegExp(u,
...
d.formatting.attributes);for(let a=0;a<d.textPieces.length;a++)c(d.textPieces[a]);c("\n")}const k=[];for(let a=0;a<d.length;a++)d[a]instanceof b.Line?g(d[a]):c(d[a]);return k};b=b||{};b.Vceditor=function(a){function d(a,b,g,k){if(!(this instanceof d))return new d(a,b,g,k);if(!q&&!A)throw Error("Couldn't find CodeMirror or ACE.  Did you forget to include codemirror.js or ace.js?");if(q&&b instanceof q)this.codeMirror_=this.editor_=b,a=this.codeMirror_.getValue();else if(A&&b&&b.session instanceof A.EditSession){if(this.ace_=
...
this.autoPlayFlag=1;this.UI.container();if(roles.hasControls()){var b=document.querySelector("#dashboardnav button");b&&b.click();virtualclass.config.makeWebSocketReady&&ioAdapter.mustSend({videoUl:{init:"studentlayout"},cf:"videoUl"})}else if(b=document.getElementById("messageLayoutVideo"))b.style.display="block"},reArrangeElements(b){const a=document.getElementById("listvideo"),c=document.createElement("div");c.id="listvideo";c.className="videos";var f=this.getActiveVideos(),g=!1;for(let a=0;a<
f.length;a++)-1>=b.indexOf(f[a].fileuuid)&&(b.push(f[a].fileuuid),g=!0);g&&(virtualclass.orderList[virtualclass.videoUl.appName].ol.order=b,virtualclass.videoUl.sendOrder(virtualclass.orderList[virtualclass.videoUl.appName].ol.order));for(f=0;f<b.length;f++)(g=document.getElementById(`linkvideo${b[f]}`))&&c.appendChild(g);a.parentNode.replaceChild(c,a)},getActiveVideos(){const b=[];for(let a=0;a<virtualclass.serverData.rawData.video.length;a++)Object.prototype.hasOwnProperty.call(virtualclass.serverData.rawData.video[a],
...
b;this.reArrangeElements(virtualclass.orderList[virtualclass.videoUl.appName].ol.order);this.sendOrder(virtualclass.orderList[virtualclass.videoUl.appName].ol.order)},async editTitle(b,a,d){d=new FormData;const f={lc_content_id:b,action:"edit",title:a,user:virtualclass.gObj.uid};for(const a in f)d.append(a,f[a]);await this.vxhr.post(`${c.webapi}&user=${virtualclass.gObj.uid}&methodname=update_content_video`,d).then(c=>{if(c=document.getElementById(`videoTitle${b}`))c.innerHTML=a,c.style.display="inline",
...
virtualclass.videoUl.destroyPlayer();virtualclass.videoUl.videoUrl=a;virtualclass.videoUl.videoId=b;let c=document.getElementById("videoPlayerCont");c?c.style.display="block":(virtualclass.videoUl.UI.container(),c=document.getElementById("videoPlayerCont"));const f=document.querySelector("iframe#player");f&&f.remove();virtualclass.videoUl.UI.switchDisplay(c,a);virtualclass.videoUl.UI.videojsPlayer(a,b);virtualclass.dashboard.close();virtualclass.userInteractivity.makeReadyContext()},videojsPlayer(b,
...
!virtualclass.dts.noteExist()&&(c=document.querySelector("#docScreenContainer"),null!=c&&(c.classList.remove("noteDisplay"),console.log("====> noteDisplay removing")),virtualclass.gObj.currWb=null,document.querySelector("#virtualclassCont").classList.remove("pdfRendering"),c=document.querySelector("#virtualclassAppLeftPanel.hideZoom"),g=document.querySelector("#virtualclassAppLeftPanel"),c||(g.classList.add("hideZoom"),g.classList.remove("showZoom")))));null==document.querySelector("#listnotes .currentNav")&&
(c=document.querySelector("#listnotes .linknotes"),null!=c&&(virtualclass.dts.currNote=c.dataset.rid,c=document.querySelector(`#mainpnotes${virtualclass.dts.currNote}`),null!=c&&(virtualclass.dts.docs.currDoc=virtualclass.dts.currNote.split("_")[0],c.click())));roles.hasAdmin()&&this.sendOrder(virtualclass.orderList[this.appName].ol.order)},selectFirstNote(){if(null==document.querySelector("#notesContainer .current")){const a=document.querySelector("#notesContainer .note");null!=a&&(this.docs.currNote=
...
document.querySelector("#DocumentShareDashboard").innerHTML=a,roles.hasControls()&&(virtualclass.vutil.attachEventToUploadTab(),null==document.querySelector("#DocumentShareDashboard .qq-gallery")&&virtualclass.vutil.modalPopup("docs",["docsuploadContainer"]),virtualclass.dashboard.modalCloseHandler()))},createMainContent(a,b,c){},createSlides(a,b){let c=document.querySelector("#notesContainer");null==c&&(c=document.createElement("div"),c.className="notes",c.id="notesContainer");for(let a=0;a<b.length;a++)if(null==
document.querySelector(`#note${b[a].id}`)){const c=document.createElement("div");c.id=`note${b[a].id}`;c.className="note";c.dataset.slide=b[a].id;c.dataset.status="true"===c.dataset.statuS||!0===c.dataset.statuS?1:"false"===c.dataset.status||!1===c.dataset.status?0:b[a].status}a.appendChild(c);roles.hasControls()&&(this.createNavigation(a,"prev"),this.createNavigation(a,"next"))},createNavigation(a,b){if(null==document.querySelector(`#docs${b}`)){const c=document.createElement("span");c.className=
`${"nvgt "}${b}`;c.id=`docs${b}`;a.appendChild(c)}},createDocsNav(a,b){let c=document.getElementById("listDocs");null==c&&(c=document.createElement("div"),c.id="listDocs",a.appendChild(c));a=this.createDocsNavLink(b);this.attachDocsNav(a,b);c.appendChild(a)},createDocsNavLink(a){const b=document.createElement("div");b.id=`link${a}`;b.className="linkdoc";b.innerHTML=a;b.dataset.screen=a;return b},attachDocsNav(a,b){a.onclick=virtualclass.dts.docs.goToDocs(b)}},createNoteNav(a){let b;virtualclass.orderList[this.appName].ol.order&&
this.indexNav.init();for(let c=0;c<virtualclass.orderList[this.appName].ol.order.length;c++)"object"!==typeof this.notes[virtualclass.orderList[this.appName].ol.order[c]]&&(b="true"===this.allNotes[virtualclass.orderList[this.appName].ol.order[c]].status||1===+this.allNotes[virtualclass.orderList[this.appName].ol.order[c]].status?1:0,this.notes[virtualclass.orderList[this.appName].ol.order[c]]=new virtualclass.page("screen-docs","notes","virtualclassDocumentShare","dts",b),this.notes[virtualclass.orderList[this.appName].ol.order[c]].init(virtualclass.orderList[this.appName].ol.order[c],
`note_${this.allNotes[virtualclass.orderList[this.appName].ol.order[c]].lc_content_id}_${virtualclass.orderList[this.appName].ol.order[c]}`),"undefined"===typeof a&&this.noteStatus(virtualclass.orderList[this.appName].ol.order[c],b)),0===c&&virtualclass.vutil.hideUploadMsg("docsuploadContainer");roles.hasControls()&&(this.indexNav.shownPage(this.indexNav.width),this.indexNav.addActiveNavigation(),document.querySelector("#dcPaging").addEventListener("change",function(){virtualclass.dts.docs.goToNavs(this.value)()}));
document.querySelector(".congrea.teacher  #dashboardContainer .modal-header button.enable")||virtualclass.vutil.showFinishBtn();this.indexNav.setTotalPages(virtualclass.orderList[virtualclass.dts.appName].ol.order.length)},addNoteHidClass(a,b,c){b>c?a.classList.add("hid","right"):a.classList.add("shw")},indexHandler(a){virtualclass.dts.docs.goToNavs(a)},createNoteNavAlt(a){let b;for(let c=0;c<virtualclass.orderList[this.appName].ol.order.length;c++)b="true"===this.allNotes[virtualclass.orderList[this.appName].ol.order[c]].status||
1===+this.allNotes[virtualclass.orderList[this.appName].ol.order[c]].status?1:0,this.notes[virtualclass.orderList[this.appName].ol.order[c]]=new virtualclass.page("screen-docs","notes","virtualclassDocumentShare","dts",b),this.notes[virtualclass.orderList[this.appName].ol.order[c]].init(virtualclass.orderList[this.appName].ol.order[c],`note_${this.allNotes[virtualclass.orderList[this.appName].ol.order[c]].lc_content_id}_${virtualclass.orderList[this.appName].ol.order[c]}`),"undefined"===typeof a&&
this.noteStatus(virtualclass.orderList[this.appName].ol.order[c],b)},removeNoteNav(a){const b=document.querySelector(`#linknotes${a}`);null!=b&&b.parentNode.removeChild(b);"object"===typeof this.notes[a]&&delete this.notes[a]},docs:{num:0,currNote:0,curr(a,b){this.currDoc=a;virtualclass.dts.docs.num=a;const c=document.querySelector("#documentScreen .screen.current");null!=c&&c.classList.remove("current");document.querySelector("#screen-docs").classList.add("current");document.querySelector("#documentScreen").dataset.screen=
...
b,c,k,m,n,r){const d=document.getElementById("qzReTbody");if(d){let f=document.getElementById(`user${r}`);null!==f&&(a=f.childNodes[0].innerHTML,f.parentNode.removeChild(f));f=document.createElement("tr");f.id=`user${r}`;d.appendChild(f);r=document.createElement("th");r.scope="row";r.innerHTML=a;f.appendChild(r);a=document.createElement("td");a.innerHTML=b;f.appendChild(a);b=document.createElement("td");b.innerHTML=c;f.appendChild(b);c=document.createElement("td");c.innerHTML=`${k}%`;f.appendChild(c);
k=document.createElement("td");k.innerHTML=m;f.appendChild(k);m=document.createElement("td");m.innerHTML=n;f.appendChild(m)}},quizTimer(a,b,c){function d(){g="asc"===c?a+((Date.now()-f)/1E3|0):a-((Date.now()-f)/1E3|0);r=g/3600|0;t=g%3600/60|0;z=g%60|0;r=10>r?`0${r}`:r;t=10>t?`0${t}`:t;z=10>z?`0${z}`:z;b.textContent=`${r}:${t}:${z}`;if(0>=g&&("asc"!==c&&(b.textContent="00 : 00 : 00 "),f=0,null!=virtualclass.gObj.CDTimer&&clearInterval(virtualclass.gObj.CDTimer),ioAdapter.mustSend({quiz:{quizMsg:"quizTimeEnd",
...
this.canvasScale,console.log("previous canvas scale ",virtualclass.zoom.prvCanvasScale),this[b].call(this))},zoomIn(b){delete virtualclass.zoom.performFitToPage;const a=virtualclass.gObj.currWb;if("object"===typeof virtualclass.wb[a]){const c=virtualclass.wb[a].canvas.lowerCanvasEl,f=virtualclass.zoom.canvasDimension.width*virtualclass.gObj.SCALE_FACTOR,g=virtualclass.zoom.canvasDimension.height*virtualclass.gObj.SCALE_FACTOR;virtualclass.zoom.canvasDimension.width=f;virtualclass.zoom.canvasDimension.height=
g;"undefined"!==typeof b?virtualclass.pdfRender[a]._zoom.call(virtualclass.pdfRender[a],c,f,g,b):virtualclass.pdfRender[a]._zoom.call(virtualclass.pdfRender[a],c,f,g)}},zoomOut(){delete virtualclass.zoom.performFitToPage;const b=virtualclass.gObj.currWb,a=virtualclass.wb[b].canvas.lowerCanvasEl,c=a.parentNode,f=a.parentNode.offsetWidth,g=1/virtualclass.gObj.SCALE_FACTOR*virtualclass.zoom.canvasDimension.width,k=1/virtualclass.gObj.SCALE_FACTOR*virtualclass.zoom.canvasDimension.height;virtualclass.zoom.canvasDimension.width=
...
c;this.topPosY=a;this.leftPosX=c;const f=this;b.onscroll=function(){f.onScroll(b)}},onScroll(b,a){let c=b.scrollTop,f=b.scrollLeft;(0<c||"undefined"!==typeof a)&&this._scroll(f,c,b,"Y");(0<f||"undefined"!==typeof a)&&this._scroll(f,c,b,"X");roles.hasControls()||"object"!==typeof virtualclass.pdfRender[virtualclass.gObj.currWb]?roles.isStudent()&&console.log("Could not call scroll function for student"):virtualclass.pdfRender[virtualclass.gObj.currWb].setScrollPosition({scX:f,scY:c})},_scroll(b,a,
c,f){if(roles.hasControls())return this.topPosY=a,this.leftPosX=b,this.scrollPosition(c,f);if("Y"===f)var d=a;else"X"===f&&(d=b);this.updateScrollPosition(d,f);return null},scrollPosition(b,a){const {canvas:c}=this;if("Y"===a)var f=b.scrollTop,g=c.height;else"X"===a&&(f=b.scrollLeft,g=c.width);this[`scrollPos${a}`]=f/g*100;b=virtualclass.vutil.getElemM(`canvasWrapper${virtualclass.gObj.currWb}`,a);this[`actualVp${a}`]=b;this[`viewPort${a}`]=b/g*100;g={};g[`vp${a}`]=this[`viewPort${a}`];g[`sc${a}`]=
this[`scrollPos${a}`];return g},scroll:{caclculatePosition(b,a,c){b=this.type=c;null==this[b]&&(this[b]={});this[b].a=0;this[b].d=a;a=`canvasWrapper${virtualclass.gObj.currWb}`;c=document.querySelector(`#${a}`);null!=c&&("X"===this.type?this[b].b=c.scrollLeft:"Y"===this.type&&(this[b].b=c.scrollTop),this[b].studentVPm=virtualclass.vutil.getElemM(a,b),this[b].c=this[b].b+this[b].studentVPm)}},setScrollPosition(b){const a=virtualclass.vutil.canvasDimensionFromPixel(this.canvas);Object.prototype.hasOwnProperty.call(b,
"scY")&&null!=b.scY&&null!=this.canvas&&this.scroll.caclculatePosition(b.scY,a.height,"Y");Object.prototype.hasOwnProperty.call(b,"scX")&&null!=b.scX&&null!=this.canvas&&this.scroll.caclculatePosition(b.scX,a.width,"X")},customMoustPointer(b,a,c){console.log("custom mouse pointer ay="+this.scroll[a].a+" by="+this.scroll[a].b+" cy="+this.scroll[a].c+" dy="+this.scroll[a].d+" ey"+this.scroll[a].e);this.scroll[a].e=c;this.scroll[a].e>this.scroll[a].c?(b=this.scroll[a].b+(this.scroll[a].d-this.scroll[a].c),
b>this.scroll[a].e&&(b=this.scroll[a].e-(this.scroll[a].b+this.scroll[a].c)/2),"Y"==a?(this.canvasWrapper.scrollTop=b,console.log("====> scroll top")):this.canvasWrapper.scrollLeft=b,this.scroll[a].b=b,this.scroll[a].c=this.scroll[a].b+this.scroll[a].studentVPm):this.scroll[a].e<this.scroll[a].b&&(b=this.scroll[a].b-this.scroll[a].a,this.scroll[a].c-b<this.scroll[a].e&&(b=(this.scroll[a].b+this.scroll[a].c)/2-this.scroll[a].e),"Y"===a?(this.canvasWrapper.scrollTop-=b,console.log("====> scroll top")):
this.canvasWrapper.scrollLeft-=b,this.scroll[a].b=b,this.scroll[a].c=this.scroll[a].b+this.scroll[a].studentVPm)},setCustomMoustPointer(b,a){var c=`scrollDiv${a}${virtualclass.gObj.currWb}`,f=document.querySelector(`#${c}mousePointer`);null==f&&(f=document.createElement("div"),f.className="mousepointer",f.id=`${c}mousePointer`,c=document.querySelector(`#scrollDiv${a}${virtualclass.gObj.currWb}`),null!=c&&c.appendChild(f));"Y"===a?f.style.top=`${b.y-this.scroll[a].a}px`:"X"===a&&(f.style.left=`${b.x-
this.scroll[a].a}px`)},sendScroll(){virtualclass.vutil.setDefaultScroll()},sendCurrentScroll(b){let a={};if(null!=this.currentScroll){a=Object.assign(a,this.currentScroll);const c=this;c.currentScrolltoUser=b;setTimeout(()=>{c.currentScrolltoUser=b;a.cf="scf";a.toUser=b;virtualclass.vutil.beforeSend({toUser:b,cf:"scf",scY:a.scY,vpY:a.vpY},b)},2E3)}},setDefaultCanvasWidth(b,a){if("_doc_0_0"!==b||0!==a.parentNode.parentNode.offsetWidth&&300!==a.parentNode.parentNode.offsetWidth)a.width=a.parentNode.parentNode.offsetWidth-
6;else{let b;"Whiteboard"===virtualclass.currApp?b=document.querySelector("#virtualclassWhiteboard.canvasContainer.current"):"DocumentShare"===virtualclass.currApp&&(b=document.querySelector("#screen-docs.current"));b||(b=document.querySelector("#virtualclassAppContainer"));a.width=b.offsetWidth-6}virtualclass.wbWrapper.util.createFabricNewInstance(b,{width:a.width,height:a.height})},calculateViewPort(b,a){const c=document.getElementById("virtualclassAppContainer");b=virtualclass.vutil.canvasDimensionFromPixel(b);
const f=a.getViewport(Math.min(c.clientWidth/b.width*virtualclass.zoom.canvasScale,c.clientHeight/b.height*virtualclass.zoom.canvasScale));return f.width<c.offsetWidth?f:a.getViewport(b.width/a.getViewport(1).width)},getAppearanceCanvasWidth(b,a){Object.prototype.hasOwnProperty.call(virtualclass.zoom,"diffrentDocumentWidth")?(b=virtualclass.zoom.diffrentDocumentWidth,console.log("==== a canvas width ",a.width),delete virtualclass.zoom.diffrentDocumentWidth):Object.prototype.hasOwnProperty.call(virtualclass.gObj,
"fitToScreen")?(b=300===a.parentNode.offsetWidth||0===a.parentNode.offsetWidth?document.querySelector("#virtualclassAppContainer").offsetWidth:a.parentNode.offsetWidth-6,"DocumentShare"===virtualclass.currApp&&virtualclass.zoom.fitToElementTooltip("fitToToPage")):Object.prototype.hasOwnProperty.call(virtualclass.zoom,"performZoom")?(b=virtualclass.zoom.canvasDimension.width,console.log("==== a canvas width performZoom ",a.width),delete virtualclass.zoom.performZoom):Object.prototype.hasOwnProperty.call(virtualclass.zoom,
"canvasDimension")?(b=virtualclass.zoom.canvasDimension.width,console.log("==== a canvas width  ",a.width)):0===a.offsetWidth||300===a.offsetWidth||virtualclass.isPlayMode?(this.setDefaultCanvasWidth(b,a),b=virtualclass.vutil.canvasDimensionFromPixel(a).width):b=a.parentNode.offsetWidth-6;return b},async renderPage(b,a){virtualclass.zoom.prvPdfDimension&&virtualclass.zoom.prvPdfDimension[2]!==b.view[2]&&(virtualclass.gObj.fitToScreen=!0,this.firstTime&&(virtualclass.gObj.pdfWidthChanged=!0));if(null!=
virtualclass.gObj.currWb){var c=this.pdfScale;null!=virtualclass.zoom.canvasScale&&""!=virtualclass.zoom.canvasScale&&0<virtualclass.zoom.canvasScale&&(c=virtualclass.zoom.canvasScale);let d;d="Whiteboard"===virtualclass.currApp&&null!=this.wbId&&virtualclass.gObj.currWb!==this.wbId?-1<this.wbId.indexOf("_doc_")?this.wbId:`_doc_${this.wbId}_${this.wbId}`:b.wbId;const k=virtualclass.wb[d].canvas.lowerCanvasEl;var f=this.getAppearanceCanvasWidth(d,k);this.firstTime?(this.firstTime=!1,null==virtualclass.zoom.canvasScale||
virtualclass.gObj.fitToScreen?(c=b.getViewport(f/b.getViewport(1).width),console.log("==== view port 1",c.width)):(c=b.getViewport(c),console.log("==== view port 2",c.width))):virtualclass.zoom.performFitToPage?(c=this.calculateViewPort(k,b),console.log("==== view port 3",c.width)):(c=b.getViewport(f/b.getViewport(1).width),console.log("==== view port 4",c.width));console.log("=====> whiteboard pdf suman calc canvas scale");virtualclass.zoom.prvCanvasScale=virtualclass.zoom.canvasScale;virtualclass.zoom.canvasScale=
c.scale;console.log("==== a suman SCAle change ",virtualclass.zoom.canvasScale);virtualclass.zoom.prvPdfDimension=b.view;f=document.getElementById(`${k.id}_pdf`);f.width=c.width;f.height=c.height;virtualclass.zoom.canvasDimension={};virtualclass.zoom.canvasDimension.width=c.width;virtualclass.zoom.canvasDimension.height=c.height;virtualclass.wbWrapper.util.createFabricNewInstance(d,{width:c.width,height:c.height});if(Object.prototype.hasOwnProperty.call(virtualclass.gObj,"fitToScreen")){const a=document.querySelector(`#canvasWrapper${virtualclass.gObj.currWb}`);
if(null!==a&&(a.scrollHeight<=a.clientHeight||a.scrollWidth<=a.clientWidth))virtualclass.pdfRender[virtualclass.gObj.currWb].onScroll(a,!0)}virtualclass.wb[d].canvas.setZoom(virtualclass.zoom.canvasScale);console.log("====> canvas set zoom",virtualclass.zoom.canvasScale);delete virtualclass.gObj.fitToScreen;c={canvasContext:f.getContext("2d"),viewport:c};await b.render(c);k.parentNode.dataset.pdfrender=!0;this[d]={pdfrender:!0};virtualclass.vutil.showZoom();null!=a&&null!=virtualclass.gObj.currWb&&
this.initWhiteboardData(b.wbId);"object"===typeof this.shownPdf&&(io.globallock=!1,virtualclass.vutil.removeClass("virtualclassCont","resizeWindow"));this.firstTimeScroll&&roles.isStudent()&&"object"===typeof virtualclass.pdfRender[virtualclass.gObj.currWb]&&(virtualclass.pdfRender[virtualclass.gObj.currWb].setScrollPosition({scX:0,scY:0}),this.firstTimeScroll=!1)}},async displayPage(b,a,c){a=await b.getPage(a);a.wbId=b.wbId;virtualclass.pdfRender[b.wbId].page=a;"undefined"!==typeof c?await this.renderPage(a,
c):await this.renderPage(a,null)},async initWhiteboardData(b){"object"===typeof virtualclass.gObj.wbData[b]&&virtualclass.gObj.wbData[b]&&0<virtualclass.gObj.wbData[b].length&&virtualclass.wbWrapper.util.replayFromLocalStroage(virtualclass.gObj.wbData[b],b)},async _zoom(b,a,c){virtualclass.vutil.setHeight(virtualclass.gObj.currWb,b,c);virtualclass.vutil.setWidth(virtualclass.gObj.currWb,b,a);b=b.parentNode;c=virtualclass.vutil.getValueWithoutPixel(b.style.width);await virtualclass.pdfRender[virtualclass.gObj.currWb].displayPage(this.shownPdf,
1);for(const a in virtualclass.pdfRender)this.zoomwhiteboardObjects(a);a>c&&b.classList.add("scrollX")},async innerFitToPage(b){if(" "!==this.shownPdf&&""!==this.shownPdf){await virtualclass.pdfRender[b].displayPage(this.shownPdf,1);for(const a in virtualclass.pdfRender)this.fitToScreenWhiteboardObjects(a)}},fitWhiteboardAtScale(b){if("object"===typeof virtualclass.wb[b]){const {vcan:d}=virtualclass.wb[b];b=virtualclass.wb[b].canvas.getObjects();if(0<b.length)for(const d in b){var {scaleX:a}=b[d],
{scaleY:c}=b[d];a=a/virtualclass.zoom.prvCanvasScale*virtualclass.zoom.canvasScale;c=c/virtualclass.zoom.prvCanvasScale*virtualclass.zoom.canvasScale;const f=b[d].x/b[d].scaleX*a,g=b[d].y/b[d].scaleY*c;b[d].scaleX=a;b[d].scaleY=c;b[d].x=f;b[d].y=g;b[d].setCoords()}d.renderAll()}},zoomwhiteboardObjects(b){virtualclass.wbWrapper.util.fitWhiteboardAtScale(b)},zoomOutWhiteboardObjects(b){virtualclass.wbWrapper.util.fitWhiteboardAtScale(b)},fitToScreenWhiteboardObjects(b){virtualclass.wbWrapper.util.fitWhiteboardAtScale(b)},
normalViewWhiteboardObjects(b){this.fitWhiteboardAtScale(b)},async _zoomOut(b,a,c){virtualclass.vutil.setHeight(virtualclass.gObj.currWb,b,c);virtualclass.vutil.setWidth(virtualclass.gObj.currWb,b,a);await virtualclass.pdfRender[virtualclass.gObj.currWb].displayPage(this.shownPdf,1);for(const a in virtualclass.pdfRender)this.zoomOutWhiteboardObjects(a)},async _fitToScreen(b,a){virtualclass.vutil.setHeight(virtualclass.gObj.currWb,b,b.height);virtualclass.vutil.setWidth(virtualclass.gObj.currWb,b,
a);b=b.parentNode;const c=b.offsetWidth;if(" "!==this.shownPdf){await virtualclass.pdfRender[virtualclass.gObj.currWb].displayPage(this.shownPdf,1);for(const a in virtualclass.pdfRender)this.fitToScreenWhiteboardObjects(a);a>c&&55<a-c?b.classList.add("scrollX"):b.classList.remove("scrollX")}},calculateScaleAtFirst(b,a){b=b.getViewport(a.width/b.getViewport(1).width);this.firstTime=!1;return b},isWhiteboardAlreadyExist(b){return null!=this.canvas},defaultScroll(){null!=virtualclass.gObj.currWb&&(this.canvasWrapper.scrollTop=
1)}}}})(window);(function(c){c.wbCommon={order:[0],wbInd:0,initNavHandler(){document.querySelector("#virtualclassWhiteboard.whiteboard .next").addEventListener("click",virtualclass.vutil.navWhiteboard.bind(virtualclass.vutil,this,this.next));document.querySelector("#virtualclassWhiteboard.whiteboard .prev").addEventListener("click",virtualclass.vutil.navWhiteboard.bind(virtualclass.vutil,this,this.prev))},initNav(b){"undefined"===typeof this.indexNav&&(this.indexNav=new pageIndexNav("WB"));const a=virtualclass.gObj.currIndex||
...
virtualclass.vutil.beforeSend({cf:"cwb",diswb:!0,wid:b,currIndex:virtualclass.gObj.currIndex});this.identifyLastNote(b)}else console.log("No element found")},newPage(){this.hideElement();virtualclass.gObj.wbCount++;let b=virtualclass.gObj.wbCount;const a=`${"_doc_0_"}${virtualclass.gObj.wbCount}`;var c=virtualclass.gObj.currWb;let f=null;0<virtualclass.orderList.Whiteboard.ol.order.length&&virtualclass.orderList.Whiteboard.getCurrentPosition(virtualclass.gObj.currWb)+1!==virtualclass.orderList.Whiteboard.ol.order.length&&
...
(g=f.parentNode.dataset.event,k=f.parentNode.parentNode):this.event.values.includes(f.parentNode.parentNode.dataset.event)&&(g=f.parentNode.parentNode.dataset.event,k=f.parentNode.parentNode.parentNode);if(g){let m,n,r=null;c=k.dataset.component;if(k.dataset.componentId&&"save"!==g)if(a=k.dataset.componentId,"reply"===g)a=null,r=k.dataset.componentId;else if("edit"===g||"markAnswer"===g||"delete"===g)r=k.dataset.parent?k.dataset.parent:null;if("save"===g){if("note"===c)m=f.value,n="create";else if("bookmark"===
