...
"";a.main.textObj=!1;a.main.action="create";a.wb={};a.wb.sentPack=!1;a.activMouse=new a.mouse;a.activMouse.init()},extend(a,c){if("object"===typeof a&&"object"===typeof c){for(const b in c)a[b]=c[b];return a}console.log("it seems that the arguments you passed are not object")},transform(a,c,g){a.translate(c.x,c.y);a.rotate(c.theta);g||a.scale(c.scaleX*(c.flipX?-1:1),c.scaleY*(c.flipY?-1:1))},getStates(b){return a.main[b]},setValInMain(b,c){return a.main[b]=c},ArrayIndexOf(a,c){if(!c||"function"!==
typeof c||!a||!a.length||1>a.length)return-1;for(let b=0;b<a.length;b++)if(c(a[b]))return b;return-1},renderAll(b){"object"!==typeof b&&(b=a.main.canvas.getContext("2d"));a.clearContext(b);this.displayPdfWhiteboard()},normalDisplay(b){console.log(`Whiteboard index ${b}`);"freeDrawing"==a.main.children[b].type?a.fhdRender(ctx,a.main.children[b]):a.render(ctx,a.main.children[b])},displayPdfWhiteboard(){const {length:b}=a.main.children;if(b){const c=a.main.canvas.getContext("2d");for(let f=0;f<b;++f)"pdf"==
