...
()=>{virtualclass.popup.closeElem()})};a.prototype.infoMsg=function(a){const b=document.getElementById("about-modal");virtualclass.popup.open(b);this.hideAllPopups();document.getElementById("infoMessage").style.display="block";document.querySelector("#infoMessageMsg").innerHTML=a;document.getElementById("infoMessageOk").innerHTML="ok";document.getElementById("infoMessageOk").addEventListener("click",()=>{virtualclass.popup.closeElem()})};a.prototype.generalMsgButton=function(a){a=document.getElementById("about-modal");
...
document.getElementById("startLiveStream");null!=c&&c.classList.remove("disabled")}this.alreadyInit||(virtualclass.isPlayMode?this.prefixUrl=`${this.uploadEndPoint}/${wbUser.lkey}/${wbUser.room}/${wbUser.session}`:null!=localStorage.mySession?this.prefixUrl=`${this.uploadEndPoint}/${wbUser.lkey}/${wbUser.room}/${localStorage.mySession}`:virtualclass.gObj.currentSession&&(this.prefixUrl=`${this.uploadEndPoint}/${wbUser.lkey}/${wbUser.room}/${virtualclass.gObj.currentSession}`),this.prefixUrl?(roles.hasControls()&&
document.getElementById("startLiveStream").addEventListener("click",this.handlLiveStream.bind(this)),this.alreadyInit=!0):setTimeout(()=>{this.init()},1500))}requestStream(c){this.xhr.get(c).then(async b=>{this.afterReceivedStream(b)}).catch(b=>{5>=this.fileList.ol.order.length?this.lastFileRequested=this.firstFile:setTimeout(()=>{this.requestInitializePacket(this.lastFileRequested)},2E3)})}getFileName(c){const b=c.lastIndexOf("/");return c.substring(b+1).replace(".chvs","")}duringPlayFirstPacket(){var c=
document.getElementById("virtualclassVideo");c&&(c.dataset.currapp="liveStream");if(c=document.getElementById("messageLayoutVideo"))c.style.display="none"}afterReceivedStream(c){const b=c.data.slice(4,c.data.length);c=this.getFileName(c.config.url);this.listStream[c]=b;this.playIfReady(c)}onBuffer(c){this.sourceBuffer&&(new Uint8Array(c),this.sourceBuffer.appendBuffer(c))}readyParticipateVideo(){this.remoteVideo||(this.remoteVideo=document.getElementById("liveStream"),this.remoteVideo.srcObject=null,
this.remoteVideo.currentTime=0)}initPLayerForParticipaes(){this.mediaSource=new MediaSource;this.mediaSource.addEventListener("sourceopen",this.mediaSourceOpen.bind(this));this.readyParticipateVideo();this.remoteVideo.src=URL.createObjectURL(this.mediaSource)}mediaSourceOpen(){"open"===this.mediaSource.readyState&&(this.sourceBuffer=this.mediaSource.addSourceBuffer(this.mimeType),this.sourceBuffer.addEventListener("error",function(c){}),this.sourceBuffer.addEventListener("updateend",function(){const c=
virtualclass.liveStream.fileList.getNextByID(virtualclass.liveStream.currentExecuted);c&&virtualclass.liveStream.playIfReady(c.id)}))}handlLiveStream(c){"0"===c.currentTarget.dataset.sharinglivestream?this.startToShare():(this.stop(),c=document.querySelector("#dashboardnav button"),null!=c&&c.click())}disableLiveStreamButton(){const c=document.getElementById("startLiveStream");null!=c&&c.classList.add("disabled")}async startToShare(){this.stopTraditionalVideo();var c=c=document.getElementById("startLiveStream");
c.dataset.sharinglivestream="1";c=null;try{c=await navigator.mediaDevices.getUserMedia(this.constraints)}catch(b){this.disableLiveStreamButton(),virtualclass.media.handleUserMediaError(b)}c&&this.handleSuccess(c)}handleSuccess(c){const b=c.getVideoTracks()[0].getSettings();this.stream=c;virtualclass.dashboard.close();this.showLiveStreamHTML();document.getElementById("liveStream").srcObject=this.stream;this.startRecorder(b.width)}startRecorder(c){let b=3E5;640<c&&1920>=c?b=5E5:1920<c&&(b=1E6);MediaRecorder.isTypeSupported(this.mimeType)||
console.error(`${this.mimeType} is not supported`);this.mediaRecorder=new MediaRecorder(this.stream,{mimeType:this.mimeType,videoBitsPerSecond:b});this.mediaRecorder.addEventListener("stop",this.stopHandler.bind(this));this.mediaRecorder.addEventListener("dataavailable",this.handleLiveStreamData.bind(this));this.mediaRecorder.start(2E3)}stopHandler(){}handleLiveStreamData(c){this.mediaRecorder&&"recording"===this.mediaRecorder.state&&this.saveToStreamServer(c.data)}async saveToStreamServer(c){c=await this.videoTypedArray(c);
ioAdapter.sendStream(c)}async videoTypedArray(c){c=await c.arrayBuffer();c=new Uint8Array(c);let b=new c.constructor(2);b[0]=72;b[1]=78;let a=new c.constructor(c.length+b.length);a.set(b,0);a.set(c,b.length);return a}stop(){var c=c=document.getElementById("startLiveStream");c.dataset.sharinglivestream="0";this.mediaRecorder&&"inactive"!==this.mediaRecorder.state&&this.mediaRecorder.stop();this.hideLiveStreamHTML();this.disableLiveStreamButton();roles.hasControls()&&(ioAdapter.mustSend({cf:"liveStream",
stop:!0}),this.clearEveryThing());setTimeout(()=>{const b=document.getElementById("startLiveStream");null!=b&&b.classList.remove("disabled")},5E3)}stopTraditionalVideo(){Object.prototype.hasOwnProperty.call(virtualclass,"videoUl")&&Object.prototype.hasOwnProperty.call(virtualclass.videoUl,"player")&&virtualclass.videoUl.destroyPlayer();const c=document.getElementById("videoPlayerCont");c&&(c.style.display="none");roles.hasControls()&&ioAdapter.mustSend({cf:"liveStream",stopVideo:!0})}clearEveryThing(){delete virtualclass.gObj.videoMode;
var c=document.getElementById("virtualclassVideo");null!=c&&(c.dataset.currapp="normalVideo",c=document.getElementById("messageLayoutVideo"))&&(c.style.display="block");this.stream&&this.stream.getTracks().forEach(function(b){b.stop()});this.mediaRecorder&&delete this.mediaRecorder;if(this.sourceBuffer){if(this.sourceBuffer.updating)this.sourceBuffer.abort();else try{this.sourceBuffer.remove(0,300)}catch(b){this.sourceBuffer.abort()}delete this.sourceBuffer;delete this.mediaSource}this.startedStream=
!1;c=document.getElementById("liveStream");null!=c&&(c.src=null);delete this.firstFile;delete this.startingPoint;delete this.startedAppending;this.fileList.emptyList();delete this.lastFileRequested;delete virtualclass.liveStream.callFromSeek;delete this.startFromPageRefresh}showLiveStreamHTML(){document.getElementById("virtualclassVideo").dataset.currapp="liveStream";document.querySelector("#startLiveStream .label").innerHTML=virtualclass.lang.getString("stopLiveSharing")}hideLiveStreamHTML(){document.getElementById("virtualclassVideo").dataset.currapp=
"normalVideo";document.querySelector("#startLiveStream .label").innerHTML=virtualclass.lang.getString("startLiveSharing")}isLiveStreamMode(){return"liveStream"===document.getElementById("virtualclassVideo").dataset.currapp}onMessage(c){c.message.fileName?setTimeout(()=>{"Video"===virtualclass.currApp&&this.mediaRecorder&&"recording"===this.mediaRecorder.state&&ioAdapter.mustSend({cf:"liveStream",url:c.message.fileName})},3E3):c.message.stop?this.clearEveryThing():c.message.stopVideo?this.stopTraditionalVideo():
c.message.url&&"Video"===virtualclass.currApp&&!virtualclass.gObj.hasOwnProperty("videoMode")&&roles.isStudent()&&(this.firstFile||(this.firstFile=c.message.url),this.lastFileRequested=c.message.url,this.fileList.insert(c.message.url,`${this.prefixUrl}/${c.message.url}.chvs`),!this.startingPoint&&this.fileList.ol.order.length>=this.bufferLength&&this.readyStartingPoint(),this.insertTime&&(clearTimeout(this.insertTime),this.pageRefresh=!0),this.insertTime=setTimeout(()=>{this.pageRefresh?5>=this.fileList.ol.order.length?
this.triggerStart(this.fileList.ol.order[0]):this.startFromPageRefresh||(this.startFromPageRefresh=!0,virtualclass.liveStream.callFromSeek||this.requestInitializePacket()):this.triggerStart(c.message.url);this.insertTime=this.pageRefresh=!1},100))}requestInitializePacket(c){this.stopTraditionalVideo();this.initPLayerForParticipaes();this.xhrInitPacket||(this.xhrInitPacket=axios.create({responseType:"arraybuffer",headers:{"Content-Type":"application/json","x-api-key":wbUser.lkey,"x-congrea-authuser":wbUser.auth_user,
...
b.config.url&&(this.currentFile=b.headers["x-congrea-seg"].split(".chvs")[0],this.startedStream=!0,this.listStream[this.currentFile]=b.data,this.firstFile=b.headers["x-congrea-seg"].split(".chvs")[0],delete this.startingPoint,this.readyStartingPoint())})}triggerStart(c){this.startedStream?this.startedStream&&0<this.fileList.ol.order.length&&(c=this.fileList.getNextByID(this.currentFile),this.requestStream(c.data),this.currentFile=c.id):(this.initPLayerForParticipaes(),this.requestStream(this.fileList.getCurrent(c)),
this.currentFile=c,this.startedStream=!0)}isMyTurn(c){return this.fileList.getNextByID(this.currentExecuted).id===c}inStreamList(c){return this.listStream[c]?this.listStream[c]:!1}readyStartingPoint(){let c=this.firstFile;if(!(-1>=this.fileList.getCurrentPosition(c))){var b;if(1<this.bufferLength){for(let a=1;a<this.bufferLength;a++)if(b=this.fileList.getNextByID(c))c=b.id;c!=this.firstFile&&(this.startingPoint=c)}}}playIfReady(c){var b=this.inStreamList(c);if(this.startingPoint&&c===this.startingPoint&&
this.inStreamList(this.firstFile)){b=this.inStreamList(this.firstFile);try{this.onBuffer(b),this.currentExecuted=this.firstFile,delete this.listStream[this.firstFile],this.startedAppending=!0,this.duringPlayFirstPacket(),this.startFromPageRefresh&&setTimeout(()=>{document.getElementById("liveStream").currentTime=this.MAX_TIME},virtualclass.isPlayMode?700:1600)}catch(a){this.requestInitializePacket(c)}}else if(this.startedAppending&&this.isMyTurn(c)&&b)try{this.onBuffer(b),delete this.listStream[c],
this.currentExecuted=c}catch(a){this.requestInitializePacket(c)}}};$.uiBackCompat=!1;
