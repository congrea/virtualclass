...
a){if(!virtualclass.videoUl.player){virtualclass.videoUl.player=videojs("dispVideo",{html5:{nativeAudioTracks:!1,nativeVideoTracks:!1,hls:{overrideNative:!0}},controlBar:{pictureInPictureToggle:!1}});if(roles.hasControls()){$(".vjs-autoPlay-button").length||virtualclass.videoUl.UI.appendAutoPlayButton(virtualclass.videoUl.player);const a=document.getElementById("autoPlayListBtn");a&&(a.className=virtualclass.videoUl.autoPlayClass)}virtualclass.videoUl.UI.attachPlayerHandler(virtualclass.videoUl.player,
...
cf:"videoUl",currTime:b.currentTime()});virtualclass.videoUl.isPaused=!0}),b.on("play",a=>{roles.hasControls()&&ioAdapter.mustSend({videoUl:{play:b.currentTime()},cf:"videoUl",videoTime:virtualclass.vutil.localToUTC()});virtualclass.videoUl.isPaused=!1}),b.on("fullscreenchange",a=>{setTimeout(()=>{virtualclass.vutil.showFullScreenButtonIfNeed()},0)}))},switchDisplay(b,a){var d=document.getElementById("dispVideo");d?d.style.display="block":virtualclass.videoUl.UI.createVideoElem(b);d=document.getElementById("messageLayoutVideo");
...
-1);virtualclass.videoUl.UI.saveYtsUrl(a)}});(b=document.querySelector(".congrea #newVideoBtn"))&&b.addEventListener("click",()=>{document.querySelector(".congrea #congreavideoContBody").style.display="block";document.querySelector(".congrea #listvideo").style.display="none"})},saveYtsUrl(b){const a=document.querySelector(".congrea #videourl"),d={};d.uuid=b;d.URL=a.value;d.title=a.value;b=virtualclass.api.addURL;"boolean"===typeof virtualclass.videoUl.getVideoId(a.value)?d.type="video_online":d.type=
...
c.length&&(d=c[c.length-1]),d&&(d.classList.remove("hid","left"),d.classList.add("shw"),c=b.querySelectorAll(".noteIndex.shw"),c.length&&(f=c[c.length-1]),f&&(AnnotationList,f.classList.remove("shw"),f.classList.add("hid","right")));"DocumentShare"===virtualclass.currApp&&(a=virtualclass.dts.docs.currNote,(a=b.querySelector(`.congrea #dcPaging #index${a}`))&&!a.classList.contains("active")&&(a.classList.add("active"),a.setAttribute("selected","selected")))}};a.prototype.removeNav=function(){const a=
b.getElementById("docShareNav");for(;a.firstChild;)a.removeChild(a.firstChild)};a.prototype.addActiveNavigation=function(a){this.addActiveClass(a);a=b.querySelector("#rightNavPage.disable");this.UI.isNodeAvailable(".noteIndex.shw.active","next")?null!=a&&("Whiteboard"===virtualclass.currApp?this.UI.setArrowStatus("rightNavPage","enable"):"DocumentShare"===virtualclass.currApp&&virtualclass.dts.indexNav.UI.setArrowStatusDocs(b.getElementById("rightNavPage"),"enable","disable")):"Whiteboard"===virtualclass.currApp?
this.UI.setArrowStatus("rightNavPage","disable"):virtualclass.dts.indexNav.UI.setArrowStatusDocs(b.getElementById("rightNavPage"),"disable","enable");a=b.querySelector("#leftNavPage.disable");this.UI.isNodeAvailable(".noteIndex.shw.active","prev")?null!=a&&("Whiteboard"===virtualclass.currApp?this.UI.setArrowStatus("leftNavPage","enable"):virtualclass.dts.indexNav.UI.setArrowStatusDocs(b.getElementById("leftNavPage"),"enable","disable")):"Whiteboard"===virtualclass.currApp?this.UI.setArrowStatus("leftNavPage",
"disable"):"DocumentShare"===virtualclass.currApp&&virtualclass.dts.indexNav.UI.setArrowStatusDocs(b.getElementById("leftNavPage"),"disable","enable")};a.prototype.addActiveClass=function(){let a;if("Whiteboard"===virtualclass.currApp)a=virtualclass.gObj.wbCount+1;else if("DocumentShare"===virtualclass.currApp)if(Array.isArray(virtualclass.orderList[virtualclass.dts.appName].ol.order)){a=virtualclass.orderList[virtualclass.dts.appName].ol.order.length;const b=virtualclass.orderList[virtualclass.dts.appName].ol.order.indexOf(virtualclass.dts.docs.currNote);
