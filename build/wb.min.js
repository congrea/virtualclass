...
d.appendChild(g);document.getElementById("recordingHeader").innerHTML=virtualclass.lang.getString("downloadFile");var h=new Blob([b],{type:"application/virtualclass"});g.addEventListener("click",function(){null!=c.webkitURL?m.href=c.webkitURL.createObjectURL(h):(m.href=c.URL.createObjectURL(h),m.onclick=a,document.body.appendChild(m));m.click();virtualclass.storage.config.endSession();io.init()})})},sendDataToServer:function(){virtualclass.storage.db.transaction(["chunkData"],"readwrite").objectStore("chunkData").clear();
