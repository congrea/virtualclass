...
ioAdapter.mustSend({pptMsg:"displayframe",cf:"ppt",user:"all",cfpt:"displayframe"});ioAdapter.mustSend({pptMsg:a,cf:"ppt",user:"all",cfpt:"setUrl"});b.style.display="visible"},validURLWithDomain:function(a){return/((http|https)?:\/\/)?([a-z\d\-]{1,63}\.)*[a-z\d\-]{1,255}\.[a-z]{2,6}\/{1,255}\s*/.test(a)},completeUrl:function(a){0<a.search("slides.com")&&0>a.search("embed")&&(a+="/embed");return a},extractUrl:function(a){if(0<a.search("src")){var b=a.match(/"(.*?)"/);if(0<b.length)return JSON.parse(b[0])}return a},
isUrlip:function(a){a.slice(0,2);return/(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)/.test(a)},saveIntoLocalStorage:function(a){if(this.validURLWithDomain(a.metaData.init)||this.isUrlip(a.metaData.init))localStorage.setItem("prevApp",JSON.stringify(a)),console.log("ppt_state",a.metaData.startFrom),localStorage.setItem("ppt_state",JSON.stringify(a.metaData.startFrom)),console.log("savinlocal"),
localStorage.setItem("pptUrl",JSON.stringify(a.metaData.init)),localStorage.setItem("autoSlideTime",JSON.stringify(virtualclass.sharePt.autoSlideTime)),localStorage.setItem("autoSlideFlag",JSON.stringify(virtualclass.sharePt.autoSlideFlag))},initTeacherLayout:function(){var a=document.getElementById("pptiframe");if(null!=document.getElementById("pptiframe")){var b={controls:!0,keyboard:!0,progress:!0,touch:!0},d;virtualclass.sharePt.autoSlideTime&&virtualclass.sharePt.autoSlideFlag?d=virtualclass.sharePt.autoSlideTime:
virtualclass.sharePt.autoSlideFlag&&(d=6E3);"undefined"!=typeof d&&1E3<d&&(b.autoSlide=d);a.contentWindow.postMessage(JSON.stringify({method:"configure",args:[b]}),"*");a.contentWindow.postMessage(JSON.stringify({method:"toggleAutoSlide"}),"*")}a=document.getElementById("pptMessageLayout");null!=a&&(a.style.display="none");virtualclass.sharePt.UI.createUrlContainer();document.getElementById("urlcontainer").style.display="block";virtualclass.sharePt.eventsObj=[]},initStudentLayout:function(){var a=
document.getElementById("pptiframe");virtualclass.sharePt.eventsObj=[];virtualclass.sharePt.UI.messageDisplay();null!=document.getElementById("pptiframe")?(""!=a.src&&virtualclass.sharePt.hideElement("pptMessageLayout"),a.contentWindow.postMessage(JSON.stringify({method:"configure",args:[{controls:!1,autoSlide:0,autoSlideStoppable:!0,keyboard:!1,progress:!1,touch:!1}]}),"*")):virtualclass.sharePt.displayElement("pptMessageLayout");virtualclass.sharePt.hideElement("urlcontainer")},hideElement:function(a){a=
document.getElementById(a);null!=a&&(a.style.display="none")},displayElement:function(a){a=document.getElementById(a);null!=a&&(a.style.display="block")},attachEvent:function(a,b,d){var c=document.getElementById(a),c=document.getElementById(a);null!=c&&c.addEventListener(b,d)},attachMessageEvent:function(a,b){c.addEventListener(a,b)},UI:{id:"virtualclassSharePresentation","class":"virtualclass",container:function(){console.log("test create main  container with id virtualclassSharePresentation");var a=
document.getElementById(this.id);null!=a&&a.parentNode.removeChild(a);a=document.createElement("div");a.id=this.id;a.className=this["class"];var b=document.getElementById(virtualclass.rWidgetConfig.id);console.log(b.parentNode);b.parentNode.insertBefore(a,b)},createIframe:function(){var a=document.getElementById("iframecontainer");null!=a&&a.parentNode.removeChild(a);console.log("test create ppt  iframe");a=document.getElementById("virtualclassSharePresentation");if(null!=a){var b=document.createElement("div");
