...
virtualclass.pbar.renderProgressBar(0,0,"progressBar","progressValue");virtualclass.storage.config.endSession()},playProgressBar:function(){this.getPlayTotTime||(this.totPlayTime=this.items.reduce(function(a,b){return b.hasOwnProperty("playTime")?{playTime:a.playTime+b.playTime}:a.playTime}),this.getPlayTotTime=!0);if(0<this.tillPlayTime){this.tillPlayTime>this.totPlayTime&&(this.tillPlayTime=this.totPlayTime);virtualclass.pbar.renderProgressBar(this.totPlayTime,this.tillPlayTime,"playProgressBar",
void 0);var a=this.convertIntoReadable(this.tillPlayTime);document.getElementById("tillRepTime").innerHTML=a.m+" : "+a.s;"undefined"==typeof alreadyCalcTotTime&&(a=this.convertIntoReadable(this.totPlayTime),document.getElementById("totalRepTime").innerHTML=a.m+" : "+a.s,alreadyCalcTotTime=!0)}else virtualclass.pbar.renderProgressBar(this.totPlayTime,0,"playProgressBar",void 0)},convertIntoReadable:function(a){a/=1E3;return{s:Math.floor(a%60),m:Math.floor(a/60)}},xhrsenddata:function(a,b,d){"undefined"!=
typeof b&&(virtualclass.recorder.error=1);"undefined"!=typeof d&&(virtualclass.recorder.mkDownloadLink=d);if(1!=virtualclass.recorder.storeDone){"undefined"!=typeof earlierTimeout&&clearTimeout(earlierTimeout);if("undefined"==typeof a||""==a)a="first";virtualclass.recorder.rnum=a;virtualclass.storage.getrowData(["chunkData"],function(a,b){1==g&&a.hasOwnProperty("rdata")&&(virtualclass.recorder.rdlength=a.rdata.length);if("string"==typeof a||"undefined"==typeof a)earlierTimeout=setTimeout(function(){virtualclass.recorder.xhrsenddata(virtualclass.recorder.rnum)},
1E3);else if(a.hasOwnProperty("status")&&"done"==a.status)console.log("should invoked download function on "),virtualclass.recorder.storeDone=1,console.log("From here actuall recorder finished"),"undefined"!=typeof virtualclass.recorder.mkDownloadLink&&""!=virtualclass.recorder.mkDownloadLink?virtualclass.recorder.mkDownloadLink():setTimeout(function(){virtualclass.recorder.afterRecording()},1E3);else if("undefined"!=typeof b&&(virtualclass.recorder.rnum=b),1==virtualclass.recorder.error)0==virtualclass.recorder.storeDone&&
(virtualclass.recorder.xhrsenddata(virtualclass.recorder.rnum,"error"),virtualclass.recorder.rnum++);else{var d=new FormData;d.append("record_data",JSON.stringify(a));d.append("user",virtualclass.gObj.uid);d.append("cn",g);d.append("sesseionkey",virtualclass.recorder.sessionKey);virtualclass.recorder.items=[];virtualclass.xhr.send(d,importfilepath,function(b){"done"===b?(virtualclass.pbar.renderProgressBar(a.totalStore,a.totalSent,"progressBar","progressValue"),virtualclass.recorder.rnum++,g++,virtualclass.recorder.xhrsenddata(virtualclass.recorder.rnum)):
"ERROR"===b?virtualclass.recorder.initMakeAvailDownloadFile():0<=h.indexOf(b)?(alert(virtualclass.lang.getString(b)),virtualclass.recorder.initMakeAvailDownloadFile()):console.log("Error message not found")})}},virtualclass.recorder.rnum)}},afterRecording:function(){g=1;virtualclass.recorder.rdlength=0;document.getElementById("progressContainer").style.display="block";setTimeout(function(){var a=document.getElementById("recordingContainer");a.classList.add("recordingFinished");console.log("Record :- after recording");
virtualclass.clearSession();virtualclass.pbar.renderProgressBar(0,0,"progressBar","progressValue");virtualclass.pbar.renderProgressBar(0,0,"indProgressBar","indProgressValue");a.classList.remove("recordingFinished")},2E3)},makeAvailDownloadFile:function(){console.log("Recorder:- DOWNLLOAD MESSAGE");var b=document.getElementById("recordingContainer"),d=c.getComputedStyle(b).display;"none"!=d&&"block"==d||setTimeout(function(){virtualclass.popup.openProgressBar();io.disconnect()},3E3);var f=document.getElementById("downloadFileCont");
null==f&&(f=document.createElement("div"),f.id="downloadFileCont");var g=document.createElement("div");g.id="errormsessage";g.innerHTML=virtualclass.lang.getString("msgForDownloadStart");var h=document.getElementById("progressContainer");h.style.display="none";f.appendChild(g);b.appendChild(f);this.alreadyDownload=!0;virtualclass.storage.getAllDataForDownload(["chunkData"],function(b){virtualclass.gObj.saveSession=!0;var d=document.createElement("button");d.id="downloadButton";d.innerHTML=virtualclass.lang.getString("downloadFile");
var m=document.createElement("a");m.id="dlink";m.href="";m.download="session.vcp";m.innerHTML=virtualclass.lang.getString("download");g.innerHTML=virtualclass.lang.getString("filenotsave");f.appendChild(d);var p=document.getElementById("recordingHeader");p.innerHTML=virtualclass.lang.getString("downloadFile");var r=new Blob([b],{type:"application/virtualclass"});virtualclass.hasOwnProperty("prevScreen")&&virtualclass.prevScreen.hasOwnProperty("currentStream")&&virtualclass.prevScreen.unShareScreen();
d.addEventListener("click",function(){null!=c.webkitURL?m.href=c.webkitURL.createObjectURL(r):(m.href=c.URL.createObjectURL(r),m.onclick=a,document.body.appendChild(m));m.click();p=document.getElementById("recordingHeader");p.innerHTML=virtualclass.lang.getString("uploadsession");f.parentNode.removeChild(f);h.style.display="block";virtualclass.recorder.hasOwnProperty("dataCame")&&clearTimeout(virtualclass.recorder.dataCame);virtualclass.clearSession();delete virtualclass.gObj.saveSession;io.disconnect();
io.init(virtualclass.uInfo)})})},sendDataToServer:function(){virtualclass.storage.db.transaction(["chunkData"],"readwrite").objectStore("chunkData").clear();virtualclass.popup.waitBlockAction("none");c.Worker&&(mvDataWorker.postMessage({rdata:virtualclass.recorder.items,makeChunk:!0}),mvDataWorker.onmessage=function(a){a.data.hasOwnProperty("done_problem")?console.log("done problem"):a.data.hasOwnProperty("status")&&"done"==a.data.status&&(console.log("Recorder:- done"),delete virtualclass.recorder.startUpload,
virtualclass.recorder.recordDone=!0);virtualclass.storage.chunkStorage(a.data)})},displayWaitPopupIfNot:function(){!1==this.waitPopup&&(virtualclass.popup.sendBackOtherElems(),document.getElementById("recordingContainer").style.display="none",virtualclass.popup.waitBlockAction("block"),virtualclass.pbar.renderProgressBar(0,0,"downloadProgressBar","downloadProgressValue"),virtualclass.popup.waitBlock(),this.waitPopup=!0)},requestDataFromServer:function(a,b){this.displayWaitPopupIfNot(virtualclass.lang.getString("plswaitwhile"));
var d=new FormData;d.append("prvfile",b);d.append("fileBundelId",a);var c=getUrlVars(exportfilepath);c.hasOwnProperty("cmid")&&d.append("id",c.cmid);virtualclass.xhr.send(d,exportfilepath,function(b){virtualclass.recorder.sendToWorker(b,a)})},sendToWorker:function(a,b){c.Worker&&(mvDataWorker.postMessage({rdata:a,getData:!0}),mvDataWorker.onmessage=function(a){f++;var d=virtualclass.recorder.isUptoBaseValue(a.data.alldata.totalSent,a.data.alldata.totalStore,30);virtualclass.recorder.ctotalStore=a.data.alldata.totalStore;
virtualclass.recorder.ctotalSent=a.data.alldata.totalSent;virtualclass.pbar.renderProgressBar(a.data.alldata.totalStore,a.data.alldata.totalSent,"downloadProgressBar","downloadProgressValue");d&&!virtualclass.recorder.alreadyAskForPlay?(a.data.alldata.totalSent>a.data.alldata.totalStore?virtualclass.recorder.askToPlay("completed"):virtualclass.recorder.askToPlay(),virtualclass.recorder.alreadyAskForPlay=!0,virtualclass.recorder.tempRecData.push(a.data.alldata.rdata)):d&&virtualclass.recorder.playStart&&
!1==virtualclass.recorder.waitServer?virtualclass.recorder.init(a.data.alldata.rdata):(virtualclass.recorder.tempRecData.push(a.data.alldata.rdata),!0==virtualclass.recorder.waitServer&&(virtualclass.recorder.alreadyPlayed=!0));a.data.alldata.rdata[a.data.alldata.rdata.length-1].hasOwnProperty("sessionEnd")?(console.log("Request file  Finished Here"),virtualclass.recorder.allFileFound=!0,!0==virtualclass.recorder.waitServer&&(virtualclass.storage.config.endSession(),a=virtualclass.recorder.tempRecData.reduce(function(a,
b){return a.concat(b)}),virtualclass.recorder.objn=0,virtualclass.recorder.init(a),virtualclass.recorder.play(),virtualclass.recorder.waitServer=!1,virtualclass.popup.closeElem())):(console.log("request file"),virtualclass.recorder.requestDataFromServer(b,f))})},playInt:function(){var a=virtualclass.recorder.tempRecData.reduce(function(a,b){return a.concat(b)});virtualclass.popup.closeElem();virtualclass.recorder.init(a);virtualclass.recorder.playStart=!0;virtualclass.recorder.tempRecData.length=
0;localStorage.setItem("mySession","thisismyplaymode");virtualclass.recorder.initController()},askToPlay:function(a){"undefined"!=typeof a?document.getElementById("askplayMessage").innerHTML=virtualclass.lang.getString("playsessionmsg"):document.getElementById("askplayMessage").innerHTML=virtualclass.lang.getString("askplayMessage");document.getElementById("askPlay").style.display="block";a=document.getElementById("playButton");null!=a&&(a.style.display="block",a.addEventListener("click",this.playInt))},
isUptoBaseValue:function(a,b,d){return a>=b*d/100?!0:!1},play:function(){if(0==this.objn){var a=document.getElementById("recPlay");this.doControlActive(a)}var b=this;this.controller.pause||("undefined"==typeof b.playTimeout&&""==b.playTimeout||clearTimeout(b.playTimeout),this.hasOwnProperty("playTime")||(this.playTime=this.items[0].playTime,d.data=JSON.parse(this.items[this.objn].recObjs),io.cfg=d.data,virtualclass.gObj.uRole="s"),"undefined"==typeof this.items[this.objn+1]||this.items[this.objn].hasOwnProperty("sessionEnd")?
(this.items[this.objn].hasOwnProperty("sessionEnd")||(d.data=this.items[this.objn].recObjs,io.onRecMessage(b.convertInto(d))),!1==virtualclass.recorder.allFileFound?(virtualclass.recorder.waitServer=!0,virtualclass.recorder.waitPopup=!1,this.displayWaitPopupIfNot(),(virtualclass.recorder.hasOwnProperty("ctotalStore")||virtualclass.recorder.hasOwnProperty("ctotalSent"))&&virtualclass.pbar.renderProgressBar(virtualclass.recorder.ctotalStore,virtualclass.recorder.ctotalSent,"downloadProgressBar","downloadProgressValue")):
this.items[this.objn].hasOwnProperty("sessionEnd")&&(virtualclass.popup.replayWindow(),virtualclass.popup.sendBackOtherElems(),document.getElementById("replayClose").addEventListener("click",function(){c.close()}),document.getElementById("replayButton").addEventListener("click",function(){virtualclass.recorder.replayFromStart.call(virtualclass.recorder)}))):b.playTimeout=setTimeout(function(){var a={};a.data=b.items[b.objn].recObjs;try{io.onRecMessage(b.convertInto(a))}catch(d){console.log("PLAY ERROR "+
...
.85*(s.other.bottom-s.other.top)+"px")}m||wf(a,h,f)}return d}function wf(a,b,c){function d(a,b,N,c){0>b&&(b=0);b=Math.round(b);c=Math.round(c);h.appendChild(D("div",null,"CodeMirror-selected","position: absolute; left: "+a+"px; top: "+b+"px; width: "+(null==N?q-a:N)+"px; height: "+(c-b)+"px"))}function n(b,c,M){var S=E(g,b),n=S.text.length,f,h;xf(ta(S),c||0,null==M?n:M,function(g,m,ja){var s=uc(a,z(b,g),"div",S,"left"),r,fc;g==m?(r=s,ja=fc=s.left):(r=uc(a,z(b,m-1),"div",S,"right"),"rtl"==ja&&(ja=
s,s=r,r=ja),ja=s.left,fc=r.right);null==c&&0==g&&(ja=p);3<r.top-s.top&&(d(ja,s.top,null,s.bottom),ja=p,s.bottom<r.top&&d(ja,s.bottom,null,r.top));null==M&&m==n&&(fc=q);if(!f||s.top<f.top||s.top==f.top&&s.left<f.left)f=s;if(!h||r.bottom>h.bottom||r.bottom==h.bottom&&r.right>h.right)h=r;ja<p+1&&(ja=p);d(ja,r.top,fc-ja,r.bottom)});return{start:f,end:h}}var f=a.display,g=a.doc,h=document.createDocumentFragment(),m=he(a.display),p=m.left,q=Math.max(f.sizerWidth,Ka(a)-f.sizer.offsetLeft)-m.right,f=b.from();
...
d.scrollbars.setScrollTop(f.scrollTop),d.scroller.scrollTop=f.scrollTop);null==n.scrollLeft||d.scroller.scrollLeft==n.scrollLeft&&!n.forceScroll||(f.scrollLeft=Math.max(0,Math.min(d.scroller.scrollWidth-Ka(c),n.scrollLeft)),d.scrollbars.setScrollLeft(f.scrollLeft),d.scroller.scrollLeft=f.scrollLeft,L(c));if(n.scrollToPos){var h=void 0,g=H(f,n.scrollToPos.from),h=H(f,n.scrollToPos.to),m=n.scrollToPos.margin;null==m&&(m=0);for(var p=0;5>p;p++){var q=!1,r=Ha(c,g),s=h&&h!=g?Ha(c,h):r,s=zc(c,Math.min(r.left,
s.left),Math.min(r.top,s.top)-m,Math.max(r.left,s.left),Math.max(r.bottom,s.bottom)+m),v=c.doc.scrollTop,u=c.doc.scrollLeft;null!=s.scrollTop&&(Hb(c,s.scrollTop),1<Math.abs(c.doc.scrollTop-v)&&(q=!0));null!=s.scrollLeft&&(hb(c,s.scrollLeft),1<Math.abs(c.doc.scrollLeft-u)&&(q=!0));if(!q)break}h=r;n.scrollToPos.isCursor&&c.state.focused&&(Ea(c,"scrollCursorIntoView")||(m=c.display,p=m.sizer.getBoundingClientRect(),g=null,0>h.top+p.top?g=!0:h.bottom+p.top>(window.innerHeight||document.documentElement.clientHeight)&&
...
"\u00a0");0<C.length&&(q=!1);r+=L+C.replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\u00a0/g,"&nbsp;")+J}}t?r+="</li>":x||(q&&(r+="&nbsp;"),r+="</div>");for(;0<w.length;)r+=a(w.pop());B&&(r='<style>ul.vceditor-todo { list-style: none; margin-left: 0; padding-left: 0; } ul.vceditor-todo > li { padding-left: 1em; text-indent: -1em; } ul.vceditor-todo > li:before { content: "\\2610"; padding-right: 5px; } ul.vceditor-todo > li.vceditor-checked:before { content: "\\2611"; padding-right: 5px; }</style>\n'+
