...
setCanvasDimension:function(a){if("undefined"!=typeof vcan.main.canvas){var d=vcan.main.canvas;ctx=vcan.main.canvas.getContext("2d");d.width=a.width;var f=roles.hasControls()||roles.hasAdmin()?65:10;d.height=a.height-f;console.log("canvas width "+d.width);a=vcan.utility.getElementOffset(document.getElementById("canvas"));vcan.main.offset.x=a.x}},getResoultion:function(a){var d={};1280>a?(d.width=1024,d.height=768):1280<=a&&1366>a?(d.width=1280,d.height=1024):1366<=a&&1920>a?(d.width=1366,d.height=
768):1920<=a&&(d.width=1920,d.height=1080);return d},reportBrowser:function(a){a=this.getErrors(a);1<a.length?virtualclass.error.push(a.join(",")+" are disabled in your browser."):1==a.length&&virtualclass.error.push(a+" is disabled in your browser.")},getErrors:function(a){var d=[],f="canvas webSocket getusermedia webaudio indexeddb localstorage typedarray".split(" ");"t"==a&&f.push("webworker","screenshare");for(a=0;a<f.length;a++)this[f[a]]||("screenshare"==f[a]&&(virtualclass.gObj.errNotScreenShare=
!0),d.push(virtualclass.lang.getString("err"+f[a])));return d},isiOSDevice:function(){return parseFloat((""+(/CPU.*OS ([0-9_]{1,5})|(CPU like).*AppleWebKit.*Mobile/i.exec(navigator.userAgent)||[0,""])[1]).replace("undefined","3_2").replace("_",".").replace("_",""))||!1},isAndroid:function(){return-1<navigator.userAgent.toLowerCase().indexOf("android")},check:function(){var a=this.isiOSDevice();this.device="desktop";if(a){var d="iOS";this.device="mobTab"}else var f=this.isAndroid(),a=this.mybrowser.detection(),
...
[d,a]))):"iOS"==d?/(iPad)/g.test(navigator.userAgent)?roles.isStudent()?8<=a?(virtualclass.vutil.initDisableAudVid(),virtualclass.gObj.iosIpadbAudTrue=!1,d=document.createElement("div"),d.innerHTML=virtualclass.lang.getString("iosAudEnable"),d.id="iosAudioTrigger",d.addEventListener("click",function(){virtualclass.vutil.firstiOSaudioCall();this.parentNode.removeChild(this)}),f=document.getElementById("audioWidget"),f.parentNode.insertBefore(d,f.nextSibling)):(virtualclass.vutil.initDisableVirtualClass(),
...
(new Date).getTime(),b.detail.hasOwnProperty("cevent")||"text"==a||"t_clearall"==f.tool.cmd||vcan.optimize.calculatePackets(v,"m",g,c);presentmousemovetime=(new Date).getTime();2E3<=presentmousemovetime-lastmousemovetime&&(b.detail.hasOwnProperty("cevent")||"text"==a||"t_clearall"==f.tool.cmd||vcan.optimize.calculatePackets(v,"m",g,c),lastmousemovetime=(new Date).getTime());f.prvObj=x.coreObj}else"move"==f.vcan.main.action&&(""!=vcan.main.currentTransform&&null!=vcan.main.currentTransform||"move"!=
f.vcan.main.action)||virtualclass.vutil.beforeSend({createArrow:!0,x:b.currX,y:b.currY,cf:"createArrow"})};d.mouseup=function(b,s){b.detail.hasOwnProperty("cevent")&&(b.clientX=b.detail.cevent.x+f.vcan.main.offset.x,b.clientY=b.detail.cevent.y+f.vcan.main.offset.y,b.x=b.detail.cevent.x+f.vcan.main.offset.x,b.y=b.detail.cevent.x+f.vcan.main.offset.y,b.pageX=b.detail.cevent.x+f.vcan.main.offset.x,b.pageY=b.detail.cevent.y+f.vcan.main.offset.y,b.currX=b.detail.cevent.x,b.currY=b.detail.cevent.y);g=b.currX;
...
g.id+"textarea"){h=c[m].value;break}}else h=c;a.font="20px Times New Roman";var p=0,s=h.split(/\r?\n/),p=a.measureText(s[0]).width;c=13;for(m=1;m<s.length;m++){c+=13;var v=a.measureText(s[m]).width;v>p&&(p=v)}p/=2;m=(new Date).getTime();a={type:"text",text:h,x:b.measure.x+p,y:b.measure.y+c,fontSize:20,fontWidth:a.measureText(h).width,mp:{x:b.measure.x,y:b.measure.y}};!1==virtualclass.wb.obj.drawTextObj.muser&&(h={mt:m,ac:"d",x:this.startPosX,y:this.startPosY,mtext:a.text},virtualclass.wb.uid++,h.uid=
...
this.height-this.height*this.average/100)},_display:function(a,d){a.beginPath();a.moveTo(this.width,this.height);a.lineTo(this.width,d);a.lineWidth=this.width;a.strokeStyle="rgba(32, 37, 247, 0.8)";a.closePath();a.stroke()},canvasForVideo:function(){var a=cthis.video.myVideo.parentNode,d=document.createElement("canvas");d.width=this.width+3;d.height=this.height;d.id="graphCanvas";a.style.position="relative";d.style.position="absolute";d.style.left=0;d.style.top=-7;a.appendChild(d)}};this.attachFunctionsToAudioWidget()},
audioSend:function(a,d){if(virtualclass.gObj.audMouseDown&&1==io.sock.readyState){var b=virtualclass.vutil.breakintobytes(virtualclass.gObj.uid,8),b=new Int8Array([101,b[0],b[1],b[2],b[3]]),c=new Int8Array(a.length+b.length);c.set(b);c.set(a,b.length);ioAdapter.sendBinary(c);virtualclass.gObj.video.audio.setAudioStatus(d)}else virtualclass.gObj.video.audio.setAudioStatus("stop")},slienceDetection:function(c,p){var s=0,v=0,x=0;for(i=0;i<p.length;i++)x=Math.abs(p[i]),s<x&&(s=x),v+=x;f=v/p.length;x=
Math.abs(f^2-d^2);d=f;5>x&&(g=s);g>s&&(g=s);h<s&&(h=s);50*g<h&&(g*=5);h/50>g&&(h=s);v=h/g;switch(!0){case 8<v:x=2.5;break;case 5<v:x=2;break;case 3<v:x=1.2;break;default:x=1}2<=v&&s>=g*x?(0==a&&0!=b&&(this.audioSend(b),b=0),this.audioSend(c,"sending"),a=9):0<a?(this.audioSend(c,"sending"),a--):2>v?this.audioSend(c,"sending"):(this.setAudioStatus("notSending"),10<v&&(h*=.8),b=c);return c},setAudioStatus:function(a){if("undefined"==typeof d)var d=document.getElementById("audioWidget").getElementsByClassName(this.sdElem)[0];
d.setAttribute("data-silence-detect",a)},makeIconNotDraggable:function(a,d){var b=document.getElementById(a,d).getContext("2d"),f=new Image;f.onload=function(){b.drawImage(f,0,0)};f.src=c.whiteboardPath+"images/"+d},attachFunctionsToAudioWidget:function(){for(var a=document.getElementById("audioWidget").getElementsByClassName("audioTool"),d=0;d<a.length;d++)"speakerPressOnce"!=a[d].id&&"speakerPressing"==a[d].id&&this.attachSpeakToStudent(a[d].id),"speakerPressing"!=a[d].id&&a[d].addEventListener("click",
this.audioToolInit)},audioToolInit:function(){var a=virtualclass.gObj.video.audio;if("speakerPressOnce"==this.id)a.clickOnceSpeaker(this.id);else if("audioTest"==this.id){var d=this;virtualclass.popup.confirmInput(virtualclass.lang.getString("audioTest"),function(b){b&&a.testInit(d.id)})}else"silenceDetect"==this.id&&(this.getElementsByTagName("a"),a.sd?(a.sd=!1,this.className+=" sdDisable"):(a.sd=!0,this.className+=" sdEnable"))},attachSpeakToStudent:function(a){var d=this,b=document.getElementById(a),
...
f;f=this.Html5Audio.audioContext.createGain();f.gain.value=.9;a.connect(f);f.connect(this.Html5Audio.audioContext.destination);null!=c&&c.ad&&c.aud&&(virtualclass.user.control.iconAttrManupulate(d,"icon-audioEnaGreen"),f=document.getElementById(c.id+"contrAudAnch"),null!=f&&f.setAttribute("data-title",virtualclass.lang.getString("audioOn")));a.onended=function(){c=JSON.parse(localStorage.getItem("virtualclass"+d));"undefined"==typeof b&&(clearTimeout(virtualclass.gObj[d].out),virtualclass.gObj[d].isplaying=
!1,0<virtualclass.gObj.video.audio.audioToBePlay[d].length?virtualclass.gObj.video.audio.getChunks(d):null!=c&&c.ad&&c.aud&&virtualclass.user.control.iconAttrManupulate(d,"icon-audioEnaOrange"))};a.start(1);virtualclass.gObj[d].isplaying=!0;"undefined"==typeof b&&(virtualclass.gObj[d].out=setTimeout(function(){virtualclass.gObj[d].isplaying=!1;0<virtualclass.gObj.video.audio.audioToBePlay[d].length&&virtualclass.gObj.video.audio.getChunks(d)},1E3*a.buffer.duration+10))},calcAverage:function(){var a=
new Uint8Array(analyser.frequencyBinCount);analyser.getByteFrequencyData(a);for(var d=0,b=a.length,c=0;c<b;c++)d+=a[c];this.graph.average=d/b},audioInGraph:function(){var a=cthis.video;if(roles.hasControls()){var d=this.graph.height-this.graph.height*this.graph.average/100;a.tempVidCont.beginPath();a.tempVidCont.moveTo(a.tempVid.width-this.graph.width,this.graph.height);a.tempVidCont.lineTo(a.tempVid.width-this.graph.width,d);a.tempVidCont.lineWidth=this.graph.width;a.tempVidCont.strokeStyle="rgba(247, 25, 77, 1)";
a.tempVidCont.closePath();a.tempVidCont.stroke()}},queue:function(a,d){this.hasOwnProperty("audioToBePlay")||(this.audioToBePlay={});this.audioToBePlay.hasOwnProperty(d)||(this.audioToBePlay[d]=[]);this.audioToBePlay[d].push(a)},getChunks:function(a,d){8<this.audioToBePlay[a].length?(this.audioToBePlay[a].length=0,virtualclass.gObj[a].isplaying=!1):0<this.audioToBePlay[a].length?(this.playRecordedAudio(this.audioToBePlay[a],a),this.audioToBePlay[a].length=0):virtualclass.gObj[a].isplaying=!1},replay:function(a,
d){repMode=!0;var b,c,f;0<this.audioNodes.length&&(b=this.mergeBuffers(this.audioNodes),c=this.Html5Audio.audioContext.currentTime+a,f=this.Html5Audio.audioContext.createBuffer(1,b.length,7800),f.getChannelData(0).set(b),b=this.Html5Audio.audioContext.createBufferSource(),b.buffer=f,b.connect(this.Html5Audio.audioContext.destination),b.start(c,d))},mergeBuffers:function(a,d){for(var b=new Float32Array(d),c=0,f=a.length,g=0;g<f;g++){var h=a[g];b.set(h,c);c+=h.length}return b},assignFromLocal:function(a,
d){this.init();for(var b=0;b<a.length;b++){var c=LZString.decompressFromBase64(a[b]),c=this.str2ab(c);samples=G711.decode(c,{alaw:"alaw"==this.encMode?!0:!1,floating_point:!0,Eight:!0});this.audioNodes.push(new Float32Array(samples));this.recordingLength+=16384}"undefined"!=typeof d&&d()},manuPulateStream:function(){var a=cthis.audio.Html5Audio.audioContext.createMediaStreamSource(cthis.stream);cthis.audio.bufferSize=16384;grec=cthis.audio.Html5Audio.audioContext.createScriptProcessor(cthis.audio.bufferSize,
1,1);grec.onaudioprocess=cthis.audio.recorderProcess.bind(cthis.audio);var d=cthis.audio.Html5Audio.audioContext.createGain();d.gain.value=.9;a.connect(d);d.connect(grec);grec.connect(cthis.audio.Html5Audio.audioContext.destination)},updateInfo:function(){this.audioStreamArr=[];repMode=this.recordAudio=!1},receivedAudioProcess:function(a){if(!virtualclass.gObj.hasOwnProperty("iosTabAudTrue")||!1!=virtualclass.gObj.iosTabAudTrue){a=this.extractData(a);var d=a[0];if("undefined"==typeof b)var b={};b.hasOwnProperty(d)||
(b[d]={},b[d].ad=!0,b=virtualclass.user.control.updateUser(d,"ad",!0),virtualclass.user.control.audioSign(b,"create"));virtualclass.gObj.video.audio.queue(a[1],d);virtualclass.gObj.hasOwnProperty(d)&&virtualclass.gObj[d].hasOwnProperty("isplaying")?!1==virtualclass.gObj[d].isplaying&&virtualclass.gObj.video.audio.getChunks(d):(virtualclass.gObj[d]={},virtualclass.gObj[d].isplaying=!0,virtualclass.gObj.video.audio.getChunks(d))}},extractData:function(a){a=new Int8Array(a);return[virtualclass.vutil.numValidateFour(a[1],
...
250<=d.offsetHeight&&"undefined"!=d.style.overflowY&&"scroll"!=d.style.overflowY&&(d.style.overflowY="scroll",document.getElementById(virtualclass.gObj.chat.mainChatBoxId).style.borderTop="3px solid #bbb")}},200)},createVideoElement:function(){var a=document.createElement("div");a.className="videoWrapper";var d=document.createElement("div");d.className="videoSubWrapper";a.appendChild(d);var b=document.createElement("video");b.id="video"+virtualclass.gObj.uid;b.autoplay=!0;d.appendChild(b);return a},
insertTempVideo:function(a){var d=document.createElement("canvas");d.id="tempVideo";a.parentNode.insertBefore(d,a)},tempVideoInit:function(){cthis.video.tempVid=document.getElementById("tempVideo");cthis.video.tempVid.width=cthis.video.width;cthis.video.tempVid.height=cthis.video.height;cthis.video.tempVidCont=cthis.video.tempVid.getContext("2d")},process:function(a){var d=new Uint8ClampedArray(a);a=virtualclass.vutil.numValidateFour(d[1],d[2],d[3],d[4]);d=d.subarray(5,d.length);virtualclass.gObj.video.video.playWithoutSlice(a,
d)}},init:function(a){cthis=this;vcan.oneExecuted=!0;a={audio:!0,video:!0};cthis.video.init();virtualclass.vutil.isPlayMode()||(virtualclass.adpt=new virtualclass.adapter,virtualclass.adpt.init(navigator).getUserMedia(a,this.handleUserMedia,this.handleUserMediaError));virtualclass.system.wbRtc.peerCon&&"undefined"==typeof localStorage.wbrtcMsg&&(virtualclass.view.multiMediaMsg("WebRtc"),localStorage.wbrtcMsg=!0)},handleUserMedia:function(a){localStorage.removeItem("dvid");var d=document.getElementById("audioWidget"),
b=localStorage.getItem("audEnable");null!=b?(b=JSON.parse(b),"false"==b.ac?"vd"==b.r?virtualclass.user.control.audioWidgetEnable():virtualclass.user.control.audioWidgetDisable():virtualclass.user.control.audioWidgetEnable()):virtualclass.vutil.elemHasAnyClass("audioWidget")&&d.classList.contains("deactive")&&virtualclass.user.control.audioWidgetEnable();cthis.video.tempStream=a;cthis.audio.init();a=document.getElementById("ml"+virtualclass.gObj.uid);null!=a&&null!=a.getElementsByTagName("video")&&
...
virtualclass.gObj.uid),virtualclass.adpt.attachMediaStream(cthis.video.myVideo,d),cthis.video.myVideo.muted=!0,d.ontimeupdate=function(){console.log("raja"+d.currentTime)},virtualclass.jId==virtualclass.gObj.uid&&(cthis.stream=cthis.video.tempStream,cthis.audio.manuPulateStream(),cthis.audio.graph.canvasForVideo()),cthis.video.myVideo.onloadedmetadata=function(){cthis.video.startToStream()});userMedia=!0},updateVidContHeight:function(){var a=document.getElementById("virtualclassCont"),a=vcan.utility.getElementOffset(a),
a=c.innerHeight-(a.y+75);document.getElementById("chat_div").style.maxHeight=a+"px"},close:function(){virtualclass.gObj.video.hasOwnProperty("smallVid")&&clearInterval(virtualclass.gObj.video.smallVid)},dispAllVideo:function(a){setTimeout(function(){var d=document.getElementById(a);if(null!=d)for(var d=d.getElementsByTagName("video"),b=0;b<d.length;b++)d[b].play()},1040)},existVideoContainer:function(a){for(var d=document.getElementsByClassName("userVideos"),b=0;b<d.length;b++)if(d[b].id=="video"+
...
null!=p?m=m+p.clientHeight+3:(roles.isEducator()||roles.hasControls())&&"Whiteboard"==virtualclass.currApp&&(m=m+46+3);a.width-=g+h+0;c.style.width=a.width+"px";c.style.height=a.height-m+"px";"virtualclassScreenShare"==d&&(g=document.getElementById(d+"Local"),a.width-=10,c.style.width=a.width,g.style.width=a.width+"px",virtualclass.vutil.setScreenInnerTagsWidth(d))},setScreenInnerTagsWidth:function(a){document.getElementById(a);document.getElementById(a+"Local");document.getElementById(a+"LocalVideo")},
...
(virtualclass[d].localCanvas=g,virtualclass[d].localCont=virtualclass[d].localCanvas.getContext("2d"),c=virtualclass[d].localtempCont.getImageData(0,0,virtualclass[d].localtempCanvas.width,virtualclass[d].localtempCanvas.height),virtualclass[d].localCont.putImageData(c,0,0));virtualclass.vutil.removeTempVideo("virtualclass"+a+"LocalTemp")}},clickOutSideCanvas:function(){this.exitTextWrapper()&&virtualclass.wb.obj.drawTextObj.textUtility(virtualclass.wb.gObj.spx,virtualclass.wb.gObj.spy)},exitTextWrapper:function(){return 0<
...
d.w,virtualclass[g].localCanvas.height=d.h),virtualclass[g].drawImages(a))}virtualclass.previous=virtualclass[g].id}}};c.screenShare=function(b){virtualclass.getSceenFirefox=function(){var a=parseInt(c.navigator.userAgent.match(/Firefox\/(.*)/)[1],10);33<=a?(virtualclass.adpt=new virtualclass.adapter,virtualclass.adpt.init(navigator).getUserMedia({video:{mozMediaSource:"window",mediaSource:"window"}},function(a,d){virtualclass.ss._init();virtualclass.ss.initializeRecorder.call(virtualclass.ss,a);
if("undefined"==typeof d)var b=a.currentTime,f=c.setInterval(function(){a||c.clearInterval(f);if(a.currentTime==b&&(c.clearInterval(f),a.onended))a.onended();b=a.currentTime},500)},function(a){"string"==typeof a?"PERMISSION_DENIED"===a&&c.open("https://addons.mozilla.org/en-US/firefox/addon/ff_screenshare/").focus():"object"==typeof a&&"PermissionDeniedError"===a.name&&c.open("https://addons.mozilla.org/en-US/firefox/addon/ff_screenshare/").focus()})):alert(virtualclass.lang.getString("notSupportBrowser",
...
"fixed");this.localCont.clearRect(0,0,this.localCanvas.width,this.localCanvas.height)},initializeRecorder:function(d){virtualclass.vutil.addClass("audioWidget","fixed");a=1;this.prevStream&&(this.ssByClick=!1);"undefined"!=typeof virtualclass.prevScreen&&virtualclass.prevScreen.hasOwnProperty("currentStream")&&virtualclass.prevScreen.unShareScreen();this.video=document.getElementById(this.local+"Video");if("VIDEO"!=this.video.tagName){var b=this.video,f=document.createElement("video");f.id=b.id;this.video.parentNode.replaceChild(f,
this.video);this.video=document.getElementById(this.local+"Video");this.video.autoplay=!0;virtualclass.vutil.createLocalTempVideo("virtualclassScreenShare",this.local+"Temp");virtualclass.vutil.initLocCanvasCont(this.local+"TempVideo")}this.currentStream=d;var g=this;virtualclass.adpt.attachMediaStream(this.video,d);this.prevStream=!0;this.currentStream.onended=function(a){g.ssByClick?(g.video.src="",g.localtempCont.clearRect(0,0,g.localtempCanvas.width,g.localtempCanvas.height),clearInterval(virtualclass.clear),
g.initPrevImage(),virtualclass.vutil.beforeSend({unshareScreen:!0,st:g.type,cf:"unshareScreen"}),g.prevStream=!1,g.prevScreen="",virtualclass.prevScreen=""):g.ssByClick=!0};d={};d.width=c.innerWidth;d.height=c.innerHeight-140;var b=document.getElementById(this.local),h=this.html.getDimension(d);b.style.width=Math.round(h.width)+"px";b.style.height=Math.round(h.height)+"px";g=this;this.video.onloadedmetadata=function(){g.width=h.width;g.height=h.height;g.localtempCanvas.width=g.video.offsetWidth;g.localtempCanvas.height=
...
[]}function g(){clearInterval(virtualclass.clear);if("undefined"!=typeof d&&"undefined"!=typeof f){if(d!=r.video.offsetWidth||f!=r.video.offsetHeight)a=1}else d=r.video.offsetWidth,f=r.video.offsetHeight;1==a?setTimeout(m,2E3):t(r.type)}function h(a){switch(!0){case 300>=a||"undefined"==typeof a:u=300;break;case 3E3>=a:u=a;break;case 5E3>=a:u=3E3;break;default:u=5E3}}function m(){console.log("RESIZE");d=r.video.offsetWidth;f=r.video.offsetHeight;w=Math.round(r.localtempCanvas.height/12);q=Math.round(r.localtempCanvas.width/
12);var t=r.type;r.localtempCanvas.width=r.video.offsetWidth;r.localtempCanvas.height=r.video.offsetHeight;r.localtempCont.drawImage(r.video,0,0,r.video.offsetWidth,r.video.offsetHeight);var x=r.localtempCont.getImageData(0,0,r.localtempCanvas.width,r.localtempCanvas.height),x=r.dc.encodeRGB(x.data),H=Math.round(r.localtempCanvas.width/q),S=Math.round(r.localtempCanvas.height/w),E,O,N,W;W=r.getContainerDimension();"undefined"!=typeof d&&"undefined"!=typeof f?(E=b(d,4),O=b(f,4)):(E=b(r.video.offsetWidth,
4),O=b(r.video.offsetHeight,4));N=b(W.width,4);W=b(W.height,4);t=new Uint8ClampedArray(["ss"==t?104:204,E[0],E[1],O[0],O[1],N[0],N[1],W[0],W[1]]);E=new Uint8ClampedArray(x.length+t.length);E.set(t);E.set(x,t.length);c.Worker&&sworker.postMessage({img:x,resize:1,resA:w,resB:q,dw:H,dh:S,offsetWidth:r.video.offsetWidth,offsetHeight:r.video.offsetHeight},[x.buffer]);ioAdapter.sendBinary(E);h(E.length/128);a=0;clearInterval(virtualclass.clear);virtualclass.clear=setInterval(g,u)}function t(a){r.localtempCanvas.width=
r.video.offsetWidth;r.localtempCanvas.height=r.video.offsetHeight;r.localtempCont.drawImage(r.video,0,0,r.video.offsetWidth,r.video.offsetHeight);a=Math.round(r.localtempCanvas.width/q);var d=Math.round(r.localtempCanvas.height/w),b=r.localtempCont.getImageData(0,0,r.video.offsetWidth,r.video.offsetHeight);c.Worker&&(sworker.postMessage({img:b.data,resA:w,resB:q,dw:a,dh:d,offsetWidth:r.video.offsetWidth,offsetHeight:r.video.offsetHeight,type:r.type},[b.data.buffer]),sworker.onmessage=function(a){if(1==
a.data.needFullScreen){a=virtualclass.getDataFullScreen(r.type);ioAdapter.sendBinary(a);var d=a.length/128}else null!=a.data.masterSlice&&(ioAdapter.sendBinary(a.data.masterSlice),d=a.data.masterSlice.length/128);h(d);clearInterval(virtualclass.clear);virtualclass.clear=setInterval(g,u)});clearInterval(virtualclass.clear)}var w=Math.round(this.localtempCanvas.height/12),q=Math.round(this.localtempCanvas.width/12),r=this;virtualclass.hasOwnProperty("wholeImage")&&clearInterval(virtualclass.wholeImage);
virtualclass.hasOwnProperty("clear")&&clearInterval(virtualclass.clear);var u=1E3;virtualclass.getDataFullScreen=function(a){r.localtempCanvas.width=r.video.offsetWidth;r.localtempCanvas.height=r.video.offsetHeight;r.localtempCont.drawImage(r.video,0,0,r.video.offsetWidth,r.video.offsetHeight);var d=r.localtempCont.getImageData(0,0,r.localtempCanvas.width,r.localtempCanvas.height),d=r.dc.encodeRGB(d.data),c=b(r.localtempCanvas.height,4),f=b(r.localtempCanvas.width,4);a=new Uint8ClampedArray(["ss"==
a?102:202,f[0],f[1],c[0],c[1]]);c=new Uint8ClampedArray(d.length+a.length);c.set(a);c.set(d,a.length);return c};clearInterval(virtualclass.clear);virtualclass.clear=setInterval(g,u)},getContainerDimension:function(){var a=document.getElementById(this.id+"Local");return{width:a.offsetWidth,height:a.offsetHeight}},drawImages:function(a,d){if("undefined"!=typeof d){var b=this.dc.decodeRGBSlice(a,this.localCont,d);this.localCont.putImageData(b,d.x,d.y)}else b=this.dc.decodeRGB(a,this.localCont,this.localCanvas),
this.localCont.putImageData(b,0,0)},drawSingleImage:function(a){a=this.dc.decodeRGB(a,this.localCont,this.localCanvas);this.localCont.putImageData(a,0,0)},dimensionStudentScreen:function(a,d){this.localCanvas=document.getElementById(virtualclass[g].local+"Video");this.localCont=virtualclass[g].localCanvas.getContext("2d");this.localCanvas.width=a;this.localCanvas.height=d;console.log("normal width "+this.localCanvas.width)},dimensionStudentScreenResize:function(a,d){this.hasOwnProperty("vac")||(this.vac=
!0,this.localCanvas=document.getElementById(virtualclass[g].local+"Video"),this.localCont=virtualclass[g].localCanvas.getContext("2d"));a.hasOwnProperty("d")&&(this.localCont.clearRect(0,0,this.localCanvas.width,this.localCanvas.height),this.localCanvas.width=a.d.w,this.localCanvas.height=a.d.h);if(a.hasOwnProperty("vc")){var b=document.getElementById(virtualclass[g].local);b.style.width=a.vc.w+"px";b.style.height=a.vc.h+"px"}"virtualclassScreenShare"==virtualclass.previous&&virtualclass.vutil.setScreenInnerTagsWidth(virtualclass.previous)},
html:{UI:function(a){a=virtualclass.vutil.createDOM("div",this.id,[this.className]);var d=virtualclass.vutil.createDOM("div",this.local,[this.label]);if(roles.hasControls()){if(virtualclass.recorder.recImgPlay)var b=virtualclass.vutil.createDOM("canvas",this.local+"Video");else b=virtualclass.vutil.createDOM("video",this.local+"Video"),b.setAttribute("autoplay",!0);d.style.cssText+=";position:relative"}else b=virtualclass.vutil.createDOM("canvas",this.local+"Video");d.appendChild(b);a.appendChild(d);
roles.hasControls()&&!virtualclass.recorder.recImgPlay&&virtualclass.vutil.createLocalTempVideo(a,this.localTemp);return a},getDimension:function(a,d){return{height:a.height,width:a.width}}},initPrevImage:function(){sworker.postMessage({initPrevImg:!0})}}}})(window);(function(c){function a(a){document.body.removeChild(a.target)}var b,d={},f=1,g=1,h={items:[],recImgPlay:!1,objn:0,playTimeout:"",totalSent:0,fileQueue:[],rnum:1,storeDone:0,emn:0,allFileFound:!1,waitServer:!1,waitPopup:!1,tempRecData:[],alreadyAskForPlay:!1,playStart:!1,error:0,mkDownloadLink:"",tillPlayTime:0,getPlayTotTime:!1,importfilepath:c.importfilepath,exportfilepath:c.exportfilepath,sessionKey:randomString(11),alreadyDownload:!1,init:function(a){var d=virtualclass.wb.vcan;if("undefined"!=
...
0,"progressBar","progressValue");virtualclass.storage.config.endSession()},playProgressBar:function(){this.getPlayTotTime||(this.totPlayTime=this.items.reduce(function(a,d){return d.hasOwnProperty("playTime")?{playTime:a.playTime+d.playTime}:a.playTime}),this.getPlayTotTime=!0);if(0<this.tillPlayTime){this.tillPlayTime>this.totPlayTime&&(this.tillPlayTime=this.totPlayTime);virtualclass.pbar.renderProgressBar(this.totPlayTime,this.tillPlayTime,"playProgressBar",void 0);var a=this.convertIntoReadable(this.tillPlayTime);
document.getElementById("tillRepTime").innerHTML=a.m+" : "+a.s;"undefined"==typeof alreadyCalcTotTime&&(a=this.convertIntoReadable(this.totPlayTime),document.getElementById("totalRepTime").innerHTML=a.m+" : "+a.s,alreadyCalcTotTime=!0)}else virtualclass.pbar.renderProgressBar(this.totPlayTime,0,"playProgressBar",void 0)},convertIntoReadable:function(a){a/=1E3;return{s:Math.floor(a%60),m:Math.floor(a/60)}},xhrsenddata:function(a,d,b){"undefined"!=typeof d&&(virtualclass.recorder.error=1);"undefined"!=
typeof b&&(virtualclass.recorder.mkDownloadLink=b);if(1!=virtualclass.recorder.storeDone){"undefined"!=typeof earlierTimeout&&clearTimeout(earlierTimeout);if("undefined"==typeof a||""==a)a="first";virtualclass.recorder.rnum=a;virtualclass.storage.getrowData(["chunkData"],function(a,d){if("string"==typeof a||"undefined"==typeof a)earlierTimeout=setTimeout(function(){virtualclass.recorder.xhrsenddata(virtualclass.recorder.rnum)},1E3);else if(a.hasOwnProperty("status")&&"done"==a.status)console.log("should invoked download function on "),
virtualclass.recorder.storeDone=1,console.log("From here actuall recorder finished"),"undefined"!=typeof virtualclass.recorder.mkDownloadLink&&""!=virtualclass.recorder.mkDownloadLink?virtualclass.recorder.mkDownloadLink():setTimeout(function(){virtualclass.recorder.afterRecording()},1E3);else if("undefined"!=typeof d&&(virtualclass.recorder.rnum=d),1==virtualclass.recorder.error)0==virtualclass.recorder.storeDone&&(virtualclass.recorder.xhrsenddata(virtualclass.recorder.rnum,"error"),virtualclass.recorder.rnum++);
else{var b=new FormData;b.append("record_data",JSON.stringify(a));b.append("user",virtualclass.gObj.uid);b.append("cn",g);b.append("sesseionkey",virtualclass.recorder.sessionKey);virtualclass.recorder.items=[];virtualclass.xhr.send(b,importfilepath,function(d){"done"===d?(virtualclass.pbar.renderProgressBar(a.totalStore,a.totalSent,"progressBar","progressValue"),virtualclass.recorder.rnum++,g++,virtualclass.recorder.xhrsenddata(virtualclass.recorder.rnum)):"ERROR"===d?virtualclass.recorder.alreadyDownload||
virtualclass.recorder.makeAvailDownloadFile():alert(d)})}},virtualclass.recorder.rnum)}},afterRecording:function(){virtualclass.clearSession();document.getElementById("progressContainer").style.display="none";var a=document.getElementById("recordFinishedMessageBox");a.style.display="block";a.classList.add("MessageBoxFinished");var d=document.getElementById("recordingContainer");d.classList.add("recordingFinished");var b=document.getElementById("recordingClose");virtualclass.recorder.alreadyDownload=
!0;b.addEventListener("click",function(){virtualclass.popup.closeElem();var b=document.getElementById("progressContainer");a.style.display="none";b.style.display="block";virtualclass.pbar.renderProgressBar(0,0,"progressBar","progressValue");virtualclass.pbar.renderProgressBar(0,0,"indProgressBar","indProgressValue");d.classList.remove("recordingFinished")})},makeAvailDownloadFile:function(){console.log("DOWNLLOAD MESSAGE");var d=document.getElementById("recordingContainer"),b=document.createElement("div");
b.id="downloadFileCont";var f=document.createElement("div");f.id="errormsessage";f.innerHTML=virtualclass.lang.getString("msgForDownloadStart");document.getElementById("progressContainer").style.display="none";b.appendChild(f);d.appendChild(b);this.alreadyDownload=!0;virtualclass.storage.getAllDataForDownload(["chunkData"],function(d){var g=document.createElement("button");g.id="downloadButton";g.innerHTML=virtualclass.lang.getString("downloadFile");var h=document.createElement("a");h.id="dlink";
h.href="";h.download="session.vcp";h.innerHTML=virtualclass.lang.getString("download");f.innerHTML=virtualclass.lang.getString("filenotsave");b.appendChild(g);document.getElementById("recordingHeader").innerHTML=virtualclass.lang.getString("downloadFile");var m=new Blob([d],{type:"application/virtualclass"});g.addEventListener("click",function(){null!=c.webkitURL?h.href=c.webkitURL.createObjectURL(m):(h.href=c.URL.createObjectURL(m),h.onclick=a,document.body.appendChild(h));h.click();virtualclass.storage.config.endSession()})})},
...
virtualclass.pbar.renderProgressBar(0,0,"downloadProgressBar","downloadProgressValue"),virtualclass.popup.waitBlock(),this.waitPopup=!0)},requestDataFromServer:function(a,d){this.displayWaitPopupIfNot(virtualclass.lang.getString("plswaitwhile"));var b=new FormData;b.append("prvfile",d);b.append("fileBundelId",a);virtualclass.xhr.send(b,exportfilepath,function(d){virtualclass.recorder.sendToWorker(d,a)})},sendToWorker:function(a,d){c.Worker&&(mvDataWorker.postMessage({rdata:a,getData:!0}),mvDataWorker.onmessage=
...
a);roles.hasAdmin()||this.toggleDisplayWriteModeMsgBox("EditorCode",a)},onmessage:function(a){this["received_"+a.message.control](a.message)},addCurrTeacherToControl:function(d){var b=a.getElementById(d);null!=b&&(virtualclass.vutil.elemHasAnyClass(d)?b.classList.add("currTeacher"):b.className="currTeacher")},removeCurrTeacherFromControl:function(d){var b=a.getElementById(d);virtualclass.vutil.elemHasAnyClass(d)&&(b.classList.remove("currTeacher"),b=d.indexOf("contr"),d=d.substring(0,b),virtualclass.user.control.updateUser(d,
"currTeacher",!1))},removeAudioFromParticipate:function(d){d=a.getElementById(d+"contrAssignCont");null!=d&&d.parentNode.removeChild(d)},disable:function(d,b,c,h){c=a.getElementById(d+"contr"+c+"Img");null!=c&&virtualclass.user.control._disable(c,b,d,h)},_disable:function(a,b,c,h){a.parentNode.setAttribute("data-title",virtualclass.lang.getString(b+"Disable"));a.setAttribute("data-"+b+"-disable","true");a.className="icon-"+b+"Img block "+b+"Img";"assign"==b?(a.parentNode.classList.remove("tooltip"),
...
a;d.appendChild(f);var v=this;a=function(){v.confirmInit(this.id,b,c)};f=document.createElement("div");f.id="confirmOk";f.className="confirmButton confirmChild";f.addEventListener("click",a);var x=document.createElement("button");x.id="confirmOkButton";x.className="icon-check";x.innerHTML="OK";f.appendChild(x);d.appendChild(f);f=document.createElement("div");f.id="confirmCancel";f.className="confirmButton confirmChild";f.addEventListener("click",a);a=document.createElement("button");a.id="confirmCancelButton";
...
if("object"==typeof this.player&&roles.hasControls())this.player.loadVideoById(b);else{var g=0;roles.hasControls()&&(g=1);var v={playerVars:{autohide:0,disablekb:1,enablejsapi:1,modestbranding:1,controls:g,rel:0,fs:0,showinfo:0,start:"undefined"!=typeof d?Math.round(d):0},videoId:b,events:{onReady:this.onPlayerReady}};"undefined"!=typeof d&&(v.start=d);console.log("Player object is CREATED");if("undefined"!=typeof f){var x=this;c.onYouTubeIframeAPIReady=function(){x.player=new YT.Player("player",
v)}}else this.player=new YT.Player("player",v);b=a.getElementById(this.UI.id);b.className+=" youTubeSharing"}},triggerOnSeekChange:function(){console.log("there should happend something after each 2 second");if(0==d||-2==f||-1==g)d=this.player.getCurrentTime(),f=this.player.getPlayerState(),g=this.player.isMuted();else{var a=Math.abs(this.player.getCurrentTime()-d);d=this.player.getCurrentTime();console.log(d);console.log(this.player.getCurrentTime());4<a&&this.ytOnSeek(this.player.getCurrentTime());
this.player.getPlayerState()!=f&&3!=this.player.getPlayerState()&&(this.ytOnChange(this.player.getPlayerState()),f=this.player.getPlayerState());this.player.isMuted()!=g&&(this.ytOnMuted(this.player.isMuted()),g=this.player.isMuted())}},ytOnMuted:function(a){a?ioAdapter.mustSend({yts:"mute",cf:"yts"}):ioAdapter.mustSend({yts:"unmute",cf:"yts"});console.log("MUTED "+a)},ytOnSeek:function(a){ioAdapter.mustSend({yts:{seekto:a},cf:"yts"});console.log("SEEK CHANGED "+a)},ytOnChange:function(a){console.log("STATE CHANGED "+
...
t+w+"]","g"),"");c.push(f)}b.update(b.from,b.to,c)}};a.prototype.onCodeMirrorChange_=function(a,b){if("object"===typeof b.from){for(var c=[];b;)c.push(b),b=b.next;b=c}for(var c=this.convertCoordinateSystemForChanges_(b),d=[],g=0;g<c.length;g++){var h=c[g],p=h.start,s=h.text,t=h.removed,v=h.origin;if(0<t.length){for(var x=this.annotationList_.getAnnotatedSpansForSpan(new m(p,t.length)),w=0,fb=0;fb<x.length;fb++){var La=x[fb];d.push({start:p,end:p+La.length,removedAttributes:La.annotation.attributes,
removed:t.substr(w,La.length),attributes:{},text:"",origin:h.origin});w+=La.length}this.annotationList_.removeSpan(new m(p,t.length))}0<s.length&&("+input"===h.origin||"paste"===h.origin?h=this.currentAttributes_||{}:v in this.outstandingChanges_?(h=this.outstandingChanges_[v].attributes,v=this.outstandingChanges_[v].origOrigin,delete this.outstandingChanges_[v]):h={},this.annotationList_.insertAnnotatedSpan(new m(p,s.length),new f(h)),d.push({start:p,end:p,removedAttributes:{},removed:"",text:s,
