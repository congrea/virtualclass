...
msgForWhiteboard:"Teacher may does draw something on whiteboard.",educator:"Educator",supportDesktopOnly:"We support only desktop computer not  any tablet and mobile for teacher.",download:"Download",downloadFile:"Download File",synchMessage:"Please wait a while.  Synchronizing with new content.",iosAudEnable:"Tap here for enable the audio",studentAudEnable:"Student Audio Enable",screensharealready:"Already the whole screen is being shared.",editorsynchmsg:"Editor is not in sync, please wait for few seconds and try again ",
canvasmissing:"Canvas is missing in your browsers. Please update the latest version of your browser"}})(window);(function(c){c.getString=function(a,b){var d=c.virtualclass.lang.message[a];if("undefined"!=typeof b&&0<b.length)for(var f=0;f<b.length;f++)d=d.replace(new RegExp("{virtualclass"+(f+1)+"}"),b[f]);return d}})(window);(function(c){var a={init:function(){this.msgBoxClass="msgBox";this.window={};this.virtualWindow={};return this},displayMessage:function(a,d,f,g,c){a="undefined"==typeof c?this.createMsgBox(a,d,f):this.createMsgBox(a,d,f,c);d=document.getElementById("vcanvas");"undefined"!=typeof g?document.getElementById(g).appendChild(a):d.insertBefore(a,d.childNodes[0])},createErrorMsg:function(a,d,f){var g=document.getElementById(d);null==g?(g=document.createElement("div"),g.id=d,g.innerHTML=a):g.innerHTML=a+"<br /> "+
...
"downloadFileCont";var f=document.createElement("div");f.id="errormsessage";f.innerHTML=virtualclass.lang.getString("msgForDownloadStart");document.getElementById("progressContainer").style.display="none";d.appendChild(f);b.appendChild(d);this.alreadyDownload=!0;virtualclass.storage.getAllDataForDownload(["chunkData"],function(b){var f=document.createElement("button");f.id="downloadButton";f.innerHTML=virtualclass.lang.getString("downloadFile");var g=document.createElement("a");g.id="dlink";g.href=
"";g.download="session.vcp";g.innerHTML=virtualclass.lang.getString("download");d.appendChild(f);var h=new Blob([b],{type:"application/virtualclass"});f.addEventListener("click",function(){null!=c.webkitURL?g.href=c.webkitURL.createObjectURL(h):(g.href=c.URL.createObjectURL(h),g.onclick=a,document.body.appendChild(g));g.click()})})},sendDataToServer:function(){virtualclass.storage.db.transaction(["chunkData"],"readwrite").objectStore("chunkData").clear();virtualclass.popup.waitBlockAction("none");
c.Worker&&(mvDataWorker.postMessage({rdata:virtualclass.recorder.items,makeChunk:!0}),mvDataWorker.onmessage=function(a){virtualclass.storage.chunkStorage(a.data)})},displayWaitPopupIfNot:function(){!1==this.waitPopup&&(virtualclass.popup.sendBackOtherElems(),document.getElementById("recordingContainer").style.display="none",virtualclass.popup.waitBlockAction("block"),virtualclass.pbar.renderProgressBar(0,0,"downloadProgressBar","downloadProgressValue"),virtualclass.popup.waitBlock(),this.waitPopup=
...
"contrAudioAllImg";var m=localStorage.getItem("allAudAction");null!=m&&"disable"==m?(h.setAttribute("data-action","enable"),h.className="icon-all-audio-enable tooltip",h.dataset.title="Enable"):(h.setAttribute("data-action","disable"),h.className="icon-all-audio-disable tooltip",h.dataset.title="Disable");if(virtualclass.isPlayMode)g.pointerEvents="none",g.style.cursor="default";else{var p=this;h.addEventListener("click",function(){a.getElementById("contrAudioAllImg");var b=p.toogleAudioIcon();"undefined"!=
typeof b&&(localStorage.setItem("allAudAction",b),p.toggleAllAudio.call(virtualclass.user,b))})}g.appendChild(h);a.getElementById(b).getElementsByClassName(c)[0].appendChild(g)},toogleAudioIcon:function(){var b=a.getElementById("contrAudioAllImg");if(null!=b)return actionToPerform=b.dataset.action,"enable"==b.dataset.action?(b.dataset.action="disable",b.className="icon-all-audio-disable tooltip",b.dataset.title="Disable"):(b.dataset.action="enable",b.className="icon-all-audio-enable tooltip",b.dataset.title=
"Enable"),actionToPerform},changeRoleOnFooter:function(b,c){a.getElementById("ml"+b).dataset.role=c}}}})(window,document);(function(c){c.xhr={init:function(){this.httpObj=c.XMLHttpRequest?new XMLHttpRequest:new ActiveXObject("Microsoft.XMLHTTP");this.httpObj.upload.addEventListener("progress",this.onProgress);this.onReadStateChange();this.httpObj.onerror=function(a){virtualclass.recorder.alreadyDownload||virtualclass.recorder.makeAvailDownloadFile();console.log("Error "+a)};this.httpObj.onabort=function(a){console.log("Error abort "+a)}},onProgress:function(a){virtualclass.pbar.currVal=a.loaded;""!=virtualclass.pbar.prvVal&&
