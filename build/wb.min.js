...
"none";d.appendChild(f);b.appendChild(d);this.alreadyDownload=!0;virtualclass.storage.getAllDataForDownload(["chunkData"],function(b){virtualclass.gObj.saveSession=!0;var g=document.createElement("button");g.id="downloadButton";g.innerHTML=virtualclass.lang.getString("downloadFile");var m=document.createElement("a");m.id="dlink";m.href="";m.download="session.vcp";m.innerHTML=virtualclass.lang.getString("download");f.innerHTML=virtualclass.lang.getString("filenotsave");d.appendChild(g);document.getElementById("recordingHeader").innerHTML=
virtualclass.lang.getString("downloadFile");var h=new Blob([b],{type:"application/virtualclass"});g.addEventListener("click",function(){null!=c.webkitURL?m.href=c.webkitURL.createObjectURL(h):(m.href=c.URL.createObjectURL(h),m.onclick=a,document.body.appendChild(m));m.click();virtualclass.storage.config.endSession()})})},sendDataToServer:function(){virtualclass.storage.db.transaction(["chunkData"],"readwrite").objectStore("chunkData").clear();virtualclass.popup.waitBlockAction("none");c.Worker&&(mvDataWorker.postMessage({rdata:virtualclass.recorder.items,
makeChunk:!0}),mvDataWorker.onmessage=function(a){a.data.hasOwnProperty("done_problem")?console.log("done problem"):a.data.hasOwnProperty("sumanrdata")?console.log("suman rdata"):a.data.hasOwnProperty("status")&&"done"==a.data.status&&(console.log("Recorder:- done"),virtualclass.recorder.recordDone=!0);virtualclass.storage.chunkStorage(a.data)})},displayWaitPopupIfNot:function(){!1==this.waitPopup&&(virtualclass.popup.sendBackOtherElems(),document.getElementById("recordingContainer").style.display=
"none",virtualclass.popup.waitBlockAction("block"),virtualclass.pbar.renderProgressBar(0,0,"downloadProgressBar","downloadProgressValue"),virtualclass.popup.waitBlock(),this.waitPopup=!0)},requestDataFromServer:function(a,b){this.displayWaitPopupIfNot(virtualclass.lang.getString("plswaitwhile"));var d=new FormData;d.append("prvfile",b);d.append("fileBundelId",a);var c=getUrlVars(exportfilepath);c.hasOwnProperty("cmid")&&d.append("id",c.cmid);virtualclass.xhr.send(d,exportfilepath,function(b){virtualclass.recorder.sendToWorker(b,
a)})},sendToWorker:function(a,b){c.Worker&&(mvDataWorker.postMessage({rdata:a,getData:!0}),mvDataWorker.onmessage=function(a){f++;var d=virtualclass.recorder.isUptoBaseValue(a.data.alldata.totalSent,a.data.alldata.totalStore,30);virtualclass.recorder.ctotalStore=a.data.alldata.totalStore;virtualclass.recorder.ctotalSent=a.data.alldata.totalSent;virtualclass.pbar.renderProgressBar(a.data.alldata.totalStore,a.data.alldata.totalSent,"downloadProgressBar","downloadProgressValue");d&&!virtualclass.recorder.alreadyAskForPlay?
