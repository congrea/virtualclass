...
navigator.getUserMedia||navigator.webkitGetUserMedia||navigator.mozGetUserMedia||navigator.msGetUserMedia;return navigator.getUserMedia?!0:!1},isIndexedDbSupport:function(){return d.indexedDB||d.webkitIndexedDB||d.mozIndexedDB||d.msIndexedDB?!0:!1},isWorkerSupport:function(){return d.Worker?!0:!1},isAudioApiSupport:function(){return d.AudioContext||d.webkitAudioContext?!0:!1},isTypedArraySupport:function(){return"ArrayBuffer"in d?!0:!1},isScreenShareSupport:function(){},setValue:function(a,c){this[a]=
c},checkBrowserFunctions:function(){this.setValue("canvas",this.isCanvasSupport());this.setValue("webSocket",this.isWebSocketSupport());this.setValue("getusermedia",this.isGetUserMediaSupport());this.setValue("indexeddb",this.isIndexedDbSupport());this.setValue("webworker",this.isWorkerSupport());this.setValue("webaudio",this.isAudioApiSupport());this.setValue("typedarray",this.isTypedArraySupport());this.setValue("screenshare",this.isScreenShareSupport());this.setValue("localstorage",this.isLocalStorageSupport())},
measureResoultion:function(a){var c=document.getElementById("vAppCont"),c=vcan.utility.getElementOffset(c),f=c.x;return{width:1024>a.width?1024-f:a.width-f,height:a.height-c.y}},setAppDimension:function(){var b=this.measureResoultion({width:d.innerWidth,height:d.innerHeight});vApp.vutil.setContainerWidth(b);"Whiteboard"==vApp.currApp&&a.setCanvasDimension(b)},setCanvasDimension:function(a){if("undefined"!=typeof vcan.main.canvas){var c=vcan.main.canvas;ctx=vcan.main.canvas.getContext("2d");c.width=
a.width;c.height=a.height-("t"==vApp.gObj.uRole?65:10);console.log("canvas width "+c.width);a=vcan.utility.getElementOffset(document.getElementById("canvas"));vcan.main.offset.x=a.x}},getResoultion:function(a){var c={};1280>a?(c.width=1024,c.height=768):1280<=a&&1366>a?(c.width=1280,c.height=1024):1366<=a&&1920>a?(c.width=1366,c.height=768):1920<=a&&(c.width=1920,c.height=1080);return c},reportBrowser:function(a){a=this.getErrors(a);if(1<a.length){alert("sss");debugger;a=a.joins(",");alert(a+" feature/s  is/are disabled on your browser")}},
getErrors:function(a){var c=[],f="canvas webSocket getusermedia webaudio indexeddb localstorage typedarray".split(" ");"t"==a&&f.push("webworker","screenshare");for(a=0;a<f.length;a++)this[f[a]]||c.push(vApp.lang.getString("err"+f[a]));return c},isiOSDevice:function(){return parseFloat((""+(/CPU.*OS ([0-9_]{1,5})|(CPU like).*AppleWebKit.*Mobile/i.exec(navigator.userAgent)||[0,""])[1]).replace("undefined","3_2").replace("_",".").replace("_",""))||!1},isAndroid:function(){return-1<navigator.userAgent.toLowerCase().indexOf("android")},
check:function(){var a=this.isiOSDevice();this.isAndroid();if(a)var c="safariMobile";else{var f=this.isAndroid(),a=this.mybrowser.detection(),c=a[0],a=parseFloat(a[1]);this.mybrowser.name=c;this.mybrowser.version=a}this.checkBrowserFunctions();"s"==vApp.gObj.uRole&&"undefined"!=typeof f&&f?"Chrome"==c?40<=a||vApp.error.push(c+" "+a+"  is not supported, we support chrome  versoin 40 or newer on Android"):vApp.error.push(c+"  is not supported, we support Chrome 40 or newer on andorid"):"Chrome"==c&&
40<=a||"Firefox"==c&&35<=a||"s"==vApp.gObj.uRole&&c=="Opera">a>=26?this.reportBrowser(vApp.gObj.uRole):"Chrome"==c&&40>a||"Firefox"==c&&35>a||"s"==vApp.gObj.uRole&&"Opera"==c&&26>a?vApp.error.push(c+" "+a+vApp.lang.getString("chFireBrowsersIssue")):"t"==vApp.gObj.uRole&&"Opera"==c&&26<=a?(this.reportBrowser(vApp.gObj.uRole),vApp.error.push(c+" "+a+vApp.lang.getString("operaBrowserIssue"))):"s"==vApp.gObj.uRole&&"Safari"==c&&8<=a?(this.reportBrowser(vApp.gObj.uRole),vApp.error.push(c+" "+vApp.lang.getString("studentSafariBrowserIssue"))):
"s"==vApp.gObj.uRole&&"safariMobile"==c&&7<=a||vApp.error.push(c+" "+a+" "+vApp.lang.getString("commonBrowserIssue"))}},a=a.init();d.addEventListener("resize",function(){d.earlierWidth!=d.innerWidth&&a.setAppDimension()});d.addEventListener("resize",function(){d.earlierWidth!=d.innerWidth&&vApp.wb.view.window.resize()});a.mybrowser.detection=function(){var a=navigator.userAgent,c,f=a.match(/(opera|chrome|safari|firefox|msie|trident(?=\/))\/?\s*([\d\.]+)/i)||[];if(/trident/i.test(f[1]))return c=/\brv[ :]+(\d+(\.\d+)?)/g.exec(a)||
[],"IE "+(c[1]||"");f=f[2]?[f[1],f[2]]:[navigator.appName,navigator.appVersion,"-?"];null!=(c=a.match(/version\/([\.\d]+)/i))&&(f[2]=c[1]);return f};d.system=a})(window);(function(d){d.packContainer=function(){return{createPacketContDiv:function(a,b){var c=document.createElement("div");"undefined"!=typeof a&&(c.id=a);"undefined"!=typeof b&&(c.clasName=b);return c},togglePacketCont:function(a){for(var b=this.className.split(" "),c=0;c<b.length;c++)if("display"==b[c]){this.className="hide";this.innerHTML="+";a._togglePacketCont(b[c]);break}else if("hide"==b[c]){this.className="display";this.innerHTML="-";a._togglePacketCont(b[c]);break}},_togglePacketCont:function(a){a=
...
[];for(var d=0,g=0;g<a.length;g++)f=a[g],f=G711.decode(f,{alaw:"alaw"==this.encMode?!0:!1,floating_point:!0,Eight:!0}),this.myaudioNodes.push(new Float32Array(f)),d+=f.length;f=this.mergeBuffers(this.myaudioNodes,d);"undefined"!=typeof c?vApp.gObj.video.audio.play(f,b,c):vApp.gObj.video.audio.play(f,b)},play:function(a,b,c){var f=JSON.parse(localStorage.getItem("vApp"+b)),d=this.Html5Audio.audioContext.createBuffer(1,a.length,8E3);d.getChannelData(0).set(a);a=this.Html5Audio.audioContext.createBufferSource();
a.buffer=d;a.buffer=d;d=this.Html5Audio.audioContext.createGain();d.gain.value=.9;a.connect(d);d.connect(this.Html5Audio.audioContext.destination);f.ad&&f.aud&&(vApp.user.control.iconAttrManupulate(b,"icon-audioEnaGreen"),document.getElementById(f.id+"contrAudAnch").setAttribute("data-title",vApp.lang.getString("audioOn")));a.onended=function(){f=JSON.parse(localStorage.getItem("vApp"+b));"undefined"==typeof c&&(clearTimeout(vApp.gObj[b].out),vApp.gObj[b].isplaying=!1,0<vApp.gObj.video.audio.audioToBePlay[b].length?
vApp.gObj.video.audio.getChunks(b):f.ad&&f.aud&&vApp.user.control.iconAttrManupulate(b,"icon-audioEnaOrange"))};a.start();console.log("stack length "+this.audioToBePlay[b].length+" UID "+b+" video Start  Duration :"+a.buffer.duration);vApp.gObj[b].isplaying=!0;"undefined"==typeof c&&(vApp.gObj[b].out=setTimeout(function(){vApp.gObj[b].isplaying=!1;0<vApp.gObj.video.audio.audioToBePlay[b].length&&vApp.gObj.video.audio.getChunks(b)},1E3*a.buffer.duration+10))},calcAverage:function(){var a=new Uint8Array(analyser.frequencyBinCount);
analyser.getByteFrequencyData(a);for(var b=0,c=a.length,f=0;f<c;f++)b+=a[f];this.graph.average=b/c},audioInGraph:function(){var a=cthis.video;if("t"==vApp.gObj.uRole){var b=this.graph.height-this.graph.height*this.graph.average/100;a.tempVidCont.beginPath();a.tempVidCont.moveTo(a.tempVid.width-this.graph.width,this.graph.height);a.tempVidCont.lineTo(a.tempVid.width-this.graph.width,b);a.tempVidCont.lineWidth=this.graph.width;a.tempVidCont.strokeStyle="rgba(247, 25, 77, 1)";a.tempVidCont.closePath();
a.tempVidCont.stroke()}},queue:function(a,b){this.hasOwnProperty("audioToBePlay")||(this.audioToBePlay={});this.audioToBePlay.hasOwnProperty(b)||(this.audioToBePlay[b]=[]);this.audioToBePlay[b].push(a)},getChunks:function(a,b){8<this.audioToBePlay[a].length?(this.audioToBePlay[a].length=0,vApp.gObj[a].isplaying=!1):0<this.audioToBePlay[a].length?(this.playRecordedAudio(this.audioToBePlay[a],a),this.audioToBePlay[a].length=0):vApp.gObj[a].isplaying=!1},replayInit:function(){vApp.storage.getAllObjs(["audioData"],
function(){vApp.gObj.video.audio.replay(0,0)})},replay:function(a,b){repMode=!0;var c,f,d;0<this.audioNodes.length&&(c=this.mergeBuffers(this.audioNodes),f=this.Html5Audio.audioContext.currentTime+a,d=this.Html5Audio.audioContext.createBuffer(1,c.length,7800),d.getChannelData(0).set(c),c=this.Html5Audio.audioContext.createBufferSource(),c.buffer=d,c.connect(this.Html5Audio.audioContext.destination),c.start(f,b))},mergeBuffers:function(a,b){for(var c=new Float32Array(b),f=0,d=a.length,g=0;g<d;g++){var h=
a[g];c.set(h,f);f+=h.length}return c},assignFromLocal:function(a,b){this.init();for(var c=0;c<a.length;c++){var f=LZString.decompressFromBase64(a[c]),f=this.str2ab(f);samples=G711.decode(f,{alaw:"alaw"==this.encMode?!0:!1,floating_point:!0,Eight:!0});this.audioNodes.push(new Float32Array(samples));this.recordingLength+=16384}"undefined"!=typeof b&&b()},manuPulateStream:function(){var a=cthis.stream;vApp.vutil.chkValueInLocalStorage("recordStart")?vApp.wb.recordStarted=localStorage.getItem("recordStart"):
(vApp.wb.recordStarted=(new Date).getTime(),localStorage.setItem("recordStart",vApp.wb.recordStarted));a=cthis.audio.Html5Audio.audioContext.createMediaStreamSource(a);cthis.audio.bufferSize=16384;grec=cthis.audio.Html5Audio.audioContext.createScriptProcessor(cthis.audio.bufferSize,1,1);grec.onaudioprocess=cthis.audio.recorderProcess.bind(cthis.audio);var b=cthis.audio.Html5Audio.audioContext.createGain();b.gain.value=.9;a.connect(b);b.connect(grec);grec.connect(cthis.audio.Html5Audio.audioContext.destination)},
updateInfo:function(){this.audioStreamArr=[];vApp.wb.pageEnteredTime=vApp.wb.recordStarted=(new Date).getTime();repMode=this.recordAudio=!1},receivedAudioProcess:function(a){a=this.extractData(a);var b=a[0];if("undefined"==typeof c)var c={};c.hasOwnProperty(b)||(c[b]={},c[b].ad=!0,c=vApp.user.control.updateUser(b,"ad",!0),vApp.user.control.audioSign(c,"create"));vApp.gObj.video.audio.queue(a[1],b);vApp.gObj.hasOwnProperty(b)&&vApp.gObj[b].hasOwnProperty("isplaying")?!1==vApp.gObj[b].isplaying&&vApp.gObj.video.audio.getChunks(b):
(vApp.gObj[b]={},vApp.gObj[b].isplaying=!0,vApp.gObj.video.audio.getChunks(b))},extractData:function(a){a=new Uint8ClampedArray(a);return[vApp.vutil.numValidateFour(a[1],a[2],a[3],a[4]),a.subarray(5,a.length)]}},video:{width:75,height:56,tempVid:"",tempVidCont:"",myVideo:"",remoteVid:"",remoteVidCont:"",maxHeight:250,init:function(){this.videoCont=document.getElementById("allVideosCont");null!=this.videoCont&&(this.videoCont.style.maxHeight=this.maxHeight+"px")},calcDimension:function(){this.myVideo.width=
...
b)},updateHightInSideBar:function(a){var b=document.getElementById("widgetRightSide").offsetHeight;document.getElementById("chatContainer").style.height=b-a+"px"},send:function(){function b(){function h(){try{f.tempVidCont.drawImage(f.myVideo,0,0,f.width,f.height)}catch(a){if("NS_ERROR_NOT_AVAILABLE"==a.name)setTimeout(h,100);else throw a;}}if("t"==vApp.gObj.uRole){if("undefined"==typeof k){var k=document.getElementById("graphCanvas");null!=k&&(cthis.audio.graph.cvCont=k.getContext("2d"))}null!=k&&
cthis.audio.graph.cvCont.clearRect(0,0,k.width,k.height)}f.tempVidCont.clearRect(0,0,f.tempVid.width,f.tempVid.height);d.navigator.userAgent.match("Firefox")?h():f.tempVidCont.drawImage(f.myVideo,0,0,f.width,f.height);"t"==vApp.gObj.uRole&&cthis.audio.graph.display();k={name:vApp.gObj.uName,id:vApp.gObj.uid};"t"==vApp.gObj.uRole&&(k.role=vApp.gObj.uRole);vApp.wb.utility.beforeSend({videoByImage:k});var k=f.tempVidCont.getImageData(0,0,f.tempVid.width,f.tempVid.height),k=vApp.dirtyCorner.encodeRGB(k.data),
s=c(vApp.gObj.uid,8),s=new Uint8ClampedArray([11,s[0],s[1],s[2],s[3]]),v=new Uint8ClampedArray(k.length+s.length);v.set(s);v.set(k,s.length);a.sendBinary(v);clearInterval(vApp.gObj.video.smallVid);k=2E3+2500*vApp.gObj.totalUser.length;g!=vApp.gObj.totalUser?(g=vApp.gObj.totalUser.length,s=vApp.gObj.totalUser.indexOf(vApp.gObj.uId),vApp.gObj.video.smallVid=setInterval(b,k+k/g*s)):vApp.gObj.video.smallVid=setInterval(b,k)}function c(a,b){for(var f=a.toString(),d=f.length;d<b;d++)f="0"+f;return f.match(/[\S]{1,2}/g)||
[]}vApp.gObj.video.hasOwnProperty("smallVid")&&clearInterval(vApp.gObj.video.smallVid);var f=this;randomTime=Math.floor(10001*Math.random())+5E3;var g=-1;vApp.gObj.video.smallVid=setInterval(b,300)},startToStream:function(){cthis.video.calcDimension();cthis.video.send()},playWithoutSlice:function(a,b){this.remoteVid=document.getElementById("video"+a);this.remoteVidCont=this.remoteVid.getContext("2d");var c=vApp.dirtyCorner.decodeRGB(b,this.remoteVidCont,this.remoteVid);this.remoteVidCont.putImageData(c,
...
document.getElementById(a);if(null!=b)for(var b=b.getElementsByTagName("video"),c=0;c<b.length;c++)b[c].play()},1040)},existVideoContainer:function(a){for(var b=document.getElementsByClassName("userVideos"),c=0;c<b.length;c++)if(b[c].id=="video"+a.id)return!0;return!1},handleUserMediaError:function(a){vApp.user.control.audioWidgetDisable();a.hasOwnProperty("name")?alert("media error:- "+vApp.lang.getString(a.name)):alert("media error:- "+vApp.lang.getString(a));vApp.wb.view.disappearBox("WebRtc");
