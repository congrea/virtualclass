...
typeof vcan.main.canvas){var d=vcan.main.canvas;ctx=vcan.main.canvas.getContext("2d");d.width=a.width;var f=roles.hasControls()?65:10;d.height=a.height-f;console.log("canvas width "+d.width);a=vcan.utility.getElementOffset(document.getElementById("canvas"));vcan.main.offset.x=a.x}},getResoultion:function(a){var d={};1280>a?(d.width=1024,d.height=768):1280<=a&&1366>a?(d.width=1280,d.height=1024):1366<=a&&1920>a?(d.width=1366,d.height=768):1920<=a&&(d.width=1920,d.height=1080);return d},reportBrowser:function(a){a=
this.getErrors(a);1<a.length?virtualclass.error.push(a.join(",")+" are disabled in your browser."):1==a.length&&virtualclass.error.push(a+" is disabled in your browser.")},getErrors:function(a){var d=[],f="canvas webSocket getusermedia webaudio indexeddb localstorage typedarray".split(" ");"t"==a&&f.push("webworker","screenshare");for(a=0;a<f.length;a++)this[f[a]]||("screenshare"==f[a]&&(virtualclass.gObj.errNotScreenShare=!0),d.push(virtualclass.lang.getString("err"+f[a])));return d},isiOSDevice:function(){return parseFloat((""+
(/CPU.*OS ([0-9_]{1,5})|(CPU like).*AppleWebKit.*Mobile/i.exec(navigator.userAgent)||[0,""])[1]).replace("undefined","3_2").replace("_",".").replace("_",""))||!1},isAndroid:function(){return-1<navigator.userAgent.toLowerCase().indexOf("android")},check:function(){var a=this.isiOSDevice();this.device="desktop";if(a){var d="iOS";this.device="mobTab"}else var f=this.isAndroid(),a=this.mybrowser.detection(),d=a[0],a=parseFloat(a[1]);this.mybrowser.name=d;this.mybrowser.version=a;this.checkBrowserFunctions(d,
a);"undefined"!=typeof f&&f?(this.device="mobTab",roles.hasControls()?(virtualclass.vutil.initDisableVirtualClass(),virtualclass.error.push("We support only desktop computer not  any tablet and mobile for teacher.")):f&&("Chrome"==d?40<=a?virtualclass.vutil.initDisableAudVid():(virtualclass.error.push(d+" "+a+"  is not supported, we support chrome  versoin 40 or newer on Android"),virtualclass.vutil.initDisableVirtualClass()):(virtualclass.error.push(d+"  is not supported, we support Chrome 40 or newer on andorid"),
virtualclass.vutil.initDisableVirtualClass()))):"Chrome"==d&&40<=a||"Firefox"==d&&35<=a||roles.isStudent()&&d=="OPR">a>=26?this.reportBrowser(virtualclass.gObj.uRole):"Chrome"==d&&40>a||"Firefox"==d&&35>a||roles.isStudent()&&"OPR"==d&&26>a?(this.reportBrowser(virtualclass.gObj.uRole),virtualclass.error.push(virtualclass.lang.getString("chFireBrowsersIssue",[d,a]))):"OPR"==d&&26<=a?(this.reportBrowser(virtualclass.gObj.uRole),roles.hasControls()&&virtualclass.error.push(virtualclass.lang.getString("operaBrowserIssue",
...
roles.isStudent()?8<=a?(virtualclass.vutil.initDisableAudVid(),virtualclass.gObj.iosIpadbAudTrue=!1,d=document.createElement("div"),d.innerHTML="Tap here for enable the audio",d.id="iosAudioTrigger",d.addEventListener("click",function(){virtualclass.vutil.firstiOSaudioCall();this.parentNode.removeChild(this)}),f=document.getElementById("audioWidget"),f.parentNode.insertBefore(d,f.nextSibling)):(virtualclass.vutil.initDisableVirtualClass(),virtualclass.error.push(virtualclass.lang.getString("ios7support"))):
...
d="virtualclass"+virtualclass.vutil.capitalizeFirstLetter(d),c=document.getElementById(d),g=5,h;if("Whiteboard"==b)h=0;else{h=document.getElementById("virtualclassOptionsCont");if(null!=h){var m=vcan.utility.getElementOffset(h);h=h.offsetWidth+m.x}else h=0;"Yts"==b?g=75:"EditorRich"==virtualclass.currApp||"EditorCode"==virtualclass.currApp?0<h?g=12:h=roles.hasControls()?70:5:g="ScreenShare"==b?70:65}var m=70,p=document.getElementById("commandToolsWrapper");null!=p&&(m=m+p.clientHeight+3);a.width-=
g+h+0;c.style.width=a.width+"px";c.style.height=a.height-m+"px";"virtualclassScreenShare"==d&&(g=document.getElementById(d+"Local"),a.width-=10,c.style.width=a.width,g.style.width=a.width+"px",virtualclass.vutil.setScreenInnerTagsWidth(d))},setScreenInnerTagsWidth:function(a){document.getElementById(a);document.getElementById(a+"Local");document.getElementById(a+"LocalVideo")},makeActiveApp:function(a,b){a!=b&&"undefined"!=typeof b&&(b+="Tool",document.getElementById(b).className=virtualclass.vutil.removeClassFromElement(b,
"active"));document.getElementById(a+"Tool").className+=" active"},initInstallChromeExt:function(a){"EXTENSION_UNAVAILABLE"==a.name&&(console.log("ask for inline installation"),chrome.webstore.install("https://chrome.google.com/webstore/detail/ijhofagnokdeoghaohcekchijfeffbjl",function(a){c.location.reload()},function(a){alert(a)}))},removeAppPanel:function(){var a=document.getElementById("virtualclassOptionsCont");null!=a&&a.parentNode.removeChild(a)},removeTempVideo:function(a){a=document.getElementById(a);
...
if("undefined"!=typeof a){if(roles.isEducator()){var b=document.getElementById(virtualclass.gObj.commandToolsWrapperId);b.parentNode.removeChild(b);virtualclass.vutil.removeClass("virtualclassCont","reclaim")}else virtualclass.vutil.addClass("virtualclassCont","assign"),virtualclass.vutil.removeClass("virtualclassCont","removedAssign");roles.hasAdmin()?virtualclass.gObj.uRole="t":virtualclass.gObj.uRole="p";localStorage.setItem("uRole",virtualclass.gObj.uRole);virtualclass.user.assignRole(virtualclass.gObj.uRole,
...
clearInterval(virtualclass.yts.tsc));roles.hasAdmin()?virtualclass.gObj.uRole="e":virtualclass.gObj.uRole="s";b=document.getElementById(virtualclass.gObj.commandToolsWrapperId);if(null!=b)for(;b.hasChildNodes();)b.removeChild(b.lastChild);else b=virtualclass.vutil.createCommandWrapper();"object"==typeof virtualclass.wb&&virtualclass.wb.utility.makeCanvasDisable();roles.hasAdmin()?(document.getElementById("virtualclassCont"),virtualclass.vutil.createReclaimButton(b),virtualclass.vutil.addClass("virtualclassCont",
"reclaim")):(virtualclass.vutil.removeClass("virtualclassCont","assign"),virtualclass.vutil.addClass("virtualclassCont","removedAssign"),null!=b&&b.parentNode.removeChild(b));localStorage.setItem("uRole",virtualclass.gObj.uRole);"object"==typeof virtualclass.wb&&virtualclass.wb.utility.uniqueArrOfObjsToStudent()}roles.hasAdmin()||(io.disconnect(),setTimeout(function(){virtualclass.uInfo.userobj.role=virtualclass.gObj.uRole;io.init(virtualclass.uInfo)},500))},createCommandWrapper:function(){var a=
document.createElement("div");a.id=virtualclass.gObj.commandToolsWrapperId;var b=document.getElementById(vcan.canvasWrapperId);null!=b?document.getElementById("containerWb").insertBefore(a,b):document.getElementById("containerWb").appendChild(a);return a},_reclaimRole:function(){this.reclaimRole();virtualclass.vutil.beforeSend({reclaimRole:!0,cf:"reclaimRole"});virtualclass.user.control.changeAttrToAssign("enable")},reclaimRole:function(){virtualclass.gObj.controlAssign=!1;this.vcResponseAssignRole(virtualclass.gObj.uid,
virtualclass.gObj.uid,!0)},vcResponseAssignRole:function(a,b,d){if(a!=b||"undefined"!=typeof d)if(virtualclass.vutil.assignRole(b),"object"==typeof virtualclass.wb&&virtualclass.wb.utility.uniqueArrOfObjsToTeacher(),roles.hasAdmin())a=document.getElementById("chat_div").getElementsByClassName("currTeacher")[0],null!=a&&virtualclass.user.control.removeCurrTeacherFromControl(a.id),virtualclass.user.control.changeAttrToAssign("enable");else{virtualclass.vutil.removeSessionTool();b=document.getElementById("ml"+
a);d=["assign"];var c=document.getElementById(a+"ControlContainer");null!=c?virtualclass.user.createAssignControl(c,a,!0):(d=virtualclass.user.createControl(a,d),b.appendChild(d));localStorage.setItem("aId",a)}virtualclass.vutil.attachClickOutSideCanvas();"mobTab"==virtualclass.system.device&&(virtualclass.vutil.disableVirtualClass(),virtualclass.view.createErrorMsg(virtualclass.lang.getString("supportDesktop"),"errorContainer","chatWidget"))},vcResponseAReclaimRole:function(a,b){a!=b&&(virtualclass.user.control._assign(b,
"notsent",a),virtualclass.user.displayStudentSpeaker(!0),null!=localStorage.getItem("aId")&&localStorage.removeItem("aId"),virtualclass.user.control.removeAudioFromParticipate(a),"mobTab"==virtualclass.system.device&&((virtualclass.system.mybrowser.name=8<=virtualclass.system.mybrowser.version)&&/(iPad)/g.test(navigator.userAgent)&&virtualclass.vutil.enableVirtualClass(),virtualclass.view.removeErrorMsg("errorContainer",!0)))},createReclaimButtonIfNeed:function(){if(roles.isEducator()){var a=virtualclass.vutil.createCommandWrapper();
virtualclass.vutil.createReclaimButton(a);return!0}return!1},whoIsTeacher:function(){for(var a=0;a<virtualclass.connectedUsers.length;a++)if("t"==virtualclass.connectedUsers[a].role)return virtualclass.connectedUsers[a].userid;return 0},enablePresentatorEditors:function(a){a={toUser:a,status:!0};virtualclass.user.control.received_editorRich(a);virtualclass.user.control.received_editorCode(a)}}})(window);(function(c){var a,b,d,f,g,h;c.studentScreen=function(){return{ssProcess:function(a,b,d,c){var f=a[0];a=new Uint8ClampedArray(b);var g,h,r,s;switch(f){case 102:case 202:r=virtualclass.vutil.numValidateTwo(a[1],a[2]);h=virtualclass.vutil.numValidateTwo(a[3],a[4]);b=a.subarray(5,a.length);this.initStudentScreen(b,{w:r,h:h},d,c);break;case 103:case 203:b=7;for(r=0;r+7<=a.length;r=s+1)f=virtualclass.vutil.numValidateTwo(a[r+1],a[r+2]),g=virtualclass.vutil.numValidateTwo(a[r+3],a[r+4]),h=parseInt(a[r+
...
a.className="enable";a=a.getElementsByClassName("ui-chatbox-input-box")[0];null!=a&&(a.disabled=!1)},toggleAllEditorController:function(b,c){b=virtualclass.vutil.smallizeFirstLetter(b);var g=a.getElementsByClassName("controleCont");if(0<g.length)for(var h=0;h<g.length;h++)if(0<g[h].id.indexOf(b)){var m=g[h].id.indexOf("Cont");0<m&&(m=g[h].id.substr(0,m),m=a.getElementById(m+"Img"),this.control.init.call(this,m,c))}},toggleDisplayEditorController:function(b,c){b=virtualclass.vutil.smallizeFirstLetter(b);
for(var g=a.getElementsByClassName("controller"+b),h=0;h<g.length;h++)g[h].style.display=c},updateUser:function(a,b,c){var h=localStorage.getItem("virtualclass"+a),m={};null==h?m.id=a:m=JSON.parse(h);m[b]=c;localStorage["virtualclass"+m.id]=JSON.stringify(m);return m},audioSign2:function(b,f){if("create"==f){if(null==a.getElementById(b.id+"AudEnableSign")){var g=a.createElement("div");g.id=b.id+"AudEnableSign";var h=a.createElement("img");imgName="audioenable";h.id=b.id+imgName+"Img";h.src=c.whiteboardPath+
...
"icon-audioDisImg")},iconAttrManupulate:function(b,c){var g=a.getElementById(b+"contrAudImg");if(null!=g)for(var h=0;h<g.classList.length;h++)if("icon-"==g.classList[h].substring(0,5)){g.classList.remove(g.classList[h]);g.classList.add(c);break}},shouldApply:function(a){var b=localStorage.getItem("virtualclass"+a);null!=b&&(b=JSON.parse(b),b.ad&&virtualclass.user.control.audioSign({id:a},"create"))},changeAttrToAssign:function(b){for(var c=a.getElementById("chatWidget").getElementsByClassName("assignImg"),
g=0;g<c.length;g++)"enable"==b?(c[g].classList.remove("block"),c[g].classList.add("enable"),c[g].parentNode.classList.add("tooltip"),c[g].parentNode.setAttribute("data-title",virtualclass.lang.getString("assignEnable")),c[g].setAttribute("data-assign-disable","false")):(c[g].classList.remove("enable"),c[g].classList.add("block"),c[g].parentNode.classList.remove("tooltip"),c[g].setAttribute("data-assign-disable","true"))}},displayStudentSpeaker:function(b){var c=a.getElementById("alwaysPress");null!=
c&&(c.style.display=b?"block":"none")},toggleAllAudio:function(b){var c=a.getElementsByClassName("controleCont");if(0<c.length)for(var g=0;g<c.length;g++)if(0<c[g].id.indexOf("Aud")){var h=c[g].id.indexOf("Cont");0<h&&(h=c[g].id.substr(0,h),h=a.getElementById(h+"Img"),this.control.init.call(this,h,b))}},UIaudioAll:function(b,c){var g=a.createElement("a");g.id="contrAudioAll";var h=a.createElement("span");h.id="contrAudioAllImg";var m=localStorage.getItem("allAudAction");null!=m&&"disable"==m?(h.innerHTML=
"En Aud All",h.setAttribute("data-action","enable")):(h.innerHTML="Dis Aud All",h.setAttribute("data-action","disable"));var p=this;h.addEventListener("click",function(){a.getElementById("contrAudioAllImg");var b=p.toogleAudioIcon();"undefined"!=typeof b&&(localStorage.setItem("allAudAction",b),p.toggleAllAudio.call(virtualclass.user,b))});g.appendChild(h);a.getElementById(b).getElementsByClassName(c)[0].appendChild(g)},toogleAudioIcon:function(){var b=a.getElementById("contrAudioAllImg");if(null!=
b)return actionToPerform=b.dataset.action,"enable"==b.dataset.action?(b.dataset.action="disable",b.innerHTML="Dis Aud All"):(b.dataset.action="enable",b.innerHTML="En Aud All"),actionToPerform},changeRoleOnFooter:function(b,c){a.getElementById("ml"+b).dataset.role=c}}}})(window,document);(function(c){c.xhr={init:function(){this.httpObj=c.XMLHttpRequest?new XMLHttpRequest:new ActiveXObject("Microsoft.XMLHTTP");this.httpObj.upload.addEventListener("progress",this.onProgress);this.onReadStateChange();this.httpObj.onerror=function(a){virtualclass.recorder.alreadyDownload||virtualclass.recorder.makeAvailDownloadFile();console.log("Error "+a)};this.httpObj.onabort=function(a){console.log("Error abort "+a)}},onProgress:function(a){virtualclass.pbar.currVal=a.loaded;""!=virtualclass.pbar.prvVal&&
