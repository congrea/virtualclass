...
a.fillStyle=f.overlayFill:f.fill&&(a.fillStyle=f.fill);f.stroke&&(a.strokeStyle=f.stroke);a.beginPath();var m;m=c=0;for(var n=-(f.width/2),r=-(f.height/2),u=0,w=f.path.length;u<w;++u)switch(m=f.path[u],m[0]){case "L":c=m[1];m=m[2];a.lineTo(c+n,m+r);break;case "M":c=m[1],m=m[2],a.moveTo(c+n,m+r)}f.fill&&a.fill();f.stroke&&(a.strokeStyle=f.stroke,a.lineWidth=f.strokeWidth,a.lineCap=a.lineJoin="round",a.stroke());!g&&f.active&&(f.drawBorders(a),f.hideCorners||f.drawCorners(a));a.restore()},makeStackObj:function(a,
...
(virtualclass.vutil.initDisableVirtualClass(),virtualclass.error.push(virtualclass.lang.getString("teacherSafariBrowserIssue",[d,a]))):(virtualclass.vutil.initDisableAudVid(),virtualclass.error.push(virtualclass.lang.getString("studentSafariBrowserIssue",[d,a])),virtualclass.user.control.mediaWidgetDisable()):(virtualclass.vutil.initDisableVirtualClass(),virtualclass.error.push(virtualclass.lang.getString("safariBrowserIssue",[d,a]))):"iOS"==d?this.isIPad()?roles.isStudent()?8<=a?(virtualclass.vutil.initDisableAudVid(),
...
a.strokeRect(~~(-(c/2)-b)+.5,~~(-(n/2)-b)+.5,~~(c+d),~~(n+d));!this.hasRotatingPoint||this.hideCorners||this.lockRotation||(b=(this.flipY?n:-n)/2,a.beginPath(),a.moveTo(0,b),a.lineTo(0,b+(this.flipY?this.rotatingPointOffset:-this.rotatingPointOffset)),a.closePath(),a.stroke());a.restore();return this}};b.drawCorners=function(a){if(this.hasControls){var b=this.cornersize,d=b/2,c=this.padding,n=-(this.width/2),r=-(this.height/2),u=b/this.scaleX,w=b/this.scaleY,t=(c+d)/this.scaleY,x=(c+d)/this.scaleX,
q=(c+d-b)/this.scaleX,d=(c+d-b)/this.scaleY,c=this.height;a.save();a.fillStyle=this.cornerColor;a.fillRect(n-x,r-t,u,w);b=n+this.width-x;a.fillRect(b,r-t,u,w);a.fillRect(n-x,r+c+d,u,w);b=n+this.width+q;a.fillRect(b,r+c+d,u,w);b=n+this.width/2-x;a.fillRect(b,r-t,u,w);b=n+this.width/2-x;a.fillRect(b,r+c+d,u,w);b=n+this.width+q;a.fillRect(b,r+c/2-t,u,w);a.fillRect(n-x,r+c/2-t,u,w);this.hasRotatingPoint&&(b=n+this.width/2-x,n=this.flipY?r+c+this.rotatingPointOffset/this.scaleY-w/2:r-this.rotatingPointOffset/
...
this.contextTop.closePath();this.isCurrentlyDrawing=!1;var h=this.utility.min(this.freeDrawingXPoints);c=this.utility.min(this.freeDrawingYPoints);var m=this.utility.max(this.freeDrawingXPoints);g=this.utility.max(this.freeDrawingYPoints);var n=[],r,u,w,t=this.freeDrawingXPoints,x=this.freeDrawingYPoints,q=this.mdTime;n.push("M ",t[0]-h," ",x[0]-c," ",q[0]," ");for(var s=1;r=t[s],u=x[s],w=q[s];s++)n.push("L ",r-h," ",u-c," ",w," ");n=n.join("");if("M 0 0 L 0 0 "!==n)return r=this.freeDrawingXPoints[this.freeDrawingXPoints.length-
1],u=this.freeDrawingYPoints[this.freeDrawingYPoints.length-1],w=new a.Path(n,this),w.mp={},w.mp.x=r,w.mp.y=u,w.init(n),w=d.readyObject(w),w.coreObj.type="freeDrawing",w=w.coreObj,w.fill=null,w.stroke=this.freeDrawingColor,w.strokeWidth=this.freeDrawingLineWidth,w.minX=h,w.minY=c,this.utility.objAdd(w),d=a.utility.setVal(w,"x",h+(m-h)/2),d=a.utility.setVal(d,"y",c+(g-c)/2),d.setCoords(),"object"==typeof b&&this.contextTop.restore(),a.renderAll(this.contextTop),d},utility:{max:function(a,b){if(a&&
...
h="";if("undefined"==typeof c){c=g.getElementsByTagName("textarea");for(var m=0;m<c.length;m++)if(c[m].id==g.id+"textarea"){h=c[m].value;break}}else h=c;a.font="20px Times New Roman";var n=0,r=h.split(/\r?\n/),n=a.measureText(r[0]).width;c=13;for(m=1;m<r.length;m++){c+=13;var u=a.measureText(r[m]).width;u>n&&(n=u)}n/=2;m=(new Date).getTime();a={type:"text",text:h,x:b.measure.x+n,y:b.measure.y+c,fontSize:20,fontWidth:a.measureText(h).width,mp:{x:b.measure.x,y:b.measure.y}};!1==virtualclass.wb.obj.drawTextObj.muser&&
...
cthis.video.tempStream,cthis.audio.manuPulateStream(),cthis.audio.graph.canvasForVideo()),cthis.video.myVideo.onloadedmetadata=function(){cthis.video.startToStream()});userMedia=!0},updateVidContHeight:function(){console.log("Updating User");var a=document.getElementById("virtualclassCont"),a=vcan.utility.getElementOffset(a),a=c.innerHeight-(a.y+75),b=document.getElementById("chat_div");b.offsetHeight>=a&&(b.style.overflowY="scroll");b.style.maxHeight=a+"px"},close:function(){virtualclass.gObj.video.hasOwnProperty("smallVid")&&
clearInterval(virtualclass.gObj.video.smallVid)},dispAllVideo:function(a){setTimeout(function(){var b=document.getElementById(a);if(null!=b)for(var b=b.getElementsByTagName("video"),d=0;d<b.length;d++)b[d].play()},1040)},existVideoContainer:function(a){for(var b=document.getElementsByClassName("userVideos"),d=0;d<b.length;d++)if(b[d].id=="video"+a.id)return!0;return!1},handleUserMediaError:function(a){a="object"==typeof a?virtualclass.lang.getString(a.name):virtualclass.lang.getString(a);virtualclass.view.createErrorMsg(a,
"errorContainer","chatWidget");virtualclass.user.control.audioWidgetDisable("vd");virtualclass.view.disappearBox("WebRtc");localStorage.setItem("dvid",!0);console.log("navigator.getUserMedia error: ",a)}}}})(window);(function(c){c.sharePt=function(){return{pptUrl:"",state:{},urlValue:"",localStoragFlag:!1,eventsObj:[],autoSlideTime:0,viewFlag:0,controlFlag:0,autoSlideFlag:0,autoSlideResumed:0,studentPpt:0,startFromFlag:0,startFrom:0,init:function(a,b){this.autoSlideResumed=this.autoSlideTime=this.autoSlideFlag=0;"undefined"!=typeof virtualclass.previous&&"undefined"!=typeof b&&"byclick"==b&&roles.hasControls()&&virtualclass.vutil.beforeSend({ppt:!0,init:"makeAppReady",cf:"ppt"});var d=document.getElementById("virtualclassSharePresentation");
null!=d&&d.parentNode.removeChild(d);this.UI.container();d=document.getElementById("pptMessageLayout");if(roles.hasControls()){this.UI.createUrlContainer();var c=document.getElementById("urlcontainer");null!=d&&(d.style.display="none");c.style.display="block";virtualclass.sharePt.attachEvent("submitpurl","click",virtualclass.sharePt.initNewPpt)}else roles.hasView()&&(c=document.getElementById("urlcontainer"),this.UI.messageDisplay(),null!=d&&(d.style.display="block"),null!=c&&(c.style.display="none"));
this.findInStorage();virtualclass.sharePt.attachMessageEvent("message",virtualclass.sharePt.pptMessageEventHandler)},findInStorage:function(){var a=localStorage.getItem("prevApp");console.log("test pApp"+typeof a);console.log(a);null!=a&&(a=JSON.parse(a),"SharePresentation"==a.name&&(a=a.metaData,null!=a&&a.init&&(this.localStoragFlag=!0,this.pptUrl=JSON.parse(localStorage.getItem("pptUrl")),null!=a.startFrom&&(virtualclass.sharePt.stateLocalStorage=a.startFrom)),this.localStoragFlag&&this.fromLocalStorage(virtualclass.sharePt.state)));
virtualclass.previrtualclass="virtualclass"+virtualclass.currApp},fromLocalStorage:function(a){null==document.getElementById("pptiframe")&&this.UI.createIframe();document.getElementById("iframecontainer").style.display="block";a=document.getElementById("pptiframe");var b=0>this.pptUrl.search("postMessage")?this.pptUrl+"?postMessage=true&postMessageEvents=true":this.pptUrl;a.setAttribute("src",b);this.setAutoSlideConfig();a=document.getElementById("pptMessageLayout");null!=a&&(console.log("message block"),
a.style.display="none");a=document.getElementById(this.UI.id);a.classList.contains("pptSharing")||a.classList.add("pptSharing")},pptMessageEventHandler:function(a){if("SharePresentation"==virtualclass.currApp){var b=document.getElementById("pptiframe"),d=a.data,d="string"==typeof d?JSON.parse(a.data):a.data;"undefined"!=typeof d&&(d.hasOwnProperty("namespace")&&"reveal"==d.namespace&&(virtualclass.sharePt.state=d.state),"ready"==d.eventName&&(virtualclass.sharePt.localStoragFlag&&!virtualclass.sharePt.startFromFlag?
(a=virtualclass.sharePt.stateLocalStorage,b.contentWindow.postMessage(JSON.stringify({method:"slide",args:[a.indexh,a.indexv,a.indexf]}),"*"),virtualclass.sharePt.localStoragFlag=0):virtualclass.sharePt.startFromFlag&&b.contentWindow.postMessage(JSON.stringify({method:"slide",args:virtualclass.sharePt.startFrom}),"*"),setTimeout(function(){null!=b&&null!=b.contentWindow&&b.contentWindow.postMessage(JSON.stringify({method:"toggleAutoSlide"}),"*")},1500)),roles.hasView()?"ready"==d.eventName&&(setTimeout(function(){virtualclass.sharePt.removeControls()},
100),virtualclass.sharePt.studentPpt&&(virtualclass.sharePt.setSlideState(virtualclass.sharePt.studentPpt),virtualclass.sharePt.studentPpt=0)):roles.hasControls()&&(console.log(d.eventName),"undefined"!=typeof d.eventName&&(virtualclass.sharePt[d.eventName].call(virtualclass.sharePt,d),virtualclass.sharePt.state=d.state)))}else null!=b&&b.removeAttribute("src")},setAutoSlideConfig:function(){localStorage.getItem("autoSlideTime")&&(virtualclass.sharePt.autoSlideTime=JSON.parse(localStorage.getItem("autoSlideTime")),
virtualclass.sharePt.autoSlideFlag=JSON.parse(localStorage.getItem("autoSlideFlag")))},onmessage:function(a){a.hasOwnProperty("autoSlide")&&(this.autoSlideTime=a.autoSlide);"undefined"!=typeof virtualclass.sharePt&&(virtualclass.system.setAppDimension(virtualclass.currApp),this.onPptMsgReceive(a),a=document.getElementById(virtualclass.sharePt.UI.id),a.classList.contains("pptSharing")||a.classList.add("pptSharing"))},ready:function(a){console.log("teacher ready for ppt ");ioAdapter.mustSend({pptMsg:a,
cf:"ppt",cfpt:"setPEvent"})},slidechanged:function(a){this.autoSlideFlag&&this.autoSlideResumed&&!this.autoSlideTime&&this.calculateAutoSlideTime(a);ioAdapter.mustSend({pptMsg:a,cf:"ppt",autoSlide:this.autoSlideTime,cfpt:"setPEvent"})},calculateAutoSlideTime:function(a){this.eventsObj.push({name:a.eventName,time:Date.now()});2<=this.eventsObj.length&&("slidechanged"==this.eventsObj[0].name&&"slidechanged"==this.eventsObj[1].name?(this.autoSlideTime=this.eventsObj[1].time-this.eventsObj[0].time,console.log("auto slide time withoutpause"+
this.autoSlideTime)):"autoslideresumed"==this.eventsObj[0].name&&"slidechanged"==this.eventsObj[1].name&&(this.autoSlideTime=this.eventsObj[1].time-this.eventsObj[0].time,console.log("withpause"+this.autoSlideTime)),console.log(this.eventsObj),console.log(this.autoSlideTime))},autoslidepaused:function(a){this.autoSlideFlag=1;this.autoSlideResumed=0;console.log(this.pause);ioAdapter.mustSend({pptMsg:a,cf:"ppt",cfpt:"setPEvent"})},autoslideresumed:function(a){this.autoSlideResumed=this.autoSlideFlag=
1;this.eventsObj=[];this.eventsObj.push({name:a.eventName,time:Date.now()});ioAdapter.mustSend({pptMsg:a,cf:"ppt",cfpt:"setPEvent"})},fragmentshown:function(a){ioAdapter.mustSend({pptMsg:a,cf:"ppt",cfpt:"setPEvent"})},fragmenthidden:function(a){ioAdapter.mustSend({pptMsg:a,cf:"ppt",cfpt:"setPEvent"})},overviewshown:function(a){ioAdapter.mustSend({pptMsg:a,cf:"ppt",cfpt:"setPEvent"})},overviewhidden:function(a){ioAdapter.mustSend({pptMsg:a,cf:"ppt",cfpt:"setPEvent"})},paused:function(a){ioAdapter.mustSend({pptMsg:a,
cf:"ppt",cfpt:"setPEvent"})},resumed:function(a){ioAdapter.mustSend({pptMsg:a,cf:"ppt",cfpt:"setPEvent"})},removeControls:function(){setTimeout(function(){document.getElementById("pptiframe").contentWindow.postMessage(JSON.stringify({method:"configure",args:[{controls:!1,autoSlide:0,keyboard:!1,progress:!1,touch:!1}]}),"*")},100)},cfpt:{displayFrame:function(){this.displayFrame()},setUrl:function(a){virtualclass.sharePt.localStoragFlag=0;virtualclass.sharePt.stateLocalStorage={};virtualclass.sharePt.state=
{indexh:0,indexv:0,indexf:0};this.setSlideUrl(a);var b=document.getElementById("pptiframe");b.onload=function(){roles.hasView()&&null!=b.contentWindow&&"undefined"!=typeof a.pptMsg.state&&b.contentWindow.postMessage(JSON.stringify({method:"setState",args:[a.pptMsg.state]}),"*")}},setPEvent:function(a){null!=document.getElementById("pptiframe")&&virtualclass.sharePt.setSlideState(a.pptMsg);this.state=a.pptMsg.state}},onPptMsgReceive:function(a){if("undefined"!=typeof a.ppt&&"undefined"!=typeof a.ppt.startFrom){virtualclass.sharePt.startFromFlag=
1;var b=document.getElementById("pptiframe");null!=b&&(0>a.ppt.init.search("postMessage")?b.setAttribute("src",a.ppt.init+"?postMessage=true&postMessageEvents=true"):b.setAttribute("src",a.ppt.init),virtualclass.sharePt.startFrom=[a.ppt.startFrom.indexh,a.ppt.startFrom.indexv,a.ppt.startFrom.indexf])}if("undefined"!=typeof a.pptMsg){var b=document.getElementById("pptiframe"),d=document.getElementById("pptMessageLayout");null!=b&&(b.onload=function(){roles.hasView()&&null!=b.contentWindow&&"undefined"!=
typeof a.pptMsg.state&&b.contentWindow.postMessage(JSON.stringify({method:"setState",args:[a.pptMsg.state]}),"*")});null!=d&&(d.style.display="none");a.hasOwnProperty("cfpt")&&"undefined"!=typeof a.cfpt&&"undefined"!=typeof virtualclass.sharePt.cfpt[a.cfpt]&&virtualclass.sharePt.cfpt[a.cfpt].call(virtualclass.sharePt,a)}},displayFrame:function(){virtualclass.system.setAppDimension(virtualclass.currApp);var a=document.getElementById(virtualclass.sharePt.UI.id);a.classList.contains("pptSharing")||a.classList.add("pptSharing")},
setSlideUrl:function(a){virtualclass.sharePt.studentPpt=0;console.log("PPT:- invoke setSlideUrl");this.autoSlideTime=this.autoSlideFlag=this.viewFlag=0;var b=document.getElementById("pptiframe");null!=b&&b.parentNode.removeChild(b);console.log("create iframe from setslide url");this.UI.createIframe();document.getElementById("iframecontainer").style.display="block";b=document.getElementById("pptiframe");b.setAttribute("src",0>a.pptMsg.search("postMessage")?a.pptMsg+"?postMessage=true&postMessageEvents=true":
a.pptMsg);console.log("test url is set");this.pptUrl=a.pptMsg;virtualclass.sharePt.localStoragFlag=0;virtualclass.sharePt.startFromFlag=0},setSlideState:function(a){var b=document.getElementById("pptiframe").contentWindow,d=[a.state.indexh,a.state.indexv,a.state.indexf],c=[a.state];switch(a.eventName){case "ready":b.postMessage(JSON.stringify({method:"setState",args:c}),"*");break;case "slidechanged":virtualclass.sharePt.studentPpt=a;console.log("test PPt:-  changed");b.postMessage(JSON.stringify({method:"slide",
args:d}),"*");break;case "autoslidepaused":this.autoSlideFlag=1;b.postMessage(JSON.stringify({method:"toggleAutoSlide"}),"*");break;case "autoslideresumed":this.autoSlideFlag=1;b.postMessage(JSON.stringify({method:"toggleAutoSlide"}),"*");break;case "fragmentshown":b.postMessage(JSON.stringify({method:"slide",args:d}),"*");break;case "fragmenthidden":b.postMessage(JSON.stringify({method:"slide",args:d}),"*");break;case "overviewshown":b.postMessage(JSON.stringify({method:"setState",args:c}),"*");
break;case "overviewhidden":b.postMessage(JSON.stringify({method:"setState",args:c}),"*"),b.postMessage(JSON.stringify({method:"slide",args:d}),"*");default:console.log("There is no event is occured")}},toProtocolRelativeUrl:function(){var a=virtualclass.sharePt.urlValue;return 0<=a.indexOf("http")?a.replace(/http:\/\/|https:\/\//,"//"):"//"+a},validUrl:function(){return virtualclass.sharePt.isUrlip(virtualclass.sharePt.urlValue)?virtualclass.sharePt.cleanupUrl(virtualclass.sharePt.urlValue):0<virtualclass.sharePt.urlValue.search("<iframe")?
virtualclass.sharePt.extractUrl(virtualclass.sharePt.urlValue):virtualclass.sharePt.validURLWithDomain(virtualclass.sharePt.urlValue)?virtualclass.sharePt.completeUrl(virtualclass.sharePt.cleanupUrl(virtualclass.sharePt.urlValue)):!1},initNewPpt:function(){virtualclass.sharePt.urlValue=document.getElementById("presentationurl").value;virtualclass.sharePt.urlValue=virtualclass.sharePt.toProtocolRelativeUrl();var a=virtualclass.sharePt.validUrl();if(a){virtualclass.sharePt.autoSlideTime=0;virtualclass.sharePt.autoSlideFlag=
0;virtualclass.sharePt.localStoragFlag=0;virtualclass.sharePt.startFromFlag=0;virtualclass.sharePt.eventsObj=[];virtualclass.sharePt.state={indexh:0,indexv:0,indexf:0};virtualclass.sharePt.stateLocalStorage={};localStorage.removeItem("autoSlideTime");localStorage.removeItem("autoSlideFlag");null==document.getElementById("iframecontainer")&&(console.log("call of iframe creater from onclick event handler"),virtualclass.sharePt.UI.createIframe());var b=document.getElementById(virtualclass.sharePt.UI.id);
b.classList.contains("pptSharing")||b.classList.add("pptSharing");virtualclass.sharePt.setPptUrl(a);virtualclass.system.setAppDimension()}else alert("Invalid Url")},cleanupUrl:function(a){var b=a.search("#");0<b&&(a=a.slice(0,b));return a},setPptUrl:function(a){console.log("test+set ppt url function");document.getElementById("iframecontainer").style.display="block";var b=document.getElementById("pptiframe");console.log("completeurl"+a);virtualclass.sharePt.pptUrl=a+"?postMessage=true&postMessageEvents=true";
b.setAttribute("src",virtualclass.sharePt.pptUrl);ioAdapter.mustSend({pptMsg:"displayframe",cf:"ppt",user:"all",cfpt:"displayframe"});ioAdapter.mustSend({pptMsg:a,cf:"ppt",user:"all",cfpt:"setUrl"});b.style.display="visible"},validURLWithDomain:function(a){return/((http|https)?:\/\/)?([a-z\d\-]{1,63}\.)*[a-z\d\-]{1,255}\.[a-z]{2,6}\/{1,255}\s*/.test(a)},completeUrl:function(a){0<a.search("slides.com")&&0>a.search("embed")&&(a+="/embed");return a},extractUrl:function(a){if(0<a.search("src")){var b=
...
JSON.stringify(a.metaData.startFrom)),console.log("savinlocal"),localStorage.setItem("pptUrl",JSON.stringify(a.metaData.init)),localStorage.setItem("autoSlideTime",JSON.stringify(virtualclass.sharePt.autoSlideTime)),localStorage.setItem("autoSlideFlag",JSON.stringify(virtualclass.sharePt.autoSlideFlag))},initTeacherLayout:function(){var a=document.getElementById("pptiframe");if(null!=document.getElementById("pptiframe")){var b={controls:!0,keyboard:!0,progress:!0,touch:!0},d;virtualclass.sharePt.autoSlideTime&&
virtualclass.sharePt.autoSlideFlag?d=virtualclass.sharePt.autoSlideTime:virtualclass.sharePt.autoSlideFlag&&(d=6E3);"undefined"!=typeof d&&1E3<d&&(b.autoSlide=d);a.contentWindow.postMessage(JSON.stringify({method:"configure",args:[b]}),"*");a.contentWindow.postMessage(JSON.stringify({method:"toggleAutoSlide"}),"*")}a=document.getElementById("pptMessageLayout");null!=a&&(a.style.display="none");virtualclass.sharePt.UI.createUrlContainer();document.getElementById("urlcontainer").style.display="block";
virtualclass.sharePt.eventsObj=[]},initStudentLayout:function(){var a=document.getElementById("pptiframe");virtualclass.sharePt.eventsObj=[];virtualclass.sharePt.UI.messageDisplay();null!=document.getElementById("pptiframe")?(""!=a.src&&virtualclass.sharePt.hideElement("pptMessageLayout"),a.contentWindow.postMessage(JSON.stringify({method:"configure",args:[{controls:!1,autoSlide:0,autoSlideStoppable:!0,keyboard:!1,progress:!1,touch:!1}]}),"*")):virtualclass.sharePt.displayElement("pptMessageLayout");
virtualclass.sharePt.hideElement("urlcontainer")},hideElement:function(a){a=document.getElementById(a);null!=a&&(a.style.display="none")},displayElement:function(a){a=document.getElementById(a);null!=a&&(a.style.display="block")},attachEvent:function(a,b,d){var c=document.getElementById(a),c=document.getElementById(a);null!=c&&c.addEventListener(b,d)},attachMessageEvent:function(a,b){c.addEventListener(a,b)},UI:{id:"virtualclassSharePresentation","class":"virtualclass",container:function(){console.log("test create main  container with id virtualclassSharePresentation");
var a=document.getElementById(this.id);null!=a&&a.parentNode.removeChild(a);a=document.createElement("div");a.id=this.id;a.className=this["class"];var b=document.getElementById(virtualclass.rWidgetConfig.id);console.log(b.parentNode);b.parentNode.insertBefore(a,b)},createIframe:function(){var a=document.getElementById("iframecontainer");null!=a&&a.parentNode.removeChild(a);console.log("test create ppt  iframe");a=document.getElementById("virtualclassSharePresentation");if(null!=a){var b=document.createElement("div");
a.appendChild(b);b.style.display="none"}b=document.createElement("div");b.setAttribute("id","iframecontainer");b.style.height="100%";b.style.width="100%";a.insertBefore(b,a.firstChild);b.style.display="none";a=document.getElementById("iframecontainer");b=document.createElement("iframe");a.appendChild(b);b.setAttribute("id","pptiframe");b.style.height="100%";b.style.width="100%"},createUrlContainer:function(){var a=document.getElementById("virtualclassSharePresentation");if(null==document.getElementById("urlcontainer")){var b=
document.createElement("div");b.setAttribute("id","urlcontainer");a.appendChild(b);a=document.createElement("input");a.setAttribute("id","presentationurl");a.setAttribute("placeholder",virtualclass.lang.getString("ppturl"));b.appendChild(a);a=document.createElement("input");a.setAttribute("type","submit");a.setAttribute("id","submitpurl");a.setAttribute("value","submit");b.appendChild(a)}},messageDisplay:function(){if(null==document.getElementById("pptMessageLayout")){var a=document.getElementById("virtualclassSharePresentation"),
...
roles.hasView()&&(virtualclass.sharePt.eventsObj=[]),virtualclass.sharePt.attachEvent("submitpurl","click",virtualclass.sharePt.initNewPpt),virtualclass.sharePt.attachMessageEvent("message",virtualclass.sharePt.pptMessageEventHandler))}else{"Yts"==virtualclass.currApp&&(virtualclass.yts.UI.removeinputURL(),virtualclass.yts.hasOwnProperty("tsc")&&clearInterval(virtualclass.yts.tsc));"SharePresentation"==virtualclass.currApp&&(roles.hasControls()&&virtualclass.sharePt.initStudentLayout(),roles.hasView()&&
(virtualclass.sharePt.eventsObj=[],virtualclass.sharePt.hideElement("pptMessageLayout"),virtualclass.sharePt.UI.createUrlContainer(),virtualclass.sharePt.displayElement("urlcontainer")),virtualclass.sharePt.attachEvent("submitpurl","click",virtualclass.sharePt.initNewPpt),virtualclass.sharePt.attachMessageEvent("message",virtualclass.sharePt.pptMessageEventHandler));virtualclass.gObj.uRole=roles.hasAdmin()?"e":"s";b=document.getElementById(virtualclass.gObj.commandToolsWrapperId);if(null!=b)for(;b.hasChildNodes();)b.removeChild(b.lastChild);
...
virtualclass.hasOwnProperty("prevScreen")&&virtualclass.prevScreen.hasOwnProperty("currentStream")&&virtualclass.prevScreen.unShareScreen();d.addEventListener("click",function(){null!=c.webkitURL?m.href=c.webkitURL.createObjectURL(r):(m.href=c.URL.createObjectURL(r),m.onclick=a,document.body.appendChild(m));m.click();n=document.getElementById("recordingHeader");n.innerHTML=virtualclass.lang.getString("uploadsession");f.parentNode.removeChild(f);h.style.display="block";virtualclass.recorder.hasOwnProperty("dataCame")&&
...
var m=a.createElement("div");m.id=b+c+"Cont";m.className="controleCont";m.appendChild(h);return[m,g]},createAssignControl:function(b,c,g,h){var m=this,n=this.createControllerElement(c,"contrAssign"),r=n[0],u=n[1],n=a.getElementById(c+"ControlContainer");null!=n?0>=n.getElementsByClassName("controleCont").length?n.appendChild(r):n.insertBefore(r,n.firstChild):b.appendChild(r);virtualclass.user.control.changeAttribute(c,u,g,"assign","aRole");"undefined"!=typeof h&&(u.className+=" currTeacher");u.addEventListener("click",
function(){m.control.init.call(m,u)})},createControlDivs:function(a,b,c){var h=this,m=!1,n=localStorage.getItem("virtualclass"+b);if(null!=n&&(m=!0,n=JSON.parse(n),n.hasOwnProperty("currTeacher")))if(virtualclass.gObj[b+"currTeacher"]={},!0==n.currTeacher){var r=virtualclass.user.control.currTeacherAlready=!0;virtualclass.gObj[b+"currTeacher"].ct=!0}else virtualclass.gObj[b+"currTeacher"].ct=!1;var u=roles.isEducator()?!1:!0,w=virtualclass.vutil.userIsOrginalTeacher(b);virtualclass.vutil.isUserTeacher(b);
for(var t=0;t<c.length;t++)if("assign"==c[t]&&w)"undefined"!=typeof r?this.createAssignControl(a,b,u,r):this.createAssignControl(a,b,u);else if("audio"==c[t]){var x=this.createControllerElement(b,"contrAud"),q=x[0],s=x[1];a.appendChild(q);q=m&&n.hasOwnProperty("aud")?n.aud?!0:!1:!0;virtualclass.user.control.changeAttribute(b,s,q,"audio","aud");w&&s.addEventListener("click",function(){h.control.init.call(h,s)})}else if("chat"==c[t]){var x=this.createControllerElement(b,"contrChat"),q=x[0],v=x[1];a.appendChild(q);
w&&v.addEventListener("click",function(){h.control.init.call(h,v)});q=m&&n.hasOwnProperty("chat")?n.chat?!0:!1:!0;virtualclass.user.control.changeAttribute(b,v,q,"chat","chat")}else if(("editorRich"==c[t]||"editorCode"==c[t])&&roles.hasAdmin()){var B=m&&n.hasOwnProperty(c[t])?n[c[t]]?!0:!1:!1,x=this.createControllerElement(b,"contr"+c[t]),q=x[0],x=x[1];q.className+=" controller"+c[t];a.appendChild(q);virtualclass.currApp!=virtualclass.vutil.capitalizeFirstLetter(c[t])&&(q.style.display="none");virtualclass.user.control.changeAttribute(b,
...
"true"==document.getElementById("sessionEndMsgCont").dataset.displaying&&this.sesseionEndWindow()};return b}(window);(function(c){c.converter=function(){function a(a,b){for(var c=a.replace(/[^A-Za-z0-9\+\/]/g,""),h=c.length,m=b?Math.ceil((3*h+1>>2)/b)*b:3*h+1>>2,n=new Uint8Array(m),r,u=0,w=0,t=0;t<h;t++){r=t&3;var x=c.charCodeAt(t),u=u|(64<x&&91>x?x-65:96<x&&123>x?x-71:47<x&&58>x?x+4:43===x?62:47===x?63:0)<<6*(3-r);if(3===r||1===h-t){for(r=0;3>r&&w<m;r++,w++)n[w]=u>>>(16>>>r&24)&255;u=0}}return n}function b(a){return 26>a?a+65:52>a?a+71:62>a?a-4:62===a?43:63===a?47:65}return{base64EncArrInt:function(a){for(var b=
...
p))return 0;var f=0;if(p.widgets)for(var g=0;g<p.widgets.length;g++)p.widgets[g].height&&(f+=p.widgets[g].height);return c?f+(Math.ceil(p.text.length/d)||1)*b:f+b}}function g(a){var b=a.doc,c=f(a);b.iter(function(a){var G=c(a);G!=a.height&&ya(a,G)})}function h(a){a.display.wrapper.className=a.display.wrapper.className.replace(/\s*cm-s-\S+/g,"")+a.options.theme.replace(/(^|\s)\s*/g," cm-s-");Gb(a)}function m(a){n(a);la(a);setTimeout(function(){M(a)},20)}function n(a){var b=a.display.gutters,c=a.options.gutters;
...
a.display.scrollbarFiller);F(b,"mousedown",function(){a.state.focused&&setTimeout(function(){a.display.input.focus()},0)});b.setAttribute("cm-not-content","true")},function(b,c){"horizontal"==c?ib(a,b):Ib(a,b)},a);a.display.scrollbars.addClass&&Jb(a.display.wrapper,a.display.scrollbars.addClass)}function B(a,b){b||(b=x(a));var c=a.display.barWidth,d=a.display.barHeight;L(a,b);for(var p=0;4>p&&c!=a.display.barWidth||d!=a.display.barHeight;p++)c!=a.display.barWidth&&a.options.lineWrapping&&jb(a),L(a,
...
b.rest[d],b,c,!1)}function Qd(a,b,c,d,p){if(b.widgets){var f=Pa(c),g=0;for(b=b.widgets;g<b.length;++g){var h=b[g],m=D("div",[h.node],"CodeMirror-linewidget");h.handleMouseEvents||m.setAttribute("cm-ignore-events","true");var n=h,q=m,s=d;if(n.noHScroll){(c.alignable||(c.alignable=[])).push(q);var r=s.wrapperWidth;q.style.left=s.fixedPos+"px";n.coverGutter||(r-=s.gutterTotalWidth,q.style.paddingLeft=s.gutterTotalWidth+"px");q.style.width=r+"px"}n.coverGutter&&(q.style.zIndex=5,q.style.position="relative",
n.noHScroll||(q.style.marginLeft=-s.gutterTotalWidth+"px"));a.display.input.setUneditable(m);p&&h.above?f.insertBefore(m,c.gutter||c.text):f.appendChild(m);ga(h,"redraw")}}}function Zc(a){return z(a.line,a.ch)}function ic(a,b){return 0>J(a,b)?b:a}function jc(a,b){return 0>J(a,b)?a:b}function Rd(a){a.state.focused||(a.display.input.focus(),Tc(a))}function kc(a){return a.options.readOnly||a.doc.cantEdit}function lc(a,b,c,d,p){var f=a.doc;a.display.shift=!1;d||(d=f.sel);var g=a.state.pasteIncoming||
"paste"==p,h=f.splitLines(b),m=null;if(g&&1<d.ranges.length)if(oa&&oa.join("\n")==b){if(0==d.ranges.length%oa.length)for(var m=[],n=0;n<oa.length;n++)m.push(f.splitLines(oa[n]))}else h.length==d.ranges.length&&(m=nc(h,function(a){return[a]}));for(n=d.ranges.length-1;0<=n;n--){var q=d.ranges[n],s=q.from(),r=q.to();q.empty()&&(c&&0<c?s=z(s.line,s.ch-c):a.state.overwrite&&!g&&(r=z(r.line,Math.min(E(f,r.line).text.length,r.ch+P(h).length))));q=a.curOp.updateInput;s={from:s,to:r,text:m?m[n%m.length]:h,
origin:p||(g?"paste":a.state.cutIncoming?"cut":"+input")};mb(a.doc,s);ga(a,"inputRead",a,s)}b&&!g&&Sd(a,b);nb(a);a.curOp.updateInput=q;a.curOp.typing=!0;a.state.pasteIncoming=a.state.cutIncoming=!1}function Td(a,b){var c=a.clipboardData&&a.clipboardData.getData("text/plain");if(c)return a.preventDefault(),ma(b,function(){lc(b,c,0,null,"paste")}),!0}function Sd(a,b){if(a.options.electricChars&&a.options.smartIndent)for(var c=a.doc.sel,d=c.ranges.length-1;0<=d;d--){var p=c.ranges[d];if(!(100<p.head.ch||
...
b.getAttribute("cm-marker"),N;if(c)b=a.findMarks(z(d,0),z(p+1,0),f(+c)),b.length&&(N=b[0].find())&&(h+=Za(a.doc,N.from,N.to).join(n));else if("false"!=b.getAttribute("contenteditable")){for(N=0;N<b.childNodes.length;N++)g(b.childNodes[N]);/^(pre|div|p)$/i.test(b.nodeName)&&(m=!0)}}}else 3==b.nodeType&&(b=b.nodeValue)&&(m&&(h+=n,m=!1),h+=b)}for(var h="",m=!1,n=a.doc.lineSeparator();;){g(b);if(b==c)break;b=b.nextSibling}return h}function Ha(a,b){this.ranges=a;this.primIndex=b}function S(a,b){this.anchor=
a;this.head=b}function va(a,b){var c=a[b];a.sort(function(a,b){return J(a.from(),b.from())});b=fa(a,c);for(c=1;c<a.length;c++){var d=a[c],p=a[c-1];if(0<=J(p.to(),d.from())){var f=jc(p.from(),d.from()),g=ic(p.to(),d.to()),d=p.empty()?d.from()==d.head:p.from()==p.head;c<=b&&--b;a.splice(--c,2,new S(d?g:f,d?f:g))}}return new Ha(a,b)}function Ca(a,b){return new Ha([new S(a,b||a)],0)}function I(a,b){if(b.line<a.first)return z(a.first,0);var c=a.first+a.size-1;if(b.line>c)return z(c,E(a,c).text.length);
...
function dd(a,b,c,d){var p=a.sel.ranges.slice(0);p[b]=c;ba(a,va(p,a.sel.primIndex),d)}function vf(a,b){var c={ranges:b.ranges,update:function(b){this.ranges=[];for(var c=0;c<b.length;c++)this.ranges[c]=new S(I(a,b[c].anchor),I(a,b[c].head))}};Z(a,"beforeSelectionChange",a,c);a.cm&&Z(a.cm,"beforeSelectionChange",a.cm,c);return c.ranges!=b.ranges?va(c.ranges,c.ranges.length-1):b}function ae(a,b,c){var d=a.history.done,p=P(d);p&&p.ranges?(d[d.length-1]=b,rc(a,b,c)):ba(a,b,c)}function ba(a,b,c){rc(a,
...
function rc(a,b,c){if(pa(a,"beforeSelectionChange")||a.cm&&pa(a.cm,"beforeSelectionChange"))b=vf(a,b);var d=c&&c.bias||(0>J(b.primary().head,a.sel.primary().head)?-1:1);ce(a,de(a,b,d,!0));c&&!1===c.scroll||!a.cm||nb(a.cm)}function ce(a,b){b.equals(a.sel)||(a.sel=b,a.cm&&(a.cm.curOp.updateInput=a.cm.curOp.selectionChanged=!0,ee(a.cm)),ga(a,"cursorActivity",a))}function fe(a){ce(a,de(a,a.sel,null,!1),Da)}function de(a,b,c,d){for(var p,f=0;f<b.ranges.length;f++){var g=b.ranges[f],h=tc(a,g.anchor,c,d),
m=tc(a,g.head,c,d);if(p||h!=g.anchor||m!=g.head)p||(p=b.ranges.slice(0,f)),p[f]=new S(h,m)}return p?va(p,b.primIndex):b}function tc(a,b,c,d){var p=!1,f=b,g=c||1;a.cantEdit=!1;a:for(;;){var h=E(a,f.line);if(h.markedSpans)for(var m=0;m<h.markedSpans.length;++m){var n=h.markedSpans[m],q=n.marker;if((null==n.from||(q.inclusiveLeft?n.from<=f.ch:n.from<f.ch))&&(null==n.to||(q.inclusiveRight?n.to>=f.ch:n.to>f.ch))){if(d&&(Z(q,"beforeCursorEnter"),q.explicitlyCleared))if(h.markedSpans){--m;continue}else break;
if(q.atomic){m=q.find(0>g?-1:1);if(0==J(m,f)&&(m.ch+=g,0>m.ch?m=m.line>a.first?I(a,z(m.line-1)):null:m.ch>h.text.length&&(m=m.line<a.first+a.size-1?z(m.line+1,0):null),!m)){if(p){if(!d)return tc(a,b,c,!0);a.cantEdit=!0;return z(a.first,0)}p=!0;m=b;g=-g}f=m;continue a}}}return f}}function Lb(a){a.display.input.showSelection(a.display.input.prepareSelection())}function ge(a,b){for(var c=a.doc,d={},p=d.cursors=document.createDocumentFragment(),f=d.selection=document.createDocumentFragment(),g=0;g<c.sel.ranges.length;g++)if(!1!==
b||g!=c.sel.primIndex){var h=c.sel.ranges[g],m=h.empty();if(m||a.options.showCursorWhenSelecting){var n=a,q=p,s=Ia(n,h.head,"div",null,null,!n.options.singleCursorHeightPerLine),r=q.appendChild(D("div","\u00a0","CodeMirror-cursor"));r.style.left=s.left+"px";r.style.top=s.top+"px";r.style.height=Math.max(0,s.bottom-s.top)*n.options.cursorHeight+"px";s.other&&(n=q.appendChild(D("div","\u00a0","CodeMirror-cursor CodeMirror-secondarycursor")),n.style.display="",n.style.left=s.other.left+"px",n.style.top=
s.other.top+"px",n.style.height=.85*(s.other.bottom-s.other.top)+"px")}m||wf(a,h,f)}return d}function wf(a,b,c){function d(a,b,G,c){0>b&&(b=0);b=Math.round(b);c=Math.round(c);h.appendChild(D("div",null,"CodeMirror-selected","position: absolute; left: "+a+"px; top: "+b+"px; width: "+(null==G?q-a:G)+"px; height: "+(c-b)+"px"))}function p(b,c,N){var ca=E(g,b),p=ca.text.length,f,h;xf(ua(ca),c||0,null==N?p:N,function(g,m,ja){var s=uc(a,z(b,g),"div",ca,"left"),r,fc;g==m?(r=s,ja=fc=s.left):(r=uc(a,z(b,m-
1),"div",ca,"right"),"rtl"==ja&&(ja=s,s=r,r=ja),ja=s.left,fc=r.right);null==c&&0==g&&(ja=n);3<r.top-s.top&&(d(ja,s.top,null,s.bottom),ja=n,s.bottom<r.top&&d(ja,s.bottom,null,r.top));null==N&&m==p&&(fc=q);if(!f||s.top<f.top||s.top==f.top&&s.left<f.left)f=s;if(!h||r.bottom>h.bottom||r.bottom==h.bottom&&r.right>h.right)h=r;ja<n+1&&(ja=n);d(ja,r.top,fc-ja,r.bottom)});return{start:f,end:h}}var f=a.display,g=a.doc,h=document.createDocumentFragment(),m=he(a.display),n=m.left,q=Math.max(f.sizerWidth,La(a)-
...
hasHeights:!1}}function gd(a,b,c,d,p){b.before&&(c=-1);var f=c+(d||"");if(b.cache.hasOwnProperty(f))a=b.cache[f];else{b.rect||(b.rect=b.view.text.getBoundingClientRect());if(!b.hasHeights){var g=b.view,h=b.rect,m=a.options.lineWrapping,n=m&&La(a);if(!g.measure.heights||m&&g.measure.width!=n){var q=g.measure.heights=[];if(m)for(g.measure.width=n,g=g.text.firstChild.getClientRects(),m=0;m<g.length-1;m++){var n=g[m],s=g[m+1];2<Math.abs(n.bottom-s.bottom)&&q.push((n.bottom+s.top)/2-h.top)}q.push(h.bottom-
h.top)}b.hasHeights=!0}g=d;m=Zd(b.map,c,g);d=m.node;h=m.start;n=m.end;c=m.collapse;var r;if(3==d.nodeType){for(q=0;4>q;q++){for(;h&&Qb(b.line.text.charAt(m.coverStart+h));)--h;for(;m.coverStart+n<m.coverEnd&&Qb(b.line.text.charAt(m.coverStart+n));)++n;if(O&&9>V&&0==h&&n==m.coverEnd-m.coverStart)r=d.parentNode.getBoundingClientRect();else if(O&&a.options.lineWrapping){var v=$a(d,h,n).getClientRects();r=v.length?v["right"==g?v.length-1:0]:hd}else r=$a(d,h,n).getBoundingClientRect()||hd;if(r.left||r.right||
0==h)break;n=h;h-=1;c="right"}if(O&&11>V){if(v=window.screen&&null!=screen.logicalXDPI&&screen.logicalXDPI!=screen.deviceXDPI)null!=id?v=id:(q=qa(a.display.measure,D("span","x")),v=q.getBoundingClientRect(),q=$a(q,0,1).getBoundingClientRect(),v=id=1<Math.abs(v.left-q.left));v&&(v=screen.logicalXDPI/screen.deviceXDPI,q=screen.logicalYDPI/screen.deviceYDPI,r={left:r.left*v,right:r.right*v,top:r.top*q,bottom:r.bottom*q})}}else 0<h&&(c=g="right"),r=a.options.lineWrapping&&1<(v=d.getClientRects()).length?
v["right"==g?v.length-1:0]:d.getBoundingClientRect();!(O&&9>V)||h||r&&(r.left||r.right)||(r=(r=d.parentNode.getClientRects()[0])?{left:r.left,right:r.left+Fb(a.display),top:r.top,bottom:r.bottom}:hd);v=r.top-b.rect.top;d=r.bottom-b.rect.top;h=(v+d)/2;g=b.view.measure.heights;for(q=0;q<g.length-1&&!(h<g[q]);q++);c={left:("right"==c?r.right:r.left)-b.rect.left,right:("left"==c?r.left:r.right)-b.rect.left,top:q?g[q-1]:0,bottom:g[q]};r.left||r.right||(c.bogus=!0);a.options.singleCursorHeightPerLine||
...
a.left,top:b-a.top}}function uc(a,b,c,d,p){d||(d=E(a.doc,b.line));var f=d;b=b.ch;d=gd(a,vc(a,d),b,p);return jd(a,f,d,c)}function Ia(a,b,c,d,p,f){function g(b,N){var h=gd(a,p,b,N?"right":"left",f);N?h.left=h.right:h.right=h.left;return jd(a,d,h,c)}function h(a,b){var G=m[b],c=G.level%2;a==kd(G)&&b&&G.level<m[b-1].level?(G=m[--b],a=ld(G)-(G.level%2?0:1),c=!0):a==ld(G)&&b<m.length-1&&G.level<m[b+1].level&&(G=m[++b],a=kd(G)-G.level%2,c=!1);return c&&a==G.to&&a>G.from?g(a-1):g(a,c)}d=d||E(a.doc,b.line);
p||(p=vc(a,d));var m=ua(d);b=b.ch;if(!m)return g(b);var n=oc(m,b),n=h(b,n);null!=Sb&&(n.other=h(b,Sb));return n}function ne(a,b){var c=0;b=I(a.doc,b);a.options.lineWrapping||(c=Fb(a.display)*b.ch);var d=E(a.doc,b.line),p=Aa(d)+a.display.lineSpace.offsetTop;return{left:c,right:c,top:p,bottom:p+d.height}}function wc(a,b,c,d){a=z(a,b);a.xRel=d;c&&(a.outside=!0);return a}function md(a,b,c){var d=a.doc;c+=a.display.viewOffset;if(0>c)return wc(d.first,0,!0,-1);var p=Xa(d,c),f=d.first+d.size-1;if(p>f)return wc(d.first+
d.size-1,E(d,f).text.length,!0,1);0>b&&(b=0);for(d=E(d,p);;)if(p=Bf(a,d,p,b,c),f=(d=Wa(d,!1))&&d.find(0,!0),d&&(p.ch>f.from.ch||p.ch==f.from.ch&&0<p.xRel))p=Q(d=f.to.line);else return p}function Bf(a,b,c,d,p){function f(d){d=Ia(a,z(c,d),"line",b,n);h=!0;if(g>d.bottom)return d.left-m;if(g<d.top)return d.left+m;h=!1;return d.left}var g=p-Aa(b),h=!1,m=2*a.display.wrapper.clientWidth,n=vc(a,b),q=ua(b),r=b.text.length;p=xc(b);var s=yc(b),v=f(p),u=h,t=f(s),w=h;if(d>t)return wc(c,s,w,1);for(;;){if(q?s==
p||s==nd(b,p,1):1>=s-p){q=d<v||d-v<=t-d?p:s;for(d-=q==p?v:t;Qb(b.text.charAt(q));)++q;return wc(c,q,q==p?u:w,-1>d?-1:1<d?1:0)}var x=Math.ceil(r/2),B=p+x;if(q)for(var B=p,Y=0;Y<x;++Y)B=nd(b,B,1);Y=f(B);if(Y>d){s=B;t=Y;if(w=h)t+=1E3;r=x}else p=B,v=Y,u=h,r-=x}}function Ta(a){if(null!=a.cachedTextHeight)return a.cachedTextHeight;if(null==ab){ab=D("pre");for(var b=0;49>b;++b)ab.appendChild(document.createTextNode("x")),ab.appendChild(D("br"));ab.appendChild(document.createTextNode("x"))}qa(a.measure,ab);
...
null==p.scrollTop||d.scroller.scrollTop==p.scrollTop&&!p.forceScroll||(f.scrollTop=Math.max(0,Math.min(d.scroller.scrollHeight-d.scroller.clientHeight,p.scrollTop)),d.scrollbars.setScrollTop(f.scrollTop),d.scroller.scrollTop=f.scrollTop);null==p.scrollLeft||d.scroller.scrollLeft==p.scrollLeft&&!p.forceScroll||(f.scrollLeft=Math.max(0,Math.min(d.scroller.scrollWidth-La(c),p.scrollLeft)),d.scrollbars.setScrollLeft(f.scrollLeft),d.scroller.scrollLeft=f.scrollLeft,M(c));if(p.scrollToPos){var h=void 0,
g=I(f,p.scrollToPos.from),h=I(f,p.scrollToPos.to),m=p.scrollToPos.margin;null==m&&(m=0);for(var n=0;5>n;n++){var q=!1,r=Ia(c,g),s=h&&h!=g?Ia(c,h):r,s=zc(c,Math.min(r.left,s.left),Math.min(r.top,s.top)-m,Math.max(r.left,s.left),Math.max(r.bottom,s.bottom)+m),v=c.doc.scrollTop,u=c.doc.scrollLeft;null!=s.scrollTop&&(Ib(c,s.scrollTop),1<Math.abs(c.doc.scrollTop-v)&&(q=!0));null!=s.scrollLeft&&(ib(c,s.scrollLeft),1<Math.abs(c.doc.scrollLeft-u)&&(q=!0));if(!q)break}h=r;p.scrollToPos.isCursor&&c.state.focused&&
...
b.left-a.left,d=b.top-a.top;return 400<G*G+d*d}var d=a.display;F(d.scroller,"mousedown",X(a,oe));O&&11>V?F(d.scroller,"dblclick",X(a,function(b){if(!Fa(a,b)){var c=sb(a,b);!c||od(a,b,"gutterClick",!0,ga)||Ka(a.display,b)||(ia(b),b=a.findWordAt(c),qc(a.doc,b.anchor,b.head))}})):F(d.scroller,"dblclick",function(b){Fa(a,b)||ia(b)});pd||F(d.scroller,"contextmenu",function(b){pe(a,b)});var p,f={end:0};F(d.scroller,"touchstart",function(a){var b;1!=a.touches.length?b=!1:(b=a.touches[0],b=1>=b.radiusX&&
...
setTimeout(Cb(Rd,a),0):a.curOp.focus=Ba();var d=+new Date,p;Bc&&Bc.time>d-400&&0==J(Bc.pos,c)?p="triple":Cc&&Cc.time>d-400&&0==J(Cc.pos,c)?(p="double",Bc={time:d,pos:c}):(p="single",Cc={time:d,pos:c});var d=a.doc.sel,f=ta?b.metaKey:b.ctrlKey,g;a.options.dragDrop&&If&&!kc(a)&&"single"==p&&-1<(g=d.contains(c))&&(0>J((g=d.ranges[g]).from(),c)||0<c.xRel)&&(0<J(g.to(),c)||0>c.xRel)?Jf(a,b,c,f):Kf(a,b,c,p,f)}function Jf(a,b,c,d){var p=a.display,f=+new Date,g=X(a,function(h){aa&&(p.scroller.draggable=!1);
a.state.draggingText=!1;Ga(document,"mouseup",g);Ga(p.scroller,"drop",g);10>Math.abs(b.clientX-h.clientX)+Math.abs(b.clientY-h.clientY)&&(ia(h),!d&&+new Date-200<f&&qc(a.doc,c),aa||O&&9==V?setTimeout(function(){document.body.focus();p.input.focus()},20):p.input.focus())});aa&&(p.scroller.draggable=!0);a.state.draggingText=g;p.scroller.dragDrop&&p.scroller.dragDrop();F(document,"mouseup",g);F(p.scroller,"drop",g)}function Kf(a,b,c,d,p){function f(b){if(0!=J(t,b))if(t=b,"rect"==d){for(var N=[],p=a.options.tabSize,
g=wa(E(n,c.line).text,c.ch,p),h=wa(E(n,b.line).text,b.ch,p),m=Math.min(g,h),g=Math.max(g,h),h=Math.min(c.line,b.line),ja=Math.min(a.lastLine(),Math.max(c.line,b.line));h<=ja;h++){var v=E(n,h).text,u=xe(v,m,p);m==g?N.push(new S(z(h,u),z(h,u))):v.length>u&&N.push(new S(z(h,u),z(h,xe(v,g,p))))}N.length||N.push(new S(c,c));ba(n,va(s.ranges.slice(0,r).concat(N),r),{origin:"*mouse",scroll:!1});a.scrollIntoView(b)}else N=q,p=N.anchor,m=b,"single"!=d&&(b="double"==d?a.findWordAt(b):new S(z(b.line,0),I(n,
z(b.line+1,0))),0<J(b.anchor,p)?(m=b.head,p=jc(N.from(),b.anchor)):(m=b.anchor,p=ic(N.to(),b.head))),N=s.ranges.slice(0),N[r]=new S(I(n,p),m),ba(n,va(N,r),rd)}function g(b){var c=++x,N=sb(a,b,!0,"rect"==d);if(N)if(0!=J(N,t)){a.curOp.focus=Ba();f(N);var p=K(m,n);(N.line>=p.to||N.line<p.from)&&setTimeout(X(a,function(){x==c&&g(b)}),150)}else{var ca=b.clientY<w.top?-20:b.clientY>w.bottom?20:0;ca&&setTimeout(X(a,function(){x==c&&(m.scroller.scrollTop+=ca,g(b))}),50)}}function h(a){x=Infinity;ia(a);m.input.focus();
Ga(document,"mousemove",B);Ga(document,"mouseup",Y);n.history.lastSelOrigin=null}var m=a.display,n=a.doc;ia(b);var q,r,s=n.sel,v=s.ranges;p&&!b.shiftKey?(r=n.sel.contains(c),q=-1<r?v[r]:new S(c,c)):(q=n.sel.primary(),r=n.sel.primIndex);if(b.altKey)d="rect",p||(q=new S(c,c)),c=sb(a,b,!0,!0),r=-1;else if("double"==d){var u=a.findWordAt(c);q=a.display.shift||n.extend?Ob(n,q,u.anchor,u.head):u}else"triple"==d?(u=new S(z(c.line,0),I(n,z(c.line+1,0))),q=a.display.shift||n.extend?Ob(n,q,u.anchor,u.head):
u):q=Ob(n,q,c);p?-1==r?(r=v.length,ba(n,va(v.concat([q]),r),{scroll:!1,origin:"*mouse"})):1<v.length&&v[r].empty()&&"single"==d&&!b.shiftKey?(ba(n,va(v.slice(0,r).concat(v.slice(r+1)),0)),s=n.sel):dd(n,r,q,rd):(r=0,ba(n,new Ha([q],0),rd),s=n.sel);var t=c,w=m.wrapper.getBoundingClientRect(),x=0,B=X(a,function(a){we(a)?g(a):h(a)}),Y=X(a,h);F(document,"mousemove",B);F(document,"mouseup",Y)}function od(a,b,c,d,p){try{var f=b.clientX,g=b.clientY}catch(h){return!1}if(f>=Math.floor(a.display.gutters.getBoundingClientRect().right))return!1;
...
g=0;a=function(a,d){var G=new FileReader;G.onload=X(b,function(){f[d]=G.result;if(++g==p){c=I(b.doc,c);var a={from:c,to:c,text:b.doc.splitLines(f.join(b.doc.lineSeparator())),origin:"paste"};mb(b.doc,a);ae(b.doc,Ca(c,bb(a)))}});G.readAsText(a)};for(var h=0;h<p;++h)a(d[h],h)}else if(b.state.draggingText&&-1<b.doc.sel.contains(c))b.state.draggingText(a),setTimeout(function(){b.display.input.focus()},20);else try{if(f=a.dataTransfer.getData("Text")){if(b.state.draggingText&&(ta?!a.altKey:!a.ctrlKey))var m=
...
"text");c=pa(a,"changes");if((d=pa(a,"change"))||c)b={from:g,to:h,text:b.text,removed:b.removed,origin:b.origin},d&&ga(a,"change",a,b),c&&(a.curOp.changeObjs||(a.curOp.changeObjs=[])).push(b);a.display.selForContextMenu=null}function tb(a,b,c,d,f){d||(d=c);if(0>J(d,c)){var g=d;d=c;c=g}"string"==typeof b&&(b=a.splitLines(b));mb(a,{from:c,to:d,text:b,origin:f})}function zc(a,b,c,d,f){var g=a.display,h=Ta(a.display);0>c&&(c=0);var m=a.curOp&&null!=a.curOp.scrollTop?a.curOp.scrollTop:g.scroller.scrollTop,
n=Yc(a),q={};f-c>n&&(f=c+n);var r=a.doc.height+Vc(g),s=c<h,h=f>r-h;c<m?q.scrollTop=s?0:c:f>m+n&&(c=Math.min(c,(h?r:f)-n),c!=m&&(q.scrollTop=c));m=a.curOp&&null!=a.curOp.scrollLeft?a.curOp.scrollLeft:g.scroller.scrollLeft;a=La(a)-(a.options.fixedGutter?g.gutters.offsetWidth:0);(g=d-b>a)&&(d=b+a);10>b?q.scrollLeft=0:b<m?q.scrollLeft=Math.max(0,b-(g?0:10)):d>a+m-3&&(q.scrollLeft=d+(g?0:10)-a);return q}function Ic(a,b,c){null==b&&null==c||Jc(a);null!=b&&(a.curOp.scrollLeft=(null==a.curOp.scrollLeft?a.doc.scrollLeft:
...
Math.max(c.bottom,d.bottom)+b.margin);a.scrollTo(b.scrollLeft,b.scrollTop)}}function Mb(a,b,c,d){var f=a.doc,g;null==c&&(c="add");"smart"==c&&(f.mode.indent?g=Pb(a,b):c="prev");var h=a.options.tabSize,m=E(f,b),n=wa(m.text,null,h);m.stateAfter&&(m.stateAfter=null);var q=m.text.match(/^\s*/)[0],r;if(!d&&!/\S/.test(m.text))r=0,c="not";else if("smart"==c&&(r=f.mode.indent(g,m.text.slice(q.length),m.text),r==ze||150<r)){if(!d)return;c="prev"}"prev"==c?r=b>f.first?wa(E(f,b-1).text,null,h):0:"add"==c?r=
n+a.options.indentUnit:"subtract"==c?r=n-a.options.indentUnit:"number"==typeof c&&(r=n+c);r=Math.max(0,r);c="";d=0;if(a.options.indentWithTabs)for(a=Math.floor(r/h);a;--a)d+=h,c+="\t";d<r&&(c+=Me(r-d));if(c!=q)return tb(f,c,z(b,0),z(b,q.length),"+input"),m.stateAfter=null,!0;for(a=0;a<f.sel.ranges.length;a++)if(h=f.sel.ranges[a],h.head.line==b&&h.head.ch<q.length){d=z(b,q.length);dd(f,a,new S(d,d));break}}function Kc(a,b,c,d){var f=b,g=b;"number"==typeof b?g=E(a,Math.max(a.first,Math.min(b,a.first+
a.size-1))):f=Q(b);if(null==f)return null;d(g,f)&&a.cm&&Ja(a.cm,f,c);return g}function ub(a,b){for(var c=a.doc.sel.ranges,d=[],f=0;f<c.length;f++){for(var g=b(c[f]);d.length&&0>=J(g.from,P(d).to);){var h=d.pop();if(0>J(h.from,g.from)){g.from=h.from;break}}d.push(g)}ma(a,function(){for(var b=d.length-1;0<=b;b--)tb(a.doc,"",d[b].from,d[b].to,"+delete");nb(a)})}function yd(a,b,c,d,f){function g(b){var d=(f?nd:Ne)(q,m,c,!0);if(null==d){if(b=!b)b=h+c,b<a.first||b>=a.first+a.size?b=r=!1:(h=b,b=q=E(a,b));
if(b)m=f?(0>c?yc:xc)(q):0>c?q.text.length:0;else return r=!1}else m=d;return!0}var h=b.line,m=b.ch,n=c,q=E(a,h),r=!0;if("char"==d)g();else if("column"==d)g(!0);else if("word"==d||"group"==d){var s=null;d="group"==d;b=a.cm&&a.cm.getHelper(b,"wordChars");for(var v=!0;!(0>c)||g(!v);v=!1){var u=q.text.charAt(m)||"\n",u=Lc(u,b)?"w":d&&"\n"==u?"n":!d||/\s/.test(u)?null:"p";!d||v||u||(u="s");if(s&&s!=u){0>c&&(c=1,g());break}u&&(s=u);if(0<c&&!g(!v))break}}n=tc(a,z(h,m),n,!0);r||(n.hitSide=!0);return n}function Oe(a,
b,c,d){var f=a.doc,g=b.left,h;"page"==d?(d=Math.min(a.display.wrapper.clientHeight,window.innerHeight||document.documentElement.clientHeight),h=b.top+c*(d-(0>c?1.5:.5)*Ta(a.display))):"line"==d&&(h=0<c?b.bottom+3:b.top-3);for(;;){var m=md(a,g,h);if(!m.outside)break;if(0>c?0>=h:h>=f.height){m.hitSide=!0;break}h+=5*c}return m}function H(a,b,d,f){c.defaults[a]=b;d&&(gb[a]=f?function(a,b,c){c!=Jd&&d(a,b,c)}:d)}function Tf(a){var b=a.split(/-(?!$)/);a=b[b.length-1];for(var c,d,f,g,h=0;h<b.length-1;h++){var m=
b[h];if(/^(cmd|meta|m)$/i.test(m))g=!0;else if(/^a(lt)?$/i.test(m))c=!0;else if(/^(c|ctrl|control)$/i.test(m))d=!0;else if(/^s(hift)$/i.test(m))f=!0;else throw Error("Unrecognized modifier name: "+m);}c&&(a="Alt-"+a);d&&(a="Ctrl-"+a);g&&(a="Cmd-"+a);f&&(a="Shift-"+a);return a}function Mc(a){return"string"==typeof a?Qa[a]:a}function vb(a,b,c,d,f){if(d&&d.shared)return Uf(a,b,c,d,f);if(a.cm&&!a.cm.curOp)return X(a.cm,vb)(a,b,c,d,f);var g=new db(a,f);f=J(b,c);d&&ra(d,g,!1);if(0<f||0==f&&!1!==g.clearWhenEmpty)return g;
...
d=Nb(a,b.to.line)&&E(a,b.to.line).markedSpans;if(!c&&!d)return null;var f=b.from.ch,g=b.to.ch,h=0==J(b.from,b.to);if(c)for(var m=0,n;m<c.length;++m){var q=c[m],r=q.marker;if(null==q.from||(r.inclusiveLeft?q.from<=f:q.from<f)||!(q.from!=f||"bookmark"!=r.type||h&&q.marker.insertLeft)){var s=null==q.to||(r.inclusiveRight?q.to>=f:q.to>f);(n||(n=[])).push(new Nc(r,q.from,s?null:q.to))}}c=n;if(d)for(var m=0,v;m<d.length;++m)if(n=d[m],q=n.marker,null==n.to||(q.inclusiveRight?n.to>=g:n.to>g)||n.from==g&&
"bookmark"==q.type&&(!h||n.marker.insertLeft))r=null==n.from||(q.inclusiveLeft?n.from<=g:n.from<g),(v||(v=[])).push(new Nc(q,r?null:n.from-g,null==n.to?null:n.to-g));d=v;h=1==b.text.length;v=P(b.text).length+(h?f:0);if(c)for(g=0;g<c.length;++g)if(m=c[g],null==m.to)(n=Vb(d,m.marker),n)?h&&(m.to=null==n.to?null:n.to+v):m.to=f;if(d)for(g=0;g<d.length;++g)m=d[g],null!=m.to&&(m.to+=v),null==m.from?(n=Vb(c,m.marker),n||(m.from=v,h&&(c||(c=[])).push(m))):(m.from+=v,h&&(c||(c=[])).push(m));c&&(c=Re(c));d&&
...
end:r.pos,string:r.current(),type:m||null,state:a?qb(g.mode,q):q}}var g=a.doc,h=g.mode,m;b=I(g,b);var n=E(g,b.line),q=Pb(a,b.line,c),r=new Qc(n.text,a.options.tabSize),s;for(d&&(s=[]);(d||r.pos<b.ch)&&!r.eol();)r.start=r.pos,m=Bd(h,r,q),d&&s.push(f(!0));return d?s:f()}function Ye(a,b,c,d,f,g,h){var m=c.flattenSpans;null==m&&(m=a.options.flattenSpans);var n=0,q=null,r=new Qc(b,a.options.tabSize),s,v=a.options.addModeClass&&[null];for(""==b&&Ve(We(c,d),g);!r.eol();){r.pos>a.options.maxHighlightLength?
(m=!1,h&&fd(a,b,d,r.pos),r.pos=b.length,s=null):s=Ve(Bd(c,r,d,v),g);if(v){var u=v[0].name;u&&(s="m-"+(s?u+" "+s:u))}if(!m||q!=s){for(;n<r.start;)n=Math.min(r.start,n+5E4),f(n,q);q=s}r.start=r.pos}for(;n<r.pos;)a=Math.min(r.pos,n+5E4),f(a,q),n=a}function ie(a,b,c,d){var f=[a.state.modeGen],g={};Ye(a,b.text,a.doc.mode,c,function(a,b){f.push(a,b)},g,d);for(c=0;c<a.state.overlays.length;++c){var h=a.state.overlays[c],m=1,n=0;Ye(a,b.text,h.mode,!0,function(a,b){for(var c=m;n<a;){var d=f[m];d>a&&f.splice(m,
...
d?b.rest[d-1]:b.line,g;c.pos=0;c.addToken=$f;var h;if(null!=Cd)h=Cd;else{h=qa(a.display.measure,document.createTextNode("A\u062eA"));var m=$a(h,0,1).getBoundingClientRect();h=m&&m.left!=m.right?Cd=3>$a(h,1,2).getBoundingClientRect().right-m.right:!1}h&&(g=ua(f))&&(c.addToken=ag(c.addToken,g));c.map=[];m=b!=a.display.externalMeasured&&Q(f);a:{h=c;var m=Ze(a,f,m),n=f.markedSpans,q=f.text,r=0;if(n)for(var s=q.length,v=0,u=1,t="",w=void 0,x=void 0,B=0,Y=void 0,K=void 0,Oa=void 0,L=void 0,U=void 0;;){if(B==
v){for(var Y=K=Oa=L=x="",U=null,B=Infinity,M=[],A=0;A<n.length;++A){var z=n[A],C=z.marker;"bookmark"==C.type&&z.from==v&&C.widgetNode?M.push(C):z.from<=v&&(null==z.to||z.to>v||C.collapsed&&z.to==v&&z.from==v)?(null!=z.to&&z.to!=v&&B>z.to&&(B=z.to,K=""),C.className&&(Y+=" "+C.className),C.css&&(x=C.css),C.startStyle&&z.from==v&&(Oa+=" "+C.startStyle),C.endStyle&&z.to==B&&(K+=" "+C.endStyle),C.title&&!L&&(L=C.title),C.collapsed&&(!U||0>Ue(U.marker,C))&&(U=z)):z.from>v&&B>z.from&&(B=z.from)}if(U&&(U.from||
0)==v){af(h,(null==U.to?s+1:U.to)-v,U.marker,null==U.from);if(null==U.to)break a;U.to==v&&(U=!1)}if(!U&&M.length)for(A=0;A<M.length;++A)af(h,0,M[A])}if(v>=s)break;for(M=Math.min(s,B);;){if(t){A=v+t.length;U||(z=A>M?t.slice(0,M-v):t,h.addToken(h,z,w?w+Y:Y,Oa,v+z.length==B?K:"",L,x));if(A>=M){t=t.slice(M-v);v=M;break}v=A;Oa=""}t=q.slice(r,r=m[u++]);w=$e(m[u++],h.cm.options)}}else for(var u=1;u<m.length;u+=2)h.addToken(h,q.slice(r,r=m[u]),$e(m[u+1],h.cm.options))}f.styleClasses&&(f.styleClasses.bgClass&&
...
(c.textClass=Dd(c.pre.className,c.textClass||""));return c}function $f(a,b,c,d,f,g,h){if(b){var m=a.splitSpaces?b.replace(/ {3,}/g,cg):b,n=a.cm.state.specialChars,q=!1;if(n.test(b))for(var r=document.createDocumentFragment(),s=0;;){n.lastIndex=s;var v=n.exec(b),u=v?v.index-s:b.length-s;if(u){var t=document.createTextNode(m.slice(s,s+u));O&&9>V?r.appendChild(D("span",[t])):r.appendChild(t);a.map.push(a.pos,a.pos+u,t);a.col+=u;a.pos+=u}if(!v)break;s+=u+1;"\t"==v[0]?(t=a.cm.options.tabSize,v=t-a.col%
t,t=r.appendChild(D("span",Me(v),"cm-tab")),t.setAttribute("role","presentation"),t.setAttribute("cm-text","\t"),a.col+=v):("\r"==v[0]||"\n"==v[0]?(t=r.appendChild(D("span","\r"==v[0]?"\u240d":"\u2424","cm-invalidchar")),t.setAttribute("cm-text",v[0])):(t=a.cm.options.specialCharPlaceholder(v[0]),t.setAttribute("cm-text",v[0]),O&&9>V?r.appendChild(D("span",[t])):r.appendChild(t)),a.col+=1);a.map.push(a.pos,a.pos+1,t);a.pos++}else{a.col+=b.length;var r=document.createTextNode(m);a.map.push(a.pos,a.pos+
b.length,r);O&&9>V&&(q=!0);a.pos+=b.length}if(c||d||f||q||h)return b=c||"",d&&(b+=d),f&&(b+=f),d=D("span",[r],b,h),g&&(d.title=g),a.content.appendChild(d);a.content.appendChild(r)}}function cg(a){for(var b=" ",c=0;c<a.length-2;++c)b+=c%2?" ":"\u00a0";return b+" "}function ag(a,b){return function(c,d,f,g,h,m,n){f=f?f+" cm-force-border":"cm-force-border";for(var q=c.pos,r=q+d.length;;){for(var s=0;s<b.length;s++){var v=b[s];if(v.to>q&&v.from<=q)break}if(v.to>=r)return a(c,d,f,g,h,m,n);a(c,d.slice(0,
v.to-q),f,g,null,m,n);g=null;d=d.slice(v.to-q);q=v.to}}}function af(a,b,c,d){var f=!d&&c.widgetNode;f&&a.map.push(a.pos,a.pos+b,f);!d&&a.cm.display.input.needsContentAttribute&&(f||(f=a.content.appendChild(document.createElement("span"))),f.setAttribute("cm-marker",c.id));f&&(a.cm.display.input.setUneditable(f),a.content.appendChild(f));a.pos+=b}function Le(a,b){return 0==b.from.ch&&0==b.to.ch&&""==P(b.text)&&(!a.cm||a.cm.options.wholeLineUpdateBefore)}function xd(a,b,c,d){function f(a,c,G){a.text=
c;a.stateAfter&&(a.stateAfter=null);a.styles&&(a.styles=null);null!=a.order&&(a.order=null);Se(a);Te(a,G);c=d?d(a):1;c!=a.height&&ya(a,c);ga(a,"change",a,b)}function g(a,b){for(var G=a,f=[];G<b;++G)f.push(new Wb(n[G],c?c[G]:null,d));return f}var h=b.from,m=b.to,n=b.text,q=E(a,h.line),r=E(a,m.line),s=P(n),v=c?c[n.length-1]:null,u=m.line-h.line;if(b.full)a.insert(0,g(0,n.length)),a.remove(n.length,a.size-n.length);else if(Le(a,b)){var t=g(0,n.length-1);f(r,r.text,v);u&&a.remove(h.line,u);t.length&&
a.insert(h.line,t)}else q==r?1==n.length?f(q,q.text.slice(0,h.ch)+s+q.text.slice(m.ch),v):(t=g(1,n.length-1),t.push(new Wb(s+q.text.slice(m.ch),v,d)),f(q,q.text.slice(0,h.ch)+n[0],c?c[0]:null),a.insert(h.line+1,t)):1==n.length?(f(q,q.text.slice(0,h.ch)+n[0]+r.text.slice(m.ch),c?c[0]:null),a.remove(h.line+1,u)):(f(q,q.text.slice(0,h.ch)+n[0],c?c[0]:null),f(r,s+r.text.slice(m.ch),v),t=g(1,n.length-1),1<u&&a.remove(h.line+1,u-1),a.insert(h.line+1,t));ga(a,"change",a,b)}function Xb(a){this.lines=a;this.parent=
null;for(var b=0,c=0;b<a.length;++b)a[b].parent=this,c+=a[b].height;this.height=c}function Yb(a){this.children=a;for(var b=0,c=0,d=0;d<a.length;++d){var f=a[d],b=b+f.chunkSize(),c=c+f.height;f.parent=this}this.size=b;this.height=c;this.parent=null}function cb(a,b,c){function d(a,G,f){if(a.linked)for(var g=0;g<a.linked.length;++g){var h=a.linked[g];if(h.doc!=G){var m=f&&h.sharedHist;if(!c||m)b(h.doc,m),d(h.doc,a,m)}}}d(a,null,!0)}function Id(a,c){if(c.cm)throw Error("This document is already in use.");
...
"*"==b.origin.charAt(0)))f.lastOp==d?(be(f.done),h=P(f.done)):f.done.length&&!P(f.done).ranges?h=P(f.done):1<f.done.length&&!f.done[f.done.length-2].ranges?(f.done.pop(),h=P(f.done)):h=void 0,m=h;if(m){var n=P(h.changes);0==J(b.from,b.to)&&0==J(b.from,n.to)?n.to=bb(b):h.changes.push(wd(a,b))}else for((h=P(f.done))&&h.ranges||sc(a.sel,f.done),h={changes:[wd(a,b)],generation:f.generation},f.done.push(h);f.done.length>f.undoDepth;)f.done.shift(),f.done[0].ranges||f.done.shift();f.done.push(c);f.generation=
...
O&&9<=V,Fe=!1,Na=!1;q.prototype=ra({update:function(a){var b=a.scrollWidth>a.clientWidth+1,c=a.scrollHeight>a.clientHeight+1,d=a.nativeBarWidth;c?(this.vert.style.display="block",this.vert.style.bottom=b?d+"px":"0",this.vert.firstChild.style.height=Math.max(0,a.scrollHeight-a.clientHeight+(a.viewHeight-(b?d:0)))+"px"):(this.vert.style.display="",this.vert.firstChild.style.height="0");b?(this.horiz.style.display="block",this.horiz.style.right=c?d+"px":"0",this.horiz.style.left=a.barLeft+"px",this.horiz.firstChild.style.width=
...
"18px";var a=this,b=function(b){(b.target||b.srcElement)!=a.vert&&(b.target||b.srcElement)!=a.horiz&&X(a.cm,oe)(b)};F(this.vert,"mousedown",b);F(this.horiz,"mousedown",b)},clear:function(){var a=this.horiz.parentNode;a.removeChild(this.horiz);a.removeChild(this.vert)}},q.prototype);s.prototype=ra({update:function(){return{bottom:0,right:0}},setScrollLeft:function(){},setScrollTop:function(){},clear:function(){}},s.prototype);c.scrollbarModel={"native":q,"null":s};T.prototype.signal=function(a,b){pa(a,
...
null);this.fastPoll()},onContextMenu:function(a){function b(){if(null!=h.selectionStart){var a=f.somethingSelected(),c="\u200b"+(a?h.value:"");h.value="\u21da";h.value=c;d.prevInput=a?"":"\u200b";h.selectionStart=1;h.selectionEnd=c.length;g.selForContextMenu=f.doc.sel}}function c(){d.contextMenuPending=!1;d.wrapper.style.position="relative";h.style.cssText=q;O&&9>V&&g.scrollbars.setScrollTop(g.scroller.scrollTop=n);if(null!=h.selectionStart){(!O||O&&9>V)&&b();var a=0,G=function(){g.selForContextMenu==
f.doc.sel&&0==h.selectionStart&&0<h.selectionEnd&&"\u200b"==d.prevInput?X(f,Fc.selectAll)(f):10>a++?g.detectingSelectAll=setTimeout(G,500):g.input.reset()};g.detectingSelectAll=setTimeout(G,200)}}var d=this,f=d.cm,g=f.display,h=d.textarea,m=sb(f,a),n=g.scroller.scrollTop;if(m&&!xa){f.options.resetSelectionOnContextMenu&&-1==f.doc.sel.contains(m)&&X(f,ba)(f.doc,Ca(m),Da);var q=h.style.cssText;d.wrapper.style.position="absolute";h.style.cssText="position: fixed; width: 30px; height: 30px; top: "+(a.clientY-
5)+"px; left: "+(a.clientX-5)+"px; z-index: 1000; background: "+(O?"rgba(255, 255, 255, .05)":"transparent")+"; outline: none; border-width: 0; outline: none; overflow: hidden; opacity: .05; filter: alpha(opacity=5);";if(aa)var r=window.scrollY;g.input.focus();aa&&window.scrollTo(null,r);g.input.reset();f.somethingSelected()||(h.value=d.prevInput=" ");d.contextMenuPending=!0;g.selForContextMenu=f.doc.sel;clearTimeout(g.detectingSelectAll);O&&9<=V&&b();if(pd){qd(a);var s=function(){Ga(window,"mouseup",
...
d,c));for(f=Za(a.doc,z(d,0),z(c,E(a.doc,c).text.length));1<b.length&&1<f.length;)if(P(b)==P(f))b.pop(),f.pop(),c--;else if(b[0]==f[0])b.shift(),f.shift(),d++;else break;for(var h=0,g=0,m=b[0],n=f[0],q=Math.min(m.length,n.length);h<q&&m.charCodeAt(h)==n.charCodeAt(h);)++h;m=P(b);n=P(f);for(q=Math.min(m.length-(1==b.length?h:0),n.length-(1==f.length?h:0));g<q&&m.charCodeAt(m.length-g-1)==n.charCodeAt(n.length-g-1);)++g;b[b.length-1]=m.slice(0,m.length-g);b[0]=b[0].slice(h);d=z(d,h);c=z(c,f.length?P(f).length-
g:0);if(1<b.length||b[0]||J(d,c))return tb(a.doc,b,d,c,"+input"),!0},ensurePolled:function(){this.forceCompositionEnd()},reset:function(){this.forceCompositionEnd()},forceCompositionEnd:function(){this.composing&&!this.composing.handled&&(this.applyComposition(this.composing),this.composing.handled=!0,this.div.blur(),this.div.focus())},applyComposition:function(a){a.data&&a.data!=a.startData&&X(this.cm,lc)(this.cm,a.data,0,a.sel)},setUneditable:function(a){a.setAttribute("contenteditable","false")},
...
g.line),c=Math.min(this.lastLine(),f.line-(f.ch?0:1))+1,f=h;f<c;++f)Mb(this,f,a);f=this.doc.sel.ranges;0==g.ch&&b.length==f.length&&0<f[d].from().ch&&dd(this.doc,d,new S(g,f[d].to()),Da)}}}),getTokenAt:function(a,b){return Xe(this,a,b)},getLineTokens:function(a,b){return Xe(this,z(a),b,!0)},getTokenTypeAt:function(a){a=I(this.doc,a);var b=Ze(this,E(this.doc,a.line)),c=0,d=(b.length-1)/2;a=a.ch;var f;if(0==a)f=b[2];else for(;;){var g=c+d>>1;if((g?b[2*g-1]:0)>=a)d=g;else if(b[2*g+1]<a)c=g+1;else{f=
...
a)},refresh:ea(function(){var a=this.display.cachedTextHeight;la(this);this.curOp.forceUpdate=!0;Gb(this);this.scrollTo(this.doc.scrollLeft,this.doc.scrollTop);r(this);(null==a||.5<Math.abs(a-Ta(this.display)))&&g(this);Z(this,"refresh",this)}),swapDoc:ea(function(a){var b=this.doc;b.cm=null;Id(this,a);Gb(this);this.display.input.reset();this.scrollTo(a.scrollLeft,a.scrollTop);this.curOp.forceScroll=!0;ga(this,"swapDoc",this,b);return b}),getInputField:function(){return this.display.input.getField()},
getWrapperElement:function(){return this.display.wrapper},getScrollerElement:function(){return this.display.scroller},getGutterElement:function(){return this.display.gutters}};xb(c);var pf=c.defaults={},gb=c.optionHandlers={},Jd=c.Init={toString:function(){return"CodeMirror.Init"}};H("value","",function(a,b){a.setValue(b)},!0);H("mode",null,function(a,c){a.doc.modeOption=c;b(a)},!0);H("indentUnit",2,b,!0);H("indentWithTabs",!1);H("smartIndent",!0);H("tabSize",4,function(a){d(a);Gb(a);la(a)},!0);H("lineSeparator",
null,function(a,b){if(a.doc.lineSep=b){var c=[],d=a.doc.first;a.doc.iter(function(a){for(var f=0;;){var g=a.text.indexOf(b,f);if(-1==g)break;f=g+b.length;c.push(z(d,g))}d++});for(var f=c.length-1;0<=f;f--)tb(a.doc,b,c[f],z(c[f].line,c[f].ch+b.length))}});H("specialChars",/[\t\u0000-\u0019\u00ad\u200b-\u200f\u2028\u2029\ufeff]/g,function(a,b,d){a.state.specialChars=new RegExp(b.source+(b.test("\t")?"":"|\t"),"g");d!=c.Init&&a.refresh()});H("specialCharPlaceholder",function(a){var b=D("span","\u2022",
"cm-invalidchar");b.title="\\u"+a.charCodeAt(0).toString(16);b.setAttribute("aria-label",b.title);return b},function(a){a.refresh()},!0);H("electricChars",!0);H("inputStyle",Ab?"contenteditable":"textarea",function(){throw Error("inputStyle can not (yet) be changed in a running editor");},!0);H("rtlMoveVisually",!jg);H("wholeLineUpdateBefore",!0);H("theme","default",function(a){h(a);m(a)},!0);H("keyMap","default",function(a,b,d){b=Mc(b);(d=d!=c.Init&&Mc(d))&&d.detach&&d.detach(a,b);b.attach&&b.attach(a,
d||null)});H("extraKeys",null);H("lineWrapping",!1,function(a){a.options.lineWrapping?(Jb(a.display.wrapper,"CodeMirror-wrap"),a.display.sizer.style.minWidth="",a.display.sizerWidth=null):(Hb(a.display.wrapper,"CodeMirror-wrap"),w(a));g(a);la(a);Gb(a);setTimeout(function(){B(a)},100)},!0);H("gutters",[],function(a){t(a.options);m(a)},!0);H("fixedGutter",!0,function(a,b){a.display.gutters.style.left=b?A(a.display)+"px":"0";a.refresh()},!0);H("coverGutterNextToScrollbar",!1,function(a){B(a)},!0);H("scrollbarStyle",
"native",function(a){v(a);B(a);a.display.scrollbars.setScrollTop(a.doc.scrollTop);a.display.scrollbars.setScrollLeft(a.doc.scrollLeft)},!0);H("lineNumbers",!1,function(a){t(a.options);m(a)},!0);H("firstLineNumber",1,m,!0);H("lineNumberFormatter",function(a){return a},m,!0);H("showCursorWhenSelecting",!1,Lb,!0);H("resetSelectionOnContextMenu",!0);H("lineWiseCopyCut",!0);H("readOnly",!1,function(a,b){"nocursor"==b?(Db(a),a.display.input.blur(),a.display.disabled=!0):(a.display.disabled=!1,b||a.display.input.reset())});
H("disableInput",!1,function(a,b){b||a.display.input.reset()},!0);H("dragDrop",!0,function(a,b,d){!b!=!(d&&d!=c.Init)&&(d=a.display.dragFunctions,b=b?F:Ga,b(a.display.scroller,"dragstart",d.start),b(a.display.scroller,"dragenter",d.simple),b(a.display.scroller,"dragover",d.simple),b(a.display.scroller,"drop",d.drop))});H("cursorBlinkRate",530);H("cursorScrollMargin",0);H("cursorHeight",1,Lb,!0);H("singleCursorHeightPerLine",!0,Lb,!0);H("workTime",100);H("workDelay",100);H("flattenSpans",!0,d,!0);
H("addModeClass",!1,d,!0);H("pollInterval",100);H("undoDepth",200,function(a,b){a.doc.history.undoDepth=b});H("historyEventDelay",1250);H("viewportMargin",10,function(a){a.refresh()},!0);H("maxHighlightLength",1E4,d,!0);H("moveInputWithCursor",!0,function(a,b){b||a.display.input.resetPosition()});H("tabindex",null,function(a,b){a.display.input.getField().tabIndex=b||""});H("autofocus",null);var of=c.modes={},cc=c.mimeModes={};c.defineMode=function(a,b){c.defaults.mode||"null"==a||(c.defaults.mode=
...
b){var c=dc.hasOwnProperty(a)?dc[a]:dc[a]={};ra(b,c)};c.defineExtension=function(a,b){c.prototype[a]=b};c.defineDocExtension=function(a,b){ka.prototype[a]=b};c.defineOption=H;var Uc=[];c.defineInitHook=function(a){Uc.push(a)};var zb=c.helpers={};c.registerHelper=function(a,b,d){zb.hasOwnProperty(a)||(zb[a]=c[a]={_global:[]});zb[a][b]=d};c.registerGlobalHelper=function(a,b,d,f){c.registerHelper(a,b,f);zb[a]._global.push({pred:d,val:f})};var qb=c.copyState=function(a,b){if(!0===b)return b;if(a.copyState)return a.copyState(b);
...
fallthrough:["basic","emacsy"]};Qa["default"]=ta?Qa.macDefault:Qa.pcDefault;c.normalizeKeyMap=function(a){var b={},c;for(c in a)if(a.hasOwnProperty(c)){var d=a[c];if(!/^(name|fallthrough|(de|at)tach)$/.test(c)){if("..."!=d)for(var f=nc(c.split(" "),Tf),g=0;g<f.length;g++){var h,m;g==f.length-1?(m=f.join(" "),h=d):(m=f.slice(0,g+1).join(" "),h="...");var n=b[m];if(!n)b[m]=h;else if(n!=h)throw Error("Inconsistent bindings for "+m);}delete a[c]}}for(var q in b)a[q]=b[q];return a};var Tb=c.lookupKey=
...
a["char"])return!1;var c=Ra[a.keyCode],d=c;if(null==d||a.altGraphKey)return!1;a.altKey&&"Alt"!=c&&(d="Alt-"+d);(nf?a.metaKey:a.ctrlKey)&&"Ctrl"!=c&&(d="Ctrl-"+d);(nf?a.ctrlKey:a.metaKey)&&"Cmd"!=c&&(d="Cmd-"+d);!b&&a.shiftKey&&"Shift"!=c&&(d="Shift-"+d);return d};c.fromTextArea=function(a,b){function d(){a.value=q.getValue()}b=b?ra(b):{};b.value=a.value;!b.tabindex&&a.tabIndex&&(b.tabindex=a.tabIndex);!b.placeholder&&a.placeholder&&(b.placeholder=a.placeholder);if(null==b.autofocus){var f=Ba();b.autofocus=
...
Ca(b))}),replaceRange:function(a,b,c,d){b=I(this,b);c=c?I(this,c):b;tb(this,a,b,c,d)},getRange:function(a,b,c){a=Za(this,I(this,a),I(this,b));return!1===c?a:a.join(c||this.lineSeparator())},getLine:function(a){return(a=this.getLineHandle(a))&&a.text},getLineHandle:function(a){if(Nb(this,a))return E(this,a)},getLineNumber:function(a){return Q(a)},getLineHandleVisualStart:function(a){"number"==typeof a&&(a=E(this,a));return Ea(a)},lineCount:function(){return this.size},firstLine:function(){return this.first},
lastLine:function(){return this.first+this.size-1},clipPos:function(a){return I(this,a)},getCursor:function(a){var b=this.sel.primary();return null==a||"head"==a?b.head:"anchor"==a?b.anchor:"end"==a||"to"==a||!1===a?b.to():b.from()},listSelections:function(){return this.sel.ranges},somethingSelected:function(){return this.sel.somethingSelected()},setCursor:ha(function(a,b,c){a=I(this,"number"==typeof a?z(a,b||0):a);ba(this,Ca(a,null),c)}),setSelection:ha(function(a,b,c){var d=I(this,a);a=I(this,b||
a);ba(this,Ca(d,a),c)}),extendSelection:ha(function(a,b,c){qc(this,I(this,a),b&&I(this,b),c)}),extendSelections:ha(function(a,b){for(var c=[],d=0;d<a.length;d++)c[d]=I(this,a[d]);$d(this,c)}),extendSelectionsBy:ha(function(a,b){$d(this,nc(this.sel.ranges,a),b)}),setSelections:ha(function(a,b,c){if(a.length){for(var d=0,f=[];d<a.length;d++)f[d]=new S(I(this,a[d].anchor),I(this,a[d].head));null==b&&(b=Math.min(a.length-1,this.sel.primIndex));ba(this,va(f,b),c)}}),addSelection:ha(function(a,b,c){var d=
this.sel.ranges.slice(0);d.push(new S(I(this,a),I(this,b||a)));ba(this,va(d,d.length-1),c)}),getSelection:function(a){for(var b=this.sel.ranges,c,d=0;d<b.length;d++){var f=Za(this,b[d].from(),b[d].to());c=c?c.concat(f):f}return!1===a?c:c.join(a||this.lineSeparator())},getSelections:function(a){for(var b=[],c=this.sel.ranges,d=0;d<c.length;d++){var f=Za(this,c[d].from(),c[d].to());!1!==a&&(f=f.join(a||this.lineSeparator()));b[d]=f}return b},replaceSelection:function(a,b,c){for(var d=[],f=0;f<this.sel.ranges.length;f++)d[f]=
a;this.replaceSelections(d,b,c||"+input")},replaceSelections:ha(function(a,b,c){for(var d=[],f=this.sel,g=0;g<f.ranges.length;g++){var h=f.ranges[g];d[g]={from:h.from(),to:h.to(),text:this.splitLines(a[g]),origin:c}}if(g=b&&"end"!=b){g=[];c=a=z(this.first,0);for(f=0;f<d.length;f++){var m=d[f],h=De(m.from,a,c),n=De(bb(m),a,c);a=m.to;c=n;"around"==b?(m=this.sel.ranges[f],m=0>J(m.head,m.anchor),g[f]=new S(m?n:h,m?h:n)):g[f]=new S(h,h)}g=new Ha(g,this.sel.primIndex)}b=g;for(g=d.length-1;0<=g;g--)mb(this,
...
(b.history=this.history);(this.linked||(this.linked=[])).push({doc:b,sharedHist:a.sharedHist});b.linked=[{doc:this,isParent:!0,sharedHist:a.sharedHist}];a=Qe(this);for(c=0;c<a.length;c++){var d=a[c],f=d.find(),g=b.clipPos(f.from),f=b.clipPos(f.to);J(g,f)&&(g=vb(b,g,f,d.primary,d.primary.type),d.markers.push(g),g.parent=d)}return b},unlinkDoc:function(a){a instanceof c&&(a=a.doc);if(this.linked)for(var b=0;b<this.linked.length;++b)if(this.linked[b].doc==a){this.linked.splice(b,1);a.unlinkDoc(this);
...
d="rrrrrrrrrrrr,rNNmmmmmmrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrmmmmmmmmmmmmmmrrrrrrrnnnnnnnnnn%nnrrrmrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrmmmmmmmmmmmmmmmmmmmNmmmm",f=/[\u0590-\u05f4\u0600-\u06ff\u0700-\u08ac]/,g=/[stwN]/,h=/[LRr]/,m=/[Lb1n]/,n=/[1n]/;return function(c){if(!f.test(c))return!1;for(var d=c.length,q=[],r=0,s;r<d;++r)q.push(a(c.charCodeAt(r)));for(var r=0,v="L";r<d;++r)s=q[r],"m"==s?q[r]=v:v=s;r=0;for(v="L";r<
...
"L");u="L"==(r?q[r-1]:"L")||s?"L":"R";for(s=r;s<v;++s)q[s]=u;r=v-1}for(var v=[],t,r=0;r<d;)if(m.test(q[r])){s=r;for(++r;r<d&&m.test(q[r]);++r);v.push(new b(0,s,r))}else{var w=r,u=v.length;for(++r;r<d&&"L"!=q[r];++r);for(s=w;s<r;)if(n.test(q[s])){w<s&&v.splice(u,0,new b(1,w,s));w=s;for(++s;s<r&&n.test(q[s]);++s);v.splice(u,0,new b(2,w,s));w=s}else++s;w<r&&v.splice(u,0,new b(1,w,r))}1==v[0].level&&(t=c.match(/^\s+/))&&(v[0].from=t[0].length,v.unshift(new b(0,0,t[0].length)));1==P(v).level&&(t=c.match(/\s+$/))&&
(P(v).to-=t[0].length,v.push(new b(0,d-t[0].length,d)));2==v[0].level&&v.unshift(new b(1,v[0].to,v[0].to));v[0].level!=P(v).level&&v.push(new b(v[0].level,d,d));return v}}();c.version="5.5.1";return c}();(function(c){c(CodeMirror)})(function(c){var a=/^(\s*)(>[> ]*|[*+-]\s|(\d+)([.)]))(\s*)/,b=/^(\s*)(>[> ]*|[*+-]|(\d+)[.)])(\s*)$/,d=/[*+-]\s/;c.commands.newlineAndIndentContinueMarkdownList=function(f){if(f.getOption("disableInput"))return c.Pass;for(var g=f.listSelections(),h=[],m=0;m<g.length;m++){var n=g[m].head,r=f.getStateAfter(n.line),u=!1!==r.list,w=0!==r.quote,t=f.getLine(n.line),r=a.exec(t);if(!g[m].empty()||!u&&!w||!r){f.execCommand("newlineAndIndent");return}b.test(t)?(f.replaceRange("",
{line:n.line,ch:0},{line:n.line,ch:n.ch+1}),h[m]="\n"):(n=r[1],u=r[5],r=d.test(r[2])||0<=r[2].indexOf(">")?r[2]:parseInt(r[3],10)+1+r[4],h[m]="\n"+n+r+u)}f.replaceSelections(h)}});(function(c){c(CodeMirror)})(function(c){c.defineMode("xml",function(a,b){function d(a,b){function c(d){b.tokenize=d;return d(a,b)}var d=a.next();if("<"==d){if(a.eat("!"))return a.eat("[")?a.match("CDATA[")?c(h("atom","]]\x3e")):null:a.match("--")?c(h("comment","--\x3e")):a.match("DOCTYPE",!0,!0)?(a.eatWhile(/[\w\._\-]/),c(m(1))):null;if(a.eat("?"))return a.eatWhile(/[\w\._\-]/),b.tokenize=h("meta","?>"),"meta";da=a.eat("/")?"closeTag":"openTag";b.tokenize=f;return"tag bracket"}if("&"==d)return(a.eat("#")?
...
a)return c.tagName=b.current(),W="tag",v;W="error";return t}function x(a,b,c){if("word"==a){a=b.current();c.context&&c.context.tagName!=a&&A.implicitlyClosed.hasOwnProperty(c.context.tagName)&&r(c);if(c.context&&c.context.tagName==a)return W="tag",q;W="tag error";return s}W="error";return s}function q(a,b,c){if("endTag"!=a)return W="error",q;r(c);return w}function s(a,b,c){W="error";return q(a,b,c)}function v(a,b,c){if("word"==a)return W="attribute",B;if("endTag"==a||"selfcloseTag"==a){b=c.tagName;
var d=c.tagStart;c.tagName=c.tagStart=null;"selfcloseTag"==a||A.autoSelfClosers.hasOwnProperty(b)?u(c,b):(u(c,b),c.context=new n(c,b,d==c.indented));return w}W="error";return v}function B(a,b,c){if("equals"==a)return L;A.allowMissing||(W="error");return v(a,b,c)}function L(a,b,c){if("string"==a)return K;if("word"==a&&A.allowUnquoted)return W="string",v;W="error";return v(a,b,c)}function K(a,b,c){return"string"==a?K:v(a,b,c)}var M=a.indentUnit,C=b.multilineTagIndentFactor||1,R=b.multilineTagIndentPastTag;
...
a.match(/^[^\s]+/,!0);void 0===a.peek()?b.linkTitle=!0:a.match(/^(?:\s+(?:"(?:[^"\\]|\\\\|\\.)+"|'(?:[^'\\]|\\\\|\\.)+'|\((?:[^)\\]|\\\\|\\.)+\)))?/,!0);b.f=b.inline=t;return A.linkHref+" url"}function K(a){lb[a]||(a=(a+"").replace(/([.?*+^$[\]\\(){}|-])/g,"\\$1"),lb[a]=new RegExp("^(?:[^\\\\]|\\\\.)*?("+a+")"));return lb[a]}var M=c.modes.hasOwnProperty("xml"),C=c.getMode(a,M?{name:"xml",htmlMode:!0}:"text/plain");void 0===b.highlightFormatting&&(b.highlightFormatting=!1);void 0===b.maxBlockquoteDepth&&
...
for(b in a)if(a.hasOwnProperty(b)){var c=f.getClientObject(b);a[b].name&&c.setName(a[b].name);(c=a[b].selection)?f.clients[b].updateSelection(f.transformSelection(Selection.fromJSON(c))):f.clients[b].removeSelection()}},reconnect:function(){f.serverReconnect()}})}function g(a){return a[a.length-1]}var h=c.Client,m=c.Cursor,n=c.UndoManager,r=c.TextOperation,u=c.WrappedOperation;a.prototype.invert=function(){return new a(this.cursorAfter,this.cursorBefore)};a.prototype.compose=function(c){return new a(this.cursorBefore,
...
function(){return this.attachedObject_};f.prototype.attachObject=function(a){this.node_.attachedObject=a};var n={equals:function(){return!1}};g.prototype.insertAnnotatedSpan=function(c,d){this.wrapOperation_(new m(c.pos,0),function(f,g){a(!g||null===g.next);var m=new h(c.length,d);if(g){a(c.pos>f&&c.pos<f+g.length);var q=new h(0,n);q.next=new h(c.pos-f,g.annotation);q.next.next=m;m.next=new h(f+g.length-c.pos,g.annotation);return q.next}return m})};g.prototype.removeSpan=function(c){0!==c.length&&
this.wrapOperation_(c,function(d,f){a(null!==f);var g=new h(0,n),m=g;c.pos>d&&(m.next=new h(c.pos-d,f.annotation),m=m.next);for(;c.end()>d+f.length;)d+=f.length,f=f.next;var q=d+f.length-c.end();0<q&&(m.next=new h(q,f.annotation));return g.next})};g.prototype.updateSpan=function(c,d){0!==c.length&&this.wrapOperation_(c,function(f,g){a(null!==g);var m=new h(0,n),q=m,s=f,v=c.pos-s;a(v<g.length);0<v&&(q.next=new h(v,g.annotation),q=q.next,s+=q.length);for(;null!==g&&c.end()>=f+g.length;)s=f+g.length-
s,q.next=new h(s,d(g.annotation,s)),q=q.next,f+=g.length,g=g.next,s=f;v=c.end()-s;0<v&&(a(v<g.length),q.next=new h(v,d(g.annotation,v)),q=q.next,s+=q.length,q.next=new h(f+g.length-s,g.annotation));return m.next})};g.prototype.wrapOperation_=function(a,b){if(0>a.pos)throw Error("Span start cannot be negative.");var c=[],g=[],m=this.getAffectedNodes_(a),q;null!==m.start?(q=m.end.next,m.end.next=null):q=m.succ;var n=b(m.startPos,m.start),v=!1,B=!1;if(n){this.mergeNodesWithSameAnnotations_(n);var L;
m.pred&&m.pred.annotation.equals(n.annotation)?(v=!0,n.length+=m.pred.length,m.beforePred.next=n,L=m.predPos):(m.beforeStart.next=n,L=m.startPos);for(;n.next;)g.push(new f(L,n)),L+=n.length,n=n.next;m.succ&&m.succ.annotation.equals(n.annotation)?(n.length+=m.succ.length,B=!0,n.next=m.succ.next):n.next=q;g.push(new f(L,n))}else m.pred&&m.succ&&m.pred.annotation.equals(m.succ.annotation)?(B=v=!0,n=new h(m.pred.length+m.succ.length,m.pred.annotation),m.beforePred.next=n,n.next=m.succ.next,g.push(new f(m.startPos-
m.pred.length,n))):m.beforeStart.next=q;v&&c.push(new d(m.predPos,m.pred));q=m.startPos;for(n=m.start;null!==n;)c.push(new d(q,n)),q+=n.length,n=n.next;B&&c.push(new d(q,m.succ));this.changeHandler_(c,g)};g.prototype.getAffectedNodes_=function(a){for(var b={},c=null,d=this.head_,f=d.next,g=0;null!==f&&a.pos>=g+f.length;)g+=f.length,c=d,d=f,f=f.next;if(null===f&&(0!==a.length||a.pos!==g))throw Error("Span start exceeds the bounds of the AnnotationList.");b.startPos=g;b.start=0===a.length&&a.pos===
...
"<li"+t+q+">";t=!0}else r+="<div"+q+">";q=!0}if(g.LINE_SENTINEL in L)v=c.ops[++s];else if(g.ENTITY_SENTINEL in L){for(M=0;M<v.text.length;M++)K=vceditor.Entity.fromAttributes(L),K=n.exportToElement(K),r+=K.outerHTML;v=c.ops[++s]}else{var K=M="",R;for(R in L){var C=L[R],A,T;R===g.BOLD||R===g.ITALIC||R===g.UNDERLINE||R===g.STRIKE?(f.assert(!0===C),A=T=R):R===g.FONT_SIZE?(A='span style="font-size: '+C,A+="string"!==typeof C||-1===C.indexOf("px",C.length-2)?'px"':'"',T="span"):R===g.FONT?(A='span style="font-family: '+
C+'"',T="span"):R===g.COLOR?(A='span style="color: '+C+'"',T="span"):R===g.BACKGROUND_COLOR?(A='span style="background-color: '+C+'"',T="span"):f.log(!1,"Encountered unknown attribute while rendering html: "+R);A&&(M+="<"+A+">");T&&(K="</"+T+">"+K)}var C=v.text,da=C.indexOf("\n");0<=da?(u=!0,v=da<C.length-1?new vceditor.TextOp("insert",C.substr(da+1),L):c.ops[++s],C=C.substr(0,da)):v=c.ops[++s];C=C.replace(/  +/g,function(a){return Array(a.length+1).join("\u00a0")}).replace(/^ /,"\u00a0").replace(/ $/,
...
this.currentLine[0].text.replace(/^ +/,"");this.currentLine[a].text=this.currentLine[a].text.replace(/ +$/g,"");for(a=0;a<this.currentLine.length;a++)this.currentLine[a].text=this.currentLine[a].text.replace(/\u00a0/g," ")}1===this.currentLine.length&&""===this.currentLine[0].text&&(this.currentLine=[])};var n,r=r||{ELEMENT_NODE:1,TEXT_NODE:3};return function(c,g){var h=(vceditor.document||document).createElement("div");h.innerHTML=c;n=g;var m=new d,q=new a;f(h,q,m);return m.lines}}();c.ParseHtml=
...
this.onCursorActivity_);this.changeId_=0;this.outstandingChanges_={};this.dirtyLines_=[]}function b(a,b){return a.line-b.line||a.ch-b.ch}function d(a){if(0===a.length)return 0;for(var b=0,c=0;c<a.length;c++)b+=a[c].length;return b+a.length-1}function f(a){this.attributes=a||{}}function g(a,b){var c=a[b];a[b]=function(){c.apply(a,arguments)}}var h=c.AnnotationList,m=c.Span,n=c.utils,r=c.AttributeConstants,u={c:"color",bc:"background-color",fs:"font-size",li:function(a){return"padding-left: "+40*a+
...
c[a]=b}),this.updateCurrentAttributes_())};a.prototype.updateTextAttributes=function(a,b,c,d,g){var h=[],n=a,t=this;this.annotationList_.updateSpan(new m(a,b-a),function(a,b){var m={},q;for(q in a.attributes)m[q]=a.attributes[q];m[r.LINE_SENTINEL]&&!g||c(m);q={};var s={};t.computeChangedAttributes_(a.attributes,m,q,s);var u;a:{for(u in q){u=!1;break a}u=!0}u||h.push({start:n,end:n+b,attributes:q,attributesInverse:s,origin:d});n+=b;return new f(m)});0<h.length&&this.trigger("attributesChange",this,
...
"string"!==typeof f&&(f+="px");var h=f.toString().toLowerCase().replace(/[^a-z0-9-_]/g,"-"),g=g+("-"+h);u[d]&&(w[d]||(w[d]={}),w[d][h]||(w[d][h]=!0,h=u[d],f="function"===typeof h?h(f):h+": "+f,this.addStyleWithCSS_((d==r.LINE_INDENT?"pre."+g:"."+g)+" { "+f+" }")))}b=b+" "+g}}return b};a.prototype.markEntity_=function(a){for(var b=c.Entity.fromAttributes(a.annotation.attributes),d=this.codeMirror,f=this,g=[],h=0;h<a.length;h++){var m=d.posFromIndex(a.pos+h),n=d.posFromIndex(a.pos+h+1),r={collapsed:!0,
atomic:!0,inclusiveLeft:!1,inclusiveRight:!1},t=this.createEntityHandle_(b,a.pos),u=this.entityManager_.renderToElement(b,t);u&&(r.replacedWith=u);m=d.markText(m,n,r);g.push(m);t.setMarker(m)}a.attachObject({clear:function(){for(var a=0;a<g.length;a++)g[a].clear()},update:function(a){a=c.Entity.fromAttributes(a);for(var b=0;b<g.length;b++)f.entityManager_.updateElement(a,g[b].replacedWith)}});this.queueRefresh_()};a.prototype.queueRefresh_=function(){var a=this;this.refreshTimer_||(this.refreshTimer_=
...
t+x+"]","g"),"");c.push(f)}b.update(b.from,b.to,c)}};a.prototype.onCodeMirrorChange_=function(a,b){if("object"===typeof b.from){for(var c=[];b;)c.push(b),b=b.next;b=c}for(var c=this.convertCoordinateSystemForChanges_(b),d=[],g=0;g<c.length;g++){var h=c[g],n=h.start,r=h.text,t=h.removed,u=h.origin;if(0<t.length){for(var w=this.annotationList_.getAnnotatedSpansForSpan(new m(n,t.length)),x=0,W=0;W<w.length;W++){var U=w[W];d.push({start:n,end:n+U.length,removedAttributes:U.annotation.attributes,removed:t.substr(x,
U.length),attributes:{},text:"",origin:h.origin});x+=U.length}this.annotationList_.removeSpan(new m(n,t.length))}0<r.length&&("+input"===h.origin||"paste"===h.origin?h=this.currentAttributes_||{}:u in this.outstandingChanges_?(h=this.outstandingChanges_[u].attributes,u=this.outstandingChanges_[u].origOrigin,delete this.outstandingChanges_[u]):h={},this.annotationList_.insertAnnotatedSpan(new m(n,r.length),new f(h)),d.push({start:n,end:n,removedAttributes:{},removed:"",text:r,attributes:h,origin:u}))}this.markLineSentinelCharactersForChanges_(b);
...
c.line-f.from.line))+1+c.ch}}for(var f=this,g=function(a){return f.codeMirror.indexFromPos(a)},h=[],m=a.length-1;0<=m;m--){var n=a[m],g=c(g,n),r=g(n.from),t=n.removed.join("\n"),u=n.text.join("\n");h.unshift({start:r,end:r+t.length,removed:t,text:u,origin:n.origin})}return h};a.prototype.markLineSentinelCharactersForChanges_=function(a){for(var b=Number.MAX_VALUE,c=-1,d=0;d<a.length;d++){var f=a[d],g=f.from.line;if(1<f.removed.length||0<=f.removed[0].indexOf(t))b=Math.min(b,g),c=Math.max(c,g);1<f.text.length?
