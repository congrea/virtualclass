...
this.initRecordFinishEvent("recordingCloseButton")},initRecordFinishEvent:function(a){var b=document.getElementById("recordingHeader");b.innerHTML=virtualclass.lang.getString("uploadsessionfinish");virtualclass.recorder.doSessionClear=!0;document.getElementById(a).addEventListener("click",function(){virtualclass.recorder.uploadInProcess=!1;setTimeout(function(){delete virtualclass.recorder.doSessionClear;document.getElementById("progressContainer").style.display="block";document.getElementById("recordFinishedMessageBox").style.display=
"none";b.innerHTML=virtualclass.lang.getString("uploadsession");virtualclass.recorder.startNewSessionAfterFinish();console.log("Socket "+io.sock.readyState);io.disconnect()},300)})},startNewSessionAfterFinish:function(){var a=document.getElementById("recordingContainer");a.classList.add("recordingFinished");console.log("Record :- after recording");virtualclass.clearSession();virtualclass.pbar.renderProgressBar(0,0,"progressBar","progressValue");virtualclass.pbar.renderProgressBar(0,0,"indProgressBar",
"indProgressValue");a.classList.remove("recordingFinished")},afterRecording:function(){g=1;virtualclass.recorder.rdlength=0;document.getElementById("progressContainer").style.display="none";virtualclass.recorder.uploadFinishedBox()},makeAvailDownloadFile:function(){console.log("Recorder:- DOWNLLOAD MESSAGE");var b=document.getElementById("recordingContainer"),d=c.getComputedStyle(b).display;"none"!=d&&"block"==d||setTimeout(function(){virtualclass.popup.openProgressBar();io.disconnect()},3E3);var f=
document.getElementById("downloadFileCont");null==f&&(f=document.createElement("div"),f.id="downloadFileCont");var g=document.createElement("div");g.id="errormsessage";g.innerHTML=virtualclass.lang.getString("msgForDownloadStart");var h=document.getElementById("progressContainer");h.style.display="none";f.appendChild(g);b.appendChild(f);this.alreadyDownload=!0;virtualclass.storage.getAllDataForDownload(["chunkData"],function(b){virtualclass.gObj.saveSession=!0;var d=document.createElement("button");
d.id="downloadButton";d.innerHTML=virtualclass.lang.getString("downloadFile");var m=document.createElement("a");m.id="dlink";m.href="";m.download="session.vcp";m.innerHTML=virtualclass.lang.getString("download");g.innerHTML=virtualclass.lang.getString("filenotsave");f.appendChild(d);var p=document.getElementById("recordingHeader");p.innerHTML=virtualclass.lang.getString("downloadFile");var r=new Blob([b],{type:"application/virtualclass"});virtualclass.hasOwnProperty("prevScreen")&&virtualclass.prevScreen.hasOwnProperty("currentStream")&&
virtualclass.prevScreen.unShareScreen();d.addEventListener("click",function(){null!=c.webkitURL?m.href=c.webkitURL.createObjectURL(r):(m.href=c.URL.createObjectURL(r),m.onclick=a,document.body.appendChild(m));m.click();p=document.getElementById("recordingHeader");p.innerHTML=virtualclass.lang.getString("uploadsession");f.parentNode.removeChild(f);h.style.display="block";virtualclass.recorder.hasOwnProperty("dataCame")&&clearTimeout(virtualclass.recorder.dataCame);virtualclass.clearSession();delete virtualclass.gObj.saveSession;
io.disconnect();io.init(virtualclass.uInfo)})})},sendDataToServer:function(){virtualclass.storage.db.transaction(["chunkData"],"readwrite").objectStore("chunkData").clear();virtualclass.popup.waitBlockAction("none");c.Worker&&(mvDataWorker.postMessage({rdata:virtualclass.recorder.items,makeChunk:!0}),mvDataWorker.onmessage=function(a){a.data.hasOwnProperty("done_problem")?console.log("done problem"):a.data.hasOwnProperty("status")&&"done"==a.data.status&&(console.log("Recorder:- done"),delete virtualclass.recorder.startUpload,
