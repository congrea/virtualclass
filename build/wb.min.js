...
st:g.type,cf:"unshareScreen"}),g.prevStream=!1,g.prevScreen="",virtualclass.prevScreen=""):g.ssByClick=!0};b={};b.width=c.innerWidth;b.height=c.innerHeight-140;var d=document.getElementById(this.local),m=this.html.getDimension(b);d.style.width=Math.round(m.width)+"px";d.style.height=Math.round(m.height)+"px";g=this;this.video.onloadedmetadata=function(){g.width=m.width;g.height=m.height;g.localtempCanvas.width=g.video.offsetWidth;g.localtempCanvas.height=g.video.offsetHeight;virtualclass.prevScreen=
...
if("undefined"!=typeof d&&"undefined"!=typeof f){if(d!=s.video.offsetWidth||f!=s.video.offsetHeight)a=1}else d=s.video.offsetWidth,f=s.video.offsetHeight;1==a?setTimeout(h,2E3):t(s.type)}function m(a){switch(!0){case 300>=a||"undefined"==typeof a:v=300;break;case 3E3>=a:v=a;break;case 5E3>=a:v=3E3;break;default:v=5E3}}function h(){console.log("RESIZE");if(roles.hasControls()){d=s.video.offsetWidth;f=s.video.offsetHeight;x=Math.round(s.localtempCanvas.height/12);q=Math.round(s.localtempCanvas.width/
12);var t=s.type;s.localtempCanvas.width=s.video.offsetWidth;s.localtempCanvas.height=s.video.offsetHeight;s.localtempCont.drawImage(s.video,0,0,s.video.offsetWidth,s.video.offsetHeight);var w=s.localtempCont.getImageData(0,0,s.localtempCanvas.width,s.localtempCanvas.height),w=s.dc.encodeRGB(w.data),K=Math.round(s.localtempCanvas.width/q),M=Math.round(s.localtempCanvas.height/x),C,R,A,U;U=s.getContainerDimension();"undefined"!=typeof d&&"undefined"!=typeof f?(C=b(d,4),R=b(f,4)):(C=b(s.video.offsetWidth,
4),R=b(s.video.offsetHeight,4));A=b(U.width,4);U=b(U.height,4);t=new Uint8ClampedArray(["ss"==t?104:204,C[0],C[1],R[0],R[1],A[0],A[1],U[0],U[1]]);C=new Uint8ClampedArray(w.length+t.length);C.set(t);C.set(w,t.length);c.Worker&&sworker.postMessage({img:w,resize:1,resA:x,resB:q,dw:K,dh:M,offsetWidth:s.video.offsetWidth,offsetHeight:s.video.offsetHeight},[w.buffer]);ioAdapter.sendBinary(C);m(C.length/128);a=0;clearInterval(virtualclass.clear);virtualclass.clear=setInterval(g,v)}}function t(a){s.localtempCanvas.width=
s.video.offsetWidth;s.localtempCanvas.height=s.video.offsetHeight;s.localtempCont.drawImage(s.video,0,0,s.video.offsetWidth,s.video.offsetHeight);a=Math.round(s.localtempCanvas.width/q);var b=Math.round(s.localtempCanvas.height/x),d=s.localtempCont.getImageData(0,0,s.video.offsetWidth,s.video.offsetHeight);c.Worker&&(sworker.postMessage({img:d.data,resA:x,resB:q,dw:a,dh:b,offsetWidth:s.video.offsetWidth,offsetHeight:s.video.offsetHeight,type:s.type},[d.data.buffer]),sworker.onmessage=function(a){if(1==
a.data.needFullScreen){a=virtualclass.getDataFullScreen(s.type);ioAdapter.sendBinary(a);var b=a.length/128}else null!=a.data.masterSlice&&(ioAdapter.sendBinary(a.data.masterSlice),b=a.data.masterSlice.length/128);m(b);clearInterval(virtualclass.clear);virtualclass.clear=setInterval(g,v)});clearInterval(virtualclass.clear)}var x=Math.round(this.localtempCanvas.height/12),q=Math.round(this.localtempCanvas.width/12),s=this;virtualclass.hasOwnProperty("wholeImage")&&clearInterval(virtualclass.wholeImage);
...
b[f];d++}this.hasOwnProperty("prvNum")||c.play();this.prvNum=d}}},initMakeAvailDownloadFile:function(){virtualclass.gObj.downloadProgress=!0;virtualclass.recorder.dataCame=setTimeout(function(){virtualclass.recorder.hasOwnProperty("recordDone")&&(virtualclass.recorder.alreadyDownload||(console.log("Recorder:- From Interval"),virtualclass.recorder.makeAvailDownloadFile()),clearTimeout(virtualclass.recorder.dataCame))},1500)},replayFromStart:function(){var a=[],a=this.items;virtualclass.storage.config.endSession();
...
d.append("user",virtualclass.gObj.uid);d.append("cn",g);d.append("sesseionkey",virtualclass.recorder.sessionKey);virtualclass.recorder.items=[];virtualclass.xhr.send(d,importfilepath,function(b){"done"===b?(virtualclass.pbar.renderProgressBar(a.totalStore,a.totalSent,"progressBar","progressValue"),virtualclass.recorder.rnum++,g++,virtualclass.recorder.xhrsenddata(virtualclass.recorder.rnum)):"ERROR"===b?virtualclass.recorder.initMakeAvailDownloadFile():alert(b)})}},virtualclass.recorder.rnum)}},afterRecording:function(){g=
1;document.getElementById("progressContainer").style.display="block";setTimeout(function(){var a=document.getElementById("recordingContainer");a.classList.add("recordingFinished");console.log("Record :- after recording");virtualclass.clearSession();virtualclass.pbar.renderProgressBar(0,0,"progressBar","progressValue");virtualclass.pbar.renderProgressBar(0,0,"indProgressBar","indProgressValue");a.classList.remove("recordingFinished")},2E3)},makeAvailDownloadFile:function(){console.log("Recorder:- DOWNLLOAD MESSAGE");
var b=document.getElementById("recordingContainer"),d=document.getElementById("downloadFileCont");null==d&&(d=document.createElement("div"),d.id="downloadFileCont");var f=document.createElement("div");f.id="errormsessage";f.innerHTML=virtualclass.lang.getString("msgForDownloadStart");var g=document.getElementById("progressContainer");g.style.display="none";d.appendChild(f);b.appendChild(d);this.alreadyDownload=!0;virtualclass.storage.getAllDataForDownload(["chunkData"],function(b){virtualclass.gObj.saveSession=
!0;var m=document.createElement("button");m.id="downloadButton";m.innerHTML=virtualclass.lang.getString("downloadFile");var h=document.createElement("a");h.id="dlink";h.href="";h.download="session.vcp";h.innerHTML=virtualclass.lang.getString("download");f.innerHTML=virtualclass.lang.getString("filenotsave");d.appendChild(m);var q=document.getElementById("recordingHeader");q.innerHTML=virtualclass.lang.getString("downloadFile");var s=new Blob([b],{type:"application/virtualclass"});virtualclass.hasOwnProperty("prevScreen")&&
virtualclass.prevScreen.hasOwnProperty("currentStream")&&virtualclass.prevScreen.unShareScreen();m.addEventListener("click",function(){null!=c.webkitURL?h.href=c.webkitURL.createObjectURL(s):(h.href=c.URL.createObjectURL(s),h.onclick=a,document.body.appendChild(h));h.click();q=document.getElementById("recordingHeader");q.innerHTML=virtualclass.lang.getString("uploadsession");d.parentNode.removeChild(d);g.style.display="block";virtualclass.recorder.hasOwnProperty("dataCame")&&clearTimeout(virtualclass.recorder.dataCame);
virtualclass.clearSession();delete virtualclass.gObj.saveSession;io.disconnect();io.init(virtualclass.uInfo)})})},sendDataToServer:function(){virtualclass.storage.db.transaction(["chunkData"],"readwrite").objectStore("chunkData").clear();virtualclass.popup.waitBlockAction("none");c.Worker&&(mvDataWorker.postMessage({rdata:virtualclass.recorder.items,makeChunk:!0}),mvDataWorker.onmessage=function(a){a.data.hasOwnProperty("done_problem")?console.log("done problem"):a.data.hasOwnProperty("status")&&
"done"==a.data.status&&(console.log("Recorder:- done"),delete virtualclass.recorder.startUpload,virtualclass.recorder.recordDone=!0);virtualclass.storage.chunkStorage(a.data)})},displayWaitPopupIfNot:function(){!1==this.waitPopup&&(virtualclass.popup.sendBackOtherElems(),document.getElementById("recordingContainer").style.display="none",virtualclass.popup.waitBlockAction("block"),virtualclass.pbar.renderProgressBar(0,0,"downloadProgressBar","downloadProgressValue"),virtualclass.popup.waitBlock(),
this.waitPopup=!0)},requestDataFromServer:function(a,b){this.displayWaitPopupIfNot(virtualclass.lang.getString("plswaitwhile"));var d=new FormData;d.append("prvfile",b);d.append("fileBundelId",a);var c=getUrlVars(exportfilepath);c.hasOwnProperty("cmid")&&d.append("id",c.cmid);virtualclass.xhr.send(d,exportfilepath,function(b){virtualclass.recorder.sendToWorker(b,a)})},sendToWorker:function(a,b){c.Worker&&(mvDataWorker.postMessage({rdata:a,getData:!0}),mvDataWorker.onmessage=function(a){f++;var d=
virtualclass.recorder.isUptoBaseValue(a.data.alldata.totalSent,a.data.alldata.totalStore,30);virtualclass.recorder.ctotalStore=a.data.alldata.totalStore;virtualclass.recorder.ctotalSent=a.data.alldata.totalSent;virtualclass.pbar.renderProgressBar(a.data.alldata.totalStore,a.data.alldata.totalSent,"downloadProgressBar","downloadProgressValue");d&&!virtualclass.recorder.alreadyAskForPlay?(a.data.alldata.totalSent>a.data.alldata.totalStore?virtualclass.recorder.askToPlay("completed"):virtualclass.recorder.askToPlay(),
virtualclass.recorder.alreadyAskForPlay=!0,virtualclass.recorder.tempRecData.push(a.data.alldata.rdata)):d&&virtualclass.recorder.playStart&&!1==virtualclass.recorder.waitServer?virtualclass.recorder.init(a.data.alldata.rdata):(virtualclass.recorder.tempRecData.push(a.data.alldata.rdata),!0==virtualclass.recorder.waitServer&&(virtualclass.recorder.alreadyPlayed=!0));a.data.alldata.rdata[a.data.alldata.rdata.length-1].hasOwnProperty("sessionEnd")?(console.log("Request file  Finished Here"),virtualclass.recorder.allFileFound=
!0,!0==virtualclass.recorder.waitServer&&(virtualclass.storage.config.endSession(),a=virtualclass.recorder.tempRecData.reduce(function(a,b){return a.concat(b)}),virtualclass.recorder.objn=0,virtualclass.recorder.init(a),virtualclass.recorder.play(),virtualclass.recorder.waitServer=!1,virtualclass.popup.closeElem())):(console.log("request file"),virtualclass.recorder.requestDataFromServer(b,f))})},playInt:function(){var a=virtualclass.recorder.tempRecData.reduce(function(a,b){return a.concat(b)});
