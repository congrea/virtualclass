...
0);var b=p.first+p.size-1;if(a.line>b)return A(b,D(p,b).text.length);var b=D(p,a.line).text.length,c=a.ch,b=null==c||c>b?A(a.line,b):0>c?A(a.line,0):a;return b}function Xb(p,a){return a>=p.first&&a<p.first+p.size}function Yb(p,a,b,c){return p.cm&&p.cm.display.shift||p.extend?(p=a.anchor,c&&(a=0>K(b,p),a!=0>K(c,p)?(p=b,b=c):a!=0>K(b,c)&&(b=c)),new Q(p,b)):new Q(c||b,b)}function yc(p,a,b,c){ba(p,new Ja([Yb(p,p.sel.primary(),a,b)],0),c)}function ae(p,a,b){for(var c=[],d=0;d<p.sel.ranges.length;d++)c[d]=
...
(n=!1,h&&nd(a,b,d,s.pos),s.pos=b.length,r=null):r=We(Jd(c,s,d,t),g);if(t){var v=t[0].name;v&&(r="m-"+(r?v+" "+r:v))}if(!n||q!=r){for(;m<s.start;)m=Math.min(s.start,m+5E4),f(m,q);q=r}s.start=s.pos}for(;m<s.pos;)a=Math.min(s.pos,m+5E4),f(a,q),m=a}function je(a,b,c,d){var f=[a.state.modeGen],g={};Ze(a,b.text,a.doc.mode,c,function(a,b){f.push(a,b)},g,d);for(c=0;c<a.state.overlays.length;++c){var h=a.state.overlays[c],n=1,m=0;Ze(a,b.text,h.mode,!0,function(a,b){for(var c=n;m<a;){var p=f[n];p>a&&f.splice(n,
1,a,f[n+1],p);n+=2;m=Math.min(a,p)}if(b)if(h.opaque)f.splice(c,n-c,a,"cm-overlay "+b),n=c+2;else for(;c<n;c+=2)p=f[c+1],f[c+1]=(p?p+" ":"")+"cm-overlay "+b},g)}return{styles:f,classes:g.bgClass||g.textClass?g:null}}function $e(a,b,c){if(!b.styles||b.styles[0]!=a.state.modeGen){var d=je(a,b,b.stateAfter=Zb(a,P(b)));b.styles=d.styles;d.classes?b.styleClasses=d.classes:b.styleClasses&&(b.styleClasses=null);c===a.doc.frontier&&a.doc.frontier++}return b.styles}function nd(a,b,c,d){var f=a.doc.mode,g=new Yc(b,
