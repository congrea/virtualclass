...
[0,""])[1]).replace("undefined","3_2").replace("_",".").replace("_",""))||!1},isAndroid:function(){return-1<navigator.userAgent.toLowerCase().indexOf("android")},check:function(){var a=this.isiOSDevice();this.isAndroid();if(a)var c="safariMobile",f=a;else{var g=this.isAndroid(),f=this.mybrowser.detection(),c=f[0],f=parseFloat(f[1]);this.mybrowser.name=c;this.mybrowser.version=f}this.checkBrowserFunctions(c,f);"undefined"!=typeof g&&g||a?"t"==vApp.gObj.uRole?(vApp.vutil.initDisableVirtualClass(),vApp.error.push("We support only desktop computer not  any tablet and mobile for teacher.")):
g&&("Chrome"==c?40<=f?vApp.vutil.initDisableAudVid():vApp.error.push(c+" "+f+"  is not supported, we support chrome  versoin 40 or newer on Android"):vApp.error.push(c+"  is not supported, we support Chrome 40 or newer on andorid")):"Chrome"==c&&40<=f||"Firefox"==c&&35<=f||"s"==vApp.gObj.uRole&&c=="Opera">f>=26?this.reportBrowser(vApp.gObj.uRole):"Chrome"==c&&40>f||"Firefox"==c&&35>f||"s"==vApp.gObj.uRole&&"Opera"==c&&26>f?(this.reportBrowser(vApp.gObj.uRole),vApp.error.push(c+" "+f+vApp.lang.getString("chFireBrowsersIssue"))):
"t"==vApp.gObj.uRole&&"Opera"==c&&26<=f?(this.reportBrowser(vApp.gObj.uRole),vApp.error.push(c+" "+f+vApp.lang.getString("operaBrowserIssue"))):"s"==vApp.gObj.uRole&&"Safari"==c&&8<=f?(vApp.vutil.initDisableAudVid(),this.reportBrowser(vApp.gObj.uRole),vApp.error.push(c+" "+vApp.lang.getString("studentSafariBrowserIssue")),vApp.user.control.audioWidgetDisable()):"s"==vApp.gObj.uRole&&"safariMobile"==c&&7<=f?vApp.vutil.initDisableAudVid():vApp.error.push(c+" "+f+" "+vApp.lang.getString("commonBrowserIssue"))}},
...
if(null!=b)for(var b=b.getElementsByTagName("video"),c=0;c<b.length;c++)b[c].play()},1040)},existVideoContainer:function(a){for(var b=document.getElementsByClassName("userVideos"),c=0;c<b.length;c++)if(b[c].id=="video"+a.id)return!0;return!1},handleUserMediaError:function(a){alert("suman bogati");debugger;a="object"==typeof a?vApp.lang.getString(a.name):vApp.lang.getString(a);vApp.wb.view.createErrorMsg(a,"errorContainer","chatWidget");vApp.user.control.audioWidgetDisable();vApp.wb.view.disappearBox("WebRtc");
console.log("navigator.getUserMedia error: ",a)}}}})(window);(function(d){var a={requestPackets:function(a){1<a.length&&(vApp.wb.gObj.myArr=a);vApp.wb.sentReq=!0;return[vApp.wb.gObj.rcvdPackId,a[0].uid]},makeQueue:function(a){vApp.wb.gObj.rcvdPackId!=vApp.wb.gObj.displayedObjId&&(vApp.wb.gObj.packQueue=vApp.wb.gObj.packQueue.concat(a.message.repObj))},sendPackets:function(a,c){if(0==a.message.getMsPckt[0])var f=-1;else for(f=0;f<vApp.wb.gObj.myrepObj.length&&a.message.getMsPckt[0]!=vApp.wb.gObj.myrepObj[f].uid;f++);for(f+=1;f<a.message.getMsPckt[1];f++)c.push(vApp.wb.gObj.myrepObj[f]);
