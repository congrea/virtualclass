...
this.video);this.video=document.getElementById(this.local+"Video");this.video.autoplay=!0;virtualclass.vutil.createLocalTempVideo("virtualclassScreenShare",this.local+"Temp");virtualclass.vutil.initLocCanvasCont(this.local+"TempVideo")}this.currentStream=b;var g=this;virtualclass.adpt.attachMediaStream(this.video,b);this.prevStream=!0;this.currentStream.onended=function(a){g.ssByClick?(g.video.src="",g.localtempCont.clearRect(0,0,g.localtempCanvas.width,g.localtempCanvas.height),clearInterval(virtualclass.clear),
g.initPrevImage(),virtualclass.vutil.beforeSend({unshareScreen:!0,st:g.type,cf:"unshareScreen"}),g.prevStream=!1,g.prevScreen="",virtualclass.prevScreen=""):g.ssByClick=!0};b={};b.width=c.innerWidth;b.height=c.innerHeight-140;var d=document.getElementById(this.local),h=this.html.getDimension(b);d.style.width=Math.round(h.width)+"px";d.style.height=Math.round(h.height)+"px";g=this;this.video.onloadedmetadata=function(){g.width=h.width;g.height=h.height;g.localtempCanvas.width=g.video.offsetWidth;g.localtempCanvas.height=
g.video.offsetHeight;virtualclass.prevScreen=g;var a=virtualclass.system.measureResoultion({width:c.innerWidth,height:c.innerHeight});g.sharing();virtualclass.vutil.setContainerWidth(a,virtualclass.currApp);roles.hasControls()&&(virtualclass.vutil.makeActiveApp(g.id,virtualclass.previrtualclass),"virtualclassYts"==virtualclass.previrtualclass&&virtualclass.yts.destroyYT());virtualclass.previrtualclass=g.id}},sharing:function(){function b(a,d){for(var c=a.toString(),f=c.length;f<d;f++)c="0"+c;return c.match(/[\S]{1,2}/g)||
[]}function g(){clearInterval(virtualclass.clear);if("undefined"!=typeof d&&"undefined"!=typeof f){if(d!=s.video.offsetWidth||f!=s.video.offsetHeight)a=1}else d=s.video.offsetWidth,f=s.video.offsetHeight;1==a?setTimeout(m,2E3):t(s.type)}function h(a){switch(!0){case 300>=a||"undefined"==typeof a:u=300;break;case 3E3>=a:u=a;break;case 5E3>=a:u=3E3;break;default:u=5E3}}function m(){console.log("RESIZE");d=s.video.offsetWidth;f=s.video.offsetHeight;w=Math.round(s.localtempCanvas.height/12);r=Math.round(s.localtempCanvas.width/
12);var t=s.type;s.localtempCanvas.width=s.video.offsetWidth;s.localtempCanvas.height=s.video.offsetHeight;s.localtempCont.drawImage(s.video,0,0,s.video.offsetWidth,s.video.offsetHeight);var v=s.localtempCont.getImageData(0,0,s.localtempCanvas.width,s.localtempCanvas.height),v=s.dc.encodeRGB(v.data),J=Math.round(s.localtempCanvas.width/r),K=Math.round(s.localtempCanvas.height/w),E,M,W,U;U=s.getContainerDimension();"undefined"!=typeof d&&"undefined"!=typeof f?(E=b(d,4),M=b(f,4)):(E=b(s.video.offsetWidth,
4),M=b(s.video.offsetHeight,4));W=b(U.width,4);U=b(U.height,4);t=new Uint8ClampedArray(["ss"==t?104:204,E[0],E[1],M[0],M[1],W[0],W[1],U[0],U[1]]);E=new Uint8ClampedArray(v.length+t.length);E.set(t);E.set(v,t.length);c.Worker&&sworker.postMessage({img:v,resize:1,resA:w,resB:r,dw:J,dh:K,offsetWidth:s.video.offsetWidth,offsetHeight:s.video.offsetHeight},[v.buffer]);ioAdapter.sendBinary(E);h(E.length/128);a=0;clearInterval(virtualclass.clear);virtualclass.clear=setInterval(g,u)}function t(a){s.localtempCanvas.width=
s.video.offsetWidth;s.localtempCanvas.height=s.video.offsetHeight;s.localtempCont.drawImage(s.video,0,0,s.video.offsetWidth,s.video.offsetHeight);a=Math.round(s.localtempCanvas.width/r);var b=Math.round(s.localtempCanvas.height/w),d=s.localtempCont.getImageData(0,0,s.video.offsetWidth,s.video.offsetHeight);c.Worker&&(sworker.postMessage({img:d.data,resA:w,resB:r,dw:a,dh:b,offsetWidth:s.video.offsetWidth,offsetHeight:s.video.offsetHeight,type:s.type},[d.data.buffer]),sworker.onmessage=function(a){if(1==
a.data.needFullScreen){a=virtualclass.getDataFullScreen(s.type);ioAdapter.sendBinary(a);var b=a.length/128}else null!=a.data.masterSlice&&(ioAdapter.sendBinary(a.data.masterSlice),b=a.data.masterSlice.length/128);h(b);clearInterval(virtualclass.clear);virtualclass.clear=setInterval(g,u)});clearInterval(virtualclass.clear)}var w=Math.round(this.localtempCanvas.height/12),r=Math.round(this.localtempCanvas.width/12),s=this;virtualclass.hasOwnProperty("wholeImage")&&clearInterval(virtualclass.wholeImage);
virtualclass.hasOwnProperty("clear")&&clearInterval(virtualclass.clear);var u=1E3;virtualclass.getDataFullScreen=function(a){s.localtempCanvas.width=s.video.offsetWidth;s.localtempCanvas.height=s.video.offsetHeight;s.localtempCont.drawImage(s.video,0,0,s.video.offsetWidth,s.video.offsetHeight);var d=s.localtempCont.getImageData(0,0,s.localtempCanvas.width,s.localtempCanvas.height),d=s.dc.encodeRGB(d.data),c=b(s.localtempCanvas.height,4),f=b(s.localtempCanvas.width,4);a=new Uint8ClampedArray(["ss"==
a?102:202,f[0],f[1],c[0],c[1]]);c=new Uint8ClampedArray(d.length+a.length);c.set(a);c.set(d,a.length);return c};clearInterval(virtualclass.clear);virtualclass.clear=setInterval(g,u)},getContainerDimension:function(){var a=document.getElementById(this.id+"Local");return{width:a.offsetWidth,height:a.offsetHeight}},drawImages:function(a,b){if("undefined"!=typeof b){var d=this.dc.decodeRGBSlice(a,this.localCont,b);this.localCont.putImageData(d,b.x,b.y)}else d=this.dc.decodeRGB(a,this.localCont,this.localCanvas),
