...
studentSafariBrowserIssue:"Your browser {vApp1} does not able to share your Cam with other users, We fully support Chrome  and Firefox.",ieBrowserIssue:"Your browser Internet Explorer is not supported, We fully support Chrome  and Firefox.",ios7support:"We support only iOS 7 (Apple operating system 7) or higher.",supportDesktop:"For teacher we support desktop computer only not any tablet and mobile phone.",notSupportIphone:"We support iPad, Tablet or Desktop coputer, not any Mobile phone",InternalError:"It seems that you try to connect a single web cam from two different browsers on a single computer.<br /> This is not compaitible with different Operating System eg:- Windows, CentOS. ",
SourceUnavailableError:"It seems that you try to connect a single web cam from two different browsers on a single computer.<br /> This is not supported for operating system like : Windows, CentOS."}})(window);(function(d){d.getString=function(a,b){var c=d.vApp.lang.message[a];if("undefined"!=typeof b&&0<b.length)for(var f=0;f<b.length;f++)c=c.replace(new RegExp("{vapp"+(f+1)+"}"),b[f]);return c}})(window);(function(d){var a={init:function(){this.msgBoxClass="msgBox";this.window={};this.virtualWindow={};return this},displayMessage:function(a,c,f,g,d){a="undefined"==typeof d?this.createMsgBox(a,c,f):this.createMsgBox(a,c,f,d);c=document.getElementById("vcanvas");"undefined"!=typeof g?document.getElementById(g).appendChild(a):c.insertBefore(a,c.childNodes[0])},createErrorContainerToBeDeleted:function(){var a=document.getElementById("errorContainer");if(null==a){a=document.createElement("div");a.id="errorContainer";
...
"Chrome"==c&&40>a||"Firefox"==c&&35>a||"s"==vApp.gObj.uRole&&"OPR"==c&&26>a?(this.reportBrowser(vApp.gObj.uRole),vApp.error.push(vApp.lang.getString("chFireBrowsersIssue",[c,a]))):"t"==vApp.gObj.uRole&&"OPR"==c&&26<=a?(this.reportBrowser(vApp.gObj.uRole),vApp.error.push(vApp.lang.getString("operaBrowserIssue",[c,a]))):"s"==vApp.gObj.uRole&&"Safari"==c&&8<=a?(vApp.vutil.initDisableAudVid(),this.reportBrowser(vApp.gObj.uRole),vApp.error.push(vApp.lang.getString("studentSafariBrowserIssue",[c,a])),vApp.user.control.audioWidgetDisable()):
"iOS"==c?/(iPad)/g.test(navigator.userAgent)?"s"==vApp.gObj.uRole?7<=a?vApp.vutil.initDisableAudVid():(vApp.vutil.initDisableVirtualClass(),vApp.error.push(vApp.lang.getString("ios7support"))):(vApp.vutil.initDisableVirtualClass(),vApp.error.push(vApp.lang.getString("supportDesktop"))):(vApp.vutil.initDisableVirtualClass(),vApp.error.push(vApp.lang.getString("notSupportIphone"))):(this.mybrowser.detectIE()?(vApp.gObj.errIE=!0,vApp.error.push(vApp.lang.getString("ieBrowserIssue"))):vApp.error.push(vApp.lang.getString("commonBrowserIssue",
[c,a])),vApp.vutil.initDisableVirtualClass())}},a=a.init();d.addEventListener("resize",function(){d.earlierWidth!=d.innerWidth&&a.setAppDimension()});d.addEventListener("resize",function(){d.earlierWidth!=d.innerWidth&&vApp.wb.view.window.resize()});a.mybrowser.detectIE=function(){var a=d.navigator.userAgent,c=a.indexOf("MSIE ");if(0<c)return parseInt(a.substring(c+5,a.indexOf(".",c)),10);if(0<a.indexOf("Trident/"))return c=a.indexOf("rv:"),parseInt(a.substring(c+3,a.indexOf(".",c)),10);c=a.indexOf("Edge/");
return 0<c?parseInt(a.substring(c+5,a.indexOf(".",c)),10):!1};a.mybrowser.detection=function(){var a=navigator.userAgent,c,f=a.match(/(opera|opr|OPR(?=\/))\/?\s*([\d\.]+)/i)||[];0>=f.length&&(f=a.match(/(chrome|safari|firefox|trident(?=\/))\/?\s*([\d\.]+)/i)||[]);if(/trident/i.test(f[1]))return c=/\brv[ :]+(\d+(\.\d+)?)/g.exec(a)||[],"IE "+(c[1]||"");f=f[2]?[f[1],f[2]]:[navigator.appName,navigator.appVersion,"-?"];null!=(c=a.match(/version\/([\.\d]+)/i))&&(f[2]=c[1]);return f};d.system=a})(window);(function(d){d.packContainer=function(){return{createPacketContDiv:function(a,b){var c=document.createElement("div");"undefined"!=typeof a&&(c.id=a);"undefined"!=typeof b&&(c.clasName=b);return c},togglePacketCont:function(a){for(var b=this.className.split(" "),c=0;c<b.length;c++)if("display"==b[c]){this.className="hide";this.innerHTML="+";a._togglePacketCont(b[c]);break}else if("hide"==b[c]){this.className="display";this.innerHTML="-";a._togglePacketCont(b[c]);break}},_togglePacketCont:function(a){a=
...
c.replaceChild(b,f)}},audio:{audioStreamArr:[],tempAudioStreamArr:[],recordingLength:0,bufferSize:0,encMode:"alaw",recordAudio:!1,resampler:new Resampler(44100,8E3,1,4096),rec:"",otherSound:!1,audioNodes:[],sdElem:"silenceDetect",Html5Audio:{audioContext:new AudioContext},init:function(){null!=localStorage.getItem("orginalTeacherId")&&(vApp.gObj.audMouseDown=!0);this.graph={height:56,width:4,average:0,display:function(){var a=cthis.video;"t"==vApp.gObj.uRole&&this._display(a.tempVidCont,this.height-
...
b)},play:function(a,b,c){var f=JSON.parse(localStorage.getItem("vApp"+b)),d=this.Html5Audio.audioContext.createBuffer(1,a.length,8E3);d.getChannelData(0).set(a);a=this.Html5Audio.audioContext.createBufferSource();a.buffer=d;a.buffer=d;d=this.Html5Audio.audioContext.createGain();d.gain.value=.9;a.connect(d);d.connect(this.Html5Audio.audioContext.destination);null!=f&&f.ad&&f.aud&&(vApp.user.control.iconAttrManupulate(b,"icon-audioEnaGreen"),document.getElementById(f.id+"contrAudAnch").setAttribute("data-title",
vApp.lang.getString("audioOn")));a.onended=function(){f=JSON.parse(localStorage.getItem("vApp"+b));"undefined"==typeof c&&(clearTimeout(vApp.gObj[b].out),vApp.gObj[b].isplaying=!1,0<vApp.gObj.video.audio.audioToBePlay[b].length?vApp.gObj.video.audio.getChunks(b):f.ad&&f.aud&&vApp.user.control.iconAttrManupulate(b,"icon-audioEnaOrange"))};a.start();vApp.gObj[b].isplaying=!0;"undefined"==typeof c&&(vApp.gObj[b].out=setTimeout(function(){vApp.gObj[b].isplaying=!1;0<vApp.gObj.video.audio.audioToBePlay[b].length&&
vApp.gObj.video.audio.getChunks(b)},1E3*a.buffer.duration+10))},calcAverage:function(){var a=new Uint8Array(analyser.frequencyBinCount);analyser.getByteFrequencyData(a);for(var b=0,c=a.length,f=0;f<c;f++)b+=a[f];this.graph.average=b/c},audioInGraph:function(){var a=cthis.video;if("t"==vApp.gObj.uRole){var b=this.graph.height-this.graph.height*this.graph.average/100;a.tempVidCont.beginPath();a.tempVidCont.moveTo(a.tempVid.width-this.graph.width,this.graph.height);a.tempVidCont.lineTo(a.tempVid.width-
...
null!=this.videoCont&&(this.videoCont.style.maxHeight=this.maxHeight+"px")},calcDimension:function(){this.myVideo.width=this.width;this.myVideo.height=this.height},removeUser:function(a){a=document.getElementById("user"+a);null!=a&&a.parentNode.removeChild(a)},createElement:function(a){var b=document.createElement("div");b.className="videoWrapper";var c=document.createElement("div");c.className="videoSubWrapper";c.id="user"+a.id;b.appendChild(c);vidId=a.id;var f=document.createElement("canvas");f.id=
"video"+vidId;f.className="userVideos";f.width=this.width;f.height=this.height;c.appendChild(f);vApp.gObj.video.util.imageReplaceWithVideo(a.id,b)},updateHightInSideBar:function(a){var b=document.getElementById("widgetRightSide").offsetHeight;document.getElementById("chatContainer").style.height=b-a+"px"},send:function(){function b(){function h(){try{f.tempVidCont.drawImage(f.myVideo,0,0,f.width,f.height)}catch(a){if("NS_ERROR_NOT_AVAILABLE"==a.name)setTimeout(h,100);else throw a;}}if("t"==vApp.gObj.uRole){if("undefined"==
typeof k){var k=document.getElementById("graphCanvas");null!=k&&(cthis.audio.graph.cvCont=k.getContext("2d"))}null!=k&&cthis.audio.graph.cvCont.clearRect(0,0,k.width,k.height)}f.tempVidCont.clearRect(0,0,f.tempVid.width,f.tempVid.height);d.navigator.userAgent.match("Firefox")?h():f.tempVidCont.drawImage(f.myVideo,0,0,f.width,f.height);"t"==vApp.gObj.uRole&&cthis.audio.graph.display();k={name:vApp.gObj.uName,id:vApp.gObj.uid};"t"==vApp.gObj.uRole&&(k.role=vApp.gObj.uRole);vApp.wb.utility.beforeSend({videoByImage:k});
var k=f.tempVidCont.getImageData(0,0,f.tempVid.width,f.tempVid.height),k=vApp.dirtyCorner.encodeRGB(k.data),s=c(vApp.gObj.uid,8),s=new Uint8ClampedArray([11,s[0],s[1],s[2],s[3]]),v=new Uint8ClampedArray(k.length+s.length);v.set(s);v.set(k,s.length);a.sendBinary(v);clearInterval(vApp.gObj.video.smallVid);k=2E3+2500*vApp.gObj.totalUser.length;g!=vApp.gObj.totalUser?(g=vApp.gObj.totalUser.length,s=vApp.gObj.totalUser.indexOf(vApp.gObj.uId),vApp.gObj.video.smallVid=setInterval(b,k+k/g*s)):vApp.gObj.video.smallVid=
setInterval(b,k)}function c(a,b){for(var f=a.toString(),d=f.length;d<b;d++)f="0"+f;return f.match(/[\S]{1,2}/g)||[]}vApp.gObj.video.hasOwnProperty("smallVid")&&clearInterval(vApp.gObj.video.smallVid);var f=this;randomTime=Math.floor(10001*Math.random())+5E3;var g=-1;vApp.gObj.video.smallVid=setInterval(b,300)},startToStream:function(){cthis.video.calcDimension();cthis.video.send()},playWithoutSlice:function(a,b){this.remoteVid=document.getElementById("video"+a);this.remoteVidCont=this.remoteVid.getContext("2d");
...
a.style.opacity=.6;a.style.pointerEvents="none"},enableVirtualClass:function(){var a=document.getElementById("vAppCont");a.style.opacity=1;a.style.pointerEvents="visible"}};d.onbeforeunload=function(){if("undefined"!=typeof vApp.storage.wholeStoreData){var a=JSON.parse(vApp.storage.wholeStoreData);a.beforeRefresh=!0;vApp.storage.wholeStore(a,"put")}localStorage.removeItem("otherRole");vApp.wb.utility.userIds=[];vApp.gObj.hasOwnProperty("audIntDisable")||vApp.gObj.video.audio.studentNotSpeak();vApp.vutil.clickOutSideCanvas();
