...
studentSafariBrowserIssue:"Your browser {vapp1} {vapp2} does not able to share your Cam with other users, We fully support Chrome  and Firefox.",ieBrowserIssue:"Your browser Internet Explorer is not supported, We fully support Chrome  and Firefox.",ios7support:"We support only iOS 7 (Apple operating system 7) or higher.",supportDesktop:"For teacher we support desktop computer only not any tablet and mobile phone.",notSupportIphone:"We support iPad, Tablet or Desktop coputer, not any Mobile phone",
...
vApp.error.push(vApp.lang.getString("studentSafariBrowserIssue",[c,a])),vApp.user.control.audioWidgetDisable()):(vApp.vutil.initDisableVirtualClass(),vApp.error.push(vApp.lang.getString("safariBrowserIssue",[c,a]))):"iOS"==c?/(iPad)/g.test(navigator.userAgent)?"s"==vApp.gObj.uRole?7<=a?vApp.vutil.initDisableAudVid():(vApp.vutil.initDisableVirtualClass(),vApp.error.push(vApp.lang.getString("ios7support"))):(vApp.vutil.initDisableVirtualClass(),vApp.error.push(vApp.lang.getString("supportDesktop"))):
(vApp.vutil.initDisableVirtualClass(),vApp.error.push(vApp.lang.getString("notSupportIphone"))):(this.mybrowser.detectIE()?(vApp.gObj.errIE=!0,vApp.error.push(vApp.lang.getString("ieBrowserIssue"))):vApp.error.push(vApp.lang.getString("commonBrowserIssue",[c,a])),vApp.vutil.initDisableVirtualClass())}},a=a.init();d.addEventListener("resize",function(){d.earlierWidth!=d.innerWidth&&a.setAppDimension()});d.addEventListener("resize",function(){d.earlierWidth!=d.innerWidth&&vApp.wb.view.window.resize()});
