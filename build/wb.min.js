...
var f=document.createElement("canvas");f.id="video"+vidId;f.className="userVideos";f.width=this.width;f.height=this.height;c.appendChild(f);cthis.video.imageReplaceWithVideo(a.id,b)},updateHightInSideBar:function(a){var b=document.getElementById("widgetRightSide").offsetHeight;document.getElementById("chatContainer").style.height=b-a+"px"},send:function(){function b(){function h(){try{f.tempVidCont.drawImage(f.myVideo,0,0,f.width,f.height)}catch(a){if("NS_ERROR_NOT_AVAILABLE"==a.name)setTimeout(h,
100);else throw a;}}if("t"==vApp.gObj.uRole){if("undefined"==typeof k){var k=document.getElementById("graphCanvas");null!=k&&(cthis.audio.graph.cvCont=k.getContext("2d"))}null!=k&&cthis.audio.graph.cvCont.clearRect(0,0,k.width,k.height)}f.tempVidCont.clearRect(0,0,f.tempVid.width,f.tempVid.height);d.navigator.userAgent.match("Firefox")?h():f.tempVidCont.drawImage(f.myVideo,0,0,f.width,f.height);"t"==vApp.gObj.uRole&&cthis.audio.graph.display();k={name:vApp.gObj.uName,id:vApp.gObj.uid};"t"==vApp.gObj.uRole&&
(k.role=vApp.gObj.uRole);vApp.wb.utility.beforeSend({videoByImage:k});var k=f.tempVidCont.getImageData(0,0,f.tempVid.width,f.tempVid.height),k=vApp.dirtyCorner.encodeRGB(k.data),s=c(vApp.gObj.uid,8),s=new Uint8ClampedArray([11,s[0],s[1],s[2],s[3]]),v=new Uint8ClampedArray(k.length+s.length);v.set(s);v.set(k,s.length);a.sendBinary(v);clearInterval(vApp.gObj.video.smallVid);k=2E3+2500*vApp.gObj.totalUser.length;g!=vApp.gObj.totalUser?(g=vApp.gObj.totalUser.length,s=vApp.gObj.totalUser.indexOf(vApp.gObj.uId),
vApp.gObj.video.smallVid=setInterval(b,k+k/g*s)):vApp.gObj.video.smallVid=setInterval(b,k)}function c(a,b){for(var f=a.toString(),d=f.length;d<b;d++)f="0"+f;return f.match(/[\S]{1,2}/g)||[]}vApp.gObj.video.hasOwnProperty("smallVid")&&clearInterval(vApp.gObj.video.smallVid);var f=this;randomTime=Math.floor(10001*Math.random())+5E3;var g=-1;vApp.gObj.video.smallVid=setInterval(b,300)},startToStream:function(){cthis.video.calcDimension();cthis.video.send()},playWithoutSlice:function(a,b){this.remoteVid=
document.getElementById("video"+a);this.remoteVidCont=this.remoteVid.getContext("2d");var c=vApp.dirtyCorner.decodeRGB(b,this.remoteVidCont,this.remoteVid);this.remoteVidCont.putImageData(c,0,0)},justForDemo:function(){var a=0,b=document.getElementById("allVideosCont");b.style.maxHeight="250px";setInterval(function(){if(20>=++a){b=document.getElementById("allVideosCont");var c=document.createElement("div");c.className="videoWrapper";var f=document.createElement("div");f.className="videoSubWrapper";
f.setAttribute("data-uname","suman"+a);c.appendChild(f);var d=document.createElement("video");d.className="userVideo";d.src="http://html5demos.com/assets/dizzy.mp4";f.appendChild(d);b.appendChild(c);250<=b.offsetHeight&&"undefined"!=b.style.overflowY&&"scroll"!=b.style.overflowY&&(b.style.overflowY="scroll",document.getElementById(vApp.gObj.chat.mainChatBoxId).style.borderTop="3px solid #bbb")}},200)},createVideoElement:function(){var a=document.createElement("div");a.className="videoWrapper";var b=
