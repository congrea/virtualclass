...
typeof myfunc)this.objs=d.getStates("replayObjs");else{var c=this;if("fromplay"==a)virtualclass.storage.getAllObjs(["allData"],function(){c.play()});else{for(var d=this.clearPopups("prvNum")?this.prvNum:0,f=0;f<a.length;f++)this.items.push(a[f]);for(k=0;k<a.length;k++){if(a[k].hasOwnProperty("bd"))for("a"==a[k].bd?b=virtualclass.dtCon.base64DecToArrInt(a[k].recObjs):"c"==a[k].bd&&(b=virtualclass.dtCon.base64DecToArrclm(a[k].recObjs)),this.items[d].recObjs=b,f=0;f<b.length;f++)this.items[d].recObjs[f]=
...
b?virtualclass.recorder.alreadyDownload||virtualclass.recorder.makeAvailDownloadFile():alert(b)})}},virtualclass.recorder.rnum)}},afterRecording:function(){virtualclass.clearSession();document.getElementById("progressContainer").style.display="none";var a=document.getElementById("recordFinishedMessageBox");a.style.display="block";a.classList.add("MessageBoxFinished");var b=document.getElementById("recordingContainer");b.classList.add("recordingFinished");var d=document.getElementById("recordingClose");
virtualclass.recorder.alreadyDownload=!0;d.addEventListener("click",function(){virtualclass.popup.closeElem();var d=document.getElementById("progressContainer");a.style.display="none";d.style.display="block";virtualclass.pbar.renderProgressBar(0,0,"progressBar","progressValue");virtualclass.pbar.renderProgressBar(0,0,"indProgressBar","indProgressValue");b.classList.remove("recordingFinished")})},makeAvailDownloadFile:function(){console.log("DOWNLLOAD MESSAGE");var b=document.getElementById("recordingContainer"),
d=document.createElement("div");d.id="downloadFileCont";var f=document.createElement("div");f.id="errormsessage";f.innerHTML=virtualclass.lang.getString("msgForDownloadStart");document.getElementById("progressContainer").style.display="none";d.appendChild(f);b.appendChild(d);this.alreadyDownload=!0;virtualclass.storage.getAllDataForDownload(["chunkData"],function(b){virtualclass.gObj.saveSession=!0;var g=document.createElement("button");g.id="downloadButton";g.innerHTML=virtualclass.lang.getString("downloadFile");
var m=document.createElement("a");m.id="dlink";m.href="";m.download="session.vcp";m.innerHTML=virtualclass.lang.getString("download");f.innerHTML=virtualclass.lang.getString("filenotsave");d.appendChild(g);document.getElementById("recordingHeader").innerHTML=virtualclass.lang.getString("downloadFile");var h=new Blob([b],{type:"application/virtualclass"});g.addEventListener("click",function(){null!=c.webkitURL?m.href=c.webkitURL.createObjectURL(h):(m.href=c.URL.createObjectURL(h),m.onclick=a,document.body.appendChild(m));
