...
"AudEnableSign"),g.parentNode.removeChild(g)},audioSign:function(b,d){if("create"==d)for(var g=a.getElementById(b.id+"contrAudImg"),k=0;k<g.classList.length;k++){if("icon-"==g.classList[k].substring(0,5)){g.classList.remove(g.classList[k]);g.classList.add("icon-audioEnaGreen");break}}else alert("removed audio sign")},iconAttrManupulate:function(b){b=a.getElementById(b+"contrAudImg");for(var d=0;d<b.classList.length;d++)if("icon-"==b.classList[d].substring(0,5)){b.classList.remove(b.classList[d]);
b.classList.add("icon-audioEnaGreen");break}},shouldApply:function(a){var b=localStorage.getItem("vApp"+a);null!=b&&(b=JSON.parse(b),console.log("uid "+a+" "+b.ad),b.ad&&vApp.user.control.audioSign({id:a},"create"))},changeAttrToAssign:function(b){for(var d=a.getElementsByClassName("assignImg"),g=0;g<d.length;g++)"enable"==b?(d[g].classList.remove("block"),d[g].classList.add("enable"),d[g].parentNode.classList.add("tooltip"),d[g].parentNode.setAttribute("data-title",vApp.lang.getString("assignDisable")),
