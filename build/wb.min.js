...
d.buffer.duration);vApp.gObj[b].isplaying=!0;"undefined"==typeof c&&(vApp.gObj[b].out=setTimeout(function(){console.log("Stack length "+vApp.gObj.video.audio.audioToBePlay[b].length+"; UID "+b+" video ended OUT :"+d.buffer.duration);vApp.gObj[b].isplaying=!1;0<vApp.gObj.video.audio.audioToBePlay[b].length&&vApp.gObj.video.audio.getChunks(b)},1E3*d.buffer.duration+10))},calcAverage:function(){var a=new Uint8Array(analyser.frequencyBinCount);analyser.getByteFrequencyData(a);for(var b=0,c=a.length,f=
...
a.length)]}},video:{width:75,height:56,tempVid:"",tempVidCont:"",myVideo:"",remoteVid:"",remoteVidCont:"",maxHeight:250,init:function(){this.videoCont=document.getElementById("allVideosCont");null!=this.videoCont&&(this.videoCont.style.maxHeight=this.maxHeight+"px")},calcDimension:function(){this.myVideo.width=this.width;this.myVideo.height=this.height},removeUser:function(a){a=document.getElementById("user"+a);null!=a&&a.parentNode.removeChild(a)},createElement:function(a){var b=document.createElement("div");
b.className="videoWrapper";var c=document.createElement("div");c.className="videoSubWrapper";c.id="user"+a.id;b.appendChild(c);vidId=a.id;var f=document.createElement("canvas");f.id="video"+vidId;f.className="userVideos";f.width=this.width;f.height=this.height;c.appendChild(f);cthis.video.imageReplaceWithVideo(a.id,b)},updateHightInSideBar:function(a){var b=document.getElementById("widgetRightSide").offsetHeight;document.getElementById("chatContainer").style.height=b-a+"px"},send:function(){function b(){function k(){try{f.tempVidCont.drawImage(f.myVideo,
0,0,f.width,f.height)}catch(a){if("NS_ERROR_NOT_AVAILABLE"==a.name)setTimeout(k,100);else throw a;}}if("t"==vApp.gObj.uRole){if("undefined"==typeof h){var h=document.getElementById("graphCanvas");null!=h&&(cthis.audio.graph.cvCont=h.getContext("2d"))}null!=h&&cthis.audio.graph.cvCont.clearRect(0,0,h.width,h.height)}f.tempVidCont.clearRect(0,0,f.tempVid.width,f.tempVid.height);d.navigator.userAgent.match("Firefox")?k():f.tempVidCont.drawImage(f.myVideo,0,0,f.width,f.height);"t"==vApp.gObj.uRole&&cthis.audio.graph.display();
h={name:vApp.gObj.uName,id:vApp.gObj.uid};"t"==vApp.gObj.uRole&&(h.role=vApp.gObj.uRole);vApp.wb.utility.beforeSend({videoByImage:h});var h=f.tempVidCont.getImageData(0,0,f.tempVid.width,f.tempVid.height),h=vApp.dirtyCorner.encodeRGB(h.data),u=c(vApp.gObj.uid,8),u=new Uint8ClampedArray([11,u[0],u[1],u[2],u[3]]),v=new Uint8ClampedArray(h.length+u.length);v.set(u);v.set(h,u.length);a.sendBinary(v);clearInterval(vApp.gObj.video.smallVid);h=2E3+2500*vApp.gObj.totalUser.length;g!=vApp.gObj.totalUser?(g=
vApp.gObj.totalUser.length,u=vApp.gObj.totalUser.indexOf(vApp.gObj.uId),vApp.gObj.video.smallVid=setInterval(b,h+h/g*u)):vApp.gObj.video.smallVid=setInterval(b,h)}function c(a,b){for(var f=a.toString(),d=f.length;d<b;d++)f="0"+f;return f.match(/[\S]{1,2}/g)||[]}vApp.gObj.video.hasOwnProperty("smallVid")&&clearInterval(vApp.gObj.video.smallVid);var f=this;randomTime=Math.floor(10001*Math.random())+5E3;var g=-1;vApp.gObj.video.smallVid=setInterval(b,300)},startToStream:function(){cthis.video.calcDimension();
