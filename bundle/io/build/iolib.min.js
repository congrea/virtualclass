...
5E3)}},userauthenticat:function(){var a=JSON.stringify({cfun:"authenticate",arg:{authuser:this.cfg.authuser,authpass:this.cfg.authpass}});this.sock.send(a)},addclient:function(){var a=JSON.stringify({cfun:"joinroom",arg:{client:this.cfg.userid,roomname:this.cfg.room,user:this.cfg.userobj}});this.sock.send(a)},send:function(a,c,b){a={cfun:c,arg:{msg:a}};b&&(a.arg.touser=b);b=JSON.stringify(a);if(this.sock&&1==this.sock.readyState)if(0<io.packetQueue.length){io.packetQueue.push(b);for(b=0;b<io.packetQueue.length;b++)this.sock.send(io.packetQueue[b]);
io.packetQueue.length=0}else this.sock.send(b);else console.log("SOCKET is not ready."),io.packetQueue.push(b)},sendBinary:function(a){this.sock&&1==this.sock.readyState&&this.sock.send(a.buffer)},onRecMessage:function(a){if(a.data instanceof ArrayBuffer)this.onRecBinary(a);else{var c=JSON.parse(a.data);this.onRecSave(c,a.data);io.onRecJson(c,a.data)}},onRecSave:function(a,c){(!a.hasOwnProperty("userto")||a.hasOwnProperty("userto")&&a.m.hasOwnProperty("eddata"))&&ioStorage.completeStorage(c)},onRecBinary:function(a){a.data instanceof
ArrayBuffer&&($.event.trigger({type:"binrec",message:a.data}),a=new Uint8Array(a.data),a=101==a[0]?new Int8Array(a):new Uint8ClampedArray(a),ioStorage.dataBinaryStore(a))},onRecJson:function(a){var c="";switch(a.type){case "joinroom":console.log("New user join room "+a.users);var b=null;null!=io.uniquesids&&$.each(a.clientids,function(a,c){void 0==io.uniquesids[a]&&(b=a)});io.uniquesids=a.clientids;$.event.trigger({type:"member_added",message:a.users,newuser:b});break;case "broadcastToAll":case "broadcast":null!==
a&&(void 0!=a.userto&&(c=a.userto),$.event.trigger({type:"newmessage",message:a.m,fromUser:a.user,toUser:c}));break;case "userleft":void 0!=a.userto&&(c=a.userto);null!=io.uniquesids&&delete io.uniquesids[a.user.userid];$.event.trigger({type:"user_logout",fromUser:a.user,message:"offline",toUser:c});break;case "leftroom":$.event.trigger({type:"member_removed",message:a.users});break;case "Unauthenticated":$.event.trigger({type:"authentication_failed",message:"Authentication failed"});break;case "Multiple_login":$.event.trigger({type:"Multiple_login"})}},
disconnect:function(){this.sock.onclose=function(){};this.sock.close();console.log("i am closing this connection")}};
