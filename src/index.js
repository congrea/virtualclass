...
window.onload = async function () {
  ...
  const disableEditor = function (editor) {
  ...
  const isTeacherExistWhenRemoveUser = function (users) {
  ...
  const removeSessionEndTool = function () {
  ...
  const getUserId = function (joinId) {
    if (virtualclass.hasOwnProperty('connectedUsers')) {
  ...
  const checkWithTeacher = function (e) {
  ...
  const isPresenterAlreadyExist = function (e) {
   ...
   * @param userId
   */
  const transferControl = function (userId) {
    ...
    setTimeout(
      () => {
        ...
        const elem = document.getElementById(`${userId}contrAssignImg`);
  ...
  const overrideRoleTeacher = function () {
  ...
  function clearEverthing() {
    ...
    const chat_room = document.getElementById('chatrm');
    if (chat_room !== null) {
